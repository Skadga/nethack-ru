...
#define PCHAR2(idx, ch, sym, tilenm, desc, clr) PCHAR(idx, ch, sym, desc, clr)
    ...
    PCHAR2( 0, ' ',  S_stone,  "dark part of a room", "камень",  NO_COLOR)
    PCHAR2( 1, '|',  S_vwall,  "vertical wall", "стена",  CLR_GRAY)
    PCHAR2( 2, '-',  S_hwall,  "horizontal wall", "стена",  CLR_GRAY)
    PCHAR2( 3, '-',  S_tlcorn, "top left corner wall", "стена",  CLR_GRAY)
    PCHAR2( 4, '-',  S_trcorn, "top right corner wall", "стена",  CLR_GRAY)
    PCHAR2( 5, '-',  S_blcorn, "bottom left corner wall", "стена", CLR_GRAY)
    PCHAR2( 6, '-',  S_brcorn, "bottom right corner wall", "стена", CLR_GRAY)
    PCHAR2( 7, '-',  S_crwall, "cross wall", "стена", CLR_GRAY)
    PCHAR2( 8, '-',  S_tuwall, "tuwall", "стена", CLR_GRAY)
    PCHAR2( 9, '-',  S_tdwall, "tdwall", "стена", CLR_GRAY)
    PCHAR2(10, '|',  S_tlwall, "tlwall", "стена", CLR_GRAY)
    PCHAR2(11, '|',  S_trwall, "trwall", "стена", CLR_GRAY)
    /* start cmap A                                                      */
    PCHAR2(12, '.',  S_ndoor,  "no door", "дверной проЄм", CLR_GRAY)
    PCHAR2(13, '-',  S_vodoor, "vertical open door", "открыта€ дверь", CLR_BROWN)
    PCHAR2(14, '|',  S_hodoor, "horizontal open door", "открыта€ дверь", CLR_BROWN)
    PCHAR2(15, '+',  S_vcdoor, "vertical closed door",
                               "закрыта€ дверь", CLR_BROWN)
    PCHAR2(16, '+',  S_hcdoor, "horizontal closed door",
                               "закрыта€ дверь", CLR_BROWN)
    PCHAR( 17, '#',  S_bars,   "железные пруть€", HI_METAL)
    PCHAR( 18, '#',  S_tree,   "дерево", CLR_GREEN)
    PCHAR( 19, '.',  S_room,   "пол в комнате", CLR_GRAY)
    PCHAR( 20, '.',  S_darkroom, "тЄмна€ часть комнаты", CLR_BLACK)
    PCHAR2(21, '`',  S_engroom, "engraving in a room", "надпись",
                                CLR_BRIGHT_BLUE)
    PCHAR2(22, '#',  S_corr,   "dark corridor", "коридор", CLR_GRAY)
    PCHAR( 23, '#',  S_litcorr, "освещЄнный коридор", CLR_GRAY)
    PCHAR2(24, '#',  S_engrcorr, "engraving in a corridor", "надпись",
                                 CLR_BRIGHT_BLUE)
    PCHAR2(25, '<',  S_upstair, "up stairs", "лестница вверх", CLR_GRAY)
    PCHAR2(26, '>',  S_dnstair, "down stairs", "лестница вниз", CLR_GRAY)
    PCHAR2(27, '<',  S_upladder, "up ladder", "верЄвочна€ лестница вверх", CLR_BROWN)
    PCHAR2(28, '>',  S_dnladder, "down ladder", "верЄвочна€ лестница вниз", CLR_BROWN)
    PCHAR( 29, '<',  S_brupstair, "лестница вверх (ответвление)", CLR_YELLOW)
    PCHAR( 30, '>',  S_brdnstair, "лестница вниз (ответвление)", CLR_YELLOW)
    PCHAR( 31, '<',  S_brupladder, "верЄвочна€ лестница вверх (ответвление)", CLR_YELLOW)
    PCHAR( 32, '>',  S_brdnladder, "верЄвочна€ лестница вниз (ответвление)", CLR_YELLOW)
    /* end cmap A */
    PCHAR( 33, '_',  S_altar,  "алтарь", CLR_GRAY)
    /* start cmap B */
    PCHAR( 34, '|',  S_grave,  "могила", CLR_WHITE)
    PCHAR2(35, '\\', S_throne, "throne", "величественный трон", HI_GOLD)
    PCHAR( 36, '{',  S_sink,   "раковина", CLR_WHITE)
    PCHAR( 37, '{',  S_fountain, "фонтан", CLR_BRIGHT_BLUE)
    /* the S_pool symbol is used for both POOL terrain and MOAT terrain */
    PCHAR2(38, '}',  S_pool,   "pool", "вода", CLR_BLUE)
    PCHAR( 39, '.',  S_ice,    "лЄд", CLR_CYAN)
    PCHAR( 40, '}',  S_lava,   "раскалЄнна€ лава", CLR_RED)
    PCHAR( 41, '}',  S_lavawall,  "стена лавы", CLR_ORANGE)
    PCHAR2(42, '.',  S_vodbridge, "vertical open drawbridge",
                                  "опущенный подъЄмный мост", CLR_BROWN)
    PCHAR2(43, '.',  S_hodbridge, "horizontal open drawbridge",
                                  "опущенный подъЄмный мост", CLR_BROWN)
    PCHAR2(44, '#',  S_vcdbridge, "vertical closed drawbridge",
                                  "подн€тый подъЄмный мост", CLR_BROWN)
    PCHAR2(45, '#',  S_hcdbridge, "horizontal closed drawbridge",
                                  "подн€тый подъЄмный мост", CLR_BROWN)
    PCHAR( 46, ' ',  S_air,    "воздух", CLR_CYAN)
    PCHAR( 47, '#',  S_cloud,  "облако", CLR_GRAY)
    /* the S_water symbol is used for WATER terrain: wall of water in the
       dungeon and Plane of Water in the endgame */
    PCHAR( 48, '}',  S_water,  "вода", CLR_BRIGHT_BLUE)
    /* end dungeon characters                                          */
    /*                                                                 */
    /* begin traps                                                     */
    /*                                                                 */
    PCHAR( 49, '^',  S_arrow_trap, "ловушка со стрелами", HI_METAL)
    PCHAR( 50, '^',  S_dart_trap, "ловушка с дротиками", HI_METAL)
    PCHAR( 51, '^',  S_falling_rock_trap, "ловушка с падающим камнем", CLR_GRAY)
    PCHAR( 52, '^',  S_squeaky_board, "скрипуча€ половица", CLR_BROWN)
    PCHAR( 53, '^',  S_bear_trap, "медвежий капкан", HI_METAL)
    PCHAR( 54, '^',  S_land_mine, "мина", CLR_RED)
    PCHAR( 55, '^',  S_rolling_boulder_trap, "ловушка с кат€щимс€ валуном", CLR_GRAY)
    PCHAR( 56, '^',  S_sleeping_gas_trap, "ловушка с усыпл€ющим газом", HI_ZAP)
    PCHAR( 57, '^',  S_rust_trap, "ловушка ржавчины", CLR_BLUE)
    PCHAR( 58, '^',  S_fire_trap, "огненна€ ловушка", CLR_ORANGE)
    PCHAR( 59, '^',  S_pit, "€ма", CLR_BLACK)
    PCHAR( 60, '^',  S_spiked_pit, "€ма с коль€ми", CLR_BLACK)
    PCHAR( 61, '^',  S_hole, "дыра", CLR_BROWN)
    PCHAR( 62, '^',  S_trap_door, "люк", CLR_BROWN)
    PCHAR( 63, '^',  S_teleportation_trap, "ловушка телепортации", CLR_MAGENTA)
    PCHAR( 64, '^',  S_level_teleporter, "ловушка телепортации уровн€", CLR_MAGENTA)
    PCHAR( 65, '^',  S_magic_portal, "магический портал", CLR_BRIGHT_MAGENTA)
    PCHAR( 66, '"',  S_web, "паутина", CLR_GRAY)
    PCHAR( 67, '^',  S_statue_trap, "ловушка-стату€", CLR_GRAY)
    PCHAR( 68, '^',  S_magic_trap, "магическа€ ловушка", HI_ZAP)
    PCHAR2(69, '^',  S_anti_magic_trap, "anti magic trap", "поле антимагии",
                                        HI_ZAP)
    PCHAR( 70, '^',  S_polymorph_trap, "ловушка полиморфии", CLR_BRIGHT_GREEN)
    PCHAR( 71, '~',  S_vibrating_square, "вибрирующий квадрат", CLR_MAGENTA)
    PCHAR( 72, '^',  S_trapped_door, "дверь-ловушка", CLR_ORANGE)
    PCHAR( 73, '^',  S_trapped_chest, "сундук-ловушка", CLR_ORANGE)
    /* end traps                                                       */
    /* end cmap B */
    /*                                                                   */
    ...
    PCHAR( 86, '#',  S_poisoncloud, "€довитое облако", CLR_BRIGHT_GREEN)
    ...
    PCHAR( 87, '$',  S_goodpos, "подход€ща€ позици€", HI_ZAP)
    /* end cmap C */
    /*                                                             */
...
#define OBJCLASS2(idx, ch, basename, sname, sym, name, explain) \
    ...
    OBJCLASS( 1,  ']', ILLOBJ, S_strange_obj, "illegal objects",
                                              "странный предмет")
    OBJCLASS( 2,  ')', WEAPON, S_weapon, "weapons", "оружие")
    OBJCLASS( 3,  '[', ARMOR,  S_armor, "armor", "часть доспехов")
    OBJCLASS( 4,  '=', RING,   S_ring, "rings", "кольцо")
    OBJCLASS( 5,  '"', AMULET, S_amulet, "amulets", "амулет")
    OBJCLASS( 6,  '(', TOOL,   S_tool, "tools",
                                       "полезный предмет (кирка, ключ, лампа...)")

    OBJCLASS( 7,  '%', FOOD,   S_food, "food", "кусок еды")
    OBJCLASS( 8,  '!', POTION, S_potion, "potions", "зелье")
    OBJCLASS( 9,  '?', SCROLL, S_scroll, "scrolls", "свиток")
    OBJCLASS(10,  '+', SPBOOK, S_book, "spellbooks", "книга заклинаний")
    OBJCLASS(11,  '/', WAND,   S_wand, "wands", "волшебна€ палочка")
    OBJCLASS2(12, '$', COIN,   GOLD_SYM, S_coin, "coins", "груда монет")
    OBJCLASS(13,  '*', GEM,    S_gem, "rocks", "самоцвет или камень")
    OBJCLASS(14,  '`', ROCK,   S_rock, "large stones", "валун или стату€")
    OBJCLASS(15,  '0', BALL,   S_ball, "iron balls", "железный шар")
    OBJCLASS(16,  '_', CHAIN,  S_chain, "chains", "железна€ цепь")
    OBJCLASS(17,  '.', VENOM,  S_venom, "venoms", "брызги €да")
