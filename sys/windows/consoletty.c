...
#endif
                    WCHAR wc = back->wcharacter;
                    if (wc >= 0xff00 && wc <= 0xffff)
                        wc = console.cpMap[wc & 0xff];
                    WriteConsoleW(console.hConOut, &wc, 1,
                                  &unused, NULL);
...
#endif
{
    ...
    default:
        ...
        wch[1] = 0;
        if (console.has_unicode) {
            if (ch >= 0 && ch < SIZE(console.cpMap))
                wch[0] = console.cpMap[ch];
            else if (ch >= 0xff00 && ch <= 0xffff)
                wch[0] = console.cpMap[ch & 0xff];
            else
                wch[0] = (WCHAR) ch;
#ifdef UTF8_FROM_CORE
            if (SYMHANDLING(H_UTF8)) {
...
console_g_putch(int in_ch)
{
    ...
    wch[1] = 0;
    if (console.has_unicode) {
        if (raw_ch >= 0xff00 && raw_ch <= 0xffff)
            wch[0] = console.cpMap[raw_ch & 0xff];
        else
            wch[0] = (usemap) ? cp437[ch] : ch;
#ifdef UTF8_FROM_CORE
        if (SYMHANDLING(H_UTF8)) {
            /* we have to convert it to UTF-8 for cell.utf8str */
            ccount = WideCharToMultiByte(
...
set_known_good_console_font(void)
{
    ...
    success = SetConsoleOutputCP(1251);
    nhassert(success);
    console.font_changed = TRUE;
#ifndef VIRTUAL_TERMINAL_SEQUENCES
...
#endif
        /* Always map the console code page (1251 for the Russian build)
           to real Unicode characters; do not special-case 1251 to the
           cp437 table. */
        for (int i = 0; i < 256; i++) {
            char c = (char)i;
...
//    console.height = max(console.height, ROWNO + 3);
  ...
  //      size, console.cursor, &unused);
    ...
    if (console.has_unicode) {
        if (console.code_page != 1251)
            apisuccess = SetConsoleOutputCP(1251);
