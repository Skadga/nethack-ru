...
*/
    ...
    if (!validate_prefix_locations(failbuf)) {
        raw_printf("Некоторые указанные каталоги некорректны:\n\t%s\n",
                   failbuf);
        nethack_exit(EXIT_FAILURE);
    }
 ...
 /*        wait_synch(); */
    ...
    if (!dlb_init()) {
        ...
        pline("NetHack не смог открыть требующийся файл \"%s\"", DLBFILE);
        if (file_exists(DLBFILE))
            pline("\nМожет быть, ты пытаешься запустить NetHack из zip-архива "
                  "?");
...
//    init_nhwindows(&argc, argv);
    ...
    if (!nhfp) {
        raw_print("Не удалось создать файл блокировки");
    } else {
...
attempt_restore:
    if ((getlock_result != -1) && (nhfp = restore_saved_game()) != 0) {
        ...
        if (ge.early_raw_messages)
            raw_print("Восстановление сохранения...");
        else
            pline("Восстановление сохранения...");
    ...
    while (argc > 1 && argv[1][0] == '-') {
        ...
        case 'u':
            if (argv[0][2])
                ...
                raw_print("После -u ожидается имя игрока");
            break;
        case 'g':
            if (argv[0][2]) {
...
eraseoldlocks(void)
{
    ...
    really_close();
#endif
    if (file_exists(fqname(gl.lock, LEVELPREFIX, 0))
        && unlink(fqname(gl.lock, LEVELPREFIX, 0)))
        return 0; /* it exists but cannot be removed */
    return (1);   /* success! */
}
...
#define OOPS_BUFSZ 512
    ...
    if ((fd = open(fq_lock, 0)) == -1) {
        ...
        error("Bad directory or name: %s\n%s\n", fq_lock,
                  strerror(errno));
        unlock_file(HLOCK);
        Sprintf(oops, "Не удаётся открыть %s", fq_lock);
        raw_print(oops);
        nethack_exit(EXIT_FAILURE);
    }
     ...
     */
    Sprintf(oops, "Ты решил %s.",
                (prompt_result == -1)
                    ? "уничтожить старую игру и начать новую"
                    : (prompt_result == 1)
                        ? "восстановить старую игру"
    ...
    if (prompt_result == 1) {          /* recover */
        if (recover_savefile()) {
            ...
            chdirx(orgdir, 0);
#endif
            raw_print("Не удалось восстановить старую игру.");
        }
        ...
        if (eraseoldlocks()) {
            ...
            chdirx(orgdir, 0);
#endif
            raw_print("Не удалось уничтожить старую игру.");
            return 0;
        }
    } else {
        unlock_file(HLOCK);
...
tty_self_recover_prompt(void)
{
    ...
    raw_print("\n");
    raw_print("\n");
    raw_print("Найдены файлы незавершённой игры, сохранённые под твоим именем. ");
    raw_print("Восстановить? [yn] ");
 ...
 tty_ask_again:
    ...
    if (pl == 1 && (c == 'n' || c == 'N')) {
        /* no to recover */
        raw_print("\n\nТы точно хочешь уничтожить старую игру, а не попытаться\n");
        raw_print("восстановить её? [yn] ");
        c = 'n';
        ct = 0;
        pl = 2;
        goto tty_ask_again;
...
other_self_recover_prompt(void)
{
    ...
    if (iflags.window_inited || WINDOWPORT(curses)) {
        c = y_n("Найдены файлы незавершённой игры, сохранённые под твоим именем. "
               "Восстановить?");
    } else {
        c = 'n';
        ct = 0;
 ...
 other_ask_again:
    ...
    if (pl == 1 && (c == 'n' || c == 'N')) {
        /* no to recover */
        c = y_n("Ты точно хочешь уничтожить старую игру, а не попытаться "
                  "восстановить её? [yn] ");
        pl = 2;
        if (!ismswin && !iscurses) {
            c = 'n';
