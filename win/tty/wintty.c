...
extern "C" {
#endif
int WINAPI SetConsoleOutputCP(unsigned int);
int WINAPI SetConsoleCP(unsigned int);
#ifdef __cplusplus
}
#endif
#endif
...
tty_init_nhwindows(int *argcp UNUSED, char **argv UNUSED)
{
    ...
    tty_putstr(BASE_WINDOW, 0,
              "\xCD\xE0\xE4 \xEF\xE5\xF0\xE5\xE2\xEE\xE4\xEE\xEC \xF0\xE0\xE1\xEE\xF2\xE0\xEB: Skadga");
    tty_putstr(BASE_WINDOW, 0, "");
...
tty_askname(void)
{
    static const char who_are_you[] = "\xCA\xF2\xEE \xF2\xFB? ";
    int c, ct, tryct = 0;
    ...
    do {
        if (++tryct > 1) {
            ...
            tty_putstr(BASE_WINDOW, 0, "\xC2\xE2\xE5\xE4\xE8 \xE8\xEC\xFF \xF1\xE2\xEE\xE5\xE3\xEE \xEF\xE5\xF0\xF1\xEE\xED\xE0\xE6\xE0...");
    ...
    while (!finished) {
        ...
        if (!page_start) {
            ...
            *rp = 0;
            ...
            if (cw->npages > 1)
                Sprintf(cw->morestr, "(%d \xE8\xE7 %d)", curr_page + 1,
                        (int) cw->npages);
            else if (msave)
        ...
        case MENU_SEARCH:
            if (cw->how == PICK_NONE) {
                ...
                tty_getlin("\xCF\xEE\xE8\xF1\xEA:", tmpbuf);
