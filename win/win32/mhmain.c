...
MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CLOSE: {
        ...
        if (program_state.gameover) {
            ...
            switch (NHMessageBox(hWnd, TEXT("Сохранить?"),
...
onWMCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
{
    ...
    case IDM_SETTING_SCREEN_TO_CLIPBOARD: {
        ...
        if (!OpenClipboard(hWnd)) {
            NHMessageBox(hWnd, TEXT("Не удалось открыть буфер обмена"),
                         MB_OK | MB_ICONERROR);
            free(p);
#ifdef ENHANCED_SYMBOLS
            free(wp);
    ...
    case IDM_SETTING_SCREEN_TO_FILE: {
        ...
        ofn.lpstrFilter = TEXT("Текстовые файлы (*.txt)\x0*.txt\x0")
            TEXT("Все файлы (*.*)\x0*.*\x0") TEXT("\x0\x0");
        ...
        if (!pFile) {
			...
			TEXT("Не удалось открыть %s для записи!"), filename);
    ...
    case IDM_CLEARSETTINGS: {
        ...
        NHMessageBox(GetNHApp()->hMainWnd,
                     TEXT("Ваши настройки Windows не будут сохранены при "
                          "выходе в этот раз."),
