    ...
    if (data->status_lines != NULL) {
        ...
        for (int line = 0; line < NHSW_LINES; line++) {
            ...
            for (int i = 0; i < status_line->status_strings.count; i++) {
                ...
                /* Always render as Unicode, converting the localized
                   one-byte (CP1251) game text explicitly.  Using the
                   ANSI DrawTextA path would interpret CP1251 bytes via
                   the system ANSI codepage, producing mojibake on
                   non-CP1251 systems. */
                BOOL useUnicode = TRUE;
                int wlen;

                ZeroMemory(wbuf, sizeof(wbuf));
                if (MultiByteToWideChar(1251, 0, str, -1, wbuf,
                                        SIZE(wbuf)) <= 0)
                    wbuf[0] = L'\0';
                wlen = wcslen(wbuf);
