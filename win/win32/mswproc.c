    ...
    if (iflags.wc_player_selection == VIA_DIALOG) {
        ...
        if (flags.randomall) {
            ...
            if (flags.initalign < 0
                ...
                if (flags.initalign < 0) {
                    raw_print("Несовместимое мировоззрение!");
                    flags.initalign =
    ...
    if (flags.initrole < 0) {
        ...
        if (pick4u == 'y' || flags.initrole == ROLE_RANDOM
            ...
            flags.initrole = pick_role(flags.initrace, flags.initgend,
                                       flags.initalign, PICK_RANDOM);
            if (flags.initrole < 0) {
                /* tty_putstr(BASE_WINDOW, 0, "Несовместимая роль!"); */
                flags.initrole = randrole(FALSE);
            }
        } else {
            /* tty_clear_nhwindow(BASE_WINDOW); */
            ...
            add_menu(win, &nul_glyphinfo, &any, '*', 0,
                     ATR_NONE, clr, "Случайно", MENU_ITEMFLAGS_NONE);
            any.a_int = i + 1; /* must be non-zero */
            add_menu(win, &nul_glyphinfo, &any, 'q', 0,
                     ATR_NONE, clr, "Выйти", MENU_ITEMFLAGS_NONE);
            Snprintf(pbuf, sizeof pbuf, "Выберите роль для вашего %s", plbuf);
            end_menu(win, pbuf);
    ...
    if (flags.initrace < 0 || !validrace(flags.initrole, flags.initrace)) {
        ...
        if (pick4u == 'y' || flags.initrace == ROLE_RANDOM
            ...
            flags.initrace = pick_race(flags.initrole, flags.initgend,
                                       flags.initalign, PICK_RANDOM);
            if (flags.initrace < 0) {
                /* tty_putstr(BASE_WINDOW, 0, "Несовместимая раса!"); */
                flags.initrace = randrace(flags.initrole);
            }
        } else { /* pick4u == 'n' */
            ...
            if (n > 1) {
                ...
                add_menu(win, &nul_glyphinfo, &any, 'q', 0,
                         ATR_NONE, clr, "Выйти", MENU_ITEMFLAGS_NONE);
                Snprintf(pbuf, sizeof pbuf, "Выберите расу вашего %s", plbuf);
                end_menu(win, pbuf);
                n = select_menu(win, PICK_ONE, &selected);
                destroy_nhwindow(win);
                if (n != 1 || selected[0].item.a_int == any.a_int)
    ...
    if (flags.initgend < 0
        ...
        if (pick4u == 'y' || flags.initgend == ROLE_RANDOM
            ...
            flags.initgend = pick_gend(flags.initrole, flags.initrace,
                                       flags.initalign, PICK_RANDOM);
            if (flags.initgend < 0) {
                /* tty_putstr(BASE_WINDOW, 0, "Несовместимый пол!"); */
                flags.initgend = randgend(flags.initrole, flags.initrace);
            }
        } else { /* pick4u == 'n' */
            ...
            if (n > 1) {
                ...
                add_menu(win, &nul_glyphinfo, &any, 'q', 0,
                         ATR_NONE, clr, "Выйти", MENU_ITEMFLAGS_NONE);
                Snprintf(pbuf, sizeof pbuf, "Выберите пол вашего %s", plbuf);
                end_menu(win, pbuf);
                n = select_menu(win, PICK_ONE, &selected);
                destroy_nhwindow(win);
                if (n != 1 || selected[0].item.a_int == any.a_int)
    ...
    if (flags.initalign < 0
        ...
        if (pick4u == 'y' || flags.initalign == ROLE_RANDOM
            ...
            flags.initalign = pick_align(flags.initrole, flags.initrace,
                                         flags.initgend, PICK_RANDOM);
            if (flags.initalign < 0) {
                /* tty_putstr(BASE_WINDOW, 0, "Несовместимое мировоззрение!"); */
            ...
            if (n > 1) {
                ...
                add_menu(win, &nul_glyphinfo, &any, 'q', 0,
                         ATR_NONE, clr, "Выйти", MENU_ITEMFLAGS_NONE);
                Snprintf(pbuf, sizeof pbuf, "Выберите мировоззрение вашего %s", plbuf);
                end_menu(win, pbuf);
                n = select_menu(win, PICK_ONE, &selected);
                destroy_nhwindow(win);
                if (n != 1 || selected[0].item.a_int == any.a_int)
...
mswin_askname(void)
{
    ...
    if (mswin_getlin_window("Кто ты?", svp.plname, PL_NSIZ) == IDCANCEL) {
        bail("bye-bye");
        /* not reached */
    }
}
    ...
    if (!f) {
        if (must_exist) {
            ...
            nh_stprintf(message, sizeof message,
                        TEXT("Внимание! Не удалось найти файл: %s\n"),
...
initMapTiles(void)
{
    ...
    if (hBmp == NULL) {
        ...
        Sprintf(errmsg, "%s (0x%lx).",
            "Не удалось загрузить тайлы из файла. Возврат к настройкам по умолчанию",
            errcode);
        raw_print(errmsg);
        return FALSE;
    }
    ...
    if (bm.bmWidth % iflags.wc_tile_width
        ...
        DeleteObject(hBmp);
        raw_print("Размеры растрового изображения тайлов не совпадают с параметрами "
                  "tile_width и tile_height. Возврат к настройкам по умолчанию.");
        return FALSE;
    }
    ...
    if (tl_num < total_tiles_used) {
        DeleteObject(hBmp);
        raw_print("Количество тайлов в изображении меньше требуемого "
                  "игрой. Возврат к настройкам по умолчанию.");
        return FALSE;
    }
...
static mswin_condition_field _condition_fields[CONDITION_COUNT] = {
    { BL_MASK_BAREH,     "Гр", 0},
    { BL_MASK_BLIND,     "Слеп", 0 },
    { BL_MASK_BUSY,      "Занят", 0 },
    { BL_MASK_CONF,      "Смут", 0 },
    { BL_MASK_DEAF,      "Глух", 0 },
    { BL_MASK_ELF_IRON,  "Желез", 0 },
    { BL_MASK_FLY,       "Лет", 0 },
    { BL_MASK_FOODPOIS,  "Отрав", 0 },
    { BL_MASK_GLOWHANDS, "Свеч", 0 },
    { BL_MASK_GRAB,      "Хват", 0 },
    { BL_MASK_HALLU,     "Галл", 0 },
    { BL_MASK_HELD,      "Держ", 0 },
    { BL_MASK_ICY,       "Лед", 0 },
    { BL_MASK_INLAVA,    "Лава", 0 },
    { BL_MASK_LEV,       "Парит", 0 },
    { BL_MASK_PARLYZ,    "Парал", 0 },
    { BL_MASK_RIDE,      "Верхом", 0 },
    { BL_MASK_SLEEPING,  "Сон", 0 },
    { BL_MASK_SLIME,     "Слизь", 0 },
    { BL_MASK_SLIPPERY,  "Катит", 0 },
    { BL_MASK_STONE,     "Камен", 0 },
    { BL_MASK_STRNGL,    "Удуш", 0 },
    { BL_MASK_STUN,      "Оглуш", 0 },
    { BL_MASK_SUBMERGED, "Глуб", 0 },
    { BL_MASK_TERMILL,   "Болезн", 0 },
    { BL_MASK_TETHERED,  "Цепь", 0 },
    { BL_MASK_TRAPPED,   "Лов", 0 },
    { BL_MASK_UNCONSC,   "Бесс", 0 },
    { BL_MASK_WOUNDEDL,  "Раны", 0 },
    { BL_MASK_HOLDING,   "Удерж", 0 },
};
