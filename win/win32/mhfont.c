...
{
    CHARSETINFO cis;
    /* Localized builds feed the game text to the drawing code as one-byte
       characters using the ANSI charset (Russian builds: CP1251).  The
       H_IBM check previously selected the OEM charset (CP866 on Russian
       Windows), which made Cyrillic display as mojibake.  Always use the
       CP1251 charset so the raw bytes are interpreted correctly even when
       the system ANSI codepage is something else. */
    if (TranslateCharsetInfo((DWORD *) (uintptr_t) 1251, &cis, TCI_SRCCODEPAGE))
