...
-- scatter some bodies
des.object({ id = "труп", x=20,y=12, montype="aligned cleric" })
des.object({ id = "труп", coord = place[1], montype="shopkeeper" })
des.object({ id = "труп", coord = place[2], montype="shopkeeper" })
des.object({ id = "труп", coord = place[3], montype="shopkeeper" })
des.object({ id = "труп", coord = place[4], montype="shopkeeper" })
des.object({ id = "труп", coord = place[5], montype="shopkeeper" })
des.object({ id = "труп", montype="watchman" })
des.object({ id = "труп", montype="watchman" })
des.object({ id = "труп", montype="watchman" })
des.object({ id = "труп", montype="watchman" })
des.object({ id = "труп", montype="watch captain" })
...
des.object({ id = "восковая свеча", coord = place[4], quantity = math.random(1,2) })

des.object({ id = "восковая свеча", coord = place[1], quantity = math.random(2,4) })
des.object({ id = "восковая свеча", coord = place[2], quantity = math.random(1,2) })
des.object({ id = "говяжий свеча", coord = place[3], quantity = math.random(1,3) })
des.object({ id = "говяжий свеча", coord = place[2], quantity = math.random(1,2) })
des.object({ id = "говяжий свеча", coord = place[4], quantity = math.random(1,2) })

-- go ahead and leave a lamp next to one corpse to be suggestive
-- and some empty wands...
des.object("масло лампа",place[2])
des.object({ id = "ударный", coord = place[1], buc="uncursed", spe=0 })
des.object({ id = "ударный", coord = place[3], buc="uncursed", spe=0 })
des.object({ id = "ударный", coord = place[4], buc="uncursed", spe=0 })
des.object({ id = "магический снаряд", coord = place[4], buc="uncursed", spe=0 })
des.object({ id = "магический снаряд", coord = place[5], buc="uncursed", spe=0 })
...
for i = 1, math.random(5,15) do
   if percent(50) then
      des.monster({ id = "Капитан Орков", coord = inside:rndcoord(1), peaceful=0 });
   else
      if percent(80) then
         des.monster({ id = "Урук-хай", coord = inside:rndcoord(1), peaceful=0 })
      else
         des.monster({ id = "Мордорский Орк", coord = inside:rndcoord(1), peaceful=0 })
      end
   end
end
...
for i = 1, math.random(1,6) do
   des.monster({ id = "Орк-Шаман", coord = near_temple:rndcoord(0), peaceful=0,
...
for i = 1, math.random(10,19) do
   if percent(90) then
      des.monster({ id = "холмовой орк", peaceful = 0 })
   else
      des.monster({ id = "Гоблин", peaceful = 0 })
   end
end
