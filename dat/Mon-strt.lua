...
-- The Grand Master
des.monster({ id = "Великий повелитель", coord = {28, 10}, inventory = function()
   des.object({ id = "ряса", spe = 6 });
end })
-- No treasure chest!
-- guards for the audience chamber
des.monster("аббат", 32, 07)
des.monster("аббат", 32, 08)
des.monster("аббат", 32, 11)
des.monster("аббат", 32, 12)
des.monster("аббат", 33, 07)
des.monster("аббат", 33, 08)
des.monster("аббат", 33, 11)
des.monster("аббат", 33, 12)
-- Non diggable walls
...
for i = 1, 8 do
   des.monster("Земляной Элементаль", spacelocs:rndcoord(1))
end
for i = 1, 4 do
   des.monster("ксорн", spacelocs:rndcoord(1))
end
...
des.object({ id="банка", coord = {29, 9}, quantity=2, montype="spinach" })
-- ensure enough vegetarian food generates for vegetarian games
des.object({ id="еда паёк", coord = {46, 4}, quantity = 4})
