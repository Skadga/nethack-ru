...
themeroom_fills = {
...

   {
      ...
      contents = function(rm)
         ...
         for i = 1, (fog:numpoints() / 4) do
            des.monster({ id = "Туманное Облако", asleep = true });
         end
...

   {
      ...
      contents = function(rm)
         ...
         for i = 1, npts do
            des.monster({ id = "Лесная Нимфа", asleep = true });
            if (percent(30)) then
...

   {
      ...
      contents = function(rm)
         des.object({ id = "сундук", buried = true, contents = function(otmp)
...

   {
      ...
      contents = function(rm)
         ...
         for i = 1, (rm.width * rm.height) / 2 do
            shuffle(zombifiable);
            local o = des.object({ id = "труп", montype = zombifiable[1],
                                 buried = true });
...

   {
      ...
      contents = function(rm)
         ...
         for i = 1, d(5,5) do
            ...
            des.object({ id = "труп", montype = mon[idx] });
         end
      end,
   },
...

   {
      ...
      contents = function(rm)
         des.object({ id = "масло лампа", lit = true });
      end
   },
...

   {
      ...
      contents = function(rm)
         ...
         des.monster({ id = "призрак", asleep = true, waiting = true,
                       coord = loc });
         if percent(65) then
            des.object({ id = "кинжал", coord = loc, buc = "not-blessed" });
         end
         if percent(55) then
            des.object({ class = ")", coord = loc, buc = "not-blessed" });
         end
         if percent(45) then
            des.object({ id = "лук", coord = loc, buc = "not-blessed" });
            des.object({ id = "стрела", coord = loc, buc = "not-blessed" });
         end
         if percent(65) then
            des.object({ class = "[", coord = loc, buc = "not-blessed" });
         end
      ...
      contents = function(rm)
         ...
         local func = function(x,y)
            if (percent(25)) then
               ...
               des.monster({ class = "m", appear_as = "obj:сундук" });
            end
         end;
         locs:iterate(func);
      end,
                     ...
                     des.room({ type = "themed",
                                 ...
                                 contents = function()
                                    if (percent(50)) then
                                       ...
                                       des.object({ id = "труп", montype = "@", coord = {0,0} });
                                    end
                                    if (percent(20)) then
      ...
      contents = function()
         des.map({ map = [[
            ...
            if itmcls[ "material" ] == "glass" then
                  ...
                  box = des.object({ id = "сундук", coord = chest_spots[1],
                                    olocked = "no" });
            else
            ...
            for i = 2, #chest_spots do
                  des.object({ id = "сундук", coord = chest_spots[i] });
            end
