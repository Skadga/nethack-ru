...
des.object({ id="статуя", coord=medloc, buc="uncursed",
                      montype="knight", historic=1, male=1,name="Perseus",
                      contents = function()
                         if percent(75) then
                            des.object({ id = "щит отражения", buc="cursed", spe=0 })
                         end
                         if percent(25) then
                            des.object({ id = "сапоги левитации", spe=0 })
                         end
                         if percent(50) then
                            des.object({ id = "сабля", buc="blessed", spe=2 })
                         end
                         if percent(50) then
                            des.object("мешок")
                         end
...
des.object({ id = "статуя", coord=altloc, contents=0 })
des.object({ id = "статуя", contents=0 })
des.object({ id = "статуя", contents=0 })
des.object({ id = "статуя", contents=0 })
des.object({ id = "статуя", contents=0 })
des.object({ id = "статуя", contents=0 })
...
des.object("чистый бумага",48,18)
des.object("чистый бумага",48,18)
--
des.trap("rust")
des.trap("rust")
...
des.monster("медуза")
des.monster("медуза")
des.monster("Лесная Нимфа")
des.monster("Лесная Нимфа")
des.monster("Водяная Нимфа")
des.monster("Водяная Нимфа")

for i=1,30 do
   des.monster({ id = "ворон", peaceful = 0 })
end
