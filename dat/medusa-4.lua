...
--
des.object("кристальный €дро", 07,08)
--
-- same spot as Medusa plus downstairs
des.object({ id="стату€", coord=medloc, buc="uncursed",
...
des.object({ id = "стату€", coord=altloc, contents=0 })
des.object({ id = "стату€", contents=0 })
des.object({ id = "стату€", contents=0 })
des.object({ id = "стату€", contents=0 })
des.object({ id = "стату€", contents=0 })
des.object({ id = "стату€", contents=0 })
...
des.monster({ id = "медуза", coord=medloc, asleep=1 })
des.monster("кракен", 07,07)
--
-- the nesting dragon
des.monster({ id = "∆Єлтый ƒракон", x=05, y=04, asleep=1 })
if percent(50) then
   des.monster({ id = "ƒетЄныш ∆Єлтого ƒракона", x=04,y=04, asleep=1 })
end
if percent(25) then
   des.monster({ id = "ƒетЄныш ∆Єлтого ƒракона", x=04, y=05, asleep=1 })
end
des.object({ id = "€йцо", x=05, y=04, montype="yellow dragon" });
if percent(50) then
   des.object({ id = "€йцо", x=05, y=04, montype="yellow dragon" });
end
if percent(25) then
...
for i=1,4 do
   des.monster("ƒетЄныш „Єрной Ќаги")
   des.monster("„Єрна€ Ќага")
end
