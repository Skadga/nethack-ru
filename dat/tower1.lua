...
des.monster({ id="вампир лорд", coord=niches[4], name=Vnames[1], waiting=1 })
des.monster({ id="вампир лорд", coord=niches[5], name=Vnames[2], waiting=1 })
des.monster({ id="вампир лорд", coord=niches[6], name=Vnames[3], waiting=1 })
-- The doors
...
-- treasures
des.object("сундук", 07,05)

des.object("сундук",niches[6])
des.object("сундук",niches[1])
des.object("сундук",niches[2])
des.object("сундук",niches[3])
des.object({ id = "сундук", coord=niches[4],
             contents = function()
                des.object({ id = "восковая свеча", quantity=math.random(4,8) })
             end
});
des.object({ id = "сундук", coord=niches[5],
             contents = function()
                des.object({ id = "говяжий свеча", quantity=math.random(4,8) })
             end
});
