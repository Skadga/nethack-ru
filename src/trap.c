...
static const char *const a_your[2] = { "", "вашу" };
static const char *const A_Your[2] = { "", "Ваш" };
static const char tower_of_flame[] = "столб пламени";
static const char *const A_gush_of_water_hits = "Поток воды бьет";
static const char *const blindgas[6] = { "гнилостного", "удушливого",
                                         "едкого",   "леденящего",
                                         "резкого",  "кусачего" };
...
#define burn_dmg(obj, descr) erode_obj(obj, descr, ERODE_BURN, EF_GREASE)
    while (1) {
        ...
        case 0:
            ...
            if (!burn_dmg(item, item ? buf : "шлем"))
                continue;
            break;
        case 1:
    ...
    int ef_flags)
{
    static NEARDATA const char
        *const action[] = { "тлеет", "ржавеет", "гниёт", "разъедается",
                            "трескается" },
        *const msg[] = { "обожжен", "заржавевший", "сгнивший", "разъеденный", "растрескавшийся" },
        *const bythe[] = { "огня", "ржавчины", "гниения", "кислоты",
                           "трещин" }; /* this could use improvement... */
    ...
    if (check_grease && otmp->greased) {
        ...
        if (flags.verbose && print && (uvictim || vismon))
            pline("%s %s не страдает от %s.", uvictim ? "Ваш" : "Чужой",
                  ostr, bythe[type]);
        return ER_NOTHING;
    } else if (otmp->oerodeproof || (otmp->blessed && !rnl(4))) {
        if (flags.verbose && (print || otmp->oerodeproof)
            && (uvictim || vismon || visobj))
            pline("Каким-то образом, %s %s не страдает от %s.",
                  uvictim ? "ваш" : !vismon ? "этот" : "чужой",
                  ostr, bythe[type]);
        ...
        const char *adverb = (erosion + 1 == MAX_ERODE) ? " совсем"
                             : erosion ? " ещё"
                               : "";
        ...
        if (uvictim || vismon || visobj) {
            ...
            static const char *const destroy_act[] =
                { "догорает дотла", "ржавеет насквозь", "полностью сгнивает",
                  "полностью растворяется", "разлетается вдребезги" };
            Strcpy(actbuf, destroy_act[type]);
            pline("%s %s %s!",
                  uvictim ? "Ваш"
                  : !vismon ? "Этот"
                    : "Чужой",
    ...
    struct monst *victim)
{
    ...
    if (ostr) {
        if (victim == &gy.youmonst)
            Your("%s защищено слоем жира!", ostr);
        else if (vismon)
            pline("%s защищено слоем жира!", ostr);
    } else if (victim == &gy.youmonst || vismon) {
        pline("%s защищено слоем жира!", Yobjnam2(otmp, ""));
    }
    if (!rn2(2)) {
        otmp->greased = 0;
        if (carried(otmp)) {
            pline("Смазка растваяется.");
            update_inventory();
        }
        return TRUE;
    }
    ...
    unsigned ftflags)
{
    ...
    if (td) {
        ...
        if (!Sokoban && !(ftflags & TOOKPLUNGE)) {
            if (t->ttyp == TRAPDOOR)
                pline("Под вами открывается люк!");
            else
                pline("Под вами зияющая дыра!");
        }
    } else
        pline("%s Разрывается под вами!", surface(u.ux, u.uy));
    ...
    if (Sokoban && Can_fall_thru(&u.uz)) {
        ; /* KMH -- You can't escape the Sokoban level traps */
             ...
             || ((Flying || is_clinger(gy.youmonst.data)
                 ...
                 && !(ftflags & TOOKPLUNGE))) {
        dont_fall = "не проваливаетесь.";
    } else if (gy.youmonst.data->msize >= MZ_HUGE) {
        dont_fall = "не пролезаете.";
    } else if (!next_to_u()) {
        dont_fall = "вы рывком отдернуты назад своим питомцем!";
    }
    if (dont_fall) {
        You1(dont_fall);
        ...
        if (!td) {
            ...
            pline("Отверстие под вами закрывается.");
        }
        return;
    }
    if ((Flying || is_clinger(gy.youmonst.data))
        ...
        You("%s вниз %s!",
            Flying ? "со свистом падаете" : "нарочно падаете",
            (t->ttyp == TRAPDOOR)
                ? "через люк"
                : "в зияющую дыру");
    }
    ...
    if (Is_stronghold(&u.uz)) {
        ...
        if (dist > 1)
            You("%s вниз по %s%sшахте!",
                controlled_flight ? "летите" : "падаете",
                dist > 3 ? "очень " : "",
                dist > 2 ? "глубокой " : "");
    }
    if (!td)
        Sprintf(msgbuf, "Отверстие в %s над вами закрывается.",
                ceiling(u.ux, u.uy));
...
{
    static const char
        historic_statue_is_gone[] = "что историческая статуя теперь исчезла";
    ...
    comes_to_life = !canspotmon(mon) ? "исчезает"
                    : golem_xform ? "превращается в плоть"
    ...
    if (u_at(x, y) || cause == ANIMATE_SPELL) {
        ...
        pline("%s Вдруг оживает.", rndmonnam((char *) 0));
    } else if (cause == ANIMATE_SHATTER) {
        if (cansee(x, y))
            Sprintf(statuename, "%s%s", shk_your(tmpbuf, statue),
                    xname(statue));
        else
            Strcpy(statuename, "статуя");
        pline("Вместо разлетания на куски, %s вдруг %s!", statuename,
              comes_to_life);
    } else { /* cause == ANIMATE_NORMAL */
        set_msg_xy(x, y);
        You("обнаруживаете %s, прикинывающегося статуэй.",
    ...
    if (!svc.context.mon_moving) {
        ...
        if (historic) {
            You_feel("чувствуете вину %s.", historic_statue_is_gone);
            adjalign(-1);
        }
    } else {
        if (historic && cansee(x, y))
            You_feel("сожалеете %s.", historic_statue_is_gone);
    ...
    struct trap *trap)
{
    ...
    if (amorphous(mptr) || is_whirly(mptr) || flaming(mptr)
        ...
        if (flaming(mptr) || acidic(mptr)) {
            if (domsg) {
                if (isyou)
                    You("%s с помощью пламени паутину!", a_your[trap->madeby_u]);
                else
                    pline_mon(mtmp,
                      "%s %s %s паутину!", Monnam(mtmp),
                      (flaming(mptr)) ? "сжигает" : "растворяет",
        ...
        if (domsg) {
            if (isyou) {
                You("проливаетесь сквозь паутину.", a_your[trap->madeby_u]);
            } else {
                pline_mon(mtmp,
                      "%s протекает сквозь %s паутину.", Monnam(mtmp),
                      a_your[trap->madeby_u]);
                seetrap(trap);
            }
        }
    ...
    unsigned trflags UNUSED)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (trap->once && trap->tseen && !rn2(15)) {
            if (in_sight && see_it)
                pline_mon(mtmp,
                      "%s задевает ловушку, но ничего не происходит.",
                      Monnam(mtmp));
            deltrap(trap);
    ...
    unsigned int trflags UNUSED)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        trap->once = 1;
        seetrap(trap);
        pline("Маленький дротик вылетает в вас!");
        ...
        if (u.usteed && !rn2(2) && steedintrap(trap, otmp)) {
            ; /* nothing */
        } else if (thitu(7, Maybe_Half_Phys(dam), &otmp, "дротик")) {
            if (otmp) {
                if (otmp->opoisoned)
                    poisoned("дротик", A_CON, "отравленный дротик",
    ...
    unsigned trflags UNUSED)
{
    ...
    if (mtmp == &gy.youmonst) {
        if (trap->once && trap->tseen && !rn2(15)) {
            pline("В %s открывается люк, но ничего не выпадает!",
            ...
            pline("В %s открывается люк, и %s падает на вашу %s!",
            ...
            if (uarmh) {
                ...
                if (passes_rocks(gy.youmonst.data)) {
                    pline("К сожалению, на вас %s.",
                          an(helm_simple_name(uarmh))); /* helm or hat */
                    dmg = 2;
                } else if (hard_helmet(uarmh)) {
                    pline("К счастью, на вас твердый шлем.");
                    dmg = 2;
                } else if (flags.verbose) {
                    pline("%s не защищает вас.", Yname2(uarmh));
                }
            } else if (passes_rocks(gy.youmonst.data)) {
                pline("Оно безвредно проходит сквозь вас.");
                harmless = TRUE;
            }
            if (!Blind)
                observe_object(otmp);
        ...
        if (trap->once && trap->tseen && !rn2(15)) {
            if (in_sight && see_it)
                pline_mon(mtmp,
                      "Над %s открывается люк, но ничего не выпадает!",
                      mon_nam(mtmp));
            deltrap(trap);
            newsym(mtmp->mx, mtmp->my);
            return Trap_Is_Gone;
    ...
    if (mtmp == &gy.youmonst) {
        if ((Levitation || Flying) && !forcetrap) {
            if (!Blind) {
                seetrap(trap);
                if (Hallucination)
                    You("замечаете складку в линолеуме.");
                else
                    You("замечаете расшатанную доску под собой.");
            }
        } else {
            seetrap(trap);
            ...
            pline("Доска под вами %s%s%s",
                  Deaf ? "вибрирует." : "скрипит",
                  Deaf ? "" : trapnote(trap, TRUE),
                  Deaf ? "" : " могуче.");
            wake_nearby(FALSE);
        }
    } else {
        ...
        if (in_sight) {
            if (!Deaf) {
                ...
                pline_mon(mtmp,
                      "Доска под %s громко скрипит%s.",
                      mon_nam(mtmp), trapnote(trap, TRUE));
                seetrap(trap);
            } else if (!mindless(mtmp->data)) {
                pline_mon(mtmp,
                      "%s на мгновение останавливается и жмится.",
                      Monnam(mtmp));
            }
        } else {
            ...
            You_hear("скрип %s %s.", trapnote(trap, TRUE),
                     (mdistu(mtmp) <= range * range)
                        ? "рядом" : "вдали");
        }
    ...
    unsigned trflags)
{
    ...
    if (is_you) {
        ...
        if (amorphous(gy.youmonst.data) || is_whirly(gy.youmonst.data)
            ...
            pline("%s медвежий капкан безвредно захлопнулся сквозь вас.",
                  A_Your[trap->madeby_u]);
            return Trap_Effect_Finished;
        }
        if (!u.usteed && gy.youmonst.data->msize <= MZ_SMALL) {
            pline("%s медвежий капкан безвредно захлопнулся над вами.",
                  A_Your[trap->madeby_u]);
            return Trap_Effect_Finished;
        }
        set_utrap((unsigned) rn1(4, 4), TT_BEARTRAP);
        if (u.usteed) {
            pline("%s медвежий капкан захлопнулся на %s %s!", A_Your[trap->madeby_u],
            ...
            pline("%s медвежий капкан захлопнулся на вашу %s!", A_Your[trap->madeby_u],
                  body_part(FOOT));
        ...
        if (mptr->msize > MZ_SMALL && !amorphous(mptr) && !m_in_air(mtmp)
            ...
            if (in_sight) {
                pline_mon(mtmp,
                      "%s попадает в капкан %s!", Monnam(mtmp),
                      a_your[trap->madeby_u]);
                seetrap(trap);
            } else {
                if (mptr == &mons[PM_OWLBEAR]
                    ...
                    You_hear("рыкание разяренного медведя!");
                }
            }
        } else if (forcetrap) {
            if (in_sight) {
                pline_mon(mtmp,
                      "%s избегает капкана %s!", Monnam(mtmp),
                      a_your[trap->madeby_u]);
                seetrap(trap);
            }
        }
    ...
    unsigned int trflags UNUSED)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (Sleep_resistance || breathless(gy.youmonst.data)) {
            You("окутаны облаком газа!");
            monstseesu(M_SEEN_SLEEP);
        } else {
            pline("Облако газа усыпляет вас!");
        ...
        if (!resists_sleep(mtmp) && !breathless(mtmp->data)
            ...
            if (sleep_monst(mtmp, rnd(25), -1) && in_sight) {
                pline_mon(mtmp,
                          "%s внезапно засыпает!", Monnam(mtmp));
                seetrap(trap);
            }
        }
    }
    ...
    unsigned int trflags UNUSED)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        switch (rn2(5)) {
        case 0:
            pline("%s вас по голове!", A_gush_of_water_hits, body_part(HEAD));
            (void) water_damage(uarmh, helm_simple_name(uarmh), TRUE);
            break;
        case 1:
            pline("%s вашу левую %s!", A_gush_of_water_hits, body_part(ARM));
            if (water_damage(uarms, "щит", TRUE) != ER_NOTHING)
                break;
 ...
 uglovecheck:
            ...
            break;
        case 2:
            pline("%s вашу правую %s!", A_gush_of_water_hits, body_part(ARM));
            (void) water_damage(uwep, 0, TRUE);
            goto uglovecheck;
        default:
            pline("%s вас!", A_gush_of_water_hits);
            ...
            if (uarmc)
                ...
                (void) water_damage(uarmu, "рубашку", TRUE);
        }
        update_inventory();
        ...
        case 0:
            if (in_sight)
                pline_mon(mtmp,
                      "%s %s по голове!", A_gush_of_water_hits,
                      mon_nam(mtmp));
        ...
        case 1:
            if (in_sight)
                pline_mon(mtmp,
                      "%s %s по левой руке!", A_gush_of_water_hits,
                      mon_nam(mtmp));
            target = which_armor(mtmp, W_ARMS);
            if (water_damage(target, "щит", TRUE) != ER_NOTHING)
                break;
            target = MON_WEP(mtmp);
 ...
 mglovecheck:
        ...
        case 2:
            if (in_sight)
                pline_mon(mtmp,
                      "%s %s по правой руке!", A_gush_of_water_hits,
                      mon_nam(mtmp));
            (void) water_damage(MON_WEP(mtmp), 0, TRUE);
            goto mglovecheck;
        default:
            if (in_sight)
                pline("%s по %s!", A_gush_of_water_hits, mon_nam(mtmp));
            ...
            if ((target = which_armor(mtmp, W_ARMC)) != 0)
                ...
                (void) water_damage(target, "рубашку", TRUE);
        }

        if (completelyrusts(mptr)) {
            if (in_sight)
                pline_mon(mtmp, "%s %s на части!", Monnam(mtmp),
                      !mlifesaver(mtmp) ? "разваливается" : "начинает разваливаться");
    ...
    unsigned int trflags UNUSED)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (in_sight)
            pline_mon(mtmp,
                 "%s вырывается из-%uпод %s у %s!", tower_of_flame,
                  surface(mtmp->mx, mtmp->my), mon_nam(mtmp));
        else if (see_it) { /* evidently `mtmp' is invisible */
            set_msg_xy(mtmp->mx, mtmp->my);
            You_see("%s вырывается из %s!", tower_of_flame,
                    surface(mtmp->mx, mtmp->my));
        }
        if (resists_fire(mtmp)) {
            if (in_sight) {
                shieldeff(mtmp->mx, mtmp->my);
                pline("%s не пострадал.", Monnam(mtmp));
            }
        } else {
    ...
    unsigned int trflags)
{
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (!Sokoban && is_clinger(gy.youmonst.data) && !plunged) {
            if (already_known) {
                You_see("%s%s под вами.", a_your[trap->madeby_u] ? "вашу " : "",
                        ttype == SPIKED_PIT ? "шипованную яму" : "яму");
            } else {
                pline("%s%s открывается под вами!",
                      trap->madeby_u ? "Ваша яма" : "Яма",
                      ttype == SPIKED_PIT ? " с шипами" : "");
                You("не проваливаетесь!");
            }
            return Trap_Effect_Finished;
        }
        if (!Sokoban) {
            ...
            *verbbuf = '\0';
            if (u.usteed) {
                if ((trflags & RECURSIVETRAP) != 0)
                    Sprintf(verbbuf, "и %s падаете",
                            x_monnam(u.usteed, steed_article, (char *) 0,
                                     SUPPRESS_SADDLE, FALSE));
                else
                    Sprintf(verbbuf, "тянете %s за собой",
                            x_monnam(u.usteed, steed_article, "бедную",
                                     SUPPRESS_SADDLE, FALSE));
            } else if (iflags.menu_requested && already_known) {
                You("осторожно %s в яму.",
                    u_locomotion("lower yourself"));
                deliberate = TRUE;
            } else if (conj_pit) {
                You("перемещаетесь в соседнюю яму.");
            } else if (adj_pit) {
                You("спотыкаетесь об обломки%s.",
                    !rn2(5) ? " между ямами" : "");
            } else {
                Strcpy(verbbuf,
                       !plunged ? "прыгаете" : (Flying ? "ныряете" : "падаете"));
            }
            if (*verbbuf)
                You("%s в %s яму!", verbbuf, a_your[trap->madeby_u]);
        }
        /* wumpus reference */
        if (Role_if(PM_RANGER) && !trap->madeby_u && !trap->once
            && In_quest(&u.uz) && Is_qlocate(&u.uz)) {
            pline("К счастью, в ней все-таки есть дно...");
            trap->once = 1;
        } else if (u.umonnum == PM_PIT_VIPER || u.umonnum == PM_PIT_FIEND) {
            pline("Как жалко. Разве это не ямы?");
        }
        if (relevant_spikes && wearing_iron_shoes(mtmp)) {
            pline("%s защищает вас от острых железных шипов.", Yname2(uarmf));
            relevant_spikes = FALSE;
        } else if (relevant_spikes) {
            const char *predicament = "на острые железные шипы";

            if (u.usteed) {
                pline("%s %s %s!",
                      upstart(x_monnam(u.usteed, steed_article, "бедную",
                                       SUPPRESS_SADDLE, FALSE)),
                      conj_pit ? "ступает" : "приземляется", predicament);
            } else
                You("%s %s!", conj_pit ? "наступаете" : "приземляетесь", predicament);
        }
        /* FIXME:
        ...
        if (!steedintrap(trap, (struct obj *) 0)) {
            if (relevant_spikes) {
                ...
                losehp(Maybe_Half_Phys(rnd(conj_pit ? 4 : adj_pit ? 6 : 10)),
                       ...
                       plunged
                       ? "намеренно нырнул в яму с железными шипами"
                       : (conj_pit || deliberate)
                         ? "ступил в яму с железными шипами"
                         : adj_pit
                           ? "споткнулся и упал в яму с железными шипами"
                           : "упал в яму с железными шипами",
                       NO_KILLER_PREFIX);
                if (!rn2(6))
                    poisoned("шипы", A_STR,
                             (conj_pit || adj_pit || deliberate)
                             ? "наступив на ядовитые шипы"
                             : "упав на ядовитые шипы",
                             /* if damage triggered life-saving,
                                poison is limited to attrib loss */
                ...
                if (!conj_pit && !deliberate
                    ...
                    losehp(Maybe_Half_Phys(rnd(adj_pit ? 3 : 6)),
                           plunged ? "намеренно нырнул в яму"
                           : "упал в яму",
                           NO_KILLER_PREFIX);
            }
        ...
        if (!grounded(mptr) || (mtmp->wormno && count_wsegs(mtmp) > 5)) {
            if (forcetrap && !Sokoban) {
                ...
                if (in_sight) {
                    seetrap(trap);
                    pline_mon(mtmp,
                             "%s не падает в яму.", Monnam(mtmp));
                }
                return Trap_Effect_Finished;
            }
            if (!inescapable)
                return Trap_Effect_Finished; /* avoids trap */
            fallverb = "затащен"; /* sokoban pit */
        }
        if (!passes_walls(mptr))
            mtmp->mtrapped = 1;
        if (in_sight) {
            pline_mon(mtmp,
                     "%s %s в %s яму!", Monnam(mtmp), fallverb,
                     a_your[trap->madeby_u]);
        ...
        if (!grounded(mptr) || (mtmp->wormno && count_wsegs(mtmp) > 5)
            ...
            if (forcetrap && !Sokoban) {
                ...
                if (in_sight) {
                    ...
                    if (tt == TRAPDOOR)
                        pline_mon(mtmp,
                            "Люк открывается, но %s не проваливается вниз.",
                              mon_nam(mtmp));
                    else /* (tt == HOLE) */
                        pline_mon(mtmp,
                                 "%s не проваливается сквозь дыру.",
                                 Monnam(mtmp));
                }
            ...
            if (inescapable) { /* sokoban hole */
                if (in_sight) {
                    pline_mon(mtmp,
                             "%s кажется, затягивается вниз!", Monnam(mtmp));
                    seetrap(trap);
                }
            } else
                return Trap_Effect_Finished;
    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        if (webmaker(gy.youmonst.data)) {
            if (webmsgok)
                pline(trap->madeby_u ? "Вы проходитесь по своей паутине."
                      : "Эдесь паутина.");
            return Trap_Effect_Finished;
        }
        if (webmsgok) {
            ...
            if (forcetrap || viasitting) {
                Strcpy(verbbuf, "вы схвачены");
            } else if (u.usteed) {
                Sprintf(verbbuf, "тянете %s в",
                        x_monnam(u.usteed, steed_article, "бедную",
                                 SUPPRESS_SADDLE, FALSE));
            } else {
                Sprintf(verbbuf, "%s в", u_locomotion("stumble"));
            }
            You("%s %s паутине!", verbbuf, a_your[trap->madeby_u]);
        }
        ...
        /* Time stuck in the web depends on your/steed's strength. */
        {
            ...
            if (str <= 3)
                ...
                tim = 0;
                if (webmsgok)
                    You("разрываете %s паутину!", a_your[trap->madeby_u] ? "вашу " : "");
                deltrap(trap);
        ...
        case PM_BUGBEAR:
            if (!in_sight) {
                ...
                You_hear("рыкание сбитого с толку медведя!");
                mtmp->mtrapped = 1;
                break;
            }
            FALLTHROUGH;
        ...
        default:
            if (mptr->mlet == S_GIANT
                ...
                pline_mon(mtmp,
                      "%s попадает в %s паутину.", Monnam(mtmp),
                      a_your[trap->madeby_u]);
                seetrap(trap);
            }
        ...
        if (tear_web) {
            if (in_sight)
                pline_mon(mtmp,
                     "%s рвется через %s паутину!", Monnam(mtmp),
                      a_your[trap->madeby_u]);
            deltrap(trap);
            newsym(mtmp->mx, mtmp->my);
            ...
            if (in_sight) {
                pline_mon(mtmp,
                      "%s избегает %s паутины!", Monnam(mtmp),
                      a_your[trap->madeby_u]);
                seetrap(trap);
            }
        }
    ...
    unsigned int trflags UNUSED)
{
    if (wearing_iron_shoes(mtmp)) {
        ...
        if (shoes->spe > 0) {
            ...
            if (mtmp == &gy.youmonst) {
                seetrap(trap);
                pline("Вялое сияние окружает %s.", yname(shoes));
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (Antimagic) {
            ...
            You_feel((dmgval2 >= hp) ? "невыносимо вялым!"
        ...
        if (!resists_magm(mtmp)) { /* lose spell energy */
            if (!mtmp->mcan && (attacktype(mptr, AT_MAGC)
                ...
                if (in_sight) {
                    seetrap(trap);
                    pline_mon(mtmp, "%s кажется вялым.",
                              Monnam(mtmp));
                }
            }
            ...
            if (DEADMONSTER(mtmp))
                monkilled(mtmp,
                          in_sight
                          ? "сдавление антимагическим полем"
                          : (const char *) 0,
                          -AD_MAGM);
            if (DEADMONSTER(mtmp))
                trapkilled = TRUE;
    ...
    unsigned int trflags)
{
    if (mtmp == &gy.youmonst) {
        ...
        seetrap(trap);
        if (viasitting)
            Strcpy(verbbuf, "задеваете"); /* follows "You sit down." */
        else if (u.usteed)
            Sprintf(verbbuf, "тянете %s на",
                    x_monnam(u.usteed, steed_article, (char *) 0,
                             SUPPRESS_SADDLE, FALSE));
        else
            Sprintf(verbbuf, "%s на", u_locomotion("step"));
        You("%s ловушку превращения!", verbbuf);
        if (wearing_iron_shoes(mtmp)) {
            deltrap(trap);
            pline("%s странно коробится.", Yname2(uarmf));
            poly_obj(
            ...
            shieldeff(u.ux, u.uy);
            You_feel("на мгновение иным.");
            /* Trap did nothing; don't remove it --KAA */
        } else {
            (void) steedintrap(trap, (struct obj *) 0);
            deltrap(trap);      /* delete trap before polymorph */
            newsym(u.ux, u.uy); /* get rid of trap symbol */
            You_feel("перемену, находящую на вас.");
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if ((Levitation || Flying) && !forcetrap) {
            if (!already_seen && rn2(3))
                return Trap_Effect_Finished;
            feeltrap(trap);
            pline("%s %s в куче земли под вами.",
                  already_seen ? "Там есть" : "Вы обнаруживаете",
                  trap->madeby_u ? "взрыватель вашей мины" : "взрыватель");
            if (already_seen && rn2(3))
                return Trap_Effect_Finished;
            Soundeffect(se_kaablamm_of_mine, 80);
            pline("БАБАХ!!!  %s %s%s!",
                  forcebungle ? "Ваша неуклюжая попытка взрывает"
                  : "Воздушные потоки взрывают",
                  already_seen ? a_your[trap->madeby_u] : "",
                  already_seen ? " мину" : "её");
        } else {
            ...
            if (recursive_mine)
                return Trap_Effect_Finished;
            feeltrap(trap);
            pline("КААБЛАММ!!!  Вы подорвались на противопехотной мине!", a_your[trap->madeby_u]);
            if (u.usteed)
...
#define MINE_TRIGGER_WT (WT_ELF / 2U)
        ...
        if (m_in_air(mtmp)) {
            ...
            if (in_sight && !already_seen) {
                pline_mon(mtmp,
                     "Под %s в куче земли появляется взводитель.",
                      mon_nam(mtmp));
                seetrap(trap);
            }
            if (rn2(3))
            ...
            if (in_sight) {
                newsym(mtmp->mx, mtmp->my);
                pline("Воздушные потоки сдувают %s!",
                          already_seen ? "Мину" : "Её");
            }
        } else if (in_sight) {
            newsym(mtmp->mx, mtmp->my);
            pline_mon(mtmp,
                  "%s%s подрывается на %s мине!",
                  !Deaf ? "КААБЛАММ!!!  " : "", Monnam(mtmp),
                  a_your[trap->madeby_u] ? "вашей" : "противопехотной");
        }
        if (!in_sight && !Deaf)
            pline("Каабламм!  %s взрыв вдали!",
                  "Вы слышите");  /* Deaf-aware */
        blow_up_landmine(trap);
    ...
    unsigned int trflags UNUSED)
{
    if (mtmp == &gy.youmonst) {
        ...
        feeltrap(trap);
        pline("%sВы задеваете ловушку с катящимся валуном!",
              !Deaf ? "Щелч!  " : "");
        if (!launch_obj(BOULDER, trap->launch.x, trap->launch.y,
            ...
            if (style & LAUNCH_KNOWN)
                pline("Валун не был отпущен.");
            else
                pline("К счастью для вас, валун не был отпущен.");
        }
    } else {
        if (!m_in_air(mtmp)) {
            ...
            newsym(mtmp->mx, mtmp->my);
            if (in_sight)
                pline_mon(mtmp, "%s%s взводит %s.",
                      !Deaf ? "Щелч!  " : "", Monnam(mtmp),
                      trap->tseen ? "ловушку с катящимся валуном" : something);
            if (launch_obj(BOULDER, trap->launch.x, trap->launch.y,
                           trap->launch2.x, trap->launch2.y, style)) {
                if (in_sight)
                    trap->tseen = TRUE;
    ...
    if (mtmp == &gy.youmonst) {
        ...
        if (see_it && !Blind) {
            ...
            if (in_sight) {
                ...
                You_see("странную вибрацию под %s.", buf);
            } else {
                /* notice something (hearing uses a larger threshold
                   for 'nearby') */
                You_see("как земля вибрирует %s.",
                        (mdistu(mtmp) <= 2 * 2)
                           ? "рядом" : "вдали");
            }
        }
...
dotrap(struct trap *trap, unsigned trflags)
{
    ...
    if (Sokoban && (is_pit(ttype) || is_hole(ttype))) {
         ...
         */
        pline("Воздушные потоки тянут вас вниз в яму.",
              a_your[trap->madeby_u],
        ...
        if (floor_trigger(ttype) && check_in_air(&gy.youmonst, trflags)) {
            if (already_seen) {
                You("%s через %s %s.", u_locomotion("step"),
...
trapnote(struct trap *trap, boolean noprefix)
{
    static const char *const tnnames[] = {
        " «до»",   " «ре-бемоль»",  " «ре»",   " «ми-бемоль»",
        " «ми»",   " «фа»",  " «фа-диез»",  " «соль»",
        " «соль-диез»", " «ля»",    " «си-бемоль»",   " «си»",
    };
    static char tnbuf[24]; /* result buffer */
    const char *tn;
    ...
    case SLP_GAS_TRAP:
        if (!resists_sleep(steed) && !breathless(steed->data)
            ...
            if (sleep_monst(steed, rnd(25), -1))
                ...
                pline("%s внезапно засыпает!", Monnam(steed));
        }
        steedhit = TRUE;
        break;
    case LANDMINE:
    ...
    int style)
{
    ...
    case ROLL | LAUNCH_UNSEEN:
        if (otyp == BOULDER) {
            if (cansee(x1, y1)) {
                You_see("%s начинает катиться.", an(xname(singleobj)));
            } else if (Hallucination) {
                Soundeffect(se_someone_bowling, 60);
                You_hear("кто-то катает шары.");
            } else {
                Soundeffect(se_rumbling, 60);
                You_hear("грохот %s.", (distu(x1, y1) <= 4 * 4) ? "рядом"
                                           : "вдали");
            }
        }
        style &= ~LAUNCH_UNSEEN;
    ...
    while (dist-- > 0 && !used_up) {
        ...
        if ((mtmp = m_at(x, y)) != 0) {
            if (otyp == BOULDER && throws_rocks(mtmp->data)) {
                if (rn2(3)) {
                    if (cansee(x, y))
                        pline_mon(mtmp, "%s хватает валун.",
                                  Monnam(mtmp));
        ...
        if (style == ROLL) {
            ...
            if ((t = t_at(x, y)) != 0
                ...
                case LANDMINE:
                    if (rn2(10) > 2) {
                        ...
                        pline("БАБАХ!!!%s",
                              cansee(x, y)
                               ? "  Катящийся валун подрывает мину."
                               : "");
                        deltrap(t);
                        del_engr_at(x, y);
                ...
                case TELEP_TRAP:
                    if (cansee(x, y))
                        pline_xy(x, y,
                                 "Внезапно катящийся валун исчезает!");
                    else if (!Deaf)
                        You_hear("как грохот внезапно прекращается.");
                    singleobj->otrapped = 0;
                ...
                case TRAPDOOR:
                    ...
                    if (flooreffects(singleobj, x2, y2, "падает")) {
                        used_up = TRUE;
                        launch_drop_spot((struct obj *) 0, 0, 0);
                    }
            ...
            if (flooreffects(singleobj, x, y, "падает")) {
                used_up = TRUE;
                launch_drop_spot((struct obj *) 0, 0, 0);
                break;
            }
            if (otyp == BOULDER && (otmp2 = sobj_at(BOULDER, x, y)) != 0) {
                const char *bmsg = " — один валун приводит другой в движение";
                coordxy fx = x + dx, fy = y + dy;

                if (!isok(fx, fy) || !dist || IS_OBSTRUCTED(levl[fx][fy].typ))
                    bmsg = " — один валун ударяется о другой";

                Soundeffect(se_loud_crash, 80);
                You_hear("громкий треск%s!", cansee(x, y) ? bmsg : "");
        ...
        if (otyp == BOULDER && closed_door(x, y)) {
            if (cansee(x, y)) {
                set_msg_xy(x, y);
                pline("Валун с треском пробивает дверь.");
            }
    ...
    if (!trap) {
        ...
        if (!rn2(40) || (is_pit(trap->ttyp) && m_easy_escape_pit(mtmp))) {
            if (sobj_at(BOULDER, mtmp->mx, mtmp->my)
                ...
                if (canseemon(mtmp)) {
                    set_msg_xy(mtmp->mx, mtmp->my);
                    if (is_pit(trap->ttyp))
                        pline("%s выкарабкивается %sиз ямы.", Monnam(mtmp),
                              m_easy_escape_pit(mtmp) ? "легко " : "");
                    else if (trap->ttyp == BEAR_TRAP || trap->ttyp == WEB)
                        pline("%s вырывается из %s.", Monnam(mtmp),
            ...
            if (trap->ttyp == BEAR_TRAP) {
                if (canseemon(mtmp))
                    pline_mon(mtmp, "%s съедает медвежий капкан!",
                              Monnam(mtmp));
                deltrap(trap);
                mtmp->meating = 5;
                mtmp->mtrapped = 0;
                ...
                if (canseemon(mtmp))
                    pline_mon(mtmp, "%s хрумкает несколько шипов!",
                              Monnam(mtmp));
                trap->ttyp = PIT;
                mtmp->meating = 5;
            }
        ...
        if (!DEADMONSTER(mtmp) && mtmp->mtrapped) {
            ...
            if (!alreadyspotted && canseemon(mtmp))
                pline_mon(mtmp, "%s появляется.", Amonnam(mtmp));
        }
    }
    return trap_result;
}
...
instapetrify(const char *str)
{
    ...
    if (poly_when_stoned(gy.youmonst.data) && polymon(PM_STONE_GOLEM))
        return;
    urgent_pline("Вы превращаетесь в камень...");
...
minstapetrify(struct monst *mon, boolean byplayer)
{
    ...
    if (cansee(mon->mx, mon->my))
        pline_mon(mon, "%s превращается в камень.", Monnam(mon));
    if (byplayer) {
        gs.stoned = TRUE;
...
selftouch(const char *arg)
{
    ...
    if (uwep && uwep->otyp == CORPSE && touch_petrifies(&mons[uwep->corpsenm])
        ...
        pline("%s касаетесь трупа %s.", arg, corpse_pmname);
        Sprintf(kbuf, "труп %s", an(corpse_pmname));
        instapetrify(kbuf);
    ...
    boolean byplayer)
{
    ...
    if (mwep && mwep->otyp == CORPSE && touch_petrifies(&mons[mwep->corpsenm])
        && !resists_ston(mon)) {
        if (cansee(mon->mx, mon->my)) {
            pline_mon(mon, "%s%s касается %s.", arg ? arg : "",
...
float_up(void)
{
    ...
    if (u.utrap) {
        if (u.utraptype == TT_PIT) {
            reset_utrap(FALSE);
            You("взлетаете вверх, из %s!", trapname(PIT, FALSE));
            gv.vision_full_recalc = 1; /* vision limits change */
            fill_pit(u.ux, u.uy);
        } else if (u.utraptype == TT_LAVA /* molten lava */
                   || u.utraptype == TT_INFLOOR) { /* solidified lava */
            Your("тело тянет вверх, но ваши %s всё ещё застряли.",
            ...
            You("чувствуете себя легче, но ваша %s всё ещё прикована к %s.",
                body_part(LEG),
                IS_ROOM(levl[cc.x][cc.y].typ) ? "полу" : "земле");
        } else if (u.utraptype == WEB) {
            You("приподнимаетесь, но всё ещё увязли в %s.",
                trapname(WEB, FALSE));
        } else { /* bear trap */
            You("всплываете немного, но ваша %s всё ещё застряла.",
                body_part(LEG));
        }
        /* when still trapped, float_vs_flight() below will block levitation */
#if 0
    } else if (Is_waterlevel(&u.uz)) {
        pline("Похоже, вы потеряли немного веса.");
#endif
    } else if (u.uinwater) {
        spoteffects(TRUE);
    } else if (u.uswallow) {
        ...
        if (is_animal(u.ustuck->data))
            You("всплываете в %s.", surface_ru(u.ux, u.uy));
        else
            You("ввинчиваетесь вверх внутрь %s.", mon_nam(u.ustuck));
    } else if (Hallucination) {
        pline("Вверх, вверх, и аааа!  Вы идёте по воздуху!");
    } else if (Is_airlevel(&u.uz)) {
        You("обретаете контроль над своими движениями.");
    } else {
        You("начинаете парить в воздухе!");
    }
    if (u.usteed && !is_floater(u.usteed->data)
        && !is_flyer(u.usteed->data)) {
        if (Lev_at_will) {
            pline("%s магически взлетает вверх!", Monnam(u.usteed));
        } else {
            You("не можете удержаться на %s.", mon_nam(u.usteed));
            dismount_steed(DISMOUNT_GENERIC);
        }
    }
    if (Flying)
        You("больше не можете контролировать свой полет.");
...
fill_pit(coordxy x, coordxy y)
{
    ...
    if ((t = t_at(x, y)) != 0 && (is_pit(t->ttyp) || is_hole(t->ttyp))
        ...
        obj_extract_self(otmp);
        (void) flooreffects(otmp, x, y, "оседает");
    }
}
    ...
    long emask) /* might cancel timeout */
{
    ...
    if (BLevitation) {
        ...
        if (trapped && u.utrap) /* u.utrap => paranoia */
            You("больше не пытаетесь всплыть из %s.",
                (u.utraptype == TT_BEARTRAP) ? "челюстей капкана"
                  : (u.utraptype == TT_WEB) ? "паутины"
                      : (u.utraptype == TT_BURIEDBALL) ? "цепи"
                          : (u.utraptype == TT_LAVA) ? "лавы"
                              : "земли"); /* TT_INFLOOR */
    ...
    if (BFlying) {
        ...
        if (Flying) {
            You("перестали левитировать и теперь летите.");
            encumber_msg(); /* carrying capacity might have changed */
            return 1;
        }
    }
    if (u.uswallow) {
        You("опускаетесь, но всё ещё %s.",
            digests(u.ustuck->data) ? "проглочены" : "поглощены");
        encumber_msg();
        return 1;
    }
    ...
    if (!Flying) {
        if (!u.uswallow && u.ustuck) {
            if (sticks(gy.youmonst.data))
                You("не можете удержать захват за %s.",
                    mon_nam(u.ustuck));
            else
                pline("Испугавшись, %s больше не может удерживать вас!",
                      mon_nam(u.ustuck));
    ...
    if (!trap) {
        trap = t_at(u.ux, u.uy);
        if (Is_airlevel(&u.uz)) {
            You("начинаете кувыркаться на месте.");
        } else if (Is_waterlevel(&u.uz) && !no_msg) {
            You_feel("становитесь тяжелее.");
            ...
            if (!(emask & W_SADDLE)) {
                if (Sokoban && trap) {
                    ...
                    selftouch("Падая, вы");
                                        ...
                                        || is_flyer(u.usteed->data))) {
                    You("усаживаетесь в седле более уверенно.");
                } else if (Hallucination) {
                    pline("Конец!  Вы %s.",
                          is_pool(u.ux, u.uy)
                             ? "вплеснулись "
                    ...
                    You("мягко опускаетесь на %s.", surface_ru(u.ux, u.uy));
                }
            }
        }
    }
...
climb_pit(void)
{
    ...
    if (Passes_walls) {
        ...
        You("поднимаетесь из %s.", pitname);
        reset_utrap(FALSE);
        fill_pit(u.ux, u.uy);
        gv.vision_full_recalc = 1; /* vision limits change */
    } else if (!rn2(2) && sobj_at(BOULDER, u.ux, u.uy)) {
        Your("%s застревает в расщелине.", body_part(LEG));
        display_nhwindow(WIN_MESSAGE, FALSE);
        clear_nhwindow(WIN_MESSAGE);
        You("освобождаете свой %s.", body_part(LEG));
        ...
        You("%s из %s.", u_locomotion("climb"), pitname);
        reset_utrap(FALSE);
        fill_pit(u.ux, u.uy);
        gv.vision_full_recalc = 1; /* vision limits change */
        ...
        You("%s к краю %s.",
                        (Sokoban && Levitation)
                        ? "изо всех сил боретесь с воздушными потоками и плывёте"
                        : u.usteed ? "едете верхом" : "ползёте",
                    pitname);
        fill_pit(u.ux, u.uy);
        gv.vision_full_recalc = 1; /* vision limits change */
        ...
        if (u.usteed)
            Norep("%s всё ещё в яме.", YMonnam(u.usteed));
        else
    ...
    struct obj *box) /* null for floor trap */
{
    ...
    if ((box && !carried(box)) ? is_pool(box->ox, box->oy) : Underwater) {
        pline("Каскад парящих пузырей вырывается из %s!",
              the(box ? xname(box) : surface(u.ux, u.uy)));
        if (Fire_resistance)
            You("невредимы.");
        else
            losehp(rnd(3), "кипящей воды", KILLED_BY);
        return;
    }
    pline("%s %s из %s!", tower_of_flame, box ? "вспыхивает" : "вырывается",
          the(box ? xname(box) : surface(u.ux, u.uy)));
    if (Fire_resistance) {
        shieldeff(u.ux, u.uy);
    ...
    if (!box && burn_floor_objects(u.ux, u.uy, see_it, TRUE) && !see_it)
        You("чувствуете запах горящей бумаги.");
    if (is_ice(u.ux, u.uy))
...
domagictrap(void)
{
    ...
    if (fate < 10) {
        ...
        if (!resists_blnd(&gy.youmonst)) {
            You("на мгновение ослеплены вспышкой света!");
            make_blinded((long) rn1(5, 10), FALSE);
            if (!Blind)
                Your1(vision_clears);
        } else if (!Blind) {
            You_see("вспышку света!");
        }

        /* deafness effects */
        if (!Deaf) {
            Soundeffect(se_deafening_roar_atmospheric, 100);
            You_hear("оглушительный рев!");
            incr_itimeout(&HDeaf, rn1(20, 30));
            disp.botl = TRUE;
        } else {
            /* magic vibrations still hit you */
            You_feel("раздражение.");
        ...
        case 11: /* toggle intrinsic invisibility */
            ...
            if (!Invis) {
                ...
                if (!Blind) {
                    if (!See_invisible)
                        You("снова видите себя!");
                    else
                        You_cant("больше видеть себя насквозь.");
                }
            } else {
                /* If we're invisible from another source */
                You_feel("сейчас немного более %s.",
                         HInvis ? "заметным" : "скрытым");
            }
        ...
        /* odd feelings */
        case 13:
            pline("Дрожь пробегает по вашей %s!", body_part(SPINE));
            break;
        case 14:
            You_hear(Hallucination ? "как луна воет на вас."
                                   : "далекий вой.");
            break;
        case 15:
            if (on_level(&u.uz, &qstart_level))
                ...
                You("внезапно тоскуете по %s.",
                    Hallucination
                        ? "Кливленду"
                        : (In_quest(&u.uz) || at_dgn_entrance("Квест"))
                              ? "вашей ближней родине"
                              : "вашей далекой родине");
            break;
        case 16:
            Your("рюкзак сильно трясётся!");
            break;
        case 17:
            You(Hallucination ? "чувствуете запах гамбургеров." : "чувствуете запах обгоревшей плоти.");
            break;
        case 18:
            You_feel("усталость.");
            break;
    ...
    coordxy x, coordxy y) /* where to place contents of burned up container */
{
    ...
    if (Is_container(obj) || obj->otyp == STATUE) {
        ...
        if (in_sight)
            pline("%s воспламеняется и сгорает.", Yname2(obj));
        if (Has_contents(obj)) {
            if (in_sight)
                pline("Его содержимое выпадает.");
        ...
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
            if (in_sight)
                pline("Дым поднимается из %s.", the(xname(obj)));
            return FALSE;
        }
    ...
    if (objects[otyp].oc_material < DRAGON_HIDE
        ...
        if (cansee(x, y)) {
            ...
            if (obj == gt.thrownobj || obj == gk.kickedobj)
                pline("%s сгорает!", is_plural(obj) ? "Они" : "Оно");
            else
                You_see("%s: %s ударяется о лаву и сгорает!", doname(obj));
        }
    ...
    boolean described)
{
    ...
    if (described) {
        ...
        pline("%s%s!",
                bufp,
                !exploded ? (one ? " взрывается!" : " взрываются!")
                          : (one ? " взрывается ещё раз!" : " взрываются ещё раз!"));
    }
    if (ga.acid_ctx.ctx_valid) {
        if (obj->dknown)
    ...
    boolean force)    /* if True, skip luck-based protection check */
{
    ...
    if (obj->otyp == CAN_OF_GREASE && obj->spe > 0) {
        ...
        if (!rn2(2)) {
            obj->greased = 0;
            if (in_invent) {
                pline("Жир на %s смывается.", yname(obj));
        ...
        if (in_invent) {
            pline("Немного %s попадает в ваш %s!", "воды", ostr);
        ...
        if (in_invent && !Blind && !Underwater) {
            pline("%s Не может попасть в ваш %s.", "Вода", ostr);
            gm.mentioned_water = !Hallucination;
        ...
        if (in_invent)
            Your("%s %s.", ostr, vtense(ostr, "меркнет"));
        ...
        if (otyp == SPE_BOOK_OF_THE_DEAD) {
            ...
            if (isok(ox, oy) && cansee(ox, oy))
                pline("Пар поднимается из %s.", the(xname(obj)));
            return 0;
        ...
        if (obj->otyp == POT_ACID) {
            ...
            if (in_invent)
                Your("%s %s ещё.", ostr, vtense(ostr, "разжижается"));
            ...
            if (in_invent)
                Your("%s %s.", ostr, vtense(ostr, "разжижается"));
...
{
    const char *surf = surface(u.ux, u.uy), *you_are_back;

    if (is_ice(u.ux, u.uy)) {
        /* "on ice" */
        surf = "лёд";
    } else if (!strcmpi(surf, "пол") || !strcmpi(surf, "земля")) {
        /* "on solid ground" */
        surf = "твёрдую землю";
    } else if (!strcmpi(surf, "мост") || !strcmpi(surf, "алтарь")
               || !strcmpi(surf, "надгробие")) {
        /* "on a bridge" */
        surf = "мост";
    } else if (!strcmpi(surf, "лестница") || !strcmpi(surf, "лава")
               || !strcmpi(surf, "дно")) {
        /* "on the stairs" */
        surf = "лестницу";
    } else {
    ...
    if (rescued) {
        you_are_back = "Вы обнаруживаете себя";
    } else {
        you_are_back = flags.verbose ? "Вы вернулись" : "Назад";
    }
    pline("%s на %s.", you_are_back, surf);
...
{
    static const char find_yourself[] = "обнаруживаете себя";
        ...
        if (is_pool(u.ux, u.uy)) {
            You("%s %s %s.", find_yourself,
                (Is_waterlevel(&u.uz) || IS_WATERWALL(lev->typ))
                  ? "в толще" : "на поверхности",
                hliquid("воды"));
            mesggiven = TRUE;
        } else if (IS_AIR(lev->typ)) {
            You("%s в %s.", find_yourself,
                Is_waterlevel(&u.uz) ? "воздушном пузыре" : "воздухе");
            mesggiven = TRUE;
        }
        break;
    ...
    case DISSOLVED: /* sunk into lava while fire resistant */
        if (is_pool(u.ux, u.uy)) {
            You("%s %s %s.", find_yourself,
                u.uinwater ? "в" : "на", hliquid("воды"));
            mesggiven = TRUE;
        } else if (is_lava(u.ux, u.uy)) {
            You("%s на поверхности %s.", find_yourself, hliquid("расплавленной лавы"));
            mesggiven = TRUE;
        }
        break;
    default:
...
drown(void)
{
    ...
    if (!u.uinwater) {
        You("%s в %s%c", is_solid ? "ныряете" : "падаете",
            waterbody_acc(u.ux, u.uy),
            (Amphibious || Swimming || Breathless) ? '.' : '!');
        if (!Swimming && !is_solid)
            You("тонете как %s.", Hallucination ? "Титаник" : "камень");
    }
    ...
    if ((i = number_leashed()) > 0) {
        pline("Поводок%s скользит%s.", (i > 1) ? "и" : "",
                              (i > 1) ? "ят" : "");
        unleash_all();
    }
    ...
    if (Amphibious || Breathless || Swimming) {
        if (Amphibious || Breathless) {
            if (flags.verbose)
                pline("Но вы не тонете.");
            if (!Is_waterlevel(&u.uz)) {
                if (Hallucination)
                    Your("киль ударяется о дно.");
                else
                    You("касаетесь дна.");
            }
        }
    ...
    if ((Teleportation || can_teleport(gy.youmonst.data)) && !Unaware
        ...
        You("пытаетесь применить заклинание телепортации."); /* utcsri!carroll */
        if (!noteleport_level(&gy.youmonst)) {
            (void) dotele(FALSE);
            if (!is_pool(u.ux, u.uy))
                return TRUE;
        } else
            pline("Попытка заклинания телепорта проваливается.");
    }
    if (u.usteed) {
        dismount_steed(DISMOUNT_GENERIC);
        if (!is_pool(u.ux, u.uy))
    ...
    if (u.usleep)
        unmul("Внезапно вы просыпаетесь!");
    ...
    if (gm.multi >= 0 && gy.youmonst.data->mmove
        ...
        You("пытаетесь выбраться из %s.", hliquid("воды"));
        if (lost)
            You("выбрасываете часть снаряжения, чтобы похудеть...");
        if (succ) {
            pline("Фух!  Это было близко.");
    ...
    for (i = 0; i < 2; i++) {
        ...
        pool_of_water = waterbody_acc(u.ux, u.uy);
        svk.killer.format = KILLED_BY;
...
could_untrap(boolean verbosely, boolean check_floor)
{
    ...
    if (near_capacity() >= HVY_ENCUMBER) {
        Strcpy(buf, "Вы слишком перенапряжены, чтобы сделать это.");
    } else if ((nohands(gy.youmonst.data) && !webmaker(gy.youmonst.data))
               || !gy.youmonst.data->mmove) {
        Strcpy(buf, "И как же вы намереваетесь это сделать?");
    } else if (u.ustuck && sticks(gy.youmonst.data)) {
        Sprintf(buf, "Вам придётся сначала отпустить %s.", mon_nam(u.ustuck));
    } else if (u.ustuck || (welded(uwep) && bimanual(uwep))) {
        Sprintf(buf, "Ваши %s слишком заняты для этого.",
        ...
        Sprintf(buf, "Вы не можете достать до пола.");
    }
    if (buf[0]) {
        if (verbosely)
            pline("%s", buf);
...
move_into_trap(struct trap *ttmp)
{
    ...
    if (test_move(u.ux, u.uy, sgn(x - u.ux), sgn(y - u.uy), TEST_MOVE)
        ...
        pline("К счастью, вы не двигаетесь %s него.",
              into_vs_onto(ttmp->ttyp) ? "в" : "в");
    }
}
    ...
    boolean force_failure)
{
    ...
    if (u.dx && u.dy && bad_rock(gy.youmonst.data, u.ux, ttmp->ty)
        ...
        if ((gi.invent && (inv_weight() + weight_cap() > WT_TOOMUCH_DIAGONAL))
            ...
            You("не можете дотянуться до %s!", trapname(ttype, FALSE));
            return 0;
        }
    }
    ...
    if (force_failure || untrap_prob(ttmp)) {
        if (rnl(5)) {
            ...
            if (mtmp) { /* must be a trap that holds monsters */
                if (ttype == BEAR_TRAP) {
                    ...
                    if (!webmaker(gy.youmonst.data)
                        ...
                        pline("Паутина липнет к вам.  Вы тоже попались!");
                    ...
                    if (mtmp->mtrapped)
                        pline("%s остается опутанным.", Monnam(mtmp));
                }
            } else if (under_u) {
            ...
            pline("%s %s трудно %s.",
                  ttmp->madeby_u ? "Ваш" : under_u ? "Этот" : "Тот",
                  trapname(ttype, FALSE),
                  (ttype == WEB) ? "снять" : "обезвредить");
        }
        return 1;
    }
    return 2;
...
reward_untrap(struct trap *ttmp, struct monst *mtmp)
{
    if (!ttmp->madeby_u) {
        if (rnl(10) < 8 && !mtmp->mpeaceful && !helpless(mtmp)
            ...
            pline("%s благодарен.", Monnam(mtmp));
        }
        ...
        if (!rn2(3) && !rnl(8) && u.ualign.type == A_LAWFUL) {
            adjalign(1);
            You_feel("что поступили правильно.");
        }
    }
}
...
disarm_holdingtrap(struct trap *ttmp)
{
    ...
    if ((mtmp = m_at(ttmp->tx, ttmp->ty)) != 0) {
        mtmp->mtrapped = 0;
        You("вытаскиваете %s из %s.", mon_nam(mtmp),
                (ttmp->ttyp == BEAR_TRAP) ? "капкана" : "паутины");
        reward_untrap(ttmp, mtmp);
    } else if (ttmp->ttyp == BEAR_TRAP) {
        You("обезвреживаете капкан.");
        ...
        if (wep && wep->oartifact
            ...
            pline("%s %s %s паутину!", bare_artifactname(uwep),
                      u_wield_art(ART_STING) ? "прорезает" : "прожигает", which);
        else if (wep)
            You("прорезаете %s паутину.", which);
        else
            You("успешно снимаете %s паутину.", which);
        deltrap(ttmp);
    }
...
disarm_landmine(struct trap *ttmp) /* Helge Hafting */
{
    ...
    if (fails < 2)
        return fails;
    You("обезвреживаете противопехотную мину.");
...
disarm_squeaky_board(struct trap *ttmp)
{
    ...
    obj = getobj("чем снять ловушку", unsqueak_ok, GETOBJ_PROMPT);
    if (!obj)
        return 0;
    ...
    You("чините скрипучую доску."); /* no madeby_u */
    deltrap(ttmp);
    newsym(u.ux + u.dx, u.uy + u.dy);
    ...
    boolean stuff) /* False: monster w/o minvent; True: w/ minvent */
{
    if (calc_capacity(xtra_wt) >= HVY_ENCUMBER) {
        pline("%s слишком %s, чтобы вы могли поднять его.", Monnam(mtmp),
              stuff ? "много несет" : "тяжел");
        if (!ttmp->madeby_u && !mtmp->mpeaceful && mtmp->mcanmove
            && !mindless(mtmp->data) && mtmp->data->mlet != S_HUMAN
            && rnl(10) < 3) {
            mtmp->mpeaceful = 1;
            set_malign(mtmp); /* reset alignment */
            pline("%s считает, что вы мило поступили, попытавшись.", Monnam(mtmp));
        }
        return 0;
    }
    return 1;
    ...
    struct trap *ttmp)
{
     ...
     */
    if (!mtmp->mtrapped) {
        pline("%s не в ловушке.", Monnam(mtmp));
        return 0;
    }
    ...
    if ((uprob = untrap_prob(ttmp)) != 0 && !helpless(mtmp)) {
        You("пытаетесь протянуть свои %s, но %s скептически отшатывается.",
    ...
    if (touch_petrifies(mtmp->data) && !uarmg && !Stone_resistance) {
        ...
        You("хватаете застрявшего %s голыми %s.",
        ...
        if (poly_when_stoned(gy.youmonst.data) && polymon(PM_STONE_GOLEM)) {
            ...
            Sprintf(kbuf, "пытаясь помочь %s выбраться из ямы", an(mtmp_pmname));
            instapetrify(kbuf);
            return 1;
        }
    }
    ...
    if (uprob) {
        You("пытаетесь схватить %s, но не можете крепко ухватить его.", mon_nam(mtmp));
        if (mtmp->msleeping) {
            mtmp->msleeping = 0;
            pline("%s просыпается.", Monnam(mtmp));
        }
        return 1;
    }

    You("протягиваете свои %s и хватаете %s.", makeplural(body_part(ARM)),
        mon_nam(mtmp));
    ...
    You("вытаскиваете %s из ямы.", mon_nam(mtmp));
    mtmp->mtrapped = 0;
    reward_untrap(ttmp, mtmp);
    fill_pit(mtmp->mx, mtmp->my);
    return 1;
...
disarm_box(struct obj *box, boolean force, boolean confused)
{
    if (box->otrapped) {
        ...
        pline("%s не был заловушкон.", xname(box));
        box->tknown = 0;
    }
}
    ...
    boolean confused)
{
    if ((box->otrapped
        ...
        if (!(box->tknown && box->dknown))
            You("находите ловушку на %s!", the(xname(box)));
        else
            pline("На %s есть ловушка.", the(xname(box)));
        box->tknown = 1;
        observe_object(box);
        if (!confused)
            exercise(A_WIS, TRUE);

        if (ynq("Обезвредить?") == 'y')
            disarm_box(box, force, confused);
    } else {
        You("не находите ловушек на %s.", the(xname(box)));
    }
}
    ...
    struct obj *container)
{
    ...
    if (!isok(x, y)) {
        pline("Опасности, таящиеся там, вне вашей досягаемости.");
        return 0;
    }
    ...
    if (autounlock_door) {
        ...
        *the_trap = '\0';
        ...
        if (ttmp && boxcnt)
            Strcat(the_trap, " и ");
        if (boxcnt)
            Strcat(the_trap, (boxcnt == 1) ? "контейнер" : "контейнеры");
        useplural = ((ttmp && boxcnt > 0) || boxcnt > 1);
        /* note: boxcnt and useplural will always be 0 for !here case */
        if (ttmp || boxcnt)
            There("%s %s %s, но вы не можете дотянуться до %s%s.",
                  useplural ? "находятся" : "находится", the_trap,
                  here ? "здесь" : "там",
                  useplural ? "них" : "него",
                  u.usteed ? ", сидя верхом" : "");
        trap_skipped = (ttmp != 0);
        ...
        if (ttmp) {
            ...
            if (boxcnt) {
                if (is_pit(ttmp->ttyp)) {
                    You_cant("многого сделать с %s%s.", the_trap,
                             u.utrap ? ", в которой вы застряли"
                                     : ", стоя на краю её");
                    trap_skipped = TRUE;
                    deal_with_floor_trap = FALSE;
                } else {
                    Snprintf(qbuf, sizeof(qbuf),
                             "Здесь есть %s и %s.  %s %s?",
                             (boxcnt == 1) ? "контейнер"
                                           : "контейнеры",
                             an(trapdescr),
                             (ttmp->ttyp == WEB) ? "Снять"
            ...
            if (deal_with_floor_trap) {
                if (u.utrap) {
                    You("не можете справиться с %s, пока вы сами в ловушке!",
                            the_trap);
                    return 1;
                }
                if ((mtmp = m_at(x, y)) != 0
                ...
                case SPIKED_PIT:
                    ...
                    if (!mtmp) {
                        pline("Попробуйте вместо этого засыпать яму.");
                        return 0;
                    }
                    return help_monster_out(mtmp, ttmp);
                default:
                    You("не можете обезвредить %s ловушку.", !here ? "ту" : "эту");
                    return 0;
                }
            }
        } /* end if */
        ...
        if (boxcnt) {
            ...
            for (otmp = svl.level.objects[x][y]; otmp; otmp = otmp->nexthere) {
                ...
                if (otmp->tknown && otmp->dknown)
                    (void) safe_qbuf(qbuf, "Обезвредить этот ", NULL,
                                     otmp, xname, ansimpleoname, "ящик");
                else
                    (void) safe_qbuf(qbuf, "Здесь есть ",
                                     ".  Проверить его на ловушки?", otmp,
                                     doname, ansimpleoname, "ящик");
                switch (ynq(qbuf)) {
                    case 'q':
                        return 0;
                    case 'y':
            ...
            There("нет здесь других сундуков и ящиков.");
        }
    ...
    if (!IS_DOOR(levl[x][y].typ)) {
        if (!trap_skipped)
            You("не знаете о ловушках там.");
        return 0;
    }

    switch (levl[x][y].doormask) {
    case D_NODOOR:
        You("не %s там двери.", Blind ? "чувствуете" : "видите");
        return 0;
    case D_ISOPEN:
        pline("Эта дверь безопасно открыта.");
        return 0;
    case D_BROKEN:
    ...
    if (((levl[x][y].doormask & D_TRAPPED) != 0
        ...
        if (levl[x][y].doormask & D_TRAPPED) {
            ...
            pline("Эта дверь не была заловушкована.");
        return 1;
    } else {
        You("не находите ловушек на двери.");
        return 1;
    }
}
                       ...
                       * otherwise left with its previous value intact */
{
    ...
    if (ishero) {
        ...
        *noticed = TRUE;
        if (!u.usteed)
            Strcpy(buf, "Вы освобождены");
        else if (u.utraptype == TT_BURIEDBALL)
            Sprintf(buf, "Вы и %s освобождены", y_monnam(u.usteed));
        else
            Sprintf(buf, "%s освобожден", noit_Monnam(u.usteed));
        /* give release message before untrap in case it triggers a message */
        pline("%s от %s%s.", buf, which, trapdescr);
        ...
        if (canspotmon(mon)) {
            *noticed = TRUE;
            pline("%s освобожден от %s%s.", Monnam(mon), which,
                  trapdescr);
        } else if (cansee(t->tx, t->ty) && t->tseen) {
            *noticed = TRUE;
            if (t->ttyp == WEB)
                pline("%s освобожден от %s%s.", Something, which,
                      trapdescr);
            else /* BEAR_TRAP */
                pline("%s%s открывается.", upstart(strcpy(buf, which)), trapdescr);
        }
    ...
    boolean disarm)
{
    ...
    You(disarm ? "взводите её!" : "задеваете ловушку!");
    display_nhwindow(WIN_MESSAGE, FALSE);
    if (Luck > -13 && rn2(13 + Luck) > 7) { /* saved by luck */
        ...
        case 12:
        case 11:
            msg = "взрывчатка не сработала";
            break;
        case 10:
        case 9:
            msg = "электрический заряд заземлился";
            break;
        case 8:
        case 7:
            msg = "пламя гаснет";
        ...
        case 5:
        case 4:
            msg = "отравленная игла промахивается";
            break;
        case 3:
        case 2:
        case 1:
        case 0:
            msg = "газовое облако рассеивается";
            break;
        default:
        ...
        case 21: {
            ...
            pline("%s!", Tobjnam(obj, "взрывается"));
            Sprintf(buf, "взрывом %s", xname(obj));
            ...
            if (costly && loss) {
                if (insider) {
                    You("должны %ld %s за уничтоженные предметы.", loss,
                        currency(loss));
                } else {
                    You("причинили ущерб на %ld %s!", loss,
                        currency(loss));
        ...
        case 18:
        case 17:
            pline("Облако ядовитого газа валит из %s.", the(xname(obj)));
            if (rn2(3))
                poisoned("облако ядовитого газа", A_STR, "облако ядовитого газа", 15,
                         FALSE);
            else
        ...
        case 14:
        case 13:
            You_feel("укол иглы в вашу %s.", body_part(bodypart));
            poisoned("игла", A_CON, "отравленная игла", 10, FALSE);
            exercise(A_CON, FALSE);
            break;
        case 12:
        case 11:
        ...
        case 6: {
            ...
            You("получаете удар разрядом электричества!");
            if (Shock_resistance) {
                shieldeff(u.ux, u.uy);
                You("похоже, не пострадали.");
        ...
        case 3:
            if (!Free_action) {
                pline("Вдруг вы замираете на месте!");
                nomul(-d(5, 6));
                gm.multi_reason = "заморожены ловушкой";
                exercise(A_DEX, FALSE);
                gn.nomovemsg = You_can_move_again;
            } else
                You("на мгновение застываете.");
            break;
        case 2:
        case 1:
        case 0:
            pline("Облако %s газа валит из %s.",
                  Blind ? ROLL_FROM(blindgas) : rndcolor(),
                  the(xname(obj)));
            if (!Stunned) {
                if (Hallucination)
                    pline("Какое классное ощущение!");
                else
                    You("%s%s...", stagger(gy.youmonst.data, "шатаетесь"),
                        Halluc_resistance ? ""
                                          : Blind ? " и кружится голова"
    ...
    boolean nocorpse)  /* True: a trap is completely burning up the target */
{
    ...
    if (!strike) {
        if (obj && cansee(mon->mx, mon->my))
            pline_mon(mon, "%s чуть не попадает под %s!",
        ...
        if (obj && cansee(mon->mx, mon->my))
            pline_mon(mon, "%s поражён %s%s",
                      Monnam(mon), doname(obj),
                      harmless ? ", но не пострадал." : "!");
        if (d_override) {
            dam = d_override;
        } else if (obj) {
            dam = dmgval(obj, mon);
...
unconscious(void)
{
    ...
    return (u.usleep
            || (gn.nomovemsg
                && (!strncmp(gn.nomovemsg, "Вы прос", 7)
                    || !strncmp(gn.nomovemsg, "Вы приходите в ", 15)
                    || !strncmp(gn.nomovemsg, "Вы снова в со", 13))));
}

static const char lava_killer[] = "расплавленной лаве";
...
lava_effects(void)
{
    ...
    if (uarmf && (uarmf->in_use
        ...
        obj = uarmf;
        pline("%s!", Yobjnam2(obj, "вспыхивает"));
        ++burnmesgcount;
    ...
    if (!Fire_resistance) {
        if (Wwalking) {
            pline("Здесь %s обжигает вас!", "лава");
            if (usurvive) {
                losehp(dmg, lava_killer, KILLED_BY); /* lava damage */
                goto burn_stuff;
            }
        } else
            You("падаете в %s!", waterbody_acc(u.ux, u.uy));
        ...
        for (obj = gi.invent; obj; obj = obj2) {
            ...
            if (obj->o_id == protect_oid) {
                ...
                if (usurvive && !Blind)
                    pline("%s испускает странное %s свечение, но остаётся неповреждённым.",
                          The(xname(obj)), "тёмно-красное");
            } else if (obj->in_use) {
                if (obj->owornmask) {
                    if (usurvive) {
                        pline("%s!", Yobjnam2(obj, "вспыхивает"));
        ...
        if (usurvive && burncount > burnmesgcount)
            pline("%s предмет%s в вашем инвентаре %s уничтожен.",
                  (burnmesgcount > 0)
                    ? ((burncount - burnmesgcount == 1) ? "Еще один" : "Другой")
                    : ((burncount == 1) ? "Один" : "Несколько"),
        ...
        for (burncount = 0; burncount < 2; ++burncount) {
            ...
            urgent_pline("Вы %s...", boil_away ? "испаряетесь"
                                                : "сгораете дотла");
            done(BURNING);
            if (safe_teleds(TELEDS_ALLOW_DRAG | TELEDS_TELEPORT))
                break; /* successful life-save */
            /* nowhere safe to land; repeat burning loop */
            pline("Вы все еще горите.");
        }
        ...
        set_utrap((unsigned) (rn1(4, 4) + ((boil_away ? 2
                                                      : rn1(4, 12)) << 8)),
                  TT_LAVA);
        You("уходите под %s%s!", waterbody_acc(u.ux, u.uy),
            !boil_away ? ", но пламя лишь слегка обжигает"
                       : " и вот-вот сгорите заживо");
        if (Fire_resistance)
            monstseesu(M_SEEN_FIRE);
        else
            monstunseesu(M_SEEN_FIRE);
...
sink_into_lava(void)
{
    static const char sink_deeper[] = "Вы глубже погружаетесь в лаву.";
    ...
    if (!u.utrap || u.utraptype != TT_LAVA) {
        ...
        if (u.utrap < (1 << 8)) {
            svk.killer.format = KILLED_BY;
            Strcpy(svk.killer.name, lava_killer);
            urgent_pline("Вы погружаетесь ниже поверхности и умираете.");
    ...
    boolean override) /* if True, ignore Hallucination */
{
    static const char *const halu_trapnames[] = {
        ...
        "бездонная яма", "ловушка полиморфа", "дьявольский телепорт",
        "падающий валун", "поле анти-антимагии", "плачущая газовая ловушка",
        "тошнотворная доска", "наэлектризованная паутина", "ловушка совомеда",
        "песчаная мина", "колеблющийся треугольник",
        /* some traps found in nethack variants */
        "смертельная ловушка", "ловушка дезинтеграции", "ледяная ловушка",
        "монохромная ловушка",
        /* plausible real-life traps */
        "ловушка-лезвие топора", "бассейн кипящего масла", "зыбучие пески",
        "поле калтропов", "циркулярная пила", "шипастый пол",
        "вращающаяся стена", "неровный пол", "ловушка-палец",
        "чёртик из коробочки", "жёлтый снег", "ловушка-обманка",
        "крысоловка", "отравленный гвоздь", "силок", "водоворот",
        "растяжка", "мотель для тараканов (tm)",
        /* sci-fi */
        "негативное пространство", "тензорное поле", "сингулярность",
        "имперский флот", "чёрная дыра", "термальный детонатор",
        "горизонт событий", "энтоптический феномен",
        /* miscellaneous suggestions */
        "благоухающая газовая отдушина", "телефонная будка",
        "взрывающиеся руны", "бесконечный лифт", "слизевая яма",
        "зона искривления", "иллюзорный пол", "куча дерьма",
        "медовая ловушка", "ловушка для туристов",
        "банановая кожура", "садовые грабли", "пищалка-подушка",
        "ловушка из коробки и палки", "ловушка для мух",
        "юридическая ловушка", "яма со змеями", "ловушка для головастиков",
        "скользкий склон", "ловушка жажды", "солнечная ловушка",
    };
    ...
    if (Hallucination && !override) {
        ...
        if (nameidx == total_names) {
            ...
            copynchars(roletrap,
                       ...
                       (int) (sizeof roletrap - sizeof " ловушка"));
            Strcat(roletrap, " ловушка");
