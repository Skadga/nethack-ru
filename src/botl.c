...
/* Russian display strings for cap; enc_stat[] stays English above so the
   hilite_status config parser keeps accepting English keywords. The config
   parser also accepts the Russian names (roundtrip after save). */
const char *const enc_stat_ru[] = {
    "",         "Навьючен",  "Перегружен",
    "Перенапряжён", "Сверхгружен", "Перегрузка"
};
...
do_statusline1(void)
{
    ...
    Sprintf(nb = eos(nb), "St:%s Dx:%-1d Co:%-1d In:%-1d Wi:%-1d Ch:%-1d",
            ...
            ACURR(A_CHA));
    Sprintf(nb = eos(nb), "%s",
            (u.ualign.type == A_CHAOTIC) ? "  Хаос"
              : (u.ualign.type == A_NEUTRAL) ? "  Нейтрал"
                : "  Закон");
#ifdef SCORE_ON_BOTL
    if (flags.showscore)
...
do_statusline2(void)
{
    ...
    if ((cap = near_capacity()) > UNENCUMBERED)
        Sprintf(nb = eos(nb), " %s", enc_stat_ru[cap]);
    if (Blind)
    ...
    int dflgs) /* 1: append trailing space; 2: include dungeon branch name */
{
    ...
    if (Is_knox(&u.uz)) {
        ...
        Sprintf(buf, "Дом %d", dunlev(&u.uz));
    } else if (In_endgame(&u.uz)) {
        /* [3.6.2: this used to be "Astral Plane" or generic "End Game"] */
        (void) endgamelevelname(buf, depth(&u.uz));
        if (!addbranch)
            (void) strsubst(buf, "План ", ""); /* just keep <element> */
        addbranch = FALSE;
    } else {
        /* ports with more room may expand this one */
        if (!addbranch)
            Sprintf(buf, "%s:%-2d", /* "Ур.:n" (grep fodder) */
                    In_tutorial(&u.uz) ? "Обучение" : "Ур.", depth(&u.uz));
        else
            Sprintf(buf, "уровень %d", depth(&u.uz));
        ret = 0;
    }
    if (addbranch) {
...
bot_via_windowport(void)
{
    ...
    Strcpy(gb.blstats[idx][BL_CAP].val,
           (cap > UNENCUMBERED) ? enc_stat_ru[cap] : "");
    ...
    if (gm.multi < 0) {
        ...
        if (condtests[bl_parlyz].enabled
            && cache_multi_reason && !cache_avail[1]) {
                cache_reslt[1] = (!strncmp(cache_multi_reason, "парализованы", 12)
                                 || !strncmp(cache_multi_reason, "заморожены", 10));
                cache_avail[1] = TRUE;
        }
...
cond_menu(void)
{
    ...
    do {
        ...
        any = cg.zeroany;
        any.a_int = 1;
        Sprintf(mbuf, "Сортировать по \"%s\" вместо \"%s\"",
        ...
        any = cg.zeroany;
        Sprintf(mbuf, "Сортировка: %s", menutitle[gc.condmenu_sortorder]);
        ...
        end_menu(tmpwin, "Управление условиями");
    ...
    if (has_hilite(fldidx)) {
        ...
        for (hl = gb.blstats[0][fldidx].thresholds; hl; hl = hl->next) {
            ...
            case BL_TH_TEXTMATCH: /* ANY_STR */
                ...
                if (fldidx == BL_TITLE)
                    /* "<name> <rank-title>", skip past "<name> " */
...
parse_status_hl2(char (*s)[QBUFSZ], boolean from_configfile)
{
    ...
    */
    ...
    if (fld == BL_FLUSH) {
        config_error_add("Неизвестное имя состояния '%s'", s[sidx]);
        return FALSE;
    }
    if (fld == BL_CONDITION)
    ...
    while (s[sidx][0]) {
        ...
        if (*s[sidx + 1] == '\0' || !strcmpi(s[sidx], "always")) {
            ...
            changed = TRUE;
                   ...
                   && (is_fld_arrayvalues(s[sidx], enc_stat,
                                          ...
                                          &kidx)
                       || is_fld_arrayvalues(s[sidx], enc_stat_ru,
                                             SLT_ENCUMBER, OVERLOADED + 1,
                                             &kidx))) {
            txt = enc_stat_ru[kidx];
            txtval = TRUE;
            ...
            config_error_add(has_ltgt_percentnumber(s[sidx])
                 ? "Неверное значение '%s', ожидается число-значение или слово"
                 : "Неправильное сравнение '%s'",
                             s[sidx]);
            return FALSE;
        }
        ...
        if (initblstats[fld].anytype == ANY_STR && (percent || numeric)) {
            config_error_add("Поле '%s' не принимает числовые значения",
        ...
        if (percent) {
            if (initblstats[fld].idxmax < 0) {
                config_error_add("Неверная процентная шкала в '%s'",
                                 initblstats[fld].fldname);
                return FALSE;
                       ...
                       || (hilite.value.a_int > 101)) {
                config_error_add(
                           "hilite_status: недопустимое значение '%s%d%%'",
                                 (hilite.rel == LT_VALUE) ? "<"
                                   : (hilite.rel == LE_VALUE) ? "<="
                                     : (hilite.rel == GT_VALUE) ? ">"
                                       : (hilite.rel == GE_VALUE) ? ">="
        ...
        for (i = 0; i < sf; ++i) {
            ...
            if (a == ATR_BOLD)
                ...
                if (c >= CLR_MAX || coloridx != -1) {
                    config_error_add("Неверный цвет '%d %d'", c, coloridx);
                    return FALSE;
                }
                coloridx = c;
            }
...
str2conditionbitmask(char *str)
{
    ...
    for (i = 0; i < sf; ++i) {
        ...
        if (!bm) {
            config_error_add("Неправильное условие '%s'", subfields[i]);
            return 0UL;
        }
...
parse_condition(char (*s)[QBUFSZ], int sidx)
{
    ...
    sidx++;
    if (!s[sidx][0]) {
        config_error_add("Неправильное условие");
        return FALSE;
    }
    while (s[sidx][0]) {
        int sf = 0;     /* subfield count */
        ...
        how = s[sidx];
        if (!how || !*how) {
            config_error_add("Неправильный формат " "цвет+атрибут");
            return FALSE;
        }
        ...
        for (i = 0; i < sf; ++i) {
            ...
            if (a == ATR_BOLD)
                ...
                if (k >= CLR_MAX) {
                    config_error_add("Неверный цвет %d", k);
                    return FALSE;
                }
                coloridx = k;
            }
...
status_hilite_menu_choose_field(void)
{
    ...
    end_menu(tmpwin, "Выберите поле подсветки:");
...
status_hilite_menu_choose_behavior(int fld)
{
    ...
    if (fld != BL_CONDITION) {
        ...
        Sprintf(buf, "Всегда подсвечивать %s", initblstats[fld].fldname);
    ...
    if (fld == BL_CONDITION) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'b', 0, ATR_NONE,
                 clr, "Условия для подсветки", MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    ...
    if (fld != BL_CONDITION && fld != BL_VERS) {
        ...
        Sprintf(buf, "%s: изменение направления", initblstats[fld].fldname);
    ...
    if (fld != BL_CAP && fld != BL_HUNGER
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'n', 0, ATR_NONE,
                 clr, "Абсолютное значение", MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    ...
    if (initblstats[fld].idxmax >= 0) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'p', 0, ATR_NONE,
                 clr, "Процентное значение", MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    ...
    if (fld == BL_HP) {
        ...
        Sprintf(buf,  "Критический уровень поля %s",
                initblstats[fld].fldname);
        add_menu(tmpwin, &nul_glyphinfo, &any, 'C', 0, ATR_NONE,
                 clr, buf, MENU_ITEMFLAGS_NONE);
        nopts++;
    ...
    if (initblstats[fld].anytype == ANY_STR
        ...
        Sprintf(buf, "%s: соответствие тексту", initblstats[fld].fldname);
        add_menu(tmpwin, &nul_glyphinfo, &any, 't', 0, ATR_NONE,
                 clr, buf, MENU_ITEMFLAGS_NONE);
        nopts++;
    }

    Sprintf(buf, "Выберите поведение подсветки поля %s:",
            initblstats[fld].fldname);
    end_menu(tmpwin, buf);
    ...
    boolean ltok, boolean gtok)
{
    ...
    if (ltok) {
        if (str)
            Sprintf(buf, "%s than %s",
                    (fld == BL_AC) ? "ниже (AC)" : "ниже", str);
        else
            Sprintf(buf, "Строго меньше");
        any = cg.zeroany;
    ...
    if (str)
        Sprintf(buf, "Равно %s", str);
    else
        Sprintf(buf, "Точное значение");
    any = cg.zeroany;
    ...
    if (gtok) {
        ...
        if (str)
            Sprintf(buf, "%s than %s",
                    (fld == BL_AC) ? "выше (AC)" : "выше", str);
        else
            Sprintf(buf, "Строго выше");
        any = cg.zeroany;
        any.a_int = 10 + GT_VALUE;
        add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr,
                 buf, MENU_ITEMFLAGS_NONE);
    }
    Sprintf(buf, "Выберите порог значения %s:", initblstats[fld].fldname);
    end_menu(tmpwin, buf);
...
status_hilite_menu_add(int origfld)
{
 ...
 choose_value:
    ...
    if (behavior == BL_TH_VAL_PERCENTAGE
        ...
        inbuf[0] = '\0';
        Sprintf(buf, "Введите %sзначение для поля %s:",
                percent ? "percentage " : "",
                initblstats[fld].fldname);
        getlin(buf, inbuf);
        ...
        if (*inp == '%') {
            if (!percent) {
                pline("Проценты не разрешены.");
                goto choose_behavior;
            }
            *inp = '\0'; /* strip '%' [this accepts trailing junk!] */
        } else if (*inp) {
            /* some random characters */
            pline("\"%s\" не является числом.", inp);
            goto choose_value;
        }
        if (!gotnum) {
            pline("Число не введено.");
            goto choose_value;
        }
        ...
        if (percent) {
            ...
            if (initblstats[fld].idxmax == -1) {
                pline("Поле '%s' не допускает процентных значений.",
                      initblstats[fld].fldname);
                behavior = BL_TH_VAL_ABSOLUTE;
                goto choose_value;
            }
            ...
            if ((val < 0 && (val != -1 || lt_gt_eq != GT_VALUE))
                ...
                pline("'%s%d%%' недопустимое процентное значение.", op, val);
                goto choose_value;
            }
        ...
        Sprintf(colorqry, "Выберите цвет, если %s равно %s%s%s:",
                initblstats[fld].fldname,
                (lt_gt_eq == LT_VALUE) ? "меньше "
                  : (lt_gt_eq == GT_VALUE) ? "больше "
                    : "",
                numstart,
                (lt_gt_eq == LE_VALUE) ? " или равно"
                  : (lt_gt_eq == GE_VALUE) ? " или равно"
                    : "");
        Sprintf(attrqry, "Выберите атрибут, если %s равно %s%s%s:",
                initblstats[fld].fldname,
                (lt_gt_eq == LT_VALUE) ? "меньше "
                  : (lt_gt_eq == GT_VALUE) ? "больше "
                    : "",
        ...
        Sprintf(colorqry, "Выберите цвет, если %s %s:",
                initblstats[fld].fldname,
                (lt_gt_eq == EQ_VALUE) ? "изменилось"
                  : (lt_gt_eq == LT_VALUE) ? "уменьшилось"
                    : "увеличилось");
        Sprintf(attrqry, "Выберите атрибут, если %s %s:",
                initblstats[fld].fldname,
                (lt_gt_eq == EQ_VALUE) ? "изменилось"
                  : (lt_gt_eq == LT_VALUE) ? "уменьшилось"
                    : "увеличилось");
        ...
        Snprintf(colorqry, sizeof(colorqry),
                "Выберите цвет для состояния %s:",
                conditionbitmask2str(cond));
        Snprintf(attrqry, sizeof(attrqry),
                "Выберите атрибут для состояния %s:",
                conditionbitmask2str(cond));
    } else if (behavior == BL_TH_TEXTMATCH) {
        char qry_buf[BUFSZ];

        Sprintf(qry_buf, "%s: %s введите значение для сравнения:",
                (fld == BL_CAP
                 || fld == BL_ALIGN
                 || fld == BL_HUNGER
                 || fld == BL_TITLE) ? "заголовок" : "значение",
                initblstats[fld].fldname);
        if (fld == BL_CAP) {
            ...
            hilite.rel = TXT_VALUE;
            Strcpy(hilite.textmatch, enc_stat_ru[rv]);
        } else if (fld == BL_ALIGN) {
            ...
            for (i = j = 0; i < 9; i++) {
                ...
                if (gu.urole.rank[i].f) {
                    ...
                    Snprintf(obuf, sizeof obuf, "%s или %s",
        ...
        Sprintf(colorqry, "Выберите цвет, если %s содержит '%s':",
                initblstats[fld].fldname, hilite.textmatch);
        Sprintf(attrqry, "Выберите атрибут, если %s содержит '%s':",
                initblstats[fld].fldname, hilite.textmatch);
    } else if (behavior == BL_TH_ALWAYS_HILITE) {
        Sprintf(colorqry, "Выберите цвет для постоянной подсветки %s:",
                initblstats[fld].fldname);
        Sprintf(attrqry, "Выберите атрибут для постоянной подсветки %s:",
                initblstats[fld].fldname);
    }
 ...
 choose_color:
    ...
    if (behavior == BL_TH_CONDITION) {
        ...
        if (tmpattr)
            Sprintf(eos(clrbuf), "&%s", tmpattr);
        pline("Добавлено правило condition/%s/%s",
        ...
        if (fld == BL_TITLE && (p = strstri(hilite.textmatch, " or ")) != 0) {
            ...
            *p = '\0'; /* chop off " or female-rank" */
            ...
            status_hilite_add_threshold(fld, &hilite);
            pline("Добавлено правило %s", status_hilite2str(&hilite));
...
status_hilite_menu_fld(int fld)
{
    ...
    if (count) {
        ...
        Sprintf(buf, "Нет правил подсветки для %s", initblstats[fld].fldname);
    ...
    if (count) {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'X', 0, ATR_NONE, clr,
                 "Удалить правила подсветки", MENU_ITEMFLAGS_NONE);
    }
...
#endif
    {
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, 'Z', 0, ATR_NONE,
                 clr, "Сбросить правила подсветки", MENU_ITEMFLAGS_NONE);
    }

    Sprintf(buf, "Правила подсветки %s:", initblstats[fld].fldname);
    end_menu(tmpwin, buf);
...
status_hilite_menu(void)
{
 ...
 shlmenu_redo:
    ...
    if (countall) {
        ...
        any.a_int = -1;
        add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
                 clr, "Подсветка всех полей, включая отключённые",
                 MENU_ITEMFLAGS_NONE);
    ...
    for (i = 0; i < MAXBLSTATS; i++) {
        ...
        if (count)
            Sprintf(eos(buf), " (количество: %d)", count);
        add_menu(tmpwin, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
                 clr, buf, MENU_ITEMFLAGS_NONE);
    }

    end_menu(tmpwin, "Выберите поле:");
