...
static const struct {
    ...
    Bitfield(greasy, 1);                  /* causes slippery fingers */
} tintxts[] = { { "протухшая", -50, 0, 0 },  /* ROTTEN_TIN = 0 */
                { "самодельная", 50, 1, 0 }, /* HOMEMADE_TIN = 1 */
                { "суп из", 20, 1, 0 },
                { "по-французски", 40, 0, 1 },
                { "маринованная", 40, 1, 0 },
                { "вареная", 50, 1, 0 },
                { "копченая", 50, 1, 0 },
                { "сушеная", 55, 1, 0 },
                ...
                { "сычуаньская", 70, 1, 0 },
                { "запеченная", 80, 0, 0 },
                { "жареная в воке", 80, 0, 1 },
                { "обжаренная", 95, 0, 0 },
                { "засахаренная", 100, 1, 0 },
                { "пюре", 500, 1, 0 },
                { "", 0, 0, 0 } };
...
choke(struct obj *food)
{
    ...
    if (Breathless || Hunger || (!Strangled && !rn2(20))) {
        ...
        if (food && food->otyp == AMULET_OF_STRANGULATION) {
            You("задыхаешься, но приходишь в себя.");
            return;
        }
        You("набиваешь желудок, а потом тебя обильно рвёт.");
         ...
         */
        if (food) {
            You("давишься своим %s.", foodword(food));
            if (food->oclass == COIN_CLASS) {
                Strcpy(svk.killer.name, "очень богатая трапеза");
            } else {
            ...
            You("давишься этим.");
            Strcpy(svk.killer.name, "быстрый перекус");
        }
        You("умираешь...");
        done(CHOKING);
    }
    ...
    int *dmg_p) /* for dishing out extra damage in lieu of Int loss */
{
    ...
    if (noncorporeal(pd)) {
        ...
        if (visflag && canspotmon(mdef))
            pline("Мозг %s съеден!", s_suffix(Monnam(mdef)));
    }
        ...
        if (magr == &gy.youmonst) {
            ...
            if (!DEADMONSTER(magr)) {
                ...
                if (magr->mtame && !visflag)
                    ...
                    You("на мгновение тебе становится грустно, потом это проходит.");
                return M_ATTK_AGR_DIED;
            }
        }
    }
    ...
    if (magr == &gy.youmonst) {
        ...
        if (mindless(pd)) { /* (cannibalism not possible here) */
            pline("%s не замечает.", Monnam(mdef));
            /* all done; no extra harm inflicted upon target */
            return M_ATTK_MISS;
        } else if (is_rider(pd)) {
            pline("Проглатывание этого смертельно.");
            Sprintf(svk.killer.name, "неразумно съел мозг %s",
                    pmname(pd, Mgender(mdef)));
        ...
        if (mindless(pd)) {
            ...
            *dmg_p += xtra_dmg;
            ...
            if (*dmg_p >= mdef->mhp && visflag && canspotmon(mdef))
                pline("Последняя мысль %s угасает...",
    ...
    if (!CANNIBAL_ALLOWED()
        ...
        if (allowmsg) {
            if (Upolyd && your_race(fptr))
                You("глубоко внутри тебя возникает дурное чувство.");
            You("Каннибал!  Пожалеешь об этом!");
        }
...
cprefx(int pm)
{
    ...
    if (flesh_petrifies(&mons[pm])) {
        if (!Stone_resistance
            ...
            Sprintf(svk.killer.name, "пробуя мясо %s",
                    mons[pm].pmnames[NEUTRAL]);
            svk.killer.format = KILLED_BY;
            You("превращаешься в камень.");
            done(STONING);
            if (svc.context.victual.piece)
    ...
    case PM_LARGE_CAT:
        ...
        if (!CANNIBAL_ALLOWED()) {
            You_feel("что есть %s было плохой идеей.",
                     mons[pm].pmnames[NEUTRAL]);
            HAggravate_monster |= FROMOUTSIDE;
        }
        break;
    ...
    case PM_PESTILENCE:
    case PM_FAMINE: {
        pline("Это мгновенно смертельно.");
        Sprintf(svk.killer.name, "неразумно съел тело %s",
                mons[pm].pmnames[NEUTRAL]);
        svk.killer.format = NO_KILLER_PREFIX;
        done(DIED);
        /* life-saving needed to reach here */
...
fix_petrification(void)
{
    ...
    if (Hallucination)
        Sprintf(buf, "Как жаль — ты только что испортил будущий %sарт!",
                ACURR(A_CHA) > 15 ? "fine " : "");
    else
        Strcpy(buf, "Ты чувствуешь себя гибким!");
...
givit(int type, struct permonst *ptr)
{
    ...
    case COLD_RES:
        ...
        if (!(HCold_resistance & FROMOUTSIDE)) {
            You_feel("наполненный горячим воздухом.");
    ...
    case DISINT_RES:
        ...
        if (!(HDisint_resistance & FROMOUTSIDE)) {
            You_feel(Hallucination ? "полностью в себе." : "очень твёрдо.");
    ...
    case SHOCK_RES: /* shock (electricity) resistance */
        ...
        if (!(HShock_resistance & FROMOUTSIDE)) {
            if (Hallucination)
                You_feel("прочно стоишь на земле.");
            else
                Your("здоровье сейчас ощущается усиленным!");
    ...
    case POISON_RES:
        ...
        if (!(HPoison_resistance & FROMOUTSIDE)) {
            You_feel(Poison_resistance ? "особенно здоровым." : "здоровым.");
    ...
    case TELEPORT:
        ...
        if (!(HTeleportation & FROMOUTSIDE)) {
            You_feel(Hallucination ? "рассеянным." : "очень нервным.");
    ...
    case TELEPORT_CONTROL:
        ...
        if (!(HTeleport_control & FROMOUTSIDE)) {
            You_feel(Hallucination ? "сосредоточенным в своём внутреннем пространстве."
                                   : "владеющим собой.");
    ...
    case TELEPAT:
        ...
        if (!(HTelepat & FROMOUTSIDE)) {
            You_feel(Hallucination ? "связанным с космосом."
                                   : "странной умственной ясностью.");
    ...
    case ACID_RES:
        ...
        if (!Acid_resistance)
            You_feel("%s.", Hallucination ? "защищённым от вспышек памяти"
                            : "менее опасённым кислотой");
        incr_itimeout(&HAcid_resistance, d(3, 6));
        break;
    case STONE_RES:
        debugpline0("Giving timed stoning resistance");
        if (!Stone_resistance)
            You_feel("%s.", Hallucination ? "необычно гибким"
                            : "менее опасённым окаменением");
...
cpostfx(int pm)
{
    ...
    case PM_QUANTUM_MECHANIC:
        Your("скорость внезапно кажется очень ненадёжной!");
        if (HFast & INTRINSIC) {
            HFast &= ~INTRINSIC;
            You("замедляешься.");
        } else {
    ...
    case PM_MASTER_MIND_FLAYER:
        if (ABASE(A_INT) < ATTRMAX(A_INT)) {
            if (!rn2(2)) {
                pline("Ням!  Это была настоящая мозговая пища!");
                (void) adjattrib(A_INT, 1, FALSE);
                break; /* don't give them telepathy, too */
            }
        } else {
            pline("Почему-то это было пресно.");
        }
        FALLTHROUGH;
    /*FALLTHRU*/
    default:
...
tin_details(struct obj *obj, int mnum, char *buf)
{
    ...
    if (r == SPINACH_TIN)
        ...
        if (vegetarian(&mons[mnum]))
            ...
            Sprintf(eos(buf), "мясо %s", mons[mnum].pmnames[NEUTRAL]);
    }
}
...
consume_tin(const char *mesg)
{
    ...
    pline1(mesg); /* "Тебе удаётся открыть банку." */
    ...
    if (r != SPINACH_TIN) {
        ...
        if (mnum == NON_PM) {
            if (Hallucination)
                pline("Она полна %s.",
                      rn2(2) ? "суфле воздушного элементаля"
                             : "обезвоженная вода");
            else
                pline("Оказывается, она пуста.");
            observe_object(tin);
            tin->known = 1;
        ...
        if (!always_eat) {
            pline("От неё пахнет %s.", what);
            if (y_n("Съесть?") == 'n') {
                if (flags.verbose)
                    You("выбросить открытую банку.");
                if (!Hallucination) {
                    observe_object(tin);
                    tin->known = 1;
                }
        ...
        You("съесть %s %s.", tintxts[r].txt, mons[mnum].pmnames[NEUTRAL]);
        ...
        if (tintxts[r].greasy) {
            ...
            pline("Еда «%s» сделала твои %s %s скользкими.",
                  tintxts[r].txt, fingers_or_gloves(TRUE),
                  alreadyglib ? "ещё более" : "очень");
        }

    } else { /* spinach... */
        if (tin->cursed) {
            pline("Внутри что-то гнилое%s%s.",
...
opentin(void)
{
    ...
    if (svc.context.tin.usedtime++ >= 50) {
        You("оставить попытку открыть банку.");
        return 0;
    }
    if (svc.context.tin.usedtime < svc.context.tin.reqtime)
        return 1; /* still busy */

    consume_tin("Тебе удаётся открыть банку.");
    return 0;
}
...
start_tin(struct obj *otmp)
{
    ...
    if (metallivorous(gy.youmonst.data)) {
        mesg = "Ты вгрызаешься прямо в металлическую банку...";
        tmp = 0;
    } else if (cantwield(gy.youmonst.data)) { /* nohands || verysmall */
        You("не можешь правильно обращаться с банкой, чтобы открыть её.");
        return;
        ...
        tmp = (uwep && uwep->blessed && uwep->otyp == TIN_OPENER) ? 0
                                                                  : rn2(2);
        if (!tmp)
            mesg = "Банка открывается как по волшебству!";
        else
            pline_The("банка кажется легко открываемой.");
    } else if (uwep) {
        switch (uwep->otyp) {
        case TIN_OPENER:
            mesg = "Ты легко открываешь банку."; /* iff tmp==0 */
        ...
        pline("Используя %s, ты пытаешься открыть банку.", yobjnam(uwep, (char *) 0));
    } else {
 no_opener:
        pline("Эту банку открыть не так просто.");
        if (Glib) {
            pline_The("банка выскальзывает из твоих %s.", fingers_or_gloves(FALSE));
            if (otmp->quan > 1L) {
                otmp = splitobj(otmp, 1L);
            }
            if (carried(otmp))
    ...
    if (!tmp) {
        ...
        set_occupation(opentin, "открывание банки", 0);
    }
    return;
}
...
/* called on the "первый кусок" of rotten food */
staticfn int
rottenfood(struct obj *obj)
{
    pline("Фу!  %s %s!",
          is_rottable(obj) ? "Гнилая" : "Ужасная", foodword(obj));
    if (!rn2(4)) {
        if (Hallucination)
            You_feel("довольно странно.");
        else
            You_feel("довольно %s.", body_part(LIGHT_HEADED));
        make_confused(HConfusion + d(2, 4), FALSE);
    } else if (!rn2(4) && !Blind) {
        pline("Внезапно всё темнеет.");
        ...
        if (!Blind)
            what = "темнеет", where = "темнота";
        else if (Levitation || Is_airlevel(&u.uz) || Is_waterlevel(&u.uz))
            what = "ты теряешь контроль над", where = "собой";
        else
            what = "ты бьёшься о",
            where = (u.usteed) ? "седло" : surface(u.ux, u.uy);
        pline_The("мир вращается, и %s %s.", what, where);
        incr_itimeout(&HDeaf, duration);
        disp.botl = TRUE;
        nomul(-duration);
        gm.multi_reason = "без сознания от гнилой еды";
        gn.nomovemsg = "Ты снова в сознании.";
...
eatcorpse(struct obj *otmp)
{
    ...
    if (!vegan(&mons[mnum]))
        if (!u.uconduct.unvegan++) {
            livelog_printf(LL_CONDUCT,
                  "впервые употребил продукты животного происхождения, съев %s",
    ...
    if (!vegetarian(&mons[mnum])) {
        if (!u.uconduct.unvegetarian && !ll_conduct)
            livelog_printf(LL_CONDUCT,
                           "впервые попробовал мясо, съев %s",
                           an(food_xname(otmp, FALSE)));
        violated_vegetarian();
    }
    ...
    if (!glob && !stoneable && !slimeable && rotted > 5L) {
        ...
        pline("Угу - этот %s был испорчен%s!",
              (mons[mnum].mlet == S_FUNGUS) ? "грибная растительность"
              : vegetarian(&mons[mnum]) ? "протоплазма"
                : "мясо",
              cannibal ? ", ты каннибал" : "");
        if (Sick_resistance) {
            pline("Впрочем, это совсем не вызывает тошноты...");
        } else {
            long sick_time;
            ...
            make_sick(sick_time, corpse_xname(otmp, "гнилая", CXN_NORMAL),
                      TRUE, SICK_VOMITABLE);

            pline("(Похоже, он умер слишком давно, чтобы быть безопасным в пищу.)");
        }
        if (carried(otmp))
            useup(otmp);
        else
        ...
        tp++;
        pline("Фух - это, наверное, было ядовито!");
        if (!Poison_resistance) {
            poison_strdmg(rnd(4), rnd(15),
                          !glob ? "ядовитый труп" : "ядовитая глоба",
                          KILLED_BY_AN);
        } else
            You("не чувствуешь действия яда.");
    ...
    if (!tp && !nonrotting_corpse(mnum) && (otmp->orotten || !rn2(7))) {
        ...
        if (!mons[otmp->corpsenm].cnutrit) {
            ...
            if (!retcode)
                pline_The("труп полностью сгнивает.");
            if (carried(otmp))
                useup(otmp);
            else
                useupf(otmp, 1L);
        ...
        if (!retcode)
            consume_oeaten(otmp, 2); /* oeaten >>= 2 */
               ...
               && (Stone_resistance || Hallucination)) {
        pline("Это вкусно как курица!");
    } else if (mnum == PM_FLOATING_EYE && u.umonnum == PM_RAVEN) {
        You("клюёшь глаз с удовольствием.");
    } else if (tp) {
        ; /* we've already delivered a message; don't add "на вкус нормально" */
    } else {
        ...
        static const char *const palatable_msgs[] = {
            ...
            "Токай", "Волокнистый", "Игривый", "Жирненький", "Жёсткий"
        };
        ...
        pline("%s%s %s %s%c",
              type_is_pname(&mons[mnum])
                 ? "" : the_unique_pm(&mons[mnum]) ? "Эта " : "Эта ",
              pmxnam,
              use_is ? "есть" : "на вкус",
                  /* tiger reference is to TV ads for "Frosted Flakes",
                     breakfast cereal targeted at kids by "Тони тигр" */
              Hallucination
                 ? (yummy ? ((u.umonnum == PM_TIGER) ? "в-в-в-кусно" : "крутая")
                          : palatable ? "безобидная" : "паршивая")
              : (yummy ? "изумительная" : palatable ?
                 &palat_msg[1] : "ужасная"),
              (yummy || !palatable) ? '!' : '.');
    }
...
fprefx(struct obj *otmp)
{
    ...
    case EGG:
        if (otmp->corpsenm == PM_PYROLISK) {
            ...
            pline("Фу.  Гнилое яйцо."); /* perhaps others like it */
    ...
    case FOOD_RATION: /* nutrition 800 */
        ...
        if (u.uhunger <= 200)
            pline("%s!", Hallucination ? "Ого, вот это да, чувак"
                                       : "Эта еда прямо в самое сердце");

        /* 700-1+800 remains below 1500, the choking threshold which
           triggers "тебе трудно это проглотить" feedback */
        else if (u.uhunger < 700)
            pline("Это насыщает твой %s!", body_part(STOMACH));
        /* [satiation message may be inaccurate if eating gets interrupted] */
        break;
    case TRIPE_RATION:
        if (carnivorous(gy.youmonst.data) && !humanoid(gy.youmonst.data)) {
            pline("Этот трефель неожиданно вкусный!");
        } else if (maybe_polyd(is_orc(gy.youmonst.data), Race_if(PM_ORC))) {
            pline(Hallucination ? "Вкусно!  Менее сытно!"
                                : "М-м, трефель... неплохо!");
        } else {
            pline("Фу - собачья еда!");
    ...
    default:
        if (otmp->otyp == SLIME_MOLD && !otmp->cursed
            ...
            pline("Вот это %s %s!",
                  Hallucination ? "превосходная" : "вкусная",
                  singular(otmp, xname));
        ...
        /* KMH -- Why should Unix have all the fun?
           ...
           message; the '#if UNIX' code will still kick in for pear. */
            ...
            pline("Вкусно!  Наверное, это «Макинтош»!");
#endif

#ifdef UNIX
        } else if (otmp->otyp == APPLE || otmp->otyp == PEAR) {
            if (!Hallucination) {
                pline("Программа аварийно завершилась.");
            } else {
                /* based on an old Usenet joke, a fake a.out manual page */
                int x = rnd(100);

                pline("%s -- программа аварийно завершилась.",
                      (x <= 75)
                         ? "Ошибка сегментации"
                         : (x <= 99)
                            ? "Ошибка шины"
 ...
 give_feedback:
            pline("Эта %s %s", singular(otmp, xname),
                  otmp->cursed
                     ? (Hallucination ? "паршивая!" : "ужасная!")
                     : (otmp->otyp == CRAM_RATION
                        || otmp->otyp == K_RATION
                        || otmp->otyp == C_RATION)
                        ? "пресная."
                        : Hallucination ? "крутая!" : "изумительная!");
        }
        break; /* default */
    } /* switch */
    return TRUE;
...
{
    pline("Волшебство распространяется по телу, пока ты перевариваешь %s.",
          (otmp->oclass == RING_CLASS) ? "кольцо" : "амулет");
}
        ...
        default:
            ...
            case RIN_INVISIBILITY:
                if (!oldprop && !EInvis && !BInvis && !See_invisible
                    && !Blind) {
                    newsym(u.ux, u.uy);
                    Your("тело становится %s прозрачным...",
                         Hallucination ? "обычной" : "странной");
                    makeknown(typ);
                }
                break;
...
static const char *const foodwords[] = {
    "еда",    "жидкость",  "воск",       "пища", "мясо",     "бумага",
    "ткань",   "кожа", "дерево",      "кость", "чешуя",    "металл",
    "металл",   "металл",   "серебро",    "золото", "платина", "мифрил",
    "пластик", "стекло",   "деликатес", "камень"
};
    ...
    case FORTUNE_COOKIE:
        ...
        if (!Blind)
            if (!u.uconduct.literate++)
                livelog_printf(LL_CONDUCT,
                    "становишься грамотным, прочитав предсказание внутри печенья");
        break;
    case LUMP_OF_ROYAL_JELLY:
        ...
        if (Upolyd) {
            ...
            if (u.uhp > u.uhpmax) {
                ...
                svk.killer.format = KILLED_BY_AN;
                Strcpy(svk.killer.name, "гнилый кусок царского желе");
                done(POISONING);
            }
        }
        if (!otmp->cursed)
        ...
        if (ismnum(otmp->corpsenm)
            ...
            if (!Stone_resistance
                && !(poly_when_stoned(gy.youmonst.data)
                     && polymon(PM_STONE_GOLEM))) {
                if (!Stoned) {
                    Sprintf(svk.killer.name, "яйцо %s",
                            mons[otmp->corpsenm].pmnames[NEUTRAL]);
                    make_stoned(5L, (char *) 0, KILLED_BY_AN,
                                svk.killer.name);
                }
            }
            /* note: no "на вкус как курица" message for eggs */
        }
        break;
    ...
    case APPLE:
        if (otmp->cursed && !Sleep_resistance) {
            ...
            if (Race_if(PM_DWARF) && Hallucination) {
                verbalize("Ой-и, ну-у, кажется, сегодня я пропущу работу.");
            } else if (Deaf || !flags.acoustics) {
                You("засыпаешь.");
            } else {
                Soundeffect(se_sinister_laughter, 100);
                You_hear("зловещий смех, пока ты засыпаешь...");
            }
...
edibility_prompts(struct obj *otmp)
{
    ...
    Strcpy(foodsmell, Tobjnam(otmp, "запах"));
    Strcpy(it_or_they, (otmp->quan == 1L) ? "его" : "их");
    ...
    if (cadaver && rotted > 5L && !Sick_resistance) {
        /* Tainted meat */
        Snprintf(buf, sizeof buf, "%s вроде %s может быть испорчен!",
                 foodsmell, it_or_they);
    } else if (stoneorslime) {
        Snprintf(buf, sizeof buf,
                 "%s вроде %s может быть чем-то очень опасным!",
                 foodsmell, it_or_they);
        ...
        /* Rotten */
        Snprintf(buf, sizeof buf, "%s вроде %s может быть гнилым!",
                 foodsmell, it_or_they);
    } else if (cadaver && poisonous(&mons[mnum]) && !Poison_resistance) {
        /* poisonous */
        Snprintf(buf, sizeof buf, "%s вроде %s может быть ядовитым!",
                 foodsmell, it_or_they);
    } else if (otmp->otyp == APPLE && otmp->cursed && !Sleep_resistance) {
        /* causes sleep, for long enough to be dangerous */
        Snprintf(buf, sizeof buf, "%s вроде %s мог быть отравлен.",
                 foodsmell, it_or_they);
    } else if (cadaver && !vegetarian(&mons[mnum])
               && !u.uconduct.unvegetarian && Role_if(PM_MONK)) {
        Snprintf(buf, sizeof buf, "%s нездоровый.", foodsmell);
    } else if (cadaver && acidic(&mons[mnum]) && !Acid_resistance) {
        Snprintf(buf, sizeof buf, "%s довольно кислый.", foodsmell);
    } else if (Upolyd && u.umonnum == PM_RUST_MONSTER && is_metallic(otmp)
               && otmp->oerodeproof) {
        Snprintf(buf, sizeof buf, "%s отвратителен для тебя сейчас.",
                 foodsmell);
     ...
     */
        ...
        Snprintf(buf, sizeof buf, "%s прогнил и незнаком тебе.",
                 foodsmell);
    } else if (!u.uconduct.unvegetarian
               && ((material == LEATHER || material == BONE
                    || material == DRAGON_HIDE)
                   || (cadaver && !vegetarian(&mons[mnum])))) {
        Snprintf(buf, sizeof buf, "%s незнаком тебе.", foodsmell);
    }

    if (*buf) {
        Snprintf(eos(buf), sizeof buf - strlen(buf), "  Всё равно съесть %s?",
                 (otmp->quan == 1L) ? "его" : "одну");
...
doeat_nonfood(struct obj *otmp)
{
    ...
    if (!u.uconduct.food++) {
        ll_conduct++;
        livelog_printf(LL_CONDUCT, "впервые съел (%s)",
    ...
    if (material == LEATHER || material == BONE
        ...
        if (material != WAX) {
            if (!u.uconduct.unvegetarian && !ll_conduct)
                livelog_printf(LL_CONDUCT,
                   "впервые попробовал мясные субпродукты, съев %s",
                               an(food_xname(otmp, FALSE)));
            violated_vegetarian();
        }
    }
...
doeat(void)
{
    ...
    if (Strangled) {
        pline("Если ты не можешь дышать воздухом, как же ты будешь есть твёрдую пищу?");
        return ECMD_OK;
    }
    ...
    if (u.uedibility) {
        ...
        if (res) {
            Your(
               "%s перестаёт покалывать, и обоняние возвращается к норме.",
                 body_part(NOSE));
            u.uedibility = 0;
            if (res == 1)
                return ECMD_OK;
    ...
    if (!is_edible(otmp)) {
        ...
        return ECMD_OK;
               ...
               != 0) {
        /* let them eat rings */
        You_cant("съесть %s, который ты носишь.", something);
        return ECMD_OK;
    ...
    if (is_metallic(otmp) && u.umonnum == PM_RUST_MONSTER
        ...
        pline("Угу - этот %s был защищён от ржавчины!", xname(otmp));
        ...
        if (welded(otmp) || (otmp->cursed && (otmp->owornmask & W_RING))) {
            ...
            You("выплюнуть %s.", the(xname(otmp)));
        } else {
            You("выплюнуть %s на %s.", the(xname(otmp)),
                surface(u.ux, u.uy));
            if (carried(otmp)) {
    ...
    if (otmp == svc.context.victual.piece) {
        ...
        /* if there's only one bite left, there sometimes won't be any
           "ты доедаешь" message when done; use different wording
           for resuming with one bite remaining instead of trying to
           determine whether or not "ты заканчиваешь" is going to be given */
        You("%s свою трапезу.",
            !one_bite_left ? "продолжить" : "дожевать");
        if (otmp)
    ...
    if (otmp->otyp == CORPSE || otmp->globby) {
        ...
        default:
            if (otmp->otyp == PANCAKE || otmp->otyp == FORTUNE_COOKIE /*eggs*/
                ...
                if (!u.uconduct.unvegan++ && !ll_conduct)
                    livelog_printf(LL_CONDUCT,
                           "впервые употребил продукты животного происхождения (%s)",
                                   food_xname(otmp, FALSE));
            break;
        }
...
use_tin_opener(struct obj *obj)
{
    ...
    if (!carrying(TIN)) {
        You("не имеешь банки, которую можно открыть.");
        return ECMD_OK;
    }
    ...
    if (obj != uwep) {
        if (obj->cursed && obj->bknown) {
            ...
            if (ynq(safe_qbuf(qbuf, "Точно взять в руки ", "?",
                              obj, doname, thesimpleoname, "это")) != 'y')
                return ECMD_OK;
        }
    ...
    if (u.uhunger >= 2000) {
        ...
        if (u.uhunger >= 1500 && !Hunger
            && (!svc.context.victual.eating
                || (svc.context.victual.eating
                    && !svc.context.victual.fullwarn))) {
            pline("Тебе трудно запихнуть всё это в себя.");
            gn.nomovemsg = "Ты наконец-то закончил.";
            if (!svc.context.victual.eating) {
                gm.multi = -2;
            } else {
                ...
                if (svc.context.victual.canchoke
                    ...
                    if (!paranoid_query(ParanoidEating, "Продолжить есть?")) {
                        reset_eat();
                        gn.nomovemsg = (char *) 0;
                    }
                }
    ...
    if (newhs == FAINTING) {
        ...
        if (u.uhs <= WEAK || rn2(20 - uhunger_div_by_10) >= 19) {
            if (!is_fainted() && gm.multi >= 0 /* %% */) {
                ...
                stop_occupation();
                You("падаешь в обморок от нехватки еды.");
                incr_itimeout(&HDeaf, duration);
                disp.botl = TRUE;
                nomul(-duration);
                gm.multi_reason = "упал в обморок от нехватки еды";
                gn.nomovemsg = "Ты приходишь в себя.";
                ga.afternmv = unfaint;
                newhs = FAINTED;
                if (!Levitation)
                    selftouch("Падая, ты");
        ...
        /* this used to be -(200 + 20 * Con) but that was when being asleep
           ...
           now uhunger becomes more negative at a slower rate */
            ...
            disp.botl = TRUE;
            bot();
            You("умираешь от голода.");
            svk.killer.format = KILLED_BY;
            Strcpy(svk.killer.name, "истощение");
            done(STARVING);
    ...
    if (newhs != u.uhs) {
        if (newhs >= WEAK && u.uhs < WEAK) {
            ...
            /* this used to be losestr(-1) which could be abused by
               ...
               substituting "в форме монстра" for sustain ability and
               "снова обретаешь человеческий облик" for ring removal might have done that too */
        ...
        case HUNGRY:
            if (Hallucination) {
                You(!incr ? "теперь у тебя ослаб голод."
                    : "начинаешь испытывать голод.");
            } else
                You("%s.", !incr ? "теперь ты только голоден"
                           : (u.uhunger < 145) ? "чувствуешь голод"
                             : "начинаешь чувствовать голод");
        ...
        case WEAK:
            if (Hallucination)
                pline(!incr ? "Ты всё ещё голоден."
              : "Голод мешает тебе двигаться.");
            else if (incr && (Role_if(PM_WIZARD) || Race_if(PM_ELF)
                              || Role_if(PM_VALKYRIE)))
                pline("%s остро нуждается в еде!",
                      (Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE))
                          ? gu.urole.name.m
                          : "Эльф");
            else
                You("%s слаб.", !incr ? "всё ещё"
                                : (u.uhunger < 45) ? "чувствуешь"
        ...
        if ((Upolyd ? u.mh : u.uhp) < 1) {
            You("умираешь от голода и истощения.");
            svk.killer.format = KILLED_BY;
            Strcpy(svk.killer.name, "истощение");
            done(STARVING);
            return;
...
eat_ok(struct obj *obj)
{
    /* 'getobj_else' will be non-zero if floor food is present and
       ...
       "тебе больше нечего есть" if not carrying any food */
    if (!obj)
     ...
     * produce the "Ты не можешь есть золото" message in getobj */
    ...
    int corpsecheck) /* 0, no check, 1, corpses, 2, tinnable corpses */
{
    ...
    if (feeding && metallivorous(uptr)) {
        ...
        if (ttmp && ttmp->tseen && ttmp->ttyp == BEAR_TRAP) {
            ...
            Sprintf(qbuf, "Здесь капкан (%s); съесть?",
                    u_in_beartrap ? "держит тебя" : "взведён");
            if ((c = yn_function(qbuf, ynqchars, 'n', TRUE)) == 'y') {
                ...
                Sprintf(qbuf,"Тебе удаётся лишь %s медвежьи капканы.",
                        u_in_beartrap ? "освободиться от" : "обезвредить");
        ...
        if (levl[u.ux][u.uy].typ == IRONBARS) {
            ...
            c = 'n';
            Strcpy(qbuf, "Здесь железные прутья");
            if (nodig || u.uhunger > 1500) {
                pline("%s но ты %s их съесть.", qbuf,
                      nodig ? "не можешь" : "слишком сыт, чтобы");
            } else {
        ...
        if (uptr != &mons[PM_RUST_MONSTER]
            ...
            if (gold->quan == 1L)
                Sprintf(qbuf, "Здесь 1 золотая монета; съесть?");
            else
                Sprintf(qbuf, "Здесь %ld золотых монет; съесть их?",
                        gold->quan);
            if ((c = yn_function(qbuf, ynqchars, 'n', TRUE)) == 'y') {
                return gold;
            } else if (c == 'q') {
    ...
    for (otmp = svl.level.objects[u.ux][u.uy]; otmp; otmp = otmp->nexthere) {
        if (corpsecheck
            ...
            /* "There is <an object> here; <verb> it?" or
               "Здесь <N предметов>; <verb> одну?" */
            Sprintf(qbuf, "Здесь %s ", otense(otmp, "есть"));
            Sprintf(qsfx, " здесь; %s %s?", verb, one ? "её" : "одну");
            (void) safe_qbuf(qbuf, qbuf, qsfx, otmp, doname, ansimpleoname,
                             one ? something : (const char *) "вещи");
            if ((c = yn_function(qbuf, ynqchars, 'n', TRUE)) == 'y')
                return  otmp;
            else if (c == 'q')
                return (struct obj *) 0;
...
vomit(void) /* A good idea from David Neves */
{
    ...
    if (cantvomit(gy.youmonst.data)) {
        ...
        Your("челюсть судорожно раскрывается.");
    } else {
        ...
        if (u.uhs >= FAINTING)
            Your("%s судорожно рыгает!", body_part(STOMACH));
        else
            spewed = TRUE;
    }
    ...
    if (spewed) {
        ...
        if (mattk) {
            You("дышишь на себя кислотой..."); /* [why?] */
            ubreatheu(mattk);
        }
        ...
        if (acidic(gy.youmonst.data)) {
            ...
            if (is_ice(u.ux, u.uy))
                melt_ice(u.ux, u.uy,
                         "Твоя желудочная кислота проходит сквозь лёд!");
        }
    }
}
...
eaten_stat(int base, struct obj *obj)
{
    ...
    if (uneaten_amt > full_amount) {
        impossible(
          "частично съеденная еда (%ld) питательнее нетронутой еды (%ld)",
...
consume_oeaten(struct obj *obj, int amt)
{
    if (!obj_nutrition(obj)) {
        ...
        if (otyp == CORPSE || otyp == EGG || otyp == TIN) {
            Strcpy(itembuf, (otyp == CORPSE) ? "труп"
                            : (otyp == EGG) ? "яйцо"
                              : (otyp == TIN) ? "банка" : "другое?");
