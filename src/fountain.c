...
{
    const char *umsg = "парите высоко над %s.";
...
dowatersnakes(void)
{
    ...
    if (!(svm.mvitals[PM_WATER_MOCCASIN].mvflags & G_GONE)) {
        if (!Blind) {
            pline("Бесконечный поток %s вырывается наружу!",
                  Hallucination ? makeplural(rndmonnam(NULL)) : "змей");
        } else {
            Soundeffect(se_snakes_hissing, 75);
            You_hear("%s шипит!", something);
        }
        while (num-- > 0)
        ...
        pline("Фонтан яростно бурлит мгновение, затем успокаивается.");
    }
}
...
dowaterdemon(void)
{
    ...
    if (!(svm.mvitals[PM_WATER_DEMON].mvflags & G_GONE)) {
        if ((mtmp = makemon(&mons[PM_WATER_DEMON], u.ux, u.uy,
                            MM_NOMSG)) != 0) {
            if (!Blind)
                You("высвобождаете %s!", a_monnam(mtmp));
            else
                You_feel("присутствие зла.");

            /* Give those on low levels a (slightly) better chance of survival
             */
            if (rnd(100) > (80 + level_difficulty())) {
                pline("В благодарность за %s освобождение, %s дарует вам желание!",
                      mhis(mtmp), mhe(mtmp));
...
dowaternymph(void)
{
    ...
    if (!(svm.mvitals[PM_WATER_NYMPH].mvflags & G_GONE)
        && (mtmp = makemon(&mons[PM_WATER_NYMPH], u.ux, u.uy,
                           MM_NOMSG)) != 0) {
        if (!Blind)
            You("привлекаете %s!", a_monnam(mtmp));
        else
            You_hear("чарующий голос.");
        mtmp->msleeping = 0;
        ...
        pline("Большой пузырь поднимается на поверхность и лопается.");
    } else {
        Soundeffect(se_loud_pop, 50);
        You_hear("громкий хлопок.");
    }
}
...
gush(coordxy x, coordxy y, genericptr_t poolcnt)
{
    ...
    if (!((*(int *) poolcnt)++))
        pline("Вода хлынула из переполненного фонтана!");
...
{
    if (!Blind)
        You("замечаете самоцвет в искрящихся водах!");
    else
        You_feel("здесь самоцвет!");
...
watchman_warn_fountain(struct monst *mtmp)
{
    if (is_watch(mtmp->data) && couldsee(mtmp->mx, mtmp->my)
        && mtmp->mpeaceful) {
        if (!Deaf) {
            pline("%s кричит:", Amonnam(mtmp));
            verbalize("Эй, прекрати пользоваться этим фонтаном!");
        } else {
            pline("%s серьёзно %s %s %s!",
                  Amonnam(mtmp),
                  nolimbs(mtmp->data) ? "трясёт" : "машет",
                  mhis(mtmp),
                  nolimbs(mtmp->data)
                  ? mbodypart(mtmp, HEAD)
...
dryup(coordxy x, coordxy y, boolean isyou)
{
    if (IS_FOUNTAIN(levl[x][y].typ)
        ...
        if (isyou && in_town(x, y) && !FOUNTAIN_IS_WARNED(x, y)) {
            ...
            if (!mtmp)
                pline("Струя убывает до ручейка.");
            return;
        }
        if (isyou && wizard) {
            if (y_n("Высушить фонтан?") == 'n')
                return;
        }
...
drinkfountain(void)
{
    ...
    if (mgkftn && u.uluck >= 0 && fate >= 10) {
        ...
        pline("Ого!  Это заставляет вас чувствовать себя великолепно!");
        ...
        pline("Струйка пара вырывается из фонтана...");
        exercise(A_WIS, TRUE);
    ...
    if (fate < 10) {
        pline("Прохладный глоток освежает вас.");
        ...
        case 19: /* Self-knowledge */
            You_feel("самопознанием...");
            display_nhwindow(WIN_MESSAGE, FALSE);
            enlightenment(MAGICENLIGHTENMENT, ENL_GAMEINPROGRESS);
            exercise(A_WIS, TRUE);
            pline("Ощущение проходит.");
            break;
        case 20: /* Foul water */
            pline("Вода отвратительна!  Вы давитесь и блюёте.");
            morehungry(rn1(20, 11));
            vomit();
            break;
        case 21: /* Poisonous */
            pline("Вода загрязнена!");
        ...
        case 25: /* See invisible */
            if (Blind) {
                if (Invisible) {
                    You("чувствуете себя прозрачным.");
                } else {
                    You("чувствуете себя очень неловко.");
                    pline("Затем это проходит.");
                }
            } else {
                You_see("образ кого-то, крадущегося за вами.");
                pline("Но он исчезает.");
            }
        ...
        case 26: /* See Monsters */
            if (monster_detect((struct obj *) 0, 0))
                pline("%s На вкус ничего не имеет.", hliquid("Вода"));
            exercise(A_WIS, TRUE);
            break;
        ...
        case 29: /* Scare */
        {
            ...
            pline("Эта %s вызывает у вас несвежее дыхание!",
                  hliquid("вода"));
        ...
        case 30: /* Gushing forth in this room */
            ...
            break;
        default:
            pline("Эта едва тёплая %s безвкусна.",
                  hliquid("вода"));
            break;
        }
    }
...
dipfountain(struct obj *obj)
{
    ...
    if (obj->otyp == LONG_SWORD && u.ulevel >= 5
        ...
        static const char lady[] = "Леди Озера";

        if (u.ualign.type != A_LAWFUL) {
            /* Ha!  Trying to cheat her. */
            pline("Ледяной туман поднимается из %s"
                  " и окутывает меч.",
                  hliquid("вода"));
            pline("Фонтан исчезает!");
            curse(obj);
            if (obj->spe > -6 && !rn2(3))
                obj->spe--;
            ...
            livelog_printf(LL_ARTIFACT,
                           "было отказано в %s!  %s сочла %s недостойным",
                           artiname(ART_EXCALIBUR), lady, uhim());
        } else {
            /* The lady of the lake acts! - Eric Backus */
            /* Be *REAL* nice */
            pline(
              "Из мрачных глубин поднимается рука, чтобы благословить меч.");
            pline("Когда рука отступает, фонтан исчезает!");
            ...
            exercise(A_WIS, TRUE);
            livelog_printf(LL_ARTIFACT, "получил %s от %s",
    ...
    case 20: /* Uncurse the item */
        if (!is_hands && obj->cursed) {
            if (!Blind)
                pline("%s Светится мгновение.", hliquid("Вода"));
            uncurse(obj);
        } else {
            pline("Чувство утраты охватывает вас.");
        }
        break;
    case 21: /* Water Demon */
        dowaterdemon();
    ...
    case 26: /* Strange feeling */
        pline("Странное покалывание пробегает по вашей %s.", body_part(ARM));
        break;
    case 27: /* Strange feeling */
        You_feel("внезапный холод.");
        break;
    case 28: /* Strange feeling */
        pline("Неудержимое желание принять ванну охватывает вас.");
        {
            ...
            if (money > 10) {
                ...
                You("потеряли часть своего золота в фонтане!");
    ...
    case 29: /* You see coins */
        ...
        (void) mkgold((long) (rnd((dunlevs_in_dungeon(&u.uz) - dunlev(&u.uz)
                      ...
                      u.ux, u.uy);
        if (!Blind)
            pline("Далеко внизу вы видите поблёскивающие монеты в %s.",
                  hliquid("вода"));
        exercise(A_WIS, TRUE);
        newsym(u.ux, u.uy);
        break;
...
wash_hands(void)
{
    ...
    You("моете свои %s%s в %s.", uarmg ? "в перчатках " : "", hands,
        hliquid("вода"));
    if (Glib) {
        make_glib(0);
        Your("%s больше не скользкие.", fingers_or_gloves(TRUE));
    }
    if (uarmg)
...
breaksink(coordxy x, coordxy y)
{
    if (cansee(x, y) || u_at(x, y))
        pline("Трубы ломаются!  Вода брызжет!");
...
drinksink(void)
{
    ...
    if (Levitation) {
        floating_above("раковина");
        return;
    }
    switch (rn2(20)) {
    case 0:
        You("делаете глоток очень холодной %s.", hliquid("вода"));
        break;
    case 1:
        You("делаете глоток очень тёплой %s.", hliquid("вода"));
        break;
    case 2:
        You("делаете глоток обжигающе горячей %s.", hliquid("вода"));
        if (Fire_resistance) {
    ...
    case 3:
        if (svm.mvitals[PM_SEWER_RAT].mvflags & G_GONE)
            pline("Раковина кажется довольно грязной.");
        else {
            mtmp = makemon(&mons[PM_SEWER_RAT], u.ux, u.uy, MM_NOMSG);
            if (mtmp)
                pline("Ой!  В раковине %s!",
                      (Blind || !canspotmon(mtmp)) ? "что-то извивающееся"
                                                   : a_monnam(mtmp));
        }
        break;
    case 4:
        ...
        pline("Какая-то %s жидкость течёт из крана.",
              Blind ? "странная" : hcolor(OBJ_DESCR(objects[otmp->otyp])));
    ...
    case 5:
        if (!(levl[u.ux][u.uy].looted & S_LRING)) {
            You("находите кольцо в раковине!");
            (void) mkobj_at(RING_CLASS, u.ux, u.uy, TRUE);
            levl[u.ux][u.uy].looted |= S_LRING;
            exercise(A_WIS, TRUE);
            newsym(u.ux, u.uy);
        } else
            pline("Какая-то грязная %s поднимается в стоке.", hliquid("вода"));
        break;
    case 6:
        breaksink(u.ux, u.uy);
        break;
    case 7:
        pline("%s Движется так, будто по своей воле!", hliquid("Вода"));
        if ((svm.mvitals[PM_WATER_ELEMENTAL].mvflags & G_GONE)
            || !makemon(&mons[PM_WATER_ELEMENTAL], u.ux, u.uy, MM_NOMSG))
            pline("Но она успокаивается.");
        break;
    case 8:
        pline("Фу, эта %s ужасна на вкус.", hliquid("вода"));
        more_experienced(1, 0);
        newexplevel();
        break;
    case 9:
        pline("Гххх... это на вкус как сточные воды!  Вы блюёте.");
        morehungry(rn1(30 - ACURR(A_CON), 11));
        vomit();
        break;
    case 10:
        pline("Эта %s содержит токсичные отходы!", hliquid("вода"));
        if (!Unchanging) {
            You("претерпеваете причудливое превращение!");
    ...
    case 11:
        ...
        You_hear("лязг из труб...");
        break;
    case 12:
        Soundeffect(se_sewer_song, 100);
        You_hear("обрывки песни из канализации...");
        break;
    case 13:
        pline("Фу, какое зловоние!");
        create_gas_cloud(u.ux, u.uy, 1, 4);
        break;
    case 19:
        if (Hallucination) {
            pline("Из мрачного стока поднимается рука... --ой--");
            break;
        }
        FALLTHROUGH;
        /*FALLTHRU*/
    default:
        You("делаете глоток %s %s.",
            rn2(3) ? (rn2(2) ? "холодной" : "тёплой") : "горячей",
            hliquid("вода"));
    }
}
...
dipsink(struct obj *obj)
{
    ...
    if (!rn2(not_looted_yet ? 25 : 15)) {
        ...
        if (Glib && is_hands)
            Your("%s всё ещё скользкие.", fingers_or_gloves(TRUE));
        return;
    } else if (is_hands) {
        (void) wash_hands();
        return;
    } else if (obj->oclass != POTION_CLASS) {
        You("держите %s под краном.", the(xname(obj)));
    ...
    You("выливаете %s%s в сток.", (obj->quan > 1L ? "один из " : ""),
        the(xname(obj)));
    switch (obj->otyp) {
    case POT_POLYMORPH:
        polymorph_sink();
    ...
    case POT_OIL:
        if (!Blind) {
            pline("Она оставляет жирную плёнку на раковине.");
            try_call = TRUE;
        } else {
            pline1(nothing_seems_to_happen);
        }
    ...
    case POT_ACID:
        ...
        try_call = TRUE;
        if (!Blind) {
            pline("Сток кажется менее засорённым.");
        } else if (!Deaf) {
            You_hear("звук всасывания.");
        } else {
            pline1(nothing_seems_to_happen);
            try_call = FALSE;
        }
    ...
    case POT_OBJECT_DETECTION:
        if (!(levl[u.ux][u.uy].looted & S_LRING)) {
            You("чувствуете кольцо, потерянное в стоке.");
            try_call = TRUE;
            break;
        }
        FALLTHROUGH;
    ...
    default:
        ...
        pline("Струйка пара поднимается...");
...
sink_backs_up(coordxy x, coordxy y)
{
    ...
    if (!Blind)
        Strcpy(buf, "Грязные отходы выскакивают из стока");
    else if (!Deaf)
        Strcpy(buf, "Вы слышите плещущий звук"); /* Deaf-aware */
    else
        Sprintf(buf, "Что-то брызгает вам в %s", body_part(FACE));
    pline("%s%s.", !Deaf ? "Бульк!  " : "", buf);

    if (!(levl[x][y].looted & S_LRING)) { /* once per sink */
        if (!Blind)
            You_see("кольцо, сияющее в его глубине.");
