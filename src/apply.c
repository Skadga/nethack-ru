...
{
    if (Underwater) {
        pline("Использование камеры под водой аннулирует гарантию.");
        return ECMD_OK;
    }
    if (!getdir((char *) 0))
        return ECMD_CANCEL;
    ...
    if (obj->cursed && !rn2(2)) {
        ...
        You("фотографируете %s %s.", s_suffix(mon_nam(u.ustuck)),
            mbodypart(u.ustuck, STOMACH));
    } else if (u.dz) {
        You("фотографируете %s.",
...
use_towel(struct obj *obj)
{
    ...
    if (!freehand()) {
        You("не имеете свободной %s!", body_part(HAND));
        return ECMD_OK;
    } else if (obj == ublindf) {
        You("не можете использовать это, пока носите!");
        return ECMD_OK;
    } else if (obj->cursed) {
        long old;
        ...
        case 2:
            ...
            Your("%s %s!", makeplural(body_part(HAND)),
                 (old ? "грязнее прежнего" : "покрываются слизью"));
            if (is_wet_towel(obj))
        ...
        case 1:
            if (!ublindf) {
                old = u.ucreamed;
                u.ucreamed += rn1(10, 3);
                pline("Фу!  На вашей %s %s грязи!", body_part(FACE),
                      (old ? "стало больше" : "теперь появилось"));
                ...
                what = (ublindf->otyp == LENSES)
                           ...
                           : (obj->otyp == ublindf->otyp) ? "other towel"
                                                          : "blindfold";
                if (ublindf->cursed) {
                    You("поправляете свою %s %s.", what,
                        rn2(2) ? "набок" : "криво");
                } else {
                    struct obj *saved_ublindf = ublindf;
                    You("сдвигаете свою %s.", what);
                    Blindf_off(ublindf);
                    dropx(saved_ublindf);
                }
            }
    ...
    if (Glib) {
        ...
        u.ucreamed = 0;
        if (!Blinded) {
            pline("Вы счистили всю грязь.");
            if (!gulp_blnd_check()) {
                set_itimeout(&HBlinded, 1L);
                make_blinded(0L, TRUE);
            }
        } else {
            Your("%s теперь чистое.", body_part(FACE));
        }
        if (is_wet_towel(obj))
            dry_a_towel(obj, -1, drying_feedback);
        return ECMD_TIME;
    ...
    Your("%s и %s уже чистые.", body_part(FACE),
...
its_dead(coordxy rx, coordxy ry, int *resp)
{
    ...
    if (!corpse && !statue) {
        ...
        if (!corpse) {
            ...
            Strcpy(buf, "Они мертвы");
        }
        /* variations on "He's dead, Jim." (Star Trek's Dr McCoy) */
        You_hear("голос говорит: \"%s, Джим.\"", buf);
        *resp = ECMD_TIME;
        return TRUE;
        ...
        You("определяете, что %s несчастное существо%s %s%s мертво.",
            one ? (here ? "this" : "that") : (here ? "these" : "those"),
            one ? "" : "s", one ? "" : "", reviver ? " в основном" : "");
        return TRUE;
        ...
        if (Blind) { /* ignore statue->dknown; it'll always be set */
            Sprintf(buf, "%s %s",
                    u_at(rx, ry) ? "Это" : "То",
                    humanoid(mptr) ? "личность" : "существо");
            what = buf;
        } else {
        ...
        pline("%s в %s состоянии для статуи.", what, how);
        return TRUE;
    }
    return FALSE; /* no corpse or statue */
}

static const char hollow_str[] = "полый звук.  Это, должно быть, потайная %s!";
...
use_stethoscope(struct obj *obj)
{
    ...
    if (nohands(gy.youmonst.data)) {
        You("не имеете рук!"); /* not `body_part(HAND)' */
        return ECMD_OK;
    } else if (Deaf) {
        You_cant("ничего не слышать!");
        return ECMD_OK;
    } else if (!freehand()) {
        You("не имеете свободной %s.", body_part(HAND));
        return ECMD_OK;
    ...
    if (u.usteed && u.dz > 0) {
        if (interference) {
            pline("%s мешает.", Monnam(u.ustuck));
        ...
        if (Underwater) {
            ...
            You_hear("треск адского пламени.");
        } else {
            pline("%s Выглядит достаточно здоровым.", surface(u.ux, u.uy));
        }
        return res;
    } else if (obj->cursed && !rn2(2)) {
        Soundeffect(se_heart_beat, 100);
        You_hear("биение своего сердца.");
        return res;
    }
    confdir(FALSE);
    if (!u.dx && !u.dy) {
    ...
    if ((mtmp = m_at(rx, ry)) != 0) {
        ...
        if (mtmp->mundetected) {
            ...
            seemimic(mtmp);
            pline("%s %s %s на самом деле %s.",
                  use_plural ? "Те" : "То", what,
                  use_plural ? "есть" : "есть", mnm);
    ...
    if (unmap_invisible(rx,ry))
        pline("Невидимый монстр, должно быть, передвинулся.");

    lev = &levl[rx][ry];
    switch (lev->typ) {
    case SDOOR:
        Soundeffect(se_hollow_sound, 100);
        You_hear(hollow_str, "дверь");
        cvt_sdoor_to_door(lev); /* ->typ = DOOR */
        recalc_block_point(rx, ry);
        feel_newsym(rx, ry);
        return res;
    case SCORR:
        You_hear(hollow_str, "проход");
    ...
    if (!its_dead(rx, ry, &res))
        You("не слышите ничего особенного."); /* not You_hear()  */
    return res;
}

static const char whistle_str[] = "издаёте %s свист.",
                  alt_whistle_str[] = "издаёте %s, резкую вибрацию.";

staticfn void
use_whistle(struct obj *obj)
{
    if (!can_blow(&gy.youmonst)) {
        You("не можете использовать свисток.");
    } else if (Underwater) {
        You("пускаете пузыри сквозь %s.", yname(obj));
    } else {
        if (Deaf)
            You_feel("щекотание воздуха на %s.", body_part(NOSE));
        else
            You(whistle_str, obj->cursed ? "пронзительный" : "высокий");
...
use_magic_whistle(struct obj *obj)
{
    if (!can_blow(&gy.youmonst)) {
        ...
        You("издаёте %s%s.", Underwater ? "очень " : "",
            Deaf ? "высокочастотную вибрацию" : "высокочастотное гудение");
        wake_nearby(TRUE);
...
m_unleash(struct monst *mtmp, boolean feedback)
{
    ...
    if (feedback) {
        if (canseemon(mtmp))
            pline_mon(mtmp, "%s вырывается из %s поводка!",
...
use_leash(struct obj *obj)
{
    ...
    if (u.uswallow) {
        ...
        You_cant((!obj->leashmon
                  ...
                  : (obj->leashmon == (int) u.ustuck->m_id)
                    ? "отвязать %s изнутри."
                    : "отвязать что-нибудь изнутри %s."),
                 noit_mon_nam(u.ustuck));
        return ECMD_OK;
    }
    if (!obj->leashmon && number_leashed() >= MAXLEASHED) {
        You("не можете больше привязывать питомцев.");
        return ECMD_OK;
    }
    ...
    if (u_at(cc.x, cc.y)) {
        ...
        pline("Привязать себя?  Очень смешно...");
        return ECMD_OK;
    }
...
use_leash_core(struct obj *obj, struct monst *mtmp, coord *cc, int spotmon)
{
    if (!spotmon && !glyph_is_invisible(levl[cc->x][cc->y].glyph)) {
        ...
        You("не можете %sпривязать что-то.", obj->leashmon ? "от" : "");
        /* trying again will work provided the monster is tame
           (and also that it doesn't change location by retry time) */
        map_invisible(cc->x, cc->y);
    } else if (!mtmp->mtame) {
        pline("%s %s привязан!", Monnam(mtmp),
              (!obj->leashmon) ? "не может быть" : "не");
    } else if (!obj->leashmon) {
        /* applying a leash which isn't currently in use */
        if (mtmp->mleashed) {
            pline("Этот %s уже привязан.",
                  spotmon ? l_monnam(mtmp) : "существо");
        } else if (unsolid(mtmp->data)) {
            pline("Поводок просто соскользнёт.");
        } else if (nolimbs(mtmp->data) && !has_head(mtmp->data)) {
            pline("У %s нет конечностей, на которые наделся бы поводок.",
                  Monnam(mtmp));
            ...
            You("накидываете поводок на %s%s.", spotmon ? "вашу " : "",
                l_monnam(mtmp));
            mtmp->mleashed = 1;
        ...
        if (obj->leashmon != (int) mtmp->m_id) {
            pline("Этот поводок не прикреплён к тому существу.");
        } else if (obj->cursed) {
            pline("Поводок не снимается!");
            set_bknown(obj, 1);
        } else {
            mtmp->mleashed = 0;
            obj->leashmon = 0;
            update_inventory();
            You("снимаете поводок с %s%s.",
        ...
        if (dist2(u.ux, u.uy, mtmp->mx, mtmp->my)
            ...
            if (!um_dist(mtmp->mx, mtmp->my, 3)) {
                ...
                if (um_dist(mtmp->mx, mtmp->my, 5)
                    ...
                    Your("поводок душит %s насмерть!", mon_nam(mtmp));
                    ...
                    pline_mon(mtmp, "%s задыхается от поводка!",
                              Monnam(mtmp));
                ...
                if (um_dist(mtmp->mx, mtmp->my, 5)) {
                    pline("%s поводок лопается!", s_suffix(Monnam(mtmp)));
...
beautiful(void)
{
    ...
    res = ((cha >= 25) ? "возвышенно" /* 25 is the maximum possible */
           : (cha >= 19) ? "великолепно" /* note: not "splendiferous" */
             : (cha >= 16) ? ((poly_gender() == 1) ? "красиво" : "красиво")
               : (cha >= 14) ? ((poly_gender() == 1) ? "обаятельно" : "обаятельно")
                 : (cha >= 11) ? "мило"
                   : (cha >= 9) ? "просто"
                     : (cha >= 6) ? "неказисто"
                       : (cha >= 4) ? "уродливо"
                         : "отвратительно"); /* 3 is the minimum possible */
    return res;
}

static const char look_str[] = "выглядите %s.";
...
use_mirror(struct obj *obj)
{
    ...
    if (obj->cursed && !rn2(2)) {
        if (!Blind)
            pline("%s Запотевает и не отражает!", mirror);
        else
    ...
    if (!u.dx && !u.dy && !u.dz) {
        if (!useeit) {
            You_cant("видеть свою %s %s.", uvisage, body_part(FACE));
        } else {
            if (u.umonnum == PM_FLOATING_EYE) {
                if (Free_action) {
                    You("на миг застываете под своим взглядом.");
                } else {
                    if (Hallucination)
                        pline("Ого!  %s смотрит в ответ!", mirror);
                    else
                        pline("Ой!  Вы заморозили себя!");
                    if (!Hallucination || !rn2(4)) {
                        nomul(-rnd(MAXULEV + 6 - u.ulevel));
                        gm.multi_reason = "глядя в зеркало";
                    }
                    gn.nomovemsg = 0; /* default, "you can move again" */
                }
            } else if (is_vampire(gy.youmonst.data)
                       || is_vampshifter(&gy.youmonst)) {
                You("не имеете отражения.");
            } else if (u.umonnum == PM_UMBER_HULK) {
                pline("Хм?  Это не похоже на вас!");
                make_confused(HConfusion + d(3, 4), FALSE);
            } else if (Hallucination) {
                You(look_str, hcolor((char *) 0));
            } else if (Sick) {
                You(look_str, "измождённо");
            } else if (u.uhs >= WEAK) {
                You(look_str, "истощённо");
            } else if (Upolyd) {
                You("выглядите как %s.", an(pmname(&mons[u.umonnum], Ugender)));
            } else {
                You("выглядите так же %s, как всегда.", uvisage);
            }
        }
        return ECMD_TIME;
    }
    if (u.uswallow) {
        if (useeit)
            You("видишь отражение %s %s.", s_suffix(mon_nam(u.ustuck)),
                mbodypart(u.ustuck, STOMACH));
        return ECMD_TIME;
    }
    if (Underwater) {
        if (useeit)
            You("%s.",
                Hallucination ? "даёшь рыбе шанс поправить макияж"
                              : "видишь отражение мутной воды");
        return ECMD_TIME;
    }
    if (u.dz) {
        if (useeit)
            You("видишь отражение %s.",
                (u.dz > 0) ? surface(u.ux, u.uy) : ceiling(u.ux, u.uy));
        return ECMD_TIME;
    }
...
#define SEENMON (MONSEEN_NORMAL | MONSEEN_SEEINVIS | MONSEEN_INFRAVIS)
    ...
    if (mtmp->msleeping) {
        if (vis)
            pline("%s слишком устал, чтобы смотреть на вашу %s.", Monnam(mtmp),
                  mirror);
    } else if (!mtmp->mcansee) {
        if (vis)
            pline("%s сейчас ничего не видит.", Monnam(mtmp));
    } else if (invis_mirror && !perceives(mtmp->data)) {
        if (vis)
            pline("%s не замечает вашей %s.", Monnam(mtmp), mirror);
        ...
        if (vis)
            pline("У %s нет отражения.", Monnam(mtmp));
    } else if (monable && mtmp->data == &mons[PM_MEDUSA]) {
        if (mon_reflects(mtmp, "Взгляд отражён %s %s!"))
            return ECMD_TIME;
        if (vis)
            pline("%s превращается в камень!", Monnam(mtmp));
        gs.stoned = TRUE;
        killed(mtmp);
        ...
        if (!rn2(4))
            tmp = 120;
        if (vis)
            pline("%s замерзает от своего отражения.", Monnam(mtmp));
        else
            You_hear("%s перестаёт двигаться.", something);
        paralyze_monst(mtmp, (int) mtmp->mfrozen + tmp);
    } else if (monable && mtmp->data == &mons[PM_UMBER_HULK]) {
        if (vis)
            pline("%s сбивает себя с толку!", Monnam(mtmp));
        mtmp->mconf = 1;
        ...
        if (vis) {
            ...
            pline("%s в твоём %s.", /* "<mon> admires self in your mirror " */
                  monverbself(mtmp, Monnam(mtmp), "admire", (char *) 0),
                  mirror);
            pline("%s берёт его!", upstart(strcpy(buf, mhe(mtmp))));
        } else
            pline("Оно крадёт вашу %s!", mirror);
        ...
        if (mtmp->mfrozen) {
            if (vis)
                You("не замечаете очевидной реакции от %s.", mon_nam(mtmp));
            else
                You_feel(
                       "немного глупо указывать зеркалом в ту сторону.");
            do_react = FALSE;
        }
        if (do_react) {
            if (vis)
                pline("%s напуган своим отражением.", Monnam(mtmp));
        ...
        if (mtmp->minvis && !See_invisible)
            ...
            pline("%s, похоже, не замечает своё отражение.", Monnam(mtmp),
                  mhis(mtmp));
        else
            pline("%s игнорирует своё отражение.", Monnam(mtmp), mhis(mtmp));
    }
    return ECMD_TIME;
#undef SEENMON
}
...
use_bell(struct obj **optr)
{
    ...
    You("звоните в %s.", the(xname(obj)));

    if (Underwater || (u.uswallow && ordinary)) {
        pline("Но звук приглушён.");

    } else if (invoking && ordinary) {
        /* needs to be recharged... */
        pline("Но он не издаёт звука.");
        ...
        if (obj->cursed && !rn2(4)
            ...
            You("призываете %s!", a_monnam(mtmp));
            if (!obj_resists(obj, 93, 100)) {
                pline("%s разбилось вдребезги!", Tobjnam(obj, "have"));
                useup(obj);
                *optr = 0;
            } else
                switch (rn2(3)) {
        ...
        if (u.uswallow) {
            ...
            pline("%s издаёт тревожный пронзительный звук...", Tobjnam(obj, "issue"));
            obj->age = svm.moves;
            learno = TRUE;
            wakem = TRUE;
            ...
            case 0:
                ...
                break;
            case 1:
                pline("%s открывается...", Something);
                learno = TRUE;
                break;
            default:
                pline("Вокруг вас открываются вещи...");
                learno = TRUE;
                break;
            }
...
{
    const char *s = (obj->spe != 1) ? "свечи" : "свеча";
    ...
    if (obj->spe <= 0) {
        ...
        pline("В %s нет %s.", xname(obj), s);
        /* only output tip if candles are in inventory */
        for (otmp = gi.invent; otmp; otmp = otmp->nobj)
            if (Is_candle(otmp))
                break;
        if (otmp)
            pline("Чтобы прикрепить свечи, применяйте их вместо %s.",
                  xname(obj));
        return;
    }
    if (Underwater) {
        You("не можете разжечь огонь под водой.");
        return;
    }
    if (u.uswallow || obj->cursed) {
        if (!Blind)
            pline("%s %s Мгновение, затем %s.", s, vtense(s, "flicker"),
                      vtense(s, "die"));
        return;
    }
    if (obj->spe < 7) {
        There("%s только %d %s в %s.", vtense(s, "are"), obj->spe, s,
              the(xname(obj)));
        if (!Blind)
            pline("%s.  %s тускло.", obj->spe == 1 ? "Он горит" : "Они горят",
                  Tobjnam(obj, "shine"));
    } else {
        pline("%s: %s горят%s", The(xname(obj)), s,
              (Blind ? "." : " ярко!"));
    }
    if (!invocation_pos(u.ux, u.uy) || On_stairs(u.ux, u.uy)) {
        pline("%s %s Стремительно расходуются!", s, vtense(s, "are"));
        ...
        if (obj->age == 0) {
            impossible("Подсвечник со свечами, но без топлива?");
            obj->age = 1;
        }
    } else {
        if (obj->spe == 7) {
            if (Blind)
                pline("%s излучает странное тепло!", Tobjnam(obj, "radiate"));
            else
                pline("%s светится странным светом!", Tobjnam(obj, "glow"));
        }
        obj->known = 1;
    }
...
use_candle(struct obj **optr)
{
    struct obj *obj = *optr;
    struct obj *otmp;
    const char *s = (obj->quan != 1) ? "свечи" : "свеча";
    ...
    Sprintf(qsfx, " к\033%s?", thesimpleoname(otmp));
    /* next, format the candles as a prefix for the candelabrum */
    (void) safe_qbuf(qbuf, "Прикрепить ", qsfx, obj, yname, thesimpleoname, s);
    /* strip temporary candelabrum suffix */
    if ((q = strstri(qbuf, " к\033")) != 0)
        Strcpy(q, " к ");
    ...
    if (y_n(safe_qbuf(qbuf, qbuf, "?", otmp, yname, thesimpleoname, "it"))
        ...
        You("прикрепляешь %ld%s %s к %s.", obj->quan, !otmp->spe ? "" : " more", s,
            the(xname(otmp)));
        if (!otmp->spe || otmp->age > obj->age)
            otmp->age = obj->age;
        otmp->spe += (int) obj->quan;
        if (otmp->lamplit && !was_lamplit)
            pline("НОвые %s магически %s!", s, vtense(s, "ignite"));
        else if (!otmp->lamplit && was_lamplit)
            pline("%s.", (obj->quan > 1L) ? "Они гаснут" : "Оно гаснет");
        if (obj->unpaid) {
            struct monst *shkp VOICEONLY
                               = shop_keeper(*in_rooms(u.ux, u.uy, SHOPBASE));

            SetVoice(shkp, 0, 80, 0);
            verbalize("Ты %s %s, ты купил %s!",
                      otmp->lamplit ? "жжёшь" : "используешь",
                      (obj->quan > 1L) ? "их" : "его",
                      (obj->quan > 1L) ? "их" : "его");
        }
        if (obj->quan < 7L && otmp->spe == 7)
            pline("%s теперь имеет семь%s прикреплённых свечей.", The(xname(otmp)),
                  otmp->lamplit ? " зажжённых" : "");
        /* подсвечника light range might increase */
        if (otmp->lamplit)
            obj_merge_light_sources(otmp, otmp);
        /* candles are no longer a separate light source */
        /* candles are now gone */
        useupall(obj);
        /* подсвечника weight is changing */
...
snuff_candle(struct obj *otmp)
{
    ...
    if ((candle || otmp->otyp == CANDELABRUM_OF_INVOCATION)
        ...
        if (otmp->where == OBJ_MINVENT ? cansee(x, y) : !Blind)
            pline("%s%sсвеч%s пламя%s погашено.", Shk_Your(buf, otmp),
                  (candle ? "" : "подсвечника "), (many ? "s'" : "'s"),
                  (many ? "s are" : " is"));
        end_burn(otmp, TRUE);
        return TRUE;
    }
    ...
    if (obj->lamplit) {
        if (obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP
            ...
            if (obj->where == OBJ_MINVENT ? cansee(x, y) : !Blind)
                pline("%s %s гаснет!", Yname2(obj), otense(obj, "go"));
            end_burn(obj, TRUE);
            return TRUE;
        }
        if (snuff_candle(obj))
...
catch_lit(struct obj *obj)
{
    ...
    if (!obj->lamplit && ignitable(obj) && get_obj_location(obj, &x, &y, 0)) {
        ...
        if (carried(obj) && obj->unpaid && costly_spot(u.ux, u.uy)) {
            ...
            check_unpaid(obj);
            SetVoice(shkp, 0, 80, 0);
            verbalize("Это, конечно, в дополнение к стоимости %s %s.",
                      yname(obj),
...
use_lamp(struct obj *obj)
{
    ...
    if (obj->lamplit) {
        if (lamp) /* lamp or lantern */
            pline("%s%s теперь погашен.", Shk_Your(buf, obj), lamp);
        else
            You("гасишь %s.", yname(obj));
        end_burn(obj, TRUE);
        return;
    }
    if (Underwater) {
    ...
    if ((!Is_candle(obj) && obj->age == 0)
        ...
        if (obj->otyp == BRASS_LANTERN) {
            if (!Blind)
                Your("фонарь разряжен.");
            else
                pline("%s", nothing_seems_to_happen);
        } else {
            pline("В этом %s нет масла.", xname(obj));
        }
        return;
    }
    if (obj->cursed && !rn2(2)) {
        if ((obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP) && !rn2(3)) {
            pline("Лампа опрокидывается и заливает вашу %s маслом.",
                      fingers_or_gloves(TRUE));
            make_glib((int) (Glib & TIMEOUT) + d(2, 10));
        } else if (!Blind) {
            pline("%s мгновение, затем %s.", Tobjnam(obj, "flicker"),
                  otense(obj, "die"));
        } else {
            pline("%s", nothing_seems_to_happen);
        }
        ...
        if (lamp) { /* lamp or lantern */
            check_unpaid(obj);
            pline("%s%s теперь зажжён.", Shk_Your(buf, obj), lamp);
        } else { /* candle(s) */
            pline("%s пламя%s %s%s", s_suffix(Yname2(obj)), plur(obj->quan),
                  otense(obj, "burn"), Blind ? "." : " ярко!");
            if (obj->unpaid && costly_spot(u.ux, u.uy)
                && obj->age == 20L * (long) objects[obj->otyp].oc_cost) {
                const char *ithem = (obj->quan > 1L) ? "их" : "его";
                struct monst *shkp VOICEONLY
                               = shop_keeper(*in_rooms(u.ux, u.uy, SHOPBASE));

                SetVoice(shkp, 0, 80, 0);
                verbalize("Ты жжёшь %s, ты купил %s!", ithem, ithem);
                bill_dummy_object(obj);
            }
        }
        begin_burn(obj, FALSE);
...
light_cocktail(struct obj **optr)
{
    ...
    if (obj->lamplit) {
        ...
        There("недостаточно кислорода для поддержания огня.");
        return;
    }
    ...
    You("зажигаете %sзелье.%s", shk_your(buf, obj),
        Blind ? "" : "  Оно излучает тусклый свет.");
    ...
    if (obj->unpaid && costly_spot(u.ux, u.uy)) {
        ...
        check_unpaid(obj);
        SetVoice(shkp, 0, 80, 0);
        verbalize("Это, конечно, в дополнение к стоимости зелья.");
        bill_dummy_object(obj);
    }
    makeknown(obj->otyp);
    ...
    if (split1off) {
        ...
        obj->nomerge = 1;
        obj = hold_another_object(obj, "Ты роняешь %s!", doname(obj),
                                  (const char *) 0);
        if (obj)
            obj->nomerge = 0;
    }
...
dorub(void)
{
    ...
    if (nohands(gy.youmonst.data)) {
        You("не можешь тереть что-либо без рук.");
        return ECMD_OK;
    }
    obj = getobj("потереть", rub_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
    if (obj->oclass == GEM_CLASS || obj->oclass == FOOD_CLASS) {
        if (is_graystone(obj)) {
            ...
            pline("Извините, я не знаю, как это использовать.");
            return ECMD_OK;
        }
    }
    if (obj != uwep) {
    ...
    if (uwep->otyp == MAGIC_LAMP) {
        if (uwep->spe > 0 && !rn2(3)) {
            ...
            You("%s дым.", !Blind ? "видите клубок" : "чувствуете запах");
        } else
            pline1(nothing_happens);
    } else if (obj->otyp == BRASS_LANTERN) {
        /* message from Adventure */
        pline("Тереть электрическую лампу не особенно полезно.");
        pline("В любом случае, ничего интересного не происходит.");
    } else
        pline1(nothing_happens);
    return ECMD_TIME;
}
...
is_valid_jump_pos(coordxy x, coordxy y, int magic, boolean showmsg)
{
    if (!magic && !(HJumping & ~INTRINSIC) && !EJumping && distu(x, y) != 5) {
         ...
         */
        if (showmsg)
            pline("Недопустимый ход!");
        return FALSE;
    } else if (distu(x, y) > (magic ? 6 + magic * 3 : 9)) {
        if (showmsg)
            pline("Слишком далеко!");
        return FALSE;
    } else if (!isok(x, y)) {
        if (showmsg)
            You("не можете прыгнуть туда!");
        return FALSE;
    } else if (!cansee(x, y)) {
        if (showmsg)
            You("не видите, куда приземлиться!");
        return FALSE;
    } else {
        coord uc, tc;
        ...
        if (diag == jDiag && IS_DOOR(lev->typ)
            ...
            && (traj == jDiag
                || ((traj & jHorz) != 0) == (lev->horizontal != 0))) {
            if (showmsg)
                You_cant("прыгать по диагонали из дверного проёма.");
            return FALSE;
        }
        uc.x = u.ux, uc.y = u.uy;
        ...
        if (!walk_path(&uc, &tc, check_jump, (genericptr_t) &traj)) {
            if (showmsg)
                There("имеется препятствие, мешающее этому прыжку.");
            return FALSE;
        }
    }
    return TRUE;
...
jump(int magic) /* 0=Physical, otherwise skill level */
{
    ...
    if (!magic && (nolimbs(gy.youmonst.data) || slithy(gy.youmonst.data))) {
        ...
        You_cant("прыгать; у вас нет ног!");
        return ECMD_OK;
    } else if (!magic && !Jumping) {
        You_cant("прыгать очень далеко.");
        return ECMD_OK;
    ...
    /* if steed is immobile, can't do physical jump but can do spell one */
        ...
        if (magic) {
            You("немного подпрыгиваете.");
            return ECMD_TIME;
        }
        pline("Вы, должно быть, шутите!");
        return ECMD_OK;
    } else if (u.uinwater) {
        if (magic) {
            You("немного барахтаетесь.");
            return ECMD_TIME;
        }
        pline("Здесь нужно плавать, а не прыгать!");
        return ECMD_OK;
    } else if (u.ustuck) {
        if (u.ustuck->mtame && !Conflict && !u.ustuck->mconf) {
            ...
            You("вырываетесь от %s.", mon_nam(mtmp));
            return ECMD_TIME;
        }
        if (magic) {
            You("немного извиваетесь в хватке %s!", mon_nam(u.ustuck));
            return ECMD_TIME;
        }
        You("не можете вырваться от %s!", mon_nam(u.ustuck));
        return ECMD_OK;
    } else if (Levitation || Is_airlevel(&u.uz) || Is_waterlevel(&u.uz)) {
        if (magic) {
            You("немного молотите руками.");
            return ECMD_TIME;
        }
        You("не имеете достаточной опоры, чтобы прыгнуть.");
        return ECMD_OK;
    } else if (!magic && near_capacity() > UNENCUMBERED) {
        You("несёте слишком много, чтобы прыгнуть!");
        return ECMD_OK;
    } else if (!magic && (u.uhunger <= 100 || ACURR(A_STR) < 6)) {
        You("не хватает сил, чтобы прыгнуть!");
        return ECMD_OK;
    } else if (!magic && Wounded_legs) {
        legs_in_no_shape("прыжка", u.usteed != 0);
        return ECMD_OK;
    } else if (u.usteed && u.utrap) {
        pline("%s застрял в ловушке.", Monnam(u.usteed));
        return ECMD_OK;
    }

    pline("Куда вы хотите прыгнуть?");
    cc.x = u.ux;
    cc.y = u.uy;
    gj.jumping_is_magic = magic;
    getpos_sethilite(display_jump_positions, get_valid_jump_position);
    if (getpos(&cc, TRUE, "желаемое положение") < 0)
        return ECMD_CANCEL; /* user pressed ESC */
    if (!is_valid_jump_pos(cc.x, cc.y, magic, TRUE)) {
        return ECMD_FAIL;
    } else if (u.usteed && u_at(cc.x, cc.y)) {
        pline("%s не способен прыгать на месте.", YMonnam(u.usteed));
        return ECMD_FAIL;
    } else {
        coord uc;
        long side;
        ...
        if (u.utrap) {
            ...
            case TT_BEARTRAP:
                ...
                break;
            case TT_PIT:
                You("выпрыгиваете из ямы!");
                break;
            case TT_WEB:
                You("разрываете паутину, вырываясь!");
                deltrap(t_at(u.ux, u.uy));
                break;
            case TT_LAVA:
                You("вытаскиваете себя над %s!", hliquid("lava"));
                cc.x = u.ux, cc.y = u.uy; /* take u_at() 'if' below */
                break;
            case TT_BURIEDBALL:
            case TT_INFLOOR:
                You("напрягаете свои %s, но всё ещё %s.",
                    makeplural(body_part(LEG)),
                    (u.utraptype == TT_INFLOOR)
                        ? "застряли в полу"
                        : "прикованы к закопанному шару");
        ...
        if (u_at(cc.x, cc.y)) {
            ...
            if ((t = t_at(cc.x, cc.y)) != 0) {
                You("подпрыгиваете и %s обратно вниз.", !Flying ? "опускаетесь" : "слетаете");
                dotrap(t, FORCETRAP | TOOKPLUNGE);
                return ECMD_TIME;
            }
            /* jumping in place takes no time and doesn't exercise anything */
            You("%s.", Hallucination ? "немного подпрыгиваете"
                                     : "решаете в конце концов не прыгать");
            return ECMD_OK;
        }
...
use_tinning_kit(struct obj *obj)
{
     ...
     */
    if (obj->spe <= 0) {
        You("похоже, у вас кончились консервные банки.");
        return;
    }
    if (!(corpse = floorfood("tin", 2)))
        return;
    if (corpse->oeaten) {
        You("не можете законсервировать %s, который частично съеден.", something);
        return;
    }
    mptr = &mons[corpse->corpsenm];
    if (touch_petrifies(mptr) && !Stone_resistance && !uarmg) {
        ...
        if (poly_when_stoned(gy.youmonst.data)) {
            You("консервируете %s без перчаток.", corpse_name);
            kbuf[0] = '\0';
        } else {
            pline("Консервировать %s без перчаток — роковая ошибка...",
                  corpse_name);
            Sprintf(kbuf, "пытаясь законсервировать %s без перчаток", corpse_name);
        }
        instapetrify(kbuf);
    }
    if (is_rider(mptr)) {
        if (revive_corpse(corpse))
            verbalize("Да...  Но Война не сохраняет своих врагов...");
        else
            pline("Труп ускользает из ваших рук.");
        return;
    }
    if (mptr->cnutrit == 0) {
        pline("Это слишком бесплотно, чтобы консервировать.");
        return;
    }
    consume_obj_charge(obj, TRUE);

    if ((can = mksobj(TIN, FALSE, FALSE)) != 0) {
        static const char you_buy_it[] = "Вы законсервировали его — вы его купили!";
        ...
        (void) hold_another_object(can, "Вы создали, но не можете поднять, %s.",
...
#define PROP_COUNT 7           /* number of properties we're dealing with */
    ...
    if (obj && obj->cursed) {
        ...
        case 2:
            if (!Confusion)
                You("внезапно чувствуешь %s.",
                    Hallucination ? "странное" : "замешательство");
    ...
    if (mtmp) {
        ...
        case OBJ_FLOOR:
            if (cansee_spot && !silent) {
                ...
                if (suppress_see)
                    pline("%s внезапно исчезает!", an(xname(figurine)));
                else
                    You_see("статуэтка превращается в %s%s!", monnambuf,
                            and_vanish);
...
figurine_location_checks(struct obj *obj, coord *cc, boolean quietly)
{
    ...
    if (!isok(x, y)) {
        if (!quietly)
            You("не можете поставить статуэтку туда.");
        return FALSE;
    }
    if (IS_OBSTRUCTED(levl[x][y].typ)
        && !(passes_walls(&mons[obj->corpsenm]) && may_passwall(x, y))) {
        if (!quietly)
            You("не можете поместить статуэтку в %s!",
                IS_TREE(levl[x][y].typ) ? "дерево" : "твёрдую скалу");
        return FALSE;
    }
    if (sobj_at(BOULDER, x, y) && !passes_walls(&mons[obj->corpsenm])
        && !throws_rocks(&mons[obj->corpsenm])) {
        if (!quietly)
            You("не можете разместить статуэтку на валуне.");
        return FALSE;
    }
    return TRUE;
}
...
use_figurine(struct obj **optr)
{
    ...
    if (!figurine_location_checks(obj, &cc, FALSE))
        return ECMD_TIME;
    You("%s, и она%s превращается.",
        (u.dx || u.dy) ? "ставишь фигурку рядом с собой"
                       : (Is_airlevel(&u.uz) || Is_waterlevel(&u.uz)
                          || is_pool(cc.x, cc.y))
                             ? "отпускаешь фигурку"
                             : (u.dz < 0 ? "подбрасываешь фигурку в воздух"
                                         : "ставишь фигурку на землю"),
        Blind ? "предположительно " : "");
...
use_grease(struct obj *obj)
{
    ...
    if (Glib) {
        pline("%s из твоих %s.", Tobjnam(obj, "slip"),
    ...
    if (obj->spe > 0) {
        ...
        otmp = getobj("смазать", grease_ok, GETOBJ_PROMPT);
        if (!otmp)
            return ECMD_CANCEL;
        if (inaccessible_equipment(otmp, "смазать", FALSE))
            return ECMD_OK;
        consume_obj_charge(obj, TRUE);

        oldglib = (int) (Glib & TIMEOUT);
        if (otmp != &hands_obj) {
            You("покрываешь %s толстым слоем смазки.", yname(otmp));
            otmp->greased = 1;
            if (obj->cursed && !nohands(gy.youmonst.data)) {
                make_glib(oldglib + rn1(6, 10)); /* + 10..15 */
                pline("Часть смазки попадает на твои %s.",
                      fingers_or_gloves(TRUE));
            }
        } else {
            make_glib(oldglib + rn1(11, 5)); /* + 5..15 */
            You("покрываешь свои %s смазкой.", fingers_or_gloves(TRUE));
        }
    } else {
        if (obj->known)
            pline("%s пуст.", Tobjnam(obj, "are"));
        else
            pline("%s кажется пустым.", Tobjnam(obj, "seem"));
    }
    update_inventory();
    return ECMD_TIME;
}
...
use_stone(struct obj *tstone)
{
    ...
    if (obj == tstone && obj->quan == 1L) {
        You_cant("тереть %s на нём самом.", the(xname(obj)));
        return ECMD_OK;
    }
    ...
    if (tstone->otyp == TOUCHSTONE && tstone->cursed
        ...
        if (Blind)
            You_feel("что-то разлетается вдребезги.");
        else if (Hallucination)
            pline("Ого, посмотрите на красивые осколки.");
        else
            pline("Резкий треск разбивает %s%s.",
    ...
    default:
        ...
        case CLOTH:
            pline("%s теперь немного более отполирован.", Tobjnam(tstone, "look"));
            return ECMD_TIME;
        case LIQUID:
            if (!obj->known) /* note: not "whetstone" */
                You("должно быть, думаете, что это мокрый камень, да?");
            else
                pline("%s теперь немного мокрее.", Tobjnam(tstone, "are"));
            return ECMD_TIME;
        case WAX:
            streak_color = "восковые";
            break; /* okay even if not touchstone */
        case WOOD:
            streak_color = "деревянные";
            break; /* okay even if not touchstone */
        case GOLD:
            do_scratch = TRUE; /* scratching and streaks */
            streak_color = "золотые";
            break;
        case SILVER:
            do_scratch = TRUE; /* scratching and streaks */
            streak_color = "серебристые";
            break;
        default:
    ...
    if (do_scratch)
        You("оставляешь %s%sцарапины на %s.",
            streak_color ? streak_color : (const char *) "",
            streak_color ? " " : "", stonebuf);
    else if (streak_color)
        You_see("%s полосы на %s.", streak_color, stonebuf);
    else
        pline(scritch);
    return ECMD_TIME;
}
...
use_trap(struct obj *otmp)
{
    ...
    const char *occutext = "устанавливая ловушку";
    ...
    if (nohands(gy.youmonst.data))
        ...
        what = digests(u.ustuck->data) ? "будучи проглоченным" : "будучи поглощённым";
    else if (Underwater)
        what = "под водой";
    else if (Levitation)
        what = "в левитации";
        ...
        what = stway->isladder ? "на лестнице" : "на ступенях";
    ...
    if (what) {
        You_cant("устанавливать ловушку %s!", what);
        reset_trapset();
        return;
    }
    ttyp = (otmp->otyp == LAND_MINE) ? LANDMINE : BEAR_TRAP;
    if (otmp == gt.trapinfo.tobj && u_at(gt.trapinfo.tx, gt.trapinfo.ty)) {
        You("продолжаете устанавливать %s%s.", shk_your(buf, otmp),
            trapname(ttyp, FALSE));
    ...
    if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
        ...
        if (Fumbling || otmp->cursed)
            ...
            chance = (rnl(10) > 5);
        You("не очень умелы в доставании с %s.", mon_nam(u.usteed));
        Sprintf(buf, "Продолжить попытку установить %s?",
        ...
        if (y_n(buf) == 'y') {
            if (chance) {
                ...
                case BEAR_TRAP: /* drop it without arming it */
                    reset_trapset();
                    You("роняете %s!", the(trapname(ttyp, FALSE)));
                    dropx(otmp);
                    return;
                }
            }
    ...
    You("начинаете устанавливать %s%s.", shk_your(buf, otmp), trapname(ttyp, FALSE));
...
set_trap(void)
{
    ...
    if (ttmp) {
        ...
        if (!gt.trapinfo.force_bungle)
            You("заканчиваете установку %s.", the(trapname(ttyp, FALSE)));
        ...
        Your("попытка установки ловушки провалилась.");
    }
    useup(otmp);
    reset_trapset();
    return 0;
...
use_whip(struct obj *obj)
{
    ...
    const char *msg_slipsfree = "Хлыст соскальзывает.";
    const char *msg_snap = "Щёлк!";
    ...
    if (u.uswallow) {
        There("is недостаточно места, чтобы щёлкнуть хлыстом.");

    } else if (Underwater) {
        There("is слишком большое сопротивление, чтобы щёлкнуть хлыстом.");

    } else if (u.dz < 0) {
        You("смахиваешь жука с %s.", ceiling(u.ux, u.uy));

    } else if (!u.dz && (IS_WATERWALL(levl[rx][ry].typ)
                         || levl[rx][ry].typ == LAVAWALL)) {
        You("вызываешь небольшой всплеск.");
        ...
        if (u.usteed && !rn2(proficient + 2)) {
            You("хлестаешь %s!", mon_nam(u.usteed));
            kick_steed();
            return ECMD_TIME;
        }
        ...
        if (Levitation || u.usteed || Flying) {
            ...
            if (otmp && otmp->otyp == CORPSE
                ...
                pline("Зачем бить мёртвую лошадь?");
                return ECMD_TIME;
            }
            if (otmp && proficient) {
                You("обматываешь хлыст вокруг %s на %s.",
        ...
        if (dam <= 0)
            dam = 1;
        You("ударяешь себя по %s хлыстом.", body_part(FOOT));
        Sprintf(buf, "убил себя с помощью %s хлыста", uhim(), uhis());
        losehp(Maybe_Half_Phys(dam), buf, NO_KILLER_PREFIX);
        return ECMD_TIME;

    } else if ((Fumbling || Glib) && !rn2(5)) {
        pline("Хлыст выскальзывает из твоих %s.", body_part(HAND));
        dropx(obj);
        ...
        if (wrapped_what) {
            ...
            cc.x = rx;
            cc.y = ry;
            You("обматываешь хлыст вокруг %s.", wrapped_what);
            if (proficient && rn2(proficient + 2)) {
                if (!mtmp || enexto(&cc, rx, ry, gy.youmonst.data)) {
                    You("вытаскиваешь себя из ямы!");
 ...
 whipattack:
        ...
        if (!canspotmon(mtmp)) {
            ...
            if (spotitnow || !glyph_is_invisible(levl[rx][ry].glyph)) {
                pline("Там есть %s, которого ты %s.",
                      !spotitnow ? "нечто" : Amonnam(mtmp),
                      !Blind ? "не смог увидеть" : "не заметил");
                if (!spotitnow)
                    map_invisible(rx, ry);
                else
                    newsym(rx, ry);
        ...
        if (otmp) {
            ...
            You("обматываешь хлыст вокруг %s.", yname(otmp));
            if (gotit && mwelded(otmp)) {
                pline("%s к %s %s%c",
                      (otmp->quan == 1L) ? "Он горит" : "Они горят", mhis(mtmp),
            ...
            if (gotit) {
                ...
                case 2:
                    /* to floor near you */
                    You("вытаскиваешь %s к %s!", yname(otmp),
                        surface(u.ux, u.uy));
                ...
                case 3:
                    ...
                    if (!rn2(25)) {
                        ...
                        hitu = thitu(hitvalu, Maybe_Half_Phys(dam),
                                     &otmp, (char *) 0);
                        if (hitu) {
                            pline("%s Бьёт тебя, когда ты пытаешься схватить его!",
                                      the(onambuf));
                        }
                        place_object(otmp, u.ux, u.uy);
                        stackobj(otmp);
                    ...
                    if (otmp->otyp == CORPSE
                        ...
                        Strcpy(kbuf, (otmp->quan == 1L) ? an(onambuf)
                                                        : onambuf);
                        pline("Подхватить %s - фатальная ошибка.", kbuf);
                    ...
                    (void) hold_another_object(otmp, "Ты роняешь %s!",
                                               doname(otmp), (const char *) 0);
                    break;
                default:
                    /* to floor beneath mon */
                    You("вырываешь %s из %s %s!", the(onambuf),
            ...
            if (M_AP_TYPE(mtmp) && !Protection_from_shape_changers
                ...
                You("щелкаешь хлыстом в сторону %s.", mon_nam(mtmp));
            }
        ...
        You("щелкаешь хлыстом в воздухе.");
...
static const char
    not_enough_room[] = "Здесь недостаточно места, чтобы использовать это.",
    where_to_hit[] = "Куда ты хочешь ударить?",
    cant_see_spot[] = "не попадёшь ни во что, если не видишь это место.",
    cant_reach[] = "не можешь дотянуться до этого места отсюда.";
...
{
    const char thump[] = "Бум!  Твой удар безвредно отскакивает от %s.";
    ...
    if (distu(cc.x, cc.y) > max_range) {
        ...
        if (autohit && u_at(cc.x, cc.y))
            pline("Не знаю, по чему ударить.");
        else
            pline("Слишком близко!");
        return ECMD_FAIL;
    ...
    if ((mtmp = m_at(gb.bhitpos.x, gb.bhitpos.y)) != (struct monst *) 0) {
        if (attack_checks(mtmp, uwep)) /* can attack proceed? */
            /* no, abort the attack attempt; result depends on
               ...
               0 => answered 'n' к "Really attack?" prompt */
        ...
        if (snickersnee_used_dist_attk(obj)) {
            pline("Клинок не дотягивается туда!");
            return ECMD_FAIL;
        }
        ...
        if (glyph_to_obj(glyph) == BOULDER
            ...
            You("бесполезно атакуешь %s.",
                ...
                : glyph_is_cmap(glyph)
                  ...
                  : (const char *) "неизвестное препятствие");
        } else {
            You("промахиваешься; там некого бить.");
        }
    }
...
use_cream_pie(struct obj *obj)
{
    ...
    if (Hallucination)
        You("устраиваешь себе кремовую маску.");
    else
        You("окунаешь своё %s в %s%s.", body_part(FACE),
              several ? "one of " : "",
    ...
    if (can_blnd((struct monst *) 0, &gy.youmonst, AT_WEAP, obj)) {
        ...
        if (!Blind || (Blind && wasblind))
            pline("По всему твоему %s %sлипкая дрянь.",
                  wascreamed ? "ещё " : "", body_part(FACE));
        else /* Blind  && !wasblind */
            You_cant("видеть сквозь липкую дрянь на своём %s.",
                     body_part(FACE));
    }
...
use_royal_jelly(struct obj **optr)
{
    ...
    eobj = getobj("натереть королевским желе", jelly_ok, GETOBJ_PROMPT);
    if (!eobj) {
        if (splitit) {
            (void) unsplitobj(obj);
    ...
    You("размазываешь королевское желе по всему %s.", yname(eobj));
    if (eobj->otyp != EGG) {
        pline1(nothing_happens);
        goto useup_jelly;
    }
    ...
    if (obj->cursed) {
        if (eobj->timed || eobj->corpsenm != oldcorpsenm)
            pline("%s %s слабо.", xname(eobj), otense(eobj, "quiver"));
        else
            pline("%s", nothing_seems_to_happen);
        kill_egg(eobj);
        goto useup_jelly;
    ...
    if ((eobj->timed && !was_timed) || eobj->spe == 2
        ...
        pline("%s %s недолго.", xname(eobj), otense(eobj, "quiver"));
    else
        pline("%s", nothing_seems_to_happen);
...
use_grapple(struct obj *obj)
{
    ...
    if (typ != P_NONE && P_SKILL(typ) >= P_SKILLED) {
        ...
        any.a_int++;
        Sprintf(buf, "предмет на %s", surface(cc.x, cc.y));
    ...
    case 1: /* Object */
        if ((otmp = svl.level.objects[cc.x][cc.y]) != 0) {
            You("вытаскиваешь объект с %s!", surface(cc.x, cc.y));
    ...
    case 2: /* Monster */
        ...
        if (verysmall(mtmp->data) && !rn2(4)
            ...
            You("подтягиваешь %s!", mon_nam(mtmp));
            mtmp->mundetected = 0;
    ...
    case 3: /* Surface */
        if (IS_AIR(levl[cc.x][cc.y].typ) || is_pool(cc.x, cc.y))
            pline("Крюк прорезает %s.", surface(cc.x, cc.y));
        else {
            You("тебя тянет рывком к %s!", surface(cc.x, cc.y));
...
#define BY_OBJECT ((struct monst *) 0)
    ...
    if (nohands(gy.youmonst.data)) {
        You_cant("сломать %s без рук!", yname(obj));
        return ECMD_OK;
    } else if (!freehand()) {
        Your("%s заняты!", makeplural(body_part(HAND)));
        return ECMD_OK;
    } else if (ACURR(A_STR) < (is_fragile ? 5 : 10)) {
        You("не хватает сил сломать %s!", yname(obj));
        return ECMD_OK;
    }
    if (!paranoid_query(ParanoidBreakwand,
                        safe_qbuf(confirm,
                                  "Ты действительно хочешь сломать ",
                                  "?", obj, yname, ysimple_name, "the wand")))
        return ECMD_OK;
    pline("Подняв %s высоко над твоей %s, ты %s его пополам!", yname(obj),
          body_part(HEAD), is_fragile ? "ломаешь" : "ломаешь");
    ...
    obj->spe++;
    /* might have "wrested" a final charge, taking it from 0 к -1;
    ...
    case WAN_STRIKING:
        ...
        pline("Стена силы обрушивается вокруг тебя!");
    ...
    for (i = 0; i <= N_DIRS; i++) {
        ...
        if (obj->otyp == WAN_DIGGING) {
            ...
            if (dcres < DIGCHECK_FAILED || dcres == DIGCHECK_FAIL_BOULDER) {
                ...
                if (typ != ROOM) {
                    ...
                    liquid_flow(x, y, typ, t_at(x, y),
                                fillmsg
                                  ? (char *) 0
                                  : "Некоторые ямы быстро заполняются %s!");
                    fillmsg = TRUE;
                } else {
            ...
            if (damage) {
                Sprintf(buf, "убил себя, сломав жезл", uhim());
    ...
    if (shop_damage)
        pay_for_damage("врываться в", FALSE);
...
doapply(void)
{
    ...
    if (nohands(gy.youmonst.data)) {
        You("не можешь использовать или применять инструменты в текущем облике.");
        return ECMD_OK;
    }
    if (check_capacity((char *) 0))
        return ECMD_OK;

    obj = getobj("использовать", apply_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
    ...
    case LENSES:
        if (obj == ublindf) {
            ...
            You("уже %s.",
                (ublindf->otyp == TOWEL) ? "накрыт полотенцем"
                : (ublindf->otyp == BLINDFOLD) ? "носишь повязку"
                  : "в очках");
        }
        break;
    case CREAM_PIE:
    ...
    case BANANA:
        if (Hallucination) {
            pline("Он звонит! ... Но никто не отвечает.");
            break;
        }
        FALLTHROUGH;
        /*FALLTHRU*/
...
{
    if (Underwater) {
        You("не хочешь намочить страницы ещё сильнее?");
        return ECMD_OK;
    }

    You("пролистываешь страницы %s.", thesimpleoname(obj));
    ...
    if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
        if (!Deaf) {
            ...
            You_hear("страницы издают неприятный %s звук.",
                     Hallucination ? "хихикающий"
                                   : "шуршащий");
        } else if (!Blind) {
            You_see("страницы слабо светятся %s.", hcolor(NH_RED));
        } else {
            You_feel("страницы дрожат.");
        }
    } else if (Blind) {
        pline("Страницы на ощупь %s.",
              Hallucination ? "свежесорванные"
                            : "шершавые и сухие");
    } else if (obj->otyp == SPE_BLANK_PAPER) {
        pline("В этой книге заклинаний %s.",
              Hallucination ? "не очень-то много сюжета"
                            : "в ней ничего не написано");
        makeknown(obj->otyp);
    } else if (Hallucination) {
        You("наслаждаешься ожившими инициалами.");
    } else if (obj->otyp == SPE_NOVEL) {
        pline("Похоже, её может быть интересно почитать.");
    } else {
        static const char *const fadeness[] = {
            "свежие",
            "слегка выцветшие",
            "сильно выцветшие",
            "чрезвычайно выцветшие",
            "едва различимые"
        };
        int findx = min(obj->spestudied, MAX_SPELL_STUDY);

        pline("Чернила%s в этой книге заклинаний %s.",
              objects[obj->otyp].oc_magic ? " волшебные" : "",
              fadeness[findx]);
    }
...
flip_coin(struct obj *obj)
{
    ...
    You("подбрасываешь %s.", an(singular(obj, xname)));
    if (Underwater) {
        pline("Она укатывается прочь.");
        lose_coin = TRUE;
    } else if (Glib || Fumbling
               || (ACURR(A_DEX) < 10 && !rn2(ACURR(A_DEX)))) {
        pline("Она выскальзывает из твоих %s.", fingers_or_gloves(FALSE));
        lose_coin = TRUE;
    }
    ...
    if (Hallucination) {
        pline(rn2(100) ? "Ого, двуглавый орёл!"
                        /* edge case */
                       : "Монета чудесным образом встаёт на ребро!");
    } else {
        pline("Выпадает %s.", rn2(2) ? "орёл" : "решка");
    }
    return ECMD_TIME;
}
