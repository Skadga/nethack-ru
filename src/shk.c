...
static const char and_its_contents[] = " и его содержимое";
static const char the_contents_of[] = "содержимое ";
...
static const char *const angrytexts[] = {
    "весьма взволнован", "разозлён", "вне себя от ярости"
};
...
money2u(struct monst *mon, long amount)
{
    ...
    if (!merge_choice(gi.invent, mongold)
        ...
        You("не найдётся места для золота!");
        dropy(mongold);
    } else {
        addinv(mongold);
        disp.botl = TRUE;
...
append_price_quote(char *buf, char **eos, int otyp) {
    ...
    if (objects[otyp].oc_sell_minseen < objects[otyp].oc_sell_maxseen) {
        ...
        eos2 += sprintf(eos2, "%sпродать %lu", sep,
                        objects[otyp].oc_sell_minseen);
    }
...
call_kops(struct monst *shkp, boolean nearshop)
{
    ...
    if (!Deaf)
        pline("Раздаётся сигнал тревоги!");
    ...
    if (!angry_guards(!!Deaf) && nokops) {
        if (flags.verbose && !Deaf)
            pline("Но никто, похоже, не откликается.");
        return;
    }
...

    {
        ...
        if (flags.verbose)
            pline("КИстоун-копы охотятся за вами!");
...
u_left_shop(char *leavestring, boolean newlev)
{
    ...
    if (!*leavestring && !muteshk(shkp)) {
        ...
        if (!Deaf && !muteshk(shkp)) {
            ...
            verbalize(not_upset ? "%s!  Пожалуйста, заплатите, прежде чем уйти."
                                : "%s!  Не уходите, не заплатив!",
                      svp.plname);
        } else {
            pline("%s %s, что вам нужно заплатить до ухода%s",
                  Shknam(shkp),
                  not_upset ? "указывает" : "даёт понять",
                  not_upset ? "." : "!");
        }
        return;
    }
...
credit_report(struct monst *shkp, int idx, boolean silent)
{
    ...
    if (idx && !silent) {
        ...
        if (amt)
            Your("%s на %ld %s.", msg, amt, currency(amt));
...
rob_shop(struct monst *shkp)
{
    ...
    if (eshkp->credit >= total) {
        Your("кредит в %ld %s использован для покрытия вашего счёта.",
             eshkp->credit, currency(eshkp->credit));
        total = 0L; /* credit gets cleared by setpaid() */
    } else {
        You("сбежали из лавки, не заплатив!");
        total -= eshkp->credit;
    }
    setpaid(shkp);
    if (!total)
    ...
    eshkp->robbed += total;
    You("украли товара на %ld %s.", total, currency(total));
    livelog_printf(LL_ACHIEVE, "украл товара на %ld %s из %s %s",
...
deserted_shop(/*const*/ char *enterstring)
{
    ...
    pline("Эта лавка%s%s.", (m < n) ? ", по-видимому, " : "",
          !n ? "заброшена" : "без присмотра");
}
...
u_entered_shop(char *enterstring)
{
    ...
    if (Invis) {
        pline("%s ощущает ваше присутствие.", Shknam(shkp));
        if (!Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Невидимые покупатели нежелательны!");
        } else {
            pline("%s стоит неподвижно, словно %s знает, что вы здесь.",
    ...
    if (ANGRY(shkp)) {
        if (!Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Итак, %s, вы посмели вернуться в %s %s?!", svp.plname,
                      s_suffix(shkname(shkp)), shtypes[rt - SHOPBASE].name);
        } else {
            pline("%s, похоже, %s из-за вашего возвращения в %s %s!",
        ...
        if (!Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Опять вы, %s?  Я не спускаю с вас %s.",
                      svp.plname, mbodypart(shkp, EYE));
        } else {
            pline("Атмосфера в %s %s недружелюбная.",
                      s_suffix(shkname(shkp)), shtypes[rt - SHOPBASE].name);
        }
    } else if (eshkp->robbed) {
        if (!Deaf) {
            Soundeffect(se_mutter_imprecations, 50);
            pline("%s бормочет проклятия в адрес воров.",
                  Shknam(shkp));
        } else {
            pline("%s просматривает %s список товаров.",
                  Shknam(shkp), noit_mhis(shkp));
        }
    } else {
        if (!Deaf && !muteshk(shkp)) {
            set_voice(shkp, 0, 80, 0);
            verbalize("%s, %s!  Добро пожаловать%s в %s %s!", Hello(shkp), svp.plname,
                      eshkp->visitct++ ? " снова" : "",
                      s_suffix(shkname(shkp)), shtypes[rt - SHOPBASE].name);
        } else {
            You("вошли в %s %s%s!",
    ...
    if (!inside_shop(u.ux, u.uy)) {
        ...
        if (pick || mattock) {
            ...
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize(not_upset
                              ? "Не оставите ли вы вашу %s%s снаружи?"
                              : "Оставьте %s%s снаружи.",
                          tool, plur(cnt));
            } else {
                pline("%s %s впускать вас с вашей %s%s.",
                      Shknam(shkp),
                      not_upset ? "не решается" : "отказывается",
                      tool, plur(cnt));
            }
            should_block = TRUE;
        } else if (u.usteed) {
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize(not_upset ? "Не оставите ли вы %s снаружи?"
                                    : "Оставьте %s снаружи.",
                          y_monnam(u.usteed));
            } else {
                pline("%s %s впускать вас, пока вы едете на %s.",
                      Shknam(shkp),
                      not_upset ? "не хочет" : "отказывается",
                      y_monnam(u.usteed));
            }
            should_block = TRUE;
        } else {
...
pick_pick(struct obj *obj)
{
    ...
    if (shkp && inhishop(shkp)) {
        ...
        if (svm.moves != pickmovetime) {
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize("Вы хитрый %s!  Убирайтесь отсюда с этой киркой!",
                      cad(FALSE));
            } else {
                pline("%s %s вашу кирку!",
                      Shknam(shkp),
                      haseyes(shkp->data) ? "уставился на"
                                          : "обеспокоен, видя");
            }
        }
...
shopper_financial_report(void)
{
    ...
    if (eshkp && !(eshkp->credit || shop_debt(eshkp))) {
        You("не имеете здесь ни кредита, ни долга.");
    ...
    for (pass = this_shkp ? 0 : 1; pass <= 1; pass++)
        for (shkp = next_shkp(fmon, FALSE); shkp;
            ...
            if ((amt = eshkp->credit) != 0)
                You("имеете кредит в %ld %s в %s %s.", amt, currency(amt),
                    s_suffix(shkname(shkp)),
                    shtypes[eshkp->shoptype - SHOPBASE].name);
            else if (shkp == this_shkp)
                You("не имеете здесь кредита.");
            if ((amt = shop_debt(eshkp)) != 0)
                You("должны %s %ld %s.", shkname(shkp), amt, currency(amt));
            else if (shkp == this_shkp)
                You("не должны здесь ничего.");
        }
}
...
check_credit(long  tmp, struct monst *shkp)
{
    ...
    if (credit == 0L) {
        ...
        pline("Цена списана с вашего кредита.");
        ESHK(shkp)->credit -= tmp;
        tmp = 0L;
    } else {
        pline("Цена частично погашена вашим кредитом.");
        ESHK(shkp)->credit = 0L;
        tmp -= credit;
    }
    return tmp;
    ...
    if (helpless(shkp)) {
        ...
        if (verbosely && canspotmon(shkp))
            pline("%s %s.", Shknam(shkp),
                  shkp->msleeping ? "просыпается" : "снова может двигаться");
        shkp->msleeping = 0;
        shkp->mfrozen = 0;
        shkp->mcanmove = 1;
    }
...
make_happy_shk(struct monst *shkp, boolean silentkops)
{
    ...
    if (!inhishop(shkp)) {
        ...
        if (on_level(&eshkp->shoplevel, &u.uz)) {
            home_shk(shkp, FALSE);
            if (canspotmon(shkp)) {
                pline("%s возвращается в %s лавку.", Shknam(shkp),
                      noit_mhis(shkp));
        ...
        if (vanished)
            pline("Удовлетворившись, %s внезапно исчезает!", shk_nam);
    } else if (wasmad)
        pline("%s успокаивается.", Shknam(shkp));
    ...
    /* <ox,oy> predate 'noit_Monnam()', let alone Shknam() */
{
    ...
    pline("%s %s!", Shknam(shkp), !ANGRY(shkp) ? "разгневался" : "в ярости");
    hot_pursuit(shkp);
}

static const char
        no_money[] = "Кроме того, у вас%s нет золота.",
        not_enough_money[] = "К тому же, у вас недостаточно, чтобы заинтересовать %s.";
    ...
    Bill *ibill) /* all used up items, if any, precede all intact items */
{
    ...
    if (ibill[0].usedup <= PartlyUsedUp) {
        Sprintf(buf, "Израсходованные предметы:",
    ...
    for (i = 0; i < ibillct; ++i) {
        ...
        if (i > 0 && ibill[i - 1].usedup <= PartlyUsedUp
            ...
            Sprintf(buf, "Неоплаченные предметы:", (i < ibillct - 1) ? "s" : "");
            add_menu_heading(win, buf);
        }
        otmp = ibill[i].obj;
        save_quan = otmp->quan;
    ...
    end_menu(win, "За какие предметы платить?");
...
dopay(void)
{
    ...
    if ((!sk && (!Blind || Blind_telepat)) || (!Blind && !seensk)) {
        There("похоже, здесь нет лавочника, который принял бы вашу плату.");
        return ECMD_OK;
    }
    ...
    if (seensk == 1) {
        ...
        if (shkp != resident && !m_next2u(shkp)) {
            pline("%s недостаточно близко, чтобы принять вашу плату.",
                  Shknam(shkp));
            return ECMD_OK;
        }
    } else {
        ...
        cc.x = u.ux;
        cc.y = u.uy;
        if (getpos(&cc, TRUE, "существо, которому вы хотите заплатить") < 0)
            return ECMD_CANCEL; /* player pressed ESC */
        cx = cc.x;
        cy = cc.y;
        if (cx < 0) {
            pline("Попробуйте ещё раз...");
            return ECMD_OK;
        }
        if (u_at(cx, cy)) {
            You("щедры к самому себе.");
            return ECMD_OK;
        }
        mtmp = m_at(cx, cy);
        if (!cansee(cx, cy) && (!mtmp || !canspotmon(mtmp))) {
            You("не можете никого там %s.", !Blind ? "увидеть" : "почувствовать");
            return ECMD_OK;
        }
        if (!mtmp) {
            There("там никого нет, чтобы принять вашу плату.");
            return ECMD_OK;
        }
        if (!mtmp->isshk) {
            pline("%s не интересуется вашей платой.", Monnam(mtmp));
            return ECMD_OK;
        }
        if (mtmp != resident && !m_next2u(mtmp)) {
            pline("%s слишком далеко, чтобы принять вашу плату.", Shknam(mtmp));
            return ECMD_OK;
        }
        shkp = mtmp;
    }
 ...
 proceed:
    ...
    if (shkp != resident && NOTANGRY(shkp)) {
        ...
        if (!ltmp) {
            You("не должны %s ничего.", shkname(shkp));
        } else if (!umoney) {
            You("%sне имеете золота.", stashed_gold ? ", по-видимому, " : "");
            if (stashed_gold)
                pline("Но у вас припрятано немного золота.");
        } else {
            if (umoney > ltmp) {
                You("отдаёте %s %ld gold piece%s, как %s просил.",
                    shkname(shkp), ltmp, plur(ltmp), noit_mhe(shkp));
                pay(ltmp, shkp);
            } else {
                You("отдаёте %s всё ваше золото%s.", shkname(shkp),
                    stashed_gold ? ", что при вас" : "");
                pay(umoney, shkp);
                if (stashed_gold)
                    pline("Но у вас спрятано золото!");
            }
            if ((umoney < ltmp / 2L) || (umoney < ltmp && stashed_gold))
                pline("К сожалению, %s выглядит недовольным.",
                      noit_mhe(shkp));
            else
    ...
    if (!eshkp->billct && !eshkp->debit) {
        ...
        if (!ltmp && NOTANGRY(shkp)) {
            You("не должны %s ничего.", shkname(shkp));
            if (!umoney)
                pline(no_money, stashed_gold ? ", по-видимому," : "");
        } else if (ltmp) {
            pline("%s хочет крови, а не золота!", shkname(shkp));
            if (umoney < ltmp / 2L || (umoney < ltmp && stashed_gold)) {
                if (!umoney)
                    pline(no_money, stashed_gold ? ", по-видимому," : "");
                else
            ...
            pline("Но поскольку %s лавка недавно была ограблена,",
                  noit_mhis(shkp));
            pline("вы %sкомпенсируете %s %s потери.",
                  (umoney < ltmp) ? "частично " : "", shkname(shkp),
                  noit_mhis(shkp));
            ...
            pline("%s хочет вашей шкуры, а не вашего золота!", Shknam(shkp));
            if (umoney < 1000L) {
                if (!umoney)
                    pline(no_money, stashed_gold ? ", по-видимому," : "");
                else
            ...
            You("пытаетесь задобрить %s, отдавая %s 1000 золотых.",
                canspotmon(shkp)
                    ? x_monnam(shkp, ARTICLE_THE, "разгневанный", 0, FALSE)
                    : shkname(shkp),
                noit_mhim(shkp));
            pay(1000L, shkp);
            if (strncmp(eshkp->customer, svp.plname, PL_NSIZ) || rn2(3))
                ...
                pline("%s по-прежнему в гневе.", shkname(shkp));
        }
        return ECMD_TIME;
    }
    if (shkp != resident) {
    ...
    if (eshkp->debit) {
        ...
        umoney = money_cnt(gi.invent);
        Sprintf(sbuf, "Вы должны %s %ld %s ", shkname(shkp), dtmp,
                currency(dtmp));
        if (loan) {
            if (loan == dtmp)
                Strcat(sbuf, ", что вы подняли в лавке.");
            else
                Strcat(sbuf,
                       " за поднятое золото и использование товара.");
        } else {
            Strcat(sbuf, " за использование товара.");
        }
        pline1(sbuf);
        if (umoney + eshkp->credit < dtmp) {
            pline("Но у вас%s недостаточно золота%s.",
                  stashed_gold ? ", по-видимому," : "",
                  eshkp->credit ? " или кредита" : "");
            return ECMD_TIME;
        } else {
            if (eshkp->credit >= dtmp) {
                ...
                eshkp->debit = 0L;
                eshkp->loan = 0L;
                Your("долг покрыт вашим кредитом.");
            } else if (!eshkp->credit) {
                money2mon(shkp, dtmp);
                eshkp->debit = 0L;
                eshkp->loan = 0L;
                You("выплачиваете этот долг.");
                disp.botl = TRUE;
            } else {
                dtmp -= eshkp->credit;
                eshkp->credit = 0L;
                ...
                eshkp->debit = 0L;
                eshkp->loan = 0L;
                pline("Этот долг частично погашен за счёт вашего кредита.");
                You("выплачиваете остаток.");
                disp.botl = TRUE;
            }
            paid = TRUE;
        }
    ...
    if (pay_done && !ANGRY(shkp) && paid) {
        if (!Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Спасибо за покупки в %s %s%s",
                      s_suffix(shkname(shkp)),
                      shtypes[eshkp->shoptype - SHOPBASE].name,
                      !eshkp->surcharge ? "!" : ".");
        } else {
            pline("%s кивает%s вам за покупки в %s %s%s",
                  Shknam(shkp), !eshkp->surcharge ? " одобрительно" : "",
    ...
    boolean *paid_p) /* output */
{
    ...
    if (!umoney && !eshkp->credit) {
        You("%sне имеете ни золота, ни кредита%s.",
            stashed_gold ? ", по-видимому, " : "", *paid_p ? "" : "");
        return TRUE;
    }
    bp = eshkp->bill_p;
    otmp = bp_to_obj(bp);
    ...
    if ((umoney + eshkp->credit) < cheapest_item(ibillct, ibill)) {
        You("У вас недостаточно золота, чтобы купить%s%s%s.",
            more_than_one ? " хоть что-нибудь" : " это",
            (ebillct > 1) ? " из того, что вы выбрали" : " из того, что в счёте");
        if (stashed_gold)
            pline("Может, у вас где-то спрятано золото?");
        return TRUE;
    }
    ...
    do {
        if (via_menu /*&& more_than_one*/ ) {
            ...
            iprompt = !more_than_one ? 'y'
                      : yn_function("Разделить счёт на позиции?", "ynq m", 'q', TRUE);
            if (iprompt == 'q')
                return TRUE;
            itemize = (iprompt == 'y');
            via_menu = (iprompt == 'm');
        ...
        if (ibill[indx].usedup >= KnownContainer) {
            ...
            if (boxbag_result == 0) {
                ...
                if (boxbag_result == 2)    /* ... but didn't explain why */
                    verbalize("Вам нужно убрать неоплаченные предметы из "
                              " того %s и купить их отдельно.",
                              simpleonames(otmp));
                buy = PAY_CANT;
            }
        } else {
    ...
    boolean unseen)
{
    ...
    if (itemize) {
        ...
        Sprintf(qsfx, " за %ld %s.  Платить?", ltmp, currency(ltmp));
    ...
    if (buy == PAY_BUY) {
        ...
        if (!unseen)
            shk_names_obj(shkp, obj,
                          consumed
                              ? "заплатили за %s стоимостью %ld gold piece%s.%s"
                              : "купили %s за %ld gold piece%s.%s",
                          ltmp, "");
    }
    ...
    long billed_quan)
{
    ...
    if (!Deaf && !muteshk(shkp)) {
        ...
        if (obj->where == OBJ_CONTAINED)
            Snprintf(which, sizeof which, "%s в %s",
                     (intact_quan > 1L) ? "те, что" : "тот, что",
                     thesimpleoname(obj->ocontainer));
            Sprintf(which, "%s", (intact_quan > 1L) ? "эти" : "этот");

        SetVoice(shkp, 0, 80, 0);
        verbalize("%s за другой %s, прежде чем купить %s.",
                  ANGRY(shkp) ? "Заплатите" : "Пожалуйста, заплатите",
                  simpleonames(obj), /* short name suffices */
                  which);
    } else {
        pline("%s %s%s на ваш счёт за другой %s: сначала оплатите.",
              Shknam(shkp),
              ANGRY(shkp) ? "гневно " : "",
              nolimbs(shkp->data) ? "кивает" : "указывает",
              simpleonames(obj));
    }
    ...
    long cost) /* 0: check for no-gold; >0: check for specified amount */
{
    ...
    if (!cost && umoney + ecredit == 0L) {
        ...
        You("%sне имеете ни золота, ни кредита.",
            (stashed_gold > 0) ? ", по-видимому, " : "");
        return TRUE;
    }
    if (cost && umoney + ecredit < cost) {
        stashed_gold = hidden_gold(TRUE);
        You("У вас%s недостаточно золота%s, чтобы заплатить за %s.",
            (stashed_gold > 0L) ? ", по-видимому," : "",
            (ecredit > 0L) ? " или кредита" : "",
            paydoname(item));
        return TRUE;
    }
    return FALSE;
    ...
    boolean silently)
{
    ...
    if (numsk > 1) {
        if (cansee(shkp->mx, shkp->my) && croaked && !silently) {
            ...
            if (has_head(shkp->data) && !rn2(2))
                Sprintf(takes, ", качает %s %s,", noit_mhis(shkp),
                        mbodypart(shkp, HEAD));
            pline("%s %sсмотрит на ваш труп%s и %s.", Shknam(shkp),
                  helpless(shkp) ? "просыпается, " : "",
                  takes, !inhishop(shkp) ? "исчезает" : "вздыхает");
        }
        taken = uinshop;
        goto skip;
    }
    ...
    if (uinshop && inhishop(shkp) && !eshkp->billct
        ...
        taken = (gi.invent != 0);
        if (taken && !silently)
            pline("%s с благодарностью наследует всё ваше имущество.",
                  Shknam(shkp));
        goto clear;
    }
    ...
    if (eshkp->following || ANGRY(shkp) || take) {
        ...
        takes[0] = '\0';
        if (helpless(shkp))
            Strcat(takes, "просыпается, ");
        if (!m_next2u(shkp))
            Strcat(takes, "подходит и ");
        Strcat(takes, "забирает");
        ...
        if (loss > umoney || !loss || uinshop) {
            ...
            if (!silently)
                pline("%s %s всё ваше имущество.", Shknam(shkp), takes);
            taken = TRUE;
        } else {
            money2mon(shkp, loss);
            disp.botl = TRUE;
            if (!silently)
                pline("%s %s %ld %s, которые %sзадолжали %s.", Shknam(shkp),
...
get_cost_of_shop_item(
    ...
    int *nochrg) /* alternate return value: 1: no charge, 0: shop owned,
                  * -1: not in a shop (so don't format as "без оплаты") */
{
    struct monst *shkp;
    struct obj *top;
    coordxy x, y;
    ...
    if (ESHK(shkp)->shoptype == CANDLESHOP
        ...
        if (!quietly) {
            if (is_izchak(shkp, TRUE) && !u.uevent.invoked) {
                if (Deaf || muteshk(shkp)) {
                    pline("%s, похоже, %s из-за того, что вы хотите это продать.",
                          Shknam(shkp),
                          (obj->spe < 7) ? "в ужасе" : "в тревоге");
                } else {
                    SetVoice(shkp, 0, 80, 0);
                    verbalize("Нет уж, я бы придержал это, будь я на вашем месте.");
                    if (obj->spe < 7) {
                        SetVoice(shkp, 0, 80, 0);
                        verbalize(
                             "Чтобы зажечь их, вам понадобится ещё %d%s candle%s.",
                                (7 - obj->spe), (obj->spe > 0) ? "" : "",
                                  plur(7 - obj->spe));
                    }
                ...
                if (!Deaf && !muteshk(shkp)) {
                    SetVoice(shkp, 0, 80, 0);
                    verbalize("Это я не приму.  Убирайте это отсюда!");
                } else {
                    pline("%s в знак отказа качает %s %s.",
                          Shknam(shkp), noit_mhis(shkp),
                          mbodypart(shkp, HEAD));
                }
            }
    ...
    const char *arg)
{
    ...
    if (was_unknown) {
        Sprintf(fmtbuf, "%%s; вы %s", fmt);
        obj_name[0] = highc(obj_name[0]);
        pline(fmtbuf, obj_name, (obj->quan > 1L) ? "их" : "его", amt,
              plur(amt), arg);
    } else {
    ...
    boolean silent)
{
    ...
    if (!Deaf && !muteshk(shkp) && !silent) {
        ...
        if (!ltmp) {
            pline("%s не интересуется %s.", Shknam(shkp), the(xname(obj)));
            return;
        }
        if (!ininv) {
            pline("%s обойдётся вам в %ld %s%s.", The(xname(obj)), ltmp,
                  currency(ltmp), (obj->quan > 1L) ? " за штуку" : "");
        } else {
        ...
        if (ltmp) {
            set_voice(shkp, 0, 80, 0);
            pline("Прейскурантная цена %s%s%s — %ld %s%s.",
                      (contentscount && !obj->unpaid) ? the_contents_of : "",
                      the(xname(obj)),
                      (contentscount && obj->unpaid) ? and_its_contents : "",
                      ltmp, currency(ltmp), (obj->quan > 1L) ? " За штуку" : "");
        } else {
            pline("%s не замечает.", Shknam(shkp));
        }
    }
}
...
append_honorific(char *buf)
{
    ...
    static const char *const honored[] = {
        "дорогой", "почтенный", "милостивейший", "досточтимый",
        "всемирно почитаемый и священный"
    };

    Strcat(buf, honored[rn2(SIZE(honored) - 1) + u.uevent.udemigod]);
    if (is_vampire(gy.youmonst.data))
        Strcat(buf, (flags.female) ? " тёмная госпожа" : " тёмный господин");
    else if (maybe_polyd(is_elf(gy.youmonst.data), Race_if(PM_ELF)))
        Strcat(buf, (flags.female) ? " хирил" : " хири");
    else
        Strcat(buf, !is_human(gy.youmonst.data) ? " создание"
                      : (flags.female) ? " госпожа"
                        : " сэр");
}
    ...
    boolean silent)
{
    ...
    if (peaceful) {
        ...
        if (!silent) {
            ...
            if (credit_use) {
                if (ESHK(shkp)->credit) {
                    You("имеете остаток кредита в %ld %s.", ESHK(shkp)->credit,
                        currency(ESHK(shkp)->credit));
                    return value;
                } else if (!value) {
                    You("не имеете остатка кредита.");
                    return 0;
                }
                still = "всё ещё ";
            }
            Sprintf(buf, "%sдолжны %s %ld %s", still, shkname(shkp),
                    value, currency(value));
            if (u_count) /* u_count > 0 implies Has_contents(obj) */
                Sprintf(eos(buf), " за %s%sего содержимое",
                        was_unpaid ? "него и " : "",
                        (c_count > u_count) ? "часть " : "");
        ...
        if (!silent) {
            if (canseemon(shkp)) {
                Norep("%s гремит: \"%s, вы вор!\"",
                      Shknam(shkp), svp.plname);
            } else if (!Deaf) {
                Norep("Слышен крик: \"Вор!\"");  /* Deaf-aware */
            }
        }
        hot_pursuit(shkp);
    ...
    boolean selling) /* T: dropped in shop; F: kicked and landed in shop */
{
    ...
    if (eshkp->debit >= gltmp) {
        ...
        eshkp->debit -= gltmp;
        Your("долг %sпогашен.", eshkp->debit ? "частично " : "");
    } else {
        ...
        if (eshkp->credit == delta)
            You("кредит в %ld %s установлен%s.",
                !selling ? " заново" : "", delta, currency(delta));
        else
            pline("к кредиту добавлено %ld %s%s; теперь итого %ld %s.",
                  delta, currency(delta), !selling ? " обратно" : "",
                  eshkp->credit, currency(eshkp->credit));
    }
}
    ...
    coordxy x, coordxy y)
{
    ...
    if (ANGRY(shkp)) { /* they become shop-objects, no pay */
        if (!Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Спасибо, негодяй!");
        } else {
            pline("%s ухмыляется с довольным видом.", Shknam(shkp));
        }
    ...
    if (!(isgold || cgold)
        ...
        if (!unpaid && (gs.sell_how != SELL_DONTSELL)
            ...
            pline("%s, похоже, не заинтересован.", Shknam(shkp));
        return;
    }
    ...
    if (eshkp->robbed) { /* bones; shop robbed by previous customer */
        ...
        if (offer && !Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize(
  "Спасибо за ваш вклад в пополнение недавно ограбленной лавки.");
        }
        subfrombill(obj, shkp);
        return;
    }
    ...
    if ((!saleitem && !(container && cltmp > 0L)) || eshkp->billct == BILLSZ
        ...
        pline("%s, похоже, не заинтересован%s.", Shknam(shkp),
              cgold ? " в остальном" : "");
        if (container)
            dropped_container(obj, shkp, FALSE);
        obj->no_charge = 1;
    ...
    if (!shkmoney) {
        ...
        if (gs.sell_how == SELL_NORMAL || ga.auto_credit) {
            ...
            pline("%s сейчас не может заплатить вам.", Shknam(shkp));
            Sprintf(qbuf, "Согласны принять %ld %s в кредит за ", tmpcr,
                    currency(tmpcr));
        ...
        if (c == 'y') {
            shk_names_obj(shkp, obj,
                          ((gs.sell_how != SELL_NORMAL)
                           ? "обменяли %s на %ld zorkmid%s в %sкредит."
                    : "отдаёте %s и получаете %ld zorkmid%s в %sкредит."),
                          tmpcr, (eshkp->credit > 0L) ? "дополнительный " : "");
            eshkp->credit += tmpcr;
            if (container)
        ...
        if (!gs.sell_response) {
             ...
             */
            Sprintf(qbuf, "%s предлагает%s %ld gold piece%s за %s%s ",
                    Shknam(shkp), short_funds ? " только" : "", offer,
                    plur(offer),
                    (cltmp && !ltmp)
                        ? ((yourc == 1L) ? "ваш предмет в " : "ваши предметы в ")
                        : "",
                    obj->unpaid ? "этот" : "ваш");
            one = !ltmp ? (yourc == 1L) : (obj->quan == 1L && !cltmp);
            Sprintf(qsfx, "%s.  Продать %s?",
                    (cltmp && ltmp)
                        ? (only_partially_your_contents
                               ? ((yourc == 1L) ? " и предмет внутри"
                                                : " и предметы внутри")
                               : and_its_contents)
                        : "",
                    one ? "его" : "их");
        ...
        case 'y':
            ...
            shk_names_obj(shkp, obj,
                         ...
                         ? "продали предметы, что в %s, за %ld gold piece%s.%s"
                         : "продали %s за %ld gold piece%s.%s")
            : "отдаёте %s и получаете %ld gold piece%s в качестве компенсации.%s",
                          offer, "");
            break;
        default:
    ...
    int mode) /* 0: deliver count 1: paged */
{
    ...
    putstr(datawin, 0, "Неоплаченные израсходованные предметы:");
    putstr(datawin, 0, "");
    ...
    if (eshkp->debit) {
        ...
        if (totused)
            putstr(datawin, 0, "");
        totused += eshkp->debit;
        buf_p = xprname((struct obj *) 0, "плата за пользование и прочие сборы",
                        GOLD_SYM, FALSE, eshkp->debit, 0L);
        putstr(datawin, 0, buf_p);
    }
    buf_p = xprname((struct obj *) 0, "Итого:", '*', FALSE, totused, 0L);
    putstr(datawin, 0, "");
    putstr(datawin, 0, buf_p);
    ...
    coordxy x, coordxy y)
{
    ...
    if (!helpless(shkp)
        ...
        if (mnearto(shkp, x, y, TRUE, RLOC_NOMSG) == 2
            && !Deaf && !muteshk(shkp)) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Прочь с дороги, мерзавец!");
        }
        if (cansee(x, y)) {
            pline("%s ловко%s хватает %s.", Shknam(shkp),
                  (x == shkp->mx && y == shkp->my) ? "" : ", потянувшись, ",
                  the(xname(obj)));
            if (!canspotmon(shkp))
                map_invisible(x, y);
            nh_delay_output();
...
shk_fixes_damage(struct monst *shkp)
{
    ...
    if (canseemon(shkp)) {
        pline("%s шепчет %s.", Shknam(shkp),
              shk_closeby ? "заклинание" : "что-то");
    } else if (!Deaf && shk_closeby) {
        Soundeffect(se_mutter_incantation, 100);
        You_hear("кто-то бормочет заклинание.");
    }
    ...
    struct monst *shkp)
{
       ...
       that's inside the shop; caller guarantees that */
    {
        ...
        if (Punished && !u.uswallow
            ...
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize("Уберите свой хлам из моей стены!");
            }
    ...
    boolean catchup)
{
    ...
    if (ttmp) {
        ...
        case BEAR_TRAP:
            ...
            if (!catchup) {
                if (canseemon(shkp) && dist2(x, y, shkp->mx, shkp->my) <= 2)
                    pline("%s снимает %s.", Shknam(shkp), ansimpleoname(otmp));
                else if (ttmp->tseen && cansee(ttmp->tx, ttmp->ty))
                    pline("%s исчезает.", trapname(ttmp->ttyp, TRUE));
            }
        ...
        case SPIKED_PIT:
            if (!catchup && ttmp->tseen && cansee(ttmp->tx, ttmp->ty))
                pline("%s засыпается.", trapname(ttmp->ttyp, TRUE));
            break;
        default:
            if (!catchup && ttmp->tseen && cansee(ttmp->tx, ttmp->ty))
                pline("%s исчезает.", trapname(ttmp->ttyp, TRUE));
    ...
    if (seeit) {
        if (IS_WALL(tmp_dam->typ)) {
            ...
            levl[x][y].seenv = SVALL;
            pline("Внезапно участок стены смыкается!");
        } else if (IS_DOOR(tmp_dam->typ)) {
            pline("Внезапно дверь лавки вновь появляется!");
        }
        newsym(x, y);
    } else if (IS_WALL(tmp_dam->typ)) {
        if (inside_shop(u.ux, u.uy) == ESHK(shkp)->shoproom)
            You_feel("большую тесноту, чем раньше.");
        else if (!Deaf && !rn2(10))
            Norep("Акустика темницы заметно изменилась.");
    }
...
shk_move(struct monst *shkp)
{
    ...
    if ((udist = distu(omx, omy)) < 3 && (shkp->data != &mons[PM_GRID_BUG]
        ...
        if (ANGRY(shkp) || (Conflict && !resist_conflict(shkp))) {
            if (Displaced)
                Your("смещённый образ не обманет %s!", shkname(shkp));
            (void) mattacku(shkp);
            return 0;
        }
        if (eshkp->following) {
            if (strncmp(eshkp->customer, svp.plname, PL_NSIZ)) {
                if (!Deaf && !muteshk(shkp)) {
                    SetVoice(shkp, 0, 80, 0);
                    verbalize("%s, %s!  Я искал %s.", Hello(shkp),
            ...
            if (svm.moves > gf.followmsg + 4) {
                if (!Deaf && !muteshk(shkp)) {
                    SetVoice(shkp, 0, 80, 0);
                    verbalize("%s, %s!  Вы случайно не забыли заплатить?",
                              Hello(shkp), svp.plname);
                } else {
                    pline("%s протягивает %s перевёрнутую %s.",
                          Shknam(shkp), noit_mhis(shkp),
                          mbodypart(shkp, HAND));
                }
                gf.followmsg = svm.moves;
                if (!rn2(9)) {
                    pline("%s не любит покупателей, которые не платят.",
                          Shknam(shkp));
                    rile_shk(shkp);
                }
            }
...
shopdig(int fall)
{
    struct monst *shkp = shop_keeper(*u.ushops);
    int lang;
    const char *grabs = "хватает";

    if (!shkp)
        return;
    if (!inhishop(shkp)) {
        if (Role_if(PM_KNIGHT)) {
            You_feel("как обычный вор.");
            adjalign(-sgn(u.ualign.type));
        }
        return;
    }
    ...
    if (!fall) {
        if (lang == 2) {
            if (!Deaf && !muteshk(shkp)) {
                ...
                if (u.utraptype == TT_PIT) {
                    verbalize(
                       "Будьте осторожны, %s, иначе можете провалиться сквозь пол.",
                              flags.female ? "мадам" : "сэр");
                } else {
                    verbalize("%s, не повреждайте здесь пол!",
                              flags.female ? "Мадам" : "Сэр");
                }
            }
        }
        ...
        if (!m_next2u(shkp)) {
            ...
            if (!m_next2u(shkp)) {
                if (lang == 2)
                    pline("%s проклинает вас в гневе и досаде!",
                          Shknam(shkp));
                else if (lang == 1)
                    growl(shkp);
                rile_shk(shkp);
                ...
                pline("%s %s и %s ваш рюкзак!", Shknam(shkp),
                      makeplural(locomotion(shkp->data, "leap")), grabs);
        } else
            pline("%s %s ваш рюкзак!", Shknam(shkp), grabs);
...
{
    boolean dugwall = (!strcmp(dmgstr, "вломиться в")    /* wand */
                    || !strcmp(dmgstr, "повредить")); /* pick-axe */
    ...
    if (muteshk(shkp)) {
        ...
        if (!Deaf) {
            SetVoice(shkp, 0, 80, 0);
            verbalize("Как вы посмели %s мою %s?", dmgstr,
                        dugwall ? "лавку" : "дверь");
        } else {
            pline("%s %s, что вы решили %s %s %s!",
                    Shknam(shkp), ROLL_FROM(angrytexts),
                    dmgstr, noit_mhis(shkp), dugwall ? "лавку" : "дверь");
        }
    } else {
        if (!Deaf) {
            pline("%s кричит:", Shknam(shkp));
            SetVoice(shkp, 0, 80, 0);
            verbalize("Кто посмел %s мою %s?", dmgstr,
                        dugwall ? "лавку" : "дверь");
        } else {
            pline("%s %s, что кто-то решил %s %s %s!",
                    Shknam(shkp), ROLL_FROM(angrytexts),
                    dmgstr, noit_mhis(shkp), dugwall ? "лавку" : "дверь");
        }
    }
...
pay_for_damage(const char *dmgstr, boolean cant_mollify)
{
    ...
    if (uinshp) {
        if (um_dist(shkp->mx, shkp->my, 1)
            ...
            pline("%s бросается к вам!", Shknam(shkp));
        ...
        if (MON_AT(x, y)) {
            if (!animal) {
                if (!Deaf && !muteshk(shkp)) {
                    ...
                    You_hear("чей-то разгневанный голос:");
                    SetVoice(shkp, 0, 80, 0);
                    verbalize("Прочь с дороги, мерзавец!");
                }
                wait_synch();
    ...
    if (Invis)
        Your("невидимость не обманет %s!", shkname(shkp));
    Sprintf(qbuf, "%sВы нанесли ущерб на %ld %s!%s  Платить?",
    ...
    if (y_n(qbuf) != 'n') {
        ...
        pline("Смягчившись, %s принимает вашу компенсацию.", shkname(shkp));
        ...
        if (shkp->mx != sx || shkp->my != sy) {
            if (was_outside && canspotmon(shkp))
                ...
                pline("%s %s.", Shknam(shkp), !was_seen ? "появляется"
                                              : is_seen ? "перемещается"
                                                : "исчезает");
        }
    } else {
        if (!animal) {
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize("О да!  Вы заплатите!");
            } else {
                pline("%s вытягивает %s %s к вашему %s!",
                      Shknam(shkp), noit_mhis(shkp),
...
price_quote(struct obj *first_obj)
{
    ...
    putstr(tmpwin, 0, "Отличные товары в продаже:");
    putstr(tmpwin, 0, "");
    for (otmp = first_obj; otmp; otmp = otmp->nexthere) {
        ...
        if (!cost) {
            Strcpy(price, "без оплаты");
            contentsonly = FALSE;
        } else {
            Sprintf(price, "%ld %s%s", cost, currency(cost),
                    (otmp->quan) > 1L ? " за штуку" : "");
        }
    ...
    if (cnt > 1) {
        ...
        if (!cost) {
            ...
            Sprintf(buf, "%s%s", contentsonly ? the_contents_of : "",
                    doname(first_obj));
            SetVoice(shkp, 0, 80, 0);
            verbalize("%s, цена %ld %s%s%s", upstart(buf), cost,
                      currency(cost), (first_obj->quan > 1L) ? " за штуку" : "",
        ...
        case 4:
            ...
            if (objects[itm->otyp].oc_name_known
                    ...
                    : (o == AMULET_CLASS || o == RING_CLASS || o == WAND_CLASS
                       ...
                       || o == SPBOOK_CLASS))
                return ", изготовлено с большим тщанием!";
            return ", безупречная работа мастера!";
        case 3:
            return ", отличнейшего качества.";
        case 2:
            return ", превосходный выбор.";
        case 1:
            return ", настоящая удача.";
        default:
            break;
        }
    } else if (itm->oartifact) {
        return ", единственный в своём роде!";
    }
    return ".";
}
...
static const char *Izchak_speaks[] = {
    "%s говорит: 'От этих торговых пассажей у меня болит голова.'",
    "%s говорит: 'Не торопитесь.  Мыслите ясно.'",
    "%s говорит: 'Всё нужно делать по одному.'",
    "%s говорит: 'Мне не нравится слабый кофе... дайте-ка мне Colombian Supremo.'",
    "%s говорит, что добиться согласия команды разработчиков трудно.",
    "%s говорит, что заметил: те, кто служит своему божеству, процветают.",
    "%s говорит: 'Не пытайтесь украсть у меня - у меня покровители среди высоких особ!'",
    "%s говорит: 'Возможно, в будущем вам что-то понадобится из этой лавки.'",
    "%s упоминает, что Долина Мёртвых служит вратами."
};
...
shk_chat(struct monst *shkp)
{
    ...
    if (!shkp->isshk) {
        ...
        pline("%s спрашивает, не попадались ли вам недавно лавки без присмотра.",
              Monnam(shkp));
    ...
    eshk = ESHK(shkp);
    if (ANGRY(shkp)) {
        pline("%s %s, насколько %s не любит %s покупателей.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "говорит" : "даёт понять",
              noit_mhe(shkp), eshk->robbed ? "неплатящих" : "невежливых");
    } else if (eshk->following) {
        if (strncmp(eshk->customer, svp.plname, PL_NSIZ)) {
            if (!Deaf && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize("%s %s!  Я искал %s.",
        ...
        pline("%s %s, что ваш счёт составляет %ld %s.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "говорит" : "даёт понять",
              total, currency(total));
    } else if (eshk->debit) {
        pline("%s %s, что вы должны %s %ld %s.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "напоминает вам" : "даёт понять",
              noit_mhim(shkp), eshk->debit, currency(eshk->debit));
    } else if (eshk->credit) {
        pline("%s призывает воспользоваться вашим кредитом в %ld %s.",
              Shknam(shkp), eshk->credit, currency(eshk->credit));
    } else if (eshk->robbed) {
        pline("%s %s о недавнем ограблении.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "жалуется" : "выражает озабоченность");
    } else if (eshk->surcharge) {
        pline("%s %s, что %s внимательно следит за вами.", Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "предупреждает вас" : "даёт понять",
              noit_mhe(shkp));
    } else if ((shkmoney = money_cnt(shkp->minvent)) < 50L) {
        pline("%s %s, что дела идут плохо.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "жалуется" : "даёт понять");
    } else if (shkmoney > 4000) {
        pline("%s %s, что дела идут хорошо.",
              Shknam(shkp),
              (!Deaf && !muteshk(shkp)) ? "говорит" : "даёт понять");
        ...
        if (!Deaf && !muteshk(shkp))
            pline("%s говорит о проблеме лавочных воров.", Shknam(shkp));
    }
}
...
kops_gone(boolean silent)
{
    ...
    if (cnt && !silent)
        pline("КОп%s (разочарованные) растворя%sются в воздухе.",
                  plur(cnt), (cnt == 1) ? "es" : "");
}
...
check_unpaid_usage(struct obj *otmp, boolean altusage)
{
    ...
    if (otmp->oclass == SPBOOK_CLASS) {
        fmt = "%sВы должны%s %ld %s.";
        Sprintf(buf, "Это не бесплатная библиотека, %s!  ", cad(FALSE));
        arg1 = rn2(2) ? buf : "";
        arg2 = ESHK(shkp)->debit > 0L ? " дополнительно" : "";
    } else if (otmp->otyp == POT_OIL) {
        fmt = "%s%sЭто будет стоить вам %ld %s (Ендорианский топливный налог).";
    } else if (altusage && (otmp->otyp == BAG_OF_TRICKS
                            || otmp->otyp == HORN_OF_PLENTY)) {
        fmt = "%s%sОпустошение этого будет стоить вам %ld %s.";
        if (!rn2(3))
            arg1 = "Ого!  ";
        if (!rn2(3))
            arg1 = "Смотри у меня!  ";
    } else {
        fmt = "%s%sПлата за пользование: %ld %s.";
        if (!rn2(3))
            arg1 = "Эй!  ";
        if (!rn2(3))
            arg2 = "Кхм.  ";
    ...
    boolean silent)
{
    ...
    if (eshkp->credit >= amount) {
        if (!silent) {
            if (eshkp->credit > amount)
                Your("кредит уменьшен на %ld %s.", amount, currency(amount));
            else
                Your("кредит аннулирован.");
        }
        eshkp->credit -= amount;
    } else {
        ...
        if (!silent) {
            if (eshkp->credit)
                Your("кредит аннулирован.");
            if (eshkp->debit)
                Your("долг увеличился на %ld %s.", delta, currency(delta));
            else
                You("должны %s %ld %s.", shkname(shkp), delta, currency(delta));
        }
        eshkp->debit += delta;
        eshkp->loan += delta;
        eshkp->credit = 0L;
...
block_door(coordxy x, coordxy y)
{
    ...
    if (shkp->mx == ESHK(shkp)->shk.x && shkp->my == ESHK(shkp)->shk.y
        ...
        pline("%s%s преграждает вам путь!", Shknam(shkp),
              Invis ? ", чувствуя ваше движение, " : "");
        return TRUE;
    }
    return FALSE;
}
...
shk_your(char *buf, struct obj *obj)
{
    ...
    if (chk_pm && type_is_pname(&mons[obj->corpsenm]))
        ...
        Strcpy(buf, "этот"); /* override ownership: "the Oracle's corpse" */
...
globby_bill_fixup(struct obj *obj_absorber, struct obj *obj_absorbed)
{
    ...
    if (!bp_absorber && !bp && !obj_absorber->no_charge) {
        ...
        if (saleable(shkp, obj_absorbed)) {
            if (eshkp->debit >= amount) {
                ...
                eshkp->debit -= amount;
                pline("Пожертвованный %s %sпогашает ваш долг.",
                          obj_typename(obj_absorbed->otyp),
                          eshkp->debit ? "Частично " : "");
            } else {
                ...
                if (eshkp->credit == delta)
                    pline("%s Устанавливает кредит в %ld %s.",
                              obj_typename(obj_absorbed->otyp),
                              delta, currency(delta));
                else
                    pline("%s Добавил %ld %s %s %ld %s.",
                              obj_typename(obj_absorbed->otyp),
                              delta, currency(delta),
                              "К вашему кредиту; теперь итого ",
                              eshkp->credit, currency(eshkp->credit));
            }
        }
        return;
    ...
    if (bp && (obj_absorber->no_charge
        ...
        SetVoice(shkp, 0, 80, 0);
        verbalize("Вы должны мне %ld %s за мой %s, что вы %s с вашим%s",
                  amount, currency(amount), obj_typename(obj_absorbed->otyp),
                  ANGRY(shkp) ? "имели наглость смешать" : "просто смешали",
                  ANGRY(shkp) ? " мерзким месивом!" : ".");
        return;
    }
    /**************************************************************
    ...
    if (otmp->unpaid) {
        if (!Deaf) {
            ...
            if (shkp && !muteshk(shkp)) {
                SetVoice(shkp, 0, 80, 0);
                verbalize("Что поставили, то и платите!");
            }
        }
