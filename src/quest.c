    ...
    if (wizard && talk) {
        if (u.ualign.type != original_alignment) {
            ...
            You("Ваш показатель сейчас %d, а требуется %d.", u.ualign.record,
                MIN_QUEST_ALIGN);
            if (yn_function("Подправить?", (char *) 0, 'y', TRUE) == 'y')
...
expulsion(boolean seal)
{
    ...
    if (seal) { /* remove the portal to the quest - sealing it off */
        ...
        if (t)
            ...
            impossible("портал задания уже исчез?");
    }
}
                               ...
                               * AoY; possibly null if carrying the Amulet */
{
    ...
    if (obj && !is_quest_artifact(obj)) {
        ...
        if (obj->otyp == AMULET_OF_YENDOR) {
            ...
            verbalize(
      "Прискорбно признать, но это лишь подделка истинного Амулета Йендора.");
        } else {
            verbalize("А, вижу, вы нашли %s.", the(xname(obj)));
        }
        return;
    }
    ...
    /* Rule 5: You aren't yet acceptable - or are you? */
        ...
        if (not_capable()) {
            ...
            /* phrasing is a bit clumsy but allows #chronicle to provide a
               ...
               easily found */
            livelog_printf(LL_ACHIEVE,
                     "%s даровал доступ для продвижения глубже в задании",
                           noit_mon_nam(mtmp));
        }
    }
}
...
quest_chat(struct monst *mtmp)
{
    ...
    case MS_GUARDIAN:
        ...
        break;
    default:
        impossible("quest_chat: неизвестный персонаж задания %s.", mon_nam(mtmp));
    }
}
