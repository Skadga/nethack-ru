...
do_write_config_file(void)
{
    ...
    if (!configfile[0]) {
        pline("Странно, не удалось определить имя файла конфигурации.");
        return ECMD_OK;
    }
    if (flags.suppress_alert < FEATURE_NOTICE_VER(3,7,0)) {
        pline("Предупреждение: saveoptions является экспериментальной!");
        wait_synch();
        pline("Некоторые настройки не сохранены!");
        wait_synch();
        pline("Все ручные настройки и комментарии удалены"
              " из файла!");
        wait_synch();
    }
...
#undef overwrite_prompt
    ...
    if (fp) {
        ...
        strbuf_empty(&buf);
        if (wrote != len)
            pline("Произошла ошибка, записаны только частичные данные (%zu/%zu).",
                  wrote, len);
    }
    return ECMD_OK;
}
