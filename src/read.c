...
tshirt_text(struct obj *tshirt, char *buf)
{
    static const char *const shirt_msgs[] = {
        /* Scott Bigham */
      "Я исследовал Подземелья Рока и получил лишь эту паршивую футболку!",
        "Это Мьёльнир у вас в кармане или вы просто рады меня видеть?",
      "Дело не в размере меча, а в том, умеете ли вы им владеть (#enhance).",
        "Дом суккубов мадам Эльвиры. Постоянный клиент",
        "Дом суккубов мадам Эльвиры. Сотрудник месяца",
        "Стража хранилища Людиоса делает это в маленьких тёмных комнатах",
        "Солдаты армии Йендора делают это большими группами",
        "Я выжил в учебном лагере армии Йендора",
        "Команда по лакроссу школы счетоводов Людиоса",
        "Фонтаны Оракула(TM): 10-й ежегодный конкурс мокрых футболок",
        "Эй, чёрный дракон! Дезинтегрируй ЭТО!",
        "Я с этим придурком -->",
        "Не вините меня, я голосовал за Ицхака!",
        "Не паникуй", /* HHGTTG */
        "Спортивный отдел школы Фуринкан",                /* Ranma 1/2 */
        "Приве-е-ет, медсестра!",                                   /* Animaniacs */
        "=^.^=",
        "100% волос гоблина - не стирать",
        "Аберзомби и Фитч",
        "Кокатрикс касается Копа",
        "Не спрашивайте, я здесь просто приключаюсь",
        "Долой штаны!",
        "d, ваша собака или убийца?",
        "БЕСПЛАТНЫЙ МОПС И ТРИТОН!",
        "Вперёд, команда муравьёв!",
        "Есть тритон?",
        "Привет, мои дорогие!", /* Charlie Drake */
        "Эй, нимфы! Украдите эту футболку!",
        "Я <3 Подземелья Рока",
        "Я <3 Мод",
        /* note: there is a similarly worded apron (alchemy smock) slogan */
        "Я валькирия. Если увидите, что я бегу, попробуйте догнать.",
        "Я не крыса-собиратель - я коллекционер",
        "Я отскочил от резинового дерева",         /* Monkey Island */
        "Клуб 'Серный пляж' острова Грабителей", /* Monkey Island */
        "Если вы можете прочитать это, я могу ударить вас древковым оружием",
        "Я в замешательстве!",
        "У меня получилось с принцессой",
        "Я хочу жить вечно или умереть в попытке.",
        "Парк 'Лишайник'",
        "ПОТЕРЯЛСЯ В МЫСЛЯХ - пришлите поисковый отряд",
        "Мясо - это Мордор",
        "Бюро добрых услуг Горняцкого городка",
        "Стража Горняцкого городка",
        /* Discworld riff; unfortunately long */
        ("Дом нежных ласк мадам Палм - весьма респектабельное"
            " заведение с дурной репутацией"),
        "Защитный рэкетир",
        "Настоящие мужчины любят Крома",
        "Кто-то украл мой талисман!",
        "Банда адских гончих",
        "Оборотни",
        "Они могут быть штормовыми великанами",
        "Не оружие убивает людей, а я",
        "Белый зомби",
        "Вы меня убиваете!",
        "Государственный университет Анхура - родина воинственных огненных муравьёв!",
        "БЕСПЛАТНЫЕ ОБЪЯТИЯ",
        "Сериальный восходитель",
        "Настоящие мужчины - валькирии",
        "Ассоциация молодых копателей пещер",
        "Захватите форт Людиос",
        "Я не мог позволить себе эту футболку, поэтому украл её!",
        "Потрошители разума - отстой",
        "На мне нет штанов",
        "Долой живых!",
        "Фермер желейных слизней",
        "Вегетарианец",
        "Привет, я - Война!",
        "Лучше зажечь свечу, чем проклинать тьму",
        "Легче проклинать тьму, чем зажечь свечу",
...
hawaiian_motif(struct obj *shirt, char *buf)
{
    static const char *const hawaiian_motifs[] = {
        ...
        "попугай",
        "тукан",
        "райская птица", /* could be a bird or a flower */
        /* sea creatures */
        "морская черепаха",
        "тропическая рыба",
        "медуза",
...
apron_text(struct obj *apron, char *buf)
{
    static const char *const apron_msgs[] = {
        "Поцелуй повара",
        "Я творю НАУКУ!",
        "Не связывайся с шеф-поваром",
        "Не заставляй меня травить тебя",
        "Кухня Геенны",
        "Крыса: другое белое мясо",
        "Не выносишь жару - убирайся из Геенны!",
        "Если нам не положено есть животных, почему они сделаны из мяса?",
        "Если тебе не нравится еда, я заколю тебя",
        ...
        "Я алхимик; если увидите, что я бегу, попробуйте догнать...",
    };
...
static const char *const candy_wrappers[] = {
    ...
    "Аполлон",                   /* Lost */
    "Лунная хрустяшка",             /* South Park */
    "Пироженка",    "Шоколадный самородок", "Маленький батончик",
    "Хрустням-ням",   "Ванилькин хрустяшка", "Ягодный батончик",
    "Шоко-штучка",   "Ом-ном", /* Cat Macro */
    "Фруктовый овсянкин",              /* Serenity */
    "Батончик Вонка",                /* Charlie and the Chocolate Factory */
};
...
doread(void)
{
    static const char find_any_braille[] = "нащупать шрифт Брайля.";
    struct obj *scroll;
    ...
    scroll = getobj("прочитать", read_ok, GETOBJ_PROMPT);
    if (!scroll)
        return ECMD_CANCEL;
    otyp = scroll->otyp;
    ...
    if (otyp == FORTUNE_COOKIE) {
        if (flags.verbose)
            You("разламываете печенье и выбрасываете кусочки.");
        outrumor(bcsign(scroll), BY_COOKIE);
        if (!Blind)
            if (!u.uconduct.literate++)
                livelog_printf(LL_CONDUCT,
                               "стал грамотным, прочитав печенье с предсказанием");
        useup(scroll);
        return ECMD_TIME;
        ...
        if ((otyp == T_SHIRT || otyp == HAWAIIAN_SHIRT) && uarm
            && scroll == uarmu) {
            pline("%s рубашка скрыта %s%s.",
                  scroll->unpaid ? "Та" : "Ваша", shk_your(buf, uarm),
                  suit_simple_name(uarm));
            return ECMD_OK;
        }
        if (otyp == HAWAIIAN_SHIRT) {
            pline("%s изображает %s.", flags.verbose ? "Рисунок" : "Он",
                  hawaiian_design(scroll, buf));
            return ECMD_TIME;
        }
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "стал грамотным, прочитав %s",
                           (scroll->otyp == T_SHIRT) ? "футболку"
                           : "фартук");
        ...
        if (scroll->o_id % 3) {
            ...
            You_cant("найти ничего для чтения на этом %s.",
                     simpleonames(scroll));
            return ECMD_OK;
        }
        pline("%s на %s.  Надпись:  %s.",
              !Blind ? "Здесь есть надпись" : "Вы нащупываете буквы",
              simpleonames(scroll), cap_text);
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "стал грамотным, прочитав %s",
                           (otyp == DUNCE_CAP) ? "колпак дурака"
                                               : "корнутаум");
        ...
        static const char *const card_msgs[] = {
            "Лепрекон Голд Тра$т - карта 'Трилистник'",
            "Кредитная карта Волшебного хранилища памяти",
            "Национальный банк Ларна",                /* Larn */
            "Первый банк Омеги",               /* Omega */
            "Банк Зорка - волшебная карта Фробоцца", /* Zork */
            "Бартерная карта гильдии торговцев Анк-Морпорка",
            "Карта безлимитных операций гильдии воров Анк-Морпорка",
            "Ассоциация ростовщиков Рансмансби",
            "Банк Геенны - карта 'Проценты 99%'",
            "Йендорский экспресс - медная карта",
            "Йендорский экспресс - серебряная карта",
            "Йендорский экспресс - золотая карта",
            "Йендорский экспресс - мифриловая карта",
            "Йендорский экспресс - платиновая карта", /* must be last */
        };

        if (Blind) {
            You("нащупываете выпуклые цифры:");
        } else {
            if (flags.verbose)
                pline("Надпись:");
            pline("\"%s\"",
        ...
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                           "стал грамотным, прочитав кредитную карту");

        return ECMD_TIME;
    } else if (otyp == CAN_OF_GREASE) {
        pline("На этом %s нет этикетки.", singular(scroll, xname));
        return ECMD_OK;
        ...
        pline("\"Маркер Magic Marker(TM) %s, красные чернила.  Водорастворимый.\"",
              upwords(buf));
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                           "стал грамотным, прочитав волшебный маркер");

        return ECMD_TIME;
    } else if (scroll->oclass == COIN_CLASS) {
        if (Blind)
            You("нащупываете выпуклые слова:");
        else if (flags.verbose)
            You("читаете:");
        pline("\"1 зоркмид.  857 GUE.  Мы верим во Фробсов.\"");
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                           "стал грамотным, прочитав гравировку на монете");

        return ECMD_TIME;
    } else if (is_art(scroll, ART_ORB_OF_FATE)) {
        if (Blind)
            You("нащупываете выгравированную подпись:");
        else
            pline("Подписано:");
        pline("\"Один.\"");
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                   "стал грамотным, прочитав божественную подпись Одина");
        ...
        if (!*wrapper) {
            pline("Обёртка шоколадного батончика пуста.");
            return ECMD_OK;
        }
        pline("На обёртке написано: \"%s\".", wrapper);
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                           "стал грамотным, прочитав обёртку конфеты");
        ...
        if (otyp == SPE_NOVEL)
            ...
            what = "мистические руны";
        else if (!scroll->dknown)
            what = "формулу на свитке";
        if (what) {
            pline("Будучи слепым, вы не можете прочитать %s.", what);
            return ECMD_OK;
        }
    }
    ...
    if (otyp == SCR_MAIL) {
        ...
        if (!u.uconduct.literate) {
            if (!scroll->spe && y_n(
             "Чтение почты нарушит чистоту \"неграмотного\".  Всё равно прочитать?"
                                   ) != 'y')
                return ECMD_OK;
        }
    }
    ...
    if (otyp != SPE_BOOK_OF_THE_DEAD && otyp != SPE_NOVEL
        ...
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "стал грамотным, прочитав %s",
                           (scroll->oclass == SPBOOK_CLASS) ? "книгу"
                           : (scroll->oclass == SCROLL_CLASS) ? "свиток"
                             : something);
    ...
    if (otyp != SCR_BLANK_PAPER) {
        ...
        if (Blind)
            pline(nodisappear
                      ? "Вы %s формулу на свитке."
                      : "Пока вы %s формулу на нём, свиток исчезает.",
                  silently ? "обдумываете" : "произносите");
        else
            pline(nodisappear ? "Вы читаете свиток."
                              : "Когда вы читаете свиток, он исчезает.");
        if (confused) {
            if (Hallucination)
                pline("Будучи совсем обкуренным, вы всё портите...");
            else
                pline("Будучи сбитым с толку, вы %s магические слова...",
                      silently ? "неправильно понимаете" : "неправильно произносите");
        }
    }
    if (!seffects(scroll)) {
...
stripspe(struct obj *obj)
{
    if (obj->blessed || obj->spe <= 0) {
        ...
        pline("%s недолго.", Yobjnam2(obj, "вибрирует"));
...
{
    pline("%s недолго.", Yobjnam2(otmp, Blind ? "вибрирует" : "светится"));
}

staticfn void
p_glow2(struct obj *otmp, const char *color)
{
    pline("%s%s%s на мгновение.", Yobjnam2(otmp, Blind ? "вибрирует" : "светится"),
...
{
    pline("%s слабо%s%s на мгновение.",
...
recharge(struct obj *obj, int curse_bless)
{
    ...
    if (obj->oclass == WAND_CLASS) {
        ...
        if (obj->spe > rn2(7) || obj->spe <= -5) {
            pline("%s мгновение, затем %s!", Yobjnam2(obj, "пульсирует"),
            ...
            pline("%s %s.", Yname2(obj),
                  s < 0 ? "вращается против часовой стрелки" : "вращается по часовой стрелке");
            if (s < 0)
        ...
        case EXPENSIVE_CAMERA:
            if (is_cursed) {
                ...
                if (obj->spe < 3)
                    Your("маркер, похоже, окончательно высох.");
                else
                    pline1(nothing_happens);
            } else if (is_blessed) {
                n = rn1(16, 15); /* 15..30 */
...
seffect_enchant_armor(struct obj **sobjp)
{
    ...
    if (!otmp) {
        strange_feeling(sobj, !Blind
                        ? "Ваша кожа светится, затем гаснет."
                        : "Ваша кожа на мгновение теплеет.");
    ...
    if (confused) {
        ...
        if (Blind) {
            otmp->rknown = FALSE;
            pline("%s на мгновение.", Yobjnam2(otmp, "теплеет"));
        } else {
            otmp->rknown = TRUE;
            pline("%s %s %s!", Yobjnam2(otmp, "покрывается"),
                  scursed ? "чёрным пятнистым" : "золотым мерцающим",
                  scursed ? "сиянием"
                  : (is_shield(otmp) ? "слоем" : "щитом"));
        }
        if (new_erodeproof && (otmp->oeroded || otmp->oeroded2)) {
            otmp->oeroded = otmp->oeroded2 = 0;
            pline("%s как новый!",
                  Yobjnam2(otmp, Blind ? "чувствуется" : "выглядит"));
        }
    ...
    if (s > (special_armor ? 5 : 3) && rn2(s)) {
        otmp->in_use = TRUE;
        pline("%s яростно %s%s%s некоторое время, затем %s.", Yname2(otmp),
              otense(otmp, Blind ? "вибрирует" : "светится"),
              (!Blind && !same_color) ? " " : "",
              (Blind || same_color) ? "" : hcolor(scursed ? NH_BLACK
                                                  : NH_SILVER),
              otense(otmp, "испаряется"));
    ...
    if (s >= 0 && Is_dragon_scales(otmp)) {
        ...
        pline("%s сливается и затвердевает!", Yname2(otmp));
    ...
    pline("%s %s%s%s%s на %s.", Yname2(otmp),
          (s == 0) ? "яростно " : "",
          otense(otmp, Blind ? "вибрирует" : "светится"),
          (!Blind && !same_color) ? " " : "",
          (Blind || same_color)
          ? "" : hcolor(scursed ? NH_BLACK : NH_SILVER),
          (s * s > 1) ? "некоторое время" : "мгновение");
    ...
    if ((otmp->spe > (special_armor ? 5 : 3))
        && (special_armor || !rn2(7)))
        pline("%s.", Yobjnam2(otmp, Blind ? "снова вибрирует" : "внезапно вибрирует"));
}
...
seffect_destroy_armor(struct obj **sobjp)
{
    ...
    if (confused) {
        if (!otmp) {
            strange_feeling(sobj, "Ваши кости чешутся.");
            *sobjp = 0; /* useup() in strange_feeling() */
    ...
    if (scursed) {
        ...
        if (gets_choice) {
            ...
            if (!objects[sobj->otyp].oc_name_known)
                pline("Это %s!", an(actualoname(sobj)));
            gk.known = TRUE;
            atmp = getobj("уничтожить", any_worn_armor_ok, GETOBJ_PROMPT);
            ...
            strange_feeling(sobj, "Ваша кожа чешется.");
            *sobjp = 0; /* useup() in strange_feeling() */
            exercise(A_STR, FALSE);
            exercise(A_CON, FALSE);
            return;
...
seffect_confuse_monster(struct obj **sobjp)
{
    ...
    if (gy.youmonst.data->mlet != S_HUMAN || scursed) {
        if (!HConfusion)
            You_feel("замешательство.");
        make_confused(HConfusion + rnd(100), FALSE);
    } else if (confused) {
        if (!sblessed) {
            Your("%s начинают %s%s.", hands,
                 altfeedback ? "покалывать" : "светиться ",
                 altfeedback ? "" : hcolor(NH_PURPLE));
            make_confused(HConfusion + rnd(100), FALSE);
        } else {
            if (altfeedback)
                pline("Вашу %s окружает слабый гул.", body_part(HEAD));
            else
                pline("Вашу %s окружает красное свечение.", body_part(HEAD));
        ...
        if (!sblessed) {
            if (altfeedback)
                Your("%s %s.", hands, u.umconf ? "покалывают ещё сильнее" : "покалывают");
            else if (!u.umconf)
                Your("%s начинают светиться %s.", hands, hcolor(NH_RED));
            else
                pline_The("%s сияние ваших %s усиливается.", hcolor(NH_RED),
                          hands);
            incr += rnd(2);
        } else {
            if (altfeedback)
                Your("%s %s.", hands,
                     u.umconf ? "сильно покалывают ещё сильнее" : "сильно покалывают");
            else
                Your("%s светятся %s %s.", hands,
                     u.umconf ? "ещё более ярким" : "ярким", hcolor(NH_RED));
            incr += rn1(8, 2);
        }
...
seffect_scare_monster(struct obj **sobjp)
{
    ...
    if (otyp == SCR_SCARE_MONSTER || !ct) {
        ...
        You_hear("%s %s.", (confused || scursed) ? "печальные завывания"
                 : "маниакальный смех",
                 !ct ? "вдалеке" : "рядом");
    }
}
...
seffect_remove_curse(struct obj **sobjp)
{
    ...
    You_feel(!Hallucination
             ? (!confused ? "как будто кто-то помогает вам."
                : "как будто вам нужна помощь.")
             : (!confused ? "себя единым со Вселенной."
                : "силу Силы против вас!"));

    if (scursed) {
        pline("Свиток рассыпается.");
    } else {
    ...
    if (u.utrap && u.utraptype == TT_BURIEDBALL) {
        ...
        pline("Застёжка на вашей %s исчезает.", body_part(LEG));
    }
    update_inventory();
}
...
seffect_enchant_weapon(struct obj **sobjp)
{
    ...
    if (confused && uwep
        ...
        if (Blind) {
            uwep->rknown = FALSE;
            Your("оружие на мгновение теплеет.");
        } else {
            uwep->rknown = TRUE;
            pline("%s %s %s!", Yobjnam2(uwep, "покрывается"),
                  scursed ? "фиолетовым пятнистым" : "золотым мерцающим",
                  scursed ? "сиянием" : "щитом");
        }
        if (new_erodeproof && (uwep->oeroded || uwep->oeroded2)) {
            uwep->oeroded = uwep->oeroded2 = 0;
            pline("%s как новый!",
                  Yobjnam2(uwep, Blind ? "чувствуется" : "выглядит"));
        }
        if (old_erodeproof && !new_erodeproof) {
            /* restore old_erodeproof before shop charges */
            uwep->oerodeproof = 1;
...
seffect_taming(struct obj **sobjp)
{
    ...
    if (!results) {
        pline("Ничего интересного %s.",
              !candidates ? "не происходит" : "кажется, не происходит");
    } else {
        pline("Округа %s становится %sдружелюбнее.",
...
seffect_light(struct obj **sobjp)
{
    ...
    if (!confused) {
        ...
        if ((svm.mvitals[pm].mvflags & G_GONE)) {
            pline("В воздухе на мгновение вспыхивают крошечные огоньки.");
        } else {
            ...
            if (sawlights) {
                pline("Вокруг вас появляются огоньки!");
                gk.known = TRUE;
            }
        }
    }
...
seffect_charging(struct obj **sobjp)
{
    ...
    useup(sobj);
    *sobjp = 0; /* it's gone */
    otmp = getobj("зарядить", charge_ok, GETOBJ_PROMPT | GETOBJ_ALLOWCNT);
    if (otmp)
...
seffect_amnesia(struct obj **sobjp)
{
    ...
    if (Hallucination) /* Ommmmmm! */
        Your("разум освобождается от мирских забот.");
    else if (!strncmpi(svp.plname, "Maud", 4))
        pline("Когда ваш разум обращается внутрь себя,"
              " вы забываете обо всём.");
    else if (rn2(2))
        pline("Кто вообще такая эта Мауд?");
    else
        pline("Думая о Мауд, вы забываете обо всём.");
...
seffect_fire(struct obj **sobjp)
{
    ...
    *sobjp = 0; /* it's gone */
    ...
    if (confused) {
        if (Underwater) {
            pline("Немного %s вокруг вас испаряется.", hliquid("water"));
        }
        else if (Fire_resistance) {
            shieldeff(u.ux, u.uy);
            monstseesu(M_SEEN_FIRE);
            if (!Blind)
                pline("О, смотрите, какой красивый огонь в ваших %s.",
                      makeplural(body_part(HAND)));
            else
                You_feel("приятное тепло в ваших %s.",
                         makeplural(body_part(HAND)));
        } else {
    ...
    if (Underwater) {
        pline("%s Вокруг вас яростно испаряется!", hliquid("water"));
    } else {
        if (sblessed) {
            if (!already_known)
                pline("Это свиток огня!");
            dam *= 5;
            pline("Где вы хотите разместить центр взрыва?");
            getpos_sethilite(display_stinking_cloud_positions,
                             can_center_cloud);
            (void) getpos(&cc, TRUE, "желаемое положение");
        ...
        if (u_at(cc.x, cc.y)) {
            pline("Свиток вспыхивает столбом пламени!");
    ...
    if (!Is_rogue_level(&u.uz) && has_ceiling(&u.uz)
        ...
        if (u.uswallow) {
            ...
            if (!avoid_ceiling(&u.uz)) {
                pline_The("%s грохочет %s!", ceiling(u.ux, u.uy),
                          sblessed ? "вокруг вас" : "над вами");
            } else {
                char matbuf[BUFSZ];
                ...
                pline("%s %s %s!",
                      upstart(matbuf), vtense(matbuf, "материализуется"),
                      sblessed ? "кругом вас" : "над вами");
            }
        }
        gk.known = 1;
        sokoban_guilt();
        ...
        if (!sblessed) {
            ...
            pline("Но больше ничего не происходит.");
    }
}
...
seffect_stinking_cloud(struct obj **sobjp)
{
    ...
    if (!already_known)
        You("нашли свиток зловонного облака!");
    gk.known = TRUE;
...
{
    if (Blind)
        You("не помните никаких магических слов на этом свитке.");
    else
        pline("Этот свиток, похоже, пуст.");
    gk.known = TRUE;
}
...
seffect_identify(struct obj **sobjp)
{
    ...
    if (is_scroll) { /* scroll of identify */
        ...
        *sobjp = 0; /* it's gone */
        ...
        if (confused || (scursed && !already_known))
            You("опознаёте это как свиток опознания.");
        else if (!already_known)
            pline("Это свиток опознания.");
        if (!already_known)
    ...
    if (gi.invent) {
        ...
        pline("Вы не несёте ничего%s для опознания.",
              (is_scroll) ? " ещё" : "");
    }
}
...
seffect_magic_mapping(struct obj **sobjp)
{
    ...
    if (is_scroll) {
        if (svl.level.flags.nommap) {
            Your("разум заполнен безумными линиями!");
            if (Hallucination)
                pline("Вау!  Современное искусство.");
            else
                Your("%s кружится в замешательстве.", body_part(HEAD));
    ...
    if (svl.level.flags.nommap) {
        Your("%s кружится, пока %s блокирует заклинание!", body_part(HEAD),
             something);
        make_confused(HConfusion + rnd(30), FALSE);
        return;
    }
    pline("В вашем разуме формируется карта!");
    ...
    if (cval) {
        ...
        pline("К сожалению, вы не можете уловить детали.");
    }
}
...
seffect_mail(struct obj **sobjp)
{
    ...
    case 2:
        ...
        pline("На этом свитке написано \"%s\".",
              odd ? "Оплата за пересылку" : "Вернуть отправителю");
        break;
    case 1:
        /* scroll of mail obtained from bones file or from wishing;
           ...
           mail if you win! */
        pline("Это, похоже, %s.",
              odd ? "письмо счастья, угрожающее вашей удаче"
              : "мусорное письмо, адресованное нашедшему Око Ларна");
        break;
    default:
#ifdef MAIL
        readmail(sobj);
    ...
    boolean skip_uswallow)
{
    ...
    if (!amorphous(gy.youmonst.data) && !Passes_walls
        ...
        You("в вас попадает %s!", doname(otmp2));
        dmg = (int) (dmgval(otmp2, &gy.youmonst) * otmp2->quan);
        if (uarmh && helmet_protects) {
            if (hard_helmet(uarmh)) {
                pline("К счастью, на вас прочный шлем.");
                if (dmg > 2)
                    dmg = 2;
            } else if (flags.verbose) {
                pline("%s вас не защищает.", Yname2(uarmh));
            }
        }
    } else
        dmg = 0;
...
drop_boulder_on_monster(coordxy x, coordxy y, boolean confused, boolean byu)
{
    ...
    if (mtmp && !amorphous(mtmp->data) && !passes_walls(mtmp->data)
        ...
        if (cansee(mtmp->mx, mtmp->my)) {
            pline("В %s попадает %s!", Monnam(mtmp), doname(otmp2));
            if (mtmp->minvis && !canspotmon(mtmp))
                map_invisible(mtmp->mx, mtmp->my);
        } else if (engulfing_u(mtmp))
            You_hear("что-то ударило %s %s над твоим %s!",
        ...
        if (helmet) {
            if (hard_helmet(helmet)) {
                if (canspotmon(mtmp))
                    pline("К счастью, %s носит прочный шлем.",
                          mon_nam(mtmp));
                else if (!Deaf)
                    You_hear("звон.");
                if (mdmg > 2)
                ...
                if (canspotmon(mtmp))
                    pline("%s %s не защищает %s.", Monnam(mtmp),
...
{
    const char *expl = !chg ? "внезапно" : "яростно вибрирует и";
    int dmg, n, k;
    ...
    struct obj *obj) /* scroll, spellbook (for spell), or wand of light */
{
    ...
    if (!on) {
        ...
        if (!Blind) {
            ...
            if (still_lit)
                pline("Окружающий свет кажется более тусклым.");
            else if (u.uswallow)
                pline("Здесь стало ещё темнее, чем раньше.");
            else
                You("окружены тьмой!");
        }
    } else { /* on */
        ...
        if (u.uswallow) {
            if (Blind)
                ...
                pline("%s %s освещён.", s_suffix(Monnam(u.ustuck)),
                      mbodypart(u.ustuck, STOMACH));
            else if (is_whirly(u.ustuck->data))
                pline("%s недолго сияет.", Monnam(u.ustuck));
            else
                pline("%s поблёскивает.", Monnam(u.ustuck));
        } else if (!Blind && (!Is_rogue_level(&u.uz)
                              || levl[u.ux][u.uy].typ != CORR)) {
            pline("Световое поле %sокружает вас!", no_op ? "на мгновение " : "");
        }
    }
...
do_class_genocide(void)
{
    ...
    for (j = 0; ; j++) {
        ...
        Strcpy(promptbuf, "Каких монстров по классу вы хотите истребить?");
        if (j > 0)
            Snprintf(eos(promptbuf), sizeof promptbuf - strlen(promptbuf),
                     " [введите %s]",
                     iflags.cmdassist
                       ? "символ или имя, представляющее класс, или '?'"
                       : "'?' чтобы увидеть предыдущие геноциды");
        getlin(promptbuf, buf);
        (void) mungspaces(buf);
        /* avoid 'that does not represent any monster' for empty input */
        if (!*buf) {
            pline("%s.", (j + 1 < 5)
                         ? "Введите букву (или знак препинания)"
                           " или имя, используемое для класса монстров, или 'none'"
                         /* next iteration gives "that's enough tries"
                            so don't suggest typing anything this time */
                         : "Класс монстров не указан");
            continue; /* try again */
        }
        ...
        if (*buf == '\033' || !strcmpi(buf, "none")
            ...
            livelog_printf(LL_GENOCIDE,
                           "отказался от проведения геноцида классов");
            return;
        }
        ...
        if (!goodcnt && class != mons[gu.urole.mnum].mlet
            ...
            if (gonecnt)
                pline("Все такие монстры уже не существуют.");
            else if (immunecnt || class == S_invisible)
                You("не имеете права истреблять таких монстров.");
                ...
                pline("Уничтожено %d монстров.", gonecnt);
                return;
            } else
                pline("Этот %s не обозначает никакого монстра.",
                      strlen(buf) == 1 ? "символ" : "ответ");
            continue;
        }
        ...
        for (i = LOW_PM; i < NUMMONS; i++) {
            if (mons[i].mlet == class) {
                ...
                if (Your_Own_Role(i) || Your_Own_Race(i)
                    ...
                    if (!ll_done++) {
                        if (!num_genocides())
                            livelog_printf(LL_CONDUCT | LL_GENOCIDE,
                                     "совершил %s первый геноцид (класс %c)",
                                           uhis(), def_monsyms[class].sym);
                        else
                            livelog_printf(LL_GENOCIDE, "уничтожил класс %c",
                    ...
                    if (!gameover)
                        pline("%s уже не существуют.", upstart(nam));
                } else if (!gameover) {
                    ...
                    if ((mons[i].msound != MS_LEADER
                        ...
                        You("не имеете права истреблять %s%s.",
                            (uniq && !named) ? "the " : "",
        ...
        if (gameover || u.uhp == -1) {
            ...
            Strcpy(svk.killer.name, "свиток геноцида");
            if (gameover)
                done(GENOCIDED);
        }
        return;
              ...
              * 5 (4 | 1) = normal genocide from throne */
{
    ...
    if (how & PLAYER) {
        ...
        for (i = 0; ; i++) {
            ...
            Strcpy(promptbuf,
                   "Какой тип монстра вы хотите истребить?");
            if (i > 0)
                Snprintf(eos(promptbuf), sizeof promptbuf - strlen(promptbuf),
                         " [введите %s]",
                         iflags.cmdassist
                           ? "имя типа монстра или '?'"
                           : "'?' чтобы увидеть предыдущие геноциды");
            getlin(promptbuf, buf);
            (void) mungspaces(buf);
            ...
            if (!*buf) {
                pline("%s.", (i + 1 < 5)
                             ? "Введите имя типа монстра или 'none'"
                             /* next iteration gives "that's enough tries"
                                so don't suggest typing anything this time */
                             : "Тип монстра не указан");
                continue; /* try again */
            }
            /* choosing "none" preserves genocideless conduct */
            if (*buf == '\033' || !strcmpi(buf, "none")
                ...
                livelog_printf(LL_GENOCIDE, "отказался от проведения геноцида");
                return;
            }
            ...
            if (mndx == NON_PM || (svm.mvitals[mndx].mvflags & G_GENOD)) {
                pline("Такие существа %s существуют в этом мире.",
                      (mndx == NON_PM) ? "не" : "больше не");
                continue;
            }
            ptr = &mons[mndx];
            ...
            if (!(ptr->geno & G_GENO)) {
                if (!Deaf) {
                     ...
                     */
                    if (flags.verbose)
                        pline("Громовой голос гремит"
                              " в пещерах:");
                    SetVoice((struct monst *) 0, 0, 80, voice_deity);
                    /* FIXME? shouldn't this override deafness? */
                    verbalize("Нет, смертный!  Этого не будет.");
                }
                continue;
            }
    ...
    if (how & REALLY) {
        if (!num_genocides())
            livelog_printf(LL_CONDUCT | LL_GENOCIDE,
                           "совершил %s первый геноцид (%s)",
                           uhis(), makeplural(realbuf));
        else
            livelog_printf(LL_GENOCIDE, "уничтожил %s", makeplural(realbuf));

        /* setting no-corpse affects wishing and random tin generation */
        svm.mvitals[mndx].mvflags |= (G_GENOD | G_NOCORPSE);
        pline("Уничтожено %s%s.", which,
        ...
        if (killplayer) {
            ...
            if (how & PLAYER) {
                ...
                Strcpy(svk.killer.name, "геноцидное замешательство");
            } else if (how & ONTHRONE) {
                /* player selected while on a throne */
                svk.killer.format = KILLED_BY_AN;
                Strcpy(svk.killer.name, "властный приказ");
        ...
        if (cnt) {
            ...
            pline("Отправлено %s%s.", (cnt > 1) ? "some " : "",
...
punish(struct obj *sobj)
{
    ...
    if (!reuse_ball)
        You("наказаны за проступок!");
    if (Punished) {
        Your("железный шар тяжелеет.");
    ...
    if (amorphous(gy.youmonst.data) || is_whirly(gy.youmonst.data)
        ...
        if (!reuse_ball) {
            pline("Появляется шар с цепью, затем падает в сторону.");
...
do_stinking_cloud(struct obj *sobj, boolean mention_stinking)
{
    ...
    pline("Где вы хотите разместить центр %sоблака?",
          mention_stinking ? "зловонного " : "");
    cc.x = u.ux;
    cc.y = u.uy;
    ...
    if (getpos(&cc, TRUE, "желаемое положение") < 0) {
        ...
        if (Hallucination)
            pline("Фу... кто-то испортил воздух.");
        else
            pline("%s запах тухлых яиц.",
                  sobj->oclass == SCROLL_CLASS ? "Свиток рассыпается с"
                                               : "Вы чувствуете");
        return;
    }
    ...
    struct _create_particular_data *d)
{
    ...
    if (!d->randmonst) {
        ...
        if (cant_revive(&d->which, FALSE, (struct obj *) 0)
            ...
            Sprintf(buf, "Создать %s вместо; принудительно %s?",
...
#define CP_TRYLIM 5
    ...
    buf[0] = '\0'; /* for EDIT_GETLIN */
    Strcpy(prompt, "Создать какого монстра?");
    do {
        getlin(prompt, buf);
        bufp = mungspaces(buf);
        if (*bufp == '\033')
        ...
        if (*bufp || altmsg || tryct < 2) {
            pline("Я никогда не слышал о таких монстрах.");
        } else {
            pline("Попробуйте снова (введите * для случайного, ESC для отмены).");
            ++altmsg;
        }
        /* when a second try is needed, expand the prompt */
        if (tryct == CP_TRYLIM)
            Strcat(prompt, " [введите имя или символ]");
    } while (--tryct > 0);
