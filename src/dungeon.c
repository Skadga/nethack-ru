...
fixup_level_locations(void)
{
     ...
     */
    quest_dnum = dname_to_dnum("Квест");
    sokoban_dnum = dname_to_dnum("Сокобан");
    mines_dnum = dname_to_dnum("Гномьи шахты");
    tower_dnum = dname_to_dnum("Башня Влада");
    tutorial_dnum = dname_to_dnum("Обучение");
...
init_dungeons(void)
{
    ...
    if (!nhl_loadlua(L, DUNGEON_FILE)) {
        char tbuf[BUFSZ];
        Sprintf(tbuf, "Не удалось открыть описание подземелья - \"%s", DUNGEON_FILE);
...
earth_sense(void)
{
    ...
    for (otmp = svl.level.buriedobjlist; otmp; otmp = otmp->nobj)
        if (u_at(otmp->ox, otmp->oy)) {
            You("ощущаете что-то, зарытое под вашими ногами.");
            return;
        }
}
...
surface(coordxy x, coordxy y)
{
    ...
    if (u_at(x, y) && u.uswallow && is_animal(u.ustuck->data))
        ...
        return digests(u.ustuck->data) ? "пасть"
               : enfolds(u.ustuck->data) ? "оболочка"
                 : "неведомое"; /* can't happen (fingers crossed...) */
    else if (IS_AIR(levtyp))
        return Is_waterlevel(&u.uz) ? "воздушный пузырь"
                                    : (levtyp == CLOUD) ? "облако" : "воздух";
    else if (is_pool(x, y))
        ...
        return "стена"; /* 'surface' during Passes_walls */
    else if (IS_DOOR(levtyp))
        return "дверной проём"; /* even for closed door */
...
/* Русские названия поверхностей в предложном падеже. */
const char *
surface_ru(coordxy x, coordxy y)
{
    struct rm *lev = &levl[x][y];
    int levtyp = SURFACE_AT(x, y);
    ...
    if (u_at(x, y) && u.uswallow && is_animal(u.ustuck->data))
        ...
        return Is_waterlevel(&u.uz) ? "воздушном пузыре"
                                    : (levtyp == CLOUD) ? "облаке" : "воздухе";
    else if (is_pool(x, y))
        return "воде";
    else if (is_ice(x, y))
        return "льду";
...
print_dungeon(boolean bymenu, schar *rlev, xint16 *rdgn)
{
    ...
    for (i = 0, dptr = svd.dungeons; i < svn.n_dgns; i++, dptr++) {
        ...
        if (nlev > 1) {
            descr = unplaced ? "глубины" : "уровни";
            Snprintf(buf, sizeof buf, "%s: %s %d-%d", dptr->dname, descr,
                     dptr->depth_start, dptr->depth_start + nlev - 1);
        } else {
            descr = unplaced ? "глубина" : "уровень";
        ...
        if (dptr->entry_lev != 1) {
            if (dptr->entry_lev == nlev)
                Strcat(buf, ", вход снизу");
            else
                Sprintf(eos(buf), ", вход на уровне %d",
        ...
        for (slev = svs.sp_levchn, last_level = 0; slev; slev = slev->next) {
            ...
            if (Is_stronghold(&slev->dlevel))
                Sprintf(eos(buf), " (наигрыш %s)", svt.tune);
            if (bymenu)
    ...
    if (bymenu) {
        ...
        end_menu(win, "На какой уровень телепортироваться?");
    ...
    for (first = TRUE, br = svb.branches; br; br = br->next) {
        if (br->end1.dnum == svn.n_dgns) {
            if (first) {
                putstr(win, 0, "");
                putstr(win, 0, "Плавающие ветви");
                first = FALSE;
            }
            Sprintf(buf, "   %s к %s", br_string(br->type),
    ...
    if (Invocation_lev(&u.uz)) {
        putstr(win, 0, "");
        Sprintf(buf, "Место призыва @ (%d,%d), герой @ (%d,%d)",
        ...
        *buf = '\0';
        ...
        if (trap)
            Sprintf(buf, "Портал @ (%d,%d), герой @ (%d,%d)",
        ...
        /* only report "no portal found" when actually expecting a portal */
                 ...
                 || Is_qstart(&u.uz) || at_dgn_entrance("Квест")
                 || Is_knox(&u.uz))
            Strcpy(buf, "Портал не найден.");
...
print_level_annotation(void)
{
    ...
    if ((annotation = get_annotation(&u.uz)) != 0)
        You("запоминаете этот уровень как %s.", annotation);
}
...
query_annotation(d_level *lev)
{
    ...
    if (mptr->custom) {
        ...
        Sprintf(tmpbuf, "Заменить аннотацию \"%.30s%s\" на?", mptr->custom,
...
#endif
    {
        ...
        if (!lev || on_level(&u.uz, lev)) {
            Strcpy(lbuf, "этот уровень подземелья");
        } else {
        ...
        Snprintf(qbuf, sizeof qbuf, "Как вы хотите назвать %s?", lbuf);
        getlin(qbuf, nbuf);
    }
...
recalc_mapseen(void)
{
    ...
    mptr->flags.quest_summons = (at_dgn_entrance("Квест")
                                 && u.uevent.qcalled
                                 && !(u.uevent.qcompleted
                                      || u.uevent.qexpelled
...
br_string2(branch *br)
{
    ...
    switch (br->type) {
    case BR_PORTAL:
        return closed_portal ? "Запечатанный портал" : "Портал";
    case BR_NO_END1:
        return "Соединение";
    case BR_NO_END2:
        return br->end1_up ? "Односторонняя лестница вверх" : "Односторонняя лестница вниз";
    case BR_STAIR:
        return br->end1_up ? "Лестница вверх" : "Лестница вниз";
    }
...
endgamelevelname(char *outbuf, int indx)
{
    ...
    *outbuf = '\0';
    ...
    if (planename)
        Sprintf(outbuf, "План %s", planename);
    else if (!*outbuf)
        Sprintf(outbuf, "неизвестный план #%d", indx);
    return outbuf;
}
...
shop_string(int rtype)
{
    ...
    const char *str = "лавка?"; /* catchall */

    if (shoptype < 0) {
        str = "необслуженная лавка";
    ...
    *outbuf = '\0';
    if (mptr->flags.castletune && u.uevent.uheard_tune) {
        ...
        if (u.uevent.uheard_tune == 2)
            Sprintf(tmp, "наигрыш «%s»", svt.tune);
        else
            Strcpy(tmp, "наигрыш из 5 нот");
        Snprintf(outbuf, bsz, " (наиграйте %s, чтобы открыть или закрыть разводной мост)", tmp);
    }
    return outbuf;
}
...
/* RU localization: builds "1 %s" / "N %s" with proper Russian plural forms */
static const char *
numbuf(const char *sing, long n)
{
    static char numbuf_out[2][BUFSZ];
    static int numbuf_idx = 0;
    static const char *const cntforms[][3] = {
        { "лавка", "лавки", "лавок" },
        { "храм", "храма", "храмов" },
        { "алтарь", "алтаря", "алтарей" },
        { "трон", "трона", "тронов" },
        { "фонтан", "фонтана", "фонтанов" },
        { "раковина", "раковины", "раковин" },
        { "могила", "могилы", "могил" },
        { "дерево", "дерева", "деревьев" },
    };
    long r100, r10;
    const char *what = sing;
    int j;
    ...
    if (n == 1) {
        ...
        r100 = n % 100;
        r10 = n % 10;
        for (j = 0; j < (int) SIZE(cntforms); j++)
            if (!strcmp(sing, cntforms[j][0])) {
                what = (r100 >= 11 && r100 <= 14) ? cntforms[j][2]
                    : (r10 == 1) ? cntforms[j][0]
                      : (r10 >= 2 && r10 <= 4) ? cntforms[j][1]
                        : cntforms[j][2];
                break;
            }
    }
    ...
    numbuf_idx ^= 1;
    if (n == 1)
        Strcpy(numbuf_out[numbuf_idx], what);
    else
        Sprintf(numbuf_out[numbuf_idx], "%ld %s", n, what);
    return numbuf_out[numbuf_idx];
}
...
#define ADD2NTOBUF(nam, var, nam2, var2) \
    do {                                                   \
        if (var && var2)                                   \
            Sprintf(eos(buf), "%s%s и %s", COMMA,          \
                    numbuf((nam), (var)), numbuf((nam2), (var2))); \
        else if (var)                                      \
            ADDNTOBUF(nam, var);                           \
        else if (var2)                                     \
            ADDNTOBUF(nam2, var2);                         \
    ...
    boolean printdun)
{
    ...
    if (In_endgame(&mptr->lev))
        ...
        Sprintf(buf, "%sУровень %d:", (final != -1) ? TAB : "", i);
    ...
    if (on_level(&u.uz, &mptr->lev))
        Sprintf(eos(buf), " <- Вы %s.",
                (final <= 0 || (final == 1 && how == ASCENDED)) ? "находитесь здесь"
                  : (final == 1 && how == ESCAPED) ? "ушли отсюда"
                    : "находились здесь");
    ...
    if (OF_INTEREST(mptr->feat)) {
        ...
        if (mptr->feat.nshop > 0) {
            if (mptr->feat.nshop > 1)
                ADDNTOBUF("лавка", mptr->feat.nshop);
            else
                Sprintf(eos(buf), "%s%s", COMMA,
        ...
        if (mptr->feat.naltar > 0 || mptr->feat.ntemple > 0) {
            ...
            /* being aware of a temple doesn't guarantee being aware of its
               ...
               to say so */
            ADD2NTOBUF("храм", mptr->feat.ntemple,
                       "алтарь", mptr->feat.naltar);
            ...
            if (Amask2align(atmp) == u.ualign.type) /* -128, -1,  0, +1 */
                Sprintf(eos(buf), " к %s", align_gname(u.ualign.type));
        }
        ADDNTOBUF("трон", mptr->feat.nthrone);
        ADDNTOBUF("фонтан", mptr->feat.nfount);
        ADDNTOBUF("раковина", mptr->feat.nsink);
        ADDNTOBUF("могила", mptr->feat.ngrave);
        ADDNTOBUF("дерево", mptr->feat.ntree);
#if 0
        ADDTOBUF("вода", mptr->feat.water);
        ADDTOBUF("лава", mptr->feat.lava);
        ADDTOBUF("лёд", mptr->feat.ice);
#endif
    ...
    *buf = '\0';
    if (mptr->flags.oracle) {
        Sprintf(buf, "%sОракул Дельфийский.", PREFIX);
    } else if (In_sokoban(&mptr->lev)) {
        Sprintf(buf, "%s%s.", PREFIX,
                mptr->flags.sokosolved ? "Решено" : "Не решено");
    } else if (mptr->flags.bigroom) {
        Sprintf(buf, "%sОчень большая комната.", PREFIX);
    } else if (mptr->flags.roguelevel) {
        Sprintf(buf, "%sПримитивная область.", PREFIX);
    } else if (on_level(&mptr->lev, &qstart_level)) {
        Sprintf(buf, "%sДом%s.", PREFIX,
                mptr->flags.notreachable ? " (нет пути назад...)" : "");
        if (u.uevent.qcompleted)
            Sprintf(buf, "%sКвест для %s выполнен.", PREFIX, ldrname());
        else if (mptr->flags.questing)
            Sprintf(buf, "%sКвест дан %s.", PREFIX, ldrname());
        ...
        Sprintf(buf, "%sФорт Лудиос.", PREFIX);
    } else if (mptr->flags.castle) {
        Snprintf(buf, sizeof buf, "%sЗамок%s.", PREFIX,
                tunesuffix(mptr, tmpbuf, sizeof tmpbuf));
    } else if (mptr->flags.valley) {
        Sprintf(buf, "%sДолина Мёртвых.", PREFIX);
    } else if (mptr->flags.vibrating_square) {
        Sprintf(buf, "%sВрата в святилище Молоха.", PREFIX);
    } else if (mptr->flags.msanctum) {
        Sprintf(buf, "%sСвятилище Молоха.", PREFIX);
    }
    if (*buf) {
        add_menu_str(win, buf);
    }
    ...
    if (mptr->flags.quest_summons) {
        Sprintf(buf, "%sПризван %s.", PREFIX, ldrname());
        add_menu_str(win, buf);
    }

    /* print out branches */
    if (mptr->br) {
        Sprintf(buf, "%s%s к %s", PREFIX, br_string2(mptr->br),
        ...
        if (mptr->br->end1_up && !In_endgame(&(mptr->br->end2)))
            Sprintf(eos(buf), ", уровень %d", depth(&(mptr->br->end2)));
        Strcat(buf, ".");
        add_menu_str(win, buf);
    }
    ...
    if (mptr->final_resting_place || final > 0) {
        ...
        for (bp = mptr->final_resting_place; bp; bp = bp->next)
            if (bp->bonesknown || wizard || final > 0)
                ++kncnt;
        if (kncnt) {
            Sprintf(buf, "%s%s", PREFIX, "Последнее пристанище:");
            add_menu_str(win, buf);
            if (died_here) {
                ...
                Snprintf(buf, sizeof(buf), "%s%sвы, %s%c", PREFIX, TAB,
                         tmpbuf, --kncnt ? ',' : '.');
                add_menu_str(win, buf);
            }
            for (bp = mptr->final_resting_place; bp; bp = bp->next) {
