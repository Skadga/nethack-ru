...
throne_mon_sound(struct monst *mtmp)
{
    if ((mtmp->msleeping || is_lord(mtmp->data)
        ...
        static const char *const throne_msg[4] = {
            "звуки светской беседы.",
            "скипетр, ударяемый при суде.",
            "Кто-то кричит \"Долой голову %s!\"",
            "Кошки королевы Берутиель!",
        };
...
beehive_mon_sound(struct monst *mtmp)
{
    if ((mtmp->data->mlet == S_ANT && is_flyer(mtmp->data))
        ...
        case 0:
            ...
            You_hear("тихое жужжание.");
            break;
        case 1:
            Soundeffect(se_angry_drone, 100);
            You_hear("злобное жужжание.");
            break;
        case 2:
...
morgue_mon_sound(struct monst *mtmp)
{
    if ((is_undead(mtmp->data) || is_vampshifter(mtmp))
        ...
        switch (rn2(2) + hallu) {
        case 0:
            You("внезапно понимаете, что неестественно тихо.");
            break;
        case 1:
...
zoo_mon_sound(struct monst *mtmp)
{
    if ((mtmp->msleeping || is_animal(mtmp->data))
        ...
        static const char *const zoo_msg[3] = {
            "звук, напоминающий слона, наступающего на арахис.",
            "звук, напоминающий тюленя, лающего.", "Доктор Дулиттл!",
        };
...
temple_priest_sound(struct monst *mtmp)
{
    if (mtmp->ispriest && inhistemple(mtmp)
        ...
        static const char *const temple_msg[] = {
            "*кто-то восхваляет %s.", "*кто-то умоляет %s.",
            "#туша животного приносится в жертву.",
            "*резкая мольба о пожертвованиях.",
        };
        const char *msg;
        int hallu = Hallucination ? 1 : 0;
        int trycount = 0,
...
oracle_sound(struct monst *mtmp)
{
    ...
    if (Hallucination || !canseemon(mtmp)) {
        ...
        static const char *const ora_msg[5] = {
            "странный ветер.",     /* Jupiter at Dodona */
            "судорожный бред.", /* Apollo at Delphi */
            "храпящие змеи.",     /* AEsculapius at Epidaurus */
            "кто-то говорит \"Больше нет сурков!\"",
            "громкое ZOT!" /* both rec.humor.oracle */
        };
...
dosounds(void)
{
    ...
    if (svl.level.flags.nfountains && !rn2(400)) {
        static const char *const fountain_msg[4] = {
            "булькающая вода.", "вода, падающая на монеты.",
            "плеск наяды.", "содовый фонтанчик!",
        };
        You_hear1(fountain_msg[rn2(3) + hallu]);
    }
    if (svl.level.flags.nsinks && !rn2(300)) {
        static const char *const sink_msg[3] = {
            "медленное капание.", "булькающий звук.", "посуда моется!",
        };
    ...
    if (svl.level.flags.has_swamp && !rn2(200)) {
        static const char *const swamp_msg[3] = {
            "слышны комары!", "пахнет болотным газом!", /* so it's a smell...*/
            "слышен Дональд Дак!",
        };
        ...
        if (gd_sound())
            ...
            case 1: {
                ...
                if (vault_occupied(u.urooms)
                    ...
                    if (gold_in_vault) {
                        You_hear(!hallu
                                     ? "кто-то считает золотые монеты."
                                     : "квотербек выкрикивает комбинацию.");
                    } else {
                        Soundeffect(se_someone_searching, 30);
                        You_hear("кто-то ищет.");
                    }
                    break;
                }
            }
            ...
            case 0:
                ...
                You_hear("шаги стражника в дозоре.");
                break;
            case 2:
                You_hear("Эбенезер Скрудж!");
                break;
            }
        return;
    }
    ...
    if (svl.level.flags.has_barracks && !rn2(200)) {
        static const char *const barracks_msg[4] = {
            "лезвия точатся.", "громкий храп.", "кости бросаются.",
            "Генерал Макартур!",
        };
        int count = 0;
    ...
    if (svl.level.flags.has_shop && !rn2(200)) {
        ...
        if (tended_shop(sroom)
            ...
            static const char *const shop_msg[3] = {
                "кто-то проклинает воришек.",
                "звон кассового аппарата.", "Нейман и Маркус спорят!",
            };
...
beg(struct monst *mtmp)
{
    ...
    if (!is_silent(mtmp->data) && mtmp->data->msound <= MS_ANIMAL) {
        ...
        if (canspotmon(mtmp))
            pline("%s выглядит изголодавшимся.", Monnam(mtmp));
...
domonnoise(struct monst *mtmp)
{
    ...
    case MS_SELL: /* pitch, pay, total */
        if (!Hallucination || is_silent(ptr) || (mtmp->isshk && !rn2(2))) {
            ...
            Sprintf(verbuf, "15 минут могут сэкономить вам 15 %s.",
    ...
    case MS_VAMPIRE: {
        ...
        if (mtmp->mtame) {
            if (kindred) {
                Sprintf(verbuf, "Доброго %s вам, Хозяин%s",
                        isnight ? "вечера" : "дня",
                        isnight ? "!" : ".  Почему бы нам не отдохнуть?");
                verbl_msg = verbuf;
            } else {
                Sprintf(verbuf, "%s%s",
                        nightchild ? "Дитя ночи, " : "",
                        midnight()
                         ? "Я больше не могу выносить эту жажду!"
                         : isnight
                          ? "Умоляю, помоги мне утолить растущую жажду!"
                          : "Я чувствую себя немного уставшим.");
                verbl_msg = verbuf;
            }
        } else if (mtmp->mpeaceful) {
            if (kindred && isnight) {
                Sprintf(verbuf, "Хорошей охоты %s!",
                        flags.female ? "сестра" : "брат");
                verbl_msg = verbuf;
            } else if (nightchild && isnight) {
                Sprintf(verbuf, "Как приятно слышать вас, дитя ночи!");
                verbl_msg = verbuf;
            } else
                verbl_msg = "Я пью только... зелья.";
        } else {
            static const char *const vampmsg[] = {
                /* These first two (0 and 1) are specially handled below */
                "Я жажду высосать твою %s!",
                "Я приду за %s без сожаления!",
                /* other famous vampire quotes can follow here if desired */
            };
            int vampindex;

            if (kindred) {
                verbl_msg = "Это мои охотничьи угодья"
                            ", где ты смеешь бродить!";
    ...
    case MS_WERE:
        if (flags.moonphase == FULL_MOON && (night() ^ !rn2(13))) {
            pline("%s запрокидывает %s голову и издаёт леденящий кровь %s!",
                  Monnam(mtmp), mhis(mtmp),
                  (ptr == &mons[PM_HUMAN_WERERAT]) ? "визг" : "вой");
            ...
            pline_msg =
                "шепчет невнятно.  Единственное, что вы разбираете: \"луна\".";
        }
        break;
    case MS_BARK:
    ...
    case MS_GROWL:
        ...
        pline_msg = mtmp->mpeaceful ? "оскаливается." : "рычит!";
        break;
    case MS_ROAR:
        Soundeffect((mtmp->mpeaceful ? se_snarl : se_roar), 80);
        pline_msg = mtmp->mpeaceful ? "оскаливается." : "ревёт!";
        break;
    case MS_SQEEK:
    ...
    case MS_BUZZ:
        ...
        pline_msg = mtmp->mpeaceful ? "жужжит." : "злобно жужжит.";
        break;
    case MS_GRUNT:
    ...
    case MS_SHRIEK:
        ...
        break;
    case MS_IMITATE:
        pline_msg = "подражает вам.";
        break;
    case MS_BONES:
        Soundeffect(se_bone_rattle, 60);
        pline("%s шумно гремит.", Monnam(mtmp));
        You("замираете на мгновение.");
        nomul(-2);
        gm.multi_reason = "напуган грохотом";
        gn.nomovemsg = 0;
        break;
    case MS_LAUGH: {
        static const char *const laugh_msg[4] = {
            "хихикает.", "посмеивается.", "хихикает.", "смеётся.",
        };
    ...
    case MS_DJINNI:
        if (mtmp->mtame) {
            verbl_msg = "Извини, у меня закончились желания.";
        } else if (mtmp->mpeaceful) {
            if (ptr == &mons[PM_WATER_DEMON])
                pline_msg = "булькает.";
            else
                verbl_msg = "Я свободен!";
        } else {
            if (ptr != &mons[PM_PRISONER])
                verbl_msg = "Это научит тебя не беспокоить меня!";
            else /* vague because prisoner might already be out of cell */
                verbl_msg = "Выпустите меня отсюда.";
        }
        break;
    case MS_BOAST: /* giants */
        if (!mtmp->mpeaceful) {
            switch (rn2(4)) {
            case 0:
                pline("%s хвастается коллекцией драгоценностей %s.", Monnam(mtmp),
                      mhis(mtmp));
                break;
            case 1:
                pline_msg = "жалуется на диету из баранины.";
                break;
            default:
                pline_msg = "кричит \"Фи Фай Фо Фу!\" и гогочет.";
    ...
    case MS_HUMANOID:
        if (!mtmp->mpeaceful) {
            if (In_endgame(&u.uz) && is_mplayer(ptr))
                ...
                pline_msg = "угрожает вам.";
            break;
        }
        /* Generic peaceful humanoid behavior. */
        if (mtmp->mflee)
            pline_msg = "не хочет иметь с вами ничего общего.";
        else if (mtmp->mhp < mtmp->mhpmax / 4)
            pline_msg = "стонет.";
        else if (mtmp->mconf || mtmp->mstun)
            verbl_msg = !rn2(3) ? "А?" : rn2(2) ? "Что?" : "Э?";
            ...
            if (t)
                t->tseen = 1;
            verbl_msg = "Я в ловушке!";
        } else if (mtmp->mhp < mtmp->mhpmax / 2)
            pline_msg = "просит зелье лечения.";
        else if (mtmp->mtame && !mtmp->isminion
                 && svm.moves > EDOG(mtmp)->hungrytime)
            verbl_msg = "Я голоден.";
        /* Specific monsters' interests */
        else if (is_elf(ptr))
            pline_msg = "проклинает орков.";
        else if (is_dwarf(ptr))
            pline_msg = "говорит о добыче руды.";
        else if (likes_magic(ptr))
            pline_msg = "говорит о магии.";
        else if (ptr->mlet == S_CENTAUR)
            pline_msg = "рассуждает об охоте.";
            ...
            case PM_HOBBIT:
                ...
                pline_msg = (mtmp->mhp < mtmp->mhpmax
                            ...
                            ? "жалуется на неприятные условия подземелья."
                            : "спрашивает вас о Едином Кольце.";
                break;
            case PM_ARCHEOLOGIST:
                pline_msg =
                "описывает недавнюю статью в журнале \"Spelunker Today\".";
                break;
            case PM_TOURIST:
                verbl_msg = "Алоха.";
                break;
            default:
                pline_msg = "рассуждает об исследовании подземелья.";
                break;
            }
        break;
    case MS_SEDUCE: {
        ...
        switch (swval) {
        case 2:
            verbl_msg = "Привет, моряк.";
            break;
        case 1:
            pline_msg = "пристает к вам.";
            break;
        default:
            pline_msg = "заискивает перед вами.";
        }
    } break;
    case MS_ARREST:
        if (mtmp->mpeaceful) {
            SetVoice(mtmp, 0, 80, 0);
            verbalize("Только факты, %s.", flags.female ? "мадам" : "сэр");
        } else {
            static const char *const arrest_msg[3] = {
                "Всё, что вы скажете, может быть использовано против вас.",
                "Вы арестованы!", "Стой именем Закона!",
            };
    ...
    case MS_CUSS:
        if (!mtmp->mpeaceful)
            ...
            verbl_msg = "Ещё не поздно.";
        else
            verbl_msg = "Мы все обречены.";
        break;
    case MS_SPELL:
        /* deliberately vague, since it's not actually casting any spell */
        pline_msg = "бормочет, кажется, заклинание.";
        break;
    case MS_NURSE:
        verbl_msg_mcan = "Я ненавижу эту работу!";
        if (uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
            verbl_msg = "Убери это оружие, прежде чем кого-нибудь ранишь!";
        else if (uarmc || uarm || uarmh || uarms || uarmg || uarmf)
            verbl_msg = Role_if(PM_HEALER)
                            ? "Док, я не могу тебе помочь, пока ты не сотрудничаешь."
                            : "Пожалуйста, разденься, чтобы я мог осмотреть тебя.";
        else if (uarmu)
            verbl_msg = "Снимите рубашку, пожалуйста.";
        else
            verbl_msg = "Расслабься, это совсем не больно.";
        break;
    case MS_GUARD:
        if (money_cnt(gi.invent))
            verbl_msg = "Пожалуйста, брось то золото и иди за мной.";
        else
            verbl_msg = "Пожалуйста, иди за мной.";
        break;
    case MS_SOLDIER: {
        static const char
            *const soldier_foe_msg[3] = {
                "Сопротивление бесполезно!", "Ты — собачье мясо!", "Сдавайся!",
            },
            *const soldier_pax_msg[3] = {
                "Какую паршивую зарплату мы тут получаем!",
                "Эта еда не годится даже для орков!",
                "Мои ноги болят, я на них весь день!",
            };
    ...
    case MS_RIDER: {
        ...
        if (ms_Death && !svc.context.tribute.Deathnotice
            ...
            if ((tribtitle = noveltitle(&book->novelidx)) != 0) {
                Sprintf(verbuf, "А, так у тебя есть экземпляр /%s/.", tribtitle);
                /* no Death featured in these two, so exclude them */
                if (strcmpi(tribtitle, "Snuff")
                    && strcmpi(tribtitle, "The Wee Free Men"))
                    Strcat(verbuf, "  Возможно, меня там неверно процитировали.");
                verbl_msg = verbuf;
            }
        ...
        /* end of tribute addition */
            ...
            pline_msg = "занят чтением экземпляра Sandman №8.";
        } else
            verbl_msg = "Кем ты себя возомнил, Война?";
        break;
    } /* case MS_RIDER */
    } /* switch */
...
dochat(void)
{
    ...
    if (is_silent(gy.youmonst.data)) {
        pline("Как %s, вы не можете говорить.",
              an(pmname(gy.youmonst.data, flags.female ? FEMALE : MALE)));
        return ECMD_OK;
    }
    if (Strangled) {
        You_cant("говорить.  Вы задыхаетесь!");
        return ECMD_OK;
    }
    if (u.uswallow) {
        pline("Там они вас не услышат.");
        return ECMD_OK;
    }
    if (Underwater) {
        Your("речь под водой неразборчива.");
        return ECMD_OK;
    }
    ...
    if (!getdir("С кем говорить? (в каком направлении)")) {
        /* decided not to chat */
        return ECMD_CANCEL;
    }
    ...
    if (u.usteed && u.dz > 0) {
        if (helpless(u.usteed)) {
            pline("%s, кажется, не замечает вас.", Monnam(u.usteed));
            return ECMD_TIME;
        } else
    ...
    if (u.dx == 0 && u.dy == 0) {
        ...
        if (u.umonnum == PM_ETTIN) {
            You("обнаруживаете, что ваша другая голова ведёт скучную беседу.");
            return 1;
        }
         */
        pline("Разговор с самим собой — дурная привычка для искателя подземелий.");
        return ECMD_OK;
    }
    ...
    if (!mtmp || mtmp->mundetected) {
        ...
        if (!Deaf && (IS_WALL(levl[tx][ty].typ)
            ...
            if (Blind && !IS_WALL(svl.lastseentyp[tx][ty])) {
                ...
                pline("Это всё равно что говорить со стеной.");
            } else {
                static const char *const walltalk[] = {
                    "жалуется на свою работу.",
...
tiphat(void)
{
    ...
    You("ненадолго снимаете %s.", helm_simple_name(uarmh));

    if (!u.dx && !u.dy) {
        if (u.usteed && u.dz > 0) {
            if (helpless(u.usteed))
                pline("%s не замечает.", Monnam(u.usteed));
            else
    ...
    if (unseen || (statue && Hallucination)) {
        ...
        if (vismon && humanoid(mtmp->data) && mtmp->mpeaceful && !Conflict) {
            ...
            pline("%s не отвечает.", Monnam(mtmp));
        } else {
 nada:
