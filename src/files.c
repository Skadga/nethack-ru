...
#endif
    if (wizard && ret != 0)
        pline("не удалось переименовать %s в %s.", tempname, fq_bones);
}
...
docompress_file(const char *filename, boolean uncomp)
{
    ...
    if (!uncomp) {
        ...
        if (compressedfile == NULL) {
            if (errno == 0) {
                pline("zlib не смогла выделить память");
            } else {
        ...
        while (1) {
            ...
            if (ferror(uncompressedfile)) {
                pline("Ошибка чтения распакованного файла");
                pline("Не удаётся сжать %s.", filename);
                fclose(uncompressedfile);
            ...
            if (len2 == 0) {
                pline("Ошибка записи сжатого файла");
                pline("Не удаётся сжать %s.", filename);
                fclose(uncompressedfile);
                gzclose(compressedfile);
                (void) unlink(cfn);
        ...
        while (1) {
            ...
            if (len == (unsigned) -1) {
                pline("Ошибка чтения сжатого файла");
                pline("Не удаётся распаковать %s.", filename);
                fclose(uncompressedfile);
                gzclose(compressedfile);
                (void) unlink(filename);
#ifdef SFCTOOL
            ...
            if (ferror(uncompressedfile)) {
                pline("Ошибка записи распакованного файла");
                pline("Не удаётся распаковать %s.", filename);
                fclose(uncompressedfile);
                gzclose(compressedfile);
                (void) unlink(filename);
...
reveal_paths(int code)
{
...
#endif /* ?SYSCF */
    if (fqn && *fqn) {
        raw_print("Ваш файл итогов игры:");
    ...
    if (cstrp && (int) strlen(cstrp) < maxlen)
        Sprintf(buf, " (in %s)", cstrp);
#endif /* PREFIXES_IN_USE */
    raw_printf("Ваш личный файл конфигурации%s:", buf);
             ...
             unsigned oid) /* book identifier */
{
    ...
    if (!tribsection || !tribtitle) {
        if (!nowin_buf)
            pline("Это %s из «%s»!", badtranslation, tribtitle);
        return grasped;
    }
    ...
    if (!fp) {
        ...
        if (!nowin_buf)
            You_feel("слишком подавлены, чтобы продолжать!");
        return grasped;
    }
 ...
 cleanup:
    ...
    if (nowin_buf) {
        ...
        if (!grasped)
            ...
            pline("Похоже, это %s из «%s»!", badtranslation, tribtitle);
    }
    return grasped;
}
...
livelog_add(long ll_type, const char *str)
{
    ...
    if (lock_file(LIVELOGFILE, SCOREPREFIX, 10)) {
        if (!(livelogfile = fopen_datafile(LIVELOGFILE, "a", SCOREPREFIX))) {
            pline("Не удаётся открыть файл журнала!");
