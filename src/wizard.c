...
resurrect(void)
{
    ...
    if (mtmp) {
        ...
        set_malign(mtmp);
        if (!Deaf) {
            pline("Раздаётся голос...");
            SetVoice(mtmp, 0, 80, 0);
            verbalize("Так ты думал, что сможешь %s меня, глупец.", verb);
        }
    }
}
...
static const char *const random_insult[] = {
    "шут",      "негодяй",   "мерзавец",    "болван",
    "проходимец",   "трус",       "кретин",     "паршивец",
    "подлец",    "корм для демонов", "тупица",     "дурак",
    "глупец",       "ворюга",      "идиот",   "плут",
    "проклятый",   "злодей",    "ничтожество",  "малодушный трус",
    "пустоголовый", "отщепенец",    "негодник", "пройдоха",
    "подонок", /* (sic.) */
    "рогоносец",     "червяк",         "несчастный",
};

static const char *const random_malediction[] = {
    "Ад скоро поглотит твои останки,", "Я хихикаю над тобой, жалкий",
    "Готовься умереть,", "Сопротивление бесполезно,",
    "Сдавайся или умри,", "Пощады не будет,",
    "Ты раскаешься в своём коварстве,", "Ты для меня как блоха,",
    "Тебе конец,", "Твоя судьба решена,",
    "Истинно, ты будешь мёртв"
};
...
cuss(struct monst *mtmp)
{
    ...
    if (mtmp->iswiz) {
        if (!rn2(5)) { /* typical bad guy action */
            ...
            SetVoice(mtmp, 0, 80, 0);
            verbalize("Верни амулет, %s!",
                      ROLL_FROM(random_insult));
        } else if (u.uhp < 5 && !rn2(2)) { /* Panic */
            SetVoice(mtmp, 0, 80, 0);
            verbalize(rn2(2) ? "Твоя жизненная сила угасает, %s!"
                             : "Вдохни поглубже, %s, это твой последний!",
                      ROLL_FROM(random_insult));
        } else if (mtmp->mhp < 5 && !rn2(2)) { /* Parthian shot */
            SetVoice(mtmp, 0, 80, 0);
            verbalize(rn2(2) ? "Я ещё вернусь." : "Я вернусь.");
        } else {
            SetVoice(mtmp, 0, 80, 0);
            verbalize("%s %s!",
        ...
        if (!rn2(is_minion(mtmp->data) ? 100 : 5))
            pline("%s поносит ваш род.", Monnam(mtmp));
        else
