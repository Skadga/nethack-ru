...
static const char explodes[] = "выделяет взрывную энергию";
...
cursed_book(struct obj *bp)
{
    ...
    case 3:
        ...
        break;
    case 4:
        pline("Эти руны слишком сложны для понимания.");
        make_confused(HConfusion + rn1(7, 16), FALSE);
        break;
    case 5:
        pline("Книга была покрыта контактным ядом!");
        if (uarmg) {
        ...
        poison_strdmg(Poison_resistance ? rn1(2, 1) : rn1(4, 3),
                      ...
                      "книга заклинаний с контактным ядом", KILLED_BY_AN);
        bp->in_use = was_in_use;
        break;
    case 6:
        if (Antimagic) {
            shieldeff(u.ux, u.uy);
            pline("Книга %s, но вы невредимы!", explodes);
        } else {
...
confused_book(struct obj *spellbook)
{
    ...
    if (!rn2(3) && spellbook->otyp != SPE_BOOK_OF_THE_DEAD) {
        ...
        pline(
         "В замешательстве вы с трудом контролируете свои действия.");
        display_nhwindow(WIN_MESSAGE, FALSE);
        You("случайно разрываете книгу заклинаний на куски.");
        trycall(spellbook);
        useup(spellbook);
        gone = TRUE;
    } else {
        You("обнаруживаете себя читающим %s строку снова и снова.",
            spellbook == svc.context.spbook.book ? "следующую" : "первую");
    }
    return gone;
}
...
deadbook(struct obj *book2)
{
    ...
    You("листаете страницы Книги Мёртвых...");
    ...
    if (invocation_pos(u.ux, u.uy) && !On_stairs(u.ux, u.uy)) {
        ...
        if (book2->cursed) {
            pline("%s!",
                      Blind ? "Книга, кажется, игнорирует вас"
                            : "Руны выглядят перепутанными. Вы не можете их прочитать");
            return;
        }

        if (!u.uhave.bell || !u.uhave.menorah) {
            pline("Холод пробегает по вашей %s.", body_part(SPINE));
            if (!u.uhave.bell) {
                Soundeffect(se_faint_chime, 30);
                You_hear("слабый звон...");
            }
            if (!u.uhave.menorah)
                pline("Двойник Влада доволен.");
            return;
        }
        ...
        if (arti_cursed) {
            pline("Инвокация не удалась!");
            /* this used to say "your artifacts" but the invocation tools
               are not artifacts */
            pline("По крайней мере одна из ваших реликвий проклята...");
            ...
            You("чувствуете, что %s не так...", something);
            goto raise_dead;
        }
        return;
    }
 ...
 raise_dead:
        ...
        switch (rn2(3)) {
        case 0:
            Your("предки недовольны вами!");
            break;
        case 1:
            pline("Надгробия на кладбище начинают двигаться!");
            break;
        default:
            pline("О боже! Ваше имя появляется в книге!");
        }
    }
    return;
}
...
book_cursed(struct obj *book)
{
    if (book->cursed && gm.multi >= 0
        ...
        pline("%s захлопывается!", Tobjnam(book, "slam"));
        set_bknown(book, 1);
        stop_occupation();
    }
}
...
learn(void)
{
    ...
    if (i == MAXSPELL) {
        ...
        if (book->spestudied > MAX_SPELL_STUDY) {
            pline("Эта книга заклинаний слишком выцвела, чтобы её можно было прочитать.");
            book->otyp = booktype = SPE_BLANK_PAPER;
            faded_to_blank = TRUE;
            /* reset spestudied as if polymorph had taken place */
            book->spestudied = rn2(book->spestudied);
        } else {
            Your("ваши знания о %s %s.", splname,
                 spellknow(i) ? "углубились" : "восстановлены");
            incrnknow(i, 1);
            book->spestudied++;
        ...
        if (book->spestudied >= MAX_SPELL_STUDY) {
            ...
            pline("Эта книга заклинаний слишком выцвела, чтобы прочитать её хотя бы раз.");
            book->otyp = booktype = SPE_BLANK_PAPER;
            faded_to_blank = TRUE;
            /* reset spestudied as if polymorph had taken place */
            book->spestudied = rn2(book->spestudied);
            ...
            if (!i)
                ...
                You("выучиваете %s.", splname);
            else
                You("добавляете %s в свой репертуар под буквой '%c'.",
                    splname, spellet(i));
        }
    }
    if (i < MAXSPELL) {
...
study_book(struct obj *spellbook)
{
    ...
    if (!confused && !Sleep_resistance
        ...
        if (dullbook > 0) {
            ...
            pline("Эта книга настолько скучна, что вы не можете держать свои %s открытыми.",
                  eyes);
    ...
    if (svc.context.spbook.delay && !confused
        ...
        You("продолжаете свои усилия, чтобы %s.",
            (booktype == SPE_NOVEL) ? "прочитать роман"
                                    : "запомнить заклинание");
    } else {
        /* KMH -- Simplified this code */
        if (booktype == SPE_BLANK_PAPER) {
            pline("Эта книга заклинаний совершенно пуста.");
            makeknown(booktype);
            return 1;
        }
        ...
        if (!spellbook->blessed && spellbook->otyp != SPE_BOOK_OF_THE_DEAD) {
            if (spellbook->cursed) {
                ...
                if (Role_if(PM_WIZARD) && read_ability < 20 && !confused) {
                    ...
                    Sprintf(qbuf,
                    "Эта книга заклинаний %sсложна для понимания. Продолжить?",
                            (read_ability < 12 ? "очень " : ""));
                    if (y_n(qbuf) != 'y') {
        ...
        if (too_hard) {
            ...
            if (gone || !rn2(3)) {
                if (!gone)
                    pline("Книга заклинаний рассыпается в пыль!");
                trycall(spellbook);
                useup(spellbook);
            } else
                spellbook->in_use = FALSE;
...
rejectcasting(void)
{
    ...
    if (Stunned) {
        You("слишком ослаблены, чтобы произносить заклинания.");
        return TRUE;
    } else if (!can_chant(&gy.youmonst)) {
        You("не можете произнести заклинание.");
        return TRUE;
         ...
         */
        Your("руки не свободны для заклинания!");
        return TRUE;
    }
    return FALSE;
}
...
getspell(int *spell_no)
{
    ...
    nspells = num_spells();
    if (!nspells) {
        You("сейчас не знаете ни одного заклинания.");
        return FALSE;
    }
    if (rejectcasting())
    ...
    if (flags.menu_style == MENU_TRADITIONAL) {
        ...
        Snprintf(qbuf, sizeof qbuf, "Какое заклинание произнести? [%s *?]", lets);
        for (retry_limit = 0; ; ++retry_limit) {
            ...
            if (idx < 0 || idx >= nspells) {
                You("не знаете такого заклинания.");
                continue; /* ask again */
            }
            *spell_no = idx;
            return TRUE;
    ...
    return dospellmenu("Выберите, какое заклинание произнести", SPELLMENU_CAST,
                       spell_no);
}
...
dowizcast(void)
{
    ...
    end_menu(win, "Какое заклинание произнести?");
            ...
            if (mon) {
                ...
                if (dmg) {
                    ...
                    if (DEADMONSTER(mon)) {
                        ...
                        pline("Вы поражаете молнией %s%s", mon_nam(mon), exclam(dmg));
                    ...
                    pline("%s сопротивляется.", Monnam(mon));
                }
                if (!DEADMONSTER(mon)) {
...
cast_protection(void)
{
    ...
    if (gain > 0) {
        if (!Blind) {
            ...
            if (u.uspellprot) {
                pline("%s Дымка вокруг вас становится плотнее.", hgolden);
            } else {
                struct permonst *pm = u.ustuck ? u.ustuck->data : 0;

                rmtyp = levl[u.ux][u.uy].typ;
                atmosphere = (pm && u.uswallow)
                                ? ((pm == &mons[PM_FOG_CLOUD]) ? "туманом"
                                   : is_whirly(pm) ? "водоворотом"
                                     : enfolds(pm) ? "складками"
                                       : is_animal(pm) ? "пастью"
                                         : "слизью")
                                : (u.uinwater ? hliquid("water")
                                   : (rmtyp == CLOUD) ? "облаком"
                                     : IS_TREE(rmtyp) ? "растительностью"
                                       : IS_STWALL(rmtyp) ? "камнем"
                                         : "воздухом");
                pline("%s Вокруг вас начинает мерцать %s дымкой.",
        ...
        Your("кожа на мгновение становится тёплой.");
    }
}
...
spelleffects_check(int spell, int *res, int *energy)
{
    ...
    *energy = SPELL_LEV_PW(spellev(spell)); /* 5 <= energy <= 35 */
    ...
    if (spellknow(spell) <= 0) {
        Your("знание этого заклинания искажено.");
        pline("Оно вызывает кошмарные образы в вашем сознании...");
        ...
        *res = ECMD_TIME;
        ...
        You("напрягаетесь, чтобы вспомнить заклинание.");
    } else if (spellknow(spell) <= KEEN / 40) { /* 500 turns left */
        You("с трудом вспоминаете заклинание.");
    } else if (spellknow(spell) <= KEEN / 20) { /* 1000 turns left */
        Your("знание этого заклинания тускнеет.");
    } else if (spellknow(spell) <= KEEN / 10) { /* 2000 turns left */
        Your("память об этом заклинании постепенно угасает.");
    }

    if (u.uhunger <= 10 && spellid(spell) != SPE_DETECT_FOOD) {
        You("слишком голодны, чтобы произнести это заклинание.");
        *res = ECMD_OK;
        return TRUE;
    } else if (ACURR(A_STR) < 4 && spellid(spell) != SPE_RESTORE_ABILITY) {
        You("недостаточно сил, чтобы произносить заклинания.");
        *res = ECMD_OK;
        return TRUE;
    } else if (check_capacity(
                "Ваша концентрация ослабевает, пока вы несёте так много вещей.")) {
        *res = ECMD_TIME;
        return TRUE;
    }
    ...
    if (u.uhave.amulet && u.uen >= *energy) {
        You_feel("амулет вытягивает вашу энергию.");
    ...
    if (*energy > u.uen) {
         ...
         */
        You("недостаточно энергии, чтобы произнести это заклинание%s.",
            (u.uen < u.uenmax) ? "" /* not at full energy => normal message */
            : (*energy > u.uenpeak) ? " ещё" /* haven't ever had enough */
              : " больше"); /* once had enough but have lost some since */
        return TRUE;
    } else {
    ...
    if (confused || (rnd(100) > chance)) {
        You("неверно произносите заклинание.");
        u.uen -= *energy / 2;
        disp.botl = TRUE;
        *res = ECMD_TIME;
        return TRUE;
...
spelleffects(int spell_otyp, boolean atme, boolean force)
{
                ...
                while (n--) {
                    if (!u.dx && !u.dy && !u.dz) {
                        if ((damage = zapyourself(pseudo, TRUE)) != 0) {
                            char buf[BUFSZ];
                            Sprintf(buf, "%s поразили себя заклинанием",
                                    uhim());
    ...
    case SPE_STONE_TO_FLESH:
        if (objects[otyp].oc_dir != NODIR) {
            ...
            if (atme) {
                 ...
                 */
                pline("Магическая энергия высвобождается!");
            }
            if (!u.dx && !u.dy && !u.dz) {
                if ((damage = zapyourself(pseudo, TRUE)) != 0) {
                    char buf[BUFSZ];

                    Sprintf(buf, "%s поразили себя заклинанием", uhim());
                    if (physical_damage)
    ...
    case SPE_CURE_SICKNESS: {
        ...
        if (was_sick || !was_slimed)
            You("%s больны.", was_sick ? "больше не" : "не");
        if (was_slimed)
    ...
    case SPE_CLAIRVOYANCE:
        ...
        /* at present, only one thing blocks clairvoyance */
            ...
            You("чувствуете остроконечную шляпу на своей %s.", body_part(HEAD));
        break;
    case SPE_PROTECTION:
        cast_protection();
        break;
...
throwspell(void)
{
    ...
    if (u.uinwater) {
        pline("Вы шутите! В такую погоду?");
        return 0;
    } else if (Is_waterlevel(&u.uz)) {
        You("лучше подождать, пока выглянет солнце.");
        return 0;
    }

    pline("Куда вы хотите произнести заклинание?");
    cc.x = u.ux;
    cc.y = u.uy;
    getpos_sethilite(display_spell_target_positions,
                     can_center_spell_location);
    if (getpos(&cc, TRUE, "желаемая позиция") < 0)
        return 0; /* user pressed ESC */
    clear_nhwindow(WIN_MESSAGE); /* discard any autodescribe feedback */

    /* The number of moves from hero to where the spell drops.*/
    if (distmin(u.ux, u.uy, cc.x, cc.y) > 10) {
        pline("Заклинание рассеивается на расстоянии!");
        return 0;
    } else if (u.uswallow) {
        pline("Заклинание прерывается!");
        ...
        Your("разум не может сосредоточиться на этом месте!");
        return 0;
    }
...
static const char *const spl_sortchoices[NUM_SPELL_SORTBY] = {
    "по букве произнесения",
    "в алфавитном порядке",
    "по уровню, от низшего к высшему",
    "по уровню, от высшего к низшему",
    "по группе навыка, в алфавитном порядке внутри группы",
    "по группе навыка, от низшего уровня к высшему в группе",
    "по группе навыка, от высшего уровня к низшему в группе",
    "сохранить текущий порядок",
    /* a menu choice rather than a sort choice */
    "переназначить буквы произнесения, сохранив текущий порядок",
};
...
spellsortmenu(void)
{
    ...
    end_menu(tmpwin, "Просмотр списка известных заклинаний отсортирован");
...
dovspell(void)
{
    ...
    if (spellid(0) == NO_SPELL) {
        ...
        while (dospellmenu("Текущие известные заклинания",
                           SPELLMENU_VIEW, &splnum)) {
            if (splnum == SPELLMENU_SORT) {
                if (spellsortmenu())
                    sortspells();
            } else {
                Sprintf(qbuf, "Перестановка заклинаний; обменять '%c' с",
                        spellet(splnum));
...
show_spells(void)
{
    ...
    if (spellid(0) == NO_SPELL) {
        pline("Вы не знали ни одного заклинания.");
        pline("%s", "");
    } else {
        pline("Заклинания:");
    ...
    int *spell_no)
{
    ...
    if (!iflags.menu_tab_sep) {
        Sprintf(buf, "%s%-20s Уровень %-12s Провал Удержание",
        ...
        Sprintf(buf, "Название\tУровень\tКатегория\tПровал\tУдержание");
        fmt = "%s\t%-d\t%s\t%-d%%\t%s";
        sep = '\t';
    }
    if (wizard)
        Sprintf(eos(buf), "%c%6s", sep, "ходов");
    ...
    if (splaction == SPELLMENU_VIEW) {
        if (spellid(1) == NO_SPELL) {
            ...
            add_menu(tmpwin, &nul_glyphinfo, &any, '+', 0,
                     ATR_NONE, clr, "[сортировать заклинания]", MENU_ITEMFLAGS_NONE);
        }
    }
