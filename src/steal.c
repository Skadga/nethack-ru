...
stealgold(struct monst *mtmp)
{
    ...
    if (fgold && (!ygold || fgold->quan > ygold->quan || !rn2(5))) {
        ...
        /* [reduce "between your rear regions"] */
        if (slithy(who->data))
            what = "колец";
        pline("%s стремительно выхватывает золото у вас %s%s!",
              Monnam(mtmp),
              (Levitation || Flying) ? "из-под " : "между ", what);
        if (!ygold || !rn2(5)) {
...
unresponsive(void)
{
    ...
    return (unconscious() || is_fainted()
            || (gm.multi_reason
                && (!strncmp(gm.multi_reason, "заморожены", 10)
                    || !strncmp(gm.multi_reason, "парализованы", 12))));
}
...
unstolenarm(void)
{
    ...
    gs.stealoid = 0;
    if (obj) {
        You("заканчиваете снимать %s.", armor_simple_name(obj));
    }
    return 0;
}
        ...
        if (otmp->o_id == gs.stealoid) {
            for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
                if (mtmp->m_id == gs.stealmid) {
                    ...
                    freeinv(otmp);
                    pline("%s крадЄт %s!", Monnam(mtmp), doname(otmp));
    ...
    struct obj *obj)
{
    ...
    verb = ((obj->owornmask & W_WEAPONS) != 0L) ? "отбирает"
           : "снимает";
...
steal(struct monst *mtmp, char *objnambuf)
{
 ...
 nothing_to_steal:
        ...
        if (Punished && !monkey_business && rn2(4)) {
            ...
            pline("%s снимает вашу невидимую цепь.", Monnambuf);
            (void) openholdingtrap(&gy.youmonst, &dummy);
        } else if (Blind) {
            pline(" то-то пытаетс€ вас ограбить, но не находит, что украсть.");
        } else if (inv_cnt(TRUE) > inv_cnt(FALSE)) {
            pline("%s пытаетс€ ограбить вас, но золото его не интересует.",
                  Monnambuf);
        } else {
            pline("%s пытаетс€ ограбить вас, но красть нечего!",
                  Monnambuf);
        }
    ...
    if (monkey_business) {
        ...
        if (ostuck || can_carry(mtmp, otmp) == 0) {
            static const char *const how[] = {
                "украсть", "выхватить", "схватить", "вз€ть"
            };
 cant_take:
            pline("%s пытаетс€ %s %s, но сдаЄтс€.", Monnambuf, ROLL_FROM(how),
    ...
    if (otmp->owornmask & (W_ARMOR | W_ACCESSORY)) {
        ...
        case ARMOR_CLASS:
            ...
            if (monkey_business || unresponsive()) {
                ...
                if (flags.female)
                    urgent_pline("%s очаровывает вас.  ¬ы с радостью %s свои %s.",
                                 !seen ? "ќна" : Monnambuf,
                                 curssv ? "позвол€ете ей вз€ть"
                                 : !slowly ? "отдаЄте"
                                   : was_doffing ? "продолжаете снимать"
                                     : "начинаете снимать",
                                 armor_simple_name(otmp));
                else
                    urgent_pline("%s соблазн€ет вас и %s свои %s.",
                                 !seen ? "ќна" : Monnam(mtmp),
                                 curssv ? "помогает вам сн€ть"
                                 : !slowly ? "вы снимаете"
                                   : was_doffing ? "вы продолжаете снимать"
                                     : "вы начинаете снимать",
                                 armor_simple_name(otmp));
                named++;
                /* the following is to set multi for later on */
                nomul(-armordelay);
                gm.multi_reason = "снимаете одежду";
                gn.nomovemsg = 0;
    ...
    if (iflags.last_msg == PLNMSG_MON_TAKES_OFF_ITEM
        ...
        ++named;
    urgent_pline("%s похищает %s.", named ? "ќна" : Monnambuf, doname(otmp));
    encumber_msg();
...
stealamulet(struct monst *mtmp)
{
    ...
    if (otmp) { /* we have something to snatch */
        ...
        pline("%s крадЄт %s!", Some_Monnam(mtmp), buf);
    ...
    int ochance, int achance) /* percent chance for ordinary item, artifact */
{
    ...
    if (carried(obj)) {
        ...
        if (cansee(mon->mx, mon->my)) {
            ...
            pline("%s выхватывает %s у вас и поглощает %s!",
                  Some_Monnam(mon), /* Monnam() or "Something" */
                  yname(obj), (obj->quan > 1L) ? "их" : "его");
        } else {
            pline("%s вырван%s из ваших рук!", upstart(yname(obj)),
                  (obj->quan > 1L) ? "ы" : "");
        }
        freeinv(obj);
        encumber_msg();
        ...
        if (canspotmon(mon))
            pline("%s поглощает %s!", Monnam(mon), yname(obj));
    }
    ...
    boolean verbosely)
{
    ...
    if (verbosely && cansee(omx, omy))
        pline_mon(mon, "%s рон€ет %s.", Monnam(mon), obj_name);
    ...
    boolean is_pet) /* If true, pet should keep wielded/worn items */
{
    ...
    if (mtmp->isgd && (otmp = findgold(mtmp->minvent)) != 0) {
        if (canspotmon(mtmp))
            pline("«олото %s %s.", Monnam(mtmp),
                  canseemon(mtmp) ? "исчезает" : ", кажетс€, исчезает");
