...
precheck(struct monst *mon, struct obj *obj)
{
    ...
    if (obj->oclass == POTION_CLASS) {
        coord cc;
        static const char *const empty = "Зелье оказывается пустым.";
        struct monst *mtmp;

        if (objdescr_is(obj, "молочный")) {
            if (!(svm.mvitals[PM_GHOST].mvflags & G_GONE)
                ...
                if (!mtmp) {
                    ...
                    if (vis) {
                        pline(
                            "Когда %s открывает бутылку, появляется огромный %s!",
                              mon_nam(mon),
                              Hallucination ? rndmonnam(NULL)
                                            : (const char *) "призрак");
                        pline("%s до смерти напуган,"
        ...
        if (objdescr_is(obj, "дымчатый")
            ...
            if (!mtmp) {
                ...
                if (vis)
                    pline_mon(mtmp, "В облаке дыма появляется %s!", a_monnam(mtmp));
                pline("%s говорит.", vis ? Monnam(mtmp) : Something);
    ...
    if (obj->oclass == WAND_CLASS && obj->cursed
        ...
        if (vis) {
            pline_mon(mon, "%s ударяет %s, и та внезапно взрывается!", Monnam(mon),
                  an(xname(obj)));
        } else {
    ...
    boolean self)
{
    ...
    if (!canseemon(mtmp)) {
        ...
        You_hear("щелчок %s.", (mdistu(mtmp) <= range * range)
                                 ? "рядом" : "вдалеке");
        unknow_object(otmp); /* hero loses info when unseen obj is used */
    } else if (self) {
        pline("%s с помощью %s!",
              monverbself(mtmp, Monnam(mtmp), "бьёт", (char *) 0),
              doname(otmp));
    } else {
        pline_mon(mtmp, "%s ударяет %s!", Monnam(mtmp), an(xname(otmp)));
        stop_occupation();
    }
    otmp->spe -= 1;
}
    ...
    boolean self)
{
    ...
    if (!canseemon(mtmp)) {
        ...
        You_hear("звук рога, играющего %s.",
        ...
        Sprintf(objbuf, "%s, направленный на", objnamp);
        /* "<mon> plays a <horn> directed at himself!" */
        pline("%s!", monverbself(mtmp, Monnam(mtmp), "играет", objbuf));
...
mreadmsg(struct monst *mtmp, struct obj *otmp)
{
    ...
    if (vismon) {
        ...
        pline_mon(mtmp, "%s читает %s!", Monnam(mtmp), onambuf);
        ...
        Snprintf(blindbuf, sizeof blindbuf, "читает %s", onambuf);
        strsubst(blindbuf, "читает",
                 mtmp->mconf ? "пытается заклинать" : "читает");
        You_hear("%s %s.",
    ...
    if (mtmp->mconf) /* (note: won't get if not seen and hero can't hear) */
        pline("Находясь в замешательстве, %s неверно произносит магические слова...",
...
mquaffmsg(struct monst *mtmp, struct obj *otmp)
{
    if (canseemon(mtmp)) {
        observe_object(otmp);
        pline_mon(mtmp, "%s пьёт %s!", Monnam(mtmp), singular(otmp, doname));
    } else if (!Deaf) {
        Soundeffect(se_mon_chugging_potion, 25);
        You_hear("звук бульканья.");
    }
}
    ...
    int how)            /* type of that object */
{
    if (tele_restrict(mtmp)) { /* mysterious force... */
        ...
        if (vismon)
            pline_mon(mtmp, "%s на мгновение выглядит дезориентированным.", Monnam(mtmp));
    } else {
...
mon_escape(struct monst *mtmp, boolean vismon)
{
    if (mon_has_special(mtmp)
        ...
        return 0;
    if (vismon)
        pline_mon(mtmp, "%s сбегает из подземелья!", Monnam(mtmp));
    mongone(mtmp);
    return 2;
}
...
#define m_flee(m)                          \
    ...
    case MUSE_UNICORN_HORN:
        ...
        if (vismon) {
            if (otmp)
                pline_mon(mtmp, "%s использует единорогов рог!", Monnam(mtmp));
            else
                pline("Конец рога %s светится!", mon_nam(mtmp));
        }
        if (!mtmp->mcansee) {
            ...
            if (vismon)
                pline_mon(mtmp, "%s выглядит устойчивее.", Monnam(mtmp));
        } else {
    ...
    case MUSE_BUGLE:
        ...
        if (vismon) {
            pline_mon(mtmp, "%s играет на %s!", Monnam(mtmp), doname(otmp));
        } else if (!Deaf) {
            Soundeffect(se_bugle_playing_reveille, 100);
            You_hear("горн, играющий побудку!");
        }
    ...
    case MUSE_SCR_TELEPORTATION: {
        ...
        if (obj_is_cursed || mtmp->mconf) {
            ...
            if (mon_has_amulet(mtmp) || In_endgame(&u.uz)) {
                if (vismon)
                    pline_mon(mtmp, "%s на мгновение выглядит очень дезориентированным.",
                          Monnam(mtmp));
            } else if (nlev == depth(&u.uz)) {
                if (vismon)
                    pline_mon(mtmp, "%s на мгновение содрогается.",
                              Monnam(mtmp));
            } else {
                get_level(&flev, nlev);
    ...
    case MUSE_WAN_DIGGING:
        ...
        if (IS_FURNITURE(levl[mtmp->mx][mtmp->my].typ)
            ...
            pline("Луч раскопок неэффективен.");
            return 2;
        }
        if (!Can_dig_down(&u.uz) && !levl[mtmp->mx][mtmp->my].candig) {
            ...
            if (t_at(mtmp->mx, mtmp->my)
                ...
                if (vismon) {
                    pline("%s Здесь слишком твёрдый, чтобы копать.",
            ...
            if (vis) {
                seetrap(t);
                pline_mon(mtmp, "%s выкопал яму в %s.", Monnam(mtmp),
                      surface(mtmp->mx, mtmp->my));
            }
        ...
        seetrap(t);
        if (vis) {
            pline_mon(mtmp, "%s проделал дыру в %s.", Monnam(mtmp),
                  surface(mtmp->mx, mtmp->my));
            pline_mon(mtmp, "%s %s вниз...", Monnam(mtmp),
                  is_flyer(mtmp->data) ? "ныряет" : "падает");
        } else if (!Deaf) {
            Soundeffect(se_crash_through_floor, 100);
            You_hear("%s проламывается сквозь %s.", something,
                     surface(mtmp->mx, mtmp->my));
        }
        fill_pit(mtmp->mx, mtmp->my);
    ...
    case MUSE_TRAPDOOR:
        ...
        if (vis) {
            pline_mon(mtmp, "%s %s в %s!", Monnam(mtmp),
    ...
    case MUSE_UPSTAIRS:
        ...
        if (Inhell && mon_has_amulet(mtmp) && !rn2(4)
            ...
            if (vismon)
                pline("Когда %s поднимается по лестнице, таинственная сила"
                      " на мгновение окружает %s...",
            ...
            if (vismon)
                pline_mon(mtmp, "%s сбегает наверх!", Monnam(mtmp));
    ...
    case MUSE_DOWNSTAIRS:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s сбегает вниз!", Monnam(mtmp));
    ...
    case MUSE_UP_LADDER:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s сбегает вверх по лестнице!", Monnam(mtmp));
    ...
    case MUSE_DN_LADDER:
        ...
        if (!stway)
            return 0;
        if (vismon)
            pline_mon(mtmp, "%s сбегает вниз по лестнице!", Monnam(mtmp));
    ...
    case MUSE_SSTAIRS:
        ...
        if (vismon)
            pline_mon(mtmp, "%s сбегает %sлестницы!", Monnam(mtmp),
                  stway->up ? "вверх по" : "вниз по");
    ...
    case MUSE_TELEPORT_TRAP:
        ...
        t = t_at(gt.trapx, gt.trapy);
        if (vis) {
            pline_mon(mtmp, "%s %s на %s!", Monnam(mtmp),
                  vtense(fakename[0], locomotion(mtmp->data, "jump")),
                  trapname(t->ttyp, FALSE));
        }
        /* if trap was in a concealed niche, it's no longer concealed */
    ...
    case MUSE_POT_HEALING:
        ...
        if (!otmp->cursed && !mtmp->mcansee)
            mcureblindness(mtmp, vismon);
        if (vismon)
            pline_mon(mtmp, "%s выглядит лучше.", Monnam(mtmp));
        if (oseen)
    ...
    case MUSE_POT_EXTRA_HEALING:
        ...
        if (!mtmp->mcansee)
            mcureblindness(mtmp, vismon);
        if (vismon)
            pline_mon(mtmp, "%s выглядит гораздо лучше.", Monnam(mtmp));
        if (oseen)
    ...
    case MUSE_POT_FULL_HEALING:
        ...
        if (!mtmp->mcansee && otmp->otyp != POT_SICKNESS)
            mcureblindness(mtmp, vismon);
        if (vismon)
            pline_mon(mtmp, "%s выглядит полностью исцелённым.", Monnam(mtmp));
        if (oseen)
            makeknown(otmp->otyp);
        m_useup(mtmp, otmp);
        return 2;
        ...
        if (hits_you) {
            ...
            if (mtmp->ispriest && *in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
                if (cansee(mtmp->mx, mtmp->my))
                    pline_mon(mtmp, "%s сопротивляется магии!", Monnam(mtmp));
...
use_offensive(struct monst *mtmp)
{
    ...
    case MUSE_SCR_EARTH: {
        ...
        /* Identify the scroll */
        if (canspotmon(mtmp)) {
            pline_The("%s грохочет %s %s!", ceiling(mtmp->mx, mtmp->my),
                      otmp->blessed ? "вокруг" : "над", mon_nam(mtmp));
            if (oseen)
                makeknown(otmp->otyp);
        } else if (cansee(mtmp->mx, mtmp->my)) {
            pline_The("%s грохочет посреди пустоты!",
    ...
    case MUSE_CAMERA: {
        if (Hallucination) {
            SetVoice(mtmp, 0, 80, 0);
            verbalize("Скажите сыр!");
        } else if (!Blind) {
            pline("%s фотографирует вас с помощью %s!",
    ...
    case MUSE_SCR_FIRE: {
        ...
        if (mtmp->mconf) {
            if (vis)
                pline("О, какой красивый огонь!");
        } else {
            struct monst *mtmp2;
            int num;

            if (vis)
                pline("Свиток взрывается столбом пламени!");
            shieldeff(mtmp->mx, mtmp->my);
            pline_mon(mtmp, "%s невредим.", Monnam(mtmp));
        ...
        if (can_carry(mon, xobj)) {
            if (vismon) {
                if (howfar > 2) /* not adjacent */
                    Norep("%s роется в %s.", Monnam(mon), contnr_nam);
                else if (takeout_indx == 0) /* adjacent, first item */
                    pline_mon(mon, "%s убирает %s из %s.", Monnam(mon),
                          doname(xobj), contnr_nam);
                else /* adjacent, additional items */
                    pline("%s убирает %s.", upstart(mpronounbuf),
                          doname(xobj));
            }
...
use_misc(struct monst *mtmp)
{
        ...
        if (otmp->cursed) {
            if (Can_rise_up(mtmp->mx, mtmp->my, &u.uz)) {
                ...
                if (on_level(&tolevel, &u.uz))
                    goto skipmsg;
                if (vismon) {
                    pline_mon(mtmp, "%s поднимается вверх сквозь %s!",
                              Monnam(mtmp),
                              ceiling(mtmp->mx, mtmp->my));
                    trycall(otmp);
                }
 ...
 skipmsg:
                if (vismon) {
                    pline_mon(mtmp, "%s выглядит неуютно.", Monnam(mtmp));
                    trycall(otmp);
                }
                m_useup(mtmp, otmp);
                return 2;
        ...
        if (vismon)
            pline_mon(mtmp, "%s выглядит опытнее.", Monnam(mtmp));
        if (oseen)
    ...
    case MUSE_POT_INVISIBILITY:
        ...
        if (vismon && mtmp->minvis) { /* was seen, now invisible */
            if (canspotmon(mtmp)) {
                pline("%s тело обретает %s прозрачность.",
                      upstart(s_suffix(nambuf)),
                      Hallucination ? "обычную" : "странную");
            } else {
                pline("Внезапно вы не можете видеть %s.", nambuf);
                if (vis)
                    map_invisible(mtmp->mx, mtmp->my);
            }
            if (oseen)
            ...
            pline("%s ненадолго кажется прозрачным.", Monnam(mtmp));
            ...
            pline("%s внезапно появляется!", Monnam(mtmp));
        }
    ...
    case MUSE_POT_POLYMORPH:
        ...
        m_useup(mtmp, otmp);
        if (vismon)
            pline_mon(mtmp, "%s внезапно мутирует.", Monnam(mtmp));
    ...
    case MUSE_POLY_TRAP:
        ...
        if (vismon || vistrapspot) {
            pline_mon(mtmp, "%s намеренно %s на %s!",
                  Some_Monnam(mtmp),
                  vtense(fakename[0], locomotion(mtmp->data, "jump")),
                  t->tseen ? trapname(t->ttyp, FALSE) : "скрытую ловушку");
    ...
    case MUSE_BULLWHIP:
        ...
        {
            const char *The_whip = vismon ? "Бычий кнут" : "Кнут";
            int where_to = rn2(4);
            struct obj *obj = uwep;
            const char *hand;
            ...
            if (vismon)
                pline_mon(mtmp, "%s щёлкает плетью в сторону вашего %s!",
                          Monnam(mtmp), hand_buf);
            if (obj->otyp == HEAVY_IRON_BALL) {
                pline("%s не удаётся обвиться вокруг %s.", The_whip, the_weapon);
                return 1;
            }
            urgent_pline("%s обвивается вокруг %s, который вы держите!", The_whip,
                         the_weapon);
            if (welded(obj)) {
                pline("%s приварен к вашему %s%c",
                      !is_plural(obj) ? "Он" : "Они", hand_buf,
                      !obj->bknown ? '!' : '.');
                /* obj->bknown = 1; */ /* welded() takes care of this */
                where_to = 0;
            }
            if (!where_to) {
                pline("Плеть соскальзывает."); /* not `The_whip' */
                return 1;
            ...
            case 1: /* onto floor beneath mon */
                pline_mon(mtmp, "%s вырывает %s из вашего %s!", Monnam(mtmp),
                          the_weapon, hand_buf);
                place_object(obj, mtmp->mx, mtmp->my);
                break;
            case 2: /* onto floor beneath you */
                pline_mon(mtmp, "%s дёргает %s к %s!", Monnam(mtmp),
                          the_weapon, surface(u.ux, u.uy));
                dropy(obj);
                break;
            case 3: /* into mon's inventory */
                pline_mon(mtmp, "%s выхватывает %s!", Monnam(mtmp), the_weapon);
...
{
    pline("По какой-то причине вы знаете о присутствии %s.",
    ...
    display_self();
    You_feel("раздражены из-за %s.", noit_mon_nam(mtmp));
    display_nhwindow(WIN_MAP, TRUE);
    docrt();
    if (unconscious()) {
        gm.multi = -1;
        gn.nomovemsg = "Раздражённые, вы приходите в полное сознание.";
    }
    newsym(mtmp->mx, mtmp->my);
    if (!canspotmon(mtmp))
        map_invisible(mtmp->mx, mtmp->my);
...
mon_reflects(struct monst *mon, const char *str)
{
    ...
    if (orefl && orefl->otyp == SHIELD_OF_REFLECTION) {
        if (str) {
            pline(str, s_suffix(mon_nam(mon)), "щитом");
        ...
        if (str)
            pline(str, s_suffix(mon_nam(mon)), "оружием");
        return TRUE;
    } else if ((orefl = which_armor(mon, W_AMUL))
               && orefl->otyp == AMULET_OF_REFLECTION) {
        if (str) {
            pline(str, s_suffix(mon_nam(mon)), "амулетом");
...
ureflects(const char *fmt, const char *str)
{
    ...
    if (EReflecting & W_ARMS) {
        ...
        /* Due to wielded artifact weapon */
        if (fmt && str)
            pline(fmt, str, "оружием");
        return TRUE;
    } else if (EReflecting & W_AMUL) {
        if (fmt && str) {
            pline(fmt, str, "медальоном");
            makeknown(AMULET_OF_REFLECTION);
        }
        return TRUE;
    } else if (EReflecting & W_ARM) {
        if (fmt && str)
            pline(fmt, str, uskin ? "блеском" : "бронёй");
        return TRUE;
    } else if (gy.youmonst.data == &mons[PM_SILVER_DRAGON]) {
        if (fmt && str)
            pline(fmt, str, "чешуёй");
        return TRUE;
    }
    return FALSE;
}
...
mcureblindness(struct monst *mon, boolean verbos)
{
    if (!mon->mcansee) {
        ...
        if (verbos && haseyes(mon->data))
            pline_mon(mon, "%s снова может видеть.", Monnam(mon));
    }
}
    ...
    boolean stoning) /* T: stop petrification, F: cure stun && confusion */
{
    ...
    if (vis) {
        ...
        pline_mon(mon, "%s %s %s.", Monnam(mon),
              ((obj->oclass == POTION_CLASS) ? "выпивает"
               : (obj->otyp == TIN)
                 ? "вскрывает и поедает содержимое"
                 : "поедает"),
              distant_name(obj, doname));
        obj->quan = save_quan;
    } else if (!Deaf)
        You_hear("%s.",
                 (obj->oclass == POTION_CLASS) ? "попивание." : "чавканье.");
    ...
    if (acid && !tinned && !resists_acid(mon)) {
        mon->mhp -= rnd(15);
        if (vis)
            pline_mon(mon, "%s мучается сильной изжогой.", Monnam(mon));
        if (DEADMONSTER(mon)) {
            pline_mon(mon, "%s умирает!", Monnam(mon));
            if (by_you)
    ...
    if (stoning && vis) {
        if (Hallucination)
            pline("Какая жалость — %s только что испортил будущее произведение искусства!",
                  mon_nam(mon));
        else
            pline_mon(mon, "%s выглядит гибким!", Monnam(mon));
    }
    if (lizard && (mon->mconf || mon->mstun)) {
        mon->mconf = 0;
        mon->mstun = 0;
        if (vis && !is_bat(mon->data) && mon->data != &mons[PM_STALKER])
            pline_mon(mon, "%s выглядит устойчивее.", Monnam(mon));
    }
...
{                   /* [by_you not honored if 'mon' triggers fire trap]. */
    ...
    if (vis)
        pline_mon(mon, "%s начинает превращаться %s.", Monnam(mon),
              green_mon(mon) ? "в слизь" : "в зелёную слизь");
    ...
    if (trap) {
        ...
        if (mon->mx == trap->tx && mon->my == trap->ty) {
            if (vis)
                pline("%s приводит в действие %s огненную ловушку!", Mnam,
                      trap->tseen ? "эту" : "некую");
        } else {
            ...
            newsym(mon->mx, mon->my);
            if (vis)
                pline("%s %s %s %s огненную ловушку!", Mnam,
                      vtense(fakename[0], locomotion(mon->data, "move")),
                      is_floater(mon->data) ? "на" : "на",
                      trap->tseen ? "эту" : "некую");
        }
        ...
        if (vis)
            pline_mon(mon, "%s.",
                      monverbself(mon, Monnam(mon), "выдыхает", "огонь на"));
        if (!rn2(3))
        ...
        if (obj->quan > 1L)
            obj = splitobj(obj, 1L);
        if (vis && !was_lit) {
            pline_mon(mon, "%s поджигает %s.", Monnam(mon), ansimpleoname(obj));
            saw_lit = TRUE;
        }
    ...
    if (dmg) {
        ...
        if (DEADMONSTER(mon)) {
            if (by_you) {
                ...
                if (vis)
                    pline_mon(mon, "%s %s огнём!", Monnam(mon),
                          nonliving(mon->data) ? "уничтожен" : "убит");
                xkilled(mon, XKILL_NOMSG | XKILL_NOCONDUCT);
            } else
                monkilled(mon, "огонь", AD_FIRE);
        } else {
            /* non-fatal damage occurred */
            if (vis)
                pline_mon(mon, "%s обожжён%s", Monnam(mon), exclam(dmg));
        }
    }
    if (vis) {
        if (res && !DEADMONSTER(mon))
            pline_mon(mon, "%s слизь сгорает дотла!", s_suffix(Monnam(mon)));
