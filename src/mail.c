...
getmailstatus(void)
{
    ...
    if (mailbox && stat(mailbox, &omstat)) {
        ...
        pline("Не удаётся получить статус MAIL=\"%s\".", mailbox);
...
newmail(struct mail_info *info)
{
    ...
    if (!Deaf) {
        ...
        pline("Сообщение:  %s.", info->display_txt);
    }
 ...
 give_up:
    ...
    if (!message_seen && info->message_typ == MSG_OTHER)
        pline("Слушайте!  \"%s.\"", info->display_txt);
}
...
readmail(struct obj *otmp UNUSED)
{
    ...
    if (Blind) {
        pline("К сожалению, вы не можете видеть, что там написано.");
    } else {
    ...
    if (stat(mailbox, &nmstat)) {
#ifdef PERMANENT_MAILBOX
        pline("Больше не удаётся получить статус MAIL=\"%s\".", mailbox);
        free_maildata();
#else
        nmstat.st_mtime = 0;
#endif
        ...
        if (nmstat.st_size) {
            static struct mail_info deliver = {
                ...
                MSG_OTHER, "У вас есть почта во внешнем мире",
#endif
                0, 0
            };
            newmail(&deliver);
...
read_simplemail(const char *mbox, boolean adminmsg)
{
    ...
    while (fgets(curline, 128, mb) != NULL) {
        ...
        *msg = '\0';
        ...
        if (adminmsg) {
            ...
            pline("Это сообщение от '%s'.", curline);
            pline("Текст:");
        }
 ...
 bail:
    ...
    if (!adminmsg)
        pline("Похоже, это бессмысленная каша.");
}
