...
/* menu heading lines used instead of object classes when sorting by in-use;
   ...
   change "Accessories" to "Кольца" or "Amulet", then back again */
static const char *inuse_headers[] = { /* [4] shown first, [1] last */
    "", "Прочее", "Надетая броня",
    "Оружие в руках/заряжено", "Аксессуары",
};
...
merged(struct obj **potmp, struct obj **pobj)
{
    ...
    if (mergable(otmp, obj)) {
        ...
        if (discovered && otmp->where == OBJ_INVENT
            ...
            pline("Вы узнаёте больше о своих предметах, сравнивая их.");
        }
...
addinv_core2(struct obj *obj)
{
    ...
    if (Role_if(PM_ARCHEOLOGIST) && obj->oclass == SCROLL_CLASS &&
        ...
        observe_object(obj);
        pline("Вы расшифровываете надпись на %s.", yname(obj));
        makeknown(obj->otyp);
        ...
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT,
                           "стал грамотным, разобрав надпись на свитке");
    }
}
...
currency(long amount)
{
    ...
    if (Hallucination) {
        res = ROLL_FROM(currencies);
        if (amount != 1L)
            res = makeplural(res);
    } else {
        res = (amount == 1L) ? "зоркмид" : "зоркмиды";
    }
    return res;
}
...
{
    return !strcmp(action, "снять") || !strcmp(action, "take off")
...
{
    if (!strcmp(word, "имя"))
        You("делаете вид, будто даёте чему-то имя.");
    else
        You("делаете вид, будто пытаетесь %s что-то.", word);
}
...
{
    if (!strcmp(action, "смазать")) {
        Sprintf(qbuf, "ваши пальцы");
    } else if (!strcmp(action, "писать на")
               || !strcmp(action, "писать чем")) {
        Sprintf(qbuf, "ваш палец");
    } else if (!strcmp(action, "владеть")) {
        Sprintf(qbuf, "ваши %s%s",
                uarmg ? "руки в перчатках"
                         : "голые руки",
                !uwep ? " (в руках)" : "");
    } else if (!strcmp(action, "приготовить")
               || !strcmp(action, "выстрелить")) {
        Sprintf(qbuf, "пустой колчан%s",
                !uquiver ? " (ничего не заряжено)" : "");
    } else {
        Sprintf(qbuf, "ваши руки");
    }
    return qbuf;
}

/* Строит русский промпт getobj с учётом падежа глагола. */
static void
getobj_prompt(char *qbuf, const char *word)
{
    if (!strcmp(word, "имя"))
        Sprintf(qbuf, "Что вы хотите назвать?");
    else if (!strcmp(word, "владеть"))
        Sprintf(qbuf, "Чем вы хотите владеть?");
    else if (!strcmp(word, "натереть королевским желе"))
        Sprintf(qbuf, "Что вы хотите натереть королевским желе?");
    else if (!strcmp(word, "писать на"))
        Sprintf(qbuf, "На чём вы хотите писать?");
    else if (!strcmp(word, "писать чем"))
        Sprintf(qbuf, "Чем вы хотите писать?");
    else if (!strcmp(word, "окунуть"))
        Sprintf(qbuf, "Во что вы хотите окунуть?");
    else if (!strcmp(word, "выстрелить"))
        Sprintf(qbuf, "Чем вы хотите выстрелить?");
    else
        Sprintf(qbuf, "Что вы хотите %s?", word);
}
    ...
    unsigned int ctrlflags) /* some control to fine-tune the behavior */
{
 ...
 need_more_cq:
    ...
    *ap = '\0';
    ...
    if (suggested == 0 && !forceprompt && !allownone) {
        You("не имеете ничего%s, чтобы %s.", inaccess ? " другого" : "",
            word);
        return (struct obj *) 0;
    }
    for (;;) {
        ...
        if (digit(ilet)) {
            ...
            if (!allowcnt) {
                pline("Количество с этой командой не допускается.");
                continue;
            }
 ...
 redo_menu:
        ...
        if (ilet == GOLD_SYM
            ...
            if (cntgiven && cnt <= 0L) {
                if (cnt < 0L)
                    pline("LRS Было бы очень интересно узнать,"
                              " Что у вас так много.");
                return (struct obj *) 0;
            }
        }
        if (cntgiven && (!strcmp(word, "throw")
            ...
            static const char only_one[] =
                "можете бросить только один предмет за раз";
            boolean coins;
            ...
            if (cnt > 1L && (!coins || cnt > otmp->quan)) {
                if (cnt > otmp->quan)
                    You("имеете только %ld%s%s.", otmp->quan,
                        (!coins && otmp->quan > 1L) ? ", а " : "",
        ...
        if (!otmp) {
            You("нет этого предмета.");
            if (gi.in_doagain)
                return (struct obj *) 0;
            continue;
        } else if (cnt < 0L || otmp->quan < cnt) {
            You("нет столько!  У вас только %ld.", otmp->quan);
            if (gi.in_doagain)
                return (struct obj *) 0;
            continue;
        }
            ...
            struct obj *otmp)
{
    ...
    if ((!strcmp(word, "call") || !strcmp(word, "назвать"))
        && (otmp->otyp == AMULET_OF_YENDOR
            || (otmp->otyp == FAKE_AMULET_OF_YENDOR && !otmp->known)))
        pline("АМулет не любит, когда его обзывают.");
    else
        ...
        unsigned *resultflags)
{
    ...
    if (!gi.invent) {
        You("не имеете ничего, чтобы %s.", word);
        if (resultflags)
    ...
    for (;;) {
        Sprintf(qbuf, "Какие предметы вы хотите %s? [%s]",
                word, ilets);
        getlin(qbuf, buf);
        if (buf[0] == '\033')
            return 0;
        ...
        if (takeoff && oc_of_sym != MAXOCLASSES) {
            if (strchr(extra_removeables, oc_of_sym)) {
                ...
                return 0;
                       ...
                       && !uquiver) {
                You("не держите ничего в руках.");
                return 0;
            } else if (oc_of_sym == RING_CLASS && !uright && !uleft) {
                You("не носите никаких колец.");
                return 0;
            } else if (oc_of_sym == AMULET_CLASS && !uamul) {
                You("не носите никакого амулета.");
                return 0;
            } else if (oc_of_sym == TOOL_CLASS && !ublindf) {
                You("не носите никакой повязки на глазах.");
                return 0;
            }
        }
    ...
    const char *word)    /* name of the action */
{
    ...
    ident = !strcmp(word, "опознать");
    take_out = !strcmp(word, "взять");
    put_in = !strcmp(word, "положить");
    nodot = (!strcmp(word, "nodot") || !strcmp(word, "выбросить") || ident
 ...
 nextclass:
    ...
    if (!takeoff && (dud || cnt))
        ...
        pline("Нет подходящих предметов.");
 ret:
...
reroll_menu(void)
{
    ...
    add_menu(win, &nul_glyphinfo, &any, flags.lootabc ? 0 : 'p', 0,
             ATR_NONE, NO_COLOR, "начать игру этим персонажем",
             MENU_ITEMFLAGS_NONE);
    any.a_char = 'y';
    add_menu(win, &nul_glyphinfo, &any, flags.lootabc ? 0 : 'r', 0,
             ATR_NONE, NO_COLOR, "пересоздать другого персонажа",
             MENU_ITEMFLAGS_NONE);
    any.a_char = 0;
    ...
    end_menu(win, "Пересоздать этого персонажа?");
    if (select_menu(win, PICK_ONE, &pick_list) > 0) {
        ...
        option = y_n("Пересоздать этого персонажа?");
    }
...
menu_identify(int id_limit)
{
    ...
    while (id_limit) {
        Sprintf(buf, "Что вы хотите опознать (%s)?",
                first ? "сначала" : "далее");
        ...
        if (n > 0) {
            ...
            pline("Выберите предмет; нажмите ESC, чтобы отказаться.");
        }
    }
}
    ...
    boolean learning_id) /* T: just read unknown identify scroll */
{
    ...
    if (!unid_cnt) {
        You("уже опознали %s своих вещей.",
            !learning_id ? "все" : "остальные");
        ...
        if (flags.menu_style == MENU_TRADITIONAL)
            do {
                n = ggetobj("опознать", identify, id_limit, FALSE,
                            (unsigned *) 0);
                if (n < 0)
...
doperminv(void)
{
    ...
    if ((windowprocs.wincap & WC_PERM_INVENT) == 0) {
        ...
        pline("Постоянный показ инвентаря не поддерживается «%s».",
              windowprocs.name);

    } else if (!iflags.perm_invent) {
        pline(
     "Постоянный инвентарь (опция 'perm_invent') сейчас не включён.");

    } else if (!gi.invent) {
        /* [should this be left for the interface to decide?] */
        pline("Показ постоянного инвентаря пуст.");
    ...
    long *out_cnt) /* optional; count player entered when selecting an item */
{
    static const char /* potential entries for perm_invent window */
        not_carrying_anything[] = "Ничего не несёте",
        not_using_anything[] = "Ничего не используете",
        only_carrying_gold[] = "Несёте только золото";
    ...
    if (wizid) {
        ...
        Sprintf(prompt, "Отладка: опознание"); /* 'title' rather than 'prompt' */
        if (unid_cnt)
            Sprintf(eos(prompt),
                    " -- неопознанных или частично опознанных предметов: %d",
                    unid_cnt);
        add_menu_str(win, prompt);
        if (!unid_cnt) {
            add_menu_str(win,
                         "(все предметы уже опознаны навсегда)");
            gotsomething = TRUE;
        } else {
            any.a_obj = &wizid_fakeobj;
            Sprintf(prompt, "выберите %s, чтобы опознать навсегда",
                    (unid_cnt == 1) ? "его" : "любые или все");
 ...
 nextclass:
    ...
    for (srtinv = sortedinvent; (otmp = srtinv->obj) != 0; ++srtinv) {
        ...
        if (!flags.sortpack || otmp->oclass == *invlet) {
            ...
            if (inuse_only) {
                ...
                if (!inusecount++)
                    add_menu_heading(win, doing_perm_invent ? "В использовании"
                                            : "Инвентарь в использовании");
            ...
            if (otmp == &inuse_fakeobj) {
                ...
                Sprintf(barehands, "%s",
                        uarmg ? "ваши руки в перчатках (без оружия)"
                               : "ваши голые руки (без оружия)");
    ...
    if (iflags.force_invmenu && want_reply) {
        ...
        if ((allowxtra && !usextra)
            ...
            any.a_char = '*';
            menutext = "(показать все)";
        } else if (!lets) {
            any.a_char = '?';
            menutext = "(показать подходящие)";
        }
        if (menutext) {
            add_menu_heading(win, "Особое");
...
display_used_invlets(char avoidlet)
{
    ...
    if (gi.invent) {
        ...
        end_menu(win, "Использованные буквы инвентаря:");
    ...
    int buriedcount) /* unpaid items under the floor (extremely rare) */
{
        ...
        for (otmp = gi.invent; otmp; otmp = otmp->nobj) {
            if (Has_contents(otmp)) {
                ...
                if (!otmp->cknown) {
                    ...
                    Sprintf(contbuf, "Содержимое: %s", xname(otmp));
                    putstr(win, 0,
    ...
    if (count > 0) {
        putstr(win, 0, "");
        putstr(win, 0,
               xprname((struct obj *) 0, "Итого:", '*', FALSE, totcost, 0L));
    }
    ...
    if (xtracount > 0) { /* floorcount + buriedcount > 0 */
        char buf[BUFSZ];
        const char
            *floorverb = (xtracount > 1) ? "лежат" : "лежит",
            /* "under the floor" might actually be "under the floor
               ...
               door/doorway") */
            *where = (buriedcount == 0) ? "на полу"
                     : (floorcount == 0) ? "под полом"
                       : "на полу или под ним";

        if (!count) {
            You("не несёте неоплаченных предметов, но %s %s %d.",
                where, floorverb, xtracount);
        } else {
            putstr(win, 0, "");
            Sprintf(buf, "(Ещё %d неоплаченных предметов %s.)",
                    xtracount, where);
            putstr(win, 0, buf);
        }
    }
...
{
    static const char
        prompt[] = "Инвентарь каких предметов показать?";
    char c = '\0';
    int n, i = 0;
    ...
    if (c == 'x' || (c == 'X' && billx && !xcnt)) {
        if (billx)
            ...
            pline("Нет использованных предметов%s.",
                  any_unpaid ? " в вашем счёте за покупки" : "");
        goto doI_done;
    }
    if (c == 'u' || (c == 'U' && any_unpaid && !ucnt)) {
        if (any_unpaid)
            ...
            You("не несёте неоплаченных предметов.");
        goto doI_done;
    }
    ...
    switch (c) {
    case 'B':
        before = "заведомо благословлённых ";
        break;
    case 'U':
        before = "заведомо непроклятых ";
        break;
    case 'C':
        before = "заведомо проклятых ";
        break;
    case 'X':
        after = ", чей статус благосл./проклят. неизвестен";
        break; /* better phrasing is desirable */
    case 'P':
        after = ", которые только что подняли";
        break;
    default:
    ...
    if (traditional) {
        if (strchr(types, c) > strchr(types, '\033')) {
            You("не имеете %sпредметов%s.", before, after);
            goto doI_done;
        }
    ...
    if (strchr("BUCXP", c)) {
        ...
        switch (c) {
        case 'B':
            t = "Предметы, известные как благословлённые";
            break;
        case 'U':
            t = "Предметы, известные как непроклятые";
            break;
        case 'C':
            t = "Предметы, известные как проклятые";
            break;
        case 'X':
            t = "Предметы неизвестного статуса благосл./проклят.";
            break;
        default: /* 'P' */
            t = "Предметы, только что поднятые";
            break;
        }
        Sprintf(title, "%s", t);
        gt.this_title = title;
...
dfeature_at(coordxy x, coordxy y, char *buf)
{
    ...
    if (IS_DOOR(ltyp)) {
        ...
        if (is_drawbridge_wall(x, y) >= 0)
            dfeature = "открытая решётка подъёмного моста", cmap = -1;
        ...
        Sprintf(altbuf, "%sалтарь %s (%s)",
                (lev->altarmask & AM_SANCTUM) ? "священный " : "",
                a_gname(),
    ...
    unsigned lookhere_flags)
{
    ...
    if (u.uswallow) {
         ...
         */
        You("пытаетесь рассмотреть, что лежит у %s (%s).", mon_nam(mtmp),
        ...
        if (otmp) {
            ...
            if (Blind)
                Strcpy(fbuf, "Вы чувствуете");
            Strcat(fbuf, ":");
            (void) display_minventory(mtmp, MINV_ALL | PICK_NONE, fbuf);
        } else {
            pline(Blind ? "Вы не ощущаете здесь предметов."
                          : "Вы не видите здесь предметов.");
        }
    ...
    if (dfeature && !strcmp(dfeature, "лужа воды") && Underwater)
        dfeature = 0;

    if (Blind) {
        boolean drift = Is_airlevel(&u.uz) || Is_waterlevel(&u.uz);

        if (dfeature && !strncmp(dfeature, "алтарь ", 7)) {
            /* don't say "altar" twice, dfeature has more info */
            You("пытаетесь ощупать, что здесь находится.");
            ...
            You("пытаетесь ощупать, что на ней лежит.");
            ...
            if (drift)
                You("ощупываете, что рядом что-то плавает.");
            else if (cant_reach)
                You("ощупываете, что лежит под вами.");
            else
                You("ощупываете %s.", surf);
        ...
        if (!can_reach_floor(trap && is_pit(trap->ttyp))) {
            pline("Но вы не можете до него дотянуться!");
            return ECMD_OK;
        }
    }
    ...
    if (dfeature && !skip_dfeature) {
        ...
        if (!strcmp(dfeature, "раскалённая лава")
            || !strcmp(dfeature, "железные прутья")
            || !strcmp(dfeature, "ice")
    ...
    if (skip_objects) {
        ...
        if (obj_cnt == 1 && otmp->quan == 1L)
            There("здесь есть %s предмет.", picked_some ? "ещё один" : "один");
        else
            There("здесь %s%s предметов.",
                  (obj_cnt == 2) ? "два"
                  : (obj_cnt < 5) ? "несколько"
                    : (obj_cnt < 10) ? "несколько"
                      : "много",
                  picked_some ? " ещё" : "");
        ...
        if (dfeature && !skip_dfeature)
            pline1(fbuf);
        read_engr_at(u.ux, u.uy); /* Eric Backus */
        pline(Blind ? "Вы ощупываете здесь %s." : "Вы видите здесь %s.",
        ...
        Sprintf(buf, "%s %s:", Blind ? "Ощущаемые" : "Лежащие",
                picked_some ? "прочие предметы" : "предметы");
        putstr(tmpwin, 0, buf);
        for (; otmp; otmp = otmp->nexthere) {
            if (otmp->otyp == CORPSE && will_feel_cockatrice(otmp, FALSE)) {
...
feel_cockatrice(struct obj *otmp, boolean force_touch)
{
    ...
    if (will_feel_cockatrice(otmp, force_touch)) {
        ...
        if (poly_when_stoned(gy.youmonst.data))
            You("коснулись %s голой %s.", kbuf,
                makeplural(body_part(HAND)));
        else
            pline("Прикосновение к %s — фатальная ошибка...", kbuf);
        /* normalize body shape here; hand, not body_part(HAND) */
        Sprintf(kbuf, "прикосновение голой рукой к %s", killer_xname(otmp));
...
doprgold(void)
{
    ...
    if (flags.verbose) {
        ...
        if (!umoney) {
            Strcpy(buf, "Ваш кошелёк пуст");
        } else {
            Sprintf(buf, "В вашем кошельке %ld %s",
                    umoney, currency(umoney));
        }
        if (hmoney) {
            if (umoney)
                Sprintf(eos(buf), ", а ещё %ld спрятано в рюкзаке",
                        hmoney);
            else
                Sprintf(eos(buf), ", но %ld %s спрятано в рюкзаке",
        ...
        if (total)
            You("несёте всего %ld %s.", total, currency(total));
        else
            You("нет денег.");
    }
...
noarmor(boolean report_uskin)
{
    if (!uskin || !report_uskin) {
        ...
        You("не носите брони, но у вас в кожу вплавлено %s.",
            uskinname);
    }
}
...
doprring(void)
{
    if (!uleft && !uright) {
        ...
        /* if either ring is a meat ring, switch to use_inuse_mode in order
           to label it/them as "Кольца" rather than "Пища" */
        if (uright) {
...
dopramulet(void)
{
    if (!uamul) {
        ...
        (void) dispinv_with_action(lets, TRUE, "Амулет");
    }
    return ECMD_OK;
}
...
doprtool(void)
{
    ...
    lets[ct] = '\0';
    if (!ct)
        You("не используете никаких инструментов.");
    else
        (void) dispinv_with_action(lets, TRUE, NULL);
    return ECMD_OK;
}
...
doprinuse(void)
{
    ...
    if (!ct)
        You("ничего не носите и не держите в руках.");
    else
...
static NEARDATA const char *names[] = {
    0, "Загрязненные объекты", "Оружие", "Доспехи", "Кольца", "Амулеты", "Инструменты",
    "Пища", "Зелья", "Свитки", "Книги заклинаний", "Палочки", "Монеты",
    "Драгоценности/камни", "Булыжники/статуи", "Железные ядра", "Цепи", "Яды"
};
static NEARDATA const char oth_symbols[] = { CONTAINED_SYM, '\0' };
static NEARDATA const char *oth_names[] = { "В сумках/ящиках" };
...
let_to_name(char let, boolean unpaid, boolean showsym)
{
    ...
    if (unpaid)
        Strcat(strcpy(gi.invbuf, "Неоплаченное "), class_name);
    else
...
doorganize(void) /* inventory organizer by Del Lamb */
{
    ...
    if (!gi.invent || (gi.invent->oclass == COIN_CLASS
        ...
        You("не несёте ничего %s.",
            !gi.invent ? "для настройки" : "настраиваемого");
        return ECMD_OK;
    }
    ...
    obj = getobj("переназначить", adjust_filter, GETOBJ_PROMPT | GETOBJ_ALLOWCNT);
...
adjust_split(void)
{
    ...
    obj = getobj("разделить", adjust_ok, GETOBJ_NOFLAGS);
    ...
    if (obj->quan == 2L) {
        ...
        dig = yn_function("Сколько отделить?", (char *) 0, '\0', TRUE);
        if (!digit(dig)) {
            pline1(Never_mind);
            return ECMD_CANCEL;
        }
    ...
    if (splitamount < 1L || splitamount >= obj->quan) {
        static const char
            Amount[] = "Отделяемое количество должно быть";

        if (splitamount < 1L)
            pline("%s не менее 1.", Amount);
        else
            pline("%s меньше %ld.", Amount, obj->quan);
        return ECMD_CANCEL;
    }
...
#define OVRFLW_INDX (GOLD_OFFSET + invlet_basic) /* past gold+2*26 letters */
    ...
    if (!splitting)
        Strcpy(qbuf, "Переназначить букву");
    else /* note: splitting->quan is the amount being left in original slot */
        Sprintf(qbuf, "Отделить %ld", obj->quan);
    Sprintf(eos(qbuf), " в какую [%s]%s?", lets,
            gi.invent ? " (? — использованные буквы)" : "");
    for (trycnt = 1; ; ++trycnt) {
 ...
 noadjust:
            ...
            pline("В ячейку «%c» можно перемещать только золотые монеты.",
                  GOLD_SYM);
            ever_mind = TRUE;
            goto noadjust;
        }
        ...
        if (trycnt == 5)
            goto noadjust;
        pline("Выберите букву ячейки инвентаря."); /* else try again */
    }
    ...
    char *title)       /* menu title */
{
    ...
    Sprintf(tmp, "%s у %s:", do_all ? "Имущество" : "Снаряжение",
            noit_Monnam(mon));
    ...
    if (do_all ? have_any : (mon->misc_worn_check || MON_WEP(mon))) {
        ...
        invdisp_nothing(title ? title : tmp, "(нет)");
        n = 0;
    }
...
cinv_doname(struct obj *obj)
{
    ...
    if (obj->otrapped && strlen(result) + sizeof "с ловушкой " <= QBUFSZ) {
        /* obj->lknown has been set before calling us so either "locked" or
           "unlocked" should always be present (for a trapped container) */
        char *q = strstri(result, "незапертый "),
             *p = strstri(result, "запертый ");

        if (p && (!q || p < q))
            (void) strsubst(p, "запертый ", "запертый с ловушкой ");
        else if (q)
            (void) strsubst(q, "незапертый ", "незапертый с ловушкой ");
    }
    return result;
}
...
cinv_ansimpleoname(struct obj *obj)
{
    ...
    if (obj->otrapped)
        (void) strsubst(result, "", "с ловушкой "); /* insert at beginning */
    return result;
}
...
display_cinventory(struct obj *obj)
{
    ...
    (void) safe_qbuf(qbuf, "Содержимое «", "»:", obj,
...
display_binventory(coordxy x, coordxy y, boolean as_if_seen)
{
    ...
    if (is_pool_or_lava(x, y) && !Underwater
        ...
        const char *real_liquid = is_pool(x, y) ? "водой" : "лавой",
                   *seen_liquid = hliquid(real_liquid);

        if (!obj->nexthere) {
            boolean more_than_1 = is_plural(obj);

            There("под %s %s %s.", seen_liquid,
                  more_than_1 ? "лежат" : "лежит", doname(obj));
            n2 = 1;
            /* "pair of boots" is singular but "beneath it" sounds strange */
            if (pair_of(obj))
                more_than_1 = TRUE;
            underwhat = more_than_1 ? "под ними" : "под ним";
        } else {
            Sprintf(qbuf, "Вещи под %s:", seen_liquid);
    ...
    if (n) {
        ...
        Sprintf(qbuf, "Вещи, зарытые %s:", underwhat);
            ...
            if (wri != 0) {
                ...
                if ((wri->tocore.tocore_flags & (too_small | prohibited))
                    ...
                    wport_id = WINDOWPORT(tty) ? "tty perm_invent"
                                               : "perm_invent";
                    pline("%s не удалось включить.", wport_id);
                    pline("Для %s нужен терминал не менее %dx%d, у вас "
                          "не %dx%d.",
