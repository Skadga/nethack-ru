...
static NEARDATA const char *ends[] = {
    /* "when you %s" */
    "погибли", "задохнулись", "были отравлены",
    "умерли от голода", "утонули", "сгорели",
    "растворились в лаве",
    "были раздавлены", "окаменели",
    "превратились в слизь", "были уничтожены геноцидом",
    "впали в панику", "были обмануты", "вышли",
    "сбежали", "вознеслись"
};
...
done_in_by(struct monst *mtmp, int how)
{
    ...
    You((how == STONING) ? "каменеете..." : "умираете...");
...
disclose(int how, boolean taken)
{
    ...
    if (gi.invent && !done_stopprint) {
        if (taken)
            Sprintf(qbuf, "Хотите увидеть, что у вас было, когда вы %s?",
                    (how == QUIT) ? "вышли" : "погибли");
        else
            Strcpy(qbuf, "Хотите опознать ваши вещи?");
    ...
    if (!done_stopprint) {
        ...
        c = ask ? yn_function("Хотите увидеть ваши характеристики?", ynqchars,
                              defquery, TRUE)
                : defquery;
        if (c == 'y')
            enlightenment((BASICENLIGHTENMENT | MAGICENLIGHTENMENT),
    ...
    if (!done_stopprint) {
        if (should_query_disclose_option('c', &defquery)) {
            ...
            Sprintf(qbuf, "Хотите увидеть ваш образ действий%s?",
    ...
    if (!done_stopprint) {
        ...
        c = ask ? yn_function("Хотите увидеть обзор подземелья?",
...
savelife(int how)
{
    ...
    gn.nomovemsg = "Вы пережили это покушение на вашу жизнь.";
    svc.context.move = 0;
    ...
    if (u.uswallow) {
        ...
        if (Upolyd && sticks(gy.youmonst.data))
            You("высвобождаетесь от %s.", mon_nam(u.ustuck));
        else
            pline("%s отпускает вас.", Monnam(u.ustuck));
        unstuck(u.ustuck);
    }
}
...
done(int how)
{
    ...
    if (how == TRICKED) {
        ...
        if (wizard) {
            You("похоже, вы очень хитрый волшебник.");
    ...
    if (Lifesaved && (how <= GENOCIDED)) {
        ...
        savelife(how);
        if (how == GENOCIDED) {
            pline("К сожалению, вы всё ещё истреблены...");
        } else {
            char killbuf[BUFSZ];
            formatkiller(killbuf, BUFSZ, how, FALSE);
            livelog_printf(LL_LIFESAVE, "избежал смерти (%s)", killbuf);
            survive = TRUE;
        }
    }
    ...
    if (!survive && (wizard || discover) && how <= GENOCIDED
        ...
        pline("Ладно, значит, вы не %s.", (how == CHOKING) ? "задохнетесь" : "умрёте");
...
really_done(int how)
{
    ...
    if (svm.moves <= 1 && how < PANICKED && !done_stopprint)
        pline("Не проходите через Старт.  Не получайте 200 %s.", currency(200L));
    ...
    if (bones_ok) {
        if (!wizard || paranoid_query(ParanoidBones, "Сохранить останки?"))
    ...
    if (how == ESCAPED || how == ASCENDED) {
        ...
        Sprintf(eos(pbuf), "%s с %ld очками,",
                (how == ASCENDED) ? "отправился к награде"
                                  : "сбежал из подземелья",
                u.urexp);
        dump_forward_putstr(endwin, 0, pbuf, done_stopprint);
        ...
        if (u.uz.dnum == 0 && u.uz.dlevel <= 0) {
            ...
            Sprintf(pbuf, "Вы %s за пределами подземелья",
                    (u.uz.dlevel < 0) ? "скончались" : ends[how]);
        } else {
            ...
            Sprintf(pbuf, "Вы %s в %s", ends[how], where);
    ...
    dump_forward_putstr(endwin, 0, pbuf, done_stopprint);
    Sprintf(pbuf,
            "Вы были на уровне %d с максимумом %d очков здоровья%s, когда вы %s.",
