...
kickdmg(struct monst *mon, boolean clumsy)
{
    ...
    if (mon->data == &mons[PM_SHADE] && !specialdmg) {
        pline_The("Ваш пинок безвредно проходит насквозь.");
    ...
    if (!DEADMONSTER(mon) && martial() && !bigmonst(mon->data) && !rn2(3)
        ...
        if (goodpos(mdx, mdy, mon, 0)) {
            pline("%s шатается от удара.", Monnam(mon));
...
kick_monster(struct monst *mon, coordxy x, coordxy y)
{
    ...
    if (Levitation && !rn2(3) && verysmall(mon->data)
        && !is_flyer(mon->data)) {
        pline("Паря в воздухе, вы промахиваетесь!");
    ...
    if (mon->mundetected
        ...
        if (!canspotmon(mon))
            ...
            newsym(x, y);
        There("%s здесь.",
              canspotmon(mon) ? a_monnam(mon) : "что-то скрытое");
    }
        ...
        for (i = 0; i < NATTK; i++) {
            ...
            if (mon->data == &mons[PM_SHADE] && !specialdmg) {
                /* doesn't matter whether it would have hit or missed,
                   and shades have no passive counterattack */
                Your("Ваш пинок безвредно проходит сквозь %s.",
                     mon_nam(mon));
                break; /* skip any additional kicks */
            } else if (tmp > kickdieroll) {
                You("пинаете %s.", mon_nam(mon));
    ...
    if (i < (j * 3) / 10) {
        if (!rn2((i < j / 10) ? 2 : (i < j / 5) ? 3 : 4)) {
            if (martial())
                goto doit;
            Your("неуклюжий пинок не причиняет урона.");
            (void) passive(mon, uarmf, FALSE, 1, AT_KICK, FALSE);
            return;
        }
        if (i < j / 10)
 ...
 doit:
    ...
    if (!rn2(clumsy ? 3 : 4) && (clumsy || !bigmonst(mon->data))
        ...
        if (!nohands(mon->data) && !rn2(martial() ? 5 : 3)) {
            pline("%s блокирует ваш %sпинок.", Monnam(mon),
                  clumsy ? "неуклюжий " : "");
            (void) passive(mon, uarmf, FALSE, 1, AT_KICK, FALSE);
            return;
        } else {
            ...
            if (mon->mx != x || mon->my != y) {
                ...
                pline("%s %s, %s уклоняясь от вашего %sпинка.",
                      Monnam(mon),
                      (can_teleport(mon->data) && !noteleport_level(mon))
                          ? "телепортируется"
                          : is_floater(mon->data)
                                ? "парит"
                                : is_flyer(mon->data) ? "пикирует"
                                                      : (nolimbs(mon->data)
                                                         || slithy(mon->data))
                                                            ? "скользит"
                                                            : "прыгает",
...
ghitm(struct monst *mtmp, struct obj *gold)
{
    ...
    if (!likes_gold(mtmp->data) && !mtmp->isshk && !mtmp->ispriest
        ...
        if (canseemon(mtmp)) {
            pline_The("%s безвредно %s %s.", xname(gold),
                      otense(gold, "задевает"), mon_nam(mtmp));
            msg_given = TRUE;
        }
    } else {
        ...
        if (cansee(mtmp->mx, mtmp->my))
            pline("%s %sхватает золото.", Monnam(mtmp),
                  was_sleeping ? "просыпается и " : "");
        ...
        if (mtmp->isshk) {
            ...
            if (robbed) {
                ...
                if (robbed < 0L)
                    robbed = 0L;
                pline("Сумма %sпокрывает %s недавние убытки.",
                          !robbed ? "" : "частично ", mhis(mtmp));
                ...
                if (mtmp->mpeaceful) {
                    ...
                    You("имеете %ld %s кредита.", ESHK(mtmp)->credit,
                        currency(ESHK(mtmp)->credit));
                } else
                    verbalize("Спасибо, мерзавец!");
            }
        } else if (mtmp->ispriest) {
            SetVoice(mtmp, 0, 80, 0);
            if (mtmp->mpeaceful)
                verbalize("Спасибо за ваше пожертвование.");
            else
                verbalize("Спасибо, мерзавец!");
        } else if (mtmp->isgd) {
            ...
            SetVoice(mtmp, 0, 80, 0);
            verbalize(umoney ? "Бросьте остальное и следуйте за мной."
                      : hidden_gold(TRUE)
                        ? "У вас всё ещё спрятано золото.  Бросьте его сейчас."
                        : mtmp->mpeaceful
                          ? "Я позабочусь об этом; прошу пройти дальше."
                          : "Я возьму это; а теперь шевелитесь.");
            ...
            if (!mtmp->mpeaceful) {
                SetVoice(mtmp, 0, 80, 0);
                if (goldreqd)
                    verbalize("Этого мало, трус!");
                else /* unbribable (watchman) */
                    verbalize("Я не беру взяток у таких мерзавцев, как ты!");
            } else if (was_angry) {
                SetVoice(mtmp, 0, 80, 0);
                verbalize("Этого достаточно.  А теперь убирайся!");
            } else {
                SetVoice(mtmp, 0, 80, 0);
                verbalize("Спасибо за подачку, %s.",
                          flags.female ? "госпожа" : "дружище");
            }
        }
        return TRUE;
    }
    ...
    coordxy y) /* before the impact, not after */
{
    ...
    for (otmp = obj->cobj; otmp; otmp = otmp2) {
        ...
        if (objects[otmp->otyp].oc_material == GLASS
            ...
            result = "звон разбитого стекла";
        } else if (otmp->otyp == EGG && !rn2(3)) {
            result = "треск яичной скорлупы";
        }
        if (result) {
            if (otmp->otyp == MIRROR)
                change_luck(-2);
            ...
            You_hear("приглушённый %s.", result);
            if (costly) {
    ...
    if (costly && loss) {
        if (!insider) {
            You("причинили урон на %ld %s!", loss, currency(loss));
            make_angry_shk(shkp, x, y);
        } else {
            You("должны %s %ld %s за разрушенные предметы.", shkname(shkp), loss,
                currency(loss));
        }
    }
}
...
really_kick_object(coordxy x, coordxy y)
{
    ...
    if ((trap = t_at(x, y)) != 0) {
        if ((is_pit(trap->ttyp) && !Passes_walls) || trap->ttyp == WEB) {
            if (!trap->tseen)
                find_trap(trap);
            You_cant("пинать %s, лежащее в %s!", something,
                     Hallucination ? "панике"
                         : (trap->ttyp == WEB) ? "паутине"
                             : "яме");
            return 1;
        }
    ...
    if (Fumbling && !rn2(3)) {
        Your("неуклюжий пинок промахнулся.");
        return 1;
    }
    ...
    if (!uarmf && gk.kickedobj->otyp == CORPSE
        ...
        if (poly_when_stoned(gy.youmonst.data) && polymon(PM_STONE_GOLEM)) {
            ...
            Sprintf(svk.killer.name, "пиная %s босиком",
    ...
    if (IS_OBSTRUCTED(levl[x][y].typ) || closed_door(x, y)) {
        if ((!martial() && rn2(20) > ACURR(A_DEX))
            ...
            if (Blind)
                pline("Оно не отходит.");
            else
                pline("%s не отходит.", The(distant_name(gk.kickedobj, xname)));
            return (!rn2(3) || martial());
        }
        if (Blind)
            pline("Оно отходит.");
        else
            pline("%s отходит.", The(distant_name(gk.kickedobj, xname)));
    ...
    if (Is_box(gk.kickedobj)) {
        ...
        if (gk.kickedobj->olocked) {
            ...
            if (!rn2(3) || (martial() && !rn2(2))) {
                pline("Крышка распахивается, затем захлопывается.");
        ...
        if (!isgold) {
            ...
            if (rn2(20)) {
                static NEARDATA const char *const flyingcoinmsg[] = {
                    "рассыпаете монеты",
                    "раскидываете монеты по всему полу",
                    "разбрасываете монеты во все стороны",
                };
    ...
    if (slide && !Blind)
        pline("Ух ты!  %s %s по %s.", Doname2(gk.kickedobj),
              otense(gk.kickedobj, "скользит"), surface(x, y));
...
kickstr(char *buf, const char *kickobjnam)
{
    ...
    if (*kickobjnam)
        ...
        what = "приставную лестницу";
    else if (gm.maploc->typ == IRONBARS)
        what = "железный прут";
    else
        what = "что-то странное";
    return strcat(strcpy(buf, "пинок в "), what);
}
...
watchman_thief_arrest(struct monst *mtmp)
{
    if (is_watch(mtmp->data) && couldsee(mtmp->mx, mtmp->my)
        && mtmp->mpeaceful) {
        mon_yells(mtmp, "Стой, вор!  Ты арестован!");
...
watchman_door_damage(struct monst *mtmp, coordxy x, coordxy y)
{
    if (is_watch(mtmp->data) && mtmp->mpeaceful
        ...
        if (levl[x][y].looted & D_WARNED) {
            mon_yells(mtmp,
                      "Стой, вандал!  Ты арестован!");
            (void) angry_guards(FALSE);
        } else {
            mon_yells(mtmp, "Эй, прекратите повреждать эту дверь!");
...
kick_dumb(coordxy x, coordxy y)
{
    ...
    if (martial() || ACURR(A_DEX) >= 16 || rn2(3)) {
        ...
        pline("Глупый ход!  Вы растянули мышцу.");
...
kick_ouch(coordxy x, coordxy y, const char *kickobjnam)
{
    ...
    if (isok(x, y)) {
        ...
        if (is_drawbridge_wall(x, y) >= 0) {
            pline("Подъёмный мост не затронут.");
...
kick_door(coordxy x, coordxy y, int avrg_attrib)
{
    ...
    if (doorbuster
        ...
        if (gm.maploc->doormask & D_TRAPPED) {
            ...
            exercise(A_STR, FALSE);
            gm.maploc->doormask = D_NODOOR;
            b_trapped("дверь", FOOT);
        } else if (ACURR(A_STR) > 18 && !rn2(5) && !shopdoor) {
            Soundeffect(se_kick_door_it_shatters, 50);
            pline("Когда вы пинаете дверь, она разлетается вдребезги!");
            exercise(A_STR, TRUE);
            gm.maploc->doormask = D_NODOOR;
        } else {
            Soundeffect(se_kick_door_it_crashes_open, 50);
            pline("Когда вы пинаете дверь, она с грохотом распахивается!");
            exercise(A_STR, TRUE);
...
kick_nondoor(coordxy x, coordxy y, int avrg_attrib)
{
    ...
    if (gm.maploc->typ == SCORR) {
        if (!Levitation && rn2(30) < avrg_attrib) {
            Soundeffect(se_crash_door, 40);
            pline("Грохот!  Вы вышибаете секретный проход!");
            exercise(A_DEX, TRUE);
            gm.maploc->typ = CORR;
            feel_newsym(x, y); /* we know it's gone */
            unblock_point(x, y); /* vision */
    ...
    if (IS_THRONE(gm.maploc->typ)) {
        ...
        if ((Luck < 0 || gm.maploc->looted) && !rn2(3)) {
            ...
            if (Blind)
                pline("ГРОХОТ!  Вы уничтожаете его.");
            else {
                pline("ГРОХОТ!  Вы уничтожаете трон.");
                newsym(x, y);
            }
            exercise(A_DEX, TRUE);
            return ECMD_TIME;
            ...
            while (i--)
                (void) mksobj_at(
                       ...
                       FALSE, TRUE);
            if (Blind)
                You("выбиваете %s!", something);
            else {
                You("выбиваете несколько декоративных монет и самоцветов!");
                newsym(x, y);
            }
    ...
    if (IS_ALTAR(gm.maploc->typ)) {
        ...
        You("пинаете %s.", (Blind ? something : "алтарь"));
        altar_wrath(x, y);
        if (!rn2(3)) {
            kick_ouch(x, y, "");
            return ECMD_TIME;
    ...
    if (IS_FOUNTAIN(gm.maploc->typ)) {
        ...
        You("пинаете %s.", (Blind ? something : "фонтан"));
        if (!rn2(3)) {
            kick_ouch(x, y, "");
            return ECMD_TIME;
        }
        ...
        if (uarmf && rn2(3))
            if (water_damage(uarmf, "металлические сапоги", TRUE)
                == ER_NOTHING) {
                Your("сапоги промокают.");
    ...
    if (IS_GRAVE(gm.maploc->typ)) {
        if (Levitation) {
            ...
            if (Blind) {
                ...
                pline("Треск!  %s разбилось!", Something);
            } else {
                pline("Надгробие опрокидывается и разбивается!");
                newsym(x, y);
            }
        }
        return ECMD_TIME;
    ...
    if (IS_TREE(gm.maploc->typ)) {
        ...
        if (rn2(3)) {
            if (!rn2(6) && !(svm.mvitals[PM_KILLER_BEE].mvflags & G_GONE))
                You_hear("низкое жужжание."); /* a warning */
            kick_ouch(x, y, "");
            return ECMD_TIME;
        }
        if (rn2(15) && !(gm.maploc->looted & TREE_LOOTED)
            ...
            if (is_plural(treefruit))
                pline("Некоторые %s падают с дерева!", xname(treefruit));
            else
                pline("%s падает с дерева!", An(xname(treefruit)));
            ...
            if (made)
                pline("Вы привлекли прежних обитателей дерева!");
            else
                You("чувствуете запах застоявшегося мёда.");
    ...
    if (IS_SINK(gm.maploc->typ)) {
        ...
        if (rn2(5)) {
            ...
            if (!Deaf)
                pline("Дзынь!  Трубы шумно вибрируют.");
            else
                pline("Дзынь!");
            exercise(A_DEX, TRUE);
            return ECMD_TIME;
            ...
            if (Blind) {
                if (!Deaf)
                    You_hear("хлюпающий звук.");
            } else {
                pline("Чёрная жижа выходит из стока!");
            }
            ...
            /* can't resist... */
            pline("%s возвращается!",
                  (Blind ? Something : "посудомоечная машина"));
...
dokick(void)
{
    ...
    if (nolimbs(gy.youmonst.data) || slithy(gy.youmonst.data)) {
        You("у вас нет ног, чтобы пинаться.");
        no_kick = TRUE;
    } else if (verysmall(gy.youmonst.data)) {
        You("слишком малы, чтобы пинаться.");
        no_kick = TRUE;
    } else if (u.usteed) {
        if (yn_function("Пнуть ваше верховое животное?", ynchars, 'y',
                        TRUE)
            == 'y') {
            You("пинаете %s.", mon_nam(u.usteed));
            kick_steed();
            return ECMD_TIME;
        } else {
            return ECMD_OK;
        ...
        legs_in_no_shape("пинка", FALSE);
        no_kick = TRUE;
    } else if (near_capacity() > SLT_ENCUMBER) {
        Your("груз слишком тяжёл, чтобы удержать равновесие для пинка.");
        no_kick = TRUE;
    } else if (gy.youmonst.data->mlet == S_LIZARD) {
        Your("ноги не могут пинать эффективно.");
        no_kick = TRUE;
    } else if (u.uinwater && !rn2(2)) {
        Your("медленный пинок не попадает ни во что.");
        no_kick = TRUE;
    } else if (u.utrap) {
        no_kick = TRUE;
        switch (u.utraptype) {
        case TT_PIT:
            if (!Passes_walls)
                pline("Здесь недостаточно места для пинка.");
            else
                no_kick = FALSE;
            break;
        case TT_WEB:
        case TT_BEARTRAP:
            You_cant("двигать ногой!");
            break;
        default:
            break;
        }
    ...
    if (u.uswallow) {
        ...
        case 1:
            if (digests(u.ustuck->data)) {
                pline("%s громко рыгает.", Monnam(u.ustuck));
                break;
            }
            FALLTHROUGH;
            /*FALLTHRU*/
        default:
            Your("слабый пинок не даёт эффекта.");
            break;
        }
        return ECMD_TIME;
    ...
    xint16 dlev)          /* if !0 send to dlev near player */
{
    ...
    if (dct && cansee(x, y)) { /* at least one object fell */
        const char *what = (dct == 1L ? "объект падает"
                                        : "объекты падают");

        if (missile)
            pline(dct == 1L ? "При ударе падает ещё один объект."
                            : dct == oct ? "При ударе падают все объекты."
                                         : "При ударе падают другие объекты.");
        else if (oct == dct)
            pline("%s %s %s.", dct == 1L ? "Один" : "Все", what,
                  gg.gate_str);
        else
            pline("%s %s %s.",
                  dct == 1L ? "Один из" : "Некоторые из",
                  dct == 1L ? "объектов падает" : what, gg.gate_str);
    }

    if (costly && shkp && price) {
        if (ESHK(shkp)->robbed > robbed) {
            You("изъяли товаров на %ld %s!", price, currency(price));
            if (cansee(shkp->mx, shkp->my)) {
                if (ESHK(shkp)->customer[0] == 0)
                    (void) strncpy(ESHK(shkp)->customer, svp.plname, PL_NSIZ);
                if (angry)
                    pline("%s в ярости!", Shknam(shkp));
                else
                    pline("«%s, вы вор!»", svp.plname);
            } else
                You_hear("крик: \"Вор!\"");
            hot_pursuit(shkp);
            (void) angry_guards(FALSE);
        ...
        if (ESHK(shkp)->debit > debit) {
            ...
            You("должны %s %ld %s за потерянные товары.", shkname(shkp), amt,
                currency(amt));
        }
    }
}
...
otransit_msg(struct obj *otmp, boolean nodrop, boolean chainthere, long num)
{
    ...
    if (num || chainthere) {
        ...
        if (num) { /* means: other objects are impacted */
            Sprintf(xbuf, " %s %s объект%s",
                    otense(otmp, "ударяет"),
                    (num == 1L) ? "другой " : "другие ",
                    (num > 1L) ? "ы" : "");
        } else { /* chain-only msg */
            Sprintf(xbuf, " %s вашей цепью", otense(otmp, "гремит"));
        }
        if (nodrop)
            Sprintf(eos(xbuf), ".");
        else
            Sprintf(eos(xbuf), " и %s %s.",
                    otense(otmp, "падает"), gg.gate_str);
        pline("%s%s", obuf, xbuf);
    } else if (!nodrop)
        pline("%s %s %s.", obuf, otense(otmp, "падает"), gg.gate_str);
}
...
down_gate(coordxy x, coordxy y)
{
    ...
    if ((ttmp = t_at(x, y)) != 0 && ttmp->tseen && is_hole(ttmp->ttyp)) {
        gg.gate_str = (ttmp->ttyp == TRAPDOOR) ? "через люк-ловушку"
                                              : "через дыру";
        return MIGR_RANDOM;
    }
    return MIGR_NOWHERE;
