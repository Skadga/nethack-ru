...
inven_inuse(boolean quietly)
{
    ...
    for (otmp = gi.invent; otmp; otmp = otmp2) {
        ...
        if (otmp->in_use) {
            if (!quietly)
                pline("Приканчиваете %s...", xname(otmp));
            useup(otmp);
        }
    }
}
...
restgamestate(NHFILE *nhfp)
{
    ...
    if (SYSOPT_CHECK_SAVE_UID
        ...
        if (!gc.converted_savefile_loaded)
            ...
            pline("Сохранённая игра не была вашей.");
    ...
    if (u.uhp <= 0 && (!Upolyd || u.mh <= 0)) {
        ...
        You("были недостаточно здоровы, чтобы пережить восстановление.");
...
dorecover(NHFILE *nhfp)
{
    ...
    You("возвращаетесь на уровень %d в %s%s.", depth(&u.uz),
        svd.dungeons[u.uz.dnum].dname,
        flags.debug ? " в режиме отладки"
                    : flags.explore ? " в режиме исследования" : "");
    curs(WIN_MAP, 1, 1);
    dotcnt = 0;
    dotrow = 2;
    if (!WINDOWPORT(X11))
...
{
    pline("Странно, эта карта не такая, как я её помню.");
    pline("Кто-то пытается здесь смухлевать...");
    pline("Эта игра недействительна.");
                      ...
                      * and show copyright in menu */
{
    ...
    *svp.plname = '\0';
    ...
    if (saved && *saved) {
        ...
        add_menu_str(tmpwin, "Выберите одну из сохранённых игр");
        ...
        add_menu(tmpwin, &nul_glyphinfo, &any, clet, 'Q', ATR_NONE, clr,
                 "Не важно (выйти)", MENU_ITEMFLAGS_SELECTED);
