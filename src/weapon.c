...
static NEARDATA const char *const odd_skill_names[] = {
    "не обучен", "голые руки", /* use barehands_or_martial[] instead */
    "бой двумя оружиями", "езда верхом", "древковое оружие", "сабля",
    "молот", "кнут",
    "атакующие заклинания", "целебные заклинания",
    "заклинания прорицания",
    "чары", "священные заклинания", "заклинания спасения",
    "заклинания материи",
};
/* indexed via is_martial() */
static NEARDATA const char *const barehands_or_martial[] = {
    "бой голыми руками", "боевые искусства"
};
...
{
    You_feel("больше уверенности в своих навыках %s.",
             (skill == P_NONE) ? "вообще"
                 : (skill <= P_LAST_WEAPON) ? "обращения с оружием"
                     : (skill <= P_LAST_SPELL) ? "колдовства"
                         : "ведения боя");
             ...
             long silverhit)
{
    ...
    if ((silverhit & (W_RINGL | W_RINGR)) != 0L) {
        ...
        Sprintf(rings, "кольц%s", both ? "а" : "о");
        Your("Ваше %s%s %s %s!",
             (l_ag || r_ag) ? "серебряное "
             : both ? ""
               : ((silverhit & W_RINGL) != 0L) ? "левое "
                 : "правое ",
             rings, vtense(rings, "обжигает"), mon_nam(mdef));
    }
}
...
possibly_unwield(struct monst *mon, boolean polyspot)
{
    ...
    if (!attacktype(mon->data, AT_WEAP)) {
        ...
        if (cansee(mon->mx, mon->my)) {
            pline_mon(mon, "%s роняет %s.", Monnam(mon), distant_name(obj, doname));
    ...
    if (obj && obj != &hands_obj) {
        ...
        if (mw_tmp && mwelded(mw_tmp)) {
            if (canseemon(mon)) {
                ...
                Sprintf(welded_buf, "%s к %s %s",
                        otense(mw_tmp, "приваривается"), mhis(mon), mon_hand);

                if (obj->otyp == PICK_AXE) {
                    pline("Поскольку оружие %s %s,", s_suffix(mon_nam(mon)),
                          welded_buf);
                    pline("%s не может применять это %s.", mon_nam(mon),
                          xname(obj));
                } else {
                    pline_mon(mon, "%s пытается применить %s.",
        ...
        if (canseemon(mon)) {
            ...
            pline_mon(mon, "%s применяет %s%c",
                      Monnam(mon), doname(obj),
                      exclaim ? '!' : '.');
            if ((arw = autoreturn_weapon(obj)) != 0 && arw->tethered != 0)
                pline_mon(mon, "%s закрепляет поводок на %s.", Monnam(mon),
                          the(xname(obj)));
            ...
            if (newly_welded) {
                ...
                if (bimanual(obj))
                    mon_hand = makeplural(mon_hand);
                pline("%s к %s %s!", Tobjnam(obj, "приваривается"),
        ...
        if (artifact_light(obj) && !obj->lamplit) {
            ...
            if (canseemon(mon))
                pline("%s %s в %s %s!", Tobjnam(obj, "сверкает"),
            ...
            /* 3.6.3: artifact might be getting wielded by invisible monst */
                ...
                pline("Свет начинает сиять %s.",
                      (mdistu(mon) <= 5 * 5) ? "рядом" : "вдали");
        }
        obj->owornmask = W_WEP;
        return 1;
    }
    ...
    if (newspe > obj->spe) {
        if (verbose) {
            const char *wetness = (newspe < 3)
                                     ? (!obj->spe ? "влажным" : "более влажным")
                                     : (!obj->spe ? "мокрым" : "более мокрым");

            if (carried(obj))
                pline("%s становится %s.", Yobjnam2(obj, (const char *) 0),
                      wetness);
            else if (mcarried(obj) && canseemon(obj->ocarry))
                pline("%s %s становится %s.", s_suffix(Monnam(obj->ocarry)),
                      xname(obj), wetness);
        }
    }
    ...
    boolean verbose)
{
    ...
    if (newspe < obj->spe) {
        if (verbose) {
            if (carried(obj))
                pline("%s высыхает%s.", Yobjnam2(obj, (const char *) 0),
                      !newspe ? " насквозь" : "");
            else if (mcarried(obj) && canseemon(obj->ocarry))
                pline("%s %s высыхает%s.", s_suffix(Monnam(obj->ocarry)),
                      xname(obj), !newspe ? " насквозь" : "");
        }
    }
...
skill_advance(int skill)
{
    ...
    You("теперь %s обучены в %s.",
        P_SKILL(skill) >= P_MAX_SKILL(skill) ? "в высшей степени" : "более",
        P_NAME(skill));
...
static const struct skill_range {
    ...
    { P_FIRST_H_TO_H, P_LAST_H_TO_H, "Навыки боя" },
    { P_FIRST_WEAPON, P_LAST_WEAPON, "Навыки владения оружием" },
    { P_FIRST_SPELL, P_LAST_SPELL, "Навыки колдовства" },
};
...
enhance_weapon_skill(void)
{
    ...
    if (wizard && y_n("Развивать навыки без практики?") == 'y')
        speedy = TRUE;
    ...
    do {
        ...
        if (eventually_advance > 0 || maxxed_cnt > 0) {
            if (eventually_advance > 0) {
                Sprintf(buf,
                        "(Навыки, отмеченные \"*\", можно усилить %s.)",
                        (u.ulevel < MAXULEV)
                            ? "когда станете опытнее"
                            : "когда появятся свободные слоты навыков");
                add_menu_str(win, buf);
            }
            if (maxxed_cnt > 0) {
                Sprintf(buf,
                 "(Навыки, отмеченные \"#\", дальнейшему улучшению не подлежат.)");
                add_menu_str(win, buf);
            }
            add_menu_str(win, "");
        }
        ...
        Strcpy(buf, (to_advance > 0) ? "Выберите навык для улучшения:"
                                     : "Текущие навыки:");
        if (wizard && !speedy)
            Sprintf(eos(buf), "  (%d слотов доступно)", u.weapon_slots);
        end_menu(win, buf);
        ...
        if (n > 0) {
            ...
            for (n = i = 0; i < P_NUM_SKILLS; i++) {
                if (can_advance(i, speedy)) {
                    if (!speedy)
                        You_feel("что могли бы быть более опасным!");
                    n++;
                    break;
                }
            }
...
drain_weapon_skill(int n) /* number of skills to drain */
{
    ...
    for (skill = 0; skill < P_NUM_SKILLS; skill++)
        if (tmpskills[skill]) {
            You("забываете %sсвою подготовку в %s.",
                P_SKILL(skill) >= P_BASIC ? "часть " : "", P_NAME(skill));
        }
}
...
setmnotwielded(struct monst *mon, struct obj *obj)
{
    ...
    if (artifact_light(obj) && obj->lamplit) {
        end_burn(obj, FALSE);
        if (canseemon(mon))
            pline("%s %s в %s %s.", The(xname(obj)),
                  otense(obj, "перестаёт светить"), s_suffix(mon_nam(mon)),
                  mbodypart(mon, HAND));
    }
    if (MON_WEP(mon) == obj)
        MON_NOWEP(mon);
