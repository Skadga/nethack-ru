...
ballrelease(boolean showmsg)
{
    if (carried(uball) && !welded(uball)) {
        if (showmsg)
            pline("Вздрогнув, вы роняете железный шар.");
        if (uwep == uball)
...
#undef CHAIN_IN_MIDDLE
 ...
 drag:
    ...
    if (near_capacity() > SLT_ENCUMBER && dist2(x, y, u.ux, u.uy) <= 2) {
        You("не можете %sтащить тяжёлый железный шар.",
    ...
    if ((is_pool(uchain->ox, uchain->oy)
        ...
        if (Levitation) {
            ...
            You("вас дёргает назад железный шар!");
...
drag_down(void)
{
    ...
    if (carried(uball) && !welded(uball))
        You("теряете хватку за железный шар.");
    ...
    if (forward) {
        if (rn2(6)) {
            pline("Железный шар тащит вас вниз по лестнице!");
        ...
        if (rn2(2)) {
            ...
            pline("Железный шар врезается в вас!");
