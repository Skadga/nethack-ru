...
waterbody_name(coordxy x, coordxy y)
{
    ...
    if (ltyp == LAVAPOOL) {
        Snprintf(pooltype, sizeof pooltype, "расплавленная %s", hliquid("лава"));
        return pooltype;
    } else if (ltyp == ICE) {
        if (!hallucinate)
            return "лёд";
        Snprintf(pooltype, sizeof pooltype, "замёрзшая %s", hliquid("вода"));
        return pooltype;
    } else if (ltyp == POOL) {
        Snprintf(pooltype, sizeof pooltype, "лужа %s", hliquid("воды"));
        return pooltype;
    } else if (ltyp == MOAT) {
        /* a bit of extra flavor over general moat */
        if (hallucinate) {
            Snprintf(pooltype, sizeof pooltype, "глубокая %s", hliquid("вода"));
            return pooltype;
        ...
        if (Is_waterlevel(&u.uz))
            return "безбрежная вода"; /* even if hallucinating */
        Snprintf(pooltype, sizeof pooltype, "стена %s", hliquid("воды"));
        return pooltype;
    } else if (ltyp == LAVAWALL) {
        Snprintf(pooltype, sizeof pooltype, "стена %s", hliquid("лавы"));
        return pooltype;
    }
    /* default; should be unreachable */
    return "вода"; /* don't hallucinate this as some other liquid */
}

/* [RU] accusative form of waterbody_name(), for use after "в ...";
   returns a pointer to a static buffer valid until the next call */
const char *
waterbody_acc(coordxy x, coordxy y)
{
    static const struct {
        const char *nom, *acc;
    } accmap[] = {
        { "расплавленная лава", "расплавленную лаву" },
        { "безбрежная вода",    "безбрежную воду" },
        { "замёрзшая вода",     "замёрзшую воду" },
        { "глубокая вода",      "глубокую воду" },
        { "стена воды",         "стену воды" },
        { "стена лавы",         "стену лавы" },
        { "лужа воды",          "лужу воды" },
        { "мелкое море",        "мелкое море" },
        { "болото",             "болото" },
        { "пруд",               "пруд" },
        { "ров",                "ров" },
        { "лёд",                "лёд" },
    ...
    static char accbuf[40];
    const char *n = waterbody_name(x, y);
    int i;

    for (i = 0; i < SIZE(accmap); ++i)
        if (!strcmp(n, accmap[i].nom)) {
            Strcpy(accbuf, accmap[i].acc);
            return accbuf;
        }
    /* hallucinated or unexpected liquid: leave as-is */
    return n;
}
...
ice_descr(coordxy x, coordxy y, char *outbuf)
{
    static const char *const icetyp[] = {
        "крепкий",    /* 0: not melting */
        "прочный",    /* 1: more than 1000 turns left */
        "устойчивый", /* 2: 101..1000 turns left */
        "зыбкий",     /* 3:  51..100 turns left */
        "тонкий",     /* 4:  15..50 turns left */
        "рыхлый",     /* 5:   1..14 turns left; matches Warning on ice */
    };
...
lookat(coordxy x, coordxy y, char *buf, char *monbuf)
{
    ...
    if (u_at(x, y) && canspotself()
        ...
        if (Underwater && !Is_waterlevel(&u.uz)) {
            ...
            Strcpy(buf, (next2u(x, y)) ? "суша" : "неизвестно");
        } else {
            Strcpy(buf, "неисследованная область");
        }
        ...
        case S_ndoor:
            if (is_drawbridge_wall(x, y) >= 0)
                Strcpy(buf, "открытая решётка подъёмного моста");
            else if ((levl[x][y].doormask & ~D_TRAPPED) == D_BROKEN)
                Strcpy(buf, "сломанная дверь");
            else
                Strcpy(buf, "дверной проём");
            break;
        case S_cloud:
            Strcpy(buf,
                   Is_airlevel(&u.uz) ? "облачная зона" : "туман/облако пара");
            break;
        case S_pool:
    ...
    char *supplemental_name)
{
    ...
    if (!fp) {
        pline("Не удаётся открыть файл 'data'!");
        return res;
    }
            ...
            if (found_in_file) {
                ...
                if (user_typed_name || without_asking || yes_to_moreinfo) {
                    if (dlb_fseek(fp, fseekoffset, SEEK_SET) < 0) {
                        pline("? Ошибка позиционирования в файле 'data'!");
                        goto checkfile_done;
                    }
                    res = TRUE;
                    if (ia_checking)
                ...
                pline("У вас нет никакой информации об этих вещах.");
            }
        }
    }
    ...
    char *out_str)      /* input/output: current description gets appended */
{
    ...
    if (glyph == NO_GLYPH) {
        ...
        if (!strcmp(mbuf, "лужа воды"))
            mbuf[4] = '\0';
        else if (!strcmp(mbuf, "расплавленная лава"))
            Strcpy(mbuf, "лава");
        x_str = mbuf;
    ...
    if (!iflags.terrainmode || (iflags.terrainmode & TER_OBJ) != 0) {
        ...
        for (i = 1; i < MAXOCLASSES; i++) {
            if ((i != ROCK_CLASS)
                ...
                if (i == ROCK_CLASS && !strcmp(oc_ptr, "валун или статуя")) {
                    if (sym == bouldersym)
                        oc_ptr = "валун"; /* discard "or statue" */
                    else if (glyph_is_statue(glyph))
                        oc_ptr = "статуя"; /* discard "boulder or" */
                    else if (looked)
    ...
    if (!clicklook) {
        if (quick) {
            ...
            add_menu(win, &nul_glyphinfo, &any,
                     ...
                     clr, "что-то на карте", MENU_ITEMFLAGS_NONE);
            any.a_char = 'i';
            add_menu(win, &nul_glyphinfo, &any,
                     ...
                     clr, "что-то у тебя с собой", MENU_ITEMFLAGS_NONE);
            any.a_char = '?';
            add_menu(win, &nul_glyphinfo, &any,
                     flags.lootabc ? 0 : any.a_char,
                     flags.lootabc ? '?' : 'n', ATR_NONE,
                     clr, "что-то ещё (по символу или имени)",
                     MENU_ITEMFLAGS_NONE);
            if (!u.uswallow && !Hallucination) {
                ...
                add_menu(win, &nul_glyphinfo, &any,
                         ...
                         clr, "ближние монстры", MENU_ITEMFLAGS_NONE);
                any.a_char = 'M';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : 0, ATR_NONE,
                         clr, "все видимые на карте монстры",
                         MENU_ITEMFLAGS_NONE);
                any.a_char = 'o';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : 0, ATR_NONE,
                         clr, "ближние предметы", MENU_ITEMFLAGS_NONE);
                any.a_char = 'O';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : 0, ATR_NONE,
                         clr, "все видимые на карте предметы",
                         MENU_ITEMFLAGS_NONE);
                any.a_char = 't';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : '^', ATR_NONE,
                         clr, "ближние ловушки", MENU_ITEMFLAGS_NONE);
                any.a_char = 'T';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : '\"', ATR_NONE,
                         clr, "все видимые или запомненные ловушки",
                         MENU_ITEMFLAGS_NONE);
                any.a_char = 'e';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         ...
                         clr, "ближние надписи", MENU_ITEMFLAGS_NONE);
                any.a_char = 'E';
                add_menu(win, &nul_glyphinfo, &any,
                         flags.lootabc ? 0 : any.a_char,
                         flags.lootabc ? any.a_char : '|', ATR_NONE,
                         clr, "все видимые или запомненные надписи",
                         MENU_ITEMFLAGS_NONE);
            }
            end_menu(win, "Что ты хочешь осмотреть:");
 ...
 dowhatiscmd:
    ...
    do {
        ...
        if (from_screen || clicklook) {
            if (from_screen) {
                if (flags.verbose)
                    pline("Пожалуйста, переместите курсор к %s.",
                          what_is_a_location);
                else
                    pline("Выберите %s.", what_is_a_location);
        ...
        if (found) {
            ...
            pline("Я никогда не слышал о таких вещах.");
        }
...
look_traps(boolean nearby)
{
    ...
    if (count)
        ...
        pline("Ловушек не видно и не вспоминается%s.", nearby ? " поблизости" : "");
    destroy_nhwindow(win);
}
...
look_engrs(boolean nearby)
{
    ...
    if (count)
        ...
        pline("Гравировок не видно и не вспоминается%s.", nearby ? " поблизости" : "");
    destroy_nhwindow(win);
}
...
doidtrap(void)
{
    ...
    if (glyph_is_trap(glyph)
        ...
        if (chesttrap || trapped_door_at(tt, x, y)) {
            pline("Это ловушка на %s.", chesttrap ? "сундуке" : "двери");
...
whatdoes_help(void)
{
    ...
    if (!fp) {
        pline("Не удаётся открыть файл данных \"%s\"!", KEYHELP);
...
dowhatdoes_core(char q, char *cbuf)
{
    ...
    if (!fp) {
        pline("Не удаётся открыть файл данных \"%s\"!", CMDHELPFILE);
        return 0;
    }
...
dowhatdoes(void)
{
    ...
    if (reslt) {
        ...
        pline("Нет такой команды '%s', код символа %d (0%03o или 0x%02x).",
...
static const struct {
    ...
    { hmenu_doextversion, "О NetHack (информация о версии)." },
    { dispfile_help, "Полное описание игры и команд." },
    { dispfile_shelp, "Список команд игры." },
    { hmenu_dohistory, "Краткая история NetHack." },
    { hmenu_dowhatis, "Информация о персонаже на экране игры." },
    { hmenu_dowhatdoes, "Информация о назначении данной клавиши." },
    { option_help, "Список опций игры." },
    { dispfile_optionfile, "Более подробное описание опций." },
    { dispfile_optmenu, "Настройка опций командой %s." },
    { dokeylist, "Полный список команд клавиатуры." },
    { hmenu_doextlist, "Список расширенных команд." },
    { domenucontrols, "Список клавиш управления меню." },
    { dispfile_usagehelp, "Описание командной строки NetHack." },
    { dispfile_license, "Лицензия NetHack." },
    { docontact, "Информация о поддержке." },
#ifdef PORT_HELP
    { port_help, "Справка и команды для %s." },
#endif
    { dispfile_debughelp, "Список команд режима волшебника." },
    { (void (*)(void)) 0, (char *) 0 }
};
...
dohelp(void)
{
    ...
    end_menu(tmpwin, "Выберите один пункт:");
