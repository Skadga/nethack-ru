...
{
    You("недостаточно ловки, чтобы дотянуться, сидя на %s.", y_monnam(u.usteed));
}
...
use_saddle(struct obj *otmp)
{
    ...
    if (!u.dx && !u.dy) {
        pline("Оседлать себя?  Очень смешно...");
        return ECMD_OK;
    }
    if (!isok(u.ux + u.dx, u.uy + u.dy)
        || !(mtmp = m_at(u.ux + u.dx, u.uy + u.dy)) || !canspotmon(mtmp)) {
        pline("Я никого там не вижу.");
        return ECMD_TIME;
    }
    ...
    if ((mtmp->misc_worn_check & W_SADDLE) != 0L
        ...
        pline("%s не нужно ещё одно.", Monnam(mtmp));
        return ECMD_TIME;
    }
    ptr = mtmp->data;
    if (touch_petrifies(ptr) && !uarmg && !Stone_resistance) {
        ...
        You("касаетесь %s.", mon_nam(mtmp));
        if (!(poly_when_stoned(gy.youmonst.data) && polymon(PM_STONE_GOLEM))) {
            Sprintf(kbuf, "пытаясь оседлать %s",
                    an(pmname(mtmp->data, Mgender(mtmp))));
            instapetrify(kbuf);
        }
    }
    if (ptr == &mons[PM_AMOROUS_DEMON]) {
        pline("Как вам не стыдно!");
        exercise(A_WIS, FALSE);
        return ECMD_TIME;
    }
    if (mtmp->isminion || mtmp->isshk || mtmp->ispriest || mtmp->isgd
        || mtmp->iswiz) {
        pline("Думаю, %s будет против.", mon_nam(mtmp));
        return ECMD_TIME;
    }
    if (!can_saddle(mtmp)) {
        You_cant("оседлать такое существо.");
        return ECMD_TIME;
    }
    ...
    /* Make the attempt */
    if (rn2(100) < chance) {
        You("надеваете седло на %s.", mon_nam(mtmp));
        if (otmp->owornmask)
...
doride(void)
{
    ...
    if (u.usteed) {
        ...
        if (wizard && y_n("Заставить скакуна подчиниться?") == 'y')
            forcemount = TRUE;
    ...
    boolean force)      /* Quietly force this animal */
{
    ...
    /* Sanity checks */
    if (u.usteed) {
        You("уже едете верхом на %s.", mon_nam(u.usteed));
        return (FALSE);
    }

    /* Is the player in the right form? */
    if (Hallucination && !force) {
        pline("Возможно, вам стоит найти назначенного водителя.");
        return (FALSE);
    }
    ...
    if (Wounded_legs) {
        ...
        legs_in_no_shape("езды", FALSE);
        Sprintf(qbuf, "Вылечить ног%s?",
                ((HWounded_legs & BOTH_SIDES) == BOTH_SIDES) ? "и" : "у");
    ...
    if (Upolyd && (!humanoid(gy.youmonst.data)
        ...
        You("не помещаетесь в седло.");
        return (FALSE);
    }
    if (!force && (near_capacity() > SLT_ENCUMBER)) {
        You_cant("сделать это, неся столько всякого скарба.");
        return (FALSE);
    }
    ...
    if (mtmp->data == &mons[PM_LONG_WORM]
        ...
        You("не можете ехать на %s, не то что на его хвосте.", a_monnam(mtmp));
        return FALSE;
    }
    if (u.uswallow || u.ustuck || u.utrap || Punished
        ...
        if (Punished || !(u.uswallow || u.ustuck || u.utrap))
            You("не можете перекинуть ногу через спину.");
        else
            You("застряли здесь на время.");
        return (FALSE);
    }
    ...
    if (!otmp) {
        pline("%s не оседлан.", Monnam(mtmp));
        return (FALSE);
    }
    ...
    if (touch_petrifies(ptr) && !Stone_resistance) {
        ...
        You("касаетесь %s.", mon_nam(mtmp));
        Sprintf(kbuf, "пытаясь сесть верхом на %s",
                an(pmname(mtmp->data, Mgender(mtmp))));
        instapetrify(kbuf);
    }
    ...
    if (mtmp->mtrapped) {
        ...
        You_cant("сесть на %s, если %s в ловушке.", mon_nam(mtmp),
                 mhe(mtmp));
        return (FALSE);
    }
    ...
    if (!force && !Role_if(PM_KNIGHT) && !(--mtmp->mtame)) {
        ...
        pline("%s %s!", Monnam(mtmp),
              mtmp->mleashed ? "упирается, и поводок соскакивает" : "упирается");
        if (mtmp->mleashed)
            m_unleash(mtmp, FALSE);
        return (FALSE);
    }
    if (!force && Underwater && !is_swimmer(ptr)) {
        You_cant("ехать на этом существе под водой.");
        return (FALSE);
    }
    if (!can_saddle(mtmp) || !can_ride(mtmp)) {
        You_cant("ехать на таком существе.");
        return FALSE;
    }
    ...
    if (!force && !is_floater(ptr) && !is_flyer(ptr) && Levitation
        && !Lev_at_will) {
        You("не можете дотянуться до %s.", mon_nam(mtmp));
        return (FALSE);
    }
    if (!force && uarm && is_metallic(uarm) && greatest_erosion(uarm)) {
        Your("%s доспехи слишком жёсткие, чтобы сесть на %s.",
             uarm->oeroded ? "ржавые" : "коррозированные",
             mon_nam(mtmp));
        return (FALSE);
    }
    if (!force
        ...
        if (Levitation) {
            pline("%s ускользает от вас.", Monnam(mtmp));
            return FALSE;
        }
        You("соскальзываете, пытаясь взобраться на %s.", mon_nam(mtmp));

        Sprintf(buf, "соскользнув с %s при попытке взобраться",
    ...
    if (!force) {
        if (Levitation && !is_floater(ptr) && !is_flyer(ptr))
            ...
            pline("%s магически всплывает!", Monnam(mtmp));
        You("взбираетесь на %s.", mon_nam(mtmp));
        if (Flying)
            You("и %s вместе взмываете в небо.", mon_nam(mtmp));
    }
    ...
    u.usteed = mtmp;
    {
        ...
        if (was_stealthy && !Stealth)
            You("больше не действуете скрытно.");
    }
...
kick_steed(void)
{
    ...
    if (helpless(u.usteed)) {
        ...
        *He = highc(*He);
        if ((u.usteed->mcanmove || u.usteed->mfrozen) && !rn2(2)) {
            ...
            if (helpless(u.usteed))
                pline("%s шевелится.", He);
            else
                /* if hallucinating, might yield "He rouses herself" or
                   "She rouses himself" */
                pline("%s встряхивается!", He);
        } else
            pline("%s не отвечает.", He);
        return;
    }
    ...
    pline("%s галопирует!", Monnam(u.usteed));
    u.ugallop += rn1(20, 30);
    return;
}
    ...
    int reason) /* Player was thrown off etc. */
{
    ...
    case DISMOUNT_FELL:
        You("%s с %s!", verb, mon_nam(mtmp));
        if (!have_spot)
            have_spot = landing_spot(&cc, reason, 1);
        if (!ulev && !ufly) {
            losehp(Maybe_Half_Phys(rn1(10, 10)), "пострадали при верховой езде",
                   KILLED_BY_AN);
        ...
        break;
    case DISMOUNT_POLY:
        You("больше не можете ехать на %s.", mon_nam(u.usteed));
        if (!have_spot)
            have_spot = landing_spot(&cc, reason, 1);
        break;
    ...
    default:
        if (otmp && otmp->cursed) {
            You("не можете.  Седло%s проклято.",
                otmp->bknown ? "" : ", кажется,");
            otmp->bknown = 1; /* ok to skip set_bknown() here */
            return;
        }
        if (!have_spot) {
            You("не можете.  Вам негде встать.");
            return;
        }
        if (!has_mgivenname(mtmp)) {
            pline("Вы прошли подземелье верхом на %s без имени.",
                  an(pmname(mtmp->data, Mgender(mtmp))));
            if (Hallucination)
                pline("Было приятно укрыться от дождя.");
        } else
            You("спешиваетесь с %s.", mon_nam(mtmp));
    }
    ...
    u.ugallop = 0L;
    {
        ...
        if (Stealth && !was_stealthy)
            You("кажетесь менее шумным теперь.");
    }
...
maybewakesteed(struct monst *steed)
{
    ...
    if (wasimmobile && !helpless(steed))
        pline("%s просыпается.", Monnam(steed));
    ...
    struct permonst *oldshape)
{
    if (!can_saddle(steed) || !can_ride(steed)) {
        ...
        if (oldshape != steed->data)
            (void) strsubst(buf, "ваш ", "ваш новый ");
        You("поправляетесь в седле на %s.", buf);
