...
static const char
    are_no_longer_twoweap[] = "больше не используете двойное оружие",
    can_no_longer_twoweap[] = "больше не можете владеть двумя оружиями сразу";
...
setuwep(struct obj *obj)
{
    ...
    if (uwep == obj && artifact_light(olduwep) && olduwep->lamplit) {
        ...
        if (!Blind)
            pline("%s сиять.", Tobjnam(olduwep, "перестаёт"));
    }
    if (uwep == obj
...
cant_wield_corpse(struct obj *obj)
{
    ...
    You("владеете %s голыми %s.",
        corpse_xname(obj, (const char *) 0, CXN_PFX_THE),
        makeplural(body_part(HAND)));
    Sprintf(kbuf, "владеете %s голыми руками", killer_xname(obj));
    instapetrify(kbuf);
    return TRUE;
}
...
empty_handed(void)
{
    return uarmg ? "с пустыми руками" /* gloves imply hands */
...
ready_weapon(struct obj *wep)
{
    ...
    if (!wep) {
        ...
        if (uwep) {
            ...
            You("уже %s.", empty_handed());
    } else if (wep->otyp == CORPSE && cant_wield_corpse(wep)) {
        /* hero must have been life-saved to get here; use a turn */
        res = ECMD_TIME; /* corpse won't be wielded */
    } else if (uarms && bimanual(wep)) {
        You("не можете владеть двуручным %s, нося щит.",
            is_sword(wep) ? "меч" : wep->otyp == BATTLE_AXE ? "топор"
                                                              : "оружие");
        res = ECMD_FAIL;
        ...
        if (will_weld(wep)) {
            ...
            if (strncmp(tmp, thestr, 4) && !strncmp(The(tmp), thestr, 4))
                ...
                tmp = "";
            pline("%s%s %s к вашей %s%s!", tmp, aobjnam(wep, "приваривается"),
                  (wep->quan == 1L) ? "сам" : "сами", /* a3 */
                  bimanual(wep) ? "" :
                      (URIGHTY ? "правой " : "левой "),
        ...
        if (artifact_light(wep) && !wep->lamplit) {
            ...
            if (!Blind)
                pline("%s сиять %s!", Tobjnam(wep, "начинает"),
        ...
        if (Race_if(PM_ELF) && !wep->oartifact
            ...
            You("чувствуете необъяснимое беспокойство при владении холодным железом.");
            change_luck(-1);
        }
#endif
        if (wep->unpaid) {
            ...
            if ((this_shkp = shop_keeper(inside_shop(u.ux, u.uy)))
                != (struct monst *) 0) {
                pline("%s говорит: \"Будь осторожнее с моим %s!\"",
...
dowield(void)
{
    ...
    clear_splitobjs();
    if (!(wep = getobj("владеть", wield_ok, GETOBJ_PROMPT | GETOBJ_ALLOWCNT))) {
        /* Cancelled */
        return ECMD_CANCEL;
    } else if (wep == uwep) {
 already_wielded:
        You("уже владеете этим!");
    ...
    if (wep == &hands_obj) {
        ...
        if (uquiver->quan > 1L && inv_cnt(FALSE) < invlet_basic
                                    && splittable(uquiver)) {
            Sprintf(qbuf, "У вас %ld %s готово. Владеть одним?",
            ...
            Strcpy(qbuf, "Владеть всеми ими вместо этого?");
        } else {
            boolean use_plural = (is_plural(uquiver) || pair_of(uquiver));

            Sprintf(qbuf, "У вас %s готово. Владеть %s вместо этого?",
                    !use_plural ? "тот" : "те",
                    !use_plural ? "его" : "их");
        }
...
doswapweapon(void)
{
    ...
    if (uwep == oldwep) {
        ...
        if (uswapwep)
            ...
            You("нет вторичного оружия наготове.");
    }
...
dowieldquiver(void)
{
    return doquiver_core("приготовить");
}
...
doquiver_core(const char *verb) /* "ready" or "fire" */
{
    ...
    if (!gi.invent) {
        ...
        You("нечего приготовить для стрельбы.");
        return ECMD_OK;
    }
    ...
    if (!newquiver) {
        ...
        /* Explicitly nothing */
        if (uquiver) {
            You("теперь у вас нет приготовленных снарядов.");
            /* skip 'quivering: prinv()' */
            setuqwep((struct obj *) 0);
        } else {
            You("у вас уже нет приготовленных снарядов!");
        }
        return ECMD_OK;
        ...
        if (uquiver && uquiver->o_id == svc.context.objsplit.parent_oid) {
            ...
            You("нельзя приготовить только часть вашего золота.");
            unsplitobj(newquiver);
            return ECMD_OK;
        }
 ...
 already_quivered:
        pline("Эти снаряды уже приготовлены!");
        return ECMD_OK;
    } else if (newquiver->owornmask & (W_ARMOR | W_ACCESSORY | W_SADDLE)) {
        You("не можете %s это!", verb);
        return ECMD_OK;
        ...
        if (uwep->quan > 1L && inv_cnt(FALSE) < invlet_basic
                                    && splittable(uwep)) {
            Sprintf(qbuf, "Вы владеете %ld %s. Приготовить %ld из них?",
            ...
            Strcpy(qbuf, "Приготовить все их вместо этого?");
        } else {
            boolean use_plural = (is_plural(uwep) || pair_of(uwep));

            Sprintf(qbuf, "Вы владеете %s. Приготовить %s вместо этого?",
                    !use_plural ? "тот" : "те",
                    !use_plural ? "его" : "их");
        }
        ...
        if (ynq(qbuf) != 'y') {
            ...
            pline("%s%s %s взято в руки.", qbuf,
                  simpleonames(uwep), otense(uwep, "остаётся"));
            return ECMD_OK;
        }
        ...
        if (uswapwep->quan > 1L && inv_cnt(FALSE) < invlet_basic
            && splittable(uswapwep)) {
            Sprintf(qbuf, "%s %ld %s. Приготовить %ld из них?",
                    u.twoweap ? "Вы владеете двумя оружиями одновременно"
                              : "Ваше запасное оружие —",
            ...
            Sprintf(qbuf, "%s ваше %s оружие. Приготовить %s вместо этого?",
                    !use_plural ? "Это" : "Те",
                    u.twoweap ? "вторичное" : "запасное",
                    !use_plural ? "его" : "их");
        }
        ...
        if (ynq(qbuf) != 'y') {
            ...
            pline("%s%s %s %s.", qbuf,
                  simpleonames(uswapwep), otense(uswapwep, "остаётся"),
                  u.twoweap ? "взято" : "как запасное оружие");
            return ECMD_OK;
        }
 ...
 quivering:
    if (!strcmp(verb, "приготовить")) {
        ...
        prinv("Вы приготовили:", newquiver, 0L);
        setuqwep(newquiver);
    }
           ...
           const char *verb) /* "rub",&c */
{
    ...
    if (obj->owornmask & (W_ARMOR | W_ACCESSORY)) {
        You_cant("%s %s, нося %s.", verb, yname(obj),
                 more_than_1 ? "их" : "его");
        return FALSE;
    }
    if (uwep && welded(uwep)) {
        if (flags.verbose) {
            ...
            if (strstri(what, "pair of ") != 0)
                more_than_1 = FALSE;
            pline(
               "Поскольку ваше оружие приварено к вашей %s, вы не можете %s %s %s.",
                  hand, verb, more_than_1 ? "те" : "тот", xname(obj));
        } else {
            You_cant("сделать это.");
        }
        return FALSE;
    }
    if (cantwield(gy.youmonst.data)) {
        You_cant("крепко держать %s.", more_than_1 ? "их" : "его");
        return FALSE;
    }
    /* check shield */
    if (uarms && bimanual(obj)) {
        You("не можете %s двуручным %s, нося щит.", verb,
            (obj->oclass == WEAPON_CLASS) ? "оружие" : "инструмент");
        return FALSE;
    }
    ...
    if (uswapwep == obj) {
        ...
        if (will_weld(obj)) {
            ...
            You("теперь владеете %s.", doname(obj));
            setuwep(obj);
        }
        if (flags.pushweapon && oldwep && uwep != oldwep)
            setuswapwep(oldwep);
...
can_twoweapon(void)
{
    ...
    if (!could_twoweap(gy.youmonst.data)) {
        if (Upolyd)
            You_cant("использовать два оружия в вашей нынешней форме.");
        else
            pline("%s не способны использовать два оружия сразу.",
        ...
        Your("%s%s %s пусто.", uwep ? "левой " : uswapwep ? "правой " : "",
             hand_s, vtense(hand_s, "are"));
    } else if (!TWOWEAPOK(uwep) || !TWOWEAPOK(uswapwep)) {
        otmp = !TWOWEAPOK(uwep) ? uwep : uswapwep;
        pline("%s %s подходящее %s оружие%s.", Yname2(otmp),
              is_plural(otmp) ? "не" : "не является",
              (otmp == uwep) ? "первичное" : "вторичное",
              plur(otmp->quan));
    } else if (bimanual(uwep) || bimanual(uswapwep)) {
        otmp = bimanual(uwep) ? uwep : uswapwep;
        pline("%s не одноручное.", Yname2(otmp));
    } else if (uarms) {
        You_cant("использовать два оружия, нося щит.");
    } else if (uswapwep->oartifact) {
        pline("%s удерживается вторым у другого оружия!",
              Yobjnam2(uswapwep, "сопротивляется"));
...
drop_uswapwep(void)
{
    ...
    /* this used to use makeplural(body_part(HAND)) but in order to be
       ...
       be the left one */
    Sprintf(left_hand, "левая %s", body_part(HAND));
    if (!obj->cursed)
        /* attempting to two-weapon while Glib */
        pline("%s с вашей %s!", Yobjnam2(obj, "соскальзывает"), left_hand);
    else if (!u.twoweap)
        /* attempting to two-weapon when uswapwep is cursed */
        pline("%s вашу хватку и %s с вашей %s!",
              Yobjnam2(obj, "вырывается"), otense(obj, "падает"), left_hand);
    else
        /* already two-weaponing but can't anymore because uswapwep has
           become cursed */
        Your("%s сводит судорогой и роняет %s!", left_hand, yobjnam(obj, (char *) 0));
    dropx(obj);
}
...
dotwoweapon(void)
{
    ...
    if (u.twoweap) {
        You("переключаетесь на своё первичное оружие.");
    ...
    if (can_twoweapon()) {
        /* Success! */
        You("начинаете бой двумя оружиями.");
...
uwepgone(void)
{
    if (uwep) {
        if (artifact_light(uwep) && uwep->lamplit) {
            end_burn(uwep, FALSE);
            if (!Blind)
                pline("%s сиять.", Tobjnam(uwep, "перестаёт"));
        }
...
chwepon(struct obj *otmp, int amount)
{
    ...
    if (!uwep || (uwep->oclass != WEAPON_CLASS && !is_weptool(uwep))) {
        ...
        if (amount >= 0 && uwep && will_weld(uwep)) { /* cursed tin opener */
            if (!Blind) {
                Sprintf(buf, "%s с %s аурой.",
                        Yobjnam2(uwep, "светится"), an(hcolor(NH_AMBER)));
                uwep->bknown = !Hallucination; /* ok to bypass set_bknown() */
            } else {
                /* cursed tin opener is wielded in right hand */
                Sprintf(buf, "Ваша правая %s покалывает.", body_part(HAND));
            }
            uncurse(uwep);
            update_inventory();
        } else {
            Sprintf(buf, "Ваша %s %s.", makeplural(body_part(HAND)),
                    (amount >= 0) ? "подёргивается" : "чешется");
        }
    ...
    if (uwep->otyp == WORM_TOOTH && amount >= 0) {
        ...
        Your("%s %s гораздо острее теперь.", simpleonames(uwep),
             multiple ? "сливаются и становятся" : "есть");
        uwep->otyp = CRYSKNIFE;
        uwep->oerodeproof = 0;
        if (multiple) {
            uwep->quan = 1L;
        ...
        Your("%s %s гораздо тупее теперь.", simpleonames(uwep),
             multiple ? "сливаются и становятся" : "есть");
    ...
    if (amount < 0 && uwep->oartifact && restrict_name(uwep, wepname)) {
        if (!Blind)
            pline("%s %s.", Yobjnam2(uwep, "слабо светится"), color);
        return 1;
    }
    ...
    if (((uwep->spe > 5 && amount >= 0) || (uwep->spe < -5 && amount < 0))
        && rn2(3)) {
        if (!Blind)
            pline("%s %s некоторое время, а затем %s.",
                  Yobjnam2(uwep, "яростно светится"), color,
                  otense(uwep, "исчезает"));
        else
            pline("%s.", Yobjnam2(uwep, "исчезает"));

        useupall(uwep); /* let all of them disappear */
        return 1;
    }
    if (!Blind) {
        xtime = (amount * amount == 1) ? "мгновение" : "некоторое время";
        pline("%s %s в течение %s.",
              Yobjnam2(uwep, amount == 0 ? "яростно светится" : "светится"), color,
              xtime);
    ...
    if (u_wield_art(ART_MAGICBANE) && uwep->spe >= 0) {
        Your("правая %s дёргается!", body_part(HAND));
    }
    ...
    if ((uwep->spe > 5)
        ...
        pline("%s.", Yobjnam2(uwep, "неожиданно вибрирует"));
...
weldmsg(struct obj *obj)
{
    ...
    pline("%s приварен к вашей %s!", Yobjnam2(obj, "are"), hand);
