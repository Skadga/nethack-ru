...
} worn[] = { { W_ARM, &uarm, "доспех" },
             { W_ARMC, &uarmc, "плащ" },
             { W_ARMH, &uarmh, "шлем" },
             { W_ARMS, &uarms, "щит" },
             { W_ARMG, &uarmg, "перчатки" },
             { W_ARMF, &uarmf, "сапоги" },
             { W_ARMU, &uarmu, "рубаха" },
             { W_RINGL, &uleft, "левое кольцо" },
             { W_RINGR, &uright, "правое кольцо" },
             { W_WEP, &uwep, "оружие" },
             { W_SWAPWEP, &uswapwep, "запасное оружие" },
             { W_QUIVER, &uquiver, "колчан" },
             { W_AMUL, &uamul, "амулет" },
             { W_TOOL, &ublindf, "лицевая повязка" }, /* blindfold|towel|lenses */
             { W_BALL, &uball, "прикованный шар" },
             { W_CHAIN, &uchain, "прикреплённая цепь" },
             { 0, 0, (char *) 0 }
};
    ...
    struct obj *obj)   /* item to make known if effect can be seen */
{
    ...
    if (give_msg && (mon->mspeed != oldspeed || petrify) && mon->data->mmove
        ...
        const char *howmuch =
            (mon->mspeed + oldspeed == MFAST + MSLOW) ? "гораздо " : "";

        if (petrify) {
            /* mimic the player's petrification countdown; "slowing down"
               even if fast movement rate retained via worn speed boots */
            if (flags.verbose)
                pline_mon(mon, "%s замедляется.", Monnam(mon));
        } else if (adjust > 0 || mon->mspeed == MFAST)
            pline_mon(mon, "%s внезапно движется %sбыстрее.",
                      Monnam(mon), howmuch);
        else
            pline_mon(mon, "%s кажется, движется %sмедленнее.",
                      Monnam(mon), howmuch);
                              ...
                              * races (gnomes) can wear suits */
{
 ...
 outer_break:
    ...
    if (!creation) {
        if (sawmon) {
            char buf[BUFSZ], oldarm[BUFSZ], newarm[BUFSZ + sizeof "другой "];
            ...
            if (old) {
                ...
                Snprintf(buf, sizeof buf, " снимает %s и", oldarm);
            } else {
                buf[0] = oldarm[0] = '\0';
            }
            ...
            if (!strcmpi(newarm, oldarm)) {
                ...
                if (!strncmpi(newarm, "", 2))
                    (void) strsubst(newarm, "", "другой ");
                else if (!strncmpi(newarm, "", 3))
                    (void) strsubst(newarm, "", "другой ");
                newarm[BUFSZ - 1] = '\0';
            }
            pline_mon(mon, "%s%s надевает %s.", Monnam(mon), buf, newarm);
            if (autocurse)
                pline("%s %s %s %s на мгновение.", s_suffix(Monnam(mon)),
                      simpleonames(best), otense(best, "светится"),
                      hcolor(NH_BLACK));
        } /* can see it */
    ...
    if (artifact_light(best) && !best->lamplit) {
        ...
        if (!creation && best->lamplit && cansee(mon->mx, mon->my)) {
            ...
            if (sawmon) /* could already see monster */
                pline("%s %s сиять %s.", Yname2(best),
                      otense(best, "начинает"), adesc);
                ...
                pline("%s начинает сиять %s.", Something, adesc);
            else /* didn't see location until new light */
                pline("%s сияет %s.", Something, adesc);
        }
    }
    ...
    if (!creation && (sawmon ^ canseemon(mon))) {
        if (mon->minvis && !See_invisible) {
            pline("Внезапно вы не видите %s.", nambuf);
            makeknown(best->otyp);
        /* } else if (!mon->minvis) {
...
mon_break_armor(struct monst *mon, boolean polyspot)
{
    ...
    if (breakarm(mdat)) {
        if ((otmp = which_armor(mon, W_ARM)) != 0) {
            if ((Is_dragon_scales(otmp) && mdat == Dragon_scales_to_pm(otmp))
                ...
                if (vis)
                    pline_mon(mon, "%s вырывается из %s доспеха!",
                              Monnam(mon), ppronoun);
                else
                    You_hear("звук треска.");
            }
            m_useup(mon, otmp);
        }
        if ((otmp = which_armor(mon, W_ARMC)) != 0
            ...
            if (otmp->oartifact) {
                if (vis)
                    pline_mon(mon, "%s %s падает!", s_suffix(Monnam(mon)),
                          cloak_simple_name(otmp));
                m_lose_armor(mon, otmp, polyspot);
            } else {
                Soundeffect(se_ripping_sound, 100);
                if (vis)
                    pline_mon(mon, "%s %s разрывается!", s_suffix(Monnam(mon)),
                          cloak_simple_name(otmp));
                else
                    You_hear("звук разрыва.");
                m_useup(mon, otmp);
            }
        }
        if ((otmp = which_armor(mon, W_ARMU)) != 0) {
            if (vis)
                pline_mon(mon, "%s рубаха рвётся в клочья!",
                          s_suffix(Monnam(mon)));
            else
                You_hear("звук разрыва.");
            m_useup(mon, otmp);
        ...
        if ((otmp = which_armor(mon, W_ARM)) != 0) {
            ...
            if (vis)
                pline_mon(mon, "%s броня падает вокруг %s!",
        ...
        if ((otmp = which_armor(mon, W_ARMC)) != 0
            ...
            if (vis) {
                if (is_whirly(mon->data))
                    pline_mon(mon, "%s %s падает, без опоры!",
                              s_suffix(Monnam(mon)), cloak_simple_name(otmp));
                else
                    pline_mon(mon, "%s уменьшается, выпадая из %s %s!",
                              Monnam(mon), ppronoun,
                              cloak_simple_name(otmp));
            }
            m_lose_armor(mon, otmp, polyspot);
        ...
        if ((otmp = which_armor(mon, W_ARMU)) != 0) {
            if (vis) {
                if (passes_thru_clothes)
                    pline_mon(mon, "%s просачивается сквозь %s рубаху!",
                              Monnam(mon), ppronoun);
                else
                    pline_mon(mon, "%s становится слишком мал для %s рубахи!",
                          Monnam(mon), ppronoun);
            }
            m_lose_armor(mon, otmp, polyspot);
        }
    ...
    if (handless_or_tiny) {
        ...
        if ((otmp = which_armor(mon, W_ARMG)) != 0) {
            if (vis)
                pline_mon(mon, "%s роняет %s перчатки%s!",
                          Monnam(mon), ppronoun,
                          MON_WEP(mon) ? " и оружие" : "");
            m_lose_armor(mon, otmp, polyspot);
        }
        if ((otmp = which_armor(mon, W_ARMS)) != 0) {
            Soundeffect(se_clank, 50);
            if (vis)
                pline_mon(mon, "%s больше не может удерживать %s щит!",
                          Monnam(mon), ppronoun);
            else
                You_hear("лязг.");
            m_lose_armor(mon, otmp, polyspot);
    ...
    if (handless_or_tiny || has_horns(mdat)) {
        if ((otmp = which_armor(mon, W_ARMH)) != 0
            ...
            if (vis)
                pline_mon(mon, "%s шлем падает на %s!",
    ...
    if (handless_or_tiny || slithy(mdat) || mdat->mlet == S_CENTAUR) {
        if ((otmp = which_armor(mon, W_ARMF)) != 0) {
            if (vis) {
                if (is_whirly(mon->data))
                    pline_mon(mon, "%s сапоги падают!",
                              s_suffix(Monnam(mon)));
                else
                    pline_mon(mon, "%s сапоги %s с %s ног!",
                              s_suffix(Monnam(mon)),
                          verysmall(mdat) ? "соскальзывает" : "отталкиваются", ppronoun);
            }
            m_lose_armor(mon, otmp, polyspot);
        }
    }
    if (!can_saddle(mon)) {
        if ((otmp = which_armor(mon, W_SADDLE)) != 0) {
            m_lose_armor(mon, otmp, polyspot);
            if (vis)
                pline_mon(mon, "%s седло падает.", s_suffix(Monnam(mon)));
        }
        if (mon == u.usteed)
            noride = TRUE;
    }
    if (noride || (mon == u.usteed && !can_ride(mon))) {
        You("больше не может ездить на %s.", mon_nam(mon));
        if (touch_petrifies(u.usteed->data) && !Stone_resistance && rnl(3)) {
            char buf[BUFSZ];

            You("касается %s.", mon_nam(u.usteed));
            Sprintf(buf, "падает с %s",
