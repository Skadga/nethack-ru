...
run_regions(void)
{
    ...
    if (gg.gas_cloud_diss_within) {
        pline("Облако газа вокруг вас рассеивается.");
...
inside_gas_cloud(genericptr_t p1, genericptr_t p2)
{
    ...
    if (!mtmp) { /* hero is indicated by Null rather than by &youmonst */
        ...
        if (!Poison_resistance) {
            ...
            You("кашляете и плюётесь кровью!");
...
region_safety(void)
{
    ...
    if (n > 1 || (n == 1 && !r)) {
        ...
        if (region_danger()) {
            ...
            You_feel("способность дышать.");
        }
    } else if (r) {
        remove_region(r);
        pline("Облако газа, окутывающее вас, рассеивается.");
    } else {
        /* cloud dissipated on its own, so nothing needs to be done */
        pline("Облако газа рассеялось.");
    }
