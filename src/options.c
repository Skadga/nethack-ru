...
static NEARDATA const char *OptS_type[OptS_Advanced+1] = {
    "Общие", "Поведение", "Карта", "Статус", "Дополнительно"
};
...
static NEARDATA const char *unlocktypes[][2] = {
    { "untrap",    "(может потерпеть неудачу)" },
    { "apply-key", "" },
    { "kick",      "(только двери)" },
    { "force",     "(только сундуки/коробки)" },
};
...
static const char n_currently_set[] = "(%d выбрано сейчас)";
...
ask_do_tutorial(void)
{
    ...
    if (!opt_set_in_config[opt_tutorial]) {
        ...
        Snprintf(buf, sizeof buf,
                 "Добавьте \"OPTIONS=!tutorial\" в %s чтобы пропустить этот вопрос.",
                 (rc && *rc && !norc) ? rc : "ваш конфигурационный файл");
        do {
            ...
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0,
                     ATR_NONE, NO_COLOR,
                     "Да, провести обучение", MENU_ITEMFLAGS_NONE);
            any.a_char = 'n';
            add_menu(win, &nul_glyphinfo, &any, any.a_char, 0,
                     ATR_NONE, NO_COLOR,
                     "Нет, просто начать игру", MENU_ITEMFLAGS_NONE);

            add_menu_str(win, "");
            add_menu_str(win, buf);
            if (pass++) /* we'll get here after <space> or <return> */
                add_menu_str(win, "(Пожалуйста, нажмите 'y' или 'n'.)");

            end_menu(win, "Вы хотите пройти обучение?");
    ...
    char *opts, char *op)
{
 ...
 goodfruit:
        ...
        if (!go.opt_initial) {
            ...
            if (give_opt_msg)
                pline("Фрукт теперь \"%s\".", svp.pl_fruit);
        }
    ...
    if (req == get_val || req == get_cnf_val) {
        ...
        if (req == get_val)
            Strcpy(opts, count_status_hilites()
                     ? "(см. \"правила подсветки статуса\" ниже)"
                     : "(none)");
#endif
        return optn_ok;
    }
    ...
    if (req == do_set) {
        ...
        if (op == empty_optstr) {
            ...
            if (flags.menu_style == MENU_TRADITIONAL
                ...
                use_menu = FALSE;
                Sprintf(qbuf, "Новое %s: [%s am] (%s)", allopt[optidx].name,
                        ocl, *tbuf ? tbuf : "всё");
                abuf[0] = '\0';
                getlin(qbuf, abuf);
            ...
            if (use_menu) {
                ...
                (void) choose_classes_menu("Что подбирать?",
                                           1, TRUE, ocl, tbuf);
                op = tbuf;
            }
        }
    ...
    if (req == do_set) {
        ...
        *(allopt[optidx].addr) = !negated;    /* <==== SET IT HERE */
        ...
        case opt_idlecheckpoint:
            pline("Для 'idlecheckpoint' нет базовой поддержки,"
                  " скомпилированной в игру."); 
        ...
        if (give_opt_msg)
            pline("Опция '%s' переключена %s.", allopt[optidx].name,
                  !negated ? "вкл" : "выкл");
...
handler_menustyle(void)
{
    ...
    end_menu(tmpwin, "Выберите стиль меню:");
    ...
    if (chngd || flags.verbose)
        pline("'menustyle' %s \"%s\".", chngd ? "изменено на" : "по-прежнему",
...
handler_align_misc(int optidx)
{
    ...
    any = cg.zeroany;
    any.a_int = ALIGN_TOP;
    add_menu(tmpwin, &nul_glyphinfo, &any, 't', 0, ATR_NONE, clr, "сверху",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_BOTTOM;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'b', 0, ATR_NONE, clr, "снизу",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_LEFT;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'l', 0, ATR_NONE, clr, "слева",
             MENU_ITEMFLAGS_NONE);
    any.a_int = ALIGN_RIGHT;
    add_menu(tmpwin, &nul_glyphinfo, &any, 'r', 0, ATR_NONE, clr, "справа",
             MENU_ITEMFLAGS_NONE);
    Sprintf(abuf, "Выберите расположение %s относительно карты:",
            (optidx == opt_align_message)
                ? "окна сообщений"
                : "окна статуса");
...
handler_autounlock(int optidx)
{
    ...
    Sprintf(buf, "Выберите действия для '%.20s':", optname);
    end_menu(tmpwin, buf);
    ...
    if ((chngd || flags.verbose) && give_opt_msg) {
        ...
        pline("'%s' %s '%s'.", optname,
              chngd ? "изменено на" : "по-прежнему", buf);
    }
    return res;
}
...
handler_msg_window(void)
{
    ...
    if (is_tty || is_curses) {
        ...
        end_menu(tmpwin, "Выберите способ показа журнала сообщений:");
        ...
        pline("Опция '%s' не поддерживается для '%s'.",
...
handler_number_pad(void)
{
    ...
    static const char *const npchoices[] = {
        " 0 (выкл)", " 1 (вкл)", " 2 (вкл, совместимо с MSDOS)",
        " 3 (вкл, телефонная раскладка)",
        " 4 (вкл, телефонная раскладка, совместимо с MSDOS)",
        "-1 (выкл, 'z' для движения вверх-влево, 'y' для палочки)"
    };
    ...
    end_menu(tmpwin, "Выберите режим цифровой клавиатуры:");
...
handler_perminv_mode(void)
{
    ...
    if (n >= 0) { /* not ESC */
        ...
        pline("'perminv_mode' %s '%s' (%s).",
              (new_pi != old_pi) ? "изменено на" : "по-прежнему",
...
handler_pickup_burden(void)
{
    ...
    end_menu(tmpwin, "Выберите уровень нагрузки:");
...
handler_runmode(void)
{
    ...
    end_menu(tmpwin, "Выберите режим отображения бега/перемещения:");
    if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
...
{
    int tmp
        = query_attr("Выберите атрибут выделения питомца", iflags.wc2_petattr);
...
handler_sortloot(void)
{
    ...
    end_menu(tmpwin, "Выберите вид сортировки добычи:");
...
handler_whatis_coord(void)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, GPCOORDS_NONE,
             0, ATR_NONE, clr, "ничего (координаты не показываются)",
             (gpc == GPCOORDS_NONE)
                ? MENU_ITEMFLAGS_SELECTED : MENU_ITEMFLAGS_NONE);
    add_menu_str(tmpwin, "");
    Sprintf(buf, "карта: верхний-левый: <%d,%d>, lower-right: <%d,%d>%s",
            1, 0, COLNO - 1, ROWNO - 1,
            flags.verbose ? "; столбец 0 не используется, вне левого края" : "");
    add_menu_str(tmpwin, buf);
    if (strcmp(windowprocs.name, "tty")) /* only show for non-tty */
        add_menu_str(tmpwin,
      "экран: строка смещена из-за верхней строки tty");
#if COLNO == 80
#define COL80ARG flags.verbose ? "; столбец 80 не используется" : ""
#else
#define COL80ARG ""
#endif
    Sprintf(buf, "экран: верхний-левый: [%02d,%02d], lower-right: [%d,%d]%s",
            0 + 2, 1, ROWNO - 1 + 2, COLNO - 1, COL80ARG);
#undef COL80ARG
    add_menu_str(tmpwin, buf);
    add_menu_str(tmpwin, "");
    end_menu(tmpwin,
        "Выберите способ показа координат позиции карты при авто-описании:");
...
handler_whatis_filter(void)
{
    ...
    add_menu(tmpwin, &nul_glyphinfo, &any, 'n',
             0, ATR_NONE, clr, "без фильтра",
             (gfilt == GFILTER_NONE)
                ? MENU_ITEMFLAGS_SELECTED : MENU_ITEMFLAGS_NONE);
    any.a_char = (GFILTER_VIEW + 1);
    add_menu(tmpwin, &nul_glyphinfo, &any, 'v',
             0, ATR_NONE, clr, "только в пределах видимости",
             (gfilt == GFILTER_VIEW)
                ? MENU_ITEMFLAGS_SELECTED : MENU_ITEMFLAGS_NONE);
    any.a_char = (GFILTER_AREA + 1);
    add_menu(tmpwin, &nul_glyphinfo, &any, 'a',
             0, ATR_NONE, clr, "в той же области",
             (gfilt == GFILTER_AREA)
                ? MENU_ITEMFLAGS_SELECTED : MENU_ITEMFLAGS_NONE);
    end_menu(tmpwin,
      "Выберите фильтр позиции при переходе к следующей/предыдущей метке:");
    if ((pick_cnt = select_menu(tmpwin, PICK_ONE, &window_pick)) > 0) {
...
handler_autopickup_exception(void)
{
 ...
 ape_again:
    ...
    if (opt_idx == 3) { /* done */
        ...
        Sprintf(apebuf, "%s исключения автоподбора",
                (opt_idx == 1) ? "Список" : "Какие убрать");
...
handler_menu_colors(void)
{
 ...
 menucolors_done:
        ...
        if (*mcbuf
            ...
            pline("Ошибка добавления цвета меню.");
            wait_synch();
        }
 ...
 msgtypes_again:
    ...
    if (opt_idx == 3) { /* done */
        ...
        if (*mtbuf
            ...
            pline("Ошибка добавления типа сообщения.");
            wait_synch();
        }
        goto msgtypes_again;
    } else { /* list (1) or remove (2) */
...
handler_versinfo(void)
{
    ...
    end_menu(tmpwin, "Выберите флаги информации о версии:");
...
handler_windowborders(void)
{
    ...
    static const char *const windowborders_text[] = {
        "Выкл: никогда не показывать рамки",
        "Вкл: всегда показывать рамки",
        "Авто: вкл, если размер дисплея не менее (24+2)x(80+2)",
        "Вкл, кроме принудительного выкл для perm_invent",
        "Авто, кроме принудительного выкл для perm_invent"
    };
    ...
    end_menu(tmpwin, "Выберите режим рамок окон:");
    if (select_menu(tmpwin, PICK_ONE, &mode_pick) > 0) {
...
{
#ifdef MICRO
    pline("\"%s\" настраивается только из %s.", optname, get_configfile());
#else
    pline("%s можно задать только из NETHACKOPTIONS или %s.", optname,
          get_configfile());
#endif
}
...
add_menu_cmd_alias(char from_ch, char to_ch)
{
    if (gn.n_menu_mapped >= MAX_MENU_MAPPED_CMDS) {
        pline("не хватает места в карте меню.");
    } else {
...
staticfn const char *opt_dispname(const char *);
...
doset_simple_menu(void)
{
 ...
 redo_opt_help:
    ...
    if (gs.simple_options_help) {
        ...
        Strcpy(buf, "Используй команду '#optionsfull',"
                    " чтобы получить полный список опций.");
        add_menu_str(tmpwin, buf);
    }
    any = cg.zeroany;
    any.a_int = -2 + 1;
    add_menu(tmpwin, &nul_glyphinfo, &any, '?', 0, ATR_NONE, NO_COLOR,
             gs.simple_options_help ? "скрыть справку" : "показать справку",
             MENU_ITEMFLAGS_NONE);
        ...
        for (i = 0; (name = allopt[i].name) != 0; i++) {
            ...
            case BoolOpt:
                ...
                if (iflags.wc_tiled_map && allopt[i].idx == opt_color)
                    continue;
                Sprintf(buf, fmtstr, opt_dispname(name), *bool_p ? "X" : " ");
                break;
            case CompOpt:
            case OthrOpt:
                    k = i;
                    ...
                    Sprintf(buf, fmtstr, opt_dispname(name),
                            ((reslt == optn_ok && buf2[0])
                             ? (const char *) buf2 : "неизвестно"));
                break;
            default:
                Sprintf(buf, "ERROR");
                break;
    ...
    if (pick_cnt > 0) {
        ...
        if (k == -2) {
            ...
            if (allopt[k].has_handler && allopt[k].optfn) {
                ...
                Sprintf(buf, "Введите значение для %s:", opt_dispname(allopt[k].name));
                getlin(buf, abuf);
                if (abuf[0] != '\033') { /* ESC */
...
term_for_boolean(int idx, boolean *b)
{
    ...
    static const char *const booleanterms[2][num_terms] = {
        { "нет", "выкл", "отключено", "вырезано из сборки" },
        { "да", "вкл", "включено", "included"},
    };
...
/* Russian display names for option menu entries; the internal names used for parsing config files and #saveoptions output are deliberately left untouched. */
staticfn const char *
opt_dispname(const char *nam)
{
    static const char *const ru_opts[][2] = {
        { "accessiblemsg", "нумерация сообщений" },
        { "acoustics", "акустика" },
        { "altmeta", "альт-мета" },
        { "armorstatus", "состояние доспехов" },
        { "ascii_map", "ASCII-карта" },
        { "autodescribe", "автоописание" },
        { "autodig", "автокопание" },
        { "autoopen", "автооткрывание" },
        { "autopickup", "автоподбор" },
        { "autoquiver", "автоколчан" },
        { "bgcolors", "фоновые цвета" },
        { "BIOS", "BIOS" },
        { "blind", "слепота" },
        { "bones", "костяшки" },
        { "checkpoint", "контрольная точка" },
        { "cmdassist", "подсказка команд" },
        { "color", "цвет" },
        { "confirm", "подтверждение" },
        { "customcolors", "собственные цвета" },
        { "customsymbols", "собственные символы" },
        { "dark_room", "тёмная комната" },
        { "deaf", "глухота" },
        { "debug_hunger", "отладочный голод" },
        { "debug_mongen", "отладка монстров" },
        { "debug_overwrite_stairs", "отладка лестниц" },
        { "dropped_nopick", "подбирать выброшенное" },
        { "eight_bit_tty", "восьмибитный ввод" },
        { "extmenu", "расширенное меню" },
        { "female", "женский род" },
        { "fireassist", "помощь при стрельбе" },
        { "fixinv", "фикс. буквы инвентаря" },
        { "force_invmenu", "всегда меню инвентаря" },
        { "fullscreen", "полный экран" },
        { "goldX", "символ золота" },
        { "guicolor", "цвет интерфейса" },
        { "help", "справка" },
        { "herecmd_menu", "меню действий рядом" },
        { "hilite_pet", "подсветка питомца" },
        { "hilite_pile", "подсветка кучи" },
        { "hitpointbar", "полоса хитов" },
        { "idlecheckpoint", "контрольная точка ожидания" },
        { "ignintr", "игнор прерываний" },
        { "implicit_uncursed", "подразумевать незаклятие" },
        { "large_font", "крупный шрифт" },
        { "legacy", "старый стиль" },
        { "lit_corridor", "светящиеся коридоры" },
        { "lootabc", "лут по алфавиту" },
        { "mail", "почта" },
        { "mention_decor", "упоминать декор" },
        { "mention_map", "упоминать карту" },
        { "mention_walls", "упоминать стены" },
        { "menu_overlay", "наложение меню" },
        { "menu_tab_sep", "табы в меню" },
        { "menucolors", "цвета меню" },
        { "mon_movement", "движение монстров" },
        { "monpolycontrol", "управление полиморфом" },
        { "montelecontrol", "управление телепортом" },
        { "news", "новости" },
        { "nudist", "нудизм" },
        { "null", "пустая опция" },
        { "pauper", "нищета" },
        { "perm_invent", "всегда показ инвентаря" },
        { "pickup_stolen", "подбирать украденное" },
        { "pickup_thrown", "подбирать брошенное" },
        { "popup_dialog", "всплывающее окно" },
        { "preload_tiles", "предзагрузка плиток" },
        { "price_quotes", "разговоры о цене" },
        { "pushweapon", "спрятать оружие" },
        { "query_menu", "меню с выбором" },
        { "quick_farsight", "быстрый осмотр" },
        { "rawio", "сырой ввод" },
        { "reroll", "пересоздать героя" },
        { "rest_on_space", "отдых на пробеле" },
        { "safe_pet", "беречь питомца" },
        { "safe_wait", "безопасное ожидание" },
        { "sanity_check", "проверка целостности" },
        { "selectsaved", "выбор сохранения" },
        { "showdamage", "показывать урон" },
        { "showexp", "показывать опыт" },
        { "showrace", "показывать расу" },
        { "showscore", "показывать счёт" },
        { "showvers", "показывать версию" },
        { "silent", "без звука" },
        { "softkeyboard", "экранная клавиатура" },
        { "sortpack", "сортировка рюкзака" },
        { "sounds", "звук" },
        { "sparkle", "мерцание" },
        { "spot_monsters", "подмечать монстров" },
        { "splash_screen", "заставка" },
        { "standout", "выделение" },
        { "status_updates", "обновление статуса" },
        { "terrainstatus", "рельеф в статусе" },
        { "tiled_map", "плиточная карта" },
        { "time", "время" },
        { "timed_delay", "задержка" },
        { "tips", "советы" },
        { "tombstone", "надгробие" },
        { "toptenwin", "окно рекордов" },
        { "travel", "путешествие" },
        { "travel_debug", "отладка путешествия" },
        { "tutorial", "обучение" },
        { "use_darkgray", "тёмно-серый" },
        { "use_inverse", "инверсия" },
        { "use_truecolor", "истинный цвет" },
        { "verbose", "подробности" },
        { "voices", "голоса" },
        { "vt_tiledata", "плитки VGA" },
        { "vt_sounddata", "звуки VGA" },
        { "weaponstatus", "состояние оружия" },
        { "whatis_menu", "меню whatis" },
        { "whatis_moveskip", "whatis без хода" },
        { "wizmgender", "пол монстров" },
        { "wizweight", "вес монстров" },
        { "wraptext", "перенос текста" },
        { "windowtype", "оконная система" },
        { "playmode", "режим игры" },
        { "name", "имя" },
        { "role", "роль" },
        { "race", "раса" },
        ...
        { "align_message", "сообщение о мировоззрении" },
        { "align_status", "мировоззрение в статусе" },
        { "altkeyhandling", "обработка Alt-клавиш" },
        { "autounlock", "автовзлом" },
        { "boulder", "валун" },
        { "catname", "имя кота" },
        { "crash_email", "email для краша" },
        { "crash_name", "имя для краша" },
        { "crash_urlmax", "ссылка при краше" },
        { "cursesgraphics", "графика curses" },
        { "DECgraphics", "графика DEC" },
        { "disclose", "раскрытие по смерти" },
        { "dogname", "имя собаки" },
        { "dungeon", "подземелье" },
        { "effects", "эффекты" },
        { "font_map", "шрифт карты" },
        { "font_menu", "шрифт меню" },
        { "font_message", "шрифт сообщений" },
        { "font_size_map", "размер шрифта карты" },
        { "font_size_menu", "размер шрифта меню" },
        { "font_size_message", "размер шрифта сообщений" },
        { "font_size_status", "размер шрифта статуса" },
        { "font_size_text", "размер шрифта текста" },
        { "font_status", "шрифт статуса" },
        { "font_text", "шрифт текста" },
        { "fruit", "фрукт" },
        { "glyph", "глиф" },
        { "hicolor", "большой цвет" },
        { "hilite_status", "подсветка статуса" },
        { "horsename", "имя лошади" },
        { "IBMgraphics", "графика IBM" },
        { "map_mode", "режим карты" },
        { "menu_deselect_all", "меню: снять всё" },
        { "menu_deselect_page", "меню: снять страницу" },
        { "menu_first_page", "меню: первая страница" },
        { "menu_headings", "заголовки меню" },
        { "menu_invert_all", "меню: инверт. всё" },
        { "menu_invert_page", "меню: инверт. страницу" },
        { "menu_last_page", "меню: последняя страница" },
        { "menu_next_page", "меню: следующая страница" },
        { "menu_objsyms", "меню: символы объектов" },
        { "menu_previous_page", "меню: предыдущая страница" },
        { "menu_search", "меню: поиск" },
        { "menu_select_all", "меню: выбрать всё" },
        { "menu_select_page", "меню: выбрать страницу" },
        { "menu_shift_left", "меню: сдвиг влево" },
        { "menu_shift_right", "меню: сдвиг вправо" },
        { "menuinvertmode", "режим инверсии меню" },
        { "menustyle", "стиль меню" },
        { "monsters", "монстры" },
        { "mouse_support", "поддержка мыши" },
        { "msg_window", "окно сообщений" },
        { "msghistory", "история сообщений" },
        { "number_pad", "цифровая клавиатура" },
        { "objects", "объекты" },
        { "packorder", "порядок в рюкзаке" },
        { "palette", "палитра" },
        { "paranoid_confirmation", "параноидное подтверждение" },
        { "perminv_mode", "режим постоянного инвентаря" },
        { "petattr", "атрибуты питомца" },
        { "pettype", "тип питомца" },
        { "pickup_burden", "подбор при грузе" },
        { "pickup_types", "типы подбора" },
        { "pile_limit", "лимит кучи" },
        { "player_selection", "выбор игрока" },
        { "roguesymset", "символы уровня Rogue" },
        { "runmode", "режим движения" },
        { "scores", "рекорды" },
        { "scroll_amount", "величина прокрутки" },
        { "scroll_margin", "поля прокрутки" },
        { "sortdiscoveries", "сортировка открытий" },
        { "sortloot", "сортировка лута" },
        { "sortvanquished", "сортировка побед" },
        { "soundlib", "библиотека звука" },
        { "statushilites", "подсветка статуса" },
        { "statuslines", "строки статуса" },
        { "subkeyvalue", "значение подклавиши" },
        { "suppress_alert", "подавление предупреждений" },
        { "symset", "набор символов" },
        { "term_cols", "столбцы терминала" },
        { "term_rows", "строки терминала" },
        { "tile_file", "файл плиток" },
        { "tile_height", "высота плитки" },
        { "tile_width", "ширина плитки" },
        { "traps", "ловушки" },
        { "vary_msgcount", "счёт сообщений" },
        { "versinfo", "информация о версии" },
        { "video", "видео" },
        { "videocolors", "видеоцвета" },
        { "videoshades", "видеотени" },
        { "video_width", "ширина видео" },
        { "video_height", "высота видео" },
        { "warnings", "предупреждения" },
        { "whatis_coord", "whatis: координаты" },
        { "whatis_filter", "whatis: фильтр" },
        { "windowborders", "рамки окон" },
        { "windowchain", "цепочка окон" },
        { "windowcolors", "цвета окон" },
        { "cond_", "состояния" },
        { "font", "шрифт" },
        { "IBM_", "IBM-графика" },
        { "autocompletions", "автодополнения" },
        { "autopickup exceptions", "исключения автоподбора" },
        { "bind keys", "назначение клавиш" },
        { "menu colors", "цвета меню" },
        { "message types", "типы сообщений" },
        { "status condition fields", "поля состояний статуса" },
        { "status highlight rules", "правила подсветки статуса" },
        { (const char *) 0, (const char *) 0 }
    };
    int i;

    for (i = 0; ru_opts[i][0]; i++)
        if (!strcmp(nam, ru_opts[i][0]))
            return ru_opts[i][1];
    return nam;
}
 ...
 rerun:
    ...
    if (!skiphelp) {
        ...
        static const char *const helptext[] = {
            "Для краткого пояснения, как это работает, нажмите '?', чтобы выбрать",
            "следующий пункт меню, а затем нажмите <enter> или <return>.",
            NULL, /* actual '?' menu entry gets inserted here */
            ("[Чтобы скрыть эту подсказку меню,"
             " отключите опцию 'cmdassist'.]"),
            "",
        };
        any = cg.zeroany;
    ...
    indexoffset = 1;
    any = cg.zeroany;
    add_menu_heading(tmpwin, "Логические параметры (выбор переключает значение):");
    any.a_int = 0;
        ...
        for (i = 0; (name = allopt[i].name) != 0; i++)
            if (allopt[i].opttyp == BoolOpt
                ...
                Sprintf(buf, fmtstr_doset, indent,
                        opt_dispname(name), term_for_boolean(i, bool_p));
                if (pass == 0)
    ...
    add_menu_str(tmpwin, "");
    add_menu_heading(tmpwin,
                     "Составные параметры (выбор предложит ввести новое значение):");
    ...
    add_menu_str(tmpwin, "");
    add_menu_heading(tmpwin, "Прочие настройки:");
    ...
    add_menu_str(tmpwin, "");
    add_menu_heading(tmpwin, "Изменяемые пути расположения:");
    for (i = 0; i < PREFIX_COUNT; i++)
        doset_add_menu(tmpwin, fqn_prefix_names[i], fmtstr_doset, -1, 0);
#endif
    end_menu(tmpwin, "Какие опции задать?");
    go.opt_need_redraw = FALSE;
    go.opt_need_glyph_reset = FALSE;
        ...
        for (pick_idx = 0; pick_idx < pick_cnt; ++pick_idx) {
            ...
            if (allopt[opt_indx].opttyp == BoolOpt) {
                ...
                if (allopt[k].has_handler && allopt[k].optfn) {
                    ...
                    Sprintf(buf, "Введите значение для %s:", opt_dispname(allopt[opt_indx].name));
                    abuf[0] = '\0';
                    getlin(buf, abuf);
                    if (abuf[0] == '\033')
                        continue;
...
{
    const char *value = "неизвестно"; /* current value */
    const char *indent;
...
#endif
        if (!buf2[0])
            Strcpy(buf2, "неизвестно");
        value = (const char *) buf2;
    }

    /* "    " replaces "a - " -- assumes menus follow that style */
    indent = !any.a_int ? "    " : "";
    Sprintf(buf, fmtstr, indent, opt_dispname(option), value);
...
handle_add_list_remove(const char *optname, int numtotal)
{
    ...
    static const struct action {
        ...
        { 'a', "добавить новое правило" },         /* [0] */
        { 'l', "показать список правил" },            /* [1] */
        { 'r', "удалить существующее правило" }, /* [2] */
        { 'x', "выйти из этого меню" },     /* [3] */
    };
    int clr = NO_COLOR;
...
static const char *opt_intro[] = {
    "",
    "                 Справка по опциям NetHack:", "",
#define CONFIG_SLOT 3 /* fill in next value at run-time */
    (char *) 0,
#if !defined(MICRO) && !defined(MACOS9)
    "или задайте переменную окружения NETHACKOPTIONS=\"<options>\"",
#endif
    "(<options> — список опций через запятую)",
#ifdef VMS
    "-- for example, $ DEFINE NETHACKOPTIONS \"noautopickup,fruit:kumquat\"",
#endif
    "или, играя, нажмите \"O\" и воспользуйтесь меню.",
    "",
    ("Логические опции (отменяются, если перед ними поставить"
     " '!' или \"no\"):"),
    (char *) 0
};
...
static const char *const opt_epilog[] = {
    "",
    "Некоторые опции можно задать только до начала игры;",
    "эти пункты будут недоступны в меню команды 'O'.",
    "Некоторые опции хранятся в файле сохранения игры и сохранят",
    "значения при восстановлении игры, даже если вы обновили свой файл",
    "конфигурации. Такие изменения повлияют на новые игры.",
    "«Прочие настройки» задаются через 'O', но при указании",
    "в конфигурационном файле для них используются свои директивы, а не OPTIONS.",
    "Подробности — в «Руководстве» (Guidebook) NetHack.",
    (char *) 0
};
...
option_help(void)
{
    ...
    Snprintf(buf, sizeof buf,
             "Задайте опции как OPTIONS=<options> в %s", get_configfile());
    ...
    for (i = 0; allopt[i].name; i++) {
        ...
        if ((is_wc_option(optname) && !wc_supported(optname))
            ...
            continue;
        next_opt(datawin, opt_dispname(optname));
    }
    next_opt(datawin, "");

    /* Compound options */
    putstr(datawin, 0, "Составные опции:");
    for (i = 0; allopt[i].name; i++) {
        if (allopt[i].opttyp != CompOpt
            || (allopt[i].setwhere == set_wizonly && !wizard))
            continue;
    ...
    /* Compound options */
    putstr(datawin, 0, "Прочие настройки:");
    for (i = 0; allopt[i].name; i++) {
        if (allopt[i].opttyp != OthrOpt)
            continue;
