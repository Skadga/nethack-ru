...
static const char brief_feeling[] =
    "на мгновение возникает %s чувство, затем оно проходит.";
    ...
    if (negated) {
        ...
        if (verbosely) {
            if (mdef == &gy.youmonst)
                ...
                pline_mon(mdef, "%s избегает вреда.", Monnam(mdef));
        }
        return TRUE;
    }
    return FALSE;
...
dynamic_multi_reason(struct monst *mon, const char *verb, boolean by_gaze)
{
    ...
    p = eos(p);
    if (by_gaze)
        Sprintf(p, "%s взглядом %s", verb, who);
    else
        Sprintf(p, "%s %s", verb, who);
    gm.multi_reason = p;
}
    ...
    struct obj *wep)    /* uwep for do_attack(), null for kick_monster() */
{
    ...
    if (!canspotmon(mtmp)
        ...
        pline("Подождите! Там есть %s, которого вы не видите!", something);
        map_invisible(gb.bhitpos.x, gb.bhitpos.y);
    ...
    if (mtmp->mundetected && !canseemon(mtmp)
        ...
        if (!tp_sensemon(mtmp) && !Detect_monsters) {
            ...
            if (!Blind && Hallucination)
                pline("%s %s %s!", mtmp->mtame ? "Одомашненное" : "Дикое",
                      notseen ? "существо" : (const char *) lmonbuf,
                      notseen ? "присутствует" : "появляется");
            else if (Blind || (is_pool(mtmp->mx, mtmp->my) && !Underwater))
                pline("Подождите! Там спрятался монстр!");
            else if ((obj = svl.level.objects[mtmp->mx][mtmp->my]) != 0)
                pline("Подождите! Там %s прячется под %s!",
    ...
    if (flags.confirm && mtmp->mpeaceful
        ...
        if (canspotmon(mtmp)) {
            ...
            Sprintf(qbuf, "Действительно атаковать %s?", mon_nam(mtmp));
...
check_caitiff(struct monst *mtmp)
{
    ...
    if (Role_if(PM_KNIGHT) && u.ualign.type == A_LAWFUL
        ...
        You("бесчестно атакуете невиновного!");
        adjalign(-1);
    }
}
...
do_attack(struct monst *mtmp)
{
    ...
    if (is_safemon(mtmp) && !svc.context.forcefight) {
        if (!u_wield_art(ART_STORMBRINGER)) {
            ...
            if (inshop || foo) {
                ...
                buf[0] = highc(buf[0]);
                You("останавливаетесь. %s стоит на пути!", buf);
                end_running(TRUE);
                return TRUE;
            } else if (mtmp->mfrozen || helpless(mtmp)
                       || (mtmp->data->mmove == 0 && rn2(6))) {
                pline("%s, кажется, не двигается!", Monnam(mtmp));
                end_running(TRUE);
                return TRUE;
            } else
                return FALSE;
    ...
    if (Upolyd && noattacks(gy.youmonst.data)) {
        ...
        You("не имеете возможности атаковать монстров физически.");
        mtmp->mstrategy &= ~STRAT_WAITMASK;
        goto atk_done;
    }

    if (check_capacity("Вы не можете сражаться при такой большой нагрузке.")
    ...
    if (gu.unweapon) {
        ...
        if (flags.verbose) {
            if (uwep)
                You("начинаете колотить монстров %s.", yname(uwep));
            else if (!cantwield(gy.youmonst.data))
                You("начинаете %s монстров с помощью %s %s.",
                    ing_suffix(Role_if(PM_MONK) ? "удара" : "дубины"),
                    uarmg ? "в перчатках" : "", /* Del Lamb */
    ...
    if (mdat->mlet == S_LEPRECHAUN && !mtmp->mfrozen && !helpless(mtmp)
        ...
        You("промахиваетесь и спотыкаетесь, падая вперёд.");
        return FALSE;
    }
    ...
    int dieroll)
{
    ...
    if (go.override_confirmation) {
        ...
        if (flags.verbose)
            Your("кровожадный клинок атакует!");
    }
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (!hmd->thrown && obj == uwep && obj->otyp == BOOMERANG
        ...
        pline("Когда вы бьёте %s, %s%s разлетается на щепки.",
              mon_nam(mon), more_than_1 ? "одна из " : "",
              yname(obj));
        if (!more_than_1)
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (!hmd->train_weapon_skill || mon == u.ustuck || u.twoweap
        ...
        ; /* no special bonuses */
               ...
               && hmd->hand_to_hand) {
        You("наносите удар %s со спины!", mon_nam(mon));
        ...
        hmd->hittxt = TRUE;
               ...
               && ((monwep = MON_WEP(mon)) != 0
                   ...
                   && !obj_resists(monwep,
                                   ...
                                   100))) {
        static const char from_your_blow[] = " от силы вашего удара!";
        char buf[BUFSZ];
        /*
        ...
        if (canseemon(mon))
            ...
            Strcpy(buf, Yobjnam2(monwep, "разбивается"));
            ...
            Sprintf(buf, "Его оружие %s", otense(monwep, "разбивается"));
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    case MIRROR:
        if (breaktest(obj)) {
            You("разбиваете %s. Это плохая примета!", ysimple_name(obj));
            change_luck(-2);
            useup(obj);
            obj = (struct obj *) 0;
    ...
    case EXPENSIVE_CAMERA:
        You("успешно уничтожаете %s. Поздравляем!",
            ysimple_name(obj));
...
#define useup_eggs(o)                    \
    ...
    case EGG: {
        ...
        if (ismnum(obj->corpsenm)
            ...
            pline("Шлёп! Вы бьёте %s %s %s яйц%s!",
                  mon_nam(mon),
                  obj->known ? "то" : cnt > 1L ? "несколько" : "одно",
            ...
            const char *eggp =
                (ismnum(mnum) && obj->known)
                    ...
                    : (cnt > 1L) ? "какие-то" : "какое-то";

            You("бьёте %s %s яйц%s.", mon_nam(mon), eggp,
                plur(cnt));
            if (touch_petrifies(hmd->mdat) && !stale_egg(obj)) {
                pline("Яйц%s уже %s живыми...", plur(cnt),
                          (cnt == 1L) ? "не живое" : "не живые");
                if (obj->timed)
                    obj_stop_timers(obj);
                obj->otyp = ROCK;
...
#undef useup_eggs
    ...
    case BLINDING_VENOM:
        ...
        if (can_blnd(&gy.youmonst, mon,
                     ...
                     obj)) {
            if (Blind) {
                pline(obj->otyp == CREAM_PIE ? "Шлёп!"
                      : "Плеск!");
            } else if (obj->otyp == BLINDING_VENOM) {
                pline("Яд ослепляет %s%s!", mon_nam(mon),
                          mon->mcansee ? "" : " Ещё сильнее");
            } else {
            ...
            pline(obj->otyp == CREAM_PIE ? "Шлёп!" : "Плеск!");
            setmangry(mon, TRUE);
        }
        {
            boolean more_than_1 = (obj->quan > 1L);
    ...
    case ACID_VENOM: /* thrown (or spit) */
        if (resists_acid(mon)) {
            Your("яд безвредно поражает %s.", mon_nam(mon));
            hmd->dmg = 0;
        } else {
            Your("яд обжигает %s!", mon_nam(mon));
            hmd->dmg = dmgval(obj, mon);
        }
        {
            boolean more_than_1 = (obj->quan > 1L);
    ...
    struct obj *obj)    /* obj can be NULL */
{
    if (!obj) { /* attack with bare hands */
        ...
        if ((hmd->thrown == HMON_THROWN
            ...
            hit(mshot_xname(obj), mon, " но не причиняет вреда.");
            wakeup(mon, TRUE);
            hmd->doreturn = TRUE;
            hmd->retval = TRUE;
            return;
    ...
    struct obj *obj)    /* obj is not NULL */
{
    ...
    if (Role_if(PM_SAMURAI)) {
        You("бесчестно используете отравленное оружие!");
        adjalign(-sgn(u.ualign.type));
    } else if (u.ualign.type == A_LAWFUL && u.ualign.record > -10) {
        You_feel("себя злым трусом за использование отравленного оружия.");
        adjalign(-1);
    }
    ...
    struct obj *obj) /* lance; obj is not NULL */
{
    ...
    You("играете в джоуст %s%s", mon_nam(mon), canseemon(mon) ? exclam(hmd->dmg) : ".");
    ...
    if (hmd->jousting < 0) {
        ...
        if (obj == uwep)
            uwepgone(); /* set gu.unweapon */
        pline("%s разлетается на куски при ударе!", Yname2(obj));
    ...
    struct obj *obj UNUSED)
{
    ...
    if (rnd(100) < P_SKILL(P_BARE_HANDED_COMBAT) && !bigmonst(hmd->mdat)
        ...
        if (canspotmon(mon))
            pline("%s %s от вашего мощного удара!", Monnam(mon),
                  makeplural(stagger(mon->data, "пошатывается")));
        if (mhurtle_to_doom(mon, hmd->dmg, &hmd->mdat))
            hmd->already_killed = TRUE;
        hmd->hittxt = TRUE;
    }
    ...
    if ((hmd->mdat == &mons[PM_BLACK_PUDDING]
        ...
        if ((mclone = clone_mon(mon, 0, 0)) != 0) {
            ...
            if (u.twoweap && flags.verbose)
                Sprintf(withwhat, " с помощью %s", yname(obj));
            pline("%s делится надвое от вашего удара%s!", Monnam(mon), withwhat);
            hmd->hittxt = TRUE;
    ...
    struct obj *obj)   /* obj can be NULL for hand_to_hand; otherwise not */
{
    if (!hmd->hittxt /*( thrown => obj exists )*/
        ...
        if (hmd->thrown)
            ...
            You("попадаете в него.");
        else /* hand_to_hand */
            You("%s %s%s",
                (obj && (is_shield(obj)
                         || obj->otyp == HEAVY_IRON_BALL)) ? "лупите"
                : (obj && (objects[obj->otyp].oc_skill == P_WHIP
                           || is_wet_towel(obj))) ? "хлещете"
                  : Role_if(PM_BARBARIAN) ? "сокрушаете"
                    : "бьёте",
    ...
    struct obj *obj UNUSED)
{
    ...
    if (!noncorporeal(hmd->mdat) && !amorphous(hmd->mdat))
        whom = strcat(s_suffix(whom), " плоть");
    ...
    int dieroll)
{
    ...
    if (hmd.needpoismsg)
        pline("Яд, кажется, не действует на %s.", mon_nam(mon));
    if (hmd.poiskilled) {
        pline("Яд оказался смертельным...");
        if (!hmd.already_killed)
        ...
        if (!mon->mconf && !resist(mon, SPBOOK_CLASS, 0, NOTELL)) {
            ...
            if (!mon->mstun && !helpless(mon) && canseemon(mon))
                pline("%s выглядит сбитым с толку.", Monnam(mon));
        }
    }
    if (hmd.unpoisonmsg)
        Your("%s больше не отравлено.", hmd.saved_oname);
    ...
    boolean verbose)
{
    ...
    if (verbose
        ...
        static const char harmlessly_thru[] = " безвредно проходит сквозь ";

        what = (!obj || shade_aware(obj)) ? "атака" : cxname(obj);
        target = youdef ? "вас" : mon_nam(mdef);
        if (!thrown) {
            whose = youagr ? "Ваша" : s_suffix(Monnam(magr));
...
m_slips_free(struct monst *mdef, struct attack *mattk)
{
    ...
    if (obj && (obj->greased || obj->otyp == OILSKIN_CLOAK)
        && (!obj->cursed || rn2(3))) {
        You("%s %s %s %s!",
            (mattk->adtyp == AD_WRAP) ? "соскальзываете с"
                                      : "хватаете, но не можете удержать",
            s_suffix(mon_nam(mdef)), obj->greased ? "намасленной" : "скользкой",
...
demonpet(void)
{
    ...
    pline("Какой-то адский демон прибыл!");
...
theft_petrifies(struct obj *otmp)
{
    ...
    instapetrify(corpse_xname(otmp, "украденного", CXN_ARTICLE));
...
steal_it(struct monst *mdef, struct attack *mattk)
{
    ...
    if (ustealo) { /* we will be taking everything */
        ...
        if (gender(mdef) == (int) u.mfemale
            ...
            You("очаровываете %s. %s охотно отдаёт %s%s имущество.",
                mon_nam(mdef), upstart(strcpy(heshe, mhe(mdef))),
                !gold ? "" : "большую часть из ", mhis(mdef));
        else
            You("соблазняете %s, и %s начинает снимать %s одежду.",
    ...
    while ((otmp = mdef->minvent) != 0) {
        ...
        if (otmp == ustealo)
            pline("%s заканчивает снимать %s доспехи.", Monnam(mdef),
                  mhis(mdef));
        /* give the object to the character */
        otmp = hold_another_object(otmp, "Вы выхватили, но уронили %s.",
                                   doname(otmp), "Вы крадёте: ");
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (completelyrusts(pd)) { /* iron golem */
            ...
            pline("%s %s на куски!", Monnam(mdef),
                  !mlifesaver(mdef) ? "разваливается" : "начинает разваливаться");
        ...
        if (completelyrusts(pd)) { /* PM_IRON_GOLEM */
            if (gv.vis && canseemon(mdef))
                pline_mon(mdef, "%s %s на куски!", Monnam(mdef),
                      !mlifesaver(mdef) ? "разваливается" : "начинает разваливаться");
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3) && !(resists_drli(mdef) || defended(mdef, AD_DRLI))
            ...
            mhm->damage = d(2, 6); /* Stormbringer uses monhp_per_lvl
                                    * (usually 1d8) */
            pline("%s становится слабее!", Monnam(mdef));
            ...
            if (DEADMONSTER(mdef) || !mdef->m_lev) {
                pline("%s %s!", Monnam(mdef),
                      nonliving(mdef->data) ? "исчезает" : "умирает");
                xkilled(mdef, XKILL_NOMSG);
            } else
                mdef->m_lev--;
        ...
        if (is_death
            ...
            if (gv.vis && canspotmon(mdef))
                pline_mon(mdef, "%s становится слабее!", Monnam(mdef));
            if (mdef->mhpmax - mhm->damage > (int) mdef->m_lev) {
                mdef->mhpmax -= mhm->damage;
            } else {
                /* limit floor of mhpmax reduction to current m_lev + 1;
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (completelyburns(pd)) { /* paper golem or straw golem */
            if (!Blind)
                /* note: the life-saved case is hypothetical because
                   life-saving doesn't work for golems */
                pline("%s %s!", Monnam(mdef),
                      !mlifesaver(mdef) ? "полностью сгорает"
                                        : "полностью охвачен пламенем");
            else
                You("чувствуете запах горящей%s.",
                    (pd == &mons[PM_PAPER_GOLEM]) ? " бумаги"
                      : (pd == &mons[PM_STRAW_GOLEM]) ? " соломы" : "");
        ...
        if (resists_fire(mdef) || defended(mdef, AD_FIRE)) {
            if (!Blind)
                pline("Огонь не нагревает %s!", mon_nam(mdef));
        ...
        hitmsg(magr, mattk);
        if (!mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            pline("Вы %s!", on_fire(pd, mattk));
            if (completelyburns(pd)) { /* paper or straw golem */
                You("вспыхиваете пламенем!");
                monstunseesu(M_SEEN_FIRE);
                /* KMH -- this is okay with unchanging */
                rehumanize();
                return;
            } else if (Fire_resistance) {
                pline("Огонь не кажется горячим!");
        ...
        if (resists_fire(mdef) || defended(mdef, AD_FIRE)) {
            if (gv.vis && canseemon(mdef))
                pline("Огонь, кажется, не жжёт %s!", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_FIRE, mhm->damage);
            mhm->damage = 0;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!Blind)
            pline("%s покрыт инеем!", Monnam(mdef));
        if (resists_cold(mdef) || defended(mdef, AD_COLD)) {
            shieldeff(mdef->mx, mdef->my);
            if (!Blind)
                pline("Иней не охлаждает %s!", mon_nam(mdef));
        ...
        hitmsg(magr, mattk);
        if (!mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            pline("Вы покрыты инеем!");
            if (Cold_resistance) {
                pline("Иней, кажется, не холодный!");
        ...
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%s покрыт инеем!", Monnam(mdef));
        if (resists_cold(mdef) || defended(mdef, AD_COLD)) {
            if (gv.vis && canseemon(mdef))
                pline("Иней, кажется, не охлаждает %s!", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
            golemeffects(mdef, AD_COLD, mhm->damage);
            mhm->damage = 0;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!Blind)
            pline("%s поражён электрошоком!", Monnam(mdef));
        if (resists_elec(mdef) || defended(mdef, AD_ELEC)) {
            if (!Blind)
                pline("Разряд не бьёт %s током!", mon_nam(mdef));
        ...
        hitmsg(magr, mattk);
        if (!mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            You("получаете электрошок!");
            if (Shock_resistance) {
                pline("Разряд не бьёт вас током!");
        ...
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%s получает электрошок!", Monnam(mdef));
        if (resists_elec(mdef) || defended(mdef, AD_ELEC)) {
            if (gv.vis && canseemon(mdef))
                pline("Разряд не бьёт %s током!", mon_nam(mdef));
            shieldeff(mdef->mx, mdef->my);
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && !rn2(3))
            if (Acid_resistance) {
                pline("Вы покрыты %s, но это кажется безвредным.",
                      hliquid("acid"));
                monstseesu(M_SEEN_ACID);
                mhm->damage = 0;
            } else {
                pline("Вы покрыты %s! Оно жжёт!", hliquid("acid"));
        ...
        if (resists_acid(mdef) || defended(mdef, AD_ACID)) {
            if (gv.vis && canseemon(mdef))
                pline("%s покрыт %s, но это кажется безвредным.",
                      Monnam(mdef), hliquid("acid"));
            mhm->damage = 0;
        } else if (gv.vis && canseemon(mdef)) {
            pline_mon(mdef, "%s покрыт %s!", Monnam(mdef), hliquid("acid"));
            pline("Оно жжёт %s!", mon_nam(mdef));
        }
        if (!rn2(30))
            erode_armor(mdef, ERODE_CORRODE);
        if (!rn2(6))
    ...
    if (magr == &gy.youmonst) {
        ...
        if (mongold) {
            ...
            if (merge_choice(gi.invent, mongold)
                ...
                addinv(mongold);
                Your("кошелёк становится тяжелее.");
            } else {
                You("хватаете золото %s, но не находите места в рюкзаке.",
                    mon_nam(mdef));
                dropy(mongold);
            }
        }
        ...
        if (gv.vis && canseemon(mdef)) {
            pline("%s крадёт немного золота у %s.", buf, mon_nam(mdef));
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (mhm->damage <= 0)
            mhm->damage = 1;
        if (mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            pline("%s не затронут.", Monnam(mdef));
        } else {
            char nambuf[BUFSZ];
            ...
            if (u_teleport_mon(mdef, FALSE) && u_saw_mon
                ...
                pline("%s внезапно исчезает!", nambuf);
        ...
        if (mhitm_mgc_atk_negated(magr, mdef, FALSE)) {
            ...
            if (flags.verbose)
                Your("ваше положение вдруг кажется %sнеопределённым!",
        ...
        if (magr->mcan || mhm->damage >= mdef->mhp || tele_restrict(mdef)) {
            ...
            if (gv.vis)
                pline_mon(mdef, "%s не затронут.", Monnam(mdef));
        } else {
            ...
            if (gv.vis && wasseen && !canspotmon(mdef) && mdef != u.usteed)
                pline("%s внезапно исчезает!", mdef_Monnam);
            if (mhm->damage >= mdef->mhp) { /* see hitmu(mhitu.c) */
                if (mdef->mhp == 1)
                    ++mdef->mhp;
                mhm->damage = mdef->mhp - 1;
    ...
    struct mhitm_data *mhm) /* optional for monster vs monster */
{
    if (magr == &gy.youmonst) {
        ...
        if (can_blnd(magr, mdef, mattk->aatyp, (struct obj *) 0)) {
            if (!Blind)
                pline("%s ослепляет вас!", Monnam(magr));
    ...
    if (magr == &gy.youmonst) {
        ...
        if (night() && !rn2(10) && !mdef->mcan) {
            if (pd == &mons[PM_CLAY_GOLEM]) {
                if (!Blind)
                    pline("Какая-то надпись исчезает с %s головы!",
        ...
        if (!magr->mcan && !rn2(10)) {
            if (!Deaf) {
                ...
                if (Blind) {
                    ...
                    pline_mon(magr, "%s посмеивается.", Monnam(magr));
                }
            }
            if (u.umonnum == PM_CLAY_GOLEM) {
                pline("Какая-то надпись исчезает с вашей головы!");
                /* KMH -- this is okay with unchanging */
                rehumanize();
                return;
            }
        ...
        if (!magr->mcan && !rn2(10)) {
            ...
            if (pd == &mons[PM_CLAY_GOLEM]) {
                if (gv.vis && canseemon(mdef)) {
                    pline("Какая-то надпись исчезает с %s головы!",
                          s_suffix(mon_nam(mdef)));
                    pline_mon(mdef, "%s уничтожен!", Monnam(mdef));
                }
                mondied(mdef);
                if (!DEADMONSTER(mdef)) {
                    mhm->hitflags = M_ATTK_MISS;
                    ...
                    You(brief_feeling, "странно печальное");
                }
                mhm->hitflags = (M_ATTK_DEF_DIED
                                 | (grow_up(magr, mdef) ? 0
                                    : M_ATTK_AGR_DIED));
                    ...
                    struct monst *mdef, struct mhitm_data *mhm)
{
    if (gv.vis && canspotmon(magr))
        pline("%s %s был отравлен!", s_suffix(Monnam(magr)),
              mpoisons_subj(magr, mattk));
    if (resists_poison(mdef)) {
        if (gv.vis && canspotmon(mdef) && canspotmon(magr))
            pline("Яд, кажется, не действует на %s.",
                        mon_nam(mdef));
    } else {
        mhm->damage += rn1(10, 6);
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        /* uhitm */
        if (!negated && !rn2(8)) {
            Your("%s был отравлен!", mpoisons_subj(magr, mattk));
            if (resists_poison(mdef)) {
                pline("Яд, кажется, не действует на %s.", mon_nam(mdef));
            } else {
                if (!rn2(10)) {
                    Your("яд оказался смертельным...");
                    mhm->damage = mdef->mhp;
                } else
                    mhm->damage += rn1(10, 6);
            }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (gn.notonhead || !has_head(pd)) {
            pline("%s, кажется, не пострадал.", Monnam(mdef));
            ...
            if (!Unchanging && pd == &mons[PM_GREEN_SLIME]) {
                if (!Slimed) {
                    You("втягиваете немного слизи и чувствуете себя неважно.");
        ...
        if ((helmet = which_armor(mdef, W_ARMH)) != 0 && rn2(8)) {
            pline("%s %s блокирует вашу атаку по %s голове.",
        ...
        if (uarmh && rn2(8)) {
            /* not body_part(HEAD) */
            Your("%s блокирует атаку по вашей голове.",
        ...
        if (gn.notonhead || !has_head(pd)) {
            if (gv.vis && canspotmon(mdef))
                pline_mon(mdef, "%s, кажется, не пострадал.", Monnam(mdef));
            /* Not clear what to do for green slimes */
            mhm->damage = 0;
        ...
        if ((mdef->misc_worn_check & W_ARMH) && rn2(8)) {
            if (gv.vis && canspotmon(magr) && canseemon(mdef)) {
                ...
                pline("%s шлем блокирует атаку %s по %s голове.", buf,
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated && !sticks(pd) && m_next2u(mdef)) {
            ...
            if (barbs)
                Your("шипы впиваются в %s!", y_monnam(mdef));
        }
    } else if (mdef == &gy.youmonst) {
        /* mhitu */
        hitmsg(magr, mattk);
        ...
        if (!sticks(pd)) {
            ...
            if (!u.ustuck && !tailmiss && !rn2(10)) {
                if (m_slips_free(mdef, mattk)) {
                    ...
                    You("%s вокруг %s!",
                        coil ? "обвиваетесь" : "замахиваетесь", mon_nam(mdef));
                    set_ustuck(mdef);
                }
            } else if (u.ustuck == mdef && !tailmiss) {
                /* Monsters don't wear amulets of magical breathing */
                if (is_pool(u.ux, u.uy) && !cant_drown(pd)) {
                    You("топите %s...", mon_nam(mdef));
                    mhm->damage = mdef->mhp;
                } else if (mattk->aatyp == AT_HUGS)
                    pline("%s раздавливается.", Monnam(mdef));
            } else {
                mhm->damage = 0;
                if (flags.verbose) {
                    if (coil && !tailmiss)
                        You("задеваете %s.", mon_nam(mdef));
                    else
                        You("задеваете %s %s.", s_suffix(mon_nam(mdef)),
                            tailmiss ? "хвост" : mbodypart(mdef, LEG));
                }
            }
        } else
            mhm->damage = 0;
        ...
        if ((!magr->mcan || u.ustuck == magr) && !sticks(pd)) {
            if (!u.ustuck && !rn2(10)) {
                if (u_slip_free(magr, mattk)) {
                    ...
                    urgent_pline("%s %s вокруг вас!",
                                 Some_Monnam(magr),
                                 coil ? "обвивается" : "замахивается");
                }
                ...
                if (is_pool(magr->mx, magr->my) && !Swimming && !Amphibious
                    ...
                    urgent_pline("%s топит вас...", Monnam(magr));
                    svk.killer.format = KILLED_BY_AN;
                    Sprintf(svk.killer.name, "%s %s",
                            moat ? "во рву" : "в водоёме",
                ...
                if (flags.verbose) {
                    if (coil)
                        pline_mon(magr, "%s задевает вас.",
                                  Monnam(magr));
                    else
                        pline_mon(magr, "%s задевает вашу %s.",
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3) && mhm->damage < mdef->mhp
            && !mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            if (!Blind)
                pline("%s заморожен вами!", Monnam(mdef));
        ...
        if (gm.multi >= 0 && !rn2(3)
            ...
            if (Free_action) {
                ...
                if (Blind)
                    ...
                    You("заморожены %s!", mon_nam(magr));
                gn.nomovemsg = You_can_move_again;
                nomul(-rnd(10));
                /* set gm.multi_reason;
                   3.6.x used "paralyzed by a monster"; be more specific */
                dynamic_multi_reason(magr, "парализованы", FALSE);
        ...
        if (mdef->mcanmove && !rn2(3)
            ...
            if (gv.vis && canspotmon(mdef)) {
                ...
                pline("%s заморожен %s.", buf, mon_nam(magr));
            }
            paralyze_monst(mdef, rnd(10));
        }
    }
    ...
    struct mhitm_data *mhm UNUSED)
{
    if (magr == &gy.youmonst) {
        ...
        if (!mdef->msleeping && !mhitm_mgc_atk_negated(magr, mdef, FALSE)
            ...
            if (!Blind)
                pline("%s усыплён вами!", Monnam(mdef));
            slept_monst(mdef);
        }
    } else if (mdef == &gy.youmonst) {
        /* mhitu */
        ...
        if (!mdef->msleeping && sleep_monst(mdef, rnd(10), -1)
            ...
            if (gv.vis && canspotmon(mdef)) {
                ...
                Strcpy(buf, Monnam(mdef));
                pline("%s усыплён %s.", buf, mon_nam(magr));
            }
            mdef->mstrategy &= ~STRAT_WAITFORU;
            slept_monst(mdef);
        }
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(4) && !slimeproof(pd)) {
            if (!munslime(mdef, TRUE) && !DEADMONSTER(mdef)) {
                ...
                You("превращаете %s в слизь.", mon_nam(mdef));
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated) {
            ...
            if (obj && drain_item(obj, FALSE)) {
                pline("%s менее эффективно.", Yobjnam2(obj, "кажутся"));
            }
        }
    } else {
        /* mhitm */
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!negated && mdef->mspeed != MSLOW) {
            ...
            if (mdef->mspeed != oldspeed && canseemon(mdef))
                pline("%s замедляется.", Monnam(mdef));
        }
    } else if (mdef == &gy.youmonst) {
        /* mhitu */
        hitmsg(magr, mattk);
        ...
        if (!negated && mdef->mspeed != MSLOW) {
            ...
            if (mdef->mspeed != oldspeed && gv.vis && canspotmon(mdef))
                pline_mon(mdef, "%s замедляется.", Monnam(mdef));
        }
    }
}
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!mdef->mconf) {
            if (canseemon(mdef))
                pline("%s выглядит сбитым с толку.", Monnam(mdef));
            mdef->mconf = 1;
        }
    } else if (mdef == &gy.youmonst) {
        /* mhitu */
        ...
        if (!magr->mcan && !rn2(4) && !magr->mspec_used) {
            ...
            if (Confusion)
                You("становитесь ещё более сбитыми с толку.");
            else
                You("становитесь сбитыми с толку.");
        ...
        if (!magr->mcan && !mdef->mconf && !magr->mspec_used) {
            if (gv.vis && canseemon(mdef))
                pline_mon(mdef, "%s выглядит сбитым с толку.", Monnam(mdef));
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
        }
    }
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!uwep && mhm->damage < mdef->mhp) {
            if (negated) {
                ...
                pline("%s не превращён.", Monnam(mdef));
            } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%s протягивает руку, и ваше тело съёживается.",
                  Monnam(magr));
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%s протягивает руку, и вы чувствуете жар и озноб.",
                  Monnam(magr));
        (void) diseasemu(pa);
        /* plus the normal damage */
    } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        /* mhitu */
        pline_mon(magr, "%s протягивает руку со смертельным прикосновением.", Monnam(magr));
        if (is_undead(pd)) {
            /* still does some damage */
            mhm->damage = (mhm->damage + 1) / 2;
            pline("Это было прикосновение смерти?");
            return;
        }
        switch (rn2(20)) {
        case 19:
        ...
        default: /* case 16: ... case 5: */
            You_feel("как ваша жизненная сила уходит...");
        ...
        case 0:
            if (Antimagic)
                shieldeff(u.ux, u.uy);
            pline("К счастью для вас, это не сработало!");
            mhm->damage = 0;
            return;
        }
    } else {
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!magr->mcan && haseyes(pd) && mdef->mcansee) {
            if (gv.vis && canseemon(mdef))
                pline_mon(mdef, "%s выглядит %sсбитым с толку.", Monnam(mdef),
                      mdef->mconf ? "более " : "");
            mdef->mconf = 1;
            mdef->mstrategy &= ~STRAT_WAITFORU;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (!resists_ston(mdef)) {
        if (gv.vis && canseemon(mdef))
            pline_mon(mdef, "%s превращается в камень!", Monnam(mdef));
        monstone(mdef);
 post_stone:
        if (!DEADMONSTER(mdef)) {
            mhm->hitflags = M_ATTK_MISS;
            ...
            You(brief_feeling, "странно грустное");
        }
        mhm->hitflags = (M_ATTK_DEF_DIED
                         | (grow_up(magr, mdef) ? 0 : M_ATTK_AGR_DIED));
        mhm->done = TRUE;
        ...
        if (mattk->aatyp == AT_HUGS && !sticks(pd)) {
            if (!u.ustuck && rn2(2)) {
                if (u_slip_free(magr, mattk)) {
                    ...
                    set_ustuck(magr);
                    pline_mon(magr, "%s хватает вас!", Monnam(magr));
                    mhm->hitflags |= M_ATTK_HIT;
                }
            } else if (u.ustuck == magr) {
                exercise(A_STR, FALSE);
                You("будете %s.",
                    (pa == &mons[PM_ROPE_GOLEM]) ? "удушены" : "раздавлены");
            }
            ...
            if (mattk->aatyp == AT_WEAP && otmp) {
                ...
                if (otmp->otyp == CORPSE
                    ...
                    mhm->damage = 1;
                    pline_mon(magr, "%s бьёт вас %s трупом.",
                              Monnam(magr),
                ...
                if (objects[otmp->otyp].oc_material == SILVER
                    && Hate_silver) {
                    pline("Серебро обжигает вашу плоть!");
                    exercise(A_CON, FALSE);
                }
                ...
                if (u.mh - tmp > 1
                    ...
                    if (cloneu())
                        You("делитесь надвое, когда %s бьёт вас!", mon_nam(magr));
                }
        ...
        if (shade_miss(magr, mdef, mwep, FALSE, vis)) {
            ...
            if (mwep->oartifact) {
                ...
                if (!artifact_hit(magr, mdef, mwep, &mhm->damage,
                                  mhm->dieroll)) {
                    if (gv.vis)
                        pline_mon(magr, "%s бьёт %s.", Monnam(magr),
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(3)) {
            if (magr->mcan) {
                if (!Deaf)
                    You_hear("кашель %s!", mon_nam(magr));
            } else {
                if (Hallucination && !Blind) {
                    Soundeffect(se_cockatrice_hiss, 50);
                    You_hear("шипение."); /* You_hear() deals with Deaf */
                    pline("%s, кажется, посылает вам воздушный поцелуй...",
                          Monnam(magr));
                } else if (!Deaf) {
                    You_hear("шипение %s!", s_suffix(mon_nam(magr)));
                } else if (!Blind) {
                    pline("%s, кажется, гримасничает.", Monnam(magr));
                }
                /*
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!rn2(4) && u.ulycn == NON_PM
            ...
            && !mhitm_mgc_atk_negated(magr, mdef, TRUE)) {
            urgent_pline("Вы чувствуете жар.");
            exercise(A_CON, FALSE);
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (!(uwep && (uwep->oclass == WEAPON_CLASS || is_weptool(uwep)))
            ...
            pline_mon(magr, "%s бьёт! (Надеюсь, вы не против.)",
                      Monnam(magr));
            if (Upolyd) {
                u.mh += rnd(7);
                if (!rn2(7)) {
            ...
            if (Role_if(PM_HEALER)) {
                if (!Deaf && !(svm.moves % 5)) {
                    ...
                    verbalize("Док, я не могу вам помочь, если вы не будете сотрудничать.");
                }
                mhm->damage = 0;
            } else
                hitmsg(magr, mattk);
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        /* uhitm */
        if (!Blind)
            pline("%s %s на мгновение.", Monnam(mdef),
                  makeplural(stagger(pd, "пошатывается")));
        mdef->mstun = 1;
        mhitm_ad_phys(magr, mattk, mdef, mhm);
        if (mhm->done)
            return;
        ...
        if (magr->mcan)
            return;
        if (canseemon(mdef))
            pline_mon(mdef, "%s %s на мгновение.", Monnam(mdef),
                  makeplural(stagger(pd, "пошатывается")));
        mdef->mstun = 1;
        mhitm_ad_phys(magr, mattk, mdef, mhm);
        if (mhm->done)
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    if (magr == &gy.youmonst) {
        ...
        const char *sidestr = (side == RIGHT_SIDE) ? "правой" : "левой",
        ...
        if ((u.usteed || Levitation || Flying) && !is_flyer(magr->data)) {
            pline("%s пытается достать до вашей %s %s!", Monst_name, sidestr, leg);
            mhm->damage = 0;
        } else if (magr->mcan) {
            pline_mon(magr, "%s трётся о вашу %s %s!", Monnam(magr),
                  sidestr, leg);
            mhm->damage = 0;
        } else {
            if (uarmf) {
                if (rn2(2) && (uarmf->otyp == LOW_BOOTS
                    ...
                    pline("%s колет открытую часть вашей %s %s!",
                          Monst_name, sidestr, leg);
                } else if (!rn2(5)) {
                    pline("%s прокалывает ваш %s сапог!", Monst_name,
                          sidestr);
                } else {
                    pline("%s царапает ваш %s сапог!", Monst_name,
                          sidestr);
                    mhm->damage = 0;
                    return;
                }
            } else
                pline("%s колет вашу %s %s!", Monst_name, sidestr, leg);
        ...
        if (is_rider(pd)) {
            if (gv.vis && canseemon(magr))
                pline_mon(magr, "%s %s!", Monnam(magr),
                      (pd == &mons[PM_FAMINE])
                          ? "слабо икает, съёживается и умирает"
                          : (pd == &mons[PM_PESTILENCE])
                                ? "судорожно кашляет и падает"
                                : "сильно рвёт и падает замертво");
            mondied(magr);
            if (!DEADMONSTER(magr)) {
                ...
                You(brief_feeling, "тошнотворное");
            mhm->hitflags = M_ATTK_AGR_DIED;
            mhm->done = TRUE;
            return;
        }
    ...
    struct monst *mdef, struct mhitm_data *mhm)
{
    ...
    if (magr == &gy.youmonst) {
        ...
        if (is_animal(magr->data)) {
            ...
            pline_mon(magr, "%s %s.", Monnam(magr),
                  Deaf ? "что-то говорит, но вы этого не слышите"
                  : magr->minvent
                    ? "хвастается товарами, которые принёс какой-то исследователь подземелья"
                  : "делает замечания о том, как сложно в последнее время воровать");
            if (!tele_restrict(magr))
                (void) rloc(magr, RLOC_MSG);
            ...
            if (!Blind)
                pline("%s пытается %s вас, но вы кажетесь %s.",
                      Adjmonnam(magr, "обычный"),
                      flags.female ? "очаровать" : "соблазнить",
                      flags.female ? "невозмутимой" : "незаинтересованным");
            if (rn2(3)) {
                if (!tele_restrict(magr))
                    (void) rloc(magr, RLOC_MSG);
                mhm->hitflags = M_ATTK_AGR_DONE; /* return 3??? */
        ...
        default:
            ...
            if (is_animal(magr->data) && *buf) {
                if (canseemon(magr))
                    pline_mon(magr, "%s пытается %s с %s.",
                              Monnam(magr),
                              locomotion(magr->data, "убежать"), buf);
            }
        ...
        if (obj) {
            ...
            Strcpy(buf, Monnam(magr));
            if (gv.vis && canseemon(mdef)) {
                pline("%s крадёт %s у %s!", buf, onambuf, mdefnambuf);
            }
            possibly_unwield(mdef, FALSE);
            mdef->mstrategy &= ~STRAT_WAITFORU;
    ...
    int specialdmg) /* blessed and/or silver bonus against various things */
{
    ...
    if (DEADMONSTER(mdef)) {
        ...
        if (mdef->mtame && !cansee(mdef->mx, mdef->my)) {
            You_feel("смущение на мгновение.");
            if (mhm.damage)
                xkilled(mdef, XKILL_NOMSG);
        } else if (!flags.verbose) {
            You("уничтожаете его!");
...
explum(struct monst *mdef, struct attack *mattk)
{
    ...
    case AD_BLND:
        if (mdef && !resists_blnd(mdef)) {
            pline("%s ослеплён вашей вспышкой света!", Monnam(mdef));
    ...
    case AD_HALU:
        if (mdef && haseyes(mdef->data) && mdef->mcansee) {
            pline("%s затронут вашей вспышкой света!", Monnam(mdef));
            mdef->mconf = 1;
        }
        break;
    case AD_COLD:
...
start_engulf(struct monst *mdef)
{
    ...
    You("%s %s%s!",
        u_digest ? "глотаете" : u_enfold ? "охватываете" : "поглощаете",
        mon_nam(mdef), u_digest ? " целиком" : "");
    nh_delay_output();
    nh_delay_output();
}
...
gulpum(struct monst *mdef, struct attack *mattk)
{
    ...
    struct obj *otmp;
    struct permonst *pd = mdef->data;
    const char *expel_verb = u_digest ? "извергаете"
                             : u_enfold ? "выпускаете"
                               : "изгоняете";
    ...
    if (!(u_digest && u.uhunger >= 1500) && !u.uswallow) {
        ...
        if (fatal_gulp && !is_rider(pd)) { /* petrification */
            ...
            if (!type_is_pname(pd))
                mnam = an(mnam);
            You("%s %s.", u_digest ? "заглатываете" : "поглощаете", mon_nam(mdef));
            Sprintf(kbuf, "%s %s%s",
                    u_digest ? "заглатывание"
                    : u_enfold ? "окутывание"
                      : "поглощение",
                    mnam, u_digest ? " целиком" : "");
            instapetrify(kbuf);
        } else {
            start_engulf(mdef);
            switch (mattk->adtyp) {
            case AD_DGST:
                /* eating a Rider or its corpse is fatal */
                if (is_rider(pd)) {
                    pline("К сожалению, переварить любую его часть смертельно.");
                    end_engulf();
                    Sprintf(svk.killer.name, "неразумно попытался съесть %s",
                ...
                if (!DEADMONSTER(mdef)) { /* monster lifesaved */
                    You("поспешно извергаете жгучее содержимое из вашего %s.",
                        body_part(STOMACH));
                } else {
                    tmp = 1 + (pd->cwt >> 8);
                    ...
                    Sprintf(msgbuf, "Вы полностью перевариваете %s.", mon_nam(mdef));
                    if (tmp != 0) {
                         ...
                         */
                        You("перевариваете %s.", mon_nam(mdef));
                        if (Slow_digestion)
                            tmp *= 2;
                        nomul(-tmp);
                        gm.multi_reason = "переваривание чего-то";
                        gn.nomovemsg = msgbuf;
                    ...
                    if (pd == &mons[PM_GREEN_SLIME]) {
                        Sprintf(msgbuf, "%s не согласуется с вами.",
            ...
            case AD_PHYS:
                if (gy.youmonst.data == &mons[PM_FOG_CLOUD]) {
                    pline("%s пропитан вашей влагой.", Monnam(mdef));
                    if ((breathless(pd) || amphibious(pd)) && !flaming(pd)) {
                        dam = 0;
                        pline("%s, кажется, невредим.", Monnam(mdef));
                    }
                } else {
                    pline("%s %s!", Monnam(mdef),
                          enfolds(gy.youmonst.data) ? "раздавлен"
                            : "избит вашим мусором");
                }
                break;
            case AD_ACID:
                pline("%s покрыт вашей слизью!", Monnam(mdef));
                if (resists_acid(mdef)) {
                    pline("Это кажется безвредным для %s.", mon_nam(mdef));
                    dam = 0;
                }
                break;
            case AD_BLND:
                if (can_blnd(&gy.youmonst, mdef, mattk->aatyp,
                             (struct obj *) 0)) {
                    if (mdef->mcansee)
                        pline("%s ничего не видит там!", Monnam(mdef));
                    mdef->mcansee = 0;
                    dam += mdef->mblinded;
                    if (dam > 127)
                        dam = 127;
            ...
            case AD_ELEC:
                if (rn2(2)) {
                    pline("Воздух вокруг %s потрескивает от электричества.",
                              mon_nam(mdef));
            ...
            case AD_COLD:
                if (rn2(2)) {
                    if (resists_cold(mdef)) {
                        pline("%s, кажется, слегка озяб.", Monnam(mdef));
                        dam = 0;
                    } else
                        pline("%s замерзает насмерть!", Monnam(mdef));
                    golemeffects(mdef, (int) mattk->adtyp, dam);
                } else
                    dam = 0;
                break;
            case AD_FIRE:
                if (rn2(2)) {
                    if (resists_fire(mdef)) {
                        pline("%s, кажется, слегка разгорячён.", Monnam(mdef));
                        dam = 0;
                    } else
                        pline("%s сгорает дотла!", Monnam(mdef));
                    golemeffects(mdef, (int) mattk->adtyp, dam);
                } else
                    dam = 0;
                break;
            ...
            if ((Slow_digestion || is_animal(gy.youmonst.data)) && u_digest) {
                pline("Очевидно, вам не понравился вкус %s.",
                      s_suffix(mon_nam(mdef)));
            }
        }
    }
    ...
    boolean wouldhavehit)
{
    if (wouldhavehit) /* monk is missing due to penalty for wearing suit */
        Your("броня довольно громоздкая...");

    if (could_seduce(&gy.youmonst, mdef, mattk))
        You("притворяетесь дружелюбным с %s.", mon_nam(mdef));
    else if (canspotmon(mdef) && flags.verbose)
        You("промахиваетесь по %s.", mon_nam(mdef));
    else
        You("промахиваетесь.");
    if (!helpless(mdef))
        wakeup(mdef, TRUE);
    ...
    boolean weapon_used)  /* True: via weapon hit */
{
    ...
    if (m_is_steadfast(mdef)) {
        if (u_def || (u.usteed && mdef == u.usteed)) {
            mdefbuf[0] = '\0';
            if (u.usteed)
                Snprintf(mdefbuf, sizeof mdefbuf, "и %s ",
                         y_monnam(u.usteed));
            You("%sне сдвигаетесь с места.", mdefbuf);
        } else if (canseemon(mdef)) {
            pline("%s не сдвигается с места.", Monnam(mdef));
        }
        return FALSE;
    }

    /* subtly vary the message text if monster won't actually move */
    knockedhow = dismount ? "из вашего седла"
                 : will_hurtle(mdef, defx + dx, defy + dy) ? "назад"
                   : "обратно";

    /* give the message */
    if (u_def || canseemon(mdef)) {
        Strcpy(magrbuf, u_agr ? "Вы" : Monnam(magr));
        Strcpy(mdefbuf, (u_def || was_u) ? "вы" : y_monnam(mdef));
        if (was_u)
            Snprintf(eos(mdefbuf), sizeof mdefbuf - strlen(mdefbuf),
                     " и %s", y_monnam(u.usteed));
        /*
         ...
         */
        pline("%s %s %s %s с %s %s!",
              magrbuf, vtense(magrbuf, "сбивает"), mdefbuf, knockedhow,
              rn2(2) ? "сильным" : "мощным", rn2(2) ? "ударом" : "толчком");
    } else if (u_agr) {
        /* hero knocks unseen foe back; noticed by touch */
        You_feel("как %s отброшен %s!", some_mon_nam(mdef), knockedhow);
    }
 ...
 use_weapon:
            ...
            if (dhit) {
                ...
                if (!u.uswallow
                    && (compat = could_seduce(&gy.youmonst, mon, mattk))
                       != 0) {
                    You("%s %s %s.",
                        (mon->mcansee && haseyes(mon->data)) ? "улыбаетесь"
                                                             : "разговариваете с",
                        mon_nam(mon),
                        (compat == 2) ? "завлекающе" : "соблазнительно");
                ...
                case AT_TUCH:
                    ...
                    verb = (mattk->aatyp == AT_TUCH) ? "касаетесь" : "царапаете";
                ...
                case AT_KICK:
                    ...
                    break;
                case AT_BUTT:
                    verb = "бодаете"; /* mbodypart(mon,HEAD)=="head" */
                ...
                if (mon->data == &mons[PM_SHADE] && !specialdmg) {
                    if (!strcmp(verb, "бьёте")
                        || (mattk->aatyp == AT_CLAW && humanoid(mon->data)))
                        verb = "атакуете";
                    Your("%s %s безвредно сквозь %s.",
                         verb, vtense(verb, ""), mon_nam(mon));
                } else {
                    ...
                    if (mattk->aatyp == AT_TENT) {
                        Your("щупальца высасывают %s.", mon_nam(mon));
                    } else {
                        if (mattk->aatyp == AT_CLAW)
                            verb = "бьёте"; /* not "claws" */
        ...
        case AT_HUGS: {
            ...
            if (mon->data == &mons[PM_SHADE]) {
                const char *verb = byhand ? "хватаете" : "обнимаете";
                ...
                if (specialdmg) {
                    ...
                    Your("%s проходит безвредно сквозь %s.",
                         verb, mon_nam(mon));
                }
                break;
            }
            ...
            if (mon == u.ustuck) {
                pline("%s %s%s.", Monnam(mon),
                      byhand ? "удушается" : "раздавливается",
                      /* extra feedback for non-breather being choked */
                      unconcerned ? ", но, кажется, не обеспокоен" : "");
                if (silverhit && flags.verbose)
                    silver_sears(&gy.youmonst, mon, silverhit);
                sum[i] = damageum(mon, mattk, specialdmg);
        ...
        case AT_ENGL:
            ...
            if ((dhit = (tmp > rnd(20 + i)))) {
                ...
                if (mon->data == &mons[PM_SHADE]) {
                    ...
                    Your("попытка окружить %s безвредна.", mon_nam(mon));
    ...
    boolean wep_was_destroyed)
{
    ...
    case AD_ACID:
        if (mhitb && rn2(2)) {
            if (Blind || !flags.verbose)
                ...
                You("обрызганы %s %s!", s_suffix(mon_nam(mon)),
                    hliquid("acid"));
    ...
    case AD_MAGM:
        ...
        if (Antimagic) {
            ...
            pline("Град магических снарядов едва не попадает в вас!");
        } else {
            You("поражены магическими снарядами, появившимися из воздуха!");
            mdamageu(mon, tmp);
    ...
    if (malive && !mon->mcan && rn2(3)) {
        ...
        case AD_PLYS:
            if (ptr == &mons[PM_FLOATING_EYE]) {
                ...
                if (mon->mcansee) {
                    if (ureflects("%s взгляд отражается вашим %s.",
                                  s_suffix(Monnam(mon)))) {
                        ;
                        ...
                        pline("%s выглядит %s%s.", Monnam(mon),
                              !rn2(2) ? "" : "довольно ",
                              !rn2(2) ? "оцепеневшим" : "одурманенным");
                    } else if (Free_action) {
                        You("на мгновение застываете под %s взглядом!",
                            s_suffix(mon_nam(mon)));
                    } else {
                        You("заморожены %s взглядом!", s_suffix(mon_nam(mon)));
                        nomul((ACURR(A_WIS) > 12 || rn2(4)) ? -tmp : -127);
                        /* set gm.multi_reason;
                           3.6.x used "frozen by a monster's gaze" */
                        dynamic_multi_reason(mon, "заморожен", TRUE);
                        gn.nomovemsg = 0;
                    }
                } else {
                    pline("%s не может защищаться.",
                          Adjmonnam(mon, "ослеплённый"));
                    if (!rn2(500))
                        change_luck(-1);
                }
            } else if (Free_action) {
                ...
                You("заморожены %s!", mon_nam(mon));
                gn.nomovemsg = You_can_move_again;
                nomul(-tmp);
                /* set gm.multi_reason;
                   3.6.x used "frozen by a monster"; be more specific */
                dynamic_multi_reason(mon, "заморожен", FALSE);
                exercise(A_DEX, FALSE);
            }
            break;
        case AD_COLD: /* brown mold or blue jelly */
            if (monnear(mon, u.ux, u.uy)) {
                if (Cold_resistance) {
                    shieldeff(u.ux, u.uy);
                    You_feel("лёгкий холодок.");
                    monstseesu(M_SEEN_COLD);
                    ugolemeffects(AD_COLD, tmp);
                    break;
                }
                monstunseesu(M_SEEN_COLD);
                You("внезапно становится очень холодно!");
                mdamageu(mon, tmp);
        ...
        case AD_FIRE:
            if (monnear(mon, u.ux, u.uy)) {
                if (Fire_resistance) {
                    shieldeff(u.ux, u.uy);
                    You_feel("лёгкое тепло.");
                    monstseesu(M_SEEN_FIRE);
                    ugolemeffects(AD_FIRE, tmp);
                    break;
                }
                monstunseesu(M_SEEN_FIRE);
                You("внезапно становится очень жарко!");
        ...
        case AD_ELEC:
            if (Shock_resistance) {
                shieldeff(u.ux, u.uy);
                You_feel("лёгкое покалывание.");
                monstseesu(M_SEEN_ELEC);
                ugolemeffects(AD_ELEC, tmp);
                break;
            }
            monstunseesu(M_SEEN_ELEC);
            You("поражены электричеством!");
            mdamageu(mon, tmp);
            break;
        default:
            break;
    ...
    if (Blind) {
        ...
        if (glyph_is_cmap(glyph)) {
            ...
            if (M_AP_TYPE(mtmp) == M_AP_FURNITURE
                ...
                Snprintf(fmtbuf, sizeof fmtbuf, "Этот %s на самом деле %%s!",
            ...
            otmp_name = (otmp && otmp->otyp != STRANGE_OBJECT)
                        ? simpleonames(otmp) : "странный предмет";
            Snprintf(fmtbuf, sizeof fmtbuf, "%s %s %s %%s!",
                     (otmp && is_plural(otmp)) ? "Те" : "Этот",
                     otmp_name, otmp ? otense(otmp, "—") : "—");
            if (fakeobj && otmp) {
            ...
            Snprintf(fmtbuf, sizeof fmtbuf,
                     "Подождите! Этот %s на самом деле %%s!", mtmp_name);
        }
        ...
        if (mtmp->minvis && !See_invisible)
            ...
            what = x_monnam(mtmp, ARTICLE_A, "спящий", 0, FALSE);
        else
            what = a_monnam(mtmp);
    }
...
nohandglow(struct monst *mon)
{
    ...
    if (u.umconf == 1) {
        if (altfeedback)
            Your("%s перестают покалывать.", hands);
        else
            Your("%s перестают светиться %s.", hands, hcolor(NH_RED));
    } else {
        if (altfeedback)
            pline("Покалывание в вашей %s ослабевает.", hands);
        else
            Your("%s больше не светятся так ярко %s.", hands, hcolor(NH_RED));
    }
    u.umconf--;
}
    ...
    struct obj *otmp) /* source of flash */
{
    ...
    if (M_AP_TYPE(mtmp) != M_AP_NOTHING) {
        ...
        if (glyph_at(mx, my) != oldglyph) {
            pline("Этот %s на самом деле %s%c", whatbuf,
                  /* y_monnam()+a_monnam() */
    ...
    if (mtmp->msleeping && haseyes(mtmp->data)) {
        mtmp->msleeping = 0;
        if (useeit) {
            pline("Вспышка будит %s.", mon_nam(mtmp));
            res = 1;
        }
        ...
        if (!resists_blnd(mtmp)) {
            ...
            if (useeit) {
                pline("%s ослеплён вспышкой!", Monnam(mtmp));
                res = 1;
            }
            ...
            if (flags.verbose) {
                if (lev->lit)
                    pline("Вспышка света озаряет %s.", mon_nam(mtmp));
                else
                    pline("%s освещён.", Monnam(mtmp));
...
light_hits_gremlin(struct monst *mon, int dmg)
{
    if (!Deaf && mdistu(mon) <= 90) {
        ...
        pline_mon(mon, "%s %s!", Monnam(mon),
                  (dmg > mon->mhp / 2) ? "воет от муки"
                                       : "кричит от боли");
    } else if (canseemon(mon)) {
        pline_mon(mon, "%s отшатывается от света!", Monnam(mon));
    }
    mon->mhp -= dmg;
