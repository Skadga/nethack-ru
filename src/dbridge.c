...
{
    return is_u(etmp) ? "вы" : mon_nam(etmp->emon);
}
    ...
    if (is_u(etmp)) {
        if (how == DROWNING) {
            ...
            if (!svk.killer.name[0]) {
                ...
                Strcpy(svk.killer.name, "падающим разводным мостом");
            }
            done(how);
            /* So, you didn't die */
            if (!e_survives_at(etmp, etmp->ex, etmp->ey)) {
                if (enexto(&xy, etmp->ex, etmp->ey, etmp->edata)) {
                    pline("%sсила переносит вас прочь...",
                          Hallucination ? "Обычная " : "Странная ");
...
#define mk_corpse(dest) (((dest & XKILL_NOCORPSE) != 0) ? AD_DGST : AD_PHYS)
        ...
        if (!DEADMONSTER(etmp->emon)) {
            ...
            if (DEADMONSTER(etmp->emon)) {
                if (seeit)
                    pline("К сожалению, %s всё ещё раздавлен.",
                          mon_nam(etmp->emon));
            } else {
...
do_entity(struct entity *etmp)
{
    ...
    if (automiss(etmp) && e_survives_at(etmp, oldx, oldy)) {
        if (e_inview && (at_portcullis || IS_DRAWBRIDGE(crm->typ))) {
            if (is_u(etmp))
                You("проходите сквозь %s!",
                    at_portcullis ? "подъёмную решётку" : "разводной мост");
            else
                pline("%s проходит сквозь %s!", Monnam(etmp->emon),
                      at_portcullis ? "подъёмную решётку" : "разводной мост");
        }
        if (is_u(etmp))
            spoteffects(FALSE);
    ...
    if (e_missed(etmp, FALSE)) {
        if (at_portcullis) {
            if (is_u(etmp))
                You("не пострадаете от подъёмной решётки.");
            else
                pline("%s не пострадал от подъёмной решётки.",
                      Monnam(etmp->emon));
        } else {
        ...
        if (crm->typ == DRAWBRIDGE_DOWN) {
            if (is_u(etmp)) {
                ...
                Strcpy(svk.killer.name,
                       "раздавлен под подъёмным мостом");
            }
            if (is_u(etmp))
                pline("Вы раздавлены под подъёмным мостом.");
            else
                pline("%s раздавлен под подъёмным мостом.",
                      Monnam(etmp->emon));            /* no jump */
            e_died(etmp,
    ...
    if (must_jump) {
        if (at_portcullis) {
            if (e_jumps(etmp)) {
                ...
                if (e_inview) {
                    if (is_u(etmp))
                        pline("Вы раздавлены падающей решёткой!");
                    else
                        pline("%s раздавлен падающей решёткой!",
                              Monnam(etmp->emon));
                } else if (!Deaf) {
                    Soundeffect(se_crushing_sound, 100);
                    You_hear("дробящий звук.");
                }
                e_died(etmp,
    ...
    if (is_db_wall(etmp->ex, etmp->ey)) {
        ...
        if (e_inview) {
            if (is_u(etmp)) {
                You("кувыркаетесь к закрытой решётке!");
                if (automiss(etmp))
                    You("проходите сквозь неё!");
                else
                    pline("Подъёмный мост закрывается...");
            } else
                if (is_u(etmp))
                    You("исчезаете за разводным мостом.");
                else
                    pline("%s исчезает за разводным мостом.",
                          Monnam(etmp->emon));
        }
        if (!e_survives_at(etmp, etmp->ex, etmp->ey)) {
            svk.killer.format = KILLED_BY_AN;
            Strcpy(svk.killer.name, "закрывающимся разводным мостом");
        ...
        if (e_survives_at(etmp, etmp->ex, etmp->ey)) {
            if (e_inview && !is_flyer(etmp->edata)
                && !is_floater(etmp->edata))
                if (is_u(etmp))
                    pline("Вы падаете с моста.");
                else
                    pline("%s падает с моста.", Monnam(etmp->emon));
            return;
        }
        ...
        if (is_pool(etmp->ex, etmp->ey) || is_lava(etmp->ex, etmp->ey))
            if (e_inview && !is_u(etmp)) {
                ...
                if (Hallucination)
                    if (is_u(etmp))
                        pline("Вы глотаете %s и исчезаете.",
                              lava ? "лаву" : "ров");
                    else
                        pline("%s глотает %s и исчезает.",
                              Monnam(etmp->emon), lava ? "лаву" : "ров");
                else
                    if (is_u(etmp))
                        pline("Вы падаете в %s.",
                              lava ? hliquid("lava") : "ров");
                    else
                        pline("%s падает в %s.", Monnam(etmp->emon),
                              lava ? hliquid("lava") : "ров");
            }
        svk.killer.format = NO_KILLER_PREFIX;
        Strcpy(svk.killer.name, "упал с разводного моста");
...
close_drawbridge(coordxy x, coordxy y)
{
    ...
    if (cansee(x, y) || cansee(x2, y2)) {
        ...
        You_hear("лязг цепей и вращение шестерёнок.");
    }
    ...
    if (OBJ_AT(x, y) && !Deaf) {
        ...
        You_hear("разрушение и дробление.");
    }
...
open_drawbridge(coordxy x, coordxy y)
{
    ...
    if (cansee(x, y) || cansee(x2, y2)) {
        You_see("как подъёмный мост %s!",
                (distu(x2, y2) < distu(x, y)) ? "опускается" : "опускается");
    } else { /* "5 gears turn" for castle drawbridge tune */
        Soundeffect(se_gears_turning_chains_rattling, 100);
        You_hear("вращение шестерёнок и лязг цепей.");
    }
...
destroy_drawbridge(coordxy x, coordxy y)
{
    ...
    if ((lev1->drawbridgemask & DB_UNDER) == DB_MOAT
        ...
        if (lev1->typ == DRAWBRIDGE_UP) {
            if (cansee(x2, y2) || u_at(x2, y2))
                pline_The("решётка подъёмного моста падает в %s!",
                          lava ? hliquid("lava") : "ров");
            else
                You_hear("громкий *ВСПЛЕСК*!");  /* Deaf-aware */
        } else {
            if (cansee(x, y) || u_at(x, y))
                pline_The("подъёмный мост обрушивается в %s!",
                          lava ? hliquid("lava") : "ров");
            else
                You_hear("громкий *ВСПЛЕСК*!");  /* Deaf-aware */
        }
        ...
        if (cansee(x, y) || u_at(x, y))
            pline("Подъёмный мост распадается!");
        else
            You_hear("громкий *ГРОХОТ*!");  /* Deaf-aware */
    ...
    if (etmp2->edata) {
        ...
        if (!automiss(etmp2)) {
            if (e_inview)
                if (is_u(etmp2))
                    You("разорваны в клочья летящими обломками.");
                else
                    pline("%s разорван в клочья летящими обломками.",
                          Monnam(etmp2->emon));
            svk.killer.format = KILLED_BY_AN;
            Strcpy(svk.killer.name, "взрывающимся разводным мостом");
            e_died(etmp2,
                   XKILL_NOCORPSE | (e_inview ? XKILL_GIVEMSG : XKILL_NOMSG),
                   CRUSHING); /*no corpse*/
        ...
        if (e_missed(etmp1, TRUE)) {
            ...
            if (e_inview) {
                if (!is_u(etmp1) && Hallucination)
                    pline("%s врезается во что-то из тяжёлого металла!",
                          Monnam(etmp1->emon));
                else
                    if (is_u(etmp1))
                        You("ударены огромным куском металла!");
                    else
                        pline("%s ударен огромным куском металла!",
                              Monnam(etmp1->emon));
            } else {
            ...
            svk.killer.format = KILLED_BY_AN;
            Strcpy(svk.killer.name, "рушащимся разводным мостом");
            e_died(etmp1,
                   XKILL_NOCORPSE | (e_inview ? XKILL_GIVEMSG : XKILL_NOMSG),
                   CRUSHING); /*no corpse*/
