...
/*    int mndx = monsndx(mdat); */
    ...
    case PM_BLACK_UNICORN:
        if (mtmp->mrevived && rn2(2)) {
            if (canseemon(mtmp))
                pline_mon(mtmp,
                      "%s недавно отросший рог рассыпается в пыль.",
...
minliquid_core(struct monst *mtmp)
{
    ...
    if (mtmp->data == &mons[PM_GREMLIN] && (inpool || infountain) && rn2(3)) {
        ...
        if (cansee(mtmp->mx, mtmp->my))
            pline_mon(mtmp, "%s ржавеет.", Monnam(mtmp));
        mtmp->mhp -= dam;
        if (mtmp->mhpmax > dam)
            mtmp->mhpmax -= dam;
        if (DEADMONSTER(mtmp)) {
    ...
    if (inlava) {
        ...
        if (!is_clinger(mtmp->data) && !likes_lava(mtmp->data)) {
            ...
            if (!resists_fire(mtmp)) {
                ...
                if (DEADMONSTER(mtmp)) {
                    if (cansee(mtmp->mx, mtmp->my))
                        pline_mon(mtmp, "%s сдаётся огню.",
                                  Monnam(mtmp));
                    mondead(mtmp); /* no corpse */
                } else if (cansee(mtmp->mx, mtmp->my)) {
                    pline_mon(mtmp, "%s слегка горит.", Monnam(mtmp));
                }
            }
            if (!DEADMONSTER(mtmp)) {
                if (m_in_air(mtmp)) {
        ...
        if ((waterwall || !is_clinger(mtmp->data))
            ...
            if (cansee(mtmp->mx, mtmp->my)) {
                if (svc.context.mon_moving)
                    pline_mon(mtmp, "%s тонет.", Monnam(mtmp));
                else
                    /* hero used fire to melt ice that monster was on */
                    You("топите %s.", mon_nam(mtmp));
            }
            if (engulfing_u(mtmp)) {
                /* This can happen after a purple worm plucks you off a
                   flying steed while you are over water. */
                pline("%s тонет, пока %s врывается и вымывает вас наружу.",
...
meatbox(struct monst *mon, struct obj *otmp)
{
    ...
    if (!engulf_contents && cansee(x, y)) {
        pline("%s содержимое высыпается на %s.",
    ...
    if (otmp == uball) {
        ...
        if (mstone) {
            if (poly_when_stoned(mtmp->data)) {
                ...
                if (vis)
                    pline_mon(mtmp, "%s превращается в камень!",
                              Monnam(mtmp));
                monstone(mtmp);
            }
        }
        ...
        if (is_metallic(otmp) && !obj_resists(otmp, 5, 95)
            ...
            if (mtmp->data == &mons[PM_RUST_MONSTER] && otmp->oerodeproof) {
                ...
                if (vis) {
                    ...
                    otmpname = distant_name(otmp, doname);
                    if (flags.verbose)
                        pline_mon(mtmp, "%s с отвращением выплёвывает %s!",
                              Monnam(mtmp), otmpname);
                }
            } else {
                if (cansee(mtmp->mx, mtmp->my)) {
        ...
        /* lastly, edible items; yum! */
            ...
            if (cansee(mtmp->mx, mtmp->my)) {
                ...
                if (otmp->oclass == SCROLL_CLASS
                    ...
                    pline("Ммм%c", otmp->blessed ? '!' : '.');
            } else {
                Soundeffect(se_slurping_sound, 30);
                if (flags.verbose)
                    You_hear("звук чавканья.");
            }
            m_consume_obj(mtmp, otmp);
...
lifesaved_monster(struct monst *mtmp)
{
    ...
    if (lifesave) {
        ...
        if (cansee(mtmp->mx, mtmp->my)) {
            pline("Но подождите...");
            pline("%s медальон начинает светиться!", s_suffix(Monnam(mtmp)));
            makeknown(AMULET_OF_LIFE_SAVING);
            /* amulet is visible, but monster might not be */
            if (canseemon(mtmp)) {
                if (attacktype(mtmp->data, AT_EXPL)
                    || attacktype(mtmp->data, AT_BOOM))
                    pline("%s воссоздаётся!", Monnam(mtmp));
                else
                    pline("%s выглядит гораздо лучше!", Monnam(mtmp));
            }
            pline("Медальон рассыпается в пыль!");
        }
        ...
        if (!surviver) {
            ...
            if (cansee(mtmp->mx, mtmp->my))
                pline("К сожалению, %s всё ещё истреблён...",
                      mon_nam(mtmp));
            mtmp->mhp = 0;
        }
    }
...
vamprises(struct monst *mtmp)
{
    ...
    if (ismnum(mndx) && mndx != monsndx(mtmp->data)
        ...
        if (closed_door(x, y)) {
            ...
            if (!seeit)
                You_hear("%s.", trapped ? "взрыв" : door_smashed);
            else if (!canspotmon(mtmp))
                You_see("%s.", trapped ? door_go_boom : door_smashed);
            else if (!Unaware)
                pline("Дверь разбита%s",
            ...
            if (trapped) {
                ...
                if (trap_killed && canspotmon(mtmp) && !Unaware)
                    pline_mon(mtmp, "%s уничтожен!", Monnam(mtmp));
            }
        }
        newsym(x, y);
        return TRUE;
...
logdeadmon(struct monst *mtmp, int mndx)
{
    ...
    if (mndx == PM_MEDUSA && howmany == 1) {
        ...
        if (howmany <= 3 || howmany == 5 || howmany == 10 || howmany == 25
            ...
            mkilled = nonliving(mtmp->data) ? "уничтожен" : "убит";
            ...
            if (herodidit)
                ...
                livelog_printf(llevent_type, "%s%s был %s%s",
...
mondead(struct monst *mtmp)
{
    ...
    if (be_sad)
        You("ненадолго испытываете грусть, и она проходит.");
    ...
    boolean was_swallowed) /* digestion */
{
    ...
    if (mdat == &mons[PM_VLAD_THE_IMPALER] || mdat->mlet == S_LICH) {
        if (cansee(mon->mx, mon->my) && !was_swallowed)
            pline_mon(mon, "%s тело рассыпается в пыль.",
                      s_suffix(Monnam(mon)));
        return FALSE;
    }
            ...
            if (was_swallowed && magr) {
                ...
                if (magr == &gy.youmonst) {
                    ...
                    if (DEADMONSTER(magr)) { /* maybe lifesaved */
                        if (canspotmon(magr))
                            pline_mon(magr, "%s разрывается!", Monnam(magr));
                    } else if (canseemon(magr))
                        pline_mon(magr, "%s словно страдает несварением.",
                                  Monnam(magr));
                }
                return FALSE;
            }
...
monstone(struct monst *mdef)
{
    ...
    if (wasinside) {
        if (digests(mdef->data))
            You("%s через отверстие в новом %s.",
    ...
    int how)
{
    ...
    if (fltxt && (mdef->wormno ? worm_known(mdef)
        ...
        pline_mon(mdef, "%s %s%s%s!", Monnam(mdef),
              nonliving(mptr) ? "уничтожен" : "убит",
              *fltxt ? " от " : "", fltxt);
    else
        ...
        const char *rxt = (how == AD_FIRE && completelyburns(mptr)) ? "roast"
                          : (how == AD_RUST && completelyrusts(mptr)) ? "rust"
                            : (how == AD_DCAY && completelyrots(mptr)) ? "rot"
                              :  0;
        if (rxt)
            pline("Пусть %s %s с миром.", noit_mon_nam(mdef), rxt);
    }
    return;
}
    ...
    int xkill_flags) /* 1: suppress mesg, 2: suppress corpse, 4: pacifist */
{
    ...
    if (!nomsg) {
        ...
        You("%s %s!",
            nonliving(mtmp->data) ? "уничтожаете" : "убиваете",
    ...
    if (accessible(x, y) || is_pool(x, y)) {
        ...
        if (!wasinside && corpse_chance(mtmp, (struct monst *) 0, FALSE)) {
            ...
            if (burycorpse && cadaver && cansee(x, y) && !mtmp->minvis
                ...
                pline("%s труп оказывается похоронен.", s_suffix(Monnam(mtmp)));
            }
        }
    }
 ...
 cleanup:
    ...
    if (mtmp->m_id == svq.quest_status.leader_m_id) { /* REAL BAD! */
        ...
        change_luck(-4);
        if (!Hallucination)
            pline("Вероятно, это была плохая идея...");
        else
            pline("Ой-ой!");
        ...
        if (!Hallucination) {
            ...
            You_hear("рокот далёкого грома...");
        } else {
            Soundeffect(se_applause, 40);
            You_hear("аплодисменты студийной аудитории!");
        }
        if (!unique_corpstat(mdat)) {
            boolean mname = has_mgivenname(mtmp);

            livelog_printf(LL_KILLEDPET, "убил %s%s%s верного %s",
...
mon_to_stone(struct monst *mtmp)
{
    if (mtmp->data->mlet == S_GOLEM) {
        ...
        if (canseemon(mtmp))
            pline_mon(mtmp, "%s затвердевает...", Monnam(mtmp));
        if (newcham(mtmp, &mons[PM_STONE_GOLEM], NO_NC_FLAGS)) {
            if (canseemon(mtmp))
                pline("Теперь это %s.", an(pmname(mtmp->data, Mgender(mtmp))));
        } else {
            if (canseemon(mtmp))
                pline("... и возвращается к норме.");
        }
    } else
...
vamp_stone(struct monst *mtmp)
{
    if (is_vampshifter(mtmp)) {
        ...
        if (mndx >= LOW_PM && mndx != monsndx(mtmp->data)
            ...
            if (canspotmon(mtmp)) {
                pline_mon(mtmp,
                      "%s восстаёт из %s с обновлённой ловкостью!",
                      Amonnam(mtmp), surface_ru(mtmp->mx, mtmp->my));
            }
            newsym(mtmp->mx, mtmp->my);
...
{
    if (!Deaf) {
        pline("%s пронзительно кричит.", Monnam(mtmp));
        stop_occupation();
    }
            ...
            if (humanoid(mon->data) || mon->isshk || mon->ispriest) {
                if (is_watch(mon->data)) {
                    ...
                    if (mon->mtame) {
                        ...
                        adjalign(-1);
                        if (!exclaimed)
                            pline_mon(mon, "%s злится!", Monnam(mon));
                    }
                }
                ...
                if (rn2(6)) {
                    ...
                    if (exclaimed && !alreadyfleeing)
                        ...
                        pline("И затем обращается в бегство.");
                }
            }
        }
    }
...
setmangry(struct monst *mtmp, boolean via_attack)
{
    if (via_attack && sengr_at("Elbereth", u.ux, u.uy, TRUE)
        ...
        if (!Blind)
            pline("Гравировка у вас под ногами угасает.");
    ...
    if (humanoid(mtmp->data) || mtmp->isshk || mtmp->isgd) {
        if (couldsee(mtmp->mx, mtmp->my))
            pline_mon(mtmp, "%s злится!", Monnam(mtmp));
    } else {
        growl(mtmp);
    }
    ...
    if (heal) {
        if (healmon(mon, heal, 0)) {
            if (cansee(mon->mx, mon->my))
                pline_mon(mon, "%s выглядит здоровее.", Monnam(mon));
        }
    }
}
...
mimic_hit_msg(struct monst *mtmp, short otyp)
{
    ...
    case M_AP_OBJECT:
        if (otyp == SPE_HEALING || otyp == SPE_EXTRA_HEALING) {
            pline_mon(mtmp, "%s кажется более ярким %s, чем раньше.",
...
usmellmon(struct permonst *mdat)
{
    ...
    if (mdat) {
        ...
        case PM_ROTHE:
        case PM_MINOTAUR:
            You("замечаете коровий запах.");
            msg_given = TRUE;
            break;
        case PM_CAVE_DWELLER:
        case PM_BARBARIAN:
        case PM_NEANDERTHAL:
            You("чувствуете запах пота.");
            msg_given = TRUE;
            break;
        /*
        case PM_PESTILENCE:
        ...
        */
        ...
        case PM_WEREWOLF:
        case PM_OWLBEAR:
            You("чувствуете запах, напоминающий логово животного.");
            msg_given = TRUE;
            break;
        /*
        case PM_PURPLE_WORM:
        ...
        */
        case PM_STEAM_VORTEX:
            You("чувствуете запах пара.");
            msg_given = TRUE;
            break;
        case PM_GREEN_SLIME:
            pline("%s воняет.", Something);
            msg_given = TRUE;
            break;
        case PM_VIOLET_FUNGUS:
        case PM_SHRIEKER:
            You("чувствуете запах грибов.");
            msg_given = TRUE;
            break;
        ...
        if (nonspecific)
            switch (mdat->mlet) {
            case S_DOG:
                You("замечаете собачий запах.");
                msg_given = TRUE;
                break;
            case S_DRAGON:
                You("чувствуете запах дракона!");
                msg_given = TRUE;
                break;
            case S_FUNGUS:
                pline("%s пахнет плесенью.", Something);
                msg_given = TRUE;
                break;
            case S_UNICORN:
                ...
                break;
            case S_ZOMBIE:
                You("чувствуете запах гниющего мяса.");
                msg_given = TRUE;
                break;
            case S_EEL:
                You("чувствуете запах рыбы.");
                msg_given = TRUE;
                break;
            case S_ORC:
                if (maybe_polyd(is_orc(gy.youmonst.data), Race_if(PM_ORC)))
                    You("замечаете приятный запах.");
                else
                    pline("Зловонная вонь вызывает лёгкую тошноту.");
                msg_given = TRUE;
                break;
            default:
                break;
...
shieldeff_mon(struct monst *mtmp)
{
    ...
    if (cansee(mtmp->mx, mtmp->my))
        pline_mon(mtmp, "%s сопротивляется!", Monnam(mtmp));
}
