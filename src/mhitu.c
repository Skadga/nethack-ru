...
hitmsg(struct monst *mtmp, struct attack *mattk)
{
    ...
    if ((compat = could_seduce(mtmp, &gy.youmonst, mattk)) != 0
        ...
        pline_mon(mtmp, "%s %s %s.", Monst_name,
              !Blind ? "улыбается вам" : !Deaf ? "говорит вам" : "касается вас",
              (compat == 2) ? "по-дружески" : "соблазнительно");
    } else {
        switch (mattk->aatyp) {
        case AT_BITE:
            verb = "кусает вас";
        ...
        again = (mtmp->m_id == gh.hitmsg_mid
                 ...
                 && mattk->aatyp == gh.hitmsg_prev->aatyp) ? " ещё раз" : "";
...
missmu(struct monst *mtmp, boolean nearmiss, struct attack *mattk)
{
    ...
    if (could_seduce(mtmp, &gy.youmonst, mattk) && !mtmp->mcan)
        pline_mon(mtmp, "%s притворяется дружелюбным.", Monnam(mtmp));
    else
        pline_mon(mtmp, "%s %sпромахивается!", Monnam(mtmp),
                  (nearmiss && flags.verbose) ? "чуть " : "");
...
u_slow_down(void)
{
    ...
    if (!Fast)
        ...
        Your("скорость кажется менее естественной.");
...
wildmiss(struct monst *mtmp, struct attack *mattk)
{
    ...
    if (unotseen) { /* !mtmp->cansee || (Invis && !perceives(mtmp->data)) */
        const char *swings = (mattk->aatyp == AT_BITE) ? "щёлкает"
                             : (mattk->aatyp == AT_KICK) ? "лягается"
                               : (mattk->aatyp == AT_STNG
                                  || mattk->aatyp == AT_BUTT
                                  || nolimbs(mtmp->data)) ? "бросается"
                                 : "размахивается";

        if (compat) {
            pline("%s пытается коснуться вас, но промахивается!", Monst_name);
        } else {
            switch (rn2(3)) {
            case 0:
                pline("%s %s вхолостую и промахивается!", Monst_name, swings);
                break;
            case 1:
                pline("%s атакует место рядом с вами.", Monst_name);
                break;
            case 2:
                pline("%s бьёт в %s!", Monst_name,
                      is_waterwall(mtmp->mux,mtmp->muy)
                        ? "пустую воду"
                        : "пустой воздух");
                break;
            default:
                pline("%s %s вхолостую!", Monst_name, swings);
                break;
            }
        }
        ...
        if (compat)
            pline("%s %s улыбается вашему %sсмещённому образу...", Monst_name,
                  (compat == 2) ? "обворожительно" : "соблазнительно",
                  Invis ? "невидимому " : "");
        else
            pline("%s бьёт по вашему %sсмещённому образу и промахивается!",
                  ...
                  Monst_name, Invis ? "невидимому " : "");

    } else if (usubmerged) { /* Underwater */
        /* monsters may miss especially on water level where
           bubbles shake the player here and there */
        if (compat)
            pline("%s тянется к вашему искажённому отражению.", Monst_name);
        else
            pline("%s обманут отражениями на воде и промахивается!",
                  Monst_name);
    ...
    boolean message)
{
        ...
        if (digests(mdat)) {
            ...
            if (!attk) {
                ...
                if (is_whirly(mdat)) {
                    switch (attk->adtyp) {
                    case AD_ELEC:
                        Strcpy(blast, " в потоке искр");
                        break;
                    case AD_COLD:
                        Strcpy(blast, " в порыве мороза");
                        break;
                    }
                } else {
    ...
    if (um_dist(mtmp->mx, mtmp->my, 1))
        pline("Брруааа...  Вы с глухим стуком приземляетесь на расстоянии.");
    spoteffects(TRUE);
}
...
mattacku(struct monst *mtmp)
{
    ...
    if (u.uundetected && !range2 && foundyou && !u.uswallow) {
        ...
        if (is_hider(gy.youmonst.data) && u.umonnum != PM_TRAPPER) {
            ...
            You("падаете с %s!", ceiling(u.ux, u.uy));
            ...
            if (!enexto(&cc, u.ux, u.uy, gy.youmonst.data)
                ...
                pline_mon(mtmp, "%s отпрянул, когда вы упали!", Monnam(mtmp));
                return 0;
            }
            ...
            if (hard_helmet(obj)) {
                Your("удар соскальзывает с %s %s.", s_suffix(mon_nam(mtmp)),
                     helm_simple_name(obj));
            } else {
                if (3 + find_mac(mtmp) <= rnd(20)) {
                    pline("%s поражён падающим колючником (это вы)!",
                          Monnam(mtmp));
                    if ((mtmp->mhp -= d(3, 6)) < 1)
                        killed(mtmp);
                } else
                    pline("%s чуть не поражён падающим колючником (это вы)!",
                          Monnam(mtmp));
            }
            ...
            /* surface hider */
            if (!youseeit) {
                pline("Оно пытается сдвинуться туда, где вы прячетесь.");
            } else {
                ...
                if (obj || u.umonnum == PM_TRAPPER
                    ...
                    if (gy.youmonst.data->mlet == S_EEL
                        ...
                        pline(
                             "Подожди, %s!  Здесь прячется скрытый %s по имени %s!",
                              m_monnam(mtmp),
                              pmname(gy.youmonst.data, Ugender), svp.plname);
                    else
                        pline(
                          "Подожди, %s!  Здесь прячется скрытый %s по имени %s под %s!",
                              m_monnam(mtmp),
    ...
    if (gy.youmonst.data->mlet == S_MIMIC && U_AP_TYPE && !range2
        ...
        if (!canspotmon(mtmp))
            map_invisible(mtmp->mx, mtmp->my);
        if (sticky && !youseeit)
            pline("Оно застревает на вас.");
        else /* see note about m_monnam() above */
            pline("Подожди, %s!  Это %s по имени %s!", m_monnam(mtmp),
                  pmname(gy.youmonst.data, Ugender), svp.plname);
        if (sticky)
            set_ustuck(mtmp);
    ...
    if (U_AP_TYPE == M_AP_OBJECT && !range2 && foundyou && !u.uswallow) {
        ...
        if (!youseeit)
            pline("%s %s!", Something,
                  ...
                  ? "пытается вас поднять"
                  : "тревожит вас");
        else /* see note about m_monnam() above */
            pline("Подожди, %s!  Этот %s на самом деле %s по имени %s!", m_monnam(mtmp),
                  mimic_obj_name(&gy.youmonst),
                  an(pmname(&mons[u.umonnum], Ugender)), svp.plname);
        if (gm.multi < 0) { /* this should always be the case */
            char buf[BUFSZ];

            Sprintf(buf, "Вы снова выглядите как %s.",
    ...
    if (u.uinvulnerable) { /* in the midst of successful prayer */
        ...
        if (mtmp == u.ustuck) {
            pline_mon(mtmp, "%s немного ослабляет хватку.", Monnam(mtmp));
        } else if (!range2) {
            if (youseeit || sensemon(mtmp))
                pline("%s начинает атаковать вас, но отступает.",
                      Monnam(mtmp));
            else
                You_feel("рядом %s движется.", something);
        }
        return 0;
    }
    ...
    for (i = 0; i < NATTK; i++) {
        ...
        case AT_ENGL:
            if (!range2) {
                if (foundyou) {
                    ...
                    pline_mon(mtmp, "%s глотает воздух!", Monnam(mtmp));
                } else {
                    if (youseeit) {
                        pline_mon(mtmp, "%s бросается вперёд и отшатывается!",
                                  Monnam(mtmp));
                    } else {
                        if (is_whirly(mtmp->data)) {
                            Soundeffect(se_rushing_wind_noise, 60);
                        }
                        You_hear("%s неподалёку.",
                                 is_whirly(mtmp->data) ? "шум ветра"
                                                       : "шлепок");
                    }
                }
            }
        ...
        if (sum[i] == M_ATTK_HIT) { /* successful attack */
            if (u.usleep && u.usleep < svm.moves && !rn2(10)) {
                gm.multi = -1;
                gn.nomovemsg = "Бой внезапно пробуждает вас.";
            }
        }
...
summonmu(struct monst *mtmp, boolean youseeit)
{
    ...
    if (is_were(mdat)) {
        ...
        if (!rn2(10)) {
            ...
            Strcpy(genericwere, "Существо");
            if (youseeit)
                pline_mon(mtmp, "%s призывает подмогу!", Monnam(mtmp));
            numhelp = were_summon(mdat, FALSE, &numseen, genericwere);
            if (youseeit) {
                if (numhelp > 0) {
                    if (numseen == 0)
                        You_feel("себя зажатым.");
                } else {
                    pline("Но никто не приходит.");
                }
            } else {
                ...
                if (numhelp > 0) {
                    if (numseen < 1) {
                        ...
                        if (numseen == 1)
                            Sprintf(buf, "%s появляется", genericwere);
                        else
                            Sprintf(buf, "%s появляются",
    ...
    struct attack *mattk)
{
    ...
    if (obj && (obj->greased || obj->otyp == OILSKIN_CLOAK)
        && (!obj->cursed || rn2(3))) {
        pline_mon(mtmp, "%s %s с ваших %s %s!", Monnam(mtmp),
              (mattk->adtyp == AD_WRAP) ? "соскальзывает"
                                        : "хватает вас, но не может удержать",
              obj->greased ? "смазанных" : "скользких",
    ...
    if (mtmp->mundetected && (hides_under(mdat) || mdat->mlet == S_EEL)) {
        ...
        if (!tp_sensemon(mtmp) && !Detect_monsters) {
            ...
            if ((obj = svl.level.objects[mtmp->mx][mtmp->my]) != 0) {
                ...
                pline("%s был спрятан под %s!", Amonbuf, what);
            }
            newsym(mtmp->mx, mtmp->my);
        }
    }
...
gulpmu(struct monst *mtmp, struct attack *mattk)
{
    ...
    if (!u.uswallow) { /* swallows you */
        ...
        if (u.usteed) {
            ...
            urgent_pline("%s бросается вперёд и стаскивает вас с %s!",
                         Some_Monnam(mtmp),
                         buf);
            dismount_steed(DISMOUNT_ENGULFED);
        } else {
            urgent_pline("%s %s!", Monnam(mtmp),
                         digests(mtmp->data) ? "проглатывает вас целиком"
                         : enfolds(mtmp->data) ? "обвивается вокруг вас"
                           : "поглощает вас");
        }
        stop_occupation();
        ...
        if (u.utrap) {
            You("высвобождаетесь из %s!",
                (u.utraptype == TT_WEB) ? "паутины" : "ловушки");
            reset_utrap(FALSE);
        }

        i = number_leashed();
        if (i > 0) {
            const char *s = (i > 1) ? "поводки" : "поводок";

            pline("%s %s.", s, (i > 1) ? "Ослабевают" : "Ослабевает");
            unleash_all();
        }
    ...
    case AD_DGST:
        ...
        if (Slow_digestion) {
            ...
            pline("%s полностью вас переваривает!", Monnam(mtmp));
            tmp = u.uhp;
            if (Half_physical_damage)
                tmp *= 2; /* sorry */
        } else {
            pline("%s%s вас переваривает!", Monnam(mtmp),
                  (u.uswldtim == 2) ? " тщательно"
                                    : (u.uswldtim == 1) ? " полностью" : "");
    ...
    case AD_PHYS:
        ...
        if (mtmp->data == &mons[PM_FOG_CLOUD]) {
            You("пропитаны влагой и %s",
                flaming(gy.youmonst.data)
                    ? "тлеете!"
                    : Breathless ? "чувствуете себя слегка неуютно."
            ...
            You("%s!", enfolds(mtmp->data) ? "раздавлены"
                                               : "избиты обломками");
            exercise(A_STR, FALSE);
        }
        break;
    case AD_ACID:
        if (Acid_resistance) {
            You("покрыты, казалось бы, безвредной слизью.");
            ...
            if (Hallucination)
                pline("Ой!  Вас облепило слизью!");
            else
                You("покрыты слизью!  Она жжётся!");
            exercise(A_STR, FALSE);
    ...
    case AD_BLND:
        if (can_blnd(mtmp, &gy.youmonst, mattk->aatyp, (struct obj *) 0)) {
            if (!Blind) {
                ...
                if (!Blinded)
                    You_cant("здесь ничего не видно!");
    ...
    case AD_ELEC:
        if (!mtmp->mcan && rn2(2)) {
            pline("Воздух вокруг вас потрескивает от электричества.");
            if (Shock_resistance) {
                shieldeff(u.ux, u.uy);
                You("кажется, невредимы.");
    ...
    if (!u.uswallow) {
        ...
        pline("%s в спешке %s вас!", Monnam(mtmp),
              digests(mtmp->data) ? "изрыгает обратно"
              : enfolds(mtmp->data) ? "выпускает"
                : "изгоняет");
        ...
        You("%s!", digests(mtmp->data) ? "изрыгнуты обратно"
                       : enfolds(mtmp->data) ? "выпущены"
                         : "изгнаны");
        if (flags.verbose
            && (digests(mtmp->data) && Slow_digestion))
            pline("Похоже, %s не нравится ваш вкус.", mon_nam(mtmp));
        expels(mtmp, mtmp->data, FALSE);
    }
    return M_ATTK_HIT;
}
    ...
    boolean ufound)
{
    ...
    if (!ufound) {
        pline("%s взрывается в %s!",
              canseemon(mtmp) ? Monnam(mtmp) : "Оно",
              is_waterwall(mtmp->mux,mtmp->muy) ? "пустой воде"
                                                : "пустом воздухе");
    } else {
        hitmsg(mtmp, mattk);
    }
    ...
    case AD_BLND:
        ...
        if (ufound && !not_affected) {
            ...
            if (mon_visible(mtmp) || (rnd(tmp /= 2) > u.ulevel)) {
                You("ослеплены вспышкой света!");
                make_blinded((long) tmp, FALSE);
                if (!Blind)
                    Your1(vision_clears);
            } else if (flags.verbose)
                You("складывается впечатление, что оно было не слишком ярким.");
        }
        break;
    case AD_HALU:
        ...
        if (ufound && !not_affected) {
            boolean chg;
            if (!Hallucination)
                You("пойманы вспышкой калейдоскопического света!");
            ...
            You("%s.", chg ? "вы перепуганы" : "кажется, не затронуты");
        }
        break;
    default:
...
gazemu(struct monst *mtmp, struct attack *mattk)
{
    static const char *const reactions[] = {
        "сбитым с толку",        /* [0] */
        "ошеломлённым",          /* [1] */
        "озадаченным", "ослеплённым", /* [2,3] */
        "раздражённым", "воспалённым", /* [4,5] */
        "усталым",               /* [6] */
        "заторможенным",         /* [7] */
    };
    int react = -1;
    ...
    case AD_STON:
        ...
        if (cancelled || !mtmp->mcansee) {
            ...
            if (is_medusa && Hallucination && !rn2(3))
                pline("Кто-то, похоже, заслуживает разреза на змею.");
            else
                pline_mon(mtmp, "%s %s.", Monnam(mtmp),
                      (is_medusa && mtmp->mcan && !react)
                          ? "выглядит не так уж и уродливо"
                          : "бесполезно взирает");
            break;
         }
        if (reflectable) {
            ...
            if (useeit)
                (void) ureflects("взгляд %s отражается вашим %s.",
                                 s_suffix(Monnam(mtmp)));
            if (mon_reflects(mtmp, !useeit ? (char *) 0
                                  : "Взгляд отражается %s %s!"))
                break;
            if (!m_canseeu(mtmp)) { /* probably you're invisible */
                if (useeit)
                    pline(
                      "%s, похоже, не замечает, что взгляд %s отразился.",
                          Monnam(mtmp), mhis(mtmp));
                break;
            }
            if (useeit)
                pline_mon(mtmp, "%s превращается в камень!", Monnam(mtmp));
            gs.stoned = TRUE;
            killed(mtmp);
        ...
        if (canseemon(mtmp) && couldsee(mtmp->mx, mtmp->my)
            ...
            You("встречаетесь взглядом с %s.", s_suffix(mon_nam(mtmp)));
            stop_occupation();
    ...
    case AD_CONF:
        if (mcanseeu && !mtmp->mspec_used && rn2(5)) {
            if (cancelled) {
                ...
                if (!Confusion)
                    pline_mon(mtmp, "взгляд %s сбивает вас с толку!",
                              s_suffix(Monnam(mtmp)));
                else
                    You("становитесь всё более и более сбитыми с толку.");
    ...
    case AD_STUN:
        if (mcanseeu && !mtmp->mspec_used && rn2(5)) {
            if (cancelled) {
                ...
                pline_mon(mtmp, "%s пристально смотрит на вас!", Monnam(mtmp));
    ...
    case AD_BLND:
        if (canseemon(mtmp) && !resists_blnd(&gy.youmonst)
            ...
            if (cancelled) {
                ...
                You("ослеплены сиянием %s!", s_suffix(mon_nam(mtmp)));
    ...
    case AD_FIRE:
        if (mcanseeu && !mtmp->mspec_used && rn2(5)) {
            if (cancelled) {
                ...
                pline_mon(mtmp, "%s атакует вас огненным взглядом!",
                          Monnam(mtmp));
                stop_occupation();
                if (Fire_resistance) {
                    shieldeff(u.ux, u.uy);
    ...
    case AD_SLEE:
        if (mcanseeu && gm.multi >= 0 && !rn2(5) && !Sleep_resistance) {
            if (cancelled) {
                ...
                pline("взгляд %s навевает на вас сонливость...",
                      s_suffix(Monnam(mtmp)));
    ...
    if (react >= 0) {
        ...
        pline_mon(mtmp, "%s выглядит %s%s.", Monnam(mtmp),
              !rn2(3) ? "" : already ? "довольно "
                                     : (!rn2(2) ? "немного " : "слегка "),
              reactions[react]);
    }
    return M_ATTK_MISS;
}
...
doseduce(struct monst *mon)
{
    ...
    if (mon->mcan || mon->mspec_used) {
        pline_mon(mon, "%s ведёт себя так, будто у %s %sголовная боль.",
                  Monnam(mon), mhe(mon), mon->mcan ? "сильная " : "");
        return 0;
    }
    if (unresponsive()) {
        pline_mon(mon, "%s, похоже, огорчён вашим безмолвием.",
                  Monnam(mon));
        return 0;
    }
    seewho = canseemon(mon);
    if (!seewho)
        pline("Кто-то ласкает вас...");
    else
        You_feel("сильное влечение к %s.", mon_nam(mon));
    /* cache the seducer's name in a local buffer */
    Strcpy(Who, (!seewho ? (fem ? "Она" : "Он") : Monnam(mon)));
    ...
    for (ring = gi.invent; ring; ring = nring) {
        ...
        if (fem) {
            if (ring->owornmask && uarmg) {
                ...
                if (!tried_gloves++)
                    mayberem(mon, Who, uarmg, "перчатки");
                if (uarmg)
                    continue; /* next ring might not be worn */
            }
            /* confirmation prompt when charisma is high bypassed if deaf */
            if (!Deaf && rn2(20) < ACURR(A_CHA)) {
                (void) safe_qbuf(qbuf, "\"Это ",
                                 " мило.  Можно мне его?\"", ring,
                                 xname, simpleonames, "кольцо");
                makeknown(RIN_ADORNMENT);
                SetVoice(mon, 0, 80, 0);
                if (y_n(qbuf) == 'n')
                    continue;
            } else
                pline("%s решает, что ей понравились %s, и забирает их.",
                      Who, yname(ring));
            makeknown(RIN_ADORNMENT);
            ...
            if (!Deaf && rn2(20) < ACURR(A_CHA)) {
                (void) safe_qbuf(qbuf, "\"Это ",
                                " мило.  Не наденешь его для меня?\"",
                                 ring, xname, simpleonames, "кольцо");
                makeknown(RIN_ADORNMENT);
                SetVoice(mon, 0, 80, 0);
                if (y_n(qbuf) == 'n')
                    continue;
            } else {
                pline("%s решает, что вы будете красивее в %s,",
                      Who, yname(ring));
                pline("и надевает его вам на палец.");
            }
            makeknown(RIN_ADORNMENT);
            if (!uright) {
                pline("%s надевает %s на ваш правый %s.",
                      Who, the(xname(ring)), body_part(HAND));
                setworn(ring, RIGHT_RING);
            } else if (!uleft) {
                pline("%s надевает %s на ваш левый %s.",
                      Who, the(xname(ring)), body_part(HAND));
    ...
    urgent_pline("%s %s%s.", Who,
                 Deaf ? "кажется, что-то шепчет вам на ухо"
                 : naked ? "шепчет нежности вам на ухо"
                   : "шепчет вам на ухо",
                 naked ? "" : ", помогая вам раздеться");
    mayberem(mon, Who, uarmc, cloak_simple_name(uarmc));
    if (!uarmc)
        mayberem(mon, Who, uarm, suit_simple_name(uarm));
    mayberem(mon, Who, uarmf, "ботинки");
    if (!tried_gloves)
        mayberem(mon, Who, uarmg, "перчатки");
    mayberem(mon, Who, uarms, "щит");
    mayberem(mon, Who, uarmh, helm_simple_name(uarmh));
    if (!uarmc && !uarm)
        mayberem(mon, Who, uarmu, "рубашку");
    ...
    if (uarm || uarmc) {
        if (!Deaf) {
            if (!(ld() && mon->female)) {
                SetVoice(mon, 0, 80, 0);
                verbalize("Вы такой %s; я бы хотел...",
                          flags.female ? "милашка" : "симпатяга");
            } else {
                ...
                verbalize("Ну, тогда вы должны мне %s%s!",
                          yourgloves ? yname(yourgloves)
                                     : "двенадцать пар перчаток",
                          yourgloves ? " и ещё одиннадцать пар перчаток"
                                     : "");
            }
        } else if (seewho)
            pline_mon(mon, "%s, похоже, вздыхает.", Monnam(mon));
    ...
    urgent_pline(
             "Время замирает, пока вы с %s лежите в объятиях друг друга...",
                 noit_mon_nam(mon));
    ...
    if (rn2(35) > min(attr_tot, 32)) {
        ...
        pline("%s, похоже, получил(а) больше удовольствия, чем вы...",
              noit_Monnam(mon));
        switch (rn2(5)) {
        case 0:
            You_feel("себя опустошённым от энергии.");
            u.uen = 0;
        ...
        case 3:
            if (!resists_drli(&gy.youmonst)) {
                You_feel("себя не в форме.");
                losexp("переутомлением");
            } else {
                You("испытываете любопытное чувство...");
            }
            exercise(A_CON, FALSE);
            exercise(A_DEX, FALSE);
            exercise(A_WIS, FALSE);
        ...
        You("кажется, получили больше удовольствия, чем %s...", noit_mon_nam(mon));
        switch (rn2(5)) {
        case 0:
            You_feel("себя полностью реализованным.");
            exercise(A_CON, TRUE);
            ...
            break;
        case 1:
            You_feel("себя готовым повторить это.");
            (void) adjattrib(A_CON, 1, TRUE);
            exercise(A_CON, TRUE);
            disp.botl = TRUE;
            break;
        case 2:
            You("всегда будете помнить %s...", noit_mon_nam(mon));
            (void) adjattrib(A_WIS, 1, TRUE);
            exercise(A_WIS, TRUE);
            disp.botl = TRUE;
            break;
        case 3:
            pline("Это был очень поучительный опыт.");
            pluslvl(FALSE);
            exercise(A_WIS, TRUE);
            break;
        case 4:
            You_feel("себя восстановленным!");
            u.uhp = u.uhpmax;
            if (Upolyd)
                u.mh = u.mhmax;
            exercise(A_STR, TRUE);
    ...
    if (mon->mtame) { /* don't charge */
        ...
        pline("%s требует, чтобы вы заплатили %s, но вы отказываетесь...",
              noit_Monnam(mon), noit_mhim(mon));
    } else if (u.umonnum == PM_LEPRECHAUN) {
        pline_mon(mon, "%s пытается забрать ваше золото, но безуспешно...",
                  noit_Monnam(mon));
    } else {
        long cost;
        ...
        if (!cost) {
            if (!Deaf) {
                SetVoice(mon, 0, 80, 0);
                verbalize("Это за мой счёт!");
            } else {
                pline("Бесплатно.");
            }
        } else {
            pline_mon(mon, "%s берёт %ld %s за оказанные услуги!",
         ...
         struct obj *obj, const char *str)
{
    ...
    if (Deaf) {
        pline("%s снимает с вас %s.", seducer, str);
    } else if (rn2(20) < ACURR(A_CHA)) {
        SetVoice(mon, 0, 80, 0); /* y_n aka yn_function is set up for this */
        Sprintf(qbuf, "\"Снять с вас %s, %s?\"", str,
                (!rn2(2) ? "любимый" : !rn2(2) ? "дорогой" : "милый"));
        if (y_n(qbuf) == 'n')
            return;
    } else {
        char hairbuf[BUFSZ];

        Sprintf(hairbuf, "дать мне провести пальцами по вашим %s",
                body_part(HAIR));
        SetVoice(mon, 0, 80, 0);
        verbalize("Снимите с себя %s; %s.", str,
                  (obj == uarm)
                     ? "давай будем поближе"
                     : (obj == uarmc || obj == uarms)
                        ? "он мешает"
                        : (obj == uarmf)
                           ? "дай потру твои ноги"
                           : (obj == uarmg)
                              ? "они слишком неуклюжие"
                              : (obj == uarmu)
                                 ? "дай я тебя помассирую"
                                 /* obj == uarmh */
                                 : hairbuf);
    }
...
{
    if ((mtmp->mhp -= tmp) <= 0) {
        pline_mon(mtmp, "%s умирает!", Monnam(mtmp));
    ...
    struct attack *mattk)
{
    ...
    case AD_ACID:
        if (!rn2(2)) {
            pline_mon(mtmp, "%s забрызган %s%s!", Monnam(mtmp),
                  ...
                  !Upolyd ? "" : "вашей ", hliquid("кислота"));
            if (resists_acid(mtmp)) {
                pline_mon(mtmp, "%s не затронут.", Monnam(mtmp));
                tmp = 0;
            }
        } else
            tmp = 0;
    ...
    if (rn2(3))
        ...
        case AD_PLYS: /* Floating eye */
            ...
            if (u.umonnum == PM_FLOATING_EYE) {
                ...
                if (mtmp->mcansee && haseyes(mtmp->data) && rn2(3)
                    ...
                    if (Blind) {
                        pline("Будучи слепым %s, вы не можете защищаться.",
                              pmname(gy.youmonst.data,
                                     flags.female ? FEMALE : MALE));
                    } else {
                        if (mon_reflects(mtmp,
                                         "Взгляд отражается прочь от %s %s."))
                            return 1;
                        pline_mon(mtmp, "%s замирает от вашего взгляда!",
                                  Monnam(mtmp));
                ...
                pline_mon(mtmp, "%s заморожен вами.", Monnam(mtmp));
                paralyze_monst(mtmp, tmp);
                return M_ATTK_AGR_DONE;
            }
            return M_ATTK_HIT;
        case AD_COLD: /* Brown mold or blue jelly */
            if (resists_cold(mtmp)) {
                ...
                pline_mon(mtmp, "%s слегка холоден.", Monnam(mtmp));
                golemeffects(mtmp, AD_COLD, tmp);
                tmp = 0;
                break;
            }
            pline_mon(mtmp, "%s внезапно очень холоден!", Monnam(mtmp));
            u.mh += (tmp + rn2(2)) / 2;
            if (u.mhmax < u.mh)
                u.mhmax = u.mh;
        ...
        case AD_FIRE: /* Red mold */
            if (resists_fire(mtmp)) {
                shieldeff(mtmp->mx, mtmp->my);
                pline_mon(mtmp, "%s слегка согрет.", Monnam(mtmp));
                golemeffects(mtmp, AD_FIRE, tmp);
                tmp = 0;
                break;
            }
            pline_mon(mtmp, "%s внезапно очень горяч!", Monnam(mtmp));
            break;
        case AD_ELEC:
            if (resists_elec(mtmp)) {
                shieldeff(mtmp->mx, mtmp->my);
                pline_mon(mtmp, "%s слегка покалывает.", Monnam(mtmp));
                golemeffects(mtmp, AD_ELEC, tmp);
                tmp = 0;
                break;
            }
            pline_mon(mtmp, "%s поражён вашим электричеством!",
                      Monnam(mtmp));
            break;
        default:
            tmp = 0;
