...
throw_obj(struct obj *obj, int shotlimit)
{
    ...
    if (!canletgo(obj, "бросить")) {
        res = ECMD_OK;
        goto unsplit_stack;
    }
    if (is_art(obj, ART_MJOLLNIR) && obj != uwep) {
        pline("%s должно быть в руках, прежде чем его можно бросить.", The(xname(obj)));
        res = ECMD_OK;
        goto unsplit_stack;
    }
    if ((is_art(obj, ART_MJOLLNIR) && ACURR(A_STR) < STR19(25))
        || (obj->otyp == BOULDER && !throws_rocks(gy.youmonst.data))) {
        pline("Это слишком тяжело.");
        res = ECMD_TIME;
        goto unsplit_stack;
    }
    if (!u.dx && !u.dy && !u.dz) {
        You("не можете бросить предмет в себя.");
        res = ECMD_OK;
        goto unsplit_stack;
    }
    u_wipe_engr(2);
    ...
    if (multishot > 1 || shotlimit > 0) {
        ...
        You("%s %d %s.", gm.m_shot.s ? "выпускаете" : "бросаете",
...
ok_to_throw(int *shotlimit_p) /* (see dothrow()) */
{
    *shotlimit_p = LIMIT_TO_RANGE_INT(0, LARGEST_INT, gc.command_count);
    ...
    if (notake(gy.youmonst.data)) {
        You("физически не способны бросать или стрелять чем-либо.");
        return FALSE;
    } else if (nohands(gy.youmonst.data)) {
        You_cant("бросать или стрелять без рук."); /* not body_part(HAND) */
        return FALSE;
...
dothrow(void)
{
    ...
    obj = getobj("бросить", throw_ok, GETOBJ_PROMPT | GETOBJ_ALLOWCNT);
    ...
    if (uwep_Throw_and_Return && (!obj || is_ammo(obj))) {
        ...
        if (!flags.autoquiver) {
            ...
            /* if we're wielding a bullwhip, apply it */
                ...
                You("нет приготовленных снарядов.");
            }
        } else {
            autoquiver();
            obj = uquiver;
            if (obj) {
                ...
                prinv("Вы приготовили:", obj, 0L);
                uquiver->owornmask |= W_QUIVER;
            } else {
                You("нет ничего подходящего для колчана.");
            }
        }
    }
...
endmultishot(boolean verbose)
{
    if (gm.m_shot.i < gm.m_shot.n) {
        if (verbose && !svc.context.mon_moving) {
            You("остановите %s после %d%s %s.",
                gm.m_shot.s ? "стреляя" : "бросая",
                gm.m_shot.i, ordin(gm.m_shot.i),
                gm.m_shot.s ? "выстрел" : "швыряет");
        }
    ...
    boolean verbosely) /* usually True; False if caller has given drop mesg */
{
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ)) {
        ...
        const char *verb = (obj->otyp == WAN_STRIKING) ? "ударяет" : "попадает";
        ...
        pline("%s %s в %s.", Doname2(obj), otense(obj, verb), surf);
    }
...
hurtle_step(genericptr_t arg, coordxy x, coordxy y)
{
    ...
    if (!isok(x, y)) {
        You_feel("духи, удерживающие вас.");
        return FALSE;
    ...
    if (!Passes_walls || !(may_pass = may_passwall(x, y))) {
        ...
        if (IS_OBSTRUCTED(levl[x][y].typ)
            ...
            why = IS_TREE(ltyp) ? "удар о дерево"
                  : IS_OBSTRUCTED(ltyp) ? "удар о стену"
                    : odoor_diag ? "удар о дверной косяк"
                      : "удар о закрытую дверь";
            if (odoor_diag)
                You("ударились о дверной косяк!");
            pline("Ой!");
        } else if (ltyp == IRONBARS) {
            why = "врезаясь в железные прутья";
            You("врезаетесь в железные прутья.  Ой!");
        } else if ((obj = sobj_at(BOULDER, x, y)) != 0) {
            why = "удар о валун";
            You("ударяетесь о %s.  Ой!", xname(obj));
        }  else if (!may_pass) {
            /* did we hit a no-dig non-wall position? */
            why = "касание края вселенной";
            You("шлёпаетесь во что-то!");
        } else if (diagonal
            ...
            if (bigmonst(gy.youmonst.data) || too_much) {
                why = "застревание в узкой трещине";
                You("%s с силой застреваете в трещине.",
                    too_much ? "и все ваши вещи " : "");
            }
        }
        if (why) {
            dmg = rnd(2 + *range);
    ...
    if ((mon = m_at(x, y)) != 0
        ...
        if (!glyph_is_monster(glyph) && !glyph_is_invisible(glyph))
            You("находите %s, ударяясь о %s.", mnam, noit_mhim(mon));
        else
            You("ударяетесь о %s.", mnam);
        wakeup(mon, FALSE);
        if (!canspotmon(mon))
        ...
        if (touch_petrifies(mon->data)
            ...
            && !uarmu && !uarm && !uarmc) {
            Sprintf(svk.killer.name, "удар о %s",
    ...
    if (is_pool(x, y) && !u.uinwater) {
        if (is_waterwall(x, y) || !(Levitation || Flying || Wwalking)) {
            ...
            Norep("Вы перемещаетесь через %s.", an(is_moat(x, y) ? "ров" : "лужа"));
        }
    } else if (is_lava(x, y) && !stopping_short) {
        Norep("Вы перемещаетесь через лаву.");
    }
    ...
    if ((ttmp = t_at(x, y)) != 0) {
        if (stopping_short) {
            ...
            pline("Земля вибрирует, когда вы проходите.");
            ...
            *range = 0;
            ...
            if (ttmp->tseen)
                You("проходите прямо над %s.", an(trapname(ttmp->ttyp, FALSE)));
        }
    }
...
mhurtle_step(genericptr_t arg, coordxy x, coordxy y)
{
    ...
    if ((mtmp = m_at(x, y)) != 0 && mtmp != mon) {
        if (canseemon(mon) || canseemon(mtmp))
            pline("%s ударяется о %s.", Monnam(mon), a_monnam(mtmp));
        ...
        pline("%s ударяется о вас.", Some_Monnam(mon));
        stop_occupation();
        ...
        if (touch_petrifies(mon->data) && !(uarmu || uarm || uarmc)) {
            Snprintf(svk.killer.name, sizeof svk.killer.name,
                     ...
                     x_monnam(mon, mon->mtame ? ARTICLE_YOUR : ARTICLE_A,
                              "несётся", EXACT_NAME | SUPPRESS_NAME, FALSE));
            instapetrify(svk.killer.name);
            newsym(u.ux, u.uy);
        }
    }
...
hurtle(int dx, int dy, int range, boolean verbose)
{
    ...
    if (Punished && !carried(uball)) {
        You_feel("рывок от железного шара.");
        nomul(0);
        return;
    } else if (u.utrap) {
        You("привязаны к %s.",
            (u.utraptype == TT_WEB) ? "паутина"
            : (u.utraptype == TT_LAVA) ? hliquid("лава")
              : (u.utraptype == TT_INFLOOR) ? surface(u.ux, u.uy)
                : (u.utraptype == TT_BURIEDBALL) ? "зарытый шар"
                  : "ловушка");
        nomul(0);
        return;
    }
    ...
    nomul(-range);
    gm.multi_reason = "движение по воздуху";
    gn.nomovemsg = ""; /* it just happens */
    if (verbose)
        You("%s в противоположном направлении.",
            (range > 1) ? "несётся" : "парит");
...
mhurtle(struct monst *mon, int dx, int dy, int range)
{
    ...
    if (mon->data->msize >= MZ_HUGE || mon == u.ustuck || mon->mtrapped) {
        if (canseemon(mon))
            pline("%s не двигается!", Monnam(mon));
        return;
    }
...
toss_up(struct obj *obj, boolean hitsroof)
{
    ...
    if (!has_ceiling(&u.uz)) {
        action = "взлетает в"; /* into "the sky" or "the water above" */
    } else if (hitsroof) {
        if (breaktest(obj)) {
            pline("%s ударяется о %s.", Doname2(obj), ceiling(u.ux, u.uy));
            breakmsg(obj, !Blind);
        ...
        action = "почти попадает";
    }
    pline("%s %s %s, затем падает обратно на ваш %s.", Doname2(obj),
    ...
    if (obj->oclass == POTION_CLASS) {
        ...
        case EGG:
            if (petrifier && !Stone_resistance
                ...
                if (uarmh)
                    Your("%s не защищает вас.", helm_simple_name(uarmh));
                goto petrify;
            }
            FALLTHROUGH;
            /*FALLTHRU*/
        case CREAM_PIE:
        case BLINDING_VENOM:
            pline("Это всё на вашем %s!", body_part(FACE));
            if (blindinc) {
                if (otyp == BLINDING_VENOM && !Blind)
                    pline("Это ослепляет вас!");
                u.ucreamed += blindinc;
            ...
            if ((less_damage && dmg < (Upolyd ? u.mh : u.uhp)) || harmless) {
                if (!artimsg) {
                    if (!harmless) /* !harmless => less_damage here */
                        pline("К счастью, на вас надет жёсткий шлем.");
                    else
                        pline("К сожалению, на вас надето %s.",
 ...
 petrify:
            ...
            /* what goes up... */
            Strcpy(svk.killer.name, "элементарная физика");
            You("превращается в камень.");
            if (obj)
        ...
        if (is_silver && Hate_silver)
            pline("Серебро обжигает вас!");
        if (harmless)
            hit(thesimpleoname(obj), &gy.youmonst, " но не больно.");
    ...
    struct obj *oldslot) /* for thrown-and-return used with !fixinv */
{
    ...
    if ((obj->cursed || obj->greased) && (u.dx || u.dy) && !rn2(7)) {
        ...
        if (ammo_and_launcher(obj, uwep)) {
            pline("%s!", Tobjnam(obj, "даёт осечку"));
        } else {
            /* only slip if it's greased or meant to be thrown */
            if (obj->greased || throwing_weapon(obj))
                /* BUG: this message is grammatically incorrect if obj has
                   a plural name; greased gloves or boots for instance. */
                pline("%s, когда вы бросаете его!", Tobjnam(obj, "соскальзывает"));
            else
                slipok = FALSE;
        }
        if (slipok) {
    ...
    if ((u.dx || u.dy || (u.dz < 1))
        ...
        && !Is_airlevel(&u.uz)) {
        You("так мало сил, %s выпадает из вашей хватки.",
            the(xname(obj)));
    ...
    if (u.uswallow) {
        ...
        if (u.dz < 0
            ...
            && !impaired) {
            pline("%s %s и возвращается в вашу руку!", Tobjnam(obj, "попадает"),
                  ceiling(u.ux, u.uy));
        ...
        if (is_ammo(obj)) {
            if (ammo_and_launcher(obj, uwep)) {
                ...
                range /= 2;
                pline("Вы не владеете %s, поэтому бросаете свой %s с помощью %s.",
    ...
    if (!gt.thrownobj) {
        ...
        if (iflags.returning_missile) { /* Mjollnir or aklys */
            if (rn2(100)) {
                ...
                if (!impaired && rn2(100)) {
                    pline("%s в вашу руку!", Tobjnam(obj, "возвращается"));
                    ...
                    if (!dmg) {
                        pline(Blind ? "%s приземляется %s ваш %s."
                                    : "%s обратно к вам, приземляясь %s ваш %s.",
                              Blind ? Something : Tobjnam(obj, "возвращается"),
                              Levitation ? "под" : "у",
                              makeplural(body_part(FOOT)));
                    } else {
                        dmg += rnd(3);
                        pline(Blind ? "%s ваш %s!"
                                    : "%s обратно к вам, ударяя ваш %s!",
                              Tobjnam(obj, Blind ? "попадает" : "летит"),
                              body_part(ARM));
                        if (obj->oartifact)
                ...
                pline("%s вернуться!", Tobjnam(obj, "не удаётся"));
        ...
        if (flooreffects(obj, gb.bhitpos.x, gb.bhitpos.y, "падает")) {
            throwit_return(TRUE);
            return;
        }
        ...
        if (mon && mon->isshk && is_pick(obj)) {
            if (cansee(gb.bhitpos.x, gb.bhitpos.y))
                pline("%s хватает %s.", Monnam(mon), the(xname(obj)));
...
tmiss(struct obj *obj, struct monst *mon, boolean maybe_wakeup)
{
    ...
    if (!canseemon(mon) || (M_AP_TYPE(mon) && M_AP_TYPE(mon) != M_AP_MONSTER))
        pline("%s %s.", The(missile), otense(obj, "промахивается"));
    else
        miss(missile, mon);
    ...
    struct obj *obj) /* gt.thrownobj or gk.kickedobj or uwep */
{
    ...
    if (uarmg && uwep && objects[uwep->otyp].oc_skill == P_BOW) {
        ...
        case GAUNTLETS_OF_DEXTERITY:
            break;
        default:
            impossible("Неизвестный тип перчаток (%d)", uarmg->otyp);
            break;
        }
    }
    ...
    if (obj->oclass == GEM_CLASS && is_unicorn(mon->data)
        ...
        if (helpless(mon)) {
            ...
            pline("%s ловит и роняет %s.", Monnam(mon), the(xname(obj)));
            return 0;
        } else {
            pline("%s ловит %s.", Monnam(mon), the(xname(obj)));
    ...
    if (hmode != HMON_APPLIED && special_obj_hits_leader(obj, mon)) {
        ...
        if (mon->mcanmove) {
            pline("%s ловит %s.", Some_Monnam(mon), the(xname(obj)));
            ...
            if ((u.uevent.invoked && objects[obj->otyp].oc_unique
                ...
                if (mon->mpeaceful && !Deaf) {
                    ...
                    verbalize("%s роль в этом окончена.",
                              s_suffix(The(xname(obj))));
                    verbalize(
               "Мы охраним это на случай, если понадобится снова, %s не дай.",
                ...
                pline("%s %s %s обратно к вам.", Some_Monnam(mon),
                      (next2u ? "руки" : "швыряет"), the(xname(obj)));
                if (!next2u)
    ...
    if (obj->oclass == WEAPON_CLASS || is_weptool(obj)
        ...
        Strcpy(trail,
               digests(md) ? " кишках" : is_whirly(md) ? " потоках воздуха" : "");
        monname = mon_nam(mon);
        if (*trail)
            monname = s_suffix(monname);
        pline("%s в %s%s.", Tobjnam(obj, "исчезает"), monname, trail);
    } else {
        tmiss(obj, mon, TRUE);
    }
...
gem_accept(struct monst *mon, struct obj *obj)
{
    static NEARDATA const char
        nogood[]     = " не заинтересован в вашем хламе.",
        acceptgift[] = " принимает ваш дар.",
        maybeluck[]  = " с колебанием",
        noluck[]     = " великодушно",
        addluck[]    = " с благодарностью";
    char buf[BUFSZ];
...
release_camera_demon(struct obj *obj, coordxy x, coordxy y)
{
    ...
    if (!rn2(3)
        && (mtmp = makemon(&mons[rn2(3) ? PM_HOMUNCULUS : PM_IMP], x, y,
                           MM_NOMSG)) != 0) {
        if (canspotmon(mtmp))
            pline("%s отпущен!", Hallucination
                                         ? An(rndmonnam(NULL))
                                         : "демон, рисующий картины");
            ...
            if (!breathless(gy.youmonst.data) || haseyes(gy.youmonst.data)) {
                ...
                if (obj->otyp != POT_WATER && !Half_gas_damage) {
                    if (!breathless(gy.youmonst.data)) {
                        ...
                        You("чувствуете странный запах...");
                    } else {
                        const char *eyes = body_part(EYE);

                        if (eyecount(gy.youmonst.data) != 1)
                            eyes = makeplural(eyes);
                        Your("%s %s.", eyes, vtense(eyes, "слезятся"));
                    }
                }
                potionbreathe(obj);
            }
...
breakmsg(struct obj *obj, boolean in_view)
{
    ...
    default: /* glass or crystal wand */
        if (obj->oclass != WAND_CLASS)
            impossible("сломать странный предмет (%d)?", obj->otyp);
        FALLTHROUGH;
        /*FALLTHRU*/
    case LENSES:
    case MIRROR:
    case CRYSTAL_BALL:
    case EXPENSIVE_CAMERA:
        to_pieces = " на тысячу кусков";
        FALLTHROUGH;
    /*FALLTHRU*/
    case POT_WATER: /* really, all potions */
        if (!in_view)
            You_hear("%s разбивается!", something);
        else
            pline("%s разбивается%s%s!", Doname2(obj),
...
throw_gold(struct obj *obj)
{
    ...
    if (!u.dx && !u.dy && !u.dz) {
        You("не можете бросить золото в себя.");
    ...
    if (u.uswallow) {
        ...
        if (digests(u.ustuck->data))
            ...
            swallower = strcat(s_suffix(swallower), " кишках");
        pline("Золото исчезает в %s.", swallower);
    ...
    if (u.dz) {
        if (u.dz < 0 && !Is_airlevel(&u.uz) && !Underwater
            && !Is_waterlevel(&u.uz)) {
            pline("Золото ударяется о %s, затем падает обратно на ваш %s.",
                      ceiling(u.ux, u.uy), body_part(HEAD));
            /* some self damage? */
            if (uarmh)
                pline("К счастью, на вас надето %s!",
    ...
    if (flooreffects(obj, gb.bhitpos.x, gb.bhitpos.y, "падает"))
        return ECMD_TIME;
    if (u.dz > 0)
        pline("Золото ударяется о %s.", surface(gb.bhitpos.x, gb.bhitpos.y));
    place_object(obj, gb.bhitpos.x, gb.bhitpos.y);
    if (*u.ushops)
