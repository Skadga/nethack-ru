...
dumplogmsg(const char *line)
{
    ...
    if (!strncmp(line, "Неизвестная команда", 19))
        return;
...
You(const char *line, ...)
{
    ...
    va_start(the_args, line);
    vpline(YouMessage(tmp, "Вы ", line), the_args);
    va_end(the_args);
}
...
You_feel(const char *line, ...)
{
    ...
    va_start(the_args, line);
    if (Unaware)
        YouPrefix(tmp, "Вы во сне чувствуете ", line);
    else
        YouPrefix(tmp, "Вы чувствуете ", line);
...
You_cant(const char *line, ...)
{
    ...
    va_start(the_args, line);
    vpline(YouMessage(tmp, "Вы не можете ", line), the_args);
    va_end(the_args);
}
...
There(const char *line, ...)
{
    ...
    va_start(the_args, line);
    vpline(YouMessage(tmp, "Там ", line), the_args);
    va_end(the_args);
}
...
You_hear(const char *line, ...)
{
    ...
    va_start(the_args, line);
    if (Underwater)
        YouPrefix(tmp, "Вы едва слышите ", line);
    else if (Unaware)
        YouPrefix(tmp, "Вы во сне слышите ", line);
    else
        YouPrefix(tmp, "Вы слышите ", line);  /* Deaf-aware */
    vpline(strcat(tmp, line), the_args);
    va_end(the_args);
}
...
You_see(const char *line, ...)
{
    ...
    va_start(the_args, line);
    if (Unaware)
        YouPrefix(tmp, "Вы во сне видите ", line);
    else if (Blind) /* caller should have caught this... */
        YouPrefix(tmp, "Вы ощущаете ", line);
    else
        YouPrefix(tmp, "Вы видите ", line);
    vpline(strcat(tmp, line), the_args);
    va_end(the_args);
}
...
impossible(const char *s, ...)
{
    ...
    Strcpy(pbuf2, "Программа в беспорядке!");
    if (program_state.something_worth_saving)
        Strcat(pbuf2, "  (Сохранение и перезагрузка могут исправить эту проблему.)");
    pline("%s", pbuf2);
    pline("Пожалуйста, сообщайте об этих сообщениях в %s.", DEVTEAM_EMAIL);
    if (sysopt.support) {
        pline("Либо свяжитесь с местной поддержкой: %s", sysopt.support);
    }

#ifdef CRASHREPORT
    if (sysopt.crashreporturl) {
        boolean report = ('y' == yn_function("Отправить отчёт сейчас?", ynchars,
                                             'n', FALSE));
...
execplinehandler(const char *line)
{
    ...
    if (f == 0) { /* child */
        ...
        pline("%s", "Сбой запуска обработчика сообщений.");
    }
#elif defined(WIN32)
    {
        intptr_t ret;
