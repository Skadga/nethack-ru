...
cursetxt(struct monst *mtmp, boolean undirected)
{
    if (canseemon(mtmp) && couldsee(mtmp->mx, mtmp->my)) {
        ...
        if (undirected)
            point_msg = "во все стороны";
        else if ((Invis && !perceives(mtmp->data)
                  && (mtmp->mux != u.ux || mtmp->muy != u.uy))
                 || is_obj_mappear(&gy.youmonst, STRANGE_OBJECT)
                 || u.uundetected)
            point_msg = "в вашем направлении";
        else if (Displaced && (mtmp->mux != u.ux || mtmp->muy != u.uy))
            point_msg = "ваш смещённый образ";
        else
            point_msg = "вас";

        pline_mon(mtmp, "%s указывает %s.", Monnam(mtmp), point_msg);
    } else if ((!(svm.moves % 4) || !rn2(4))) {
        if (!Deaf)
            Norep("Вы слышите глухое проклятие.");   /* Deaf-aware */
    }
}
    ...
    boolean foundyou)              /* knows hero's precise location */
{
    ...
    if (!foundyou && thinks_it_foundyou
        ...
        pline_mon(mtmp, "%s колдует на %s!",
                 canseemon(mtmp) ? Monnam(mtmp) : "Something",
                 is_waterwall(mtmp->mux, mtmp->muy) ? "пустую воду"
                                                    : "воздух");
        return M_ATTK_MISS;
    }
    ...
    if (rn2(ml * 10) < (mtmp->mconf ? 100 : 20)) { /* fumbled attack */
        ...
        if (canseemon(mtmp) && !Deaf) {
            ...
            pline("Воздух трещит вокруг %s.", mon_nam(mtmp));
        }
        return M_ATTK_MISS;
    }
    if (canspotmon(mtmp) || !is_undirected_spell(spellnum)) {
        pline_mon(mtmp, "%s колдует%s!",
    ...
    switch (mattk->adtyp) {
    case AD_FIRE:
        pline("Вы окутаны пламенем.");
        if (Fire_resistance) {
            shieldeff(u.ux, u.uy);
            pline("Но вы сопротивляетесь эффектам.");
...
touch_of_death(struct monst *mtmp)
{
    ...
    You_feel("истощённым...");
    (void) death_inflicted_by(kbuf, "прикосновение смерти", mtmp);
...
{
    pline("О нет, %s использует прикосновение смерти!", mhe(mtmp));
    if (nonliving(gy.youmonst.data) || is_demon(gy.youmonst.data)) {
        You("выглядите не мертвее, чем раньше.");
    } else if (!Antimagic && rn2(mtmp->m_lev) > 12) {
        if (Hallucination) {
            You("испытываете выход из тела.");
        } else {
            touch_of_death(mtmp);
        }
        monstunseesu(M_SEEN_MAGR);
        ...
        pline("Вам повезло, это не сработало!");
    }
}
...
mcast_summon_mons(struct monst *mtmp)
{
    ...
    if (!count) {
        ...
        verbalize("Уничтожь вора, мой питомец%s!", plur(count));
    } else {
        boolean one = (count == 1);
        const char *mappear = one ? "Появляется чудовище"
                                  : "Появляются чудовища";
        ...
        if (Invis && !perceives(mtmp->data)
            && (mtmp->mux != u.ux || mtmp->muy != u.uy))
            pline("%s %s рядом с вами!", mappear,
                  one ? "" : "");
        else if (Displaced && (mtmp->mux != u.ux || mtmp->muy != u.uy))
            pline("%s %s вашего смещённого образа!", mappear,
                  one ? "вокруг" : "вокруг");
        else
            pline("%s из ниоткуда!", mappear);
    }
}
...
mcast_destroy_armor(void)
{
    if (Antimagic) {
        shieldeff(u.ux, u.uy);
        monstseesu(M_SEEN_MAGR);
        pline("Поле силы окружает вас!");
    } else if (!destroy_arm()) {
        Your("ваша кожа чешется.");
    } else {
...
mcast_weaken_you(struct monst *mtmp, int dmg)
{
    if (Antimagic) {
        shieldeff(u.ux, u.uy);
        monstseesu(M_SEEN_MAGR);
        You_feel("ненадолго ослабленным.");
    } else {
        char kbuf[BUFSZ];

        You("внезапно чувствуете себя слабее!");
        dmg = mtmp->m_lev - 6;
...
mcast_stun_you(int dmg)
{
    if (Antimagic || Free_action) {
        ...
        monstseesu(M_SEEN_MAGR);
        if (!Stunned)
            You_feel("ненадолго дезориентированным.");
        make_stunned(1L, FALSE);
    } else {
        You(Stunned ? "пытаетесь удержать равновесие." : "пошатываетесь...");
...
mcast_geyser(int dmg)
{
     ...
     */
    pline("Внезапный гейзер ударяет вас из ниоткуда!");
    dmg = d(8, 6);
...
mcast_fire_pillar(struct monst *mtmp, int dmg)
{
    ...
    pline("Столп огня ударяет вокруг вас!");
    orig_dmg = dmg = d(8, 6);
    if (Fire_resistance) {
        shieldeff(u.ux, u.uy);
        monstseesu(M_SEEN_FIRE);
...
mcast_lightning(struct monst *mtmp, int dmg)
{
    ...
    pline("Молния ударяет в вас сверху!");
...
mcast_psi_bolt(int dmg)
{
    ...
    if (dmg <= 5)
        You("у вас слегка болит %s.", body_part(HEAD));
    else if (dmg <= 10)
        Your("ваш мозг горит!");
    else if (dmg <= 20)
        Your("%s внезапно сильно болит!", body_part(HEAD));
    else
        Your("%s внезапно очень сильно болит!", body_part(HEAD));
    return dmg;
}
...
mcast_open_wounds(int dmg)
{
    ...
    if (dmg <= 5)
        Your("ваша кожа сильно чешется на мгновение.");
    else if (dmg <= 10)
        pline("На вашем теле появляются раны!");
    else if (dmg <= 20)
        pline("На вашем теле появляются тяжёлые раны!");
    else
        Your("ваше тело покрыто болезненными ранами!");
    return dmg;
}
...
mcast_insects(struct monst *mtmp)
{
    ...
    if (!seecaster) {
        if (newseen <= oldseen || Unaware) {
            ...
            You_hear("кого-то, призывающего %s.", what);
        } else {
            char *arg;
            ...
            if (!Deaf) {
                ...
                You_hear("кого-то, призывающего что-то, и %s %s.", arg,
                         vtense(arg, "appear"));
            } else {
        ...
        fmt = "%s колдует на кучу хвороста, но ничего не происходит.%s";
        what = "";
    } else if (let == S_SNAKE) {
        fmt = "%s превращает кучу хвороста в %s!";
    } else if (Invis && !perceives(mtmp->data)
               && (mtmp->mux != u.ux || mtmp->muy != u.uy)) {
        fmt = "%s призывает %s рядом с вами!";
    } else if (Displaced && (mtmp->mux != u.ux || mtmp->muy != u.uy)) {
        fmt = "%s призывает %s вокруг вашего смещённого образа!";
    } else {
        fmt = "%s призывает %s!";
    }
    if (fmt) {
...
mcast_blind_you(void)
{
    ...
    if (!Blinded) {
        ...
        pline("Чешуя покрывает ваш %s!", (num_eyes == 1)
                                       ? body_part(EYE)
...
mcast_paralyze(struct monst *mtmp)
{
    ...
    nomul(-dmg);
    gm.multi_reason = "парализован чудовищем";
    gn.nomovemsg = 0;
    return dmg;
}
...
mcast_confuse_you(struct monst *mtmp)
{
    if (Antimagic) {
        shieldeff(u.ux, u.uy);
        monstseesu(M_SEEN_MAGR);
        You_feel("ненадолго в замешательстве.");
    } else {
        ...
        if (Hallucination)
            You_feel("%s!", oldprop ? "ещё более одурманенным" : "одурманенным");
        else
            You_feel("%sв замешательстве!", oldprop ? "более " : "");
        monstunseesu(M_SEEN_MAGR);
    }
}
...
mcast_spell(struct monst *mtmp, int dmg, int spellnum)
{
    ...
    case MCAST_AGGRAVATION:
        You_feel("что монстры знают о вашем присутствии.");
        aggravate();
        dmg = 0;
        break;
    case MCAST_CURSE_ITEMS:
        You_feel("как будто вам нужна помощь.");
        rndcurse();
        dmg = 0;
        break;
...
buzzmu(struct monst *mtmp, struct attack *mattk)
{
    ...
    if (lined_up(mtmp) && rn2(3)) {
        nomul(0);
        if (canseemon(mtmp))
            pline_mon(mtmp, "%s поражает вас %s!", Monnam(mtmp),
