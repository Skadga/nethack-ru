        ...
        boolean devour)
{
    ...
    if (is_pool(mtmp->mx, mtmp->my) && !Underwater) {
        ...
        if (sawpet || (seeobj && canspotmon(mtmp))) {
            ...
            if (tunnels(mtmp->data))
                pline_mon(mtmp, "%s роет.", noit_Monnam(mtmp));
            else
                pline_mon(mtmp, "%s %s %s.", noit_Monnam(mtmp),
                      devour ? "пожирает" : "ест", obj_name);
        } else if (seeobj) {
            obj_name = distant_name(obj, doname);
            pline("Оно %s %s.", devour ? "пожирает" : "ест", obj_name);
        }
    }
    if (obj->unpaid) {
    ...
    if (mtmp->data == &mons[PM_RUST_MONSTER] && obj->oerodeproof) {
        ...
        if (canseemon(mtmp)) {
            ...
            if (flags.verbose)
                pline("%s с отвращением выплёвывает %s!",
        ...
        if (obj->unpaid) {
            ...
            pline("Этот %s будет стоить вам %ld %s.", objnambuf, oprice,
                  currency(oprice));
...
dog_starve(struct monst *mtmp)
{
    if (mtmp->mleashed && mtmp != u.usteed)
        Your("ваш поводок провисает.");
    else if (cansee(mtmp->mx, mtmp->my))
        pline_mon(mtmp, "%s умирает от голода.", Monnam(mtmp));
    else
    ...
    if (svm.moves > edog->hungrytime + DOG_WEAK) {
        if (!carnivorous(mtmp->data) && !herbivorous(mtmp->data)) {
            ...
            if (cansee(mtmp->mx, mtmp->my))
                pline_mon(mtmp, "%s обезумел от голода.", Monnam(mtmp));
            else if (couldsee(mtmp->mx, mtmp->my))
                beg(mtmp);
            else
                You_feel("обеспокоены из-за %s.", y_monnam(mtmp));
            stop_occupation();
    ...
    if (nix != omx || niy != omy) {
        ...
        if (mfp.info[chi] & ALLOW_U) {
            if (mtmp->mleashed) { /* play it safe */
                pline_mon(mtmp, "%s вырывается из поводка %s!",
...
quickmimic(struct monst *mtmp)
{
    ...
    if (spotted || seeloc || canspotmon(mtmp)) {
        ...
        if (glyph_at(mtmp->mx, mtmp->my) != prev_glyph)
            ...
            You("чувствуете, что %s выглядит довольно %s.", buf, what);
