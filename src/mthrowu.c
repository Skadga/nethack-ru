...
static NEARDATA const char *breathwep[] = {
    "осколки", "огонь", "мороз", "усыпляющий газ", "разрушающая волна",
    "молнию", "ядовитый газ", "кислоту", "странное дыхание #8",
    "странное дыхание #9"
};

/* hallucinatory ray types */
static const char *const hallublasts[] = {
    "астероиды", "бусы", "пузыри", "бабочки", "шампанское", "хаос",
    "монеты", "сахарная вата", "крошки", "тёмная материя", "тьма", "данные",
    "пылинки", "смайлики", "эмоции", "энтропия", "цветы", "пена",
    "туман", "гамма-лучи", "желе", "самоцветы", "призраки", "стеклянные осколки",
    "блёстки", "хорошие флюиды", "гравий", "гравитация", "подливка", "ругательства",
    "священный свет", "шершни", "горячий воздух", "дефисы", "гипноз", "инфракрасное излучение",
    "насекомые", "жаргон", "лазерные лучи", "листья", "молнии", "логические элементы",
    "магма", "шарики", "математика", "мегабайты", "металлическая стружка",
    "метапаттерны", "метеоры", "туман", "грязь", "музыка", "наниты", "иголки",
    "шум", "ностальгия", "масло", "краска", "фотоны", "пиксели", "плазма",
    "полярность", "порох", "бонусы", "призматический свет", "чистая логика",
    "пурпур", "радиоволны", "радуги", "рок-музыка", "ракетное топливо", "верёвка",
    "печаль", "соль", "песок", "свитки", "тина", "улыбки", "снежинки",
    "искры", "зеркальность", "споры", "звёзды", "пар", "тетраэдры",
    "текст", "прошлое", "смерчи", "токсичные отходы", "ультрафиолетовый свет",
    "вирусы", "вода", "волноформы", "ветер", "рентгеновские лучи", "зоркмиды"
};
    ...
    const char *name) /* if null, then format `*objp' */
{
    ...
    if (u.uac + tlev <= (dieroll = rnd(20))) {
        ...
        if (Blind || !flags.verbose) {
            pline("Он промахивается.");
        } else if (u.uac + tlev <= dieroll - 2) {
            if (onm != onmbuf)
                Strcpy(onmbuf, onm); /* [modifiable buffer for upstart()] */
            pline("%s %s вас.", upstart(onmbuf),
                   vtense(onmbuf, "промахивается мимо вас"));
        } else
            You("почти попадаете под удар %s.", onm);
        return 0;
    } else {
        if (Blind || !flags.verbose)
            You("попадаете под удар%s", exclam(dam));
        else
            You("попадаете под удар %s%s", onm, exclam(dam));

        if (is_acid && Acid_resistance) {
            pline("Похоже, это вам не вредит.");
            ...
            pline(named ? "Оно проходит сквозь вас, не причиняя вреда."
                          : "Оно не причиняет вам вреда.");
...
monshoot(struct monst *mtmp, struct obj *otmp, struct obj *mwep)
{
    ...
    if (canseemon(mtmp)) {
        ...
        pline("%s %s %s%s%s!", Monnam(mtmp),
              gm.m_shot.s ? "выстреливает" : "швыряет", onm,
              mtarg ? " в " : "", trgbuf);
    ...
    boolean verbose) /* give messages even when you can't see what happened */
{
    ...
    if (tmp < rnd(20)) {
        ...
        if (vis) {
            if (otmp->otyp == EGG) {
                pline("Шлёп!  %s поражён %s яйцом!", Monnam(mtmp),
                ...
                if (!harmless)
                    ...
                    Sprintf(how, " но безвредно проходит сквозь %.9s.",
                            mhim(mtmp));
                hit(distant_name(otmp, mshot_xname), mtmp, how);
            }
        } else if (verbose && !gm.mtarget)
            pline("%s%s поражён%s", (otmp->otyp == EGG) ? "Шлёп!  " : "",
                  Monnam(mtmp), exclam(damage));

        if (otmp->opoisoned && is_poisonable(otmp)) {
            if (resists_poison(mtmp)) {
                if (vis)
                    pline_The("Яд, похоже, не действует на %s.",
                              mon_nam(mtmp));
            } else {
                if (rn2(30)) {
                    damage += rnd(6);
        ...
        if (material == SILVER && mon_hates_silver(mtmp)) {
            ...
            if (vis) {
                ...
                if (flesh) /* s_suffix returns a modifiable buffer */
                    m_name = strcat(s_suffix(m_name), " плоть");
                pline_The("Серебро обжигает %s!", m_name);
            } else if (verbose && !gm.mtarget) {
                pline("%s облянойкова!", flesh ? "Его плоть" : "Оно");
            }
        }
        if (otmp->otyp == ACID_VENOM && cansee(mtmp->mx, mtmp->my)) {
            if (resists_acid(mtmp)) {
                if (vis || (verbose && !gm.mtarget))
                    pline("%s не затронут.", Monnam(mtmp));
            } else {
                if (vis)
                    pline_The("%s обжигает %s!", hliquid("кислота"),
                              mon_nam(mtmp));
                else if (verbose && !gm.mtarget)
                    pline("Оно обожжено!");
            }
                ...
                if (vis || (verbose && !gm.mtarget))
                    pline("%s %s!", Monnam(mtmp),
                          (nonliving(mtmp->data) || is_vampshifter(mtmp)
                           || !canspotmon(mtmp)) ? "уничтожен" : "убит");
        ...
        if (!DEADMONSTER(mtmp)
            ...
            if (vis && mtmp->mcansee)
                ...
                pline("%s ослеплён %s.", Monnam(mtmp),
                      (otmp->oclass == VENOM_CLASS) ? "ядом"
                          : (otmp->otyp == CREAM_PIE) ? "пирогом"
    ...
    struct monst *mon)
{
    ...
    if (gem->otyp <= LAST_GLASS_GEM && is_unicorn(gy.youmonst.data)) {
        ...
        if (gem->otyp >= FIRST_GLASS_GEM) {
            You("ловите %s.", gem_xname);
            You("не интересуетесь %s мусором.", mon_s_name);
            makeknown(gem->otyp);
            dropy(gem);
        } else {
            You("принимаете %s дар в духе, в котором он и был предназначен.",
                mon_s_name);
            (void) hold_another_object(gem, "Вы ловите, но роняете %s.",
                                       gem_xname, "Вы ловите:");
        }
        return TRUE;
    }
...
u_catch_thrown_obj(struct obj *otmp)
{
    ...
    if (!Blind && !Confusion && !Stunned && !Fumbling
        ...
        Snprintf(buf, BUFSZ, "Вы ловите %s!", simpleonames(otmp));
        (void) hold_another_object(otmp, "Вы ловите, но роняете %s.",
    ...
    if ((singleobj->cursed || singleobj->greased) && (dx || dy) && !rn2(7)) {
        if (canseemon(mon) && flags.verbose) {
            if (is_ammo(singleobj))
                pline("%s даёт осечку!", Monnam(mon));
            else
                pline("%s, когда %s бросает его!",
                      Tobjnam(singleobj, "выскальзывает"), mon_nam(mon));
        }
        dx = rn2(3) - 1;
        dy = rn2(3) - 1;
        ...
        if (mtmp && shade_miss(mon, mtmp, singleobj, TRUE, TRUE)) {
            ...
            if (hitu && can_blnd((struct monst *) 0, &gy.youmonst,
                ...
                if (singleobj->otyp == CREAM_PIE) {
                    if (!Blind)
                        pline("Фу!  Вас вымазали.");
                    else
                        pline("На вашем %s %s липкое.",
                              body_part(FACE), something);
        ...
        if (!range || MT_FLIGHTCHECK(FALSE, forcehit)) {
            ...
            if (singleobj) { /* hits_bars might have destroyed it */
                ...
                if (range && cansee(gb.bhitpos.x, gb.bhitpos.y)
                    ...
                    pline("%s %s на раковину.",
                          The(mshot_xname(singleobj)),
                          otense(singleobj, Hallucination ? "шлёпается"
                                                          : "падает"));
                    ...
                    pline("%s промахивается.", The(mshot_xname(singleobj)));
                if (!tethered_weapon) {
    ...
    if (otmp && made_it_back) {
        ...
        if (!impaired && rn2(100)) {
            ...
            if (!do_not_annoy || (svm.moves - do_not_annoy) > 500L) {
                pline("%s обратно в %s руку.", Tobjnam(otmp, "возвращается"),
            ...
            if (!dmg) {
                if (canseemon(magr)) {
                    pline("%s к %s, приземляясь %s %s %s.",
                          Tobjnam(otmp, "возвращается"), mon_nam(magr),
                          mlevitating ? "под" : "у", mhis(magr),
                          makeplural(mbodypart(magr, FOOT)));
                } else if (!Deaf) {
                    You_hear("%s падает возле %s.", Something, mon_nam(magr));
                }
            } else {
                dmg += rnd(3);
                if (canseemon(magr)) {
                    pline("%s обратно к %s, попадая %s в %s!",
                          Tobjnam(otmp, "летит"), mon_nam(magr),
                          mhis(magr), body_part(ARM));
                } else if (!Deaf) {
                    You_hear("%s со стуком попадает в %s!", something,
                             mon_nam(magr));
                }
                hits_thrower = TRUE;
            }
        ...
        if (notcaught) {
            ...
            if (!Deaf && !Underwater) {
                ...
                if (is_pool(x, y) || (is_lava(x, y) && !is_flammable(otmp))) {
                    ...
                    pline((weight(otmp) > 9) ? "Плюх!" : "Бултых!");
                }
            }
...
spitmm(struct monst *mtmp, struct attack *mattk, struct monst *mtarg)
{
    ...
    if (mtmp->mcan) {
        if (!Deaf && mdistu(mtmp) < BOLT_LIM * BOLT_LIM) {
            if (canspotmon(mtmp)) {
                pline("Сухой хрип доносится из горла %s.",
                      s_suffix(mon_nam(mtmp)));
            } else {
                Soundeffect(se_dry_throat_rattle, 50);
                You_hear("сухое дребезжание поблизости.");
            }
        }
        return M_ATTK_MISS;
    }
    if (m_lined_up(mtarg, mtmp)) {
        ...
        if (!rn2(BOLT_LIM-distmin(mtmp->mx,mtmp->my,tx,ty))) {
            if (canseemon(mtmp))
                pline("%s выплёвывает яд!", Monnam(mtmp));
            if (!utarg)
                gm.mtarget = mtarg;
        ...
        *objp = 0; /* object is now gone */
        ...
        if (obj_type == POT_ACID) {
            if (cansee(barsx, barsy) && !nodissolve) {
                pline("Железные прутья растворены!");
            } else {
                Soundeffect(se_angry_snakes, 100);
                You_hear(Hallucination ? "рассерженные змеи!"
                                       : "шипение.");
            }
            if (!nodissolve)
