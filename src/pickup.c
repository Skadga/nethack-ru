...
static const char
    slightloadpfx[] = "У вас небольшие затруднения",
    moderateloadpfx[] = "У вас затруднения",
    nearloadpfx[] = "У вас большие затруднения",
    overloadpfx[] = "У вас крайние затруднения";
    ...
    int *menu_on_demand) /* to tell caller that user picked 'm' */
{
 ...
 ask_again:
        ...
        *one_at_a_time = *everything = FALSE;
        ...
        Sprintf(qbuf, "Какие вещи вы хотите %s? [%s]", action,
                ilets);
        getlin(qbuf, inbuf);
        if (*inbuf == '\033')
            return FALSE;
            ...
            if (sym == ' ')
                ...
                *everything = TRUE;
                ...
                if (strchr(ilets, sym)) {
                    ...
                    if (!where)
                        where = !strcmp(action, "поднять") ? "здесь"
                                : !strcmp(action, "взять") ? "внутри" : "";
                    if (*where)
                        There("нет %c %s.", sym, where);
                    else
                        You("нет %c.", sym);
                    not_everything = TRUE;
                }
...
fatal_corpse_mistake(struct obj *obj, boolean remotely)
{
    ...
    pline("Прикосновение к %s смертельно.",
...
rider_corpse_revival(struct obj *obj, boolean remotely)
{
    ...
    pline("У вашего %s труп внезапно шевелится...",
          remotely ? "попытка взять" : "коснуться");
...
describe_decor(void)
{
    ...
    doorhere = dfeature && (!strcmp(dfeature, "открытая дверь")
                            || !strcmp(dfeature, "дверной проём"));
    waterhere = dfeature && !strcmp(dfeature, "вода");
    ...
    if (ltyp == iflags.prev_decor && !IS_FURNITURE(ltyp)) {
        ...
        if (strcmp(dfeature, "болото") && ltyp != ICE)
            dfeature = an(dfeature);

        if (flags.verbose) {
            Sprintf(outbuf, "Здесь есть %s.", dfeature);
        } else {
            if (dfeature != fbuf)
...
pickup(int what) /* should be a long */
{
    ...
    if (!u.uswallow) {
        ...
        if ((gm.multi && !svc.context.run)
            ...
            if (notake(gy.youmonst.data) && OBJ_AT(u.ux, u.uy)
                && (autopickup || flags.pickup))
                You("физически не способны поднять что-либо.");
            return 0;
        }
    ...
    if (flags.menu_style != MENU_TRADITIONAL || iflags.menu_requested) {
        ...
        if (count) { /* looking for N of something */
            ...
            Sprintf(qbuf, "Поднять %d чего?", count);
            ...
            n = query_objlist("Что поднять?", objchain_p,
 ...
 menu_pickup:
        ...
        if (ct == 1 && count) {
            ...
            There("здесь %s предметов.", (ct <= 10) ? "несколько" : "много");
            if (!query_classes(oclasses, &selective, &all_of_a_type,
                ...
                n = query_objlist("Что поднять?", objchain_p, traverse_how,
                                  &pick_list, PICK_ANY,
        ...
        for (obj = *objchain_p; obj; obj = obj2) {
            ...
            if (!all_of_a_type) {
                ...
                (void) safe_qbuf(qbuf, "Поднять ", "?", obj, doname,
              ...
              boolean (*allow)(OBJ_P)) /* allow function */
{
    ...
    *pick_list = (menu_item *) 0;
    ...
    if (engulfer) {
        ...
        any = cg.zeroany;
        if (sorted && n > 1) {
            Sprintf(buf, "%s существ",
                    digests(u.ustuck->data) ? "Проглочен" : "Поглощён");
    ...
    if (n > 0) {
        ...
        for (i = k = 0, mi = *pick_list; i < n; i++, mi++) {
            ...
            if (curr == &fake_hero_object) {
                ...
                You_cant("поднимитесь сами!");
                continue;
            }
            if (engulfer_minvent && curr->owornmask != 0L) {
                You_cant("поднимите %s.", ysimple_name(curr));
                continue;
            }
    ...
    *pick_list = (menu_item *) 0;
    ...
    if ((qflags & CHOOSE_ALL) != 0) {
        ...
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 ...
                 do_worn ? "Автовыбор всего надетого или взятого в руки"
                         : "Автовыбор всех подходящих предметов",
                 MENU_ITEMFLAGS_SKIPINVERT);
        verify_All = (how == PICK_ANY) && ParanoidAutoAll;
        if (!verify_All) {
            if (!ga.A_first_hint++ || iflags.cmdassist)
                add_menu_str(win,
                   "    (игнорируется, если не выбраны другие варианты)");
        } else if (show_a) {
            if (!ga.A_second_hint++ || iflags.cmdassist)
                add_menu_str(win,
                      "    (если не выбраны другие варианты, подразумевается 'a')");
        }
        /* blank separator */
        add_menu_str(win, "");
    }
    ...
    if (show_a) {
        ...
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 do_worn ? "Все надетые и взятые в руки типы" : "Все типы",
                 MENU_ITEMFLAGS_SKIPINVERT);
        ++invlet; /* invlet = 'b'; */
    }
    ...
    if (do_unpaid) {
        ...
        add_menu(win, &nul_glyphinfo, &any, invlet, 0,
                 ATR_NONE, clr, "Неоплаченные предметы", MENU_ITEMFLAGS_SKIPINVERT);
    }
    ...
    if (do_usedup) {
        ...
        any.a_int = 'x';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 "Неоплаченные предметы уже израсходованы", MENU_ITEMFLAGS_SKIPINVERT);
    }
    ...
    if (do_blessed) {
        ...
        any.a_int = 'B';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 "Предметы, известные как благословлённые", MENU_ITEMFLAGS_SKIPINVERT);
    }
    if (do_cursed) {
        invlet = 'C';
        any = cg.zeroany;
        any.a_int = 'C';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 "Предметы, известные как проклятые", MENU_ITEMFLAGS_SKIPINVERT);
    }
    if (do_uncursed) {
        invlet = 'U';
        any = cg.zeroany;
        any.a_int = 'U';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 "Предметы, известные как непроклятые", MENU_ITEMFLAGS_SKIPINVERT);
    }
    if (do_buc_unknown) {
        invlet = 'X';
        any = cg.zeroany;
        any.a_int = 'X';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
                 "Предметы неизвестного статуса благосл./проклят.",
                 MENU_ITEMFLAGS_SKIPINVERT);
    }
    if (num_justpicked) {
        char tmpbuf[BUFSZ];
        ...
        if (num_justpicked == 1)
            Sprintf(tmpbuf, "Только что поднято: %s",
                    doname(find_justpicked(olist)));
        else
            Strcpy(tmpbuf, "Предметы, которые вы только что подняли");
        invlet = 'P';
        any = cg.zeroany;
        any.a_int = 'P';
        add_menu(win, &nul_glyphinfo, &any, invlet, 0, ATR_NONE, clr,
    ...
    if (n > 0 && verify_All) {
        ...
        for (i = 0; i < n; ++i)
            if ((*pick_list)[i].item.a_int == 'A') {
                ...
                switch (paranoid_ynq(ParanoidConfirm,
                                     "Действительно автовыбрать всё?", TRUE)) {
                case 'y':
        ...
        pline("Не выбрано подходящих предметов.");
    }
 query_done:
    destroy_nhwindow(win);
            ...
            int *wt_before, int *wt_after)
{
    ...
    *wt_after = wt;
    ...
    if (qq < count) {
        ...
        if (container) {
            Sprintf(where, "в %s", the(xname(container)));
            verb = "нести";
        } else {
            Strcpy(where, "лежащие здесь");
            verb = telekinesis ? "взять" : "поднять";
        }
    } else {
        /* lint suppression */
        *obj_nambuf = *where = '\0';
    ...
    if (qq > 0) {
        if (qq < count)
            You("можете только %s %s из %s %s.", verb,
                (qq == 1L) ? "один" : "несколько", obj_nambuf, where);
        *wt_after = wt;
        return qq;
    }

    if (!container)
        Strcpy(where, "здесь"); /* slightly shorter form */
    if (gi.invent || umoney) {
        prefx1 = "вы не можете";
        prefx2 = "";
        suffx = " поднять";
    } else {
        prefx1 = (obj->quan == 1L) ? "оно должно" : "они должны";
        prefx2 = " упасть на пол, чтобы поднять";
        suffx = "";
    }
    There("%s %s %s, но %s%s%s.", obj_nambuf, where, verb,
          prefx1, prefx2, suffx);
    ...
    boolean telekinesis)
{
    ...
    if (obj->otyp == BOULDER && Sokoban) {
        You("не можете обхватить %s вокруг этого %s.", body_part(HAND),
            xname(obj));
        return -1;
    }
    ...
    if (obj->otyp == LOADSTONE
        ...
        You("несёте слишком много, чтобы поднять %s %s.",
            (obj->quan == 1L) ? "другой" : "больше", xname(obj));
        return -1;
    }
    ...
    *cnt_p = carry_count(obj, container, *cnt_p, telekinesis,
    ...
    if (*cnt_p < 1L) {
        ...
        Your("заплечный мешок не вмещает больше предметов%s.",
                ...
                Sprintf(qbuf, "%s %s ",
                        ...
                        : (next_encumbr >= HVY_ENCUMBER) ? nearloadpfx
                          : (next_encumbr >= MOD_ENCUMBER) ? moderateloadpfx
                            : slightloadpfx,
                        !container ? "поднимая" : "снимая");
                (void) safe_qbuf(qbuf, qbuf, ".  Продолжить?", obj, doname,
                                 ansimpleoname, something);
                obj->quan = savequan;
                switch (ynq(qbuf)) {
                case 'q':
    ...
    boolean telekinesis) /* not picking it up directly by hand */
{
    ...
    if (obj == uchain) { /* do not pick up attached chain */
        ...
        You_cant("поднимите %s.", ysimple_name(obj));
        return 0;
        ...
        if (obj->blessed) {
            ...
            pline("Свиток%s %s в пыль, когда вы %s %s.", plur(obj->quan),
                      otense(obj, "Превращается"), telekinesis ? "Поднимает" : "Поднимает",
                      (obj->quan == 1L) ? "Его" : "Их");
            trycall(obj);
            useupf(obj, obj->quan);
    ...
    pickup_prinv(obj, count, "поднимая");
    if (obj->ghostly)
        fix_ghostly_obj(obj);
...
encumber_msg(void)
{
    ...
    if (go.oldcap < newcap) {
        switch (newcap) {
        case 1:
            Your("движения слегка замедлены из-за ноши.");
            break;
        case 2:
            You("перераспределите ношу.  Движение затруднено.");
            break;
        case 3:
            You("%s под тяжёлой ношей.  Движение очень трудно.",
                stagger(gy.youmonst.data, "пошатываетесь"));
            break;
        default:
            You("%s двигаетесь на ладонь с этой ношей!",
                newcap == 4 ? "едва можете" : "даже не можете");
            break;
        }
        disp.botl = TRUE;
        ...
        switch (newcap) {
        case 0:
            Your("движения теперь свободны.");
            break;
        case 1:
            Your("движения лишь слегка замедлены ношей.");
            break;
        case 2:
            You("перераспределите ношу.  Движение всё ещё затруднено.");
            break;
        case 3:
            You("%s под вашей ношей.  Движение всё ещё очень трудно.",
                stagger(gy.youmonst.data, "пошатываетесь"));
            break;
        }
        disp.botl = TRUE;
...
{
    const char *verb = looting ? "ограбить" : "опрокинуть";
    ...
    if (!can_reach_floor(t && is_pit(t->ttyp))) {
        ...
        You("не можете %s вещи, что глубоко в %s.", verb,
            hliquid(is_lava(x, y) ? "лаве" : "воде"));
        return FALSE;
    } else if (nolimbs(gy.youmonst.data)) {
        pline("Без конечностей вы не можете %s ничего.", verb);
        return FALSE;
    } else if (looting && !freehand()) {
        pline("Без свободного %s вы не можете ничего ограбить.",
              body_part(HAND));
        return FALSE;
    }
    return TRUE;
    ...
    int ccount) /* ...number of them (N) */
{
    ...
    if (cobj->olocked) {
        ...
        if (ccount < 2 && (svl.level.objects[cobj->ox][cobj->oy] == cobj))
            pline("%s заперто.",
                  cobj->lknown ? "Это" : "Хмм, оказывается, это");
        else
#endif
        if (cobj->lknown)
            pline("%s заперто.", The(xname(cobj)));
        else
            pline("Хмм, %s оказывается запертым.", the(xname(cobj)));
        cobj->lknown = 1;
...
doloot_core(void)
{
    ...
    const char *dont_find_anything = "ничего не находите";
    struct monst *mtmp;
    int prev_inquiry = 0;
    ...
    if (nohands(gy.youmonst.data)) {
        You("нет рук!"); /* not `body_part(HAND)' */
        return ECMD_OK;
    }
    if (Confusion) {
        if (rn2(6) && reverse_loot())
            return ECMD_TIME;
        if (rn2(2)) {
            pline("Будучи в замешательстве, вы ничего не находите для ограбления.");
 ...
 lootcont:
    if ((num_conts = container_at(cc.x, cc.y, TRUE)) > 0) {
        ...
        if (num_conts > 1) {
            ...
            end_menu(win, "Какие контейнеры ограбить?");
        ...
        You("нужно раскопать могилу, чтобы эффективно её ограбить...");
    }
 ...
 lootmon:
    if (c != 'y' && (mon_beside(u.ux, u.uy) || iflags.menu_requested)) {
        ...
        if (!get_adjacent_loc("В каком направлении ограбить?",
                              "Неверное место для ограбления", u.ux, u.uy, &cc))
            return ECMD_OK;
        underfoot = u_at(cc.x, cc.y);
        if (underfoot && container_at(cc.x, cc.y, FALSE))
            goto lootcont;
        if (u.dz < 0) {
            You("%s для ограбления на %s.", dont_find_anything,
                ceiling(cc.x, cc.y));
            return ECMD_TIME;
        }
        mtmp = m_at(cc.x, cc.y);
        ...
        if (!looted_mon) {
            if (!underfoot && container_at(cc.x, cc.y, FALSE)) {
                if (mtmp) {
                    You_cant("ограбить что-либо %sтам, с %s на пути.",
                             prev_inquiry ? "иначе " : "", mon_nam(mtmp));
                    return (timepassed ? ECMD_TIME : ECMD_OK);
                } else {
                    You("нужно быть у контейнера, чтобы его ограбить.");
                }
            } else {
                You("%s %s%sздесь для ограбления.", dont_find_anything,
                    (prev_inquiry || prev_loot) ? "иначе " : "",
                    !underfoot ? "t" : "");
                return (timepassed ? ECMD_TIME : ECMD_OK);
            }
        }
    } else if (c != 'y' && c != 'n') {
        You("%s %s для ограбления.", dont_find_anything,
            underfoot ? "здесь" : "там");
    }
    return (timepassed ? ECMD_TIME : ECMD_OK);
}
...
reverse_loot(void)
{
    ...
    if (!rn2(3)) {
        ...
        for (n = inv_cnt(TRUE), otmp = gi.invent; otmp;
             --n, otmp = otmp->nobj)
            if (!rn2(n + 1)) {
                prinv("Вы находите старую добычу:", otmp, 0L);
                return TRUE;
            }
        return FALSE;
    }
    ...
    if (!IS_THRONE(levl[x][y].typ)) {
        ...
        if (g_at(x, y))
            pline("Хорошо, теперь здесь есть добыча.");
    } else {
        ...
        if (coffers) {
            ...
            verbalize("Спасибо за ваш вклад в уменьшение долга.");
            freeinv(goldob);
            ...
            pline("Казначейство принимает ваш вклад.");
            if (!rn2(10))
                levl[x][y].looted = T_LOOTED;
        } else {
            You("бросьте %s.", doname(goldob));
            dropx(goldob);
        }
    }
    return TRUE;
...
loot_mon(struct monst *mtmp, int *passed_info, boolean *prev_loot)
{
    ...
    if (mtmp && mtmp != u.usteed && (otmp = which_armor(mtmp, W_SADDLE))) {
        if (passed_info)
            *passed_info = 1;
        Sprintf(qbuf, "Хотите снять седло с %s?",
                x_monnam(mtmp, ARTICLE_THE, (char *) 0,
                         SUPPRESS_SADDLE, FALSE));
        if ((c = yn_function(qbuf, ynqchars, 'n', TRUE)) == 'y') {
            if (nolimbs(gy.youmonst.data)) {
                You_cant("сделать это без конечностей."); /* not body_part(HAND) */
                return 0;
            }
            if (otmp->cursed) {
                You("не можете.  Седло, кажется, приклеено к %s.",
                    x_monnam(mtmp, ARTICLE_THE, (char *) 0,
                             SUPPRESS_SADDLE, FALSE));
            ...
            otmp = hold_another_object(otmp, "Вы бросаете %s!", doname(otmp),
                                       (const char *) 0);
            nhUse(otmp);
            timepassed = rnd(3);
            if (prev_loot)
...
in_container(struct obj *obj)
{
    ...
    if (!gc.current_container) {
        ...
        You("должно быть, шутите.");
        return 0;
    } else if (obj == gc.current_container) {
        pline("Это было бы интересное топологическое упражнение.");
        return 0;
    } else if (obj->owornmask & (W_ARMOR | W_ACCESSORY)) {
        Norep("Вы не можете %s %s, который носите.",
              Icebox ? "охладить" : "спрятать", something);
        return 0;
    } else if ((obj->otyp == LOADSTONE) && obj->cursed) {
        set_bknown(obj, 1);
        pline("Камень%s не покинет вас.", plur(obj->quan));
        return 0;
         ...
         */
        pline("%s не может быть заключён в такие одежды.", The(xname(obj)));
        return 0;
    } else if (obj->otyp == LEASH && obj->leashmon != 0) {
        pline("%s прикреплён к вашему питомцу.", Tobjnam(obj, "are"));
        return 0;
    } else if (obj == uwep) {
        if (welded(obj)) {
            weldmsg(obj);
    ...
    if (obj->otyp == ICE_BOX || Is_box(obj) || obj->otyp == BOULDER
        ...
        You("не помещается %s в %s.", buf, the(xname(gc.current_container)));
        return 0;
    }
    ...
    if (Icebox && !age_is_relative(obj)) {
        ...
        livelog_printf(LL_ACHIEVE, "только что взорвали %s сумку хранения", uhis());
        /* explicitly mention what item is triggering the explosion */
        urgent_pline(
              "Когда вы кладёте %s внутрь, вас поражает магический взрыв!",
                     doname(obj));
    ...
    if (gc.current_container) {
        ...
        You("положите %s в %s.", doname(obj), buf);
...
out_container(struct obj *obj)
{
    ...
    otmp = addinv(obj);
    pickup_prinv(otmp, count, "снимая");
...
mbag_item_gone(boolean held, struct obj *item, boolean silent)
{
    ...
    if (!silent) {
        if (item->dknown)
            pline("%s %s исчез!", Doname2(item), otense(item, "have"));
        else
            You("%s %s исчезают!", Blind ? "замечаете" : "видите", doname(item));
    }
...
observe_quantum_cat(struct obj *box, boolean makecat, boolean givemsg)
{
    ...
    if (itsalive) {
        ...
        if (livecat) {
            ...
            if (givemsg) {
                if (!canspotmon(livecat))
                    You("кажется, %s коснулся вашего %s.", something,
                        body_part(FOOT));
                else
                    pline("%s внутри коробки ещё жив!",
                          Monnam(livecat));
            }
        ...
        if (givemsg)
            pline("%s Внутри коробки мёртв!",
                      Hallucination ? rndmonnam((char *) 0) : "Домашний кот");
        if (deadcat) {
...
explain_container_prompt(boolean more_containers)
{
    static const char *const explaintext[] = {
        ...
        "",
        " : -- Осмотр: рассмотреть содержимое",
        " o -- Взять: достать вещи",
        " i -- Положить: поместить вещи внутрь",
        " b -- Оба: сначала достать, затем положить",
        " r -- Наоборот: сначала положить, затем достать",
        " s -- Спрятать: положить один предмет", "",
        " n -- Следующий: ограбить следующий выбранный контейнер",
        " q -- Выход: закончено",
        " ? -- Помощь: показать этот текст.",
        "", 0
    };
...
u_handsy(void)
{
    if (nohands(gy.youmonst.data)) {
        ...
        You("нет свободного %s.", body_part(HAND));
        return FALSE;
    }
    return TRUE;
}
    ...
    boolean more_containers) /* True iff #loot multiple and this isn't last */
{
    ...
    if (obj->olocked) {
        pline("%s заперто.", Tobjnam(obj, "are"));
        if (held)
            You("нужно положить его, чтобы открыть.");
        return ECMD_OK;
    } else if (obj->otrapped) {
        if (held)
            You("откройте %s...", the(xname(obj)));
        (void) chest_trap(obj, HAND, FALSE);
        /* even if the trap fails, you've used up this turn */
        if (gm.multi >= 0) { /* in case we didn't become paralyzed */
            nomul(-1);
            gm.multi_reason = "открытие контейнера";
            gn.nomovemsg = "";
        }
        ga.abort_looting = TRUE;
        return ECMD_TIME;
    ...
    if (!outokay) /* preformat the empty-container message */
        Sprintf(emptymsg, "%s %sпусто.",
                Ysimple_name2(gc.current_container),
                (quantum_cat || cursed_mbag) ? "теперь " : "");
    ...
    for (;;) { /* repeats iff '?' or ':' gets chosen */
        ...
        if (!outmaybe)
            (void) safe_qbuf(qbuf, (char *) 0, " пусто.  Что с ним делать?",
                             gc.current_container, Yname2, Ysimple_name2,
                             "Это");
        else
            (void) safe_qbuf(qbuf, "Что делать с ", "?", gc.current_container,
                             yname, ysimple_name, "его");
    ...
    if ((loot_in || stash_one) && !inokay) {
        You("у вас нет ничего%s, чтобы %s.", gi.invent ? " иначе" : "",
            stash_one ? "спрятать" : "положить");
    ...
    if (loot_in) {
        ...
        if ((otmp = getobj("спрятать", stash_ok,
...
menu_loot(int retry, boolean put_in)
{
    ...
    const char *action = put_in ? "Положить" : "Взять";
    ...
    if (retry) {
        ...
        Sprintf(buf, "%s какого типа предметы?", action);
    ...
    boolean more_containers)
{
    ...
    any.a_int = 1; /* ':' */
    Sprintf(buf, "Осмотреть %s", thesimpleoname(obj));
    add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
             ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
    if (outokay) {
        any.a_int = 2; /* 'o' */
        Sprintf(buf, "достать %s", something);
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
    }
    if (inokay) {
        any.a_int = 3; /* 'i' */
        Sprintf(buf, "положить %s", something);
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
    }
    if (outokay) {
        any.a_int = 4; /* 'b' */
        Sprintf(buf, "%sдостать, затем положить", inokay ? "оба; " : "");
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
    }
    if (inokay) {
        any.a_int = 5; /* 'r' */
        Sprintf(buf, "%sположить, затем достать",
                outokay ? "оба наоборот; " : "");
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
        any.a_int = 6; /* 's' */
        Sprintf(buf, "спрятать один предмет в %s", thesimpleoname(obj));
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, buf, MENU_ITEMFLAGS_NONE);
    }
    add_menu_str(win, "");
    if (more_containers) {
        any.a_int = 7; /* 'n' */
        add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
                 ATR_NONE, clr, "ограбить следующий контейнер",
                 MENU_ITEMFLAGS_SELECTED);
    }
    any.a_int = 8; /* 'q' */
    Strcpy(buf, alreadyused ? "готово" : "ничего не делать");
    add_menu(win, &nul_glyphinfo, &any, menuselector[any.a_int], 0,
             ATR_NONE, clr, buf,
...
choose_tip_container_menu(void)
{
    ...
    if (gi.invent) {
        ...
        add_menu(win, &nul_glyphinfo, &any, i, 0, ATR_NONE,
                 clr, "опрокинуть несомый предмет",
                 MENU_ITEMFLAGS_SELECTED);
    }
    end_menu(win, "Какой контейнер опрокинуть?");
    n = select_menu(win, PICK_ONE, &pick_list);
    destroy_nhwindow(win);
    /*
...
dotip(void)
{
    ...
    if (boxes > 0
        ...
        Sprintf(buf, "Вы не можете опрокинуть %s, неся так много.",
                !flags.verbose ? "контейнер" : (boxes > 1) ? "один" : "его");
        if (!check_capacity(buf) && able_to_loot(cc.x, cc.y, FALSE)) {
            if (boxes > 1) {
                ...
                for (cobj = svl.level.objects[cc.x][cc.y]; cobj;
                    ...
                    if (!Is_container(cobj))
                        continue;
                    c = ynq(safe_qbuf(qbuf, "Здесь есть ", " здесь, опрокинуть его?",
                                      cobj,
                                      doname, ansimpleoname, "контейнер"));
                    if (c == 'q')
                        return ECMD_OK;
                    if (c == 'n')
                        continue;
    ...
    cobj = getobj("опрокинуть", tip_ok, GETOBJ_PROMPT);
    if (!cobj)
        return ECMD_CANCEL;
    ...
    if (spillage) {
        ...
        if (is_pool(u.ux, u.uy))
            Sprintf(buf, " и постепенно %s", vtense(spillage, "рассеивается"));
        else if (is_lava(u.ux, u.uy))
            Sprintf(buf, " и немедленно %s прочь",
                    vtense(spillage, "сгорает"));
        pline("Некоторый %s %s на %s%s.", spillage,
              vtense(spillage, "проливается"), surface(u.ux, u.uy), buf);
    ...
    if (cobj->oclass == POTION_CLASS) /* can't pour potions... */
        pline("%s %s Надёжно запечатан.", xname(cobj), otense(cobj, "are"));
    else if (uarmh && cobj == uarmh)
        return tiphat() ? ECMD_TIME : ECMD_OK;
    else if (cobj->otyp == STATUE)
        pline("Ничего интересного не происходит.");
    else
...
tipcontainer(struct obj *box) /* or bag */
{
...

    {
        ...
        if (targetbox)
            pline("%s в %s.",
                  box->cobj->nobj ? "Предметы катятся" : "Предмет катится",
                  the(xname(targetbox)));
        else
            pline("%s наружу%c",
              box->cobj->nobj ? "Предметы проливаются" : "Предмет проливается",
              terse ? ':' : '.');
            ...
            if (targetbox) {
                if (Is_mbag(targetbox) && mbag_explodes(otmp, 0)) {
                    livelog_printf(LL_ACHIEVE,
                                 "только что взорвали %s сумку хранения опрокидыванием",
                                   uhis());
                    /* explicitly mention what item is triggering explosion */
                    urgent_pline(
                   "Когда %s %s внутри, вас поражает магический взрыв!",
                                 doname(otmp), otense(otmp, "катится"));
                ...
                if (altarizing) {
                    ...
                    pline("%s %s на %s.", Doname2(otmp),
                          otense(otmp, "бросить"), surface(ox, oy));
                } else {
    ...
    boolean *cancelled)
{
    ...
    if (n_conts < 1 || !u_handsy()) {
        if (n_conts >= 1)
            pline("Опрокидывание содержимого только на пол...");
        *cancelled = FALSE;
        return (struct obj *) 0;
    }
#endif
    ...
    add_menu(win, &nul_glyphinfo, &any, '-', 0, ATR_NONE, clr,
             ...
             "на полу", MENU_ITEMFLAGS_SELECTED);
    add_menu_str(win, "");
    ...
    Sprintf(buf, "Куда опрокинуть содержимое %s", doname(box));
    end_menu(win, buf);
    n = select_menu(win, PICK_ONE, &pick_list);
    destroy_nhwindow(win);
    ...
    boolean allowempty)    /* affects result when box is empty */
{
    ...
    if (box->olocked) {
        pline("%s заперто.", upstart(thesimpleoname(box)));
        ...
        if (gm.multi >= 0) { /* in case we didn't become paralyzed */
            nomul(-1);
            gm.multi_reason = "опрокидывание контейнера";
            gn.nomovemsg = "";
        }
        ...
        if (!Has_contents(box)) /* evidently a live cat came out */
            ...
            pline("%sящик теперь пуст.", Shk_Your(yourbuf, box));
        ...
        box->cknown = 1;
        pline("%s пуст.", upstart(thesimpleoname(box)));
