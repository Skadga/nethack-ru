...
mon_yells(struct monst *mon, const char *shout)
{
    if (Deaf) {
        ...
        if (canspotmon(mon)) {
            pline_mon(mon, "%s кричит:", Amonnam(mon));
        } else {
            /* Soundeffect(se_someone_yells, 75); */
            You_hear("чей-то крик:");
        }
        SetVoice(mon, 0, 80, 0);
        verbalize1(shout);
    }
        ...
        if (picking_lock(&x, &y) && IS_DOOR(levl[x][y].typ)
            ...
            if (couldsee(mtmp->mx, mtmp->my)) {
                if (levl[x][y].looted & D_WARNED) {
                    ...
                    mon_yells(mtmp, "Эй, прекрати взламывать этот замок!");
...
bee_eat_jelly(struct monst *mon, struct obj *obj)
{
    ...
    if (!mtmp) {
        ...
        if (obj->quan > 1L)
            obj = splitobj(obj, 1L);
        if (canseemon(mon))
            pline_mon(mon, "%s ест %s.", Monnam(mon), an(xname(obj)));
        delobj(obj);
        ...
        if (!mtmp->mflee && fleemsg && canseemon(mtmp)
            ...
            if (!mtmp->mcanmove || !mtmp->data->mmove) {
                ...
                if (Unaware) {
                    ...
                    verbalize("Яркий свет!");
                }
            } else {
                pline_mon(mtmp, "%s обращается в бегство.", Monnam(mtmp));
            }
        }
...
mind_blast(struct monst *mtmp)
{
    ...
    if (canseemon(mtmp))
        pline_mon(mtmp, "%s концентрируется.", Monnam(mtmp));
    if (mdistu(mtmp) > BOLT_LIM * BOLT_LIM) {
        You("ощущаете слабую волну пси-энергии.");
        return;
    }
    pline("Волна психической энергии накатывает на вас!");
    if (mtmp->mpeaceful
        && (!Conflict || resist_conflict(mtmp))) {
        pline("Это довольно успокаивает.");
    ...
    for (m2 = fmon; m2; m2 = nmon) {
        ...
        if ((telepathic(m2->data) && (rn2(2) || m2->mblinded)) || !rn2(10)) {
            ...
            if (cansee(m2->mx, m2->my))
                pline("Оно фиксируется на %s.", mon_nam(m2));
            m2->mhp -= rnd(15);
            if (DEADMONSTER(m2))
...
itsstuck(struct monst *mtmp)
{
    if (sticks(gy.youmonst.data) && mtmp == u.ustuck && !u.uswallow) {
        pline_mon(mtmp, "%s не может сбежать от вас!", Monnam(mtmp));
        return TRUE;
    }
    return FALSE;
}
    ...
    if (webmaker(mtmp->data)
        ...
        if (rn2(1000) < prob
            ...
            if (cansee(mtmp->mx, mtmp->my)) {
                ...
                pline_mon(mtmp, "%s плетёт паутину.", upstart(mbuf));
                trap->tseen = 1;
            }
        ...
        canseeit = didseeit || cansee((who)->mx, (who)->my);    \
            ...
            if ((here->doormask & (D_LOCKED | D_CLOSED)) != 0
                ...
                if (btrapped) {
                    ...
                    if (flags.verbose) {
                        if (canseeit && canspotmon(mtmp)) {
                            pline_mon(mtmp, "%s отпирает и открывает дверь.",
                                  Monnam(mtmp));
                        } else if (canseeit) {
                            You_see("дверь отпирается и открывается.");
                        } else if (!Deaf) {
                            You_hear("дверь отпирается и открывается.");
                        }
                    }
                }
                ...
                if (btrapped) {
                    ...
                    if (flags.verbose) {
                        if (canseeit && canspotmon(mtmp)) {
                            pline_mon(mtmp, "%s открывает дверь.", Monnam(mtmp));
                        } else if (canseeit) {
                            You_see("дверь открывается.");
                        } else if (!Deaf) {
                            You_hear("дверь открывается.");
                        }
                    }
                }
                ...
                if (btrapped) {
                    ...
                    if (flags.verbose) {
                        if (canseeit && canspotmon(mtmp)) {
                            pline_mon(mtmp, "%s с грохотом сносит дверь.",
                                      Monnam(mtmp));
                        } else if (canseeit) {
                            You_see("дверь с грохотом распахивается.");
                        } else if (!Deaf) {
                            You_hear("дверь с грохотом распахивается.");
                        }
                    }
                }
...
#undef UnblockDoor
            ...
            if (!(levl[mtmp->mx][mtmp->my].wall_info & W_NONDIGGABLE)
                && (dmgtype(ptr, AD_RUST) || dmgtype(ptr, AD_CORR)
                    || metallivorous(ptr))) {
                if (canseemon(mtmp))
                    pline_mon(mtmp, "%s прогрызает железные прутья.",
                              Monnam(mtmp));
