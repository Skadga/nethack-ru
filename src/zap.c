...
static const char are_blinded_by_the_flash[] = "ослеплены вспышкой!";
...
static const char *const flash_types[] = {
    "магическая стрела", /* Wands must be 0-9 */
    "огненный разряд", "ледяной разряд", "луч сна", "луч смерти",
    "молния", "", "", "", "",

    "магическая стрела", /* Spell equivalents must be 10-19 */
    "огненный шар", "конус холода", "луч сна", "палец смерти",
    "молния", /* there is no spell, used for retribution */
    "", "", "", "",

    "взрыв стрел", /* Dragon breath equivalents 20-29*/
    "взрыв огня", "взрыв мороза", "взрыв усыпляющего газа",
    "взрыв дезинтеграции", "взрыв молнии",
    "взрыв ядовитого газа", "взрыв кислоты", "", ""
};
...
bhitm(struct monst *mtmp, struct obj *otmp)
{
    ...
    const char *zap_type_text = "заклинание";
    struct obj *obj;
...
#define box_or_door(monst) \
    ...
    case SPE_SLOW_MONSTER:
        if (!resist(mtmp, otmp->oclass, 0, NOTELL)) {
            ...
            if (engulfing_u(mtmp) && is_whirly(mtmp->data)) {
                You("развеиваете %s!", mon_nam(mtmp));
                pline("Открывается огромная дыра...");
    ...
    case POT_POLYMORPH:
        if (mtmp->data == &mons[PM_LONG_WORM] && has_mcorpsenm(mtmp)) {
            ...
            if (mtmp->cham == NON_PM && !rn2(25)) {
                if (canseemon(mtmp)) {
                    pline("%s содрогается!", Monnam(mtmp));
                    learn_it = TRUE;
                }
                /* svc.context.bypasses = TRUE; ## for make_corpse() */
    ...
    case WAN_MAKE_INVISIBLE: {
        ...
        if (!oldinvis && knowninvisible(mtmp)) {
            pline("%s становится прозрачным!", nambuf);
            reveal_invis = TRUE;
            learn_it = TRUE;
        ...
        /* zap which hits steed will only release saddle if it
           ...
           here overrides the more logical target ordering */
            ...
            if (mtmp->data->msize < MZ_HUMAN && !m_is_steadfast(mtmp)) {
                if (canseemon(mtmp))
                    pline("%s отброшен назад!",
                          Monnam(mtmp));
                mhurtle(mtmp, mtmp->mx - u.ux, mtmp->my - u.uy, rnd(2));
            } else {
                if (canseemon(mtmp))
                    pline("%s не двигается с места.", Monnam(mtmp));
            }
            if (!DEADMONSTER(mtmp)) {
            ...
            if (cansee(mtmp->mx, mtmp->my)) {
                ...
                pline("%s падает на %s.", buf,
                      surface_ru(mtmp->mx, mtmp->my));
            } else if (canspotmon(mtmp)) {
                pline("%s падает вниз.", buf);
            }
        ...
        if (mtmp->data != &mons[PM_PESTILENCE]) {
            ...
            if (canseemon(mtmp)) {
                if (disguised_mimic) {
                    ...
                    pline("%s выглядит%s лучше.", Monnam(mtmp),
                          otyp == SPE_EXTRA_HEALING ? " намного" : "");
            }
    ...
    case SPE_STONE_TO_FLESH:
        if (mtmp->data->mlet == S_GOLEM) {
            ...
            if (monsndx(mtmp->data) == PM_STONE_GOLEM
                ...
                mesg = "превращается в плоть!";
            else if (monsndx(mtmp->data) == PM_FLESH_GOLEM)
                mesg = "кажется более плотским...";
            else
                mesg = "на мгновение кажется довольно плотским.";
    ...
    case SPE_DRAIN_LIFE:
        ...
        if (resists_drli(mtmp)) {
            ...
            if (DEADMONSTER(mtmp) || mtmp->mhpmax <= 0 || mtmp->m_lev < 1) {
                ...
                mtmp->m_lev--;
                if (canseemon(mtmp))
                    pline("%s внезапно кажется слабее!", Monnam(mtmp));
            }
        }
        break;
    case WAN_NOTHING:
...
release_hold(void)
{
    ...
    if (!mtmp) {
        ...
        if (digests(mtmp->data)) {
            if (!Blind)
                pline("%s открывает рот!", Monnam(mtmp));
            else
                You_feel("внезапный поток воздуха!");
        }
        ...
        You("освобождаетесь от %s.", mon_nam(mtmp));
    } else { /* held but not swallowed */
        char relbuf[BUFSZ];

        unstuck(u.ustuck);
        if (!nohands(mtmp->data))
            Sprintf(relbuf, "из хватки %s", s_suffix(mon_nam(mtmp)));
        else
            Sprintf(relbuf, "у %s", mon_nam(mtmp));
        You("освобождены %s.", relbuf);
    }
}
...
probe_monster(struct monst *mtmp)
{
    ...
    if (mtmp->minvent) {
        ...
        pline("%s ничего не несёт%s.", noit_Monnam(mtmp),
              engulfing_u(mtmp) ? ", кроме вас" : "");
    }
}
...
revive(struct obj *corpse, boolean by_hero)
{
    ...
    if (by_hero) {
        ...
        if (cansee(x, y)) {
            ...
            if (one_of) /* could be simplified to ''corpse->quan = 1L;'' */
                corpse->quan--;
            pline("%s переливается радужными цветами.", upstart(buf));
    ...
    if (has_omid(corpse)) {
        ...
        if (ghost && ghost->data == &mons[PM_GHOST]) {
            if (canseemon(ghost))
                pline("%s внезапно втягивается в своё прежнее тело!",
                      Monnam(ghost));
...
unturn_dead(struct monst *mon)
{
    ...
    while ((otmp = otmp2) != 0) {
        ...
        if ((mtmp2 = revive(otmp, !svc.context.mon_moving)) != 0) {
            ...
            if (youseeit)
                pline("%s%s внезапно %s%s%s!", owner, corpse,
                      nonliving(mtmp2->data) ? "оживает" : "восстаёт",
                      different_type ? " как " : "",
                      different_type ? an(mon_pmname(mtmp2)) : "");
            else if (canseemon(mtmp2))
                pline("%s внезапно появляется!", Amonnam(mtmp2));
        } else {
...
unturn_you(void)
{
    ...
    if (is_undead(gy.youmonst.data)) {
        You_feel("испуганы и %sошеломлены.", Stunned ? "ещё больше " : "");
        make_stunned((HStun & TIMEOUT) + (long) rnd(30), FALSE);
    } else {
        You("содрогаетесь от ужаса.");
    }
}
...
create_polymon(struct obj *obj, int okind)
{
    ...
    if (mtmp && cansee(mtmp->mx, mtmp->my)) {
        pline("Некоторые %sобъекты сливаются, и из кучи возникает %s!", material,
              a_monnam(mtmp));
    }
}
        ...
        if ((!obj->no_charge
            ...
            if (shkp->mpeaceful) {
                if (*u.ushops
                    ...
                    pline("%s злится!", Shknam(shkp));
                    hot_pursuit(shkp);
                }
            } else
                Norep("%s в ярости!", Shknam(shkp));
        }
    }
    delobj(obj);
    return otmp;
...
stone_to_flesh_obj(struct obj *obj) /* nonnull */
{
            ...
            if (obj->otyp == STATUE) {
                ...
                if (mon) {
                    ...
                    if (cansee(mon->mx, mon->my))
                        pline("Фигурка %sоживает!",
                                  golem_xform ? "Превращается в плоть и " : "");
                }
            }
            if (mon) {
                ptr = mon->data;
    ...
    if (smell) {
        ...
        if (Role_if(PM_MONK) || !u.uconduct.unvegetarian
            ...
            Norep("Вы чувствуете запах мяса.");
        else
            Norep("Вы чувствуете восхитительный запах.");
    }
    newsym(oox, ooy);
    return res;
}
...
bhito(struct obj *obj, struct obj *otmp)
{
    ...
    if (obj->bypass) {
        ...
        if (svc.context.bypasses) {
            ...
            debugpline1("%s на мгновение.", Tobjnam(obj, "пульсирует"));
            obj->bypass = 0;
        }
    }
    ...
    if (obj == uball) {
        ...
        case SPE_POLYMORPH:
            ...
            if (!u.uconduct.polypiles++)
                livelog_printf(LL_CONDUCT, "превратил %s первый объект",
                               uhis());
        ...
        case WAN_PROBING:
            ...
            if (Is_container(obj) || obj->otyp == STATUE) {
                ...
                if (Is_box(obj) && !obj->tknown) {
                    ...
                    if (obj->otrapped)
                        pline("%s в ловушке!", Tobjnam(obj, "оказывается"));
                    obj->tknown = 1;
                }

                if (!obj->cobj) {
                    pline("%s пусто.", Tobjnam(obj, "оказывается"));
        ...
        case SPE_FORCE_BOLT:
            ...
            if (obj->otyp == BOULDER) {
                ...
                if (cansee(obj->ox, obj->oy))
                    pline("Валун рассыпается.");
                else
                    You_hear("звук осыпания.");
                fracture_rock(obj);
            } else if (obj->otyp == STATUE) {
                if (break_statue(obj)) {
                    if (cansee(obj->ox, obj->oy)) {
                        if (Hallucination)
                            pline("%s Разлетается вдребезги.", rndmonnam(NULL));
                        else
                            pline("Статуя разлетается вдребезги.");
                    } else
                        You_hear("звук осыпания.");
                }
            } else {
                ...
                if (!mtmp) {
                    ...
                    if (cansee(ox, oy)) {
                        ...
                        if (Role_if(PM_HEALER) && !Deaf
                            ...
                            if (!Hallucination)
                                You_hear("как %s оживает.", corpsname);
                            else
                                You_hear("звук дефибриллятора.");
                            learn_it = by_u ? TRUE : gz.zap_oseen;
                        }
                        if (canspotmon(mtmp))
                            ...
                            pline("%s появляется.", Monnam(mtmp));
                    }
                    if (learn_it)
                        exercise(A_WIS, TRUE);
                }
...
zappable(struct obj *wand)
{
    if (wand->spe < 0 || (wand->spe == 0 && rn2(WAND_WREST_CHANCE)))
        return 0;
    if (wand->spe == 0)
        You("выжимаете последний заряд из изношенной палочки.");
    wand->spe--;
    return 1;
}
...
{
    You_feel("познавшим себя...");
    display_nhwindow(WIN_MESSAGE, FALSE);
    enlightenment(MAGICENLIGHTENMENT, ENL_GAMEINPROGRESS);
    pline("Ощущение утихает.");
    exercise(A_WIS, TRUE);
}
...
zapnodir(struct obj *obj)
{
    ...
    case WAN_WISHING:
        if (Luck + rn2(5) < 0) {
            pline("К несчастью, ничего не происходит.");
            known = FALSE;
        } else {
            known = !!obj->dknown;
...
dozap(void)
{
    ...
    if (nohands(gy.youmonst.data)) {
        You("не можете применить ничего в текущей форме.");
        return ECMD_OK;
    }
    if (check_capacity((char *) 0))
        return ECMD_OK;
    obj = getobj("пустить луч", zap_ok, GETOBJ_NOFLAGS);
    if (!obj)
        return ECMD_CANCEL;
    ...
    if (!zappable(obj)) {
        ...
        if (!Blind)
            pline("%s светится и гаснет.", The(xname(obj)));
        /* make him pay for knowing !NODIR */
    } else if (need_dir && !u.dx && !u.dy && !u.dz) {
        if ((damage = zapyourself(obj, TRUE)) != 0) {
            char buf[BUFSZ];

            Sprintf(buf, "выстрелил %sв себя из %s",
    ...
    if (obj && obj->spe < 0) {
        pline("%s в пыль.", Tobjnam(obj, "превращается"));
...
zapyourself(struct obj *obj, boolean ordinary)
{
    ...
    case WAN_LIGHTNING:
        ...
        orig_dmg = d(12, 6);
        if (!Shock_resistance) {
            You("бьёте себя током!");
            damage = orig_dmg;
            exercise(A_CON, FALSE);
            monstunseesu(M_SEEN_ELEC);
        } else {
            shieldeff(u.ux, u.uy);
            You("пускаете в себя луч, но, кажется, невредимы.");
    ...
    case SPE_FIREBALL:
        You("взрываете огненный шар прямо над собой!");
    ...
    case FIRE_HORN:
        ...
        if (Fire_resistance) {
            shieldeff(u.ux, u.uy);
            You_feel("довольно тепло.");
            monstseesu(M_SEEN_FIRE);
            ugolemeffects(AD_FIRE, orig_dmg);
        } else {
            pline("Вы подожгли себя!");
            damage = orig_dmg;
    ...
    case SPE_MAGIC_MISSILE:
        ...
        if (Antimagic) {
            ...
            damage = d(4, 6);
            pline("Идиот!  Вы подстрелили себя!");
            monstunseesu(M_SEEN_MAGR);
        }
        break;
    ...
    case SPE_DRAIN_LIFE:
        if (!Drain_resistance) {
            ...
            losexp("высасывание жизненной силы");
        }
    ...
    case WAN_MAKE_INVISIBLE: {
        ...
        if (BInvis && uarmc->otyp == MUMMY_WRAPPING) {
            ...
            You_feel("довольно сильный зуд под %s.", yname(uarmc));
            break;
        }
    ...
    case SPE_SLEEP:
        ...
        if (Sleep_resistance) {
            shieldeff(u.ux, u.uy);
            You("не чувствуете сонливости!");
            monstseesu(M_SEEN_SLEEP);
        } else {
            if (ordinary)
                pline("Луч сна попадает в вас!");
            else
                You("засыпаете!");
    ...
    case SPE_FINGER_OF_DEATH:
        if (nonliving(gy.youmonst.data) || is_demon(gy.youmonst.data)) {
            pline((obj->otyp == WAN_DEATH)
                      ? "Палочка выпускает в вас, видимо, безвредный луч."
                      : "Вы выглядите не более мёртвым, чем раньше.");
            break;
        }
        learn_it = TRUE;
        Sprintf(svk.killer.name, "выстрелил %sв себя из луча смерти", uhim());
        ...
        urgent_pline("Вы облучаете себя чистой энергией!");
        urgent_pline("Вы умираете.");
    ...
    case SPE_EXTRA_HEALING:
        ...
        You_feel("%sлучше.", obj->otyp == SPE_EXTRA_HEALING ? "намного " : "");
        break;
    case WAN_LIGHT: /* (broken wand) */
        /* assert( !ordinary ); */
        damage = d(obj->spe, 25);
    ...
    int amt)          /* pseudo-damage used to determine blindness duration */
{
    ...
    if (dmg && gy.youmonst.data == &mons[PM_GREMLIN]) {
        ...
        if (dmg > 20)
            dmg = 20;
        pline("Ой, этот свет больно бьёт%c", (dmg > 2 || u.mh <= 5) ? '!' : '.');
        ...
        how = (obj->oclass == SPBOOK_CLASS) ? "заклинание света"
              : (!obj->oartifact) ? ansimpleoname(obj)
                : bare_artifactname(obj);
        Sprintf(buf, "%s %s выстрелил в себя из %s", ordinary ? "поразили" : "взрывом",
                uhim(), how);
...
{
    static const char
        writing_vanishes[] = "Часть письмен исчезает с %s головы!",
        your[] = "вашей"; /* should be extern */
        ...
        if (Upolyd) { /* includes lycanthrope in creature form */
            ...
            if (u.umonnum == PM_CLAY_GOLEM) {
                if (!Blind)
                    ...
                    You_feel("%s головой.", Hallucination ? "тяжёлой" : "лёгкой");
                u.mh = 0; /* fatal; death handled by rehumanize() */
            }
            if (Unchanging && u.mh > 0)
                Your("амулет на миг раскаляется, затем остывает.");
            else
                rehumanize();
        }
    } else {
...
zap_updown(struct obj *obj) /* wand or spell, nonnull */
{
    ...
    case WAN_PROBING:
        ptmp = 0;
        if (u.dz < 0) {
            You("тянетесь зондом к %s.", ceiling(x, y));
        } else { /* down */
            const char *surf;
            ...
            You("зондируете пространство под %s.", surf);
            ptmp += display_binventory(x, y, TRUE);
        }
        if (!ptmp)
            Your("зонд ничего не обнаруживает.");
    ...
    case SPE_KNOCK:
        ...
        if (is_db_wall(x, y) && find_drawbridge(&xx, &yy)) {
            ...
            pline("Лестница на миг словно колеблется.");
            disclose = TRUE;
        }
    ...
    case SPE_WIZARD_LOCK:
        ...
        if (((levl[x][y].typ == DRAWBRIDGE_DOWN)
            ...
            if (!striking && closeholdingtrap(&gy.youmonst, &disclose)) {
                ...
                if (Blind && !ttmp->tseen) {
                    pline("%s под вами разлетается вдребезги.", Something);
                } else if (!ttmp->tseen) { /* => !Blind */
                    pline("Под вами люк-ловушка; он разлетается вдребезги.");
                } else {
                    pline("Люк-ловушка под вами разлетается вдребезги.");
                    disclose = TRUE;
                }
                ttmp->ttyp = HOLE;
                ttmp->tseen = 1;
                ...
                ttmp->ttyp = TRAPDOOR;
                if (Blind || !ttmp->tseen) {
                    pline("Под вами кружится %s.",
                          is_ice(x, y) ? "мороз" : "пыль");
                } else {
                    ttmp->tseen = 1;
                    newsym(x, y);
                    pline("Под вами появляется люк-ловушка.");
                    disclose = TRUE;
                }
    ...
    case SPE_STONE_TO_FLESH:
        if (Is_airlevel(&u.uz) || Is_waterlevel(&u.uz) || Underwater
            ...
            pline("Кровь капает на ваше %s.", body_part(FACE));
            ...
            */
            ...
            if (!(e && e->engr_type == ENGRAVE)) {
                if (is_pool(u.ux, u.uy) || is_ice(u.ux, u.uy))
                    ...
                    pline("Кровь %s %s вашей %s.",
                          is_lava(u.ux, u.uy) ? "кипит" : "скапливается",
                          Levitation ? "под" : "у",
    ...
    const char *force)  /* usually either "." or "!" via exclam() */
{
    ...
    pline("%s %s %s%s", The(str), vtense(str, "ударяет"),
          verbosely ? mon_nam(mtmp) : "его", force);
}

void
miss(const char *str, struct monst *mtmp)
{
    pline("%s %s %s.", The(str), vtense(str, "промахивается"),
          ((cansee(gb.bhitpos.x, gb.bhitpos.y) || canspotmon(mtmp))
           && flags.verbose) ? mon_nam(mtmp) : "его");
}
    ...
    struct obj *obj) /* zapped wand, or book for cast spell */
{
    ...
    if (obj->otyp == WAN_PROBING) {
        ...
        if (ltyp == SDOOR) {
            ...
            newsym(x, y);
            if (cansee(x, y)) {
                pline("Зондирование обнаруживает потайную дверь.");
                learn_it = TRUE;
        ...
        /* secret corridor likewise, although only ones within view will
           ...
           above has already converted the spot to regular corridor */
            ...
            unblock_point(x, y);
            newsym(x, y);
            pline("Зондирование обнаруживает потайной коридор.");
            learn_it = TRUE;
                                   ...
                                   * if object is destroyed */
{
    ...
    while (range-- > 0) {
        ...
        if (!mtmp && ttmp && (ttmp->ttyp == WEB)
            ...
            && !rn2(3)) {
            if (cansee(x, y)) {
                pline("%s застревает в паутине!", Yname2(obj));
                ttmp->tseen = TRUE;
                newsym(x, y);
            }
            if (was_returning)
        ...
        if (skiprange_start && (range == skiprange_start) && allow_skip) {
            if (is_pool(x, y) && !mtmp) {
                in_skip = TRUE;
                if (!Blind)
                    pline("%s %s%s.", Yname2(obj), otense(obj, "скачет"),
                          skipcount ? " снова" : "");
                else
                    You_hear("как %s скачет.", yname(obj));
                skipcount++;
        ...
        if (in_skip) {
            if (range <= skiprange_end) {
                ...
                if (!Blind && canspotmon(mtmp))
                    pline("%s %s над %s.", Yname2(obj), otense(obj, "проносится"),
                          mon_nam(mtmp));
                mtmp = (struct monst *) 0;
            }
        }
        ...
        if (weapon == THROWN_WEAPON && range > 0
            ...
            if ((bobj = sobj_at(BOULDER, x, y)) != 0) {
                if (cansee(x, y))
                    pline("%s попадает в %s.", The(distant_name(obj, xname)),
                          an(xname(bobj)));
                range = 0;
            } else if (obj == uball) {
    ...
    if (shopdoor)
        pay_for_damage("разрушение", FALSE);
    ...
    coordxy sx, coordxy sy)
{
    ...
    case ZT_LIGHTNING:
        ...
        break;
    case ZT_POISON_GAS:
        poisoned("взрыв", A_DEX, "ядовитый взрыв", 15, FALSE);
        break;
    case ZT_ACID:
        if (Acid_resistance) {
            pline("%s Не причиняет вреда.", hliquid("acid"));
            monstseesu(M_SEEN_ACID);
            dam = 0;
        } else {
            pline("%s Обжигает!", hliquid("acid"));
            dam = d(nd, 6);
            exercise(A_STR, FALSE);
     ...
     */
    {
        ...
        const char *verb = (abstyp < 10) /* wand */
                           ? ((otmp && otmp->oclass == TOOL_CLASS) ? "сыграли"
                              : "поразили")
                           : (abstyp < 20) ? "сотворили"
                             : (abstyp < 30) ? "выдохнули"
                               : "вообразили"; /* should never happen */
    ...
    for (obj = svl.level.objects[x][y]; obj; obj = obj2) {
        ...
        if (obj->oclass == SCROLL_CLASS || obj->oclass == SPBOOK_CLASS
            ...
            if (delquan) {
                ...
                if (give_feedback) {
                    if (delquan > 1L)
                        pline("%ld %s горят.", delquan, buf2);
                    else
                        pline("%s горит.", An(buf1));
                }
            }
        }
    }
    ...
    const char *fltxt)
{
    ...
    if (canseemon(mon)) {
        if (!m_amulet)
            pline("%s дезинтегрирован!", Monnam(mon));
        else
            hit(fltxt, mon, "!");
    }
    ...
    boolean forcemiss)
{
    ...
    if (u.uswallow) {
        ...
        if (!u.ustuck) {
            ...
            pline("%s врезается в %s%s", The(flash_str(fltyp, FALSE)),
 ...
 buzzmonst:
                ...
                if (mon_reflects(mon, (char *) 0)) {
                    ...
                    if (is_rider(mon->data)
                        ...
                        if (canseemon(mon)) {
                            ...
                            pline("%s дезинтегрируется.", Monnam(mon));
                            pline("%s тело снова становится целым у ваших %s!",
                                  s_suffix(Monnam(mon)),
                                  (eyecount(gy.youmonst.data) == 1)
                                      ? body_part(EYE)
                                      : makeplural(body_part(EYE)));
                            pline("%s воскресает!", Monnam(mon));
                        }
                        mon->mhp = mon->mhpmax;
                    ...
                    if (mon->data == &mons[PM_DEATH] && damgtype == ZT_DEATH) {
                        if (canseemon(mon)) {
                            hit(flash_str(fltyp, FALSE), mon, ".");
                            pline("%s поглощает смертоносный %s!", Monnam(mon),
                                  type == ZT_BREATH(ZT_DEATH) ? "взрыв"
                                                              : "луч");
                            pline("Похоже, он стал даже сильнее прежнего.");
                        }
                        break; /* Out of while loop */
                    }
            ...
            if (u.usteed && !rn2(3) && !mon_reflects(u.usteed, (char *) 0)) {
                ...
                range -= 2;
                pline_dir(xytodir(-dx, -dy), "%s попадает в вас!",
                          The(flash_str(fltyp, FALSE)));
                if (Reflecting) {
                    if (!Blind) {
                        (void) ureflects("Но %s отражается от вашего %s!",
                                         "он");
                    } else
                        pline("По какой-то причине вы не затронуты.");
                ...
                pline("%s с шумом пролетает мимо вас!", The(flash_str(fltyp, FALSE)));
            } else if (damgtype == ZT_LIGHTNING) {
                Your("%s покалывает.", body_part(ARM));
            }
 ...
 make_bounce:
            ...
            if ((--range > 0 && isok(lsx, lsy) && cansee(lsx, lsy))
                ...
                if (Is_airlevel(&u.uz)) { /* nothing to bounce off of */
                    ...
                    pline("%s Отскакивает!", flash_str(fltyp, FALSE));
            }
    ...
    if (shopdamage)
        pay_for_damage(damgtype == ZT_FIRE ? "сгорает"
                       : damgtype == ZT_COLD ? "разбивается"
                         /* "damage" indicates wall rather than door */
                         : damgtype == ZT_ACID ? "повредить"
                           : damgtype == ZT_DEATH ? "дезинтегрировать"
                             : "разрушить",
                       FALSE);
                               ...
                               * when a lit potion of oil explodes */
{
    ...
    case ZT_FIRE:
        ...
        if (is_ice(x, y)) {
            ...
            const char *msgtxt = (!Deaf)
                                 ? "Вы слышите шипение газа." /* Deaf-aware */
                                 : (type >= 0)
                                   ? "Это было на удивление незаметно."
                                   : (char *) 0;
            ...
            if (lev->typ != POOL) { /* MOAT or DRAWBRIDGE_UP or WATER */
                t = (struct trap *) 0;
                if (on_water_level)
                    msgtxt = (see_it || !Deaf) ? "Часть воды закипает." : 0;
                else if (see_it)
                    msgtxt = "Часть воды испаряется.";
            } else {
                rangemod -= 3;
    ...
    case ZT_COLD:
        if (is_pool(x, y) || is_lava(x, y) || lavawall) {
            ...
            if (IS_WATERWALL(lev->typ) || (lavawall && rn2(chance))) {
                ...
                if (see_it)
                    pline("%s На миг замерзает.",
                              hliquid(lavawall ? "lava" : "water"));
                else
                    You_hear("лёгкое потрескивание.");
                rangemod -= 1000; /* stop */
            } else {
                char buf[BUFSZ];
                ...
                if (see_it) {
                    if (lava)
                        Norep("%s остывает и затвердевает.",
                              hliquid("lava"));
                    else if (moat)
                        Norep("%s покрыт ледяным мостом!", buf);
                    else
                        Norep("%s замерзает.", hliquid("water"));
                    newsym(x, y);
                } else if (!lava) {
                    You_hear("потрескивание.");
                }
                if (u_at(x, y)) {
                    if (u.uinwater) { /* not just `if (Underwater)' */
                        ...
                        if (Passes_walls) {
                            You("проходите сквозь затвердевшую породу.");
                            reset_utrap(TRUE);
                        } else {
                            set_utrap(rn1(50, 20), TT_INFLOOR);
                            You("крепко застреваете в остывающей породе.");
                        }
                    }
    ...
    case ZT_ACID:
        if (lev->typ == IRONBARS) {
            ...
            if ((lev->wall_info & W_NONDIGGABLE) != 0) {
                if (see_it)
                    Norep("%s %s местами, но остаётся целым.",
                          defsyms[S_bars].explanation,
                          (damgtype == ZT_ACID) ? "разъедается" : "плавится");
                /* but nothing actually happens... */
            } else {
                rangemod -= 3;
                if (see_it)
                    Norep("%s %s.", defsyms[S_bars].explanation,
                          (damgtype == ZT_ACID) ? "разъедается" : "плавится");
                dissolve_bars(x, y);
    ...
    if (levl[x][y].typ == SDOOR) {
        ...
        newsym(x, y);
        if (see_it)
            pline("%s %s вскрывает потайную дверь.",
                  yourzap ? "Ваш" : "Этот", zapverb);
    ...
    if (closed_door(x, y)) {
        ...
        case ZT_FIRE:
            ...
            see_txt = "Дверь охвачена пламенем!";
            sense_txt = "чувствуете запах дыма.";
            break;
        case ZT_COLD:
            new_doormask = D_NODOOR;
            see_txt = "Дверь замерзает и разлетается вдребезги!";
            hear_txt = "глубокий треск.";
            break;
        case ZT_DEATH:
            ...
            if (abs(type) != ZT_BREATH(ZT_DEATH))
                goto def_case;
            new_doormask = D_NODOOR;
            see_txt = "Дверь дезинтегрируется!";
            hear_txt = "треск разлетающейся древесины.";
            break;
        case ZT_LIGHTNING:
            new_doormask = D_BROKEN;
            see_txt = "Дверь разлетается на щепки!";
            hear_txt = "потрескивание.";
            break;
        default:
 def_case:
            if (exploding_wand_typ > 0) {
                ...
                if (exploding_wand_typ == WAN_STRIKING) {
                    new_doormask = D_BROKEN;
                    see_txt = "Дверь с грохотом распахивается!";
                    sense_txt = "чувствуете порыв прохладного воздуха.";
                    break;
                }
            }
            if (see_it) {
                ...
                if (exploding_wand_typ)
                    pline("Дверь остаётся целой.");
                else
                    pline("Дверь поглощает %s %s!", yourzap ? "Ваш" : "Весь",
                              zapverb);
            } else
                You_feel("вибрацию.");
            break;
    ...
    if (OBJ_AT(x, y) && damgtype == ZT_FIRE)
        if (burn_floor_objects(x, y, FALSE, type > 0) && couldsee(x, y)) {
            newsym(x, y);
            You("%s дыма.", !Blind ? "видите клуб" : "чувствуете запах");
        }
...
fracture_rock(struct obj *obj) /* no texts here! */
{
    ...
    if (by_you && get_obj_location(obj, &x, &y, 0) && costly_spot(x, y)) {
        ...
        if (billable(&shkp, obj, objroom, FALSE)) {
            ...
            You("разбиваете %s %s.", s_suffix(shkname(shkp)), xname(obj));
...
break_statue(struct obj *obj)
{
    ...
    if (by_you && Role_if(PM_ARCHEOLOGIST)
        ...
        You_feel("виновным в повреждении такой исторической статуи.");
        adjalign(-1);
    }
    obj->spe = 0;
    fracture_rock(obj);
...
item_what(int dmgtyp)
{
    ...
    if (wizard) {
        ...
        if (what) /* strlen(what) will be less than 30 */
            Sprintf(whatbuf, " вашей %.40s", what);
    }
    return whatbuf;
}
...
const char *const destroy_strings[][3] = {
    /* also used in trap.c */
    { "замерзает и разбивается", "замерзают и разбиваются", "разбитое зелье" },
    { "закипает и взрывается", "закипают и взрываются", "кипящее зелье" },
    { "воспламеняется и взрывается", "воспламеняются и взрываются", "взрывающееся зелье" },
    { "загорается и сгорает", "загораются и сгорают", "горящий свиток" },
    { "загорается и сгорает", "", "горящая книга" },
    { "превращается в пыль и исчезает", "", "" },
    { "разваливается и взрывается", "", "взрывающаяся палочка" },
};
    ...
    case AD_FIRE:
        ...
        if (obj->otyp == SPE_BOOK_OF_THE_DEAD) {
            ...
            if (u_carry ? !Blind : vis) {
                pline("%s светится странным %s, но остаётся целым.",
    ...
    if (chargeit) {
        ...
        if (u_carry || vis) {
            mult = (cnt == 1L) ? ((quan == 1L) ? "" /* 1 of 1 */
                                  : "Одна из ")      /* 1 of N */
                   : ((cnt < quan) ? "Некоторые из "     /* n of N */
                      : (quan == 2L) ? "Обе из "   /* 2 of 2 */
                        : "Все из ");               /* N of N */
            pline("%s%s %s!", mult,
        ...
        if (dmg) {
            ...
            if (xresist) {
                ...
                if (dmgtyp == AD_FIRE && osym == FOOD_CLASS)
                    how = "взрывающийся сгусток слизи";
...
wishcmdassist(int triesleft)
{
  ...
  "Детали желания:",
  "",
  "Введите название объекта, например \"зелье обнаружения монстров\",",
  "\"свиток с надписью README\", \"эльфийский мифриловый плащ\", или \"Grimtooth\"",
  "(без кавычек).",
  "",
  "Для типов объектов, которые бывают в стопках, вы можете указать имя во множественном числе",
  "например \"зелья лечения\", или указать количество, например \"1000",
  "золотых\", хотя этот аспект вашего желания может не сбыться.",
  "",
  "Вы также можете указать различные значения префиксов, которые могут использоваться",
  "для изменения предмета, например \"незаклятое\" или \"нержавеющее\" или \"+1\".",
  "Можно указать большинство модификаторов, видимых при просмотре инвентаря.",
  "",
  "Вы можете указать 'nothing', чтобы явно отклонить это желание.",
  0,
    },
        preserve_wishless[] = "Таким образом вы сохраните показатель 'wishless'.",
        retry_info[] =
                    "Если вы указали нераспознанное название объекта %s%s раз%s,",
        retry_too[] = "будет выдана случайно выбранная вещь.",
        suppress_cmdassist[] =
            "(Отключить эту помощь с помощью !cmdassist в вашем конфигурационном файле.)",
        *cardinals[] = { "ноль",  "один",  "два", "три", "четыре", "пять" },
...
makewish(void)
{
    ...
    if (flags.verbose)
        You("можете пожелать объект.");
 retry:
    Strcpy(promptbuf, "Чего вы желаете");
    if (iflags.cmdassist && tries > 0)
        Strcat(promptbuf, " (введите 'help' для помощи)");
    Strcat(promptbuf, "?");
    ...
    if (!otmp) {
        pline("Ничего, соответствующего описанию, в игре не существует.");
        if (++tries < MAXWISHTRY)
            goto retry;
        ...
        livelog_printf(LL_WISH, "отказался от желания");
        return;
    ...
    Snprintf(wish, sizeof wish, "\"%s\", получено \"%s\"", bufcpy, doname(otmp));
    /* KMH, conduct */
    if (!u.uconduct.wishes++)
        livelog_printf((LL_CONDUCT | LL_WISH | maybe_LL_arti),
                       "совершил %s первое желание - %s", uhis(), wish);
    else if (!oldwisharti && u.uconduct.wisharti)
        livelog_printf((LL_CONDUCT | LL_WISH | LL_ARTIFACT),
                       "совершил %s первое желание артефакта - %s", uhis(), wish);
    else
        livelog_printf((LL_WISH | maybe_LL_arti), "пожелал %s", wish);
    ...
    const char *verb = ((Is_airlevel(&u.uz) || u.uinwater)
                        ...
                        : (otmp->otyp == CORPSE && otmp->wishedfor)
                          ? "материализуется" : "падает"),
               *oops_msg = (u.uswallow
                            ? "Ой!  %s вне вашей досягаемости!"
                            : (Is_airlevel(&u.uz) || Is_waterlevel(&u.uz)
                               || levl[u.ux][u.uy].typ < IRONBARS
                               || levl[u.ux][u.uy].typ >= ICE)
                               ? "Ой!  %s улетает от вас!"
                               : !(otmp->otyp == CORPSE && otmp->wishedfor)
                                 ? "Ой!  %s оказывается на полу!"
                                 : "Осторожно! %s на полу!");
    ...
    boolean nohallu) /* suppress hallucination (for death reasons) */
{
    ...
    if (Hallucination && !nohallu) {
        ...
        Sprintf(fltxt, "взрыв %s", rnd_hallublast());
    } else {
