...
#define ORC_LEADER 1
static const char *const orcfruit[] = { "кактус-весло", "корень гномов" };
...
fumaroles(void)
{
    ...
    if (snd && !Deaf)
        Norep("Вы слышите %sшипение!", loud ? "громкое " : "");  /* Deaf-aware */
}
...
water_friction(void)
{
    ...
    if (eff)
        pline("Водная помутность влияет на ваши движения.");
}
