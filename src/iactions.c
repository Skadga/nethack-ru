...
item_reading_classification(struct obj *obj, char *outbuf)
{
    ...
    *outbuf = '\0';
    if (otyp == FORTUNE_COOKIE) {
        Strcpy(outbuf, "Прочитать послание внутри этого печенья");
    } else if (otyp == T_SHIRT) {
        Strcpy(outbuf, "Прочитать надпись на футболке");
    } else if (otyp == ALCHEMY_SMOCK) {
        Strcpy(outbuf, "Прочитать надпись на фартуке");
    } else if (otyp == HAWAIIAN_SHIRT) {
        Strcpy(outbuf, "Рассмотреть узор на футболке");
        ...
        const char *magic = ((obj->dknown
                             ...
                             ? ", чтобы активировать его магию" : "");

        Sprintf(outbuf, "Прочитать этот свиток%s", magic);
        ...
        Sprintf(outbuf, "%s этот %s",
                (novel || blank) ? "Прочитать" : tome ? "Изучить" : "Штудировать",
                novel ? simpleonames(obj) /* "novel" or "paperback book" */
                      : tome ? "том" : "книга заклинаний");
    } else {
        res = IA_NONE;
    }
    return res;
...
itemactions(struct obj *otmp)
{
    ...
    menu_item *selected;
    struct monst *mtmp;
    const char *light = otmp->lamplit ? "Погасить" : "Зажечь";
    ...
    if (otmp == uwep || otmp == uswapwep || otmp == uquiver) {
        const char *verb = (otmp == uquiver) ? "В колчан" : "Взять в руку",
                   *action = (otmp == uquiver) ? "убрать из боеготовности" : "вынуть из руки",
                   *which = is_plural(otmp) ? "эти" : "этот",
    ...
    if (otmp->oclass == COIN_CLASS)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Подбросить монету");
    else if (otmp->otyp == CREAM_PIE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a',
                   "Ударить себя этим пирогом");
    else if (otmp->otyp == BULLWHIP)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Хлестнуть этим кнутом");
    else if (otmp->otyp == GRAPPLING_HOOK)
        ia_addmenu(win, IA_APPLY_OBJ, 'a',
                   "Зацепить что-то этим крюком");
    else if (otmp->otyp == BAG_OF_TRICKS && objects[otmp->otyp].oc_name_known)
        /* bag of tricks skips this unless discovered */
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Залезть в этот мешок");
    else if (Is_container(otmp))
        /* bag of tricks gets here only if not yet discovered */
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Открыть этот контейнер");
    else if (otmp->otyp == CAN_OF_GREASE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Использовать банку, чтобы смазать предмет");
    else if (otmp->otyp == LOCK_PICK
             || otmp->otyp == CREDIT_CARD
             || otmp->otyp == SKELETON_KEY)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Использовать этот инструмент, чтобы вскрыть замок");
    else if (otmp->otyp == TINNING_KIT)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Использовать этот набор, чтобы закатать труп");
    else if (otmp->otyp == LEASH)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Привязать питомца к этому поводку");
    else if (otmp->otyp == SADDLE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Надеть это седло на питомца");
    else if (otmp->otyp == MAGIC_WHISTLE
             || otmp->otyp == TIN_WHISTLE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Дунуть в этот свисток");
    else if (otmp->otyp == EUCALYPTUS_LEAF)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Использовать этот лист как свисток");
    else if (otmp->otyp == STETHOSCOPE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Послушать через стетоскоп");
    else if (otmp->otyp == MIRROR)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Показать кому-то его отражение");
    else if (otmp->otyp == BELL || otmp->otyp == BELL_OF_OPENING)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Позвонить в колокол");
    else if (otmp->otyp == CANDELABRUM_OF_INVOCATION) {
        Sprintf(buf, "%s канделябр", light);
        ...
        const char *s = multiple ? "эти" : "этот";
        struct obj *o = carrying(CANDELABRUM_OF_INVOCATION);

        if (o && o->spe < 7)
            Sprintf(buf, "Прикрепить %s к канделябру или %s %s", s,
                    !otmp->lamplit ? "зажечь" : "погасить", /* [lowercase] */
                    multiple ? "их" : "его");
        else
        ...
        Sprintf(buf, "%s этот источник света", light);
        ia_addmenu(win, IA_APPLY_OBJ, 'a', buf);
    } else if (otmp->otyp == POT_OIL && objects[otmp->otyp].oc_name_known) {
        Sprintf(buf, "%s это масло", light);
        ia_addmenu(win, IA_APPLY_OBJ, 'a', buf);
    } else if (otmp->oclass == POTION_CLASS) {
        /* FIXME? this should probably be moved to 'D' rather than be 'a' */
        Sprintf(buf, "Окунуть что-нибудь в %s зелье%s",
                is_plural(otmp) ? "одно из этих" : "это", plur(otmp->quan));
        ia_addmenu(win, IA_DIP_OBJ, 'a', buf);
    } else if (otmp->otyp == EXPENSIVE_CAMERA)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Сделать фотографию");
    else if (otmp->otyp == TOWEL)
        ia_addmenu(win, IA_APPLY_OBJ, 'a',
                   "Вытереться этим полотенцем");
    else if (otmp->otyp == CRYSTAL_BALL)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Вглядеться в этот хрустальный шар");
    else if (otmp->otyp == MAGIC_MARKER)
        ia_addmenu(win, IA_APPLY_OBJ, 'a',
                   "Написать что-то этим маркером");
    else if (otmp->otyp == FIGURINE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Заставить эту статуэтку преобразиться");
    else if (otmp->otyp == UNICORN_HORN)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Использовать этот рог единорога");
    else if (otmp->otyp == HORN_OF_PLENTY
             && objects[otmp->otyp].oc_name_known)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Дунуть в рог изобилия");
    else if (otmp->otyp >= WOODEN_FLUTE && otmp->otyp <= DRUM_OF_EARTHQUAKE)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Сыграть на этом музыкальном инструменте");
    else if (otmp->otyp == LAND_MINE || otmp->otyp == BEARTRAP)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Взвести эту ловушку");
    else if (otmp->otyp == PICK_AXE || otmp->otyp == DWARVISH_MATTOCK)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Копать этим инструментом для копания");
    else if (otmp->oclass == WAND_CLASS)
        ia_addmenu(win, IA_APPLY_OBJ, 'a', "Сломать этот жезл");
    ...
    if (!already_worn) {
        Sprintf(buf, "Бросить %s", (otmp->quan > 1L) ? "стопку" : "предмет");
        ia_addmenu(win, IA_DROP_OBJ, 'd', buf);
    }

    /* e: eat item */
    if (otmp->otyp == TIN) {
        Sprintf(buf, "Открыть %s%s и съесть содержимое",
                (otmp->quan > 1L) ? "одну из этих консервных банок" : "эту банку",
                (otmp->otyp == TIN && uwep && uwep->otyp == TIN_OPENER)
                ? " своей открывалкой" : "");
        ia_addmenu(win, IA_EAT_OBJ, 'e', buf);
    } else if (is_edible(otmp)) {
        Sprintf(buf, "Съесть %s", (otmp->quan > 1L) ? "одно из этих" : "это");
        ia_addmenu(win, IA_EAT_OBJ, 'e', buf);
    }
    ...
    if (otmp->otyp == TOWEL) {
        ia_addmenu(win, IA_ENGRAVE_OBJ, 'E',
                   "Вытереть пол этим полотенцем");
    } else if (otmp->otyp == MAGIC_MARKER) {
        ia_addmenu(win, IA_ENGRAVE_OBJ, 'E',
                   "Нацарапать граффити на полу");
    } else if (otmp->oclass == WEAPON_CLASS || otmp->oclass == WAND_CLASS
             || otmp->oclass == GEM_CLASS || otmp->oclass == RING_CLASS) {
        Sprintf(buf, "%s на %s с помощью %s",
                (is_blade(otmp) || otmp->oclass == WAND_CLASS
                 || ((otmp->oclass == GEM_CLASS || otmp->oclass == RING_CLASS)
                     && objects[otmp->otyp].oc_tough)) ? "Гравировать" : "Написать",
                surface(u.ux, u.uy),
                (otmp->quan > 1L) ? "одного из этих предметов" : "этого предмета");
    ...
    if (otmp == uquiver) {
        ...
        Sprintf(buf, "%s %s", shoot ? "Стрелять" : "Бросить",
                (otmp->quan > 1L) ? "одно из этих" : "это");
        if (shoot) {
            assert(uwep != NULL);
            Sprintf(eos(buf), ", используя своё %s", simpleonames(uwep));
        }
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ) && !u.uswallow) {
        ...
        if (otmp->otyp == CORPSE)
            ia_addmenu(win, IA_SACRIFICE, 'O',
                       "Принести этот труп в жертву на этом алтаре");
        else if (otmp->otyp == AMULET_OF_YENDOR
                 || otmp->otyp == FAKE_AMULET_OF_YENDOR)
            ia_addmenu(win, IA_SACRIFICE, 'O',
                       "Принести этот амулет в жертву на этом алтаре");
    }
    ...
    if (otmp->unpaid
        ...
        && inhishop(mtmp)) {
        Sprintf(buf, "Купить неоплаченный %s",
                (otmp->quan > 1L) ? "стопку" : "предмет");
    ...
    if (!already_worn) {
        ...
        *buf = '\0';
        if (otmp->oclass == AMULET_CLASS) {
            Strcpy(buf, !uamul ? "Надеть этот амулет"
                               : "[на тебе уже надет амулет]");
        } else if (otmp->oclass == RING_CLASS || otmp->otyp == MEAT_RING) {
            if (!uleft || !uright)
                Strcpy(buf, "Надеть это кольцо");
            else
                Sprintf(buf, "[оба %s уже заняты]",
                        makeplural(body_part(FINGER)));
        } else if (otmp->otyp == BLINDFOLD || otmp->otyp == TOWEL
                   || otmp->otyp == LENSES) {
            if (ublindf)
                Strcpy(buf, "[на тебе уже очки]");
            else if (otmp->otyp == LENSES)
                Strcpy(buf, "Надеть эти линзы");
            else
                Sprintf(buf, "Надеть это%s",
                        (otmp->otyp == TOWEL) ? ", чтобы завязать себе глаза" : "");
        }
        if (*buf)
    ...
    if (otmp->oclass == POTION_CLASS) {
        Sprintf(buf, "Выпить %s",
                (otmp->quan > 1L) ? "одно из этих зелий" : "это зелье");
    ...
    if ((otmp->oclass == GEM_CLASS || otmp->oclass == WEAPON_CLASS)
        && otmp != uquiver) {
        Sprintf(buf, "Приготовить %s к лёгкой %s клавишей \'f\'",
                (otmp->quan > 1L) ? "стопку" : "предмет",
                ammo_and_launcher(otmp, uwep) ? "стрельбе" : "бросанию");
    ...
    if (otmp->owornmask & W_ACCESSORY) {
        Sprintf(buf, "Снять %s",
                (otmp->owornmask & W_AMUL) ? "амулет"
                : (otmp->owornmask & W_RING) ? "кольцо"
                  : (otmp->owornmask & W_TOOL) ? "очки"
                    : "аксессуар"); /* catchall -- can't happen */
        ia_addmenu(win, IA_TAKEOFF_OBJ, 'R', buf);
    }
    if (otmp->otyp == OIL_LAMP || otmp->otyp == MAGIC_LAMP
        || otmp->otyp == BRASS_LANTERN) {
        Sprintf(buf, "Потереть %s", simpleonames(otmp));
        ia_addmenu(win, IA_RUB_OBJ, 'R', buf);
    } else if (otmp->oclass == GEM_CLASS && is_graystone(otmp))
        ia_addmenu(win, IA_RUB_OBJ, 'R', "Потереть что-то об этот камень");
    ...
    if (!already_worn) {
         ...
         */
        Sprintf(buf, "%s %s%s", shoot ? "Стрелять" : "Бросить",
                (otmp->quan == 1L) ? "этот предмет"
                : (otmp->otyp == GOLD_PIECE) ? "их"
                  : "одно из этих",
    ...
    if (otmp->owornmask & W_ARMOR)
        ia_addmenu(win, IA_TAKEOFF_OBJ, 'T', "Снять эти доспехи");
    if ((Is_container(otmp) && (Has_contents(otmp) || !otmp->cknown))
        ...
        ia_addmenu(win, IA_TIP_CONTAINER, 'T',
                   "Вытрясти всё содержимое из этого контейнера");
    ...
    if ((otmp->otyp == FAKE_AMULET_OF_YENDOR && !otmp->known)
        ...
        ia_addmenu(win, IA_INVOKE_OBJ, 'V',
                   "Попытаться вызвать уникальную силу этого объекта");
    ...
    if (otmp == uwep || cantwield(gy.youmonst.data)) {
        ...
        Sprintf(buf, "Взять %s как своё оружие",
                (otmp->quan > 1L) ? "стопку" : "предмет");
        ia_addmenu(win, IA_WIELD_OBJ, 'w', buf);
    } else if (otmp->otyp == TIN_OPENER) {
        ia_addmenu(win, IA_WIELD_OBJ, 'w',
                   "Взять открывалку для консервов, чтобы легко открывать консервы");
        ...
        Sprintf(buf, "Взять %s в свои руки",
                (otmp->quan > 1L) ? "стопку" : "предмет");
        ia_addmenu(win, IA_WIELD_OBJ, 'w', buf);
    }
    ...
    if (!already_worn) {
        if (otmp->oclass == ARMOR_CLASS) {
            ...
            if (!o)
                Strcpy(buf, "Надеть эти доспехи");
            else
                Sprintf(buf, "[на тебе уже %s]", an(armor_simple_name(o)));
    ...
    if (otmp == uwep && uswapwep)
        ia_addmenu(win, IA_SWAPWEAPON, 'x',
                   "Обменять с твоим запасным оружием");
    else if (otmp == uwep)
        ia_addmenu(win, IA_SWAPWEAPON, 'x',
                   "Подготовить как запасное оружие");
    else if (otmp == uswapwep)
        ia_addmenu(win, IA_SWAPWEAPON, 'x',
                   "Обменять с твоим основным оружием");
...
#define MAYBETWOWEAPON(obj) \
    ...
    if ((otmp == uwep || otmp == uswapwep)
        ...
        Sprintf(buf, "Переключить бой двумя оружиями: %s", u.twoweap ? "выкл" : "вкл");
...
#undef MAYBETWOWEAPON
    ...
    if (otmp->oclass == WAND_CLASS)
        ia_addmenu(win, IA_ZAP_OBJ, 'z',
                   "Разрядить этот жезл, чтобы высвободить его магию");

    /* ?: Look up an item in the game's database */
    if (ia_checkfile(otmp)) {
        Sprintf(buf, "Найти информацию о %s",
                (otmp->quan > 1L) ? "этих" : "этом");
        ia_addmenu(win, IA_WHATIS_OBJ, '/', buf);
    }

    Sprintf(buf, "Что сделать с %s?", the(cxname(otmp)));
    end_menu(win, buf);
