...
make_confused(long xtime, boolean talk)
{
    ...
    if (!xtime && old) {
        if (talk)
            You_feel("меньше %s сейчас.", Hallucination ? "в отрыве" : "в замешательстве");
    }
...
make_stunned(long xtime, boolean talk)
{
    ...
    if (!xtime && old) {
        if (talk)
            You_feel("%s сейчас.",
                     Hallucination ? "менее шатко" : "чуть устойчивее");
    }
    if (xtime && !old) {
        if (talk) {
            if (u.usteed)
                You("покачиваетесь в седле.");
            else
                You("%s...", stagger(gy.youmonst.data, "пошатываться"));
        }
    }
    if ((!xtime && old) || (xtime && !old))
        disp.botl = TRUE;
          ...
          int type)
{
    ...
    if (xtime > 0L) {
        ...
        if (!old) {
            /* newly sick */
            You_feel("смертельно больны.");
        } else {
            /* already sick */
            if (talk)
                You_feel("%s хуже.", xtime <= Sick / 2L ? "намного " : "ещё ");
        }
        ...
        if (u.usick_type) { /* only partly cured */
            ...
            if (talk)
                You_feel("вылечены.  Какое облегчение!");
...
make_vomiting(long xtime, boolean talk)
{
    ...
    if (!xtime && old)
        if (talk)
            You_feel("теперь не так тошно.");
}

static const char vismsg[] = "на миг словно %s, но сейчас оно %s.";
static const char eyemsg[] = "%s на миг %s.";
...
make_blinded(long xtime, boolean talk)
{
    ...
    if (can_see_now && !u_could_see) { /* regaining sight */
        if (talk) {
            if (Hallucination)
                pline("Клёво!  Всё снова космично!");
            else
                You("снова видите.");
        }
    } else if (old && !xtime) {
        ...
        if (talk) {
            if (!haseyes(gy.youmonst.data) || PermaBlind) {
                ...
                Your(eyemsg, eyes, vtense(eyes, "чешутся"));
            } else { /* Eyes of the Overworld */
                Your(vismsg, "проясниться", Hallucination ? "печальнее" : "нормальным");
            }
        }
    }
    ...
    if (u_could_see && !can_see_now) { /* losing sight */
        if (talk) {
            if (Hallucination)
                pline("О, облом!  Всё темно!  Помогите!");
            else
                pline("Облако тьмы окутывает вас.");
        }
        ...
        if (talk) {
            if (!haseyes(gy.youmonst.data) || PermaBlind) {
                ...
                if (eyecount(gy.youmonst.data) != 1)
                    eyes = makeplural(eyes);
                Your(eyemsg, eyes, vtense(eyes, "дёргаются"));
            } else { /* Eyes of the Overworld */
                Your(vismsg, "тускнеть", Hallucination ? "счастливее" : "нормальным");
            }
        }
    }
    ...
    long mask)    /* nonzero if resistance status should change by mask */
{
    ...
    message = (!xtime) ? "Всё %s сейчас ТАК скучно."
                       : "Ух ты!  Всё %s так космично!";
    verb = (!Blind) ? "выглядит" : "ощущается";
    ...
    if (mask) {
        ...
        if (!changed && !HHallucination && old && talk) {
            if (!haseyes(gy.youmonst.data)) {
                ...
                Your(vismsg, "расплющиться", "нормальным");
            }
        }
    }
...
make_deaf(long xtime, boolean talk)
{
    ...
    if ((xtime != 0L) ^ (old != 0L)) {
        disp.botl = TRUE;
        if (talk)
            You(old && !Deaf ? "снова слышите."
                             : "не можете ничего слышать.");
    }
}
...
{
    pline("%s %s.",
          Hallucination ? "Клёво, чувак!  Ты"
                        : "Ого!  Внезапно, ты",
          See_invisible ? "можете видеть себя насквозь"
                        : "не можете себя видеть");
}
...
ghost_from_bottle(void)
{
    ...
    if (!mtmp) {
        pline("Эта бутылка оказывается пустой.");
        return;
    }
    if (Blind) {
        pline("Когда вы открываете бутылку, появляется %s.", something);
        return;
    }
    pline("Когда вы открываете бутылку, появляется огромный %s!",
          Hallucination ? rndmonnam(NULL) : (const char *) "призрак");
    if (flags.verbose)
        You("до смерти напуганы и не можете двигаться.");
    nomul(-3);
    gm.multi_reason = "испугавшись до смерти";
    gn.nomovemsg = "Вы приходите в себя.";
}
...
dodrink(void)
{
    ...
    if (Strangled) {
        pline("Если вы не можете дышать воздухом, как вы можете пить жидкость?");
        return ECMD_OK;
    }
    ...
    if (!iflags.menu_requested) {
        ...
        if (Underwater && !u.uswallow) {
            if (y_n("Пить воду вокруг вас?") == 'y') {
                pline("Вы знаете, что живёт в этой воде?");
                return ECMD_TIME;
            }
            ++drink_ok_extra;
        }
    ...
    otmp = getobj("пить", drink_ok, GETOBJ_NOFLAGS);
    if (!otmp)
        return ECMD_CANCEL;
...
dopotion(struct obj *otmp)
{
    ...
    if (gp.potion_nothing) {
        gp.potion_unkn++;
        You("на мгновение возникает %s чувство, затем оно проходит.",
            Hallucination ? "обычное" : "странное");
    }
...
peffect_restore_ability(struct obj *otmp)
{
    gp.potion_unkn++;
    if (otmp->cursed) {
        pline("Уф!  От этого вы чувствуете себя посредственно!");
        return;
    } else {
        int i, ii;
        ...
        pline("Ух ты!  От этого вы чувствуете себя %s!",
              (!otmp->blessed) ? "хорошо"
              : unfixable_trouble_count(FALSE) ? "лучше"
                : "отлично");
...
peffect_hallucination(struct obj *otmp)
{
    ...
    if ((otmp->blessed && !rn2(3)) || (!otmp->cursed && !rn2(6))) {
        You("воспринимаете себя...");
        display_nhwindow(WIN_MESSAGE, FALSE);
        enlightenment(MAGICENLIGHTENMENT, ENL_GAMEINPROGRESS);
        Your("осознание возвращается в норму.");
        exercise(A_WIS, TRUE);
    }
}
...
peffect_water(struct obj *otmp)
{
    if (!otmp->blessed && !otmp->cursed) {
        pline("На вкус как %s.", hliquid("water"));
        u.uhunger += rnd(10);
        newuhs(FALSE);
        return;
    }
    ...
    if (mon_hates_blessings(&gy.youmonst) /* undead or demon */
        ...
        if (otmp->blessed) {
            pline("Жжётся, как %s!", hliquid("acid"));
            exercise(A_CON, FALSE);
            if (ismnum(u.ulycn)) {
                Your("родство с %s исчезает!",
                     makeplural(mons[u.ulycn].pmnames[NEUTRAL]));
                if (gy.youmonst.data == &mons[u.ulycn])
                    you_unwere(FALSE);
                set_ulycn(NON_PM); /* cure lycanthropy */
            }
            losehp(Maybe_Half_Phys(d(2, 6)), "зелье святой воды",
                   KILLED_BY_AN);
        } else if (otmp->cursed) {
...
{
    gp.potion_unkn++;
    pline("Ух!  На вкус как %s%s!",
          otmp->odiluted ? "разбавленное " : "",
          Hallucination ? "вино из одуванчиков" : "жидкий огонь");
    if (!otmp->blessed) {
    ...
    if (otmp->cursed) {
        You("теряете сознание.");
        gm.multi = -rnd(15);
        gn.nomovemsg = "Вы просыпаетесь с головной болью.";
    }
}
...
peffect_enlightenment(struct obj *otmp)
{
    if (otmp->cursed) {
        gp.potion_unkn++;
        You("испытываете тревожное чувство...");
        exercise(A_WIS, FALSE);
    } else {
        if (otmp->blessed) {
...
peffect_invisibility(struct obj *otmp)
{
    ...
    if (otmp->cursed) {
        pline("По какой-то причине вы чувствуете, что ваше присутствие известно.");
        aggravate();
...
peffect_see_invisible(struct obj *otmp)
{
    ...
    gp.potion_unkn++;
    if (otmp->cursed)
        pline("Фу!  На вкус это %s.",
              Hallucination ? "переспелое" : "гнилое");
    else
        pline(
              Hallucination
              ? "Это на вкус как 10%% натуральный %s%s напиток."
              : "Это на вкус как %s%s.",
              otmp->odiluted ? "восстановленный " : "", fruitname(TRUE));
    ...
    if (msg && !Blind) {  /* Blind possible if polymorphed */
        You("видите себя насквозь, но при этом вы заметны!");
        gp.potion_unkn--;
    }
}
...
{
    if (Free_action) {
        You("на миг коченеете.");
    } else {
        if (Levitation || Is_airlevel(&u.uz) || Is_waterlevel(&u.uz))
            You("неподвижно застываете.");
        else if (u.usteed)
            You("застываете на месте!");
        else
            Your("%s примёрзли к %s!", makeplural(body_part(FOOT)),
                 surface(u.ux, u.uy));
        nomul(-(rn1(10, 25 - 12 * bcsign(otmp))));
        gm.multi_reason = "заморожен зельем";
...
{
    pline("Фу!  На вкус как яд.");
    if (otmp->blessed) {
        pline("(Но на самом деле это было слегка залежавшееся %s.)", fruitname(TRUE));
        if (!Role_if(PM_HEALER)) {
            /* NB: blessed otmp->fromsink is not possible */
            losehp(1, "слегка загрязнённое зелье", KILLED_BY_AN);
        }
    } else {
        if (Poison_resistance)
            pline("(Но на самом деле это было биологически загрязнённое %s.)",
                  fruitname(TRUE));
        if (Role_if(PM_HEALER)) {
            pline("К счастью, у вас есть иммунитет.");
        } else {
            char contaminant[BUFSZ];
            int typ = rn2(A_MAX);

            Sprintf(contaminant, "%s%s",
                    (Poison_resistance) ? "слегка " : "",
                    (otmp->fromsink) ? "загрязнённая водопроводная вода"
                    : "загрязнённое зелье");
            if (!Fixed_abil) {
    ...
    if (Hallucination) {
        You("приходите в себя от потрясения!");
...
peffect_confusion(struct obj *otmp)
{
    if (!Confusion) {
        if (Hallucination) {
            pline("Что за психоделическое ощущение!");
            gp.potion_unkn++;
        } else
            pline("А?  Что?  Где я?");
    } else
...
{
    if (otmp->cursed) {
        pline("Уф!  Это зелье было отвратительным на вкус!");
        gp.potion_unkn++;
    } else if (Fixed_abil) {
        gp.potion_nothing++;
...
peffect_speed(struct obj *otmp)
{
    ...
    if (is_speed && !otmp->cursed && !(HFast & INTRINSIC)) {
        Your("проворность ощущается очень естественно.");
        HFast |= FROMOUTSIDE;
    }
}
        ...
        if (on_lvl_1 ? u.uhave.amulet : Can_rise_up(u.ux, u.uy, &u.uz)) {
            ...
            if (on_lvl_1) {
                ...
                if (on_level(&newlevel, &u.uz)) {
                    pline("На вкус оно было плохим.");
                    return;
                }
            }
            You("поднимаетесь вверх, сквозь %s!", ceiling(u.ux, u.uy));
...
peffect_levitation(struct obj *otmp)
{
    ...
    if (otmp->cursed) {
        ...
        if (BLevitation) {
            ...
            You("ударяетесь %s о %s.", body_part(HEAD),
                ceiling(u.ux, u.uy));
...
peffect_gain_energy(struct obj *otmp)
{
    ...
    if (otmp->cursed)
        ...
        pline("Магические энергии проходят по вашему телу.");
...
peffect_oil(struct obj *otmp)
{
    ...
    if (otmp->lamplit) {
        ...
        pline("На вкус как касторка.");
    } else {
        pline("Это было гладко!");
    }
...
peffect_acid(struct obj *otmp)
{
    if (Acid_resistance) {
        ...
        pline("Это на вкус %s.", Hallucination ? "терпко" : "кисло");
    } else {
        int dmg;
...
{
    You_feel("немного %s.", Hallucination ? "нормально" : "странно");
    if (!Unchanging) {
...
strange_feeling(struct obj *obj, const char *txt)
{
    if (flags.beginner || !txt)
        You("на мгновение возникает %s чувство, затем оно проходит.",
            Hallucination ? "нормальное" : "странное");
    else
        pline1(txt);
...
static const char *bottlenames[] = { "бутылка", "фиал", "фляга", "графин",
                              "колба",  "банка",   "флакон" };
static const char *hbottlenames[] = {
    "кувшин", "кувшин", "бочка", "жестянка", "мешок", "коробка", "стакан", "мензурка",
    "бокал", "ваза", "цветочный горшок", "кастрюля", "штуковина", "кружка", "чайная чашка",
    "чайник", "бочонок", "ведро", "термос", "амфора", "бурдюк", "свёрток",
    "миска", "ампула"
};
...
#define COST_none (-1)
    ...
    if (func) {
        ...
        if (useeit) {
            ...
            if (altfmt)
                pline("%s с ореолом %s.", objphrase, an(glowcolor));
            else
    ...
    boolean seeit)   /* True: give "<obj> glows <color>" message */
{
    ...
    if (worsen)
        ...
        bless(otmp);
    H2Opotion_dip(otmp, obj, seeit, seeit ? Yobjnam2(obj, "светится") : "");
    dealloc_obj(otmp);
...
potionhit(struct monst *mon, struct obj *obj, int how)
{
    ...
    if (isyou) {
        tx = u.ux, ty = u.uy;
        distance = 0;
        pline("%s Разбивается о ваше %s и рассыпается на осколки.", botlnam,
                  body_part(HEAD));
        ...
        if (!cansee(tx, ty)) {
            ...
            if (hit_saddle && saddle) {
                ...
                Sprintf(buf, "%s %s", s_suffix(mnam),
                        (gn.notonhead ? "тело" : "голову"));
            } else {
                Strcpy(buf, mnam);
            }
            Soundeffect(se_potion_crash_and_break, 60);
            pline("%s Разбивается о %s и рассыпается на осколки.", botlnam,
                      buf);
        }
    ...
    if (obj->otyp != POT_OIL && !hit_saddle && cansee(tx, ty))
        pline("%s.", Tobjnam(obj, "испаряется"));
    ...
    if (isyou) {
        ...
        if (useeit && !affected)
            pline("%s %s.", buf, aobjnam(saddle, "намокает"));
    } else {
 ...
 do_healing:
            ...
            if (mon->mhp < mon->mhpmax) {
                ...
                if (canseemon(mon))
                    pline("%s снова выглядит здоровым и крепким.", Monnam(mon));
            }
            if (cureblind)
        ...
        case POT_SICKNESS:
            ...
            if (dmgtype(mon->data, AD_DISE)
                ...
                || resists_poison(mon)) {
                if (canseemon(mon))
                    pline("%s выглядит невредимым.", Monnam(mon));
                break;
            }
 do_illness:
            if (mon->mhp > 2) {
                mon->mhp /= 2;
                if (canseemon(mon))
                    pline("%s выглядит довольно больным.", Monnam(mon));
            }
            break;
        case POT_CONFUSION:
        case POT_BOOZE:
        ...
        case POT_INVISIBILITY: {
            ...
            if (sawit && !canspotmon(mon)) {
                ...
                pline("%s на миг кажется прозрачным.", Monnam(mon));
        ...
        case POT_WATER:
            if (mon_hates_blessings(mon) /* undead or demon */
                ...
                if (obj->blessed) {
                    pline("%s %s от боли!", Monnam(mon),
                          is_silent(mon->data) ? "корчится" : "вопит");
                    ...
                    angermon = FALSE;
                    if (canseemon(mon))
                        pline("%s выглядит здоровее.", Monnam(mon));
                    healmon(mon, d(2, 6), 0);
...
potionbreathe(struct obj *obj)
{
    ...
    case TOWEL:
        pline("Немного пара безвредно проходит мимо вас.");
        break;
    case POT_RESTORE_ABILITY:
    case POT_GAIN_ABILITY:
        if (obj->cursed) {
            if (!breathless(gy.youmonst.data)) {
                pline("Уф!  Это зелье ужасно пахнет!");
            } else if (haseyes(gy.youmonst.data)) {
                const char *eyes = body_part(EYE);

                if (eyecount(gy.youmonst.data) != 1)
                    eyes = makeplural(eyes);
                Your("%s %s!", eyes, vtense(eyes, "щиплют"));
            }
            break;
        } else {
            i = rn2(A_MAX); /* start at a random point */
    ...
    case POT_SICKNESS:
        ...
        break;
    case POT_HALLUCINATION:
        You("видите мимолётное видение.");
        break;
    case POT_CONFUSION:
    case POT_BOOZE:
        if (!Confusion)
            You_feel("небольшое головокружение.");
        make_confused(itimeout_incr(HConfusion, rnd(5)), FALSE);
        break;
    case POT_INVISIBILITY:
        if (!Blind && !Invis) {
            kn++;
            pline("На мгновение вы %s!",
                  See_invisible ? "мог видеть себя насквозь"
                                : "не мог видеть себя");
        }
        break;
    case POT_PARALYSIS:
        kn++;
        if (!Free_action) {
            pline("%s, кажется, держит вас.", Something);
            nomul(-rnd(5));
            gm.multi_reason = "заморожен зельем";
            gn.nomovemsg = You_can_move_again;
            exercise(A_DEX, FALSE);
    ...
    case POT_SLEEPING:
        ...
        if (!Free_action && !Sleep_resistance) {
            You_feel("довольно усталым.");
            nomul(-rnd(5));
            gm.multi_reason = "отсыпаясь после волшебного зелья";
            gn.nomovemsg = You_can_move_again;
            exercise(A_DEX, FALSE);
        } else {
            You("зеваете.");
    ...
    case POT_SPEED:
        if (!Fast)
            Your("колени кажутся более гибкими.");
...
dodip(void)
{
    static const char Dip_[] = "Окунуть ";
    ...
    obj = getobj("окунуть", at_here ? dip_hands_ok : dip_ok, GETOBJ_PROMPT);
    if (!obj)
        return ECMD_CANCEL;
    if (inaccessible_equipment(obj, "окунуть", FALSE))
        return ECMD_OK;

    is_hands = (obj == &hands_obj);
    shortestname = (is_hands || is_plural(obj) || pair_of(obj)) ? "их"
                                                                : "его";
    drink_ok_extra = 0;
    /*
     ...
     */
    if (is_hands) {
        Snprintf(obuf, sizeof obuf, "ваши %s", makeplural(body_part(HAND)));
    } else {
 ...
 into? [abdeghjkmnpqstvwyzBCEFHIKLNOQRTUWXZ#-# or ?*] "));
    ...
    if (!iflags.menu_requested) {
        ...
        if (!can_reach_floor(FALSE)) {
            ...
            Snprintf(qbuf, sizeof(qbuf), "%s%s в фонтан?", Dip_,
            ...
            Snprintf(qbuf, sizeof(qbuf), "%s%s в раковину?", Dip_,
                     flags.verbose ? obuf : shortestname);
            if (y_n(qbuf) == 'y') {
                if (!is_hands)
                    obj->pickup_prev = 0;
            ...
            const char *pooltype = waterbody_acc(u.ux, u.uy);

            Snprintf(qbuf, sizeof(qbuf), "%s%s в %s?", Dip_,
    ...
    Snprintf(qbuf, sizeof qbuf, "окунуть %s в",
             flags.verbose ? obuf : shortestname);
...
dip_into(void)
{
    ...
    potion = getobj("окунуть", drink_ok, GETOBJ_NOFLAGS);
    if (!potion || potion->oclass != POTION_CLASS)
        return ECMD_CANCEL;

    /* "What do you want to dip into <the potion>? [abc or ?*] " */
    Snprintf(qbuf, sizeof qbuf, "окунуть в %s%s",
             is_plural(potion) ? "одна из " : "", thesimpleoname(potion));
...
dip_potion_explosion(struct obj *obj, int dmg)
{
    if (obj->cursed || obj->otyp == POT_ACID
        ...
        obj->in_use = 1;
        pline("%sОни взрываются!", !Deaf ? "БУМ!  " : "");
...
potion_dip(struct obj *obj, struct obj *potion)
{
    ...
    if (potion == obj && potion->quan == 1L) {
        pline("Это бутылка из-под зелья, а не бутылка Клейна!");
        return ECMD_OK;
    }
    if (obj == &hands_obj) {
        You("не можете втиснуть свои %s в горлышко бутылки!",
            body_part(HAND));
        return ECMD_OK;
    }
    ...
    if (potion->otyp == POT_WATER) {
        ...
        const char *obj_glows = Yobjnam2(obj, "светится");
        ...
        if (obj_unpolyable(obj->otyp == POT_POLYMORPH ? potion : obj)) {
            ...
            if (!u.uconduct.polypiles++)
                livelog_printf(LL_CONDUCT, "превратил %s первым предметом",
                               uhis());
        ...
        Strcpy(qbuf, "Те"); /* assume full stack */
        if (amt > (obj->odiluted ? 2 : magic ? 3 : 7)) {
            ...
            if ((long) amt < obj->quan) {
                ...
                Sprintf(qbuf, "%ld из ", obj->quan);
            }
        }
        /* [N of] the {obj(s)} mix(es) with [one of] {the potion}... */
        pline("%s %s %s с %s%s...", qbuf, simpleonames(obj),
              otense(obj, "смешивается"), (potion->quan > 1L) ? "одна из " : "",
        ...
        if (mixture != STRANGE_OBJECT) {
            ...
            default:
                useupall(obj);
                pline("Смесь %sиспаряется.",
                          !Blind ? "Ярко светится и " : "");
                return ECMD_TIME;
            }
        }
        ...
        if (obj->otyp == POT_WATER && !Hallucination) {
            pline("Смесь пузырится%s.", Blind ? "" : ", Затем проясняется");
        } else if (!Blind) {
            pline("Смесь выглядит %s.",
        ...
        freeinv(obj);
        hold_potion(obj, "Вы роняете %s!", doname(obj), (const char *) 0);
        return ECMD_TIME;
    }

    if (potion->otyp == POT_ACID && obj->otyp == CORPSE
        && obj->corpsenm == PM_LICHEN) {
        pline("%s %s %s по краям.", The(cxname(obj)),
              otense(obj, "скручивается"), Blind ? "сморщенный"
    ...
    if (is_poisonable(obj)) {
        if (potion->otyp == POT_SICKNESS && !obj->opoisoned) {
            ...
            if (potion->quan > 1L)
                Sprintf(buf, "Одна из %s", the(xname(potion)));
            else
                Strcpy(buf, The(xname(potion)));
            pline("%s образует покрытие на %s.", buf, the(xname(obj)));
            obj->opoisoned = TRUE;
            poof(potion);
            return ECMD_TIME;
            ...
            pline("Покрытие исчезает с %s.", the(xname(obj)));
            obj->opoisoned = 0;
            poof(potion);
            return ECMD_TIME;
        }
    ...
    if (potion->otyp == POT_OIL) {
        ...
        if (potion->lamplit) { /* burning */
            ...
            pline("Проливается и покрывает ваше %s маслом",
            ...
            if (!Blind)
                pline("%s %s с маслянистым блеском.", Yname2(obj),
                      otense(obj, "блестит"));
            else /*if (!uarmg)*/
                pline("%s %s маслянисто.", Yname2(obj), otense(obj, "ощущается"));
        } else {
            pline("%s %s менее %s.", Yname2(obj),
                  otense(obj, !Blind ? "выглядит" : "ощущается"),
                  (obj->oeroded && obj->oeroded2)
                      ? "корродированный и ржавый"
                      : obj->oeroded ? "ржавый" : "корродированный");
            if (obj->oeroded > 0)
                obj->oeroded--;
            if (obj->oeroded2 > 0)
                obj->oeroded2--;
 ...
 more_dips:
    ...
    if ((obj->otyp == OIL_LAMP || obj->otyp == MAGIC_LAMP)
        ...
        if (obj->age > 1000L) {
            pline("%s %s.", Yname2(obj), otense(obj, "полна"));
            potion->in_use = FALSE; /* didn't go poof */
        } else {
            You("наполняете %s маслом.", yname(obj));
    ...
    if ((obj->otyp == UNICORN_HORN || obj->otyp == AMETHYST)
        ...
        if (!Blind) {
            ...
            *newbuf = '\0';
            if (mixture == POT_WATER && singlepotion->dknown)
                Sprintf(newbuf, "прояснилось");
            else if (!Blind)
                Sprintf(newbuf, "становится %s",
                        hcolor(OBJ_DESCR(objects[mixture])));
            if (*newbuf)
                pline("%sЗелье%s %s.", oldbuf,
                          more_than_one ? ", В которое вы окунули" : "",
                          newbuf);
            else
                pline("Что-то происходит.");
        ...
        hold_potion(singlepotion, "Вы жонглируете и роняете %s!",
...
djinni_from_bottle(struct obj *obj)
{
    ...
    if (!(mtmp = makemon(&mons[PM_DJINNI], u.ux, u.uy, MM_NOMSG))) {
        pline("Оказывается, оно пустое.");
        return;
    }

    if (!Blind) {
        pline("В облаке дыма появляется %s!", a_monnam(mtmp));
        pline("%s говорит.", Monnam(mtmp));
    } else {
        You("чувствуете едкий запах.");
        pline("%s говорит.", Something);
    }
    ...
    switch (chance) {
    case 0:
        verbalize("Я у вас в долгу.  Я исполню одно желание!");
        /* give a wish and discard the monster (mtmp set to null) */
        mongrantswish(&mtmp);
        break;
    case 1:
        verbalize("Спасибо, что освободили меня!");
        (void) tamedog(mtmp, (struct obj *) 0, FALSE);
        break;
    case 2:
        verbalize("Вы освободили меня!");
        mtmp->mpeaceful = TRUE;
        set_malign(mtmp);
        break;
    case 3:
        verbalize("Наконец-то!");
        if (canspotmon(mtmp))
            pline("%s исчезает.", Monnam(mtmp));
        mongone(mtmp);
        break;
    default:
        verbalize("Вы потревожили меня, глупец!");
    ...
    struct monst *mtmp) /* optional attacker whose heat triggered it */
{
    ...
    if (mon == &gy.youmonst) {
        ...
        if (mtmp2) {
            ...
            if (canspotmon(mon))
                pline("%s множится%s!", Monnam(mon), reason);
        }
    }
    return mtmp2;
}
...
{
   if (!Very_fast)
       You("внезапно движетесь %sбыстрее.", Fast ? "" : "намного ");
   else
       Your("%s получают новую энергию.", makeplural(body_part(LEG)));
