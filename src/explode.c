    ...
    int expltype) /* explosion type: controls color of explosion glyphs */
{
                    ...
                    if (!svc.context.mon_moving) {
                        ...
                        if (cansee(mtmp->mx, mtmp->my) || canspotmon(mtmp))
                            pline("%s %s!", Monnam(mtmp),
                                  xkflg ? "полностью сгорел"
                                        : nonliving(mtmp->data) ? "уничтожен"
                                                                        : "убит");
    ...
    if (shopdamage) {
        pay_for_damage((adtyp == AD_FIRE) ? "сгорает"
                          : (adtyp == AD_COLD) ? "разбивается"
                             : (adtyp == AD_DISN) ? "дезинтегрирует"
                                : "разрушает",
                       FALSE);
    }
    ...
    struct obj *obj)         /* only scatter this obj */
{
    ...
    while ((otmp = (individual_object ? obj
        ...
        if (otmp == uball || otmp == uchain) {
            ...
            pline("Цепь разлетается вдребезги!");
            unpunish();
            if (waschain)
                continue;
        }
