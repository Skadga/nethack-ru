...
const char unavailcmd[] = "Команда '%s' недоступна.";

/* for rejecting #if !SHELL, !SUSPEND */
static const char cmdnotavail[] = "Команда '%s' недоступна.";
...
can_do_extcmd(const struct ext_func_tab *extcmd)
{
    ...
    if (!wizard && (ecflags & WIZMODECMD)) {
        ...
        You_cant("сделать это, пока вы закопаны!");
        return FALSE;
...
doextcmd(void)
{
    ...
    do {
        ...
        if (iflags.menu_requested && !accept_menu_prefix(&extcmdlist[idx])) {
            pline("'%s'-префикс не действует на команду %s.",
    ...
    const struct ext_func_tab *efp) /* if Null, add a footnote to the menu */
{
    ...
    if (!efp) {
        ...
        add_menu_str(menuwin, "[A] команда автозавершается");
        Sprintf(qbuf, "[m] команда принимает префикс '%s'",
                visctrl(cmd_from_func(do_reqmenu)));
...
doextlist(void)
{
    ...
    static const char *const headings[] = { "Расширенные команды",
                                      "Отладочные расширенные команды" };
    int clr = NO_COLOR;
    ...
    while (redisplay) {
        ...
        add_menu_str(menuwin, "Список расширенных команд");
        add_menu_str(menuwin, "");

        Sprintf(buf, "Переключить на команды, которые %s не автодополняются",
                menumode ? "включая" : "исключая");
        any.a_int = 1;
        ...
        if (!*searchbuf) {
            ...
            add_menu(menuwin, &nul_glyphinfo, &any, ':', 's', ATR_NONE,
                     clr, "Поиск расширенных команд",
                     MENU_ITEMFLAGS_NONE);
        } else {
            Strcpy(buf, "Вернуться из поиска");
        ...
        if (wizard) {
          ...
          onelist ? "Показать отладочные команды отдельным разделом"
       : "Показать всё по алфавиту, включая отладочные команды",
                     MENU_ITEMFLAGS_NONE);
        }
        add_menu_str(menuwin, "");
        ...
        if (*searchbuf && !n)
            add_menu_str(menuwin, "нет совпадений");
        else
        ...
        if (search) {
            Strcpy(promptbuf, "Поисковая фраза для списка расширенных команд");
            Strcat(promptbuf, "?");
      ...
      "Exceeded %d extended commands in doextcmd() menu; 'extmenu' disabled.",
        ...
        for (i = 0; choices[i]; ++i) {
            ...
            if (!acount || one_per_line) {
                Sprintf(prompt, "%s%s [%s]", wastoolong ? "или " : "",
                        choices[i]->ef_txt, choices[i]->ef_desc);
            } else if (acount == 1) {
                Sprintf(prompt, "%s%s or %s", wastoolong ? "или " : "",
        ...
        Snprintf(prompt, sizeof(prompt), "Расширенная команда: %s", cbuf);
        end_menu(win, prompt);
...
domonability(void)
{
    ...
    if (might_hide && webmaker(uptr)) {
        c = yn_function("Спрятаться [h] или сплести паутину [s]?",
    ...
    if (can_breathe(uptr))
        ...
        You("вопит.");
        if (u.uburied)
            pline("К сожалению, звук плохо проходит сквозь камень.");
        else
            aggravate();
        ...
        pline("Любая ваша особая способность чисто рефлекторная.");
    } else {
        You("не имеете особой способности в обычной форме!");
    }
    return ECMD_OK;
}
...
{
    if (discover) {
        You("уже находитесь в режиме исследования.");
    } else {
        const char *oldmode = !wizard ? "обычная игра" : "режим отладки";

        if (!authorize_explore_mode()) {
            if (!wizard) {
                You("не можете войти в режим исследования.");
                return ECMD_OK;
            } else {
                pline(
                 "Примечание: обычно вам не разрешают входить в режим исследования.");
                /* keep going */
            }
        }
        pline("Берегитесь!  Из режима исследования не будет возврата к %s,",
              oldmode);
        if (paranoid_query(ParanoidQuit,
                           "Хотите войти в режим исследования?")) {
            discover = TRUE;
            wizard = FALSE;
            clear_nhwindow(WIN_MESSAGE);
            You("теперь находитесь в режиме исследования без подсчёта очков.");
        } else {
            clear_nhwindow(WIN_MESSAGE);
            pline("Продолжение в режиме %s.", oldmode);
        }
    }
    return ECMD_OK;
}
...
makemap_prepost(boolean pre, boolean wiztower)
{
    ...
    if (pre) {
        ...
        {
            static const char Unachieve[] = "достижение %s аннулировано.";
...
const char *levltyp[MAX_TYPE + 2] = {
    "камень", "вертикальная стена", "горизонтальная стена", "стена-угол сверху-слева",
    "стена-угол сверху-справа", "стена-угол снизу-слева",
    "стена-угол снизу-справа", "стена-крест", "стена-тройник вверх", "стена-тройник вниз",
    "стена-тройник влево", "стена-тройник вправо", "стена-подъёмного моста", "дерево",
    "потайная дверь", "потайной коридор", "пруд", "ров", "вода",
    "подъёмный мост поднят", "озеро лавы", "стена лавы", "железные прутья", "дверь",
    "коридор", "комната", "лестница", "стремянка", "фонтан", "трон", "раковина",
    "могила", "алтарь", "лёд", "подъёмный мост опущен", "воздух", "облако",
    /* not a real terrain type, but used for undiggable stone
       by wiz_map_levltyp() */
    "недоступно/некопаемо",
...
doterrain(void)
{
    ...
    add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE, clr,
             "известная карта без монстров, предметов и ловушек",
             MENU_ITEMFLAGS_SELECTED);
    any.a_int = 2;
    add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
             clr, "известная карта без монстров и предметов",
             MENU_ITEMFLAGS_NONE);
    any.a_int = 3;
    add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
             clr, "известная карта без монстров",
             MENU_ITEMFLAGS_NONE);
    if (discover || wizard) {
        any.a_int = 4;
        add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
                 clr, "полная карта без монстров, предметов и ловушек",
                 MENU_ITEMFLAGS_NONE);
        if (wizard) {
            any.a_int = 5;
            add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
                     clr, "внутренние коды levl[][].typ в 36-ричной системе",
                     MENU_ITEMFLAGS_NONE);
            any.a_int = 6;
            add_menu(men, &nul_glyphinfo, &any, 0, 0, ATR_NONE,
                     clr, "легенда кодов levl[][].typ в 36-ричной системе",
                     MENU_ITEMFLAGS_NONE);
        }
    }
    end_menu(men, "Что показать?");
...
lookaround_known_room(coordxy x, coordxy y)
{
    ...
    if (u_have_seen_whole_selection(sel)) {
        ...
        You("%s %s %s.",
            u_at(x, y) && u_in && u_can_see_whole_selection(sel) ? "находитесь в"
            : (u_at(x, y)) ? "помните это как" : "помните то как",
            an(selection_size_description(sel, qbuf)),
            rmno >= 0 ? "комната" : "область");
    } else if (u_have_seen_bounds_selection(sel)) {
        You("предполагаете, что %s является %s %s.",
            u_at(x, y) ? "это" : "то",
            an(selection_size_description(sel, qbuf)),
            rmno >= 0 ? "комната" : "область");
    } else {
        You("не можете предположить размер обла%ss.",
            u_at(x, y) ? "это" : "то");
    }
...
dotoggleoption(void)
{
    if (gc.cmd_bind && gc.cmd_bind->param) {
        ...
        pline("Используйте #optionsfull, чтобы изменить любую опцию.");
        return ECMD_OK;
    }
}
...
{
    if (iflags.menu_requested) {
        Norep("Двойной префикс %s, отменено.",
              visctrl(cmd_from_func(do_reqmenu)));
        iflags.menu_requested = FALSE;
        return ECMD_CANCEL;
    }
...
do_rush(void)
{
    if ((gd.domove_attempting & DOMOVE_RUSH)) {
        Norep("Двойной префикс rush, отменено.");
        svc.context.run = 0;
...
{
    if ((gd.domove_attempting & DOMOVE_RUSH)) {
        Norep("Двойной префикс run, отменено.");
        svc.context.run = 0;
        gd.domove_attempting = 0;
        return ECMD_CANCEL;
    }
...
do_fight(void)
{
    if (svc.context.forcefight) {
        Norep("Двойной префикс fight, отменено.");
...
do_repeat(void)
{
    ...
    if (!gi.in_doagain) {
        ...
        if (!cmdq_peek(CQ_REPEAT)) {
            Norep("Нет доступной команды для повтора.");
            return ECMD_FAIL;
        }
...
struct ext_func_tab extcmdlist[] = {
    { '#',    "#", "выполнить расширенную команду",
              doextcmd, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { M('?'), "?", "показать все расширенные команды",
              doextlist, IFBURIED | AUTOCOMPLETE | GENERALCMD | CMD_M_PREFIX,
              NULL },
    { M('a'), "adjust", "настроить буквы инвентаря",
              doorganize, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { M('A'), "annotate", "дать имя текущему уровню",
              donamelevel, IFBURIED | AUTOCOMPLETE | GENERALCMD | CMD_M_PREFIX, NULL },
    { 'a',    "apply", "применить инструмент (кирка, ключ, лампа...)",
              doapply, CMD_M_PREFIX, NULL },
    { C('x'), "attributes", "показать ваши характеристики",
              doattributes, IFBURIED | GENERALCMD, NULL },
    { '@',    "autopickup", "переключить опцию 'autopickup' вкл/выкл",
              dotogglepickup, IFBURIED | GENERALCMD, NULL },
#ifdef CRASHREPORT
    { '\0',   "bugreport", "отправить отчёт об ошибке",
              dobugreport, GENERALCMD | NOFUZZERCMD, NULL },
#endif
    { 'C',    "call", "дать имя монстра, конкретному предмету или типу предмета",
              docallcmd, IFBURIED | GENERALCMD, NULL },
    { 'Z',    "cast", "пустить (сотворить) заклинание",
              docast, IFBURIED, NULL },
    { M('c'), "chat", "поговорить с кем-либо",
              dotalk, IFBURIED | AUTOCOMPLETE, NULL },
    { 'v',    "chronicle", "показать журнал основных событий",
              do_gamelog, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { 'c',    "close", "закрыть дверь",
              doclose, 0, NULL },
    { M('C'), "conduct", "показать добровольные испытания, которые вы соблюдаете",
              doconduct, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { '\0',   "debugfuzzer", "запустить проверку случайными вводами",
              wiz_fuzzer, IFBURIED | WIZMODECMD | NOFUZZERCMD, NULL },
    { M('d'), "dip", "окунуть предмет во что-либо",
    ...
    { 'd',    "drop", "выбросить предмет",
              dodrop, 0, NULL },
    { 'D',    "droptype", "выбросить предметы определённых типов",
              doddrop, 0, NULL },
    { 'e',    "eat", "съесть что-нибудь",
              doeat, CMD_M_PREFIX, NULL },
    { 'E',    "engrave", "вырезать надпись на полу",
              doengrave, 0, NULL },
    { M('e'), "enhance", "развить или проверить навыки оружия и заклинаний",
    ...
    { M('X'), "exploremode", "войти в режим исследования (открытий)",
              enter_explore_mode, IFBURIED | GENERALCMD | NOFUZZERCMD, NULL },
    { 'F',    "fight", "префикс: принудительный бой, даже если не видно монстра",
              do_fight, PREFIXCMD, NULL },
    { 'f',    "fire", "стрелять снарядами из колчана",
              dofire, 0, NULL },
    { M('f'), "force", "взломать замок",
              doforce, AUTOCOMPLETE, NULL },
    { M('g'), "genocided",
              "показать монстров, которые были истреблены или вымерли",
              dogenocided,
              IFBURIED | AUTOCOMPLETE | GENERALCMD | CMD_M_PREFIX, NULL },
    { ';',    "glance", "показать, какому типу соответствует символ карты",
              doquickwhatis, IFBURIED | GENERALCMD, NULL },
    { '?',    "help", "показать справку",
              dohelp, IFBURIED | GENERALCMD, NULL },
    { '\0',   "herecmdmenu", "показать меню команд, доступных здесь",
              doherecmdmenu, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { '\0',    "history", "показать сводку по развитию игры",
              dohistory, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { 'i',    "inventory", "показать ваш инвентарь",
              ddoinv, IFBURIED | GENERALCMD, NULL },
    { 'I',    "inventtype", "показать инвентарь одного конкретного класса предметов",
              dotypeinv, IFBURIED | GENERALCMD, NULL },
    { M('i'), "invoke", "активировать особые силы предмета",
              doinvoke, IFBURIED | AUTOCOMPLETE, NULL },
    { M('j'), "jump", "прыгнуть в другое место",
              dojump, AUTOCOMPLETE, NULL },
    { C('d'), "kick", "пнуть что-нибудь",
              dokick, 0, NULL },
    { '\\',   "known", "показать, какие типы предметов открыты",
              dodiscovered, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { '`',    "knownclass", "показать открытые типы для одного класса предметов",
              doclassdisco, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { '\0',   "levelchange", "изменить уровень опыта",
              wiz_level_change, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { '\0',   "lightsources", "показать подвижные источники света",
              wiz_light_sources, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { ':',    "look", "посмотреть, что здесь находится",
              dolook, IFBURIED, NULL },
    { '\0',   "lookaround", "описать, что вы видите",
              dolookaround, IFBURIED | GENERALCMD, NULL },
    { M('l'), "loot", "обыскать ящик на полу",
              doloot, AUTOCOMPLETE | CMD_M_PREFIX, NULL },
    { '\0',   "migratemons",
#ifdef DEBUG_MIGRATING_MONS
              "показать перемещающихся монстров и переместить N случайных",
#else
              "показать перемещающихся монстров",
#endif
              wiz_migrate_mons, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { M('m'), "monster", "использовать особую способность монстра",
              domonability, IFBURIED | AUTOCOMPLETE, NULL },
    { M('n'), "name", "то же, что call; дать имя монстра, предмета или типа предмета",
              docallcmd, IFBURIED | AUTOCOMPLETE | GENERALCMD, NULL },
    { M('o'), "offer", "принести жертву богам",
              dosacrifice, AUTOCOMPLETE | CMD_M_PREFIX, NULL },
    { 'o',    "open", "открыть дверь",
              doopen, 0, NULL },
    /* 'm #options' runs doset() */
    { 'O',    "options", "показать настройки опций",
              doset_simple, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    /* 'm #optionsfull' runs doset_simple() */
    { '\0',   "optionsfull", "показать все настройки опций, возможно изменить их",
    ...
    { C('o'), "overview", "показать сводку по исследованному подземелью",
              dooverview,
              IFBURIED | AUTOCOMPLETE | GENERALCMD | CMD_M_PREFIX, NULL },
    /* [should #panic actually autocomplete?] */
    { '\0',   "panic", "проверить механизм паники (фатально для игры)",
              wiz_panic, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { 'p',    "pay", "оплатить ваш счёт за покупки",
              dopay, CMD_M_PREFIX, NULL },
    { '|',    "perminv", "прокручивать постоянное отображение инвентаря",
              doperminv, IFBURIED | GENERALCMD | NOFUZZERCMD, NULL },
    { ',',    "pickup", "подобрать вещи в текущем месте",
              dopickup, CMD_M_PREFIX, NULL },
    { '\0',   "polyself", "полиморфировать себя",
              wiz_polyself, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { M('p'), "pray", "помолиться богам о помощи",
              dopray, IFBURIED | AUTOCOMPLETE, NULL },
    { C('p'), "prevmsg", "просмотреть последние сообщения игры",
              doprev_message, IFBURIED | GENERALCMD | CMD_INSANE, NULL },
    { 'P',    "puton", "надеть аксессуар (кольцо, амулет и т.д.)",
              doputon, 0, NULL },
    { 'q',    "quaff", "выпить что-нибудь",
              dodrink, CMD_M_PREFIX, NULL },
    { '\0',   "quit", "выйти без сохранения текущей игры",
              done2, IFBURIED | AUTOCOMPLETE | GENERALCMD | NOFUZZERCMD,
              NULL },
    { 'Q',    "quiver", "выбрать снаряды для колчана",
              dowieldquiver, 0, NULL },
    { 'r',    "read", "прочитать свиток или книгу заклинаний",
              doread, 0, NULL },
    { C('r'), "redraw", "перерисовать экран",
              doredraw, IFBURIED | GENERALCMD | CMD_INSANE, NULL },
    { 'R',    "remove", "снять аксессуар (кольцо, амулет и т.д.)",
              doremring, 0, NULL },
    { C('a'), "repeat", "повторить предыдущую команду",
    ...
    { 'm',    "reqmenu", "префикс: запросить меню или изменить команду",
              do_reqmenu, PREFIXCMD, NULL },
    { C('_'), "retravel", "переместиться к ранее выбранному месту путешествия",
              dotravel_target, 0, NULL },
    { M('R'), "ride", "оседлать или спешиться с седланного скакуна",
              doride, AUTOCOMPLETE, NULL },
    { M('r'), "rub", "потереть лампу или камень",
              dorub, AUTOCOMPLETE, NULL },
    { 'G',    "run", "префикс: бежать, пока не увидишь что-нибудь интересное",
              do_run, PREFIXCMD, NULL },
    { 'g',    "rush", "префикс: мчаться, пока не увидишь что-нибудь интересное",
              do_rush, PREFIXCMD, NULL },
    { 'S',    "save", "сохранить игру и выйти",
              dosave, IFBURIED | GENERALCMD | NOFUZZERCMD, NULL },
    { '\0',   "saveoptions", "сохранить конфигурацию игры",
              do_write_config_file,
              IFBURIED | GENERALCMD | NOFUZZERCMD, NULL },
    { 's',    "search", "искать ловушки и потайные двери",
              dosearch, IFBURIED | CMD_M_PREFIX, "поиск" },
    { '*',    "seeall", "показать всё используемое снаряжение",
              doprinuse, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { AMULET_SYM, "seeamulet", "показать надетый в данный момент амулет",
              dopramulet, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { ARMOR_SYM, "seearmor", "показать надетую в данный момент броню",
              doprarm, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { RING_SYM, "seerings", "показать надетые в данный момент кольца",
              doprring, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { TOOL_SYM, "seetools", "показать используемые в данный момент инструменты",
              doprtool, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { WEAPON_SYM, "seeweapon", "показать зажатое в данный момент оружие",
              doprwep, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { '!',    "shell",
              "выйти из игры в оболочку ('exit' чтобы вернуться)",
    ...
    { GOLD_SYM, "showgold", "показать золото, возможно кредит или долг в магазине",
              doprgold, IFBURIED | GENERALCMD | CMD_M_PREFIX, NULL },
    { SPBOOK_SYM, "showspells", "список и перестановка известных заклинаний",
              dovspell, IFBURIED | GENERALCMD, NULL },
    { '^',    "showtrap", "описать соседнюю обнаруженную ловушку",
              doidtrap, IFBURIED | GENERALCMD, NULL },
    { M('s'), "sit", "сесть",
              dosit, AUTOCOMPLETE, NULL },
    { '\0',   "stats", "показать статистику памяти",
              wiz_show_stats, IFBURIED | AUTOCOMPLETE | WIZMODECMD, NULL },
    { C('z'), "suspend", "перевести игру в фоновый режим ('fg' чтобы вернуться)",
              dosuspend_core, (IFBURIED | GENERALCMD | NOFUZZERCMD
#ifndef SUSPEND
                               | CMD_NOT_AVAILABLE
#endif /* SUSPEND */
                               ), NULL },
    { 'x',    "swap", "поменять местами зажатое и вторичное оружие",
              doswapweapon, 0, NULL },
    { 'T',    "takeoff", "снять один предмет брони",
              dotakeoff, 0, NULL },
    { 'A',    "takeoffall", "снять всю броню",
              doddoremarm, 0, NULL },
    { C('t'), "teleport", "телепортироваться по уровню",
...
static const struct {
    ...
    { NHKF_ESC, "отменить текущий запрос или незавершённый префикс", FALSE },
    { NHKF_COUNT,
      "Префикс: для цифр перед командой с числом повторений", TRUE },
...
handler_rebind_keys_add(boolean keyfirst)
{
    ...
    if (key) {
        ...
        if (bind && bind->cmd) {
            Sprintf(buf, "Клавиша '%s' в данный момент привязана к \"%s\".",
                    key2txt(key, buf2), bind->cmd->ef_txt);
        } else {
            Sprintf(buf, "Клавиша '%s' ни к чему не привязана.",
                    key2txt(key, buf2));
        }
    ...
    if (key)
        Sprintf(buf, "К какой команде привязать '%s'?", key2txt(key, buf2));
    else
        Sprintf(buf, "К какой команде привязать?");
    end_menu(win, buf);
    ...
    if (npick > 0) {
        ...
        if (i == -1) {
            ...
            if ((ec->flags & CMD_PARAM) != 0) {
                ...
                parambuf[0] = '\0';
                Sprintf(querybuf, "Команда %s требует параметр:", ec->ef_txt);
 ...
 bindit:
        ...
        if (bind_key(key, cmdstr, TRUE)) {
            if (prevcmd && prevcmd->cmd != ec) {
                pline("Изменена клавиша '%s' с \"%s\" на \"%s\".",
                      key2txt(key, buf2), prevcmd->cmd->ef_txt, cmdstr);
            } else if (!prevcmd) {
                pline("Клавиша '%s' привязана к \"%s\".",
                      key2txt(key, buf2), cmdstr);
            }
        } else {
            pline("Привязка клавиши не удалась?!");
        }
    }
}
...
handler_rebind_keys(void)
{
 ...
 redo_rebind:
    ...
    any.a_int = 1;
    add_menu(win, &nul_glyphinfo, &any, '\0', 0, ATR_NONE, clr,
             "привязать клавишу к команде", MENU_ITEMFLAGS_NONE);
    any.a_int = 2;
    add_menu(win, &nul_glyphinfo, &any, '\0', 0, ATR_NONE, clr,
             "привязать команду к клавише", MENU_ITEMFLAGS_NONE);
    if (count_bind_keys()) {
        any.a_int = 3;
        add_menu(win, &nul_glyphinfo, &any, '\0', 0, ATR_NONE, clr,
                 "просмотреть изменённые привязки клавиш", MENU_ITEMFLAGS_NONE);
    }
    end_menu(win, "Что сделать?");
    npick = select_menu(win, PICK_ONE, &picks);
    destroy_nhwindow(win);
    if (npick > 0) {
        i = picks->item.a_int;
...
handler_change_autocompletions(void)
{
    ...
    end_menu(win, "Какие команды автодополняются?");
...
key2extcmddesc(uchar key)
{
    ...
    if (movecmd(k = key, MV_WALK))
        Strcpy(key2cmdbuf, "движение"); /* "move or attack"? */
    else if (movecmd(k = key, MV_RUSH))
        Strcpy(key2cmdbuf, "бег");
    else if (movecmd(k = key, MV_RUN))
        Strcpy(key2cmdbuf, "бег");
    if (digit(key) || (gc.Cmd.num_pad && digit(unmeta(key)))) {
        key2cmdbuf[0] = '\0';
        if (!gc.Cmd.num_pad)
            Strcpy(key2cmdbuf, "начало или продолжение числа");
        else if (key == '5' || key == M_5)
            Sprintf(key2cmdbuf, "префикс %s",
                    (!!gc.Cmd.pcHack_compat ^ (key == M_5)) ? "бег" : "бег");
        else if (key == '0' || (gc.Cmd.pcHack_compat && key == M_0))
            Strcpy(key2cmdbuf, "синоним для 'i'");
        if (*key2cmdbuf)
            return key2cmdbuf;
    }
        ...
        if (!strncmpi(key2cmdbuf, "prefix:", 7) && !strcmpi(txt, "reqmenu"))
            (void) strsubst(key2cmdbuf, "prefix:",
                     /* relies on implicit concatenation of literal strings */
                            "двигательный префикс:"
                            " движение без автоподбора и без атаки"
                            "\n"
                            "префикс для действий (не движения):"); /* and rest of buf */
...
bind_mousebtn(int btn, const char *command)
{
    ...
    if (btn < 1 || btn > NUM_MOUSE_BUTTONS) {
        config_error_add("Неверная кнопка мыши, допустимы 1-%i",
                         NUM_MOUSE_BUTTONS);
        return FALSE;
    }
    btn--;
...
bind_key(uchar key, const char *command, boolean user)
{
    ...
    for (extcmd = extcmdlist; extcmd->ef_txt; extcmd++) {
        ...
        if ((extcmd->flags & CMD_PARAM) != 0) {
            if (!p) {
                config_error_add("'%s' требует параметр", buf);
            } else {
                struct Cmd_bind *bind = cmdbind_get(key);
                int maxlen = min(30, strlen(p)) + 1;

                if (maxlen <= 1) {
                    config_error_add("Обязательный параметр не может быть пустым");
                } else {
            ...
            config_error_add("'%s' не принимает параметр", buf);
...
dokeylist(void)
{
    ...
    putstr(datawin, 0, "");
    Sprintf(buf, "%7s %s", "", "    Полный список текущих привязок клавиш");
    putstr(datawin, 0, buf);
    for (extcmd = extcmdlist; extcmd->ef_txt; ++extcmd)
        if (spkey_gap || !keylist_func_has_key(extcmd, keys_used)) {
            Sprintf(buf, "%7s %s", "",
                               "(также команды без назначенной клавиши)");
            putstr(datawin, 0, buf);
            break;
        }
    ...
    /* directional keys */
    putstr(datawin, 0, "");
    putstr(datawin, 0, "Клавиши направления:");
    show_direction_keys(datawin, '.', FALSE); /* '.'==self in direct'n grid */

    if (!iflags.num_pad) {
        putstr(datawin, 0, "");
        putstr(datawin, 0,
     "Ctrl+<направление> будет идти в указанном направлении, пока не увидит что-то очень");
        Sprintf(buf, "%7s %s", "", "интересное.");
        putstr(datawin, 0, buf);
    ...
    Strcat(buf,
          "+<направление> будет идти в указанном направлении, пока не столкнётесь с");
    putstr(datawin, 0, buf);
    Sprintf(buf, "%7s %s", "", "препятствием.");
    putstr(datawin, 0, buf);

    putstr(datawin, 0, "");
    putstr(datawin, 0, "Прочие клавиши:");
    for (i = 0; misc_keys[i].desc; ++i) {
        if (misc_keys[i].numpad && !iflags.num_pad)
            continue;
        j = misc_keys[i].nhkf;
...
#endif
    Strcat(buf, " прерывание: выход из NetHack (SIGINT)");
    putstr(datawin, 0, buf);
...
#define IGNORECMD (WIZMODECMD | INTERNALCMD | MOVEMENTCMD)
    ...
    if (keylist_putcmds(datawin, TRUE, GENERALCMD, IGNORECMD, keys_used)) {
        putstr(datawin, 0, "");
        putstr(datawin, 0, "Общие команды:");
    ...
    if (keylist_putcmds(datawin, TRUE, 0, GENERALCMD | IGNORECMD, keys_used)) {
        putstr(datawin, 0, "");
        putstr(datawin, 0, "Игровые команды:");
    ...
    if (wizard && keylist_putcmds(datawin, TRUE,
        ...
        putstr(datawin, 0, "");
        putstr(datawin, 0, "Команды режима отладки:");
...
key2txt(uchar c, char *txt) /* sufficiently long buffer */
{
    ...
    if (c == ' ')
        ...
        Sprintf(txt, "<ввод>"); /* "<return>" won't fit */
    else if (c == '\177')
...
parseautocomplete(char *autocomplete, boolean condition)
{
    ...
    raw_printf("Ошибка автозавершения: некорректная расширенная команда '%s'.",
               autocomplete);
    wait_synch();
}
...
update_rest_on_space(void)
{
    ...
    static const struct ext_func_tab restonspace = {
        ' ', "wait", "отдых на один ход (через опцию 'rest_on_space')",
        donull, (IFBURIED | CMD_M_PREFIX), "ожидание"
    };
...
rhack(int key)
{
 ...
 do_cmdq_extcmd:
            ...
            if (!can_do_extcmd(tlist)) {
                ...
                const char *which = (pfxidx != 0) ? visctrl(pfxidx)
                                    : (prefix_seen->ef_funct == do_reqmenu)
                                      ? "движение-без-подхвата или запрос-меню"
                                      : prefix_seen->ef_txt;
                ...
                if (was_m_prefix) {
                    custompline(SUPPRESS_HISTORY,
                          "Команда %s не принимает префикс '%s'.",
                          tlist->ef_txt, which);
                } else {
                    uchar ch = tlist->key;
                    ...
                    pline(
                "Префикс '%s' должен сопровождаться командой движения%s.",
                          which,
                          (up || down) ? " кроме вверх или вниз" : "");
                }
                res = ECMD_FAIL;
                prefix_seen = 0;
            } else {
                ...
                if ((tlist->flags & PREFIXCMD) != 0) {
                    ...
                    You_cant("добраться туда отсюда...");
                    reset_cmd_vars(TRUE);
                    return;
    ...
    if (bad_command) {
        custompline(SUPPRESS_HISTORY, "Неизвестная команда '%s'.", visctrl(key));
        cmdq_clear(CQ_CANNED);
        cmdq_clear(CQ_REPEAT);
...
getdir(const char *s)
{
 ...
 retry:
    ...
    if (gi.in_doagain || *readchar_queue) {
        ...
        dirsym = yn_function((s && *s != '^') ? s : "В каком направлении?",
                             (char *) 0, '\0', FALSE);
 ...
 got_dirsym:
    if (dirsym == gc.Cmd.spkeys[NHKF_GETDIR_SELF]
         ...
         */
        Sprintf(qbuf,
            "нужное место, затем нажмите '%s' для левого клика, '%s' для правого",
        ...
        if (!strchr(quitchars, dirsym)) {
            ...
            if (help_requested || iflags.cmdassist) {
                did_help = help_dir((s && *s == '^') ? dirsym : '\0',
                                    ...
                                    : "Неверная клавиша направления!");
                if (help_requested)
                    goto retry;
            }
            if (!did_help)
                pline("Какое странное направление!");
        }
        return 0;
    } else if (is_mov && !dxdy_moveok()) {
        You_cant("ориентироваться в том направлении.");
        return 0;
    }
    if (!u.dz)
        confdir(FALSE);
    ...
    const char *msg)
{
    ...
    if (*buf) {
        ...
        Sprintf(buf, "подсказка (cmdassist): %s", msg);
        putstr(win, 0, buf);
        putstr(win, 0, "");
    }
    ...
    if (!prefixhandling && (letter(sym) || sym == '[')) {
        ...
        if ((explain = dowhatdoes_core(ctrl, buf2)) != 0
            ...
            Sprintf(buf, "Вы пытаетесь использовать ^%c%s?", sym,
            ...
            putstr(win, 0, "");
            putstr(win, 0,
                  "Чтобы использовать эту команду, удерживайте <Ctrl> как shift");
            Sprintf(buf, "и нажмите клавишу <%c>.", sym);
            putstr(win, 0, buf);
            putstr(win, 0, "");
        }
    }

    Sprintf(buf, "Допустимые клавиши направления %s%s%s:",
            prefixhandling ? " для " : "", prefixhandling ? dothat : "",
            NODIAG(u.umonnum) ? " в вашей текущей форме" : "");
    putstr(win, 0, buf);
    ...
    if (!prefixhandling) {
        ...
        if (!prefixhandling) {
            ...
            Sprintf(buf,   "       %4s  нацелиться на себя",
    ...
    if (msg) {
        ...
        putstr(win, 0, "");
        putstr(win, 0,
               "(Подавить это сообщение можно с помощью !cmdassist в файле настроек.)");
    }
...
directionname(int dir)
{
    static NEARDATA const char *const dirnames[N_DIRS_Z] = {
        "запад",    "северо-запад", "север", "северо-восток", "восток",
        "юго-восток", "юг", "юго-запад", "вниз", "вверх",
    };
...
there_cmd_menu_self(winid win, coordxy x, coordxy y, int *act UNUSED)
{
    ...
    if (IS_FOUNTAIN(typ) && can_reach_floor(FALSE))
        mcmd_addmenu(win, MCMD_DIP, "Окунуть что-нибудь в фонтан"), ++K;
    if (IS_THRONE(typ))
        mcmd_addmenu(win, MCMD_SIT, "Сесть на трон"), ++K;
    if (IS_ALTAR(typ))
        mcmd_addmenu(win, MCMD_OFFER, "Принести жертву на алтаре"), ++K;

    if (stway && stway->up) {
        Sprintf(buf, "Подняться по %s",
                stway->isladder ? "приставной лестнице" : "лестнице");
        mcmd_addmenu(win, MCMD_UP, buf), ++K;
    }
    if (stway && !stway->up) {
        Sprintf(buf, "Спуститься по %s",
                stway->isladder ? "приставной лестнице" : "лестнице");
        mcmd_addmenu(win, MCMD_DOWN, buf), ++K;
    }
    if (u.usteed) { /* another movement choice */
        Sprintf(buf, "Спешиться с %s",
    ...
    if (OBJ_AT(x, y)) {
        ...
        Sprintf(buf, "Подобрать %s", otmp->nexthere ? "предметы" : doname(otmp));
        mcmd_addmenu(win, MCMD_PICKUP, buf), ++K;

        if (Is_container(otmp)) {
            Sprintf(buf, "Обыскать %s", doname(otmp));
            mcmd_addmenu(win, MCMD_LOOT, buf), ++K;

            Sprintf(buf, "Опорожнить %s", doname(otmp));
            mcmd_addmenu(win, MCMD_TIP, buf), ++K;
        }
        if (otmp->oclass == FOOD_CLASS) {
            Sprintf(buf, "Съесть %s", doname(otmp));
    ...
    if (gi.invent) {
        mcmd_addmenu(win, MCMD_INVENTORY, "Инвентарь"), ++K;
        mcmd_addmenu(win, MCMD_DROP, "Выбросить предметы"), ++K;
    }
    mcmd_addmenu(win, MCMD_REST, "Отдохнуть один ход"), ++K;
    mcmd_addmenu(win, MCMD_SEARCH, "Поискать вокруг"), ++K;
    mcmd_addmenu(win, MCMD_LOOK_HERE, "Посмотреть, что здесь"), ++K;

    if (num_spells() > 0)
        mcmd_addmenu(win, MCMD_CAST_SPELL, "Сотворить заклинание"), ++K;

    if ((ttmp = t_at(x, y)) != 0 && ttmp->tseen) {
        if (ttmp->ttyp != VIBRATING_SQUARE)
            mcmd_addmenu(win, MCMD_UNTRAP_HERE,
                         "Попытаться обезвредить ловушку"), ++K;
    }
    return K;
}
    ...
    int *act)
{
    ...
    if (IS_DOOR(typ)) {
        ...
        if ((dm & (D_CLOSED | D_LOCKED))) {
            mcmd_addmenu(win, MCMD_OPEN_DOOR, "Открыть дверь"), ++K;
            ...
            if (key_or_pick || card) {
                Sprintf(buf, "%sотпереть дверь",
                        key_or_pick ? "отпереть или " : "");
            ...
            mcmd_addmenu(win, MCMD_UNTRAP_DOOR,
                         "Поискать ловушку на двери"), ++K;
            /* [what about #force?] */
            mcmd_addmenu(win, MCMD_KICK_DOOR, "Пнуть дверь"), ++K;
        } else if ((dm & D_ISOPEN) && (mod == CLICK_2)) {
            mcmd_addmenu(win, MCMD_CLOSE_DOOR, "Закрыть дверь"), ++K;
        }
    }

    if (typ <= SCORR)
        mcmd_addmenu(win, MCMD_SEARCH, "Поискать потайные двери"), ++K;

    if ((ttmp = t_at(x, y)) != 0 && ttmp->tseen) {
        mcmd_addmenu(win, MCMD_LOOK_TRAP, "Осмотреть ловушку"), ++K;
        if (ttmp->ttyp != VIBRATING_SQUARE)
            mcmd_addmenu(win, MCMD_UNTRAP_TRAP,
                                 "Попытаться обезвредить ловушку"), ++K;
        mcmd_addmenu(win, MCMD_MOVE_DIR, "Наступить на ловушку"), ++K;
    }

    if (levl[x][y].glyph == objnum_to_glyph(BOULDER))
        mcmd_addmenu(win, MCMD_MOVE_DIR, "Толкнуть валун"), ++K;
    ...
    if ((mtmp && !(mtmp->mpeaceful || mtmp->mtame))
        ...
        Sprintf(buf, "Атаковать %s", mtmp ? mon_nam(mtmp) : "невидимое существо");
...
there_cmd_menu_far(winid win, coordxy x, coordxy y, int mod)
{
    ...
    if (mod == CLICK_1) {
        if (linedup(u.ux, u.uy, x, y, 1)
            ...
            mcmd_addmenu(win, MCMD_THROW_OBJ, "Бросить что-нибудь"), ++K;

        mcmd_addmenu(win, MCMD_TRAVEL, "Переместиться сюда"), ++K;
    }
    return K;
}
    ...
    int *act UNUSED)
{
    ...
    if (mod == CLICK_1 || mod == CLICK_2) { /* ignore iflags.clicklook here */
        ...
        if (!u_at(x, y) || Upolyd || glyph_at(x, y) != hero_glyph)
            mcmd_addmenu(win, MCMD_LOOK_AT, "Посмотреть на символ карты"), ++K;
    }
    return K;
}
...
there_cmd_menu(coordxy x, coordxy y, int mod)
{
    ...
    if (!K) {
        ...
        end_menu(win, "Что вы хотите сделать?");
        npick = select_menu(win, PICK_ONE, &picks);
        ch = '\033';
    }
    destroy_nhwindow(win);
...
dotravel(void)
{
    ...
    if (iflags.menu_requested) {
        ...
        pline("Куда вы хотите переместиться?");
        if (getpos(&cc, TRUE, "желаемый пункт назначения") < 0) {
            /* user pressed ESC */
            iflags.getloc_travelmode = FALSE;
            return ECMD_CANCEL;
        }
...
dotravel_target(void)
{
    if (!isok(iflags.travelcc.x, iflags.travelcc.y)) {
        ...
        pline("Место назначения не задано.");
        return ECMD_OK;
    ...
    char *res)
{
    if (yn_menuable_resp(resp)) {
        ...
        if (resp == rightleftchars) {
            yn_func_menu_opt(win, 'r', "Вправо", def);
            yn_func_menu_opt(win, 'l', "Влево", def);
        } else if (resp == hidespinchars) {
            yn_func_menu_opt(win, 'h', "Спрятаться", def);
            yn_func_menu_opt(win, 's', "Сплести паутину", def);
        } else {
            yn_func_menu_opt(win, 'y', "Да", def);
            yn_func_menu_opt(win, 'n', "Нет", def);
        }
        if (resp == ynaqchars)
            yn_func_menu_opt(win, 'a', "Все", def);
        if (resp == ynqchars || resp == ynaqchars || resp == hidespinchars)
            yn_func_menu_opt(win, 'q', "Выйти", def);
        end_menu(win, query);
