...
charm_snakes(int distance)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (mtmp->data->mlet == S_SNAKE && mtmp->mcanmove
            ...
            if (canseemon(mtmp)) {
                if (!could_see_mon)
                    You("замечаете %s, покачивающегося в такт музыке.", a_monnam(mtmp));
                else
                    pline("%s замирает, а затем покачивается в такт музыке%s.",
                          Monnam(mtmp),
                          was_peaceful ? "" : ", и теперь кажется спокойнее");
            }
        }
    }
}
...
calm_nymphs(int distance)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (mtmp->data->mlet == S_NYMPH && mtmp->mcanmove
            ...
            if (canseemon(mtmp))
                pline(
                    "%s с удовольствием слушает музыку, а затем затихает.",
                      Monnam(mtmp));
        }
    }
}
...
awaken_soldiers(struct monst *bugler  /* monster that played instrument */)
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        ...
        if (is_mercenary(mtmp->data) && mtmp->data != &mons[PM_GUARD]) {
            ...
            mtmp->mstrategy &= ~STRAT_WAITMASK;
            if (canseemon(mtmp))
                pline("%s теперь готов к бою!", Monnam(mtmp));
            else if (!Deaf)
                Norep("%s звук подготовки боевого снаряжения.",
...
do_pit(coordxy x, coordxy y, unsigned tu_pit)
{
    ...
    if ((otmp = sobj_at(BOULDER, x, y)) != 0) {
        if (cansee(x, y))
            pline("КАДУМ!  Валун падает в пропасть%s!",
                  u_at(x, y) ? " below you" : "");
        if (mtmp)
            mtmp->mtrapped = 0;
    ...
    if (mtmp) {
        if (!is_flyer(mtmp->data) && !is_clinger(mtmp->data)) {
            ...
            if (!m_already_trapped) { /* suppress messages */
                if (cansee(x, y)) {
                    pline("%s падает в пропасть!", Monnam(mtmp));
            ...
            mselftouch(mtmp, "Падая, ", TRUE);
            if (!DEADMONSTER(mtmp)) {
                mtmp->mhp -= rnd(m_already_trapped ? 4 : 6);
                if (DEADMONSTER(mtmp)) {
                    if (!cansee(x, y)) {
                        pline("Оно уничтожено!");
                    } else {
                        You("разрушаете %s!",
                            mtmp->mtame
        ...
        if (Levitation || Flying || is_clinger(gy.youmonst.data)) {
            ...
            You("вас яростно швыряет из стороны в сторону!");
            set_utrap(rn1(6, 2), TT_PIT);
...
{
    static const char into_a_chasm[] = " в расщелину";
    coordxy x, y;
    struct monst *mtmp;
    ...
    for (x = start_x; x <= end_x; x++)
            ...
            if ((mtmp = m_at(x, y)) != 0) {
                ...
                if (mtmp->mundetected) {
                    ...
                    if (ceiling_hider(mtmp->data)) {
                        if (cansee(x, y)) {
                            pline("%s сотрясается с потолка!",
                                  Amonnam(mtmp));
                        } else if (!is_flyer(mtmp->data)) {
                            Soundeffect(se_thump, 50);
                            You_hear("глухой удар.");
                        }
                    }
                }
        ...
        */
            ...
            case FOUNTAIN: /* make the fountain disappear */
                if (cansee(x, y))
                    pline_The("фонтан обрушивается%s.", into_a_chasm);
                do_pit(x, y, tu_pit);
                break;
            case SINK:
                if (cansee(x, y))
                    pline_The("кухонная раковина проваливается%s.", into_a_chasm);
                do_pit(x, y, tu_pit);
                break;
            case ALTAR:
                ...
                if (cansee(x, y))
                    pline_The("%s алтарь обрушивается%s.",
            ...
            case GRAVE:
                if (cansee(x, y))
                    pline_The("надгробие опрокидывается%s.", into_a_chasm);
                do_pit(x, y, tu_pit);
                break;
            case THRONE:
                if (cansee(x, y))
                    pline_The("трон обрушивается%s.", into_a_chasm);
                do_pit(x, y, tu_pit);
                break;
            case SCORR:
                levl[x][y].typ = CORR;
                unblock_point(x, y);
                if (cansee(x, y))
                    pline("Обнаружен скрытый коридор.");
                FALLTHROUGH;
                /*FALLTHRU*/
            case CORR:
            case ROOM:
            ...
            case SDOOR:
                ...
                if (cansee(x, y))
                    pline("Обнаружена скрытая дверь.");
                FALLTHROUGH;
                /*FALLTHRU*/
...
static const char *beats[] = {
    "степпер", "один дроп", "медленный ту-степ", "тройная дробь",
    "двойной шаффл", "замедленный шаффл", "вторая линия", "поезд"
};
...
#define PLAY_HALLU    0x04
    ...
    switch (mode) {
    case PLAY_NORMAL:
        You("начинаете играть на %s.", yname(instr));
        break;
    case PLAY_STUNNED:
        if (!Deaf)
            You("излучаете противный гудящий звук.");
        else
            You_feel("монотонную вибрацию.");
        break;
    case PLAY_CONFUSED:
        if (!Deaf)
            You("издаёте резкий шум.");
        else
            You_feel("резкую вибрацию.");
        break;
    case PLAY_HALLU:
        You("распространяете калейдоскопическое зрелище порхающих бабочек.");
        break;
    ...
    default:
        pline("То, что вы исполняете, совсем не похоже на музыку...");
        break;
    }
#undef PLAY_NORMAL
#undef PLAY_STUNNED
...
#undef PLAY_HALLU
    ...
    case MAGIC_FLUTE: /* Make monster fall asleep */
        ...
        You("%sиздаёте %s%s музыку.", !Deaf ? "" : ", кажется, ",
            Hallucination ? "свистящую" : "негромкую",
            same_old_song ? ", знакомую" : "");
    ...
    case WOODEN_FLUTE: /* May charm snakes */
        ...
        if (!Deaf)
            pline("%s%s.", Tobjnam(instr, do_spec ? "заливается" : "гудит"),
                  same_old_song ? " знакомой мелодией" : "");
        else
            You_feel("как %s вибрирует.", yname(instr));
        Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
        if (do_spec)
    ...
    case FROST_HORN: /* Idem wand of cold */
        ...
        if (!getdir((char *) 0)) {
            pline("%s.", Tobjnam(instr, "вибрирует"));
            break;
        } else if (!u.dx && !u.dy && !u.dz) {
            if ((damage = zapyourself(instr, TRUE)) != 0) {
                char buf[BUFSZ];

                Sprintf(buf, "использовав магический рог на себя");
                Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
            ...
            if (!Blind)
                pline_The("из рога вырывается %s!", flash_str(type, FALSE));
            Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
    ...
    case TOOLED_HORN: /* Awaken or scare monsters */
        if (!Deaf)
            You("издаёте ужасающий, низкий%s звук.",
                same_old_song ? ", но знакомый," : "");
        else
            You("дуете в рог.");
    ...
    case BUGLE: /* Awaken & attract soldiers */
        if (!Deaf)
            You("извлекаете громкий%s звук из %s.",
                same_old_song ? ", знакомый" : "", yname(instr));
        else
            You("дуете в горн.");
        Hero_playnotes(obj_to_instr(&itmp), improvisation, 80);
    ...
    case MAGIC_HARP: /* Charm monsters */
        ...
        if (!Deaf)
            pline("%s очень привлекательную%s музыку.",
                  Tobjnam(instr, "извлекает"),
                  same_old_song ? " и знакомую" : "");
        else
            You_feel("очень успокаивающие вибрации.");
        Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
    ...
    case WOODEN_HARP: /* May calm Nymph */
        ...
        if (!Deaf)
            pline("%s %s.", Yname2(instr),
                  ...
                  ? "издаёт знакомую, певучую мелодию"
                  : (do_spec) ? "издаёт певучую мелодию"
                    : (same_old_song) ? "дребезжит знакомый мотив"
                      : "дребезжит");
        else
            You_feel("успокаивающие вибрации.");
        Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
        if (do_spec)
    ...
    case DRUM_OF_EARTHQUAKE: /* create several pits */
        ...
        You("издаёте тяжёлый, громоподобный раскат!");
        Hero_playnotes(obj_to_instr(&itmp), "C", 100);
        pline_The("%s сотрясается вокруг вас!", generic_lvl_desc());
    ...
    case LEATHER_DRUM: /* Awaken monsters */
        if (!mundane) {
            if (!Deaf) {
                You("поднимаете %sоглушительный грохот!",
                    same_old_song ? "знакомый " : "");
            ...
            You("%s %s.",
                rn2(2) ? "искромсали" : rn2(2) ? "сумели выдать" : "выдали",
                an(ROLL_FROM(beats)));
            Hero_playnotes(obj_to_instr(&itmp), improvisation, 50);
        }
...
do_play_instrument(struct obj *instr)
{
    ...
    if (Underwater) {
        You_cant("играть музыку под водой!");
        return ECMD_OK;
        ...
        You("не умеете играть на этом инструменте.");
        return ECMD_OK;
    }
    if (instr->otyp != LEATHER_DRUM && instr->otyp != DRUM_OF_EARTHQUAKE
        && !(Stunned || Confusion || Hallucination)) {
        c = ynq("Импровизировать?");
        if (c == 'q')
            goto nevermind;
    }
    ...
    if (u.uevent.uheard_tune == 2)
        c = ynq("Сыграть мелодию прохода?");
    if (c == 'q') {
        goto nevermind;
    } else if (c == 'y') {
        Strcpy(buf, svt.tune);
    } else {
        getlin("Какую мелодию вы играете? [5 нот, A-G]", buf);
        (void) mungspaces(buf);
        if (*buf == '\033')
            goto nevermind;
    ...
    You(!Deaf ? "извлекаете странный звук из %s!"
              : "чувствуете, как %s издаёт вибрации", the(xname(instr)));
            ...
            if (ok) { /* There is a drawbridge near */
                ...
                if (tumblers) {
                    if (gears) {
                        ...
                        You_hear("щёлкают %d защёлки, вращаются %d шестерни.",
                                 tumblers, gears);
                    } else {
                        Soundeffect(se_tumbler_click, 50);
                        You_hear("щёлкают %d защёлки.", tumblers);
                    }
                } else if (gears) {
                    You_hear("вращаются %d шестерни.", gears);
