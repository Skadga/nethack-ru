...
const struct Role roles[NUM_ROLES+1] = {
    { { "Археолог", 0 },
      { { "Землекоп", 0 },
        ...
        { "Эксгуматор", 0 },
        { "Землерой", 0 },
        { "Спелеолог-любитель", 0 },
        { "Спелеолог", 0 },
        { "Коллекционер", 0 },
        { "Хранитель", 0 } },
      "Кецалькоатль", "Камаштли", "Уэуэтеотль", /* Central American */
      "Arc",
    ...
    { { "Варвар", 0 },
      { { "Грабитель", "Plunderess" },
        ...
        { "Опустошитель", 0 },
        { "Убийца", 0 },
        { "Вождь", "Предводительница" },
        { "Завоеватель", "Завоевательница" } },
      "Митра", "Кром", "Сет", /* Hyborian */
      "Bar",
    ...
    { { "Целитель", 0 },
      { { "Корневед", 0 },
        ...
        { "Бальзамировщик", 0 },
        { "Санитар", 0 },
        { "Костоправ", "Костоправка" },
        { "Травник", 0 },
        { "Магистр", "Магистра" },
        { "Лекарь", 0 },
        { "Хирург", 0 } },
      "_Афина", "Гермес", "Посейдон", /* Greek */
      "Hea",
    ...
    { { "Рыцарь", 0 },
      { { "Галант", 0 },
        ...
        { "Сеньор", "Дама" },
        { "Паладин", 0 } },
      "Луг", "_Бригитта", "Мананнан-Мак-Лир", /* Celtic */
      "Kni",
      "Camelot Castle",
      "the Isle of Glass",
      PM_KNIGHT,
    ...
    { { "Жрец", "Жрица" },
      { { "Искатель", 0 },
        ...
        { "Лама", 0 },
        { "Патриарх", "Матриарх" },
        { "Верховный жрец", "Верховная жрица" } },
    ...
    { { "Следопыт", 0 },
      {
        ...
        { "Скаут", 0 },
        { "Арбалетчик", 0 }, /* One skilled at crossbows */
        { "Лучник", 0 },
        { "Снайпер", 0 },
        { "Стрелок", "Стрелок-ж" } },
      "Меркурий", "_Венера", "Марс", /* Roman/planets */
      "Ran",
      "Orion's camp",
      ...
      -4 },
    { { "Самурай", 0 },
      { { "Хатамото", 0 },       /* Banner Knight */
        { "Ронин", 0 },          /* no allegiance */
        { "Ниндзя", "Куноити" }, /* secret society */
        { "Дзёсю", 0 },          /* heads a castle */
        { "исю", 0 },         /* has a territory */
        { "Кокусю", 0 },        /* heads a province */
        { "Даймё", 0 },         /* a samurai lord */
        { "Кугэ", 0 },           /* Noble of the Court */
        { "игун", 0 } },       /* supreme commander, warlord */
      "_Аматэрасу Омиками", "Райдзин", "Сусаноо", /* Japanese */
      "Sam",
    ...
    { { "Волшебник", 0 },
      { { "Призыватель", 0 },
        ...
        { "Чудотворец", 0 },
        { "Маг", 0 },
        { "Заклинатель", "Заклинательница" },
        { "Чародей", "Колдунья" },
        { "Некромант", 0 },
        { "Волшебник", 0 },
        { "Маг", 0 } },
      "Птах", "Тот", "Анхур", /* Egyptian */
      "Wiz",
      "the Lonely Tower",
...
const struct Gender genders[] = {
    { "мужской", "он", "его", "его", "Mal", ROLE_MALE },
    { "женская", "она", "её", "её", "Fem", ROLE_FEMALE },
    { "средний", "оно", "его", "его", "Ntr", ROLE_NEUTER },
    /* used by pronoun_gender() when hallucinating */
    { "группа", "они", "их", "их", "Grp", 0 },
};

/* Table of all alignments */
const struct Align aligns[] = {
    { "закон", "законопослушный", "Law", ROLE_LAWFUL, A_LAWFUL },
    { "равновесие", "нейтральный", "Neu", ROLE_NEUTRAL, A_NEUTRAL },
    { "хаос", "хаотичный", "Cha", ROLE_CHAOTIC, A_CHAOTIC },
    { "зло", "безверный", "Una", 0, A_NONE }
};
...
rolefilterstring(char *outbuf, int which)
{
    ...
    case RS_ALGNMNT:
        ...
        break;
    default:
        impossible("rolefilterstring: неверный аспект роли (%d)", which);
        Strcpy(outbuf, " ?");
        break;
    }
...
build_plselection_prompt(
    ...
    int rolenum, int racenum, int gendnum, int alignnum)
{
    const char *defprompt = "Подобрать персонажа для тебя? [ynaq] ";
    ...
    Strcpy(tmpbuf, "Подобрать ");
    if (racenum != ROLE_NONE || validrole(rolenum))
        Strcat(tmpbuf, "твой ");
    else
        Strcat(tmpbuf, "некоего ");
    /* <your> */
    ...
    Strcpy(buf, tmpbuf); /* RU translation: no possessive */
    ...
    if (num_post_attribs) {
        ...
        if (gr.role_pa[BP_ALIGN]) {
            (void) promptsep(eos(buf), num_post_attribs);
            Strcat(buf, "мировоззрение");
        }
    }
    Strcat(buf, " для тебя? [ynaq] ");
    return buf;
}
...
role_menu_extra(int which, winid where, boolean preselect)
{
    ...
    if (constrainer) {
        ...
        Sprintf(buf, "%4s%s вынуждает %s", "", constrainer, forcedvalue);
        add_menu_str(where, buf);
    } else if (what) {
        any.a_int = RS_menu_arg(which);
        Sprintf(buf, "Выбрать%s %s сначала", (f >= 0) ? " ещё" : "", what);
        ...
        Sprintf(setfiltering, "%s фильтрацию роли/расы и прочего",
                gotrolefilter() ? "Сбросить" : "Задать");
...
Hello(struct monst *mtmp)
{
    switch (Role_switch) {
    case PM_KNIGHT:
        return "Приветствую"; /* Olde English */
    case PM_SAMURAI:
        return (mtmp && mtmp->data == &mons[PM_SHOPKEEPER])
                    ? "Добро пожаловать"
                    : "Здравствуйте"; /* Japanese */
    case PM_TOURIST:
        return "Алоха"; /* Hawaiian */
    case PM_VALKYRIE:
        return
#ifdef MAIL_STRUCTURES
               (mtmp && mtmp->data == &mons[PM_MAIL_DAEMON]) ? "Привет" :
#endif
               "Приветствую"; /* Norse */
    default:
        return "Привет";
...
Goodbye(void)
{
    switch (Role_switch) {
    case PM_KNIGHT:
        return "Всего доброго"; /* Olde English */
    case PM_SAMURAI:
        return "Прощайте"; /* Japanese */
    case PM_TOURIST:
        return "Алоха"; /* Hawaiian */
    case PM_VALKYRIE:
        return "Прощай"; /* Norse */
 ...
 makepicks:
        ...
        if (nextpick == RS_ROLE) {
            ...
            if (ROLE < 0) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || ROLE == ROLE_RANDOM) {
                    ...
                    Strcpy(pbuf, "Выберите роль или профессию");
                    end_menu(win, pbuf);
            ...
            if (RACE < 0 || !validrace(ROLE, RACE)) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || RACE == ROLE_RANDOM) {
                    ...
                    if (n > 1) {
                        ...
                        role_menu_extra(RS_filter, win, FALSE);
                        role_menu_extra(ROLE_NONE, win, FALSE); /* quit */
                        Strcpy(pbuf, "Выберите расу или вид");
                        end_menu(win, pbuf);
                        n = select_menu(win, PICK_ONE, &selected);
                        if (n > 0) {
                            choice = selected[0].item.a_int;
        ...
        if (nextpick == RS_ALGNMNT) {
            ...
            if (ALGN < 0 || !validalign(ROLE, RACE, ALGN)) {
                ...
                if (pick4u == 'y' || pick4u == 'a' || ALGN == ROLE_RANDOM) {
                    ...
                    if (k < 0) {
                        pline("Несовместимная ориентация!");
                    ...
                    if (n > 1) {
                        ...
                        role_menu_extra(RS_filter, win, FALSE);
                        role_menu_extra(ROLE_NONE, win, FALSE); /* quit */
                        Strcpy(pbuf, "Выберите мировоззрение или веру");
                        end_menu(win, pbuf);
                        n = select_menu(win, PICK_ONE, &selected);
                        if (n > 0) {
                            choice = selected[0].item.a_int;
    ...
    while (getconfirmation) {
        ...
        add_menu(win, &nul_glyphinfo, &any, 'y', 0,
                 ATR_NONE, clr, "Да; начать игру", MENU_ITEMFLAGS_SELECTED);
        any.a_int = 2;
        add_menu(win, &nul_glyphinfo, &any, 'n', 0,
                 ATR_NONE, clr, "Нет; выбрать роль снова", MENU_ITEMFLAGS_NONE);
        if (iflags.renameallowed) {
            any.a_int = 3;
            add_menu(win, &nul_glyphinfo, &any, 'a', 0, ATR_NONE,
                     clr, "Не сейчас; выбрать другое имя",
                     MENU_ITEMFLAGS_NONE);
        }
        any.a_int = -1;
        add_menu(win, &nul_glyphinfo, &any, 'q', 0,
                 ATR_NONE, clr, "Выйти", MENU_ITEMFLAGS_NONE);
        Sprintf(pbuf, "Всё верно? [yn%sq]", iflags.renameallowed ? "a" : "");
        end_menu(win, pbuf);
        n = select_menu(win, PICK_ONE, &selected);
...
reset_role_filtering(void)
{
    ...
    add_menu_str(win, "Недопустимые роли");
    setup_rolemenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu_str(win, "");
    add_menu_str(win, "Недопустимые расы");
    setup_racemenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu_str(win, "");
    add_menu_str(win, "Недопустимые полы");
    setup_gendmenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    add_menu_str(win, "");
    add_menu_str(win, "Недопустимые мировоззрения");
    setup_algnmenu(win, FALSE, ROLE_NONE, ROLE_NONE, ROLE_NONE);

    Sprintf(filterprompt, "Отметьте всё подходящее%s",
            gotrolefilter() ? " и/или уберите то, что больше не подходит" : "");
