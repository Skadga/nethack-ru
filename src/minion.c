...
msummon(struct monst *mon)
{
    ...
    if (mon) {
        ...
        if (u_wield_art(ART_DEMONBANE) && is_demon(ptr)) {
            if (canseemon(mon))
                pline("%s на мгновение выглядит озадаченным.", Monnam(mon));
            return 0;
        }
    ...
    while (cnt > 0) {
        ...
        if (mtmp) {
            ...
            if (cnt == 1 && canseemon(mtmp)) {
                ...
                pline("%s появляется %s %s!", Amonnam(mtmp),
                      cloud, what);
            }
        }
        cnt--;
...
summon_minion(aligntyp alignment, boolean talk)
{
    ...
    if (mon) {
        if (talk) {
            if (!Deaf)
                pline("Голос %s гремит:", align_gname(alignment));
            else
                You_feel("%s громовой голос:",
                         s_suffix(align_gname(alignment)));
            SetVoice(mon, 0, 80, 0);
            verbalize("Ты ответишь за свою дерзость!");
            if (canspotmon(mon))
                pline("%s появляется перед вами.", Amonnam(mon));
...
demon_talk(struct monst *mtmp)
{
    ...
    if (u_wield_art(ART_EXCALIBUR) || u_wield_art(ART_DEMONBANE)) {
        if (canspotmon(mtmp))
            pline("%s выглядит очень сердитым.", Amonnam(mtmp));
        else
            You_feel("нарастающим напряжением.");
    ...
    if (is_dprince(mtmp->data) && mtmp->minvis) {
        ...
        if (wasunseen && canspotmon(mtmp)) {
            pline("%s появляется перед вами.", Amonnam(mtmp));
    ...
    if (gy.youmonst.data->mlet == S_DEMON) { /* Won't blackmail their own. */
        if (!Deaf)
            pline("%s говорит: \"Удачной охоты, %s.\"", Amonnam(mtmp),
                  flags.female ? "Сестра" : "Брат");
        else if (canseemon(mtmp))
            pline("%s что-то говорит.", Amonnam(mtmp));
    ...
    if (!demand || gm.multi < 0) { /* you have no gold or can't move */
        ...
        if (!Deaf)
            pline("%s требует %ld %s за безопасный проход.",
                  Amonnam(mtmp), demand, currency(demand));
        else if (canseemon(mtmp))
            pline("%s словно требует что-то.", Amonnam(mtmp));
        offer = 0L;
        if (!Deaf &&
            ((offer = bribe(mtmp, "Сколько вы предложите?")) >= demand)) {
            pline("%s исчезает, смеясь над трусливыми смертными.",
                  Amonnam(mtmp));
        } else if (offer > 0L
                   && (long) rnd(5 * ACURR(A_CHA)) > (demand - offer)) {
            pline("%s угрожающе хмурится на вас, затем исчезает.",
                  Amonnam(mtmp));
        } else {
            pline("%s злится...", Amonnam(mtmp));
            mtmp->mpeaceful = 0;
            set_malign(mtmp);
            return 0;
        }
    ...
    livelog_printf(LL_UMONST, "подкупили %s на %ld %s за беспрепятственный проход",
                   x_monnam(mtmp, ARTICLE_A, (char *) 0, EXACT_NAME, FALSE),
                   offer, (offer == 1L) ? "зоркмид" : "зоркмиды");
    mongone(mtmp);
    return 1;
}
...
bribe(struct monst *mtmp, const char *prompt)
{
    ...
    /*JAR880815 - */
    if (offer < 0L) {
        You("пытаетесь недодать сдачу %s, но спотыкаетесь.", mon_nam(mtmp));
        return 0L;
    } else if (offer == 0L) {
        You("отказываетесь.");
        return 0L;
    } else if (offer >= umoney) {
        You("отдаёте %s всё своё золото.", mon_nam(mtmp));
        offer = umoney;
    } else {
        You("даёте %s %ld %s.", mon_nam(mtmp), offer, currency(offer));
    }
    ...
    struct monst *mon) /* if Null, angel hasn't been created yet */
{
    ...
    if (mon) {
        if (canspotmon(mon)) {
            if (!Deaf) {
                pline("%s отчитывает вас, говоря:", Monnam(mon));
                SetVoice(mon, 0, 80, 0);
                verbalize("Раз ты жаждешь битвы, получи ещё!");
            } else {
                pline("%s исчезает!", Monnam(mon));
            }
        }
        mongone(mon);
    }
...
gain_guardian_angel(void)
{
    ...
    if (Conflict) {
       if (!Deaf)
            ...
            You_feel("громовой голос:");
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Твоё желание битвы будет исполнено!");
        ...
        if (!Deaf)
            ...
            You_feel("тихий голос:");
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Ты оказался достоин меня!");
        mm.x = u.ux;
        mm.y = u.uy;
        if (enexto(&mm, mm.x, mm.y, &mons[PM_ANGEL])
            ...
            newsym(mtmp->mx, mtmp->my);
            if (!Blind)
                pline("Рядом с вами появляется ангел.");
            else
                You_feel("присутствие доброго ангела рядом с вами.");
