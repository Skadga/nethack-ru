    ...
    if ((money_cnt(gi.invent) || hidden_gold(TRUE))
        ...
        if (grd->mpeaceful) {
            if (canspotmon(grd)) /* see or sense via telepathy */
                pline("%s приходит в ярость.", Monnam(grd));
...
invault(void)
{
    ...
    if (!guard) {
        ...
        if ((otmp = sobj_at(BOULDER, guard->mx, guard->my)) != 0) {
            ...
            (*func)("%s разлетаются вдребезги.",
                    (bcnt == 1) ? an(bname) : makeplural(bname));
        }
        spotted = canspotmon(guard);
        if (spotted) {
            pline("Внезапно входит один из %s Хранилища!",
                  makeplural(pmname(guard->data, Mgender(guard))));
            newsym(guard->mx, guard->my);
        } else {
            pline("Кто-то ещё вошёл в Хранилище.");
        ...
        if (u.uswallow) {
            ...
            if (!Deaf) {
                ...
                verbalize("Что здесь происходит?");
            }
            if (!spotted)
                pline("Другое присутствие исчезает.");
            mongone(guard);
            return;
        }
        if (U_AP_TYPE == M_AP_OBJECT || u.uundetected) {
            if (U_AP_TYPE == M_AP_OBJECT
                ...
                if (!Deaf) {
                    SetVoice(guard, 0, 80, 0);
                    verbalize("Эй!  Кто оставил здесь %s?",
                              mimic_obj_name(&gy.youmonst));
                }
            /* You're mimicking some object or you're hidden. */
            pline("В замешательстве %s разворачивается и уходит.", mhe(guard));
            mongone(guard);
            return;
        }
        if (Strangled || is_silent(gy.youmonst.data) || gm.multi < 0) {
            ...
            if (Deaf) {
                pline("%s фыркает и поворачивается, чтобы уйти.", noit_Monnam(guard));
            } else {
                SetVoice(guard, 0, 80, 0);
                verbalize("Я вернусь, когда ты будешь готов поговорить со мной!");
            }
            mongone(guard);
            return;
        }
        ...
        trycount = 5;
        do {
            getlin(Deaf ? "Вы должны назвать своё имя. -"
                        : "\"Здравствуй, незнакомец, кто ты?\" -", buf);
            (void) mungspaces(buf);
        ...
        if (!strcmpi(buf, "Croesus") || !strcmpi(buf, "Kroisos")
            ...
            if (!svm.mvitals[PM_CROESUS].died) {
                if (Deaf) {
                    if (!Blind)
                        pline("%s машет рукой на прощание.", noit_Monnam(guard));
                } else {
                    SetVoice(guard, 0, 80, 0);
                    verbalize(
                         "Ах да, конечно.  Извините, что побеспокоил вас.");
                }
                mongone(guard);
            } else {
                ...
                if (Deaf) {
                   if (!Blind)
                        pline("%s что-то беззвучно произносит и выглядит очень злым!",
                              noit_Monnam(guard));
                } else {
                   SetVoice(guard, 0, 80, 0);
                   verbalize(
                           "Вернулся из мёртвых?  Я это исправлю!");
                }
        ...
        if (Deaf) {
            pline("%s не %sузнаёт вас.", noit_Monnam(guard),
                    (Blind) ? "" : "кажется, ");
        } else {
            SetVoice(guard, 0, 80, 0);
            verbalize("Я вас не знаю.");
        }
        umoney = money_cnt(gi.invent);
        if (!umoney && !hidden_gold(TRUE)) {
            if (Deaf) {
                pline("%s топает%s.", noit_Monnam(guard),
                      (Blind) ? "" : " и манит");
            } else {
                SetVoice(guard, 0, 80, 0);
                verbalize("Прошу, следуйте за мной.");
            }
        } else {
            if (!umoney) {
                if (Deaf) {
                    if (!Blind)
                        pline("%s свирепо смотрит на вас%s.", noit_Monnam(guard),
                              gi.invent ? " и ваши вещи" : "");
                } else {
                   SetVoice(guard, 0, 80, 0);
                   verbalize("У вас спрятано золото.");
                }
            }
            if (Deaf) {
                if (!Blind)
                    pline(
                       "%s протягивает %s ладонь и манит %s другой рукой.",
                          noit_Monnam(guard), noit_mhis(guard),
                          noit_mhis(guard));
            } else {
                SetVoice(guard, 0, 80, 0);
                verbalize(
                    "Скорее всего, всё ваше золото было украдено из этого хранилища.");
                SetVoice(guard, 0, 80, 0);
                verbalize("Пожалуйста, бросьте это золото и следуйте за мной.");
            }
...
wallify_vault(struct monst *grd)
{
    ...
    if (movedgold || fixed) {
        if (in_fcorridor(grd, grd->mx, grd->my) || cansee(grd->mx, grd->my))
            pline("%s шепчет заклинание.", noit_Monnam(grd));
        else
            You_hear("далёкое песнопение.");
        if (movedgold)
            pline("Таинственная сила переносит золото в хранилище.");
        if (fixed)
            pline("Повреждённые стены хранилища магически восстанавливаются!");
    }
}
...
gd_mv_monaway(struct monst *grd, int nx, int ny)
{
    ...
    if (mtmp && mtmp != grd) {
        if (!Deaf) {
            SetVoice(grd, 0, 80, 0);
            verbalize("Прочь с дороги, отребье!");
        }
...
gd_pick_corridor_gold(struct monst *grd, int goldx, int goldy)
{
    ...
    if (see_it) { /* cansee(goldx, goldy) */
        pline("%s%s подбирает золото%s.", Some_Monnam(grd),
              (grd->mpeaceful && EGD(grd)->warncnt > 5)
                 ? " успокаивается и" : "",
              under_u ? " у вас из-под ног" : "");
    }
    ...
    boolean disappear_msg_seen)
{
    ...
    if (!semi_dead && (in_fcorridor(grd, u.ux, u.uy) || cansee(x, y))) {
        if (!disappear_msg_seen && see_guard)
            pline("Внезапно %s исчезает.", noit_mon_nam(grd));
        return 1;
    }
    return -2;
}
    ...
    if (!cansee(grd->mx, grd->my) || !mon_visible(grd))
        You_hear("%s.",
                    m_carrying(grd, TIN_WHISTLE)
                        ? "пронзительный звук свистка стража"
                        : "гневные крики");
    else
        You(um_dist(grd->mx, grd->my, 2)
                ? "видите, как приближается %s."
                : "сталкиваетесь лицом к лицу с %s.",
            /* "an angry guard" */
            x_monnam(grd, ARTICLE_A, "сердитый", 0, FALSE));
}
...
gd_move(struct monst *grd)
{
    ...
    if (egrd->witness) {
        if (!Deaf) {
            SetVoice(grd, 0, 80, 0);
            verbalize("Как ты смеешь %s это золото, негодяй!",
                      (egrd->witness & GD_EATGOLD) ? "пожирать" : "уничтожать");
        }
        egrd->witness = 0;
        grd->mpeaceful = 0;
        return -1;
    ...
    if (egrd->fcend == 1) {
        if (u_in_vault && (u_carry_gold || um_dist(grd->mx, grd->my, 1))) {
            if (egrd->warncnt == 3 && !Deaf) {
                ...
                Sprintf(buf, "%sследуйте за мной!",
                        u_carry_gold ? (!umoney ? "бросьте это спрятанное золото и "
                                                : "бросьте это золото и ")
                                     : "");
                SetVoice(grd, 0, 80, 0);
                if (egrd->dropgoldcnt || !u_carry_gold)
                    verbalize("Повторяю, %s", buf);
                else
            ...
            if (egrd->warncnt == 7) {
                ...
                if (!Deaf) {
                    SetVoice(grd, 0, 80, 0);
                    verbalize("Ты был предупреждён, негодник!");
                }
                grd->mpeaceful = 0;
    ...
    if (egrd->fcend > 1) {
        if (egrd->fcend > 2 && in_fcorridor(grd, grd->mx, grd->my)
            ...
            pline("%s, озадаченный, исчезает.", noit_Monnam(grd));
        ...
        if (u_carry_gold && (in_fcorridor(grd, u.ux, u.uy)
            ...
            if (egrd->warncnt < 6) {
                ...
                if (Deaf) {
                    if (!Blind)
                        pline("%s требовательно протягивает %s ладонь!",
                              noit_Monnam(grd), noit_mhis(grd));
                } else {
                    SetVoice(grd, 0, 80, 0);
                    verbalize("Брось всё своё золото, негодяй!");
                }
                return 0;
            } else {
                if (Deaf) {
                    if (!Blind)
                        pline("%s в яростном восторге потирает %s руки!",
                              noit_Monnam(grd), noit_mhis(grd));
                } else {
                    SetVoice(grd, 0, 80, 0);
                    verbalize("Будь по-твоему, негодник!");
                }
                grd->mpeaceful = 0;
                return -1;
            }
 ...
 proceed:
    ...
    if ((nx != ggx || ny != ggy) || (grd->mx != ggx || grd->my != ggy)) {
        ...
        if (!find_guard_dest(grd, &egrd->gdx, &egrd->gdy)
            ...
            pline("%s, озадаченный, исчезает.", Monnam(grd));
            return gd_move_cleanup(grd, semi_dead, TRUE);
        } else
            goto nextpos;
    }
 newpos:
    ...
    if (newspot && g_at(nx, ny)) {
        ...
        mpickgold(grd);
        if (canspotmon(grd))
            pline("%s подбирает немного золота.", Monnam(grd));
    } else
        newsym(grd->mx, grd->my);
    restfakecorr(grd);
    return 1;
...
paygd(boolean silently)
{
    ...
    if (u.uinvault) {
        if (!silently)
            Your("%ld %s уходят в Волшебное хранилище памяти.",
        ...
        mnexto(grd, RLOC_NOMSG);
        if (!silently)
            pline("%s возвращает ваше золото в хранилище.", Monnam(grd));
        gdx = svr.rooms[EGD(grd)->vroom].lx + rn2(2);
        gdy = svr.rooms[EGD(grd)->vroom].ly + rn2(2);
        Sprintf(buf, "Крезу: вот золото, изъятое у %s-%s.",
                svp.plname,
