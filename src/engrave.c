...
wipe_engr_at(coordxy x, coordxy y, xint16 cnt, boolean magical)
{
    ...
    if (ep && ep->engr_type != HEADSTONE && !ep->nowipeout) {
        debugpline1("запрошено стирание %d символов", cnt);
        if (ep->engr_type != BURN || is_ice(x, y) || (magical && !rn2(2))) {
            if (ep->engr_type != DUST && ep->engr_type != ENGR_BLOOD) {
                cnt = rn2(1 + 50 / (cnt + 1)) ? 0 : 1;
                debugpline1("реально стирается %d символов", cnt);
            }
...
{
    struct engr *ep = engr_at(x, y);
    const char *eloc = surface_ru(x, y);
    int sensed = 0;
    ...
    if (ep && ep->engr_txt[actual_text][0]) {
        ...
        case DUST:
            if (!Blind) {
                sensed = 1;
                pline("%s написано здесь в %s.", Something,
                      is_ice(x, y) ? "инее" : "пыли");
            }
            break;
        case ENGRAVE:
        case HEADSTONE:
            if (!Blind || can_reach_floor(TRUE)) {
                sensed = 1;
                pline("%s выгравировано здесь на %s.", Something, eloc);
            }
            break;
        case BURN:
            if (!Blind || can_reach_floor(TRUE)) {
                sensed = 1;
                pline("Некоторый текст был %s на %s здесь.",
                      is_ice(x, y) ? "растоплен" : "выжжен", eloc);
            }
            break;
        case MARK:
            if (!Blind) {
                sensed = 1;
                pline("Здесь есть граффити на %s.", eloc);
            }
            break;
        case ENGR_BLOOD:
            ...
            if (!Blind) {
                sensed = 1;
                You_see("сообщение, нацарапанное кровью здесь.");
            }
            break;
        default:
            impossible("%s написано весьма странным способом.", Something);
            sensed = 1;
        }
        ...
        if (sensed) {
            ...
            You("%s: \"%s\"%s", (Blind) ? "ощущаете слова" : "читаете", et,
                endpunct);
...
u_can_engrave(void)
{
    ...
    if (u.uswallow) {
        if (is_animal(u.ustuck->data)) {
            pline("Что вы собираетесь писать?  \"Здесь был Иона\"?");
            return FALSE;
        ...
        You_cant("писать на %s!", surface(u.ux, u.uy));
        return FALSE;
...
doengrave_sfx_item_WAN(struct _doengrave_ctx *de)
{
    ...
    case WAN_STRIKING:
        Strcpy(de->post_engr_text,
               "Палочка безуспешно сопротивляется твоей попытке написать!");
        break;
    case WAN_SLOW_MONSTER:
        if (!Blind) {
            Sprintf(de->post_engr_text, "Жучки на %s замедляются!",
                    surface(u.ux, u.uy));
        }
        break;
    case WAN_SPEED_MONSTER:
        if (!Blind) {
            Sprintf(de->post_engr_text, "Жучки на %s ускоряются!",
                    surface(u.ux, u.uy));
        }
        break;
    case WAN_POLYMORPH:
    ...
    case WAN_DEATH:
        if (!Blind) {
            Sprintf(de->post_engr_text, "Жучки на %s замирают!",
                    surface(u.ux, u.uy));
        }
        break;
    case WAN_COLD:
        if (!Blind)
            Strcpy(de->post_engr_text,
                   "Несколько кубиков льда падают из палочки.");
    ...
    case WAN_DIGGING:
        ...
        if (!objects[de->otmp->otyp].oc_name_known) {
            if (flags.verbose)
                pline("Это %s — палочка раскапывания!", xname(de->otmp));
            de->doknown = TRUE;
        }
        Strcpy(de->post_engr_text,
               (Blind && !Deaf)
               ? "Вы слышите сверление!"    /* Deaf-aware */
               : Blind
                  ? "Вы чувствуете толчки."
                  : IS_GRAVE(levl[u.ux][u.uy].typ)
                     ? "Щепки вылетают от надгробия."
                     : de->frosted
                        ? "Льдинки вылетают с ледяной поверхности!"
                        : (svl.level.locations[u.ux][u.uy].typ
                          == DRAWBRIDGE_DOWN)
                           ? "Щепки вылетают с моста."
                           : "Гравий вылетает с пола.");
        break;
        /* type = BURN wands */
    case WAN_FIRE:
        de->ptext = TRUE;
        ...
        if (!objects[de->otmp->otyp].oc_name_known) {
            if (flags.verbose)
                pline("Это %s — палочка огня!", xname(de->otmp));
            de->doknown = TRUE;
        }
        Strcpy(de->post_engr_text, Blind ? "Вы чувствуете, как палочка нагревается."
                                         : "Пламя вылетает из палочки.");
        break;
    case WAN_LIGHTNING:
        de->ptext = TRUE;
        de->type = BURN;
        if (!objects[de->otmp->otyp].oc_name_known) {
            if (flags.verbose)
                pline("Это %s — палочка молний!", xname(de->otmp));
            de->doknown = TRUE;
        }
        if (!Blind) {
            Strcpy(de->post_engr_text, "Молния бьёт из палочки.");
            de->doblind = TRUE;
        } else {
            Strcpy(de->post_engr_text, !Deaf
                   ? "Вы слышите треск!"     /* Deaf-aware */
                   : "У вас волосы встают дыбом!");
        }
        break;
        /* type = MARK wands */
...
doengrave_sfx_item(struct _doengrave_ctx *de)
{
    ...
    case BALL_CLASS:
    case ROCK_CLASS:
        You_cant("выгравировать таким большим предметом!");
        de->ptext = FALSE;
        break;
    ...
    case SCROLL_CLASS:
    case SPBOOK_CLASS:
        pline("%s покроется %s.", Yname2(de->otmp),
              de->frosted ? "весь инеем" : "слишком грязным");
        de->ptext = FALSE;
        break;
        ...
        if (zappable(de->otmp)) {
            ...
            if (can_reach_floor(TRUE)) {
                ...
                /* empty wand just doesn't write */
                    ...
                    pline("Палочка слишком изношена, чтобы гравировать.");
            }
        }
        break;
    ...
    case WEAPON_CLASS:
        if (is_art(de->otmp, ART_FIRE_BRAND)) {
            ...
            if (welded(de->otmp))
                pline("%s может лишь поцарапать %s.",
                      Yname2(de->otmp), surface(u.ux, u.uy));
            else if ((int) de->otmp->spe <= -3)
                pline("%s слишком тупой для гравировки.",
    ...
    case TOOL_CLASS:
        if (de->otmp == ublindf) {
            pline(
                "Это немного сложно, чтобы гравировать этим, не думаете?");
            de->ret = ECMD_FAIL;
            return FALSE;
        }
        switch (de->otmp->otyp) {
        ...
        case TOWEL:
            ...
            if (de->oep) {
                if (de->oep->engr_type == DUST
                    ...
                    if (!Blind)
                        You("стираете послание здесь.");
                    else
                        pline("%s %s.", Yobjnam2(de->otmp, "get"),
                              de->frosted ? "в инее" : "в пыли");
                    de->dengr = TRUE;
                } else {
                    pline("%s не может стереть эту надпись.",
                          Yname2(de->otmp));
                }
            } else {
                pline("%s %s.", Yobjnam2(de->otmp, "get"),
    ...
    case VENOM_CLASS:
        ...
        pline("Пишете ядовитое письмо пером?");
        break;

    case ILLOBJ_CLASS:
        impossible("Вы гравируете запрещённым предметом!");
        break;
    }
...
doengrave_ctx_verb(struct _doengrave_ctx *de)
{
    switch (de->type) {
    default:
        de->everb = de->adding ? "дописать к странной надписи на"
                               : "странно писать на";
        break;
    case DUST:
        de->everb = de->adding ? "дописать к надписи в" : "писать в";
        de->eloc = de->frosted ? "инея" : "пыль";
        break;
    case HEADSTONE:
        de->everb = de->adding ? "дописать к эпитафии на" : "выгравировать на";
        break;
    case ENGRAVE:
        de->everb = de->adding ? "дописать к надписи в" : "гравировать в";
        break;
    case BURN:
        de->everb = de->adding ? (de->frosted ? "дописать к тексту, впаянному в"
                                  : "дописать к тексту, выжженному в")
                       : (de->frosted ? "впаять в" : "выжечь в");
        break;
    case MARK:
        de->everb = de->adding ? "дописать к граффити на" : "нацарапать на";
        break;
    case ENGR_BLOOD:
        de->everb = de->adding ? "дописать к каракулям на" : "нацарапать на";
        break;
    }
}
...
doengrave(void)
{
    ...
    de->otmp = getobj("писать чем", stylus_ok, GETOBJ_PROMPT);
    ...
    if (!freehand() && de->otmp != uwep && !de->otmp->owornmask) {
        pline("У вас нет свободной %s, чтобы писать!", body_part(HAND));
        goto doengr_exit;
    }

    if (de->jello) {
        You("щекочете %s с помощью %s.", mon_nam(u.ustuck), de->writer);
        Your("послание растворяется...");
        goto doengr_exit;
    }
    if (!can_reach_floor(TRUE)) {
        if (de->otmp->oclass != WAND_CLASS) {
            ...
            You("жестикулируете своей палочкой в сторону %s под вами.",
                surface(u.ux, u.uy));
    ...
    if (IS_ALTAR(levl[u.ux][u.uy].typ)) {
        if (!initial_msg_given)
            You("делаете движение в сторону алтаря %s.", de->writer);
        altar_wrath(u.ux, u.uy);
        goto doengr_exit;
    }
    if (IS_GRAVE(levl[u.ux][u.uy].typ)) {
        if (de->otmp == &hands_obj) { /* using only finger */
            You("лишь сделаете маленькое пятно на %s.",
                surface(u.ux, u.uy));
            goto doengr_exit;
    ...
    if (*de->buf) {
        ...
        if (!Blind) {
            if (tmp_ep != 0) {
                pline("Надпись теперь гласит: \"%s\".", de->buf);
                tmp_ep->eread = 1;
                tmp_ep->erevealed = 1;
                de->disprefresh = TRUE;
            }
    ...
    if (de->zapwand && (de->otmp->spe < 0)) {
        pline("%s %sпревращается в пыль.", The(xname(de->otmp)),
              Blind ? "" : "ярко вспыхивает, а затем ");
        if (!IS_GRAVE(levl[u.ux][u.uy].typ))
            You(
    "вам не удастся ничего добиться, пытаясь писать в %s своей пылью.",
                de->frosted ? "инея" : "пыль");
        useup(de->otmp);
    ...
    if (de->oep) {
        ...
        if (de->type == HEADSTONE) {
            ...
            c = yn_function("Хотите добавить к текущей надписи?",
                            ynqchars, 'y', TRUE);
            if (c == 'q') {
                pline1(Never_mind);
                goto doengr_exit;
        ...
        if (c == 'n' || Blind) {
            if (de->oep->engr_type == DUST
                ...
                || de->oep->engr_type == MARK) {
                if (!Blind) {
                    You("стираете послание, которое было %s здесь.",
                    ...
                    de->eow = TRUE;
                       ...
                       || de->type == ENGR_BLOOD) {
                You("не можете стереть послание, которое %s %s здесь.",
                    (de->oep->engr_type == BURN)
                        ? (de->frosted ? "впаяно в" : "выжжено в")
                        : "выгравировано в",
                    surface(u.ux, u.uy));
                de->ret = ECMD_TIME;
                goto doengr_exit;
                ...
                if (!Blind || can_reach_floor(TRUE))
                    You("перезапишете текущее послание.");
                de->eow = TRUE;
            }
        } else if (de->oep
                   && Strlen(de->oep->engr_txt[actual_text]) >= BUFSZ - 1) {
            There("здесь нет места, чтобы добавить что-то ещё.");
            de->ret = ECMD_TIME;
            goto doengr_exit;
        }
    }
    ...
    Sprintf(de->qbuf, "Что вы хотите %s на %s здесь?",
            de->everb, de->eloc);
    ...
    if (de->len == 0 || strchr(de->ebuf, '\033')) {
        if (de->zapwand) {
            if (!Blind)
                pline("%s, затем %s.", Tobjnam(de->otmp, "светится"),
                      otense(de->otmp, "гаснет"));
            de->ret = ECMD_TIME;
            goto doengr_exit;
        } else {
            pline1(Never_mind);
    ...
    if (de->len != 1 || (!strchr(de->ebuf, 'x') && !strchr(de->ebuf, 'X')))
        if (!u.uconduct.literate++)
            livelog_printf(LL_CONDUCT, "стал грамотным, выгравировав \"%s\"",
                           de->ebuf);
...
engrave(void)
{
    ...
    if (svc.context.engraving.pos.x != u.ux
        ...
        You("не можете продолжить гравировку.");
        return 0;
    }
    ...
    if (dulling_wep && !is_blade(stylus)) {
        impossible("гравировка не лезвийным оружием");
    } else if (svc.context.engraving.type == MARK && !marker) {
        impossible("граффити не маркером");
    }
    ...
    if (dulling_wep) {
        ...
        if (stylus->quan > 1L) {
            if (firsttime)
                pline("Один из %s тупеет.", yname(stylus));
        ...
        if (splitstack) {
            ...
            stylus = hold_another_object(stylus, "Вы роняете один %s!",
        ...
        if (stylus->spe < ink_cost) {
            impossible("слишком сухой маркер годен для граффити?");
            ink_cost = stylus->spe;
            truncate = TRUE;
        }
        stylus->spe -= ink_cost;
    ...
    default:
        finishverb = "твою странную надпись";
        break;
    case DUST:
        finishverb = is_ice(u.ux, u.uy) ? "писание в инее"
                     : "писание в пыли";
        break;
    case HEADSTONE:
    case ENGRAVE:
        finishverb = "гравировку";
        break;
    case BURN:
        finishverb = is_ice(u.ux, u.uy) ? "вплавление твоего послания в лёд"
                     : "выжигание твоего послания в пол";
        break;
    case MARK:
        finishverb = "портит подземелье";
        break;
    case ENGR_BLOOD:
        finishverb = "царапать";
    }
    ...
    if (endc - svc.context.engraving.nextc > space_left) {
        You("закончилось место, чтобы писать.");
    ...
    if (truncate && *endc != '\0') {
        *endc = '\0';
        You("смогли написать лишь \"%s\".", svc.context.engraving.text);
    } else {
...
engraving_sanity_check(void)
{
    ...
    if (head_engr && (Is_airlevel(&u.uz) || Is_waterlevel(&u.uz))) {
        impossible("целостность надписей: на плане воздуха/воды");
        return;
    }
    ...
    for (ep = head_engr; ep; ep = ep->nxt_engr) {
        ...
        if (!isok(x, y)) {
            impossible("целостность надписей: !isok <%i,%i>", x, y);
            continue;
        }
        levtyp = SURFACE_AT(x, y);
        if (is_pool_or_lava(x, y) || IS_AIR(levtyp) || !ACCESSIBLE(levtyp)) {
            impossible("целостность надписей: недопустимая поверхность (%d: \"%s\")",
                       levtyp, surface(x, y));
            continue;
        }
    }
...
disturb_grave(coordxy x, coordxy y)
{
    ...
    if (!IS_GRAVE(lev->typ)) {
        impossible("Тревожите могилу, которая не могила? (%d)", lev->typ);
    } else if (lev->disturbed) {
        impossible("Уже потревоженную могилу тревожите?");
    } else {
        You("тревожите нежить!");
        lev->disturbed = 1;
