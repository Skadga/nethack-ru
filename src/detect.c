...
gold_detect(struct obj *sobj)
{
    ...
    if (!gk.known) {
        ...
        if (gy.youmonst.data == &mons[PM_GOLD_GOLEM])
            Sprintf(buf, "Вы чувствуете себя на миллион %s!", currency(2L));
        else if (money_cnt(gi.invent) || hidden_gold(TRUE))
            Strcpy(buf,
               "Вы чувствуете беспокойство о своём будущем финансовом положении.");
        else if (steedgold)
            Sprintf(buf, "Вы чувствуете интерес к финансовому положению %s.",
            ...
            Strcpy(buf, "Вы чувствуете себя материально бедным.");
    ...
    if (stale)
        docrt();
    You("замечаете немного золота между вашими %s.", makeplural(body_part(FOOT)));
    return 0;
 ...
 outgoldmap:
    ...
    You_feel("очень жадным и ощущаете золото!");
    exercise(A_WIS, TRUE);

    browse_map(ter_typ, "золото");
...
food_detect(struct obj *sobj)
{
    ...
    const char *what = confused ? something : "еду";
    ...
    if (!ct && !ctu) {
        ...
        if (stale) {
            docrt();
            You("чувствуете нехватку %s поблизости.", what);
            if (sobj && sobj->blessed) {
                if (!u.uedibility)
                    Your("%s начинает покалывать.", body_part(NOSE));
                u.uedibility = 1;
            }
        } else if (sobj) {
            char buf[BUFSZ];

            Sprintf(buf, "Ваш %s дёргается%s.", body_part(NOSE),
        ...
        gk.known = TRUE;
        You("%s %s поблизости.", sobj ? "чуете" : "ощущаете", what);
        if (sobj && sobj->blessed) {
            if (!u.uedibility)
                Your("%s начинает покалывать.", body_part(NOSE));
            u.uedibility = 1;
        ...
        if (sobj) {
            if (sobj->blessed) {
                Your("%s %s покалывать, и вы чуете %s.", body_part(NOSE),
                     u.uedibility ? "продолжает" : "начинает", what);
                u.uedibility = 1;
            } else
                Your("%s покалывает, и вы чуете %s.", body_part(NOSE), what);
        } else
            You("ощущаете %s.", what);
        exercise(A_WIS, TRUE);

        browse_map(ter_typ, "еда");
              ...
              int class)            /* an object class, 0 for all */
{
    ...
    if (class < 0 || class >= MAXOCLASSES) {
        impossible("object_detect:  недопустимый класс %d", class);
        class = 0;
    }
    ...
    if (Hallucination || (Confusion && class == SCROLL_CLASS))
        ...
        Strcpy(stuff, class ? def_oc_syms[class].name : "предметы");
    if (boulder && class != ROCK_CLASS)
        Strcat(stuff, " и/или большие камни");
    ...
    if (!clear_stale_map(!class ? ALL_CLASSES : class, 0) && !ct) {
        if (!ctu) {
            if (detector)
                strange_feeling(detector, "Вы чувствуете, что чего-то не хватает.");
            return 1;
        }
        You("ощущаете %s поблизости.", stuff);
        return 0;
    }
    ...
    You("обнаруживаете %s %s.", ct ? "присутствие" : "отсутствие", stuff);

    if (!ct)
        display_nhwindow(WIN_MAP, TRUE);
    else
        browse_map(ter_typ, "предмет");
               ...
               int mclass)       /* monster class, 0 for all */
{
    ...
    if (!mcnt) {
        ...
        if (!swallowed)
            display_self();
        You("ощущаете присутствие монстров.");
        if (woken)
            pline("Монстры ощущают ваше присутствие.");
        ...
        if ((otmp && otmp->blessed) && !unconstrained) {
            ...
            browse_map(TER_DETECT | TER_MON, "интересующий монстр");
            EDetect_monsters &= ~I_SPECIAL;
        }
...
display_trap_map(int cursed_src)
{
    ...
    You_feel("%s.", cursed_src ? "очень жадным" : "пойманным");

    browse_map(ter_typ, cursed_src ? "золото" : "интересующая ловушка");
    ...
    struct obj *sobj) /* Null if crystal ball, scroll if gold detection */
{
    ...
    if (!found) {
        ...
        Sprintf(buf, "Ваши %s перестают чесаться.", makeplural(body_part(TOE)));
        strange_feeling(sobj, buf);
        return 1;
    }
    /* traps exist, but only under me - no separate display required */
    Your("%s чешутся.", makeplural(body_part(TOE)));
    return 0;
}
...
furniture_detect(void)
{
    ...
    if (!found)
        There("похоже, на этом уровне нет ничего интересного.");
    else if (!revealed)
        /* [what about clipped map with points of interest outside of the
            currently shown area?] */
        Your("карта уже показывает все нужные места.");
...
use_crystal_ball(struct obj **optr)
{
    ...
    if (Blind) {
        pline("Жаль, вы не можете видеть %s.", the(xname(obj)));
        return;
    }
    ...
    if (charged && (obj->cursed || rnd(oops) > ACURR(A_INT))) {
        ...
        case 1:
            pline("%s слишком сложно для понимания!", Tobjnam(obj, "are"));
            break;
        case 2:
            pline("%s вас!", Tobjnam(obj, "confuse"));
            make_confused((HConfusion & TIMEOUT) + impair, FALSE);
            break;
        case 3:
            if (!resists_blnd(&gy.youmonst)) {
                pline("%s ваше зрение!", Tobjnam(obj, "damage"));
                make_blinded(BlindedTimeout + impair, FALSE);
                if (!Blind)
                    Your1(vision_clears);
            } else {
                pline("%s ваше зрение.", Tobjnam(obj, "assault"));
                You("не затронуты!");
            }
            break;
        case 4:
            pline("%s ваш разум!", Tobjnam(obj, "zap"));
        ...
        case 5:
            ...
            *optr = obj = 0; /* it's gone */
            ...
            losehp(Maybe_Half_Phys(rnd(30)), "взрыв хрустального шара",
                   KILLED_BY_AN);
            break;
        }
        if (obj)
    ...
    if (Hallucination) {
        ...
        gm.multi_reason = "глядя в Магический шар 8 (tm)";
        gn.nomovemsg = "";

        if (!charged) {
            pline("Всё, что вы видите — это странное %s марево.", hcolor((char *) 0));
            if (obj->spe < 0)
                goto implode; /* destroy it when it has been cancelled */
        } else {
            switch (rnd(6)) {
            case 1:
                You("понимаете несколько клёвых комков раскалённой лавы.");
                break;
            case 2:
                pline("Ого!  Психоделические цвета, %s!",
                      poly_gender() == 1 ? "красотка" : "чувак");
                break;
            case 3:
                pline("Кристалл пульсирует зловещим %s светом!",
                          hcolor((char *) 0));
                break;
            case 4:
                You_see("золотые рыбки плавают над светящимися скалами.");
                break;
            case 5:
                You_see(
                    "крошечные снежинки кружатся вокруг крошечного фермерского домика.");
                break;
            default:
                pline("Ого... как калейдоскоп!");
                break;
            }
            consume_obj_charge(obj, TRUE);
        }
    ...
    if (flags.verbose)
        You("можете искать предмет, монстра или особый символ на карте.");
    ch = yn_function("Что вы ищете?", (char *) 0, '\0', TRUE);
    ...
    You("вглядываетесь в %s...", the(xname(obj)));
    nomul(-rnd(charged ? 10 : 2));
    gm.multi_reason = "глядя в хрустальный шар";
    gn.nomovemsg = "";

    if (!charged) {
        pline("Видение неясно.");
 ...
 implode:   /* no damage to hero but 'multi' has a small negative value */
        ...
        if (ret) {
            if (!rn2(100)) /* make them nervous */
                You_see("Маг Йендора смотрит на вас.");
            else
                pline("Видение неясно.");
        }
    }
    ...
    struct obj *sobj) /* scroll--actually fake spellbook--object */
{
    ...
    if (!svl.level.flags.hero_memory || unconstrained
        ...
        You("ощущаете своё окружение.");
        if (extended || glyph_is_monster(glyph_at(u.ux, u.uy)))
            ter_typ |= TER_MON;
        browse_map(ter_typ, "что-либо интересное");
        refresh = TRUE;
    }
    reconstrain_map();
    ...
    if (levl[zx][zy].typ == SDOOR
        ...
        if (levl[zx][zy].doormask & D_TRAPPED) {
            if (distu(zx, zy) < 3)
                ...
                Norep("Вы %s взрыв!",
                      cansee(zx, zy) ? "видите" : (!Deaf ? "слышите"
                                                      : "ощущаете сотрясение"));
...
findit(void)
{
    ...
    if (found.num_sdoors) {
        if (found.num_sdoors > 1)
            Sprintf(eos(buf), "%d потайных дверей", found.num_sdoors);
        else
            Strcat(buf, "потайная дверь");
    ...
    if (found.num_scorrs) {
        ...
        if (found.num_scorrs > 1)
            Sprintf(eos(buf), "%d потайных коридоров", found.num_scorrs);
        else
            Strcat(buf, "потайной коридор");
        num += found.num_scorrs;
    }
    if (found.num_traps) {
        if (*buf) /* "doors, corrs, and traps" or "{doors|corrs} and traps"
                   * or "..., traps ..." */
            Strcat(buf, (k == 3 && !found.num_mons) ? ", и "
                        : (k == 2) ? " и " : ", ");
        if (found.num_traps > 1)
            Sprintf(eos(buf), "%d ловушек", found.num_traps);
        else
            Strcat(buf, "ловушка");
        num += found.num_traps;
    }
    ...
    if (found.num_mons) {
        if (*buf)
            Strcat(buf, (k > 2) ? ", и " : " и ");
        if (found.num_mons > 1)
            Sprintf(eos(buf), "%d скрытых монстров", found.num_mons);
        else
            Strcat(buf, "скрытый монстр");
        num += found.num_mons;
    }
    if (*buf)
        You("раскрываете %s!", buf);

    if (found.num_invis) {
        if (found.num_invis > 1)
            Sprintf(buf, "%d%s невидимых монстров", found.num_invis,
                    found.num_kept_invis ? " других" : "");
        else
            Sprintf(buf, "%sневидимый монстр",
                    found.num_kept_invis ? "ещё один " : "");
        You("обнаруживаете %s!", buf);
        num += found.num_invis;
    }
    ...
    if (found.num_cleared_invis) {
        ...
        if (!num)
            You_feel("%s менее параноидальным.",
                     found.num_kept_invis ? "чуть-чуть " : "");
        num += found.num_cleared_invis;
    }
    /* note: num_kept_invis is not included in the final result */

    if (!num)
        You("ничего не находите.");
#if FOUND_FLASH_COUNT == 0
    else if (tmp_num) {
        flush_screen(1);
        display_nhwindow(WIN_MAP, TRUE);
...
openit(void)
{
    ...
    if (u.uswallow) {
        if (digests(u.ustuck->data)) {
            /* purple worm */
            if (Blind)
                pline("Его пасть открывается!");
            else
                pline("%s открывает пасть!", Monnam(u.ustuck));
#if 0   /* expels() will take care of this */
        } else if (enfolds(u.ustuck->data)) {
            /* trapper or lurker above */
            pline("%s разворачивается!", Monnam(u.ustuck));
#endif
        }
...
find_trap(struct trap *trap)
{
    ...
    You("находите %s.", an(trapname(trap->ttyp, FALSE)));
...
mfind0(struct monst *mtmp, boolean via_warning)
{
    ...
    if (M_AP_TYPE(mtmp)) {
        ...
        if (mtmp->mundetected && (is_hider(mtmp->data)
            ...
            if (via_warning && found_something) {
                set_msg_xy(x, y);
                Your("чувство опасности заставляет вас сделать второй %s.",
                     Blind ? "осмотр поблизости" : "осмотр рядом");
    ...
    if (found_something) {
        ...
        if (!canspotmon(mtmp)) {
            map_invisible(x, y);
            set_msg_xy(x, y);
            You_feel("невидимого монстра!");
        } else if (!sensemon(mtmp)) {
            set_msg_xy(x, y);
            You("находите %s.", mtmp->mtame ? y_monnam(mtmp) : a_monnam(mtmp));
        }
        return 1;
    }
    return 0;
...
dosearch(void)
{
    if (cmd_safety_prevention("Поиск", "ещё один поиск",
                          "Вы уже нашли монстра.",
    ...
    unsigned which_subset) /* TER_TRP | TER_OBJ | TER_MON | TER_FULL */
{
    ...
    if ((Hallucination || Stunned || Confusion) && !full) {
        You("слишком дезориентированы для этого.");
    } else {
        coordxy x, y;
        ...
        flush_screen(1);
        if (full) {
            Strcpy(buf, "реальная местность");
        } else {
            Strcpy(buf, "известная местность");
            if (keep_traps)
                Sprintf(eos(buf), "%s ловушек",
                        (keep_objs || keep_mons) ? "," : " и");
            if (keep_objs)
                Sprintf(eos(buf), "%s%s объектов",
                        (keep_traps || keep_mons) ? "," : "",
                        keep_mons ? "" : " и");
            if (keep_mons)
                Sprintf(eos(buf), "%s и монстров",
                        (keep_traps || keep_objs) ? "," : "");
        }
        pline("Показываю только %s...", buf);
        ...
        browse_map(which_subset, "что-либо интересное");
