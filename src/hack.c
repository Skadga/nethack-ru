    ...
    boolean costly)
{
...

    {
        ...
        if (!u.usteed) {
            ...
            if (givemesg)
                pline("%s вы двигаете %s.",
                      easypush ? "Без усилий"
                                 : "С большим трудом", what);
            if (!easypush)
                exercise(A_STR, TRUE);
        } else {
            if (givemesg)
                pline("%s двигает %s.", YMonnam(u.usteed), what);
        }
...
cannot_push_msg(struct obj *otmp, coordxy sx, coordxy sy)
{
    ...
    what = the(xname(otmp));
    if (u.usteed)
        pline("%s пытается сдвинуть %s, но не может.",
              YMonnam(u.usteed), what);
    else
        You("пытаетесь сдвинуть %s, но безуспешно.", what);
    if (Blind)
        feel_location(sx, sy);
}
...
cannot_push(struct obj *otmp, coordxy sx, coordxy sy)
{
    if (throws_rocks(gy.youmonst.data)) {
        ...
        if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
            You("недостаточно опытны, чтобы %s %s из %s.",
                willpickup ? "поднять" : "отодвинуть",
             ...
             */
            pline("Однако вы %s%s.",
                  willpickup ? "легко поднимаете его"
                             : "перебираетесь через него",
                  (canpickup && !willpickup)
                             ? " и могли бы поднять его"
                             : "");
    ...
    if (could_move_onto_boulder(sx, sy)) {
        pline(
           "Однако вы можете протиснуться в небольшое отверстие.");
        sokoban_guilt();
        return 0;
    } else {
        return -1;
...
{
    if (u.usteed)
        pline("%s толкает %s, и тот внезапно исчезает!",
                YMonnam(u.usteed), the(xname(otmp)));
    else
        You("толкаете %s, и тот внезапно исчезает!",
            the(xname(otmp)));
    ...
    while ((otmp = sobj_at(BOULDER, sx, sy)) != 0) {
        ...
        if (svc.context.nopick) {
            ...
            if (throws_rocks(gy.youmonst.data)) {
                ...
                You("%s по валуну здесь.", u_locomotion("step"));
                /* ["over" seems weird on air level but what else to say?] */
                sokoban_guilt();
                res = 0; /* move to <sx,sy> */
            } else if (could_move_onto_boulder(sx, sy)) {
                You("протискиваетесь %s валун.",
                    Flying ? "над" : "об");
                sokoban_guilt();
                res = 0; /* move to <sx,sy> */
            } else {
                There("валун на вашем пути.");
        ...
        if (Levitation || Is_airlevel(&u.uz)) {
            ...
            if (Blind)
                feel_location(sx, sy);
            You("не хватает рычагов, чтобы сдвинуть %s.", the(xname(otmp)));
        ...
        if (verysmall(gy.youmonst.data) && !u.usteed) {
            if (Blind)
                feel_location(sx, sy);
            pline("Вы слишком малы, чтобы сдвинуть этот %s.", xname(otmp));
        ...
        if (isok(rx, ry) && !IS_OBSTRUCTED(levl[rx][ry].typ)
            ...
            if (Sokoban && u.dx && u.dy) {
                if (Blind)
                    feel_location(sx, sy);
                pline("%s не покатится по диагонали по этому %s.",
                      The(xname(otmp)), surface(sx, sy));
                return cannot_push(otmp, sx, sy);
            }

            if (revive_nasty(rx, ry,
                             "Вы чувствуете движение с той стороны.")) {
                return -1;
            }
            ...
            if (mtmp && !noncorporeal(mtmp->data)
                ...
                if (Blind)
                    feel_location(sx, sy);
                if (canspotmon(mtmp)) {
                    pline("С той стороны %s.", a_monnam(mtmp));
                    deliver_part1 = TRUE;
                } else {
                    Soundeffect(se_monster_behind_boulder, 50);
                    You_hear("монстр за %s.", the(xname(otmp)));
                    if (!Deaf)
                        deliver_part1 = TRUE;
                    map_invisible(rx, ry);
                }
                if (flags.verbose) {
                    ...
                    pline("%s%s не может сдвинуть %s.",
                          deliver_part1 ? "Возможно, поэтому " : "",
                          deliver_part1 ? you_or_steed
                                        : upstart(you_or_steed),
                          deliver_part1 ? "его" : the(xname(otmp)));
                }
                return cannot_push(otmp, sx, sy);
            }
            ...
            if (ttmp) {
                ...
                case TRAPDOOR:
                    ...
                    if (Blind)
                        pline("Бульк! Вы больше не чувствуете %s.",
                              the(xname(otmp)));
                    else
...
still_chewing(coordxy x, coordxy y)
{
    ...
    if (!boulder
        ...
        You("слишком сыты, чтобы съесть решётку.");
        nomul(0);
        return 1;
        ...
        if (flags.verbose)
            You("%s %s.",
                svc.context.digging.chew ? "продолжаете грызть" : "начинаете грызть",
                boulder
                    ? "валун"
                    : IS_TREE(lev->typ)
                        ? "дерево"
    ...
    if (!u.uconduct.food++)
        livelog_printf(LL_CONDUCT,
                       "впервые поел, прогрызя %s",
                       boulder ? "валун"
                       : IS_TREE(lev->typ) ? "дерево"
                         : IS_OBSTRUCTED(lev->typ) ? "камень"
                           : (lev->typ == IRONBARS) ? "железные прутья"
                             : "дверь");
    u.uhunger += rnd(20);

    if (boulder) {
        delobj(boulder);         /* boulder goes bye-bye */
        You("съедаете валун."); /* yum */
        ...
        digtxt = "прогрызаете дыру в стене.";
        ...
        if (lev->doormask & D_TRAPPED) {
            ...
            digtxt = "прогрызаете потайную дверь.";
        ...
        if (lev->doormask & D_TRAPPED) {
            ...
            digtxt = "прогрызаете дверь.";
            lev->doormask = D_BROKEN;
        }

    } else { /* STONE or SCORR */
        digtxt = "прогрызаете проход в скале.";
        lev->typ = CORR;
    }
...
dosinkfall(void)
{
    static const char fell_on_sink[] = "упал на раковину";
    struct obj *obj;
    int dmg;
    ...
    if (!ufall) {
        You((innate_lev || blockd_lev) ? "нетвёрдо шатаетесь мгновение."
                                       : "восстанавливаете контроль над полётом.");
    } else {
    ...
    int mode)
{
    ...
    if (IS_OBSTRUCTED(tmpr->typ) || tmpr->typ == IRONBARS) {
        ...
        if (Passes_walls && may_passwall(x, y)) {
            ...
            if (!(Passes_walls || passes_bars(gy.youmonst.data))) {
                if (mode == DO_MOVE && flags.mention_walls)
                    You("не можете пройти сквозь решётку.");
                return FALSE;
            }
            ...
            if (mode == DO_MOVE) {
                if (is_db_wall(x, y)) {
                    ...
                    if (sym == S_stone)
                        Strcpy(buf, "сплошной камень");
                    else if (sym >= 0)
                        Strcpy(buf, an(defsyms[sym].explanation));
                    else
                        Sprintf(buf, "невозможно [фоновый глиф=%d]",
                                glyph);
                    pline_dir(xytodir(dx, dy), "Это %s.", buf);
                }
            }
            return FALSE;
        }
        ...
        if (closed_door(x, y)) {
            ...
            if (Passes_walls) {
                ...
                if (mode == DO_MOVE)
                    You("просачиваетесь под дверь.");
            } else if (Underwater) {
                if (mode == DO_MOVE)
                    pline("Там препятствие.");
                return FALSE;
            } else if (tunnels(gy.youmonst.data)
                       && !needspick(gy.youmonst.data)) {
                /* Eat the door. */
                ...
                if (mode == DO_MOVE) {
                    if (amorphous(gy.youmonst.data))
                        You("пытаетесь просочиться под дверь,"
                            " но ваши вещи не проходят.");
                    if (flags.autoopen && !svc.context.run
                        ...
                        if (Blind || Stunned || ACURR(A_DEX) < 10
                            || Fumbling) {
                            if (u.usteed) {
                                You_cant("провести %s через ту закрытую дверь.",
                                         y_monnam(u.usteed));
                            } else {
                                pline("Ой! Вы ударяетесь о дверь.");
                                exercise(A_DEX, FALSE);
                            }
 ...
 testdiag:
            if (dx && dy && !Passes_walls
                ...
                if (mode == DO_MOVE) {
                    ...
                    if (Underwater || flags.mention_walls)
                        You_cant("двигаться по диагонали в целый дверной проём.");
                }
                return FALSE;
            }
        }
    ...
    if (dx && dy && bad_rock(gy.youmonst.data, ux, y)
        ...
        case 3:
            if (mode == DO_MOVE)
                You("не можете пройти туда.");
            return FALSE;
        case 2:
            if (mode == DO_MOVE)
                You("несёте слишком много, чтобы пролезть.");
            return FALSE;
        case 1:
            if (mode == DO_MOVE)
                Your("тело слишком велико, чтобы пролезть.");
            return FALSE;
        default:
        ...
        if (mode == DO_MOVE)
            pline("%s преграждает вам путь.", YMonnam(m_at(ux, y)));
        return FALSE;
    }
    ...
    if (dx && dy && !Passes_walls && IS_DOOR(ust->typ)
        ...
        if (mode == DO_MOVE && flags.mention_walls)
            You_cant("двигаться по диагонали из целого дверного проёма.");
        return FALSE;
    }
    ...
    if (sobj_at(BOULDER, x, y) && (Sokoban || !Passes_walls)) {
        if (mode != TEST_TRAV && svc.context.run >= 2
            ...
            if (mode == DO_MOVE && flags.mention_walls)
                pline_dir(xytodir(dx,dy), "Валун преграждает вам путь.");
            return FALSE;
        }
        if (mode == DO_MOVE) {
                        ...
                        if (nx == ux && ny == uy) {
                            if (mode == TRAVP_TRAVEL || mode == TRAVP_VALID) {
                                ...
                                if (mode == TRAVP_TRAVEL
                                    ...
                                    svc.context.run = 8;
                                    if (visited)
                                        You("останавливаетесь, не зная, куда идти.");
                                    else
                                        iflags.travelcc.x
                                        = iflags.travelcc.y = 0;
                                }
    ...
    struct trap *desttrap) /* nonnull if another trap at <x,y> */
{
    ...
    case TT_BEARTRAP:
        if (flags.verbose) {
            predicament = "попали в медвежий капкан";
            if (u.usteed)
                Norep("%s %s.", upstart(steedname), predicament);
            else
                Norep("Вы %s.", predicament);
        }
    ...
    case TT_WEB:
        if (u_wield_art(ART_STING)) {
            ...
            pline("Жало рассекает паутину!");
            break;
        }
        if (--u.utrap) {
            if (flags.verbose) {
                predicament = "прилипли к паутине";
                if (u.usteed)
                    Norep("%s %s.", upstart(steedname), predicament);
                else
                    Norep("Вы %s.", predicament);
            ...
            if (u.usteed)
                pline("%s вырывается из паутины.", upstart(steedname));
            else
                You("высвобождаетесь.");
        }
        break;
    case TT_LAVA:
        ...
        if (!is_lava(x, y)) {
            ...
            if ((u.utrap & 0xff) == 0) {
                u.utrap = 0;
                if (u.usteed)
                    You("выводите %s на край %s.", steedname,
                        hliquid("lava"));
                else
                    You("подтягиваетесь к краю %s.",
                        hliquid("lava"));
            }
        }
        u.umoved = TRUE;
    ...
    case TT_BURIEDBALL:
        ...
        if (anchored) {
            ...
            if (buried_ball(&cc) && dist2(x, y, cc.x, cc.y) <= 2) {
                ...
                if (flags.verbose)
                    Norep("Вы двигаетесь в пределах досягаемости цепи.");
                return TRUE;
            }
        }
        if (--u.utrap) {
            if (flags.verbose) {
                if (anchored) {
                    predicament = "прикованы к";
                    culprit = "закопанному шару";
                } else {
                    predicament = "застряли в";
                    culprit = surface(u.ux, u.uy);
                }
                if (u.usteed) {
                    if (anchored)
                        Norep("Вы и %s %s %s.", steedname, predicament,
                              culprit);
                    else
                        Norep("%s %s %s.", upstart(steedname), predicament,
                              culprit);
                } else
                    Norep("Вы %s %s.", predicament, culprit);
            }
        } else {
 wriggle_free:
            if (u.usteed)
                pline("%s наконец %s.", upstart(steedname),
                      !anchored ? "вырывается" : "вырывает шар");
            else
                You("наконец %s.",
                    !anchored ? "высвобождаетесь" : "вырываете шар");
            if (anchored)
...
notice_mon(struct monst *mtmp)
{
    if (a11y.mon_notices && !a11y.mon_notices_blocked) {
        ...
        if (spot && !mtmp->mspotted && !DEADMONSTER(mtmp)) {
            ...
            You("%s %s.", canseemon(mtmp) ? "видите" : "замечаете",
                x_monnam(mtmp,
...
static boolean
loco_eq(const char *a, const char *b)
{
    return (strcmp(a, b) == 0) || (b[0] >= 'a' && b[0] <= 'z'
                                   && a[0] == (char) (b[0] - 'a' + 'A')
                                   && strcmp(a + 1, b + 1) == 0);
}
...
u_locomotion(const char *def)
{
    ...
    if (Levitation)
        ...
        verb = locomotion(gy.youmonst.data, def);

    /* translate English locomotion words to Russian */
    if (loco_eq(verb, "float"))
        rv = "парите";
    else if (loco_eq(verb, "fly"))
        rv = "летите";
    else if (loco_eq(verb, "flutter"))
        rv = "порхаете";
    else if (loco_eq(verb, "slither"))
        rv = "скользите";
    else if (loco_eq(verb, "ooze"))
        rv = "ползёте";
    else if (loco_eq(verb, "wiggle"))
        rv = "ёрзаете";
    else if (loco_eq(verb, "pulsate"))
        rv = "пульсируете";
    else if (loco_eq(verb, "crawl"))
        rv = "ползёте";
    else if (loco_eq(verb, "step"))
        rv = "ступаете";
    else if (loco_eq(verb, "jump"))
        rv = "прыгаете";
    else if (loco_eq(verb, "climb"))
        rv = "карабкаетесь";
    else if (loco_eq(verb, "stumble"))
        rv = "спотыкаетесь";
    else if (loco_eq(verb, "lower yourself"))
        rv = "опускаетесь";
    else if (loco_eq(verb, "run"))
        rv = "бегите";
    else
        return verb;
    ...
    if (capitalize) {
        Strcpy(capbuf, rv);
        uc = (unsigned char) capbuf[0];
        capbuf[0] = (uc >= 0xE0 && uc <= 0xFF) ? (char) (uc - 0x20)
                                               : highc(capbuf[0]);
        return capbuf;
    }
    return rv;
...
handle_tip(int tip)
{
    ...
    if (tip >= 0 && tip < NUM_TIPS && !(svc.context.tips & (1 << tip))) {
        ...
        switch (tip) {
        case TIP_ENHANCE:
            pline("(Подсказка: используйте команду #enhance, чтобы их развивать.)");
            break;
        case TIP_SWIM:
            pline("(Подсказка: используйте префикс '%s', чтобы шагнуть вперёд, если действительно этого хотите.)",
                  visctrl(cmd_from_func(do_reqmenu)));
            break;
        case TIP_UNTRAP_MON:
            pline("(Подсказка: возможно, #untrap поможет?)");
            break;
        case TIP_GETPOS:
            l_nhcore_call(NHCORE_GETPOS_TIP);
            break;
        default:
            impossible("Неизвестная подсказка в handle_tip(%i)", tip);
            break;
        }
        return TRUE;
    }
...
domove_bump_mon(struct monst *mtmp, int glyph)
{
    ...
    if (svc.context.nopick && !svc.context.travel
        ...
        if (M_AP_TYPE(mtmp) && !Protection_from_shape_changers
            ...
            /* m_monnam(): "dog" или "Fido", no "invisible dog" или "it" */
            pline("Извините, %s.", m_monnam(mtmp));
        else
            You("двигаетесь прямо на %s.", mon_nam(mtmp));
        return TRUE;
    }
    return FALSE;
}
...
domove_fight_web(coordxy x, coordxy y)
{
    ...
    if (svc.context.forcefight && trap && trap->ttyp == WEB && trap->tseen) {
        ...
        if (uwep && (u_wield_art(ART_STING)
            ...
            /* guaranteed success */
            pline("%s %s сквозь паутину!", bare_artifactname(uwep),
                  u_wield_art(ART_STING) ? "разрезает" : "сжигает");
        ...
        /* is_blade() includes daggers (which are classified as PIERCE)
           but doesn't include axes and slashing polearms */
            ...
            You_cant("разрезать паутину %s%s%s!", uwepstr,
                     !onewep ? " или " : "", !onewep ? scndstr : "");
            return TRUE;
        ...
        /* weapon is ok; check whether hit is successful */
            ...
            You("%s по нитям паутины впустую.",
                uwep ? "рубите" : "бьётесь");
            return TRUE;

        /* hit has succeeded */
        } else {
            You("%s сквозь паутину.", uwep ? "прорубаете" : "пробиваете");
    ...
    coordxy x, coordxy y)
{
    ...
    if (mtmp->mtrapped && is_pit(trap->ttyp)
        ...
        You("стоп. %s не пролезет.", YMonnam(mtmp));
        didnt_move = TRUE;
        ...
        if (!trap->tseen) {
            ...
            which = just_an(anbuf, what); /* "a " или "an " */
        }
        You("стоп. %s не может выбраться из %s%s.", YMonnam(mtmp), which, what);
        ...
        You("стоп. %s не желает меняться местами.", YMonnam(mtmp));
        didnt_move = TRUE;
    } else {
        mtmp->mtrapped = 0;
        remove_monster(x, y);
        ...
        You("%s %s.", mtmp->mpeaceful ? "меняетесь местами с" : "пугаете",
            x_monnam(mtmp,
                     mtmp->mtame ? ARTICLE_YOUR
                     : (!has_mgivenname(mtmp)
        ...
        case Trap_Killed_Mon:
             ...
             */
            {
                ...
                if (!u.uconduct.killer++)
                    livelog_printf(LL_CONDUCT, "совершил убийство впервые");
             ...
             */
            if (rn2(4)) {
                You_feel("чувство вины за потерю питомца.");
                u.ugangr++;
                adjalign(-15);
            }
            break;
...
{
    static const char unknown_obstacle[] = "неизвестное препятствие";
    ...
    if (svc.context.forcefight
        ...
        if (boulder) {
            ...
            Strcpy(buf, "разрежённый воздух");
        }

 futile:
        You("%s%s %s.",
            !(boulder || solid) ? "" : !explo ? "безвредно " : "тщетно ",
            explo ? "взрываетесь" : "атакуете", buf);
...
air_turbulence(void)
{
    if (Is_airlevel(&u.uz) && rn2(4) && !Levitation && !Flying) {
        ...
        case 0:
            ...
            break;
        case 1:
            You_cant("хорошо контролировать свои движения.");
            break;
        case 2:
            pline("Трудно идти по разрежённому воздуху.");
            exercise(A_DEX, TRUE);
            break;
        }
        return TRUE;
    ...
    if (u.uinwater) {
        ...
        *y = u.uy + u.dy;
        ...
        if (isok(*x, *y) && !is_pool(*x, *y) && !Is_waterlevel(&u.uz)
            ...
            You("несёте слишком много, чтобы выбраться из воды.");
            nomul(0);
            return TRUE;
        }
    }
...
avoid_trap_andor_region(coordxy x, coordxy y)
{
    ...
    if (ParanoidTrap && !Blind && !Stunned && !Confusion && !Hallucination
        ...
        Snprintf(qbuf, sizeof qbuf,
                 "Перед вами облако %s действительно войдёте в него?",
                 (reg_damg(newreg) > 0) ? "ядовитого газа" : "пара");
    ...
    if (ParanoidTrap && !Stunned && !Confusion
        ...
        Snprintf(qbuf, sizeof qbuf,
                 "Перед вами %s действительно %s %s?",
                 defsyms[trap_to_defsym(traptype)].explanation,
                 into ? "шагнёте" : "наступите",
                 into ? "в неё" : "на неё");
...
move_out_of_bounds(coordxy x, coordxy y)
{
    if (!isok(x, y)) {
        ...
        if (flags.mention_walls) {
            ...
            You("уже продвинулись так далеко %s, как только возможно.",
...
carrying_too_much(void)
{
    ...
    if (((wtcap = near_capacity()) >= OVERLOADED
        ...
        && !Is_airlevel(&u.uz)) {
        if (wtcap < OVERLOADED) {
            You("не хватает сил, чтобы двигаться.");
            exercise(A_CON, FALSE);
        } else
            You("падаете под тяжестью груза.");
        nomul(0);
        return TRUE;
    }
    return FALSE;
...
escape_from_sticky_mon(coordxy x, coordxy y)
{
    if (u.ustuck && (x != u.ustuck->mx || y != u.ustuck->my)) {
        ...
        if (!m_next2u(u.ustuck)) {
            ...
            mtmp = u.ustuck;
            set_ustuck((struct monst *) 0);
            You("отпускаете %s.", y_monnam(mtmp));
        } else {
            ...
            default:
                if (Conflict || u.ustuck->mconf || !u.ustuck->mtame) {
                    You("не можете вырваться от %s!", y_monnam(u.ustuck));
                    nomul(0);
                    return TRUE;
                }
                FALLTHROUGH;
            ...
            case 2:
                mtmp = u.ustuck;
                set_ustuck((struct monst *) 0);
                You("вырываетесь от %s.", y_monnam(mtmp));
                break;
            }
        }
    }
...
domove_core(void)
{
    ...
    if (mtmp) {
        if (displaceu) {
            ...
            pline("%s меняется с вами местами...",
    ...
    if (cause_delay) {
        nomul(-2);
        gm.multi_reason = "волоча железный шар";
        gn.nomovemsg = "";
    }
...
overexert_hp(void)
{
    ...
    if (*hp > 1) {
        *hp -= 1;
        ...
        You("теряете сознание от перенапряжения!");
        exercise(A_CON, FALSE);
...
invocation_message(void)
{
    ...
    if (invocation_pos(u.ux, u.uy) && !On_stairs(u.ux, u.uy)) {
        ...
        if (u.usteed)
            Sprintf(buf, "под %s", y_monnam(u.usteed));
        else if (Levitation || Flying)
            Strcpy(buf, "под вами");
        else
            Sprintf(buf, "под вашими %s", makeplural(body_part(FOOT)));

        You_feel("странное колебание %s.", buf);
        u.uevent.uvibrated = 1;
        if (otmp && otmp->spe == 7 && otmp->lamplit)
            pline("%s %s!", The(xname(otmp)),
                  Blind ? "ощутимо пульсирует" : "светится странным светом");
    }
}
    ...
    boolean newspot) /* true if called by spoteffects */
{
    ...
    if (u.uinwater) {
        ...
        if (!is_pool(u.ux, u.uy)) {
            if (Is_waterlevel(&u.uz)) {
                You("выскакиваете в воздушный пузырь.");
                iflags.last_msg = PLNMSG_BACK_ON_GROUND;
            } else if (is_lava(u.ux, u.uy)) {
                You("покидаете %s...", hliquid("water")); /* oops! */
            } else {
            ...
            You("выскакиваете из %s, как пробка!", hliquid("water"));
        } else if (Flying) {
            You("вылетаете из %s.", hliquid("water"));
        } else if (Wwalking) {
            You("медленно поднимаетесь над поверхностью.");
        } else {
            still_inwater = TRUE;
        }
        if (!still_inwater) {
...
spoteffects(boolean pick)
{
    ...
    if (Warning && is_ice(u.ux, u.uy)) {
        static const char *const icewarnings[] = {
            "Лёд кажется очень мягким и мокрым.",
            "Вы чувствуете, как лёд сдвигается под вами!",
            "Лёд сейчас треснет!", /* The Dead Zone */
        };
    ...
    if ((mtmp = m_at(u.ux, u.uy)) != 0 && !u.uswallow) {
        ...
        case S_PIERCER:
            pline("%s внезапно падает с потолка!", Amonnam(mtmp));
            if (mtmp->mtame) { /* jumps to greet you, not attack */
                ;
            } else if (hard_helmet(uarmh)) {
                pline("Его удар скользит по вашему %s.",
                      helm_simple_name(uarmh));
            } else if (u.uac + 3 <= rnd(20)) {
                You("чуть не задеты %s!",
                    x_monnam(mtmp, ARTICLE_A, "падающего", 0, TRUE));
            } else {
                int dmg;
        ...
        default: /* monster surprises you. */
            if (mtmp->mtame)
                pline("%s спрыгивает к вам с потолка.", Amonnam(mtmp));
            else if (mtmp->mpeaceful) {
                You("застаёте врасплох %s!",
                    Blind && !sensemon(mtmp) ? something : a_monnam(mtmp));
                mtmp->mpeaceful = 0;
            } else
                pline("%s атакует вас врасплох!", Amonnam(mtmp));
            break;
        }
...
check_special_room(boolean newlev)
{
    ...
    for (ptr = &u.uentered[0]; *ptr; ptr++) {
        ...
        switch (rt) {
        case ZOO:
            pline("Добро пожаловать в зоопарк сокровищ Дэвида!");
            break;
        case SWAMP:
            pline("Здесь %s довольно %s.", Blind ? "ощущается" : "выглядит",
                  Blind ? "влажно" : "грязно");
            break;
        case COURT:
            You("входите в роскошную%s комнату!",
                /* the throne room in Sam quest home level lacks a throne */
                !furniture_present(THRONE, roomno) ? "" : " тронный");
            break;
        case LEPREHALL:
            You("входите в зал лепреконов!");
            break;
        case MORGUE:
            if (midnight()) {
                const char *run = u_locomotion("Run");

                pline("%s прочь! %s прочь!", run, run);
            } else
                You("испытываете жуткое чувство...");
            break;
        case BEEHIVE:
            You("входите в гигантский улей!");
            break;
        case COCKNEST:
            You("входите в отвратительное гнездо!");
            break;
        case ANTHOLE:
            You("входите в муравейник!");
            break;
        case BARRACKS:
            if (monstinroom(&mons[PM_SOLDIER], roomno)
                ...
                You("входите в военные казармы!");
            else
                You("входите в заброшенные казармы.");
            break;
        case DELPHI: {
            ...
            if (oracle) {
                ...
                if (!oracle->mpeaceful)
                    verbalize("Вы в Дельфах, %s.", svp.plname);
                else
                    verbalize("%s, %s, добро пожаловать в Дельфы!",
...
pickup_checks(void)
{
    ...
    if (u.uswallow) {
        if (!u.ustuck->minvent) {
            if (digests(u.ustuck->data)) {
                You("подбираете %s язык.", s_suffix(mon_nam(u.ustuck)));
                pline("Но он какой-то склизкий, так что вы его бросаете.");
            } else
                You("не %s здесь ничего, что можно подобрать.",
                    Blind ? "ощущаете" : "видите");
            return 1;
        } else {
    ...
    if (is_pool(u.ux, u.uy)) {
        if (Wwalking || is_floater(gy.youmonst.data)
            ...
            You("не можете нырнуть в %s, чтобы подобрать вещи.",
                hliquid("water"));
            return 0;
        } else if (!Underwater) {
            You_cant("даже увидеть дно, не говоря уже о том, чтобы подобрать %s.", something);
            return 0;
        }
    }
    if (is_lava(u.ux, u.uy)) {
        if (Wwalking || is_floater(gy.youmonst.data)
            || is_clinger(gy.youmonst.data) || (Flying && !Breathless)) {
            You_cant("достать дно, чтобы подобрать вещи.");
            return 0;
        } else if (!likes_lava(gy.youmonst.data)) {
            You("сгорите дотла, пытаясь подобрать вещи.");
            return 0;
        }
    }
    if (!OBJ_AT(u.ux, u.uy)) {
        ...
        if (IS_THRONE(lev->typ))
            pline("Это, должно быть, весит%s тонну!", lev->looted ? " почти" : "");
            ...
            pline("Она не снимается с петель.");
        else if (IS_ALTAR(lev->typ))
            pline("Двигать алтарь было бы очень плохой идеей.");
        else if (lev->typ == STAIRS)
            pline("Лестница прочно закреплена.");
        else
            There("здесь нечего подбирать.");
        return 0;
    }
    ...
    if (!can_reach_floor(traphere && is_pit(traphere->ttyp))) {
        ...
        if (traphere && uteetering_at_seen_pit(traphere)) {
            You("не можете достать дно ямы.");
        } else if (u.usteed && P_SKILL(P_RIDING) < P_BASIC) {
            rider_cant_reach();
        } else if (Blind) {
            You("не можете ничего здесь достать.");
        } else {
...
lookaround(void)
{
    ...
    if (NODIAG(u.umonnum) && u.dx && u.dy) {
        You("не можете двигаться по диагонали.");
        nomul(0);
        return;
    }
        ...
        for (y = u.uy - 1; y <= u.uy + 1; y++) {
            ...
            if ((mtmp = m_at(x, y)) != 0
                ...
                if ((svc.context.run != 1 && !is_safemon(mtmp))
                    ...
                    if (flags.mention_walls)
                        pline_xy(x, y, "%s преграждает вам путь.",
            ...
            if (IS_OBSTRUCTED(levl[x][y].typ) || levl[x][y].typ == ROOM
                ...
                if (svc.context.run != 1 && !svc.context.travel) {
                    if (flags.mention_walls) {
                        set_msg_xy(x, y);
                        You("останавливаетесь перед дверью.");
                    }
                    goto stop;
                }
 ...
 stop:
    ...
    if (corrct > 1 && svc.context.run == 2) {
        if (flags.mention_walls)
            pline("Коридор здесь расширяется.");
        goto stop;
    }
...
unmul(const char *msg_override)
{
    ...
    if (*gn.nomovemsg) {
        ...
        if (Upolyd && !strncmpi(gn.nomovemsg, "Вы пережили это ", 16))
            You("есть %s.",
...
maybe_wail(void)
{
    ...
    if (Role_if(PM_WIZARD) || Race_if(PM_ELF) || Role_if(PM_VALKYRIE)) {
        ...
        who = (Role_if(PM_WIZARD) || Role_if(PM_VALKYRIE)) ? gu.urole.name.m
                                                           : "Эльф";
        if (u.uhp == 1) {
            pline("%s вот-вот умрёт.", who);
        } else {
            for (i = 0, powercnt = 0; i < SIZE(powers); ++i)
                if (u.uprops[powers[i]].intrinsic & INTRINSIC)
                    ++powercnt;

            pline((powercnt >= 4) ? "%s, все ваши силы будут утрачены..."
                                  : "%s, ваша жизненная сила иссякает.",
                  who);
        }
    } else {
        Soundeffect(se_wailing_of_the_banshee, 75);
        You_hear(u.uhp == 1 ? "завывания Банши..."
                            : "вой КwnAnwn...");
    }
}
...
showdamage(int dmg)
{
    ...
    pline("[HP %i, %i осталось]", -dmg, Upolyd ? u.mh : u.uhp);
}
...
check_capacity(const char *str)
{
    if (near_capacity() >= EXT_ENCUMBER) {
        if (str)
            ...
            You_cant("делать это, неся столько вещей.");
        return 1;
    }
    return 0;
}
