 ...
 *      Moloch is unaligned.
 */
static const char *const Moloch = "Молох";

static const char *const godvoices[] = {
    "гремит", "грохочет", "звенит", "громыхает",
};
...
fix_curse_trouble(struct obj *otmp, const char *what)
{
    ...
    if (otmp == uarmg && Glib) {
        make_glib(0);
        Your("%s больше не скользят.", gloves_simple_name(uarmg));
        if (!otmp->cursed)
            return;
    }
    if (!Blind || (otmp == ublindf && Blindfolded_only)) {
        pline("%s %s.",
                what ? what : (const char *) Yobjnam2(otmp, "мягко светится"),
                hcolor(NH_AMBER));
...
fix_worst_trouble(int trouble)
{
    ...
    static NEARDATA const char leftglow[] = "Ваше левое кольцо мягко светится",
                               rightglow[] = "Ваше правое кольцо мягко светится";

    switch (trouble) {
    case TROUBLE_STONED:
        make_stoned(0L, "Вы чувствуете себя более гибким.", 0, (char *) 0);
        break;
    case TROUBLE_SLIMED:
        make_slimed(0L, "Слизь исчезает.");
        break;
    case TROUBLE_STRANGLED:
        if (uamul && uamul->otyp == AMULET_OF_STRANGULATION) {
            Your("амулет исчезает!");
            useup(uamul);
        }
        You("снова можете дышать.");
        Strangled = 0;
        disp.botl = TRUE;
        break;
    case TROUBLE_LAVA:
    ...
    case TROUBLE_STARVING:
        ...
        /* FALLTHRU*/
    case TROUBLE_HUNGRY:
        Your("%s доволен.", body_part(STOMACH));
        init_uhunger();
        disp.botl = TRUE;
        break;
    case TROUBLE_SICK:
    ...
    case TROUBLE_COLLAPSING:
        ...
        You_feel("%sсильнее.",
                 (AMAX(A_STR) - ABASE(A_STR) > 6) ? "гораздо " : "");
    ...
    case TROUBLE_POISONED:
        ...
        if (Hallucination)
            pline("У вас в баке тигр.");
        else
            You_feel("снова в добром здравии.");
        for (i = 0; i < A_MAX; i++) {
            if (ABASE(i) < AMAX(i)) {
                ABASE(i) = AMAX(i);
                disp.botl = TRUE;
    ...
    case TROUBLE_BLIND: { /* handles deafness as well as blindness */
        ...
        if (Blinded) {
            ...
            Sprintf(msgbuf, "Ваши %s чувствуют себя лучше", eyes);
            u.ucreamed = 0;
        ...
        if (cure_deaf) {
            make_deaf(0L, FALSE);
            if (!Deaf)
                Sprintf(eos(msgbuf), "%s снова можете слышать",
                        !*msgbuf ? "Вы" : ", и вы");
        }
        if (*msgbuf)
            pline("%s.", msgbuf);
    ...
    case TROUBLE_HALLUCINATION:
        pline("Похоже, вы снова в Канзасе.");
        (void) make_hallucinated(0L, FALSE, 0L);
        break;
    case TROUBLE_SADDLE:
        otmp = which_armor(u.usteed, W_SADDLE);
        if (!Blind) {
            pline("%s %s.", Yobjnam2(otmp, "мягко светится"), hcolor(NH_AMBER));
            set_bknown(otmp, 1);
        }
        uncurse(otmp);
        break;
...
god_zaps_you(aligntyp resp_god)
{
    if (u.uswallow) {
        pline(
          "Внезапно с небес в вас ударяет молния!");
        pline("Она поражает %s!", mon_nam(u.ustuck));
        if (!resists_elec(u.ustuck)) {
            pline("%s поджаривается до хруста!", Monnam(u.ustuck));
            ...
            pline("%s, похоже, невредим.", Monnam(u.ustuck));
    } else {
        pline("Внезапно в вас ударяет молния!");
        if (Reflecting) {
            shieldeff(u.ux, u.uy);
            if (Blind)
                pline("По какой-то причине вы невредимы.");
            else
                (void) ureflects("%s отражается от вашего %s.", "Оно");
            monstseesu(M_SEEN_REFL);
        } else if (Shock_resistance) {
            shieldeff(u.ux, u.uy);
            pline("Похоже, это на вас не действует.");
    ...
    pline("%s не остановить...", align_gname(resp_god));
    if (u.uswallow) {
        pline("Луч дезинтеграции, нацеленный в вас, попадает в %s!",
              mon_nam(u.ustuck));
        if (!resists_disint(u.ustuck)) {
            pline("%s рассыпается в кучу пыли!", Monnam(u.ustuck));
            xkilled(u.ustuck, XKILL_NOMSG | XKILL_NOCORPSE | XKILL_NOCONDUCT);
        } else
            pline("%s, похоже, невредим.", Monnam(u.ustuck));
    } else {
        pline("Луч дезинтеграции попадает в вас!");
        ...
        if (!Disint_resistance) {
            ...
            You("нежитесь в его %s сиянии минуту...", NH_BLACK);
            godvoice(resp_god, "Я этому не верю!");
            monstseesu(M_SEEN_DISINT);
        }
        if (Is_astralevel(&u.uz) || Is_sanctum(&u.uz)) {
            /* one more try for high altars */
            SetVoice((struct monst *) 0, 0, 80, voice_deity);
            verbalize("Тебе не уйти от моего гнева, смертный!");
            summon_minion(resp_god, FALSE);
            summon_minion(resp_god, FALSE);
            summon_minion(resp_god, FALSE);
            SetVoice((struct monst *) 0, 0, 80, voice_deity);
            verbalize("Уничтожьте %s, мои слуги!", uhim());
        }
    }
}
...
{
    You("%s!", !via_disintegration ? "поджариваетесь"
                                   : "рассыпаетесь в кучу пыли");
    svk.killer.format = KILLED_BY;
    Sprintf(svk.killer.name, "гнев %s", align_gname(resp_god));
    done(DIED);
}
...
angrygods(aligntyp resp_god)
{
    ...
    case 0:
    case 1:
        You_feel("что %s %s.", align_gname(resp_god),
                 Hallucination ? "обижен" : "недоволен");
        break;
    case 2:
    case 3:
        ...
        pline("\"Ты %s, %s.\"",
              (ugod_is_angry() && resp_god == u.ualign.type)
                  ? "сошёл с пути"
                  : "заносчив",
              gy.youmonst.data->mlet == S_HUMAN ? "смертный" : "тварь");
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Ты должен заново усвоить уроки!");
    ...
    case 5:
        gods_angry(resp_god);
        if (!Blind && !Antimagic)
            pline("%s сияние окружает вас.", An(hcolor(NH_BLACK)));
        if (rn2(2) || !attrcurse())
            rndcurse();
        break;
    case 7:
    case 8:
        godvoice(resp_god, (char *) 0);
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Как ты смеешь %s меня?",
...
at_your_feet(const char *str)
{
    ...
    if (u.uswallow) {
        ...
        pline("%s %s в %s %s.", str, vtense(str, "падает"),
              s_suffix(mon_nam(u.ustuck)), mbodypart(u.ustuck, STOMACH));
    } else {
        pline("%s %s %s ваших %s!", str,
              vtense(str, Blind ? "приземляется" : "появляется"),
              Levitation ? "под вашими" : "у ваших",
...
#define ok_wep(o) ((o) && ((o)->oclass == WEAPON_CLASS || is_weptool(o)))
    ...
    case A_LAWFUL:
        ...
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Я короную тебя...  Дланью Эльберета!");
        livelog_printf(LL_DIVINEGIFT,
                       "коронован \"Дланью Эльберета\" по воле %s",
                       u_gname());
        break;
    case A_NEUTRAL:
        ...
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Ты станешь моим Посланником Равновесия!");
        livelog_printf(LL_DIVINEGIFT, "стал %s Посланником Равновесия",
                       s_suffix(u_gname()));
        break;
    case A_CHAOTIC:
        ...
        what = (((already_exists && !in_hand) || class_gift != STRANGE_OBJECT)
                ...
                : "красть души");
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Ты избран %s во Славу Мою!", what);
        livelog_printf(LL_DIVINEGIFT, "избран %s во славу %s",
                       what, u_gname());
        break;
    }
    ...
    case A_LAWFUL:
        if (class_gift != STRANGE_OBJECT) {
            ...
            if (!Blind)
                Your("меч ярко сверкает на мгновение.");
    ...
    case A_CHAOTIC: {
        ...
        Sprintf(swordbuf, "%s меч", hcolor(NH_BLACK));
        if (class_gift != STRANGE_OBJECT) {
            ; /* already got bonus above */
        } else if (obj && in_hand) {
            Your("%s зловеще гудит!", swordbuf);
            observe_object(obj);
        } else if (!already_exists) {
    ...
    if (otmp->otyp != SPE_BLANK_PAPER && !rn2(4)
        ...
        if ((spe_let = force_learn_spell(otmp->otyp)) != '\0') {
            ...
            if (spe_knowledge == spe_Unknown) /* prior to learning */
                ...
                pline("Божественное знание %s заполняет ваш разум!  Заклинание '%c'.",
                      spe_name, spe_let);
            else
                Your("знание заклинания '%c' - %s %s.",
                     spe_let, spe_name,
                     (spe_knowledge == spe_Forgotten) ? "восстановлено"
                                                      : "обновлено");
        }
...
pleased(aligntyp g_align)
{
    ...
    You_feel("что %s %s.", align_gname(g_align),
             (u.ualign.record >= DEVOUT)
                 ? Hallucination ? "доволен как слон" : "весьма доволен"
                 : (u.ualign.record >= STRIDENT)
                       ? Hallucination ? "в игривом настроении" : "доволен"
                       : Hallucination ? "сыт" : "удовлетворён");
    ...
    if (pat_on_head)
            ...
            if (uwep && (welded(uwep) || uwep->oclass == WEAPON_CLASS
                ...
                *repair_buf = '\0';
                if (uwep->oeroded || uwep->oeroded2)
                    Sprintf(repair_buf, " и теперь как новый");

                if (uwep->cursed) {
                    if (!Blind) {
                        pline("%s %s%s.", Yobjnam2(uwep, "мягко светится"),
                              hcolor(NH_AMBER), repair_buf);
                        iflags.last_msg = PLNMSG_OBJ_GLOWS;
                    } else
                        You_feel("власть %s над %s.", u_gname(),
                                 yname(uwep));
                    uncurse(uwep);
        ...
        case 3:
            ...
            if (!u.uevent.uopened_dbridge && !u.uevent.gehennom_entered) {
                if (u.uevent.uheard_tune < 1) {
                    ...
                    SetVoice((struct monst *) 0, 0, 80, voice_deity);
                    verbalize("Внемли, %s!", is_human(gy.youmonst.data)
                                               ? "смертный"
                                               : "тварь");
                    SetVoice((struct monst *) 0, 0, 80, voice_deity);
                    verbalize(
                       "Чтобы войти в замок, ты должен сыграть нужную мелодию!");
                    u.uevent.uheard_tune++;
                    break;
                } else if (u.uevent.uheard_tune < 2) {
                    Soundeffect(se_divine_music, 50);
                    You_hear("божественную музыку...");
                    pline("Звучит так:  \"%s\".", svt.tune);
                    u.uevent.uheard_tune++;
        ...
        case 2:
            if (!Blind)
                You("окружены %s сиянием.", an(hcolor(NH_GOLDEN)));
        ...
        case 4: {
            ...
            if (Blind)
                You_feel("власть %s.", u_gname());
            else
                You("окружены %s аурой.", an(hcolor(NH_LIGHT_BLUE)));
            for (otmp = gi.invent; otmp; otmp = nextobj) {
                ...
                if (otmp->cursed
                    && (otmp != uarmh /* [see worst_cursed_item()] */
                        || uarmh->otyp != HELM_OF_OPPOSITE_ALIGNMENT)) {
                    if (!Blind) {
                        pline("%s %s.", Yobjnam2(otmp, "мягко светится"),
                              hcolor(NH_AMBER));
                        iflags.last_msg = PLNMSG_OBJ_GLOWS;
        ...
        case 5: {
            static NEARDATA const char msg[] =
                "\"и потому дарую тебе дар %s!\"";

            godvoice(u.ualign.type,
                     "Ты порадовал меня своими успехами,");
            if (!(HTelepat & INTRINSIC)) {
                ...
                if (!(HProtection & INTRINSIC)) {
                    ...
                    u.ublessed++;
                pline(msg, "моей защиты");
            }
            SetVoice((struct monst *) 0, 0, 80, voice_deity);
            verbalize("Используй его с умом во имя моё!");
            break;
        }
        case 7:
        case 8:
...
water_prayer(boolean bless_water)
{
    ...
    if (!Blind && changed) {
        pline("%s%s%s на алтаре на мгновение светятся %s.",
              ((other && changed > 1L) ? "Некоторые зелья"
                                       : (other ? "Одно из зелий" : "Зелья")),
...
godvoice(aligntyp g_align, const char *words)
{
    ...
    pline("Голос %s %s: %s%s%s", align_gname(g_align),
...
{
    godvoice(g_align, "Ты разгневал меня.");
}
...
consume_offering(struct obj *otmp)
{
    if (Hallucination)
        switch (rn2(3)) {
        case 0:
            Your("жертва обзаводится крыльями и пропеллером и с рёвом улетает!");
            break;
        case 1:
            Your("жертва раздувается всё больше и больше и лопается!");
            break;
        case 2:
            Your(
     "жертва рассыпается в облако танцующих частиц и исчезает!");
            break;
        }
    else if (Blind && u.ualign.type == A_LAWFUL)
        Your("жертва исчезает!");
    else
        Your("жертва поглощается %s!",
...
offer_too_soon(aligntyp altaralign)
{
    ...
    You_feel("%s.", Hallucination
                    ...
                    : (altaralign == u.ualign.type)
                        ? "порыв вернуться на поверхность"
...
desecrate_altar(boolean highaltar, aligntyp altaralign)
{
    ...
    You_feel("как воздух вокруг вас наэлектризовался...");
    pline("Внезапно вы понимаете, что %s заметил вас...",
          align_gname(altaralign));
    Sprintf(gvbuf, "Так, смертный!  Ты смеешь осквернить мой %s!",
            highaltar ? "Великий Храм" : "алтарь");
...
offer_real_amulet(struct obj *otmp, aligntyp altaralign)
{
    static NEARDATA const char
        cloud_of_smoke[] = "Облако %s дыма окружает вас...";
    ...
    You("приносите Амулет Йендора в дар %s...", a_gname());

    if (altaralign == A_NONE) {
        /* Moloch's high altar at the bottom of Gehennom. */
        if (u.ualign.record > -99)
            u.ualign.record = -99;
        pline("Невидимый хор поёт, и вас окутывает тьма...");
        /*[apparently shrug/snarl can be sensed without being seen]*/
        pline("%s пожимает плечами и сохраняет власть над %s,", Moloch, u_gname());
        pline("а затем безжалостно прерывает вашу жизнь.");
        Sprintf(svk.killer.name, "%s равнодушие", s_suffix(Moloch));
        svk.killer.format = KILLED_BY;
        done(DIED);
        /* life-saved (or declined to die in wizard/explore mode) */
        pline("%s злобно рычит и пробует снова...", Moloch);
        ...
        /* And the opposing team picks you up and carries you off
           on their shoulders. */
        adjalign(-99);
        pline("%s принимает ваш дар и обретает власть над %s...",
              a_gname(), u_gname());
        pline("%s в ярости...", u_gname());
        pline("К счастью, %s позволяет вам жить...", a_gname());
        ...
        u.uevent.ascended = 1;
        adjalign(10);
        pline("Невидимый хор поёт, и вас окутывает сияние...");
        godvoice(altaralign, "Смертный, ты хорошо справился!");
        display_nhwindow(WIN_MESSAGE, FALSE);
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize(
          "В награду за службу дарую тебе бессмертие!");
        You("восходите в ранг Полубог%s...",
            flags.female ? "ини" : "");
        done(ASCENDED);
        /*NOTREACHED*/
    }
    ...
    aligntyp altaralign)
{
    ...
    You_hear("раскат грома поблизости.");
    if (!otmp->known) {
        You("понимаете, что допустили %s.",
            Hallucination ? "оплошность" : "ошибку");
        otmp->known = TRUE;
        change_luck(-1);
    } else {
        /* don't you dare try to fool the gods */
        if (Deaf)
            pline("О, нет."); /* didn't hear thunderclap */
        change_luck(-3);
        adjalign(-1);
        u.ugangr += 3;
    ...
    aligntyp altaralign)
{
    ...
    if (ugod_is_angry() || (altaralign == A_NONE && Inhell)) {
        if (u.ualignbase[A_CURRENT] == u.ualignbase[A_ORIGINAL]
            && altaralign != A_NONE) {
            You("остро чувствуете, что %s разгневан...", u_gname());
            consume_offering(otmp);
            pline("%s принимает вашу присягу.", a_gname());
            ...
            u.ugangr += 3;
            adjalign(-5);
            pline("%s отвергает вашу жертву!", a_gname());
            godvoice(altaralign, "Страдай, неверный!");
            change_luck(-5);
        ...
        consume_offering(otmp);
        You("чувствуете конфликт между %s и %s.", u_gname(), a_gname());
        if (rn2(8 + u.ulevel) > 5) {
            struct monst *pri;
            boolean shrine;

            You_feel("как власть %s растёт.", u_gname());
            exercise(A_WIS, TRUE);
            change_luck(1);
            shrine = on_shrine();
            ...
            pline("К несчастью, вы чувствуете, как власть %s слабеет.", u_gname());
            change_luck(-1);
    ...
    aligntyp altaralign)
{
    ...
    if (is_demon(gy.youmonst.data)) {
        You("находите эту идею весьма приятной.");
        exercise(A_WIS, TRUE);
    } else if (u.ualign.type != A_CHAOTIC) {
        pline("Вы пожалеете об этом гнусном оскорблении!");
        exercise(A_WIS, FALSE);
    }
    ...
    if (highaltar
        ...
        pline("Алтарь запятнан %s кровью.", gu.urace.adj);
        ...
        if (altaralign == A_CHAOTIC && u.ualign.type != A_CHAOTIC) {
            pline(
            "Кровь заливает алтарь, и он исчезает в %s облаке!",
                    an(hcolor(NH_BLACK)));
            levl[u.ux][u.uy].typ = ROOM;
            levl[u.ux][u.uy].altarmask = 0;
            newsym(u.ux, u.uy);
            angry_priest();
            demonless_msg = "облако рассеивается";
        } else {
            /* either you're chaotic or altar is Moloch's or both */
            pline("Кровь заливает алтарь!");
            change_luck(altaralign == A_NONE ? -2 : 2);
            demonless_msg = "кровь сворачивается";
        }
        if ((pm = dlord(altaralign)) != NON_PM
            ...
            if (!strcmpi(dbuf, "it"))
                Strcpy(dbuf, "нечто ужасное");
            else
                dmon->mstrategy &= ~STRAT_APPEARMSG;
            You("призвали %s!", dbuf);
            if (sgn(u.ualign.type) == sgn(dmon->data->maligntyp))
                dmon->mpeaceful = TRUE;
            You("в ужасе и не можете шевелиться.");
            nomul(-3);
            gm.multi_reason = "в ужасе от демона";
            gn.nomovemsg = 0;
        } else
...
bestow_artifact(uchar max_giftvalue)
{
    ...
    if (do_bestow) {
        ...
        if (wizard)
            do_bestow = y_n("Одарить артефактом?") == 'y';
        else
    ...
    if (do_bestow) {
        ...
        if (otmp) {
            ...
            Strcpy(buf, (Hallucination ? "безделушка"
                            : Blind ? "предмет"
                            : ansimpleoname(otmp)));
            if (!Blind)
                Sprintf(eos(buf), " по имени %s",
                        bare_artifactname(otmp));
            at_your_feet(upstart(buf));
            dropy(otmp);
            godvoice(u.ualign.type, "Используй мой дар с умом!");
            u.ugifts++;
            u.ublesscnt = rnz(300 + (50 * nartifacts));
            exercise(A_WIS, TRUE);
            livelog_printf (LL_DIVINEGIFT | LL_ARTIFACT,
                            "награждён %s по воле %s",
...
dosacrifice(void)
{
    ...
    if (!on_altar() || u.uswallow) {
        You("не %s.",
            (Levitation || Flying) ? "над алтарём" : "на алтаре");
        return ECMD_OK;
    } else if (Confusion || Stunned) {
        You("слишком недееспособны, чтобы совершить обряд.");
        return ECMD_OK;
    }
    highaltar = (levl[u.ux][u.uy].altarmask & AM_SANCTUM);

    otmp = floorfood("пожертвовать", 1);
    if (!otmp)
        return ECMD_OK;
...
eval_offering(struct obj *otmp, aligntyp altaralign)
{
    ...
    if (is_undead(ptr)) { /* Not demons--no demon corpses */
        ...
        if (unicalign == altaralign) {
             ...
             */
            pline("Такое действие — оскорбление %s!",
                  (unicalign == A_CHAOTIC) ? "хаоса"
                     : unicalign ? "закона" : "равновесия");
            (void) adjattrib(A_WIS, -1, TRUE);
            return -1;
        } else if (u.ualign.type == altaralign) {
            ...
            if (u.ualign.record < ALIGNLIM)
                You_feel("должным образом %s.", align_str(u.ualign.type));
            else
                You_feel("вы окончательно на верном пути.");
            adjalign(5);
            value += 3;
...
#define MAXVALUE 24 /* Highest corpse value (besides Wiz) */
    ...
    if (!u.uconduct.gnostic++)
        livelog_printf(LL_CONDUCT, "отказался от атеизма"
                                   ", принеся %s в жертву на алтаре %s",
    ...
    if (has_omonst(otmp)
             ...
             * not a real monster */
        pline("Так вот как вы платите за верность?!");
        adjalign(-3);
    ...
    if (u.ugangr) {
        ...
        if (u.ugangr != saved_anger) {
            if (u.ugangr) {
                pline("%s, похоже, %s.", u_gname(),
                      Hallucination ? "в ударе" : "слегка умиротворён");

                if ((int) u.uluck < 0)
                    change_luck(1);
            } else {
                pline("%s, похоже, %s.", u_gname(),
                      Hallucination ? "космичен (не новость)"
                                    : "умиротворён");
            ...
            if (Hallucination)
                pline("Боги кажутся высокими.");
            else
                You("чувствуете свою несостоятельность.");
        }
        ...
        if (u.ublesscnt != saved_cnt) {
            if (u.ublesscnt) {
                if (Hallucination)
                    You("понимаете, что боги не такие, как вы и я.");
                else
                    You("чувствуете надежду.");
                if ((int) u.uluck < 0)
                    change_luck(1);
            } else {
                if (Hallucination)
                    pline("В целом пахнет жареным луком.");
                else
                    You("чувствуете примирение.");
                if ((int) u.uluck < 0)
                    u.uluck = 0;
            }
        }
        ...
        if (u.uluck != orig_luck) {
            if (Blind)
                You("думаете, что %s коснулось вашего %s.", something,
                    body_part(FOOT));
            else
                You(Hallucination
                ? "видите крабью траву у ваших %s.  Забавно для подземелья."
                        : "замечаете четырёхлистный клевер у ваших %s.",
                    makeplural(body_part(FOOT)));
        }
    }
}
...
can_pray(boolean praying) /* false means no messages should be given */
{
    ...
    if (is_demon(gy.youmonst.data) /* ok if chaotic or none (Moloch) */
        ...
        if (praying)
            pline("Мысль о молитве %s богу отвратительна.",
                      gp.p_aligntyp ? "Законному" : "Нейтральному");
        return FALSE;
    }

    if (praying)
        You("начинаете молиться %s.", align_gname(gp.p_aligntyp));
...
dopray(void)
{
    ...
    if (ParanoidPray) {
        ok = paranoid_query(ParanoidConfirm,
                            "Вы уверены, что хотите молиться?");
#if 0
    ...
    if (!u.uconduct.gnostic++)
         ...
         */
        livelog_printf(LL_CONDUCT, "отказался от атеизма молитвой");
    ...
    if (wizard && gp.p_type >= 0) {
        static const char forcesuccess[] = "Заставить богов быть довольными?";
    ...
    nomul(-3);
    gm.multi_reason = "молясь";
    gn.nomovemsg = "Вы завершаете молитву.";
    ga.afternmv = prayer_done;

    if (gp.p_type == 3 && !Inhell) {
        /* if you've been true to your god you can't die while you pray */
        if (!Blind)
            You("окружены мерцающим светом.");
...
prayer_done(void) /* M. Stephenson (1.0.3b) */
{
    ...
    if (gp.p_type == -2) {
        ...
        You("%s дьявольский смех вокруг вас...",
            !Deaf ? "слышите" : "чувствуете");
        wake_nearby(FALSE);
        adjalign(-2);
        exercise(A_WIS, FALSE);
        if (!Inhell) {
            /* hero's god[dess] seems to be keeping his/her head down */
            pline("Больше ничего не происходит."); /* not actually true... */
            return 1;
        ...
        godvoice(alignment,
                 (alignment == A_LAWFUL)
                    ? "Мерзкая тварь, ты смеешь призывать меня?"
                    : "Не ходи больше, извращение природы!");
        You_feel("будто разваливаетесь на части.");
        /* KMH -- Gods have mastery over unchanging */
        rehumanize();
        /* no Half_physical_damage adjustment here */
        losehp(rnd(20), "остаточный эффект обращения нежити", KILLED_BY_AN);
        exercise(A_CON, FALSE);
        return 1;
    }
    if (Inhell) {
        pline("Раз вы в Геенне, %s не может вам помочь.",
    ...
    if (!mtmp->mpeaceful
        ...
        if (Confusion) {
            if (!turn_undead_msg_cnt++)
                pline("К сожалению, ваш голос срывается.");
            mtmp->mflee = 0;
            mtmp->mfrozen = 0;
            mtmp->mcanmove = 1;
...
doturn(void)
{
    ...
    if (!Role_if(PM_CLERIC) && !Role_if(PM_KNIGHT)) {
        ...
        You("не знаете, как обращать нежить!");
        return ECMD_OK;
    }
    if (!u.uconduct.gnostic++)
        livelog_printf(LL_CONDUCT, "отказался от атеизма, обращая нежить");
    ...
    if (!can_chant(&gy.youmonst)) {
        ...
        You("не можете %s к %s, дабы отвратить зло.",
            Strangled ? "обратиться" : "воззвать", Gname);
    ...
    if ((u.ualign.type != A_CHAOTIC
        ...
        pline("По какой-то причине %s, похоже, игнорирует вас.", Gname);
        aggravate();
        exercise(A_WIS, FALSE);
        return ECMD_TIME;
    }
    if (Inhell) {
        pline("Раз вы в Геенне, %s %s помочь вам.",
              /* not actually calling upon Moloch but use alternate
                 phrasing anyway if hallucinatory feedback says it's him */
              Gname, !strcmp(Gname, Moloch) ? "не пожелает" : "не сможет");
        aggravate();
        return ECMD_TIME;
    }
    pline("Воззвав к %s, вы произносите тайную формулу.", Gname);
    exercise(A_WIS, TRUE);
    ...
    gm.multi_reason = "пытаясь обратить монстров";
...
static const char *const hallu_gods[] = {
    "Летающий макаронный монстр", /* Church of the FSM */
    "Эрида",                         /* Discordianism */
    "Марсиане",                 /* every science fiction ever */
    "Ксом",                          /* Crawl */
    "АнДоР дРаКоН",                 /* ADOM */
    "Центральный банк Йендора",   /* economics */
    "Зубная фея",                  /* real world(?) */
    "Ом",                           /* Discworld */
    "Йавгмот",                     /* Magic: the Gathering */
    "Моргот",                      /* LoTR */
    "Ктулху",                      /* Lovecraft */
    "Ори",                      /* Stargate */
    "судьба",                      /* why not? */
    "ваш Друг Компьютер",     /* Paranoia */
};
...
halu_gname(aligntyp alignment)
{
    ...
    if (!gnam) {
        ...
        gnam = "ваш Друг Компьютер"; /* Paranoia */
    }
    if (*gnam == '_')
        ++gnam;
    return gnam;
...
altar_wrath(coordxy x, coordxy y)
{
    ...
    if (u.ualign.type == altaralign && u.ualign.record > -rn2(4)) {
        godvoice(altaralign, "Как ты смеешь осквернять мой алтарь!");
        (void) adjattrib(A_WIS, -1, FALSE);
        u.ualign.record--;
    } else {
        pline("%s %s%s:",
              !Deaf ? "Голос (неужели это"
                    : "Несмотря на глухоту, вы, кажется, слышите",
              align_gname(altaralign),
              !Deaf ? "?) шепчет" : " говорит");
        SetVoice((struct monst *) 0, 0, 80, voice_deity);
        verbalize("Ты заплатишь, неверный!");
