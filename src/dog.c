...
initedog(struct monst *mtmp, boolean everything)
{
    ...
    if (!u.uconduct.pets && program_state.in_moveloop) {
         ...
         * unique monster */
        livelog_printf(LL_CONDUCT, "обзавелся %s первым питомцем (%s)",
...
pick_familiar_pm(struct obj *otmp, boolean quietly)
{
    ...
    if (otmp) { /* figurine; otherwise spell */
        ...
        if (!pm && !quietly)
            There("кажется, ничего нет для духа.");
    }
    return pm;
}
    ...
    do {
        ...
        if (otmp) { /* figurine */
            if (!mtmp) {
                ...
                if (!quietly)
                    pline(
                           "Статуэтка извивается и затем разбивается на куски!");
                break;
    ...
    if (otmp) { /* figurine; resulting monster might not become a pet */
        ...
        if (chance > 0) {
            ...
            if (chance == 2) {  /* hostile (cursed figurine) */
                if (!quietly)
                    You("испытываете дурное предчувствие.");
                mtmp->mpeaceful = 0;
                set_malign(mtmp);
            }
        }
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp2) {
        ...
        if (((monnear(mtmp, u.ux, u.uy) && levl_follower(mtmp))
            ...
            if (mtmp == u.usteed) {
                ...
                if (canseemon(mtmp))
                    pline_mon(mtmp, "%s всё ещё %s.", Monnam(mtmp),
                             mtmp->meating ? "ест" : "в ловушке");
                stay_behind = TRUE;
            } else if (mon_has_amulet(mtmp)) {
                if (canseemon(mtmp))
                    pline("%s на мгновение выглядит очень дезориентированным.",
                          Monnam(mtmp));
                stay_behind = TRUE;
            }
            if (stay_behind) {
                if (mtmp->mleashed) {
                    pline("%s поводок внезапно ослабевает.",
                          humanoid(mtmp->data)
                              ? (mtmp->female ? "Her" : "His")
                              : "Its");
                    m_unleash(mtmp, FALSE);
            ...
            pline("%s поводок провисает.", s_suffix(Monnam(mtmp)));
            m_unleash(mtmp, FALSE);
        }
    }
}
        ...
        if (!quietly && cansee(mtmp->mx, mtmp->my)) {
            if (haseyes(gy.youmonst.data)) {
                if (haseyes(mtmp->data))
                    pline_mon(mtmp,
                             "%s %s посмотреть вам в %s.", Monnam(mtmp),
                             mtmp->mpeaceful ? "seems unable" : "refuses",
                             body_part(EYE));
                else
                    pline_mon(mtmp, "%s избегает вашего взгляда.", Monnam(mtmp));
            }
        }
    } else {
