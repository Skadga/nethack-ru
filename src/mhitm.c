...
static const char brief_feeling[] =
    "имеете %s предчувствие на мгновение, затем оно проходит.";
...
noises(struct monst *magr, struct attack *mattk)
{
    ...
    if (!Deaf && (farq != gf.far_noise || svm.moves - gn.noisetime > 10)) {
        ...
        You_hear("%s%s.",
                 (mattk->aatyp == AT_EXPL) ? "взрыв" : "какие-то шумы",
                 farq ? " вдали" : "");
    }
}
    ...
    struct attack *mattk) /* attack and damage types */
{
    ...
    if (gv.vis) {
        pline("%s %s %s.", Monnam(magr),
              (magr->mcan || !could_seduce(magr, mdef, mattk)) ? "промахивается"
                  : "притворяется дружелюбным с",
            ...
            if (monnear(mtmp, mon->mx, mon->my)) {
                if (!u.uswallow && (mtmp == u.ustuck)) {
                    if (!rn2(4)) {
                        ...
                        pline("%s отпускает вас!", Monnam(mtmp));
                    } else
                        break;
                }
    ...
    boolean quietly)
{
    ...
    if (touch_petrifies(pd) && !resists_ston(magr)) {
        if (!which_armor(magr, W_ARMG)) {
            ...
            if (!quietly && canspotmon(magr)) {
                if (gv.vis) {
                    pline("%s пытается убрать %s со %s пути.", Monnam(magr),
                          mon_nam(mdef), is_rider(pa) ? "" : mhis(magr));
                }
                pline_mon(magr, "%s превращается в камень!", Monnam(magr));
            }
            monstone(magr);
            if (!DEADMONSTER(magr))
                return M_ATTK_HIT; /* lifesaved */
            else if (magr->mtame && !gv.vis)
                You(brief_feeling, "особенно печальное");
    ...
    if (gv.vis && !quietly)
        pline("%s убирает %s со %s пути!", Monnam(magr), mon_nam(mdef),
        ...
        if (canseemon(mdef) && !sensemon(mdef)) {
            if (Unaware) {
                ...
                if (iflags.last_msg == PLNMSG_HIDE_UNDER
                    ...
                    pline_mon(mdef, "%s выходит из укрытия.", Monnam(mdef));
                else if (mdef->m_id == gl.last_hider)
                    You("замечаете %s.", mon_nam(mdef));
                else
                    pline("Внезапно вы замечаете %s.", a_monnam(mdef));
            }
        }
    }
    ...
    for (i = 0; i < NATTK; i++) {
                ...
                if ((mdef->data == &mons[PM_BLACK_PUDDING]
                    ...
                    if ((mclone = clone_mon(mdef, 0, 0)) != 0) {
                        if (gv.vis && canspotmon(mdef))
                            pline("%s делится, когда %s ударяет его!",
        ...
        case AT_ENGL:
            if (mdef->data == &mons[PM_SHADE]) { /* no silver teeth... */
                if (gv.vis)
                    pline("%s попытка поглотить %s тщетна.",
    ...
    struct attack *mattk)
{
    if ((unsolid(mdef->data) || gn.notonhead)
        ...
        if ((gv.vis && canspotmon(mdef)) /* mon-vs-mon */
            ...
            const char *verb = (mattk->adtyp == AD_DGST) ? "глотает"
                               : (mattk->adtyp == AD_STCK) ? "прилипает"
                                 : "хватает";
            ...
            Strcpy(magrnam, (magr == &gy.youmonst) ? "Ваш"
                                                   : s_suffix(Monnam(magr)));
            if (!tailmiss) {
                Strcpy(mdefnam, (mdef == &gy.youmonst) ? "вы"
                                                       : mon_nam(mdef));
            } else {
                /* hero poly'd into long worm can't grow tail
                   so no 'youmonst' handling is needed here */
                Sprintf(mdefnam, "%s хвост", s_suffix(some_mon_nam(mdef)));
            }
            /* unsolid grab misses are actually somewhat iffy--how come
               ordinary attacks don't also pass right through? */
            pline("%.99s %s попытка %s %.99s!", magrnam, verb,
                  !tailmiss ? "проходит насквозь" : "не может удержать",
                  mdefnam);
        }
        return TRUE;
    }
    ...
    if (gv.vis) {
        ...
        if (compat) {
            Snprintf(buf, sizeof buf, "%s %s", magr_name,
                    mdef->mcansee ? "улыбается" : "разговаривает с");
            pline("%s %s %s.", buf, mon_nam(mdef),
                  (compat == 2) ? "увлекательно" : "соблазнительно");
        } else {
            buf[0] = '\0';
            switch (mattk->aatyp) {
            case AT_BITE:
                Snprintf(buf, sizeof buf, "%s кусает", magr_name);
                break;
            case AT_STNG:
                Snprintf(buf, sizeof buf, "%s жалит", magr_name);
                break;
            case AT_BUTT:
                Snprintf(buf, sizeof buf, "%s бодает", magr_name);
                break;
            case AT_TUCH:
                Snprintf(buf, sizeof buf, "%s касается", magr_name);
                break;
            case AT_TENT:
                Snprintf(buf, sizeof buf, "%s щупальца сосут",
                         s_suffix(magr_name));
                break;
            case AT_HUGS:
                if (magr != u.ustuck) {
                    Snprintf(buf, sizeof buf, "%s сжимает", magr_name);
                    break;
                }
                FALLTHROUGH;
                /*FALLTHRU*/
            default:
                if (!weaponhit || !mwep || !mwep->oartifact)
                    Snprintf(buf, sizeof buf, "%s ударяет", magr_name);
                break;
            }
            if (*buf)
            ...
            if (mon_hates_silver(mdef) && silverhit) {
                ...
                if (!noncorporeal(mdef->data) && !amorphous(mdef->data)) {
                    if (mdef != magr) {
                        ...
                        (void) strsubst(mdef_name, "себя", "свой");
                        (void) strsubst(mdef_name, "себя", "своя");
                        (void) strsubst(mdef_name, "себя", "свой");
                    }
                    Strcat(mdef_name, " плоть");
                }

                pline("%s %s обжигает %s!", magr_name, /* s_suffix(magr_name), */
...
gazemm(struct monst *magr, struct monst *mdef, struct attack *mattk)
{
    ...
    if (gv.vis) {
        Sprintf(buf, "%s пристально смотрит %s",
                altmesg ? Adjmonnam(magr, "ослеплён") : Monnam(magr),
                altmesg ? "по направлению к" : "в");
        pline("%s %s...", buf,
              canspotmon(mdef) ? mon_nam(mdef) : "что-то");
    }
    ...
    if (magr->mcan || !mdef->mcansee
        ...
        if (gv.vis && canspotmon(mdef))
            pline("но ничего не происходит.");
        return M_ATTK_MISS;
    }
    /* call mon_reflects 2x, first test, then, if visible, print message */
    if (magr->data == &mons[PM_MEDUSA] && mon_reflects(mdef, (char *) 0)) {
        if (canseemon(mdef))
            (void) mon_reflects(mdef, "Взгляд отражается прочь от %s %s.");
        if (mdef->mcansee) {
            ...
            if (mdef->minvis && !perceives(magr->data)) {
                if (canseemon(magr)) {
                    pline(
                      "%s не замечает, что %s взгляд отразился.",
                          Monnam(magr), mhis(magr));
                }
                return M_ATTK_MISS;
            }
            if (canseemon(magr))
                pline_mon(magr, "%s обращён в камень!", Monnam(magr));
            monstone(magr);
            if (!DEADMONSTER(magr))
                return M_ATTK_MISS;
            return M_ATTK_AGR_DIED;
    ...
    struct attack *mattk)
{
    ...
    if (gv.vis) {
        pline("%s %s %s.", Monnam(magr),
              digests(magr->data) ? "проглатывает"
              : enfolds(magr->data) ? "заключает"
                : "поглощает",
              mon_nam(mdef));
    }
    ...
    if (is_vampshifter(mdef)
        ...
        if (gv.vis) {
            ...
            pline("%s извергает %s.", Monnam(magr),
                  canspotmon(mdef) ? "его" : something);
            if (canspotmon(mdef)) {
    ...
    if ((status & (M_ATTK_AGR_DIED | M_ATTK_DEF_DIED))
        ...
        if (cansee(dx, dy)) {
            pline("%s %s!", Monnam(mdef),
                  digests(magr->data) ? "извергнут"
                    : enfolds(magr->data) ? "отпущен"
                      : "изгнан");
        }
...
explmm(struct monst *magr, struct monst *mdef, struct attack *mattk)
{
    ...
    if (cansee(magr->mx, magr->my))
        pline_mon(magr, "%s взрывается!", Monnam(magr));
    else
        noises(magr, mattk);
    ...
    if (magr->mtame) /* give this one even if it was visible */
        You(brief_feeling, "меланхоличное");
...
{
    static const char freaky[] = " подвергается странной метаморфозе";
    ...
    if (mdef == &gy.youmonst) {
        if (Antimagic) {
            ...
            if (u.ulycn == NON_PM) {
                You("подвергаетесь странной метаморфозе.");
                polyself(POLY_NOFLAGS);
            } else if (u.umonnum != u.ulycn) {
                You_feel("неестественное влечение нарастает.");
                you_were();
            } else {
                You_feel("естественное влечение нарастает.");
                you_unwere(FALSE);
            }
            dmg = 0;
        }
        ...
        if (resists_magm(mdef)) {
            ...
            if (gv.vis) { /* either seen or adjacent */
                boolean was_seen = !!strcmpi("Оно", Before),
                        verbosely = flags.verbose || !was_seen;

                if (canspotmon(mdef))
                    pline("%s%s%s превращается в %s.", Before,
                          verbosely ? freaky : "", verbosely ? " и" : "",
                          x_monnam(mdef, ARTICLE_A, (char *) 0,
                                   (SUPPRESS_NAME | SUPPRESS_IT
                                    | SUPPRESS_INVISIBLE), FALSE));
...
slept_monst(struct monst *mon)
{
    if (helpless(mon) && mon == u.ustuck
        ...
        pline_mon(mon, "%s хватка ослабевает.", s_suffix(Monnam(mon)));
        unstuck(mon);
    }
}
    ...
    struct obj *otemp)  /* attacker's weapon */
{
    if (flags.verbose && !Blind && mon_visible(magr)) {
        ...
        pline("%s %s %s%s %s в %s.", Monnam(magr), mswings_verb(otemp, bash),
              (otemp->quan > 1L) ? "один из " : "", mhis(magr), xname(otemp),
              mon_nam(mdef));
    }
}
    ...
    struct obj *mwep)
{
    ...
    case AD_ACID:
        if (mhitb && !rn2(2)) {
            ...
            if (canseemon(magr))
                pline("%s забрызган %s %s!", buf,
                      s_suffix(mon_nam(mdef)), hliquid("кислота"));
            if (resists_acid(magr)) {
                if (canseemon(magr))
                    pline("%s не затронут.", Monnam(magr));
                tmp = 0;
            }
        } else
            tmp = 0;
    ...
    if (rn2(3))
        ...
        case AD_PLYS: /* Floating eye */
            ...
            if (mddat == &mons[PM_FLOATING_EYE]) {
                ...
                if (magr->mcansee && haseyes(madat) && mdef->mcansee
                    ...
                    Strcat(buf, " взгляд отражается %s %s.");
                    if (mon_reflects(magr,
                                     canseemon(magr) ? buf : (char *) 0))
                        return (mdead | mhit);
                    Strcpy(buf, Monnam(magr));
                    if (canseemon(magr))
                        pline("%s заморожен взглядом %s!", buf,
                ...
                Strcpy(buf, Monnam(magr));
                if (canseemon(magr))
                    pline("%s заморожен %s.", buf, mon_nam(mdef));
                paralyze_monst(magr, tmp);
                return (mdead | mhit);
            }
            return 1;
        case AD_COLD:
            if (resists_cold(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%s слегка холоден.", Monnam(magr));
            ...
            if (canseemon(magr))
                pline_mon(magr, "%s внезапно очень холоден!", Monnam(magr));
            ...
            if (!magr->mstun) {
                ...
                if (canseemon(magr))
                    pline_mon(magr, "%s %s...", Monnam(magr),
                          makeplural(stagger(magr->data, "пошатывается")));
            }
            tmp = 0;
            break;
        case AD_FIRE:
            if (resists_fire(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%s слегка согрет.", Monnam(magr));
            ...
            if (canseemon(magr))
                pline_mon(magr, "%s внезапно очень горяч!", Monnam(magr));
            break;
        case AD_ELEC:
            if (resists_elec(magr)) {
                if (canseemon(magr)) {
                    pline_mon(magr, "%s слегка покалывает.", Monnam(magr));
            ...
            if (canseemon(magr))
                pline_mon(magr, "%s поражён электричеством!",
                          Monnam(magr));
            break;
        default:
            tmp = 0;
...
xdrainenergym(struct monst *mon, boolean givemsg)
{
    if (mon->mspec_used < 20 /* limit draining */
        ...
        if (givemsg)
            pline_mon(mon, "%s кажется вялым.", Monnam(mon));
    }
}
