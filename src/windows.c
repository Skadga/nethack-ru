                    ...
                    char *class_select)
{
    ...
    if (category == 1 && next_accelerator <= 'z') {
        ...
        any = cg.zeroany;
        any.a_int = (int) ' ';
        Sprintf(buf, "%c  %s", (char) any.a_int, "Все классы предметов");
        ...
        if (!strcmp(prompt, "Что подбирать?")) {
            add_menu_str(win,
                   "Примечание: если ничего не выбрано, подразумевается \"\u0432сё\".");
            ...
            add_menu_str(win, flags.pickup
                        ? "Отключите 'autopickup', чтобы не подбирать ничего."
           : "Включите 'autopickup', чтобы подбирать эти предметы автоматически.");
        }
    }
    end_menu(win, prompt);
