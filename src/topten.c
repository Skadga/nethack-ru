    ...
    boolean incl_helpless)
{
    static NEARDATA const char *const killed_by_prefix[] = {
        ...
        "погиб от ", "задохнулся от ", "отравлен ", "умер от ",
        /* DROWNING, BURNING, DISSOLVED, CRUSHING, */
        "утонул в ", "сгорел в ", "растворён в ", "раздавлен ",
        /* STONING, TURNED_SLIME, GENOCIDED, */
        "окаменел от ", "превращён в слизь ", "погиб от ",
    ...
    *t0 = zerott;
    ...
    if (wizard || discover) {
        if (how != PANICKED)
            HUP {
                ...
                topten_print("");
                Sprintf(pbuf,
             "Поскольку вы играли в режиме %s, список результатов проверяться не будет.",
                        wizard ? "волшебника" : "исследователя");
                topten_print(pbuf);
            }
        goto showwin;
    }
        ...
        if ((sysopt.pers_is_uid ? t1->uid == t0->uid
            ...
            if (rank0 < 0) {
                ...
                HUP {
                    ...
                    Sprintf(pbuf,
                         "Вы не побили свой предыдущий результат в %ld очков.",
                            t1->points);
                    topten_print(pbuf);
                    topten_print("");
                }
    ...
    if (flg) { /* rewrite record file */
        ...
        if (!done_stopprint)
            if (rank0 > 0) {
                if (rank0 <= 10) {
                    topten_print("Вы попали в список десяти лучших!");
                } else {
                    char pbuf[BUFSZ];

                    Sprintf(pbuf,
                            "Вы заняли %d место в списке %d лучших.",
...
outentry(int rank, struct toptenentry *t1, boolean so)
{
    ...
    hppos = COLNO - (int) (sizeof "  HP [макс]" - sizeof "");
    while (lngr >= hppos) {
