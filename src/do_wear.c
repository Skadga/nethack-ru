...
static NEARDATA const char see_yourself[] = "видеть себя";
static NEARDATA const char unknown_type[] = "Неизвестный тип %s (%d)";
static NEARDATA const char c_armor[] = "доспехи", c_suit[] = "доспех",
                           c_shirt[] = "рубашка", c_cloak[] = "плащ",
                           c_gloves[] = "перчатки", c_boots[] = "ботинки",
                           c_helmet[] = "шлем", c_shield[] = "щит",
                           c_weapon[] = "оружие", c_sword[] = "меч",
                           c_axe[] = "топор", c_that_[] = "это";
...
/* plural "пальцы" or optionally "перчатки" */
const char *
fingers_or_gloves(boolean check_gloves)
{
    return ((check_gloves && uarmg)
            ? gloves_simple_name(uarmg) /* "перчатки" or "рукавицы" */
            : makeplural(body_part(FINGER))); /* "пальцы" */
}
...
{
    if (flags.verbose)
        You("были одеты в %s.", doname(otmp));
}
...
on_msg(struct obj *otmp)
{
    /* on_msg() for rings and amulets just shows add-to-invent feedback
       [after caller calls setworn(), for suffix: "(на {левой|правой} руке)"
       or "(надето)"]; eyewear too unless giving verbose message below */
    ...
    if (flags.verbose) {
        ...
        how[0] = '\0';
        if (otmp->otyp == TOWEL)
            Sprintf(how, " вокруг вашей %s", body_part(HEAD));
        You("теперь носите %s%s.",
    ...
    boolean on)
{
    ...
    if (!oldprop /* extrinsic stealth from something else */
        ...
        if (on) {
            if (!is_boots(obj))
                You("двигаетесь очень тихо.");
            else if (Levitation || Flying)
                You("парите незаметно.");
            else
                You("идёте очень тихо.");
        } else {
            boolean riding = (u.usteed != NULL);

            You("%s%s шумные.", riding ? "и " : "точно",
    ...
    boolean on)
{
    ...
    if (!oldprop /* extrinsic displacement from something else */
        ...
        You_feel("что монстрам%s трудно определить ваше местоположение.",
                 on ? "" : " больше не");
    }
}
...
Boots_on(void)
{
    ...
    case WATER_WALKING_BOOTS:
         ...
         * the surface" затем "you finish your dressing maneuver".
         */
    ...
    case SPEED_BOOTS:
        ...
        if (!oldprop && !(HFast & TIMEOUT)) {
            ...
            You_feel("что ускоряетесь%s.",
                     (oldprop || HFast) ? " ещё немного" : "");
        }
        break;
    case ELVEN_BOOTS:
...
Boots_off(void)
{
    ...
    case SPEED_BOOTS:
        if (!Very_fast && !svc.context.takeoff.cancelled_don) {
            makeknown(otyp);
            You_feel("что замедляетесь%s.", Fast ? " немного" : "");
        }
        break;
    case WATER_WALKING_BOOTS:
...
Cloak_on(void)
{
    ...
    case MUMMY_WRAPPING:
        ...
        if ((HInvis || EInvis) && !Blind) {
            newsym(u.ux, u.uy);
            You("можете %s!", See_invisible ? "больше не видеть себя насквозь"
                                         : see_yourself);
        }
        break;
    case CLOAK_OF_INVISIBILITY:
        ...
        if (!oldprop && !HInvis && !Blind) {
            makeknown(uarmc->otyp);
            newsym(u.ux, u.uy);
            pline("Внезапно вы можете %s себя.",
                  See_invisible ? " видеть насквозь" : " не видеть");
        }
        break;
    case OILSKIN_CLOAK:
        pline("%s очень плотно.", Tobjnam(uarmc, "сидит"));
        break;
...
Cloak_off(void)
{
    ...
    case MUMMY_WRAPPING:
        if (Invis && !Blind) {
            newsym(u.ux, u.uy);
            You("можете %s.", See_invisible ? "видеть себя насквозь"
                                         : "больше не видеть себя");
        }
        break;
    case CLOAK_OF_INVISIBILITY:
        if (!oldprop && !HInvis && !Blind) {
            ...
            newsym(u.ux, u.uy);
            pline("Внезапно вы можете %s.",
                  See_invisible ? "больше не видеть себя насквозь"
                                : see_yourself);
        }
        break;
    /* Alchemy smock gives poison _and_ acid resistance */
...
Helmet_on(void)
{
    ...
    case DUNCE_CAP:
        if (uarmh && !uarmh->cursed) {
            if (Blind)
                pline("%s на мгновение.", Tobjnam(uarmh, "вибрирует"));
            else
                pline("%s %s на мгновение.", Tobjnam(uarmh, "светится"),
                      hcolor(NH_BLACK));
            curse(uarmh);
    ...
    boolean voluntary) /* True: taking protective armor off on purpose */
{
    ...
    if (touch_petrifies(&mons[obj->corpsenm]) && !Stone_resistance) {
        ...
        You("%s %s голым %s.",
            (how && is_gloves(how)) ? "теперь держите" : "держите",
            corpse_xname(obj, (const char *) 0, CXN_ARTICLE),
            makeplural(body_part(HAND)));
        /* "снимая" ought to be "снимая" but that makes the
           tombstone text more likely to be truncated */
        if (how)
            Sprintf(hbuf, "%s %s", voluntary ? "снимая" : "теряя",
                    is_gloves(how) ? gloves_simple_name(how)
                    : strsubst(simpleonames(how), "набор ", ""));
        else
            Strcpy(hbuf, "сопротивление истекло");
        Snprintf(kbuf, sizeof kbuf, "%s при держании %s",
    ...
    boolean on_purpose) /* voluntary removal; not applicable for putting on */
{
    ...
    case BLUE_DRAGON_SCALE_MAIL:
        if (puton) {
            if (!Very_fast)
                You("ускоряетесь%s.", Fast ? " ещё немного" : "");
            EFast |= W_ARM;
        } else {
            EFast &= ~W_ARM;
            if (!Very_fast && !svc.context.takeoff.cancelled_don)
...
Armor_on(void)
{
    ...
    if (artifact_light(uarm) && !uarm->lamplit) {
        ...
        if (!Blind)
            pline("%s %s светом %s!",
                  Yname2(uarm), otense(uarm, "начинает"),
...
Armor_off(void)
{
    ...
    if (was_arti_light && !artifact_light(otmp)) {
        end_burn(otmp, FALSE);
        if (!Blind)
            pline("%s светится.", Tobjnam(otmp, "перестаёт"));
    }
...
Amulet_on(struct obj *amul)
{
    ...
    case AMULET_OF_MAGICAL_BREATHING: {
        ...
        if (was_in_poison_gas) {
            ...
            on_msg(uamul);
            on_msg_done = TRUE;
            You("больше не страдаете от отравляющего газа.");
        }
    ...
    case AMULET_OF_MAGICAL_BREATHING:
        ...
        if (Underwater) {
            if (!cant_drown(gy.youmonst.data) && !Swimming) {
                You("внезапно вдыхаете опасное количество %s!",
                    hliquid("воды"));
        ...
        if (region_danger()) {
            /* "дыхание": wouldn't get here otherwise */
            You("дышите отравляющим газом!");
            mkn = TRUE;
        }
        break;
    case AMULET_OF_STRANGULATION:
        ...
        if (Strangled) {
            ...
            disp.botl = TRUE;
            if (Breathless)
                Your("%s больше не сжат!", body_part(NECK));
            else
                You("можете дышать легче!");
            mkn = TRUE;
        }
        break;
    case AMULET_OF_RESTFUL_SLEEP:
...
Ring_on(struct obj *obj)
{
    ...
    case RIN_SEE_INVISIBLE:
        ...
        if (Invis && !oldprop && !HSee_invisible && !Blind) {
            newsym(u.ux, u.uy);
            pline("Внезапно вы прозрачны, но здесь!");
            learnring(obj, TRUE);
        }
        break;
...
Ring_off_or_gone(struct obj *obj, boolean gone)
{
    ...
    if (!(u.uprops[objects[obj->otyp].oc_oprop].extrinsic & mask))
        impossible("Странно... я не знал, что у вас это кольцо.");
    if (gone)
        setnotworn(obj);
    else
    ...
    case RIN_SEE_INVISIBLE:
        ...
        if (Invisible && !Blind) {
            newsym(u.ux, u.uy);
            pline("Внезапно вы не можете видеть себя.");
            learnring(obj, TRUE);
        }
        break;
    case RIN_INVISIBILITY:
        if (!Invis && !BInvis && !Blind) {
            newsym(u.ux, u.uy);
            Your("тело, кажется, проявляется%s.",
                 See_invisible ? " полностью" : "..");
            learnring(obj, TRUE);
        }
        break;
    case RIN_LEVITATION:
...
Blindf_off(struct obj *otmp)
{
    ...
    if (!otmp)
        otmp = ublindf;
    if (!otmp) {
        impossible("Blindf_off без предмета на глаза?");
        return;
    }
    ...
    if (Blind) {
        if (was_blind) {
            /* "всё ещё не можете видеть" makes no sense when removing lenses
               since they can't have been the cause of your blindness */
            if (otmp->otyp != LENSES)
                You("всё ещё не можете видеть.");
        } else {
            changed = TRUE; /* !was_blind */
            /* "Вы носили Очи Верхнего мира." */
            You_cant("видеть что-либо сейчас!");
            /* set ball&chain variables before the hero goes blind */
            if (Punished)
                set_bc(0);
        }
    ...
    struct obj *stolenobj) /* no mesg if stolenobj is already being doffed */
{
    ...
    if (putting_on || otmp != stolenobj) {
        Sprintf(buf, "Вы прекращаете %s %s.",
                putting_on ? "надевание" : "снимая",
                thesimpleoname(otmp));
    } else {
...
armor_or_accessory_off(struct obj *obj)
{
    ...
    if (obj == uskin
        ...
        if (obj != uskin) {
            ...
            Snprintf(why, sizeof why, " не сняв сначала ваш %s",
                     what);
        } else {
            Strcpy(why, "; она вросла");
        }
        You_cant("снять это%s.", why);
        return ECMD_OK;
    }
    ...
    if (obj->owornmask & W_ARMOR) {
         ...
         * ring (on right hand)" желательно, но "you were wearing a
        ...
        impossible("снятие неизвестного аксессуара: %s",
...
dotakeoff(void)
{
    ...
    if (!Narmorpieces && !Naccessories) {
        ...
        if (uskin)
            pline("%s Срослись с вашей кожей!",
                      uskin->otyp >= GRAY_DRAGON_SCALES
                          ? "Чешуя дракона"
                          : "Кольчуга из чешуи дракона");
        else
            pline("Не надето ни доспехов, ни аксессуаров.");
        return ECMD_OK;
    }
    if (Narmorpieces != 1 || ParanoidRemove || gi.item_action_in_progress)
        otmp = getobj("снять", takeoff_ok, GETOBJ_NOFLAGS);
    if (!otmp)
        return ECMD_CANCEL;
...
doremring(void)
{
    ...
    if (!Naccessories && !Narmorpieces) {
        pline("Не надето ни аксессуаров, ни доспехов.");
        return ECMD_OK;
    }
    if (Naccessories != 1 || ParanoidRemove || cmdq_peek(CQ_CANNED))
        otmp = getobj("снять", remove_ok, GETOBJ_NOFLAGS);
    if (!otmp)
        return ECMD_CANCEL;
...
cursed(struct obj *otmp)
{
    ...
    if ((otmp == uwep) ? welded(otmp) : (int) otmp->cursed) {
        ...
        if (Glib && otmp->bknown
            ...
            pline("Несмотря на ваши скользкие %s, вы не можете.",
                  fingers_or_gloves(TRUE));
        else
            You("не можете.  %s проклято.", use_plural ? "Они" : "Оно");
        set_bknown(otmp, 1);
        return 1;
    }
    return 0;
...
armoroff(struct obj *otmp)
{
    ...
    if (delay) {
        ...
        case ARM_SHIRT:
            ...
            break;
        default:
            impossible("Снятие неизвестной брони (%d: %d), задержка %d",
        ...
        if (what) {
            ...
            Snprintf(offdelaybuf, sizeof offdelaybuf,
                     "Вы заканчиваете снимать свой %s.", what);
        ...
        case ARM_SHIRT:
            ...
            break;
        default:
            impossible("Снятие неизвестной брони (%d: %d), без задержки",
                       otmp->otyp, objects[otmp->otyp].oc_armcat);
            break;
        }
        /* We want off_msg() after removing the item to
           avoid "Вы носили ____ (надето)." */
        off_msg(otmp);
    }
...
{
    You("уже носите %s%c", cc, (cc == c_that_) ? '!' : '.');
}

staticfn void
already_wearing2(const char *cc1, const char *cc2)
{
    You_cant("надеть %s, потому что там уже надето %s.", cc1, cc2);
}
...
canwearobj(struct obj *otmp, long *mask, boolean noisy)
{
    ...
    if (verysmall(gy.youmonst.data) || nohands(gy.youmonst.data)) {
        if (noisy)
            You("не можете носить никакую броню в вашем текущем облике.");
        return 0;
    }
    ...
    if (which && cantweararm(gy.youmonst.data)
        ...
        if (noisy)
            pline("%s Не подойдёт вашему телу.", which);
        return 0;
    ...
    if (welded(uwep) && bimanual(uwep) && (is_suit(otmp) || is_shirt(otmp))) {
        if (noisy)
            You("не можете сделать это, держа ваш %s.",
    ...
    if (is_helmet(otmp)) {
        if (uarmh) {
            ...
            if (noisy)
                pline("%s Не наденется поверх ваших рог%s.",
        ...
        if (uarms) {
            ...
            if (noisy)
                You("не можете носить щит, держа двуручное %s.",
            ...
            if (noisy)
                You("не можете носить щит, держа два оружия.");
            err++;
        } else
            *mask = W_ARMS;
        ...
        if (uarmf) {
            ...
            if (noisy)
                You("нет ног..."); /* not body_part(FOOT) */
            err++;
            ...
            /* break_armor() pushes boots off for centaurs, so don't let
               ...
               makeplural(body_part(FOOT)) would yield "задние копыта" here,
               which sounds odd, so use hard-coded "копыта" */
            if (noisy)
                You("слишком много копыт, чтобы носить %s.", c_boots);
            err++;
        } else if (u.utrap
            ...
            if (u.utraptype == TT_BEARTRAP) {
                ...
                if (noisy)
                    Your("%s привязаны к закопанному шару!",
                         body_part(LEG));
            }
            err++;
        } else
        ...
        if (uarmg) {
            ...
            if (noisy)
                You("не можете надеть перчатки поверх вашего %s.",
                    is_sword(uwep) ? c_sword : c_weapon);
            err++;
        } else if (Glib) {
            /* prevent slippery bare fingers from transferring to
               gloved fingers */
            if (noisy)
                Your("%s слишком скользкие, чтобы надеть %s.",
        ...
        if (uarm || uarmc || uarmu) {
            if (uarmu) {
                ...
                if (noisy)
                    You_cant("надеть это поверх вашего %s.",
                             (uarm && !uarmc) ? c_armor
        ...
        if (uarmc) {
            if (noisy)
                You("не можете надеть броню поверх %s.", cloak_simple_name(uarmc));
            err++;
        } else if (uarm) {
            if (noisy)
                already_wearing("какая-то броня");
            err++;
        } else
            *mask = W_ARM;
    } else {
...
accessory_or_armor_on(struct obj *obj)
{
    ...
    if (armor) {
        ...
        if (obj->otyp == HELM_OF_OPPOSITE_ALIGNMENT
            ...
            if (u.ualignbase[A_CURRENT] == u.ualignbase[A_ORIGINAL])
                You("едва избегаете потери всех шансов на вашу цель.");
            else /* converted */
                You("внезапно вас охватывает стыд, и вы передумываете.");
        ...
        if (ring) {
            ...
            if (nolimbs(gy.youmonst.data)) {
                You("не можете заставить кольцо прилипнуть к телу.");
                return ECMD_OK;
            }
            if (uleft && uright) {
                There("больше нет %s%s, чтобы занять.",
                      humanoid(gy.youmonst.data) ? "колец-" : "",
            ...
            if (uleft) {
                ...
                do {
                    Sprintf(qbuf, "Какие %s%s, Правая или Левая?",
                            humanoid(gy.youmonst.data) ? "колец-" : "",
                            body_part(FINGER));
            ...
            if (uarmg && Glib) {
                Your(
              "%s слишком скользкие, чтобы снять их, поэтому вы не можете надеть кольцо.",
            ...
            if (uarmg && uarmg->cursed) {
                res = !uarmg->bknown;
                set_bknown(uarmg, 1);
                You("не можете снять ваши %s, чтобы надеть кольцо.", c_gloves);
            ...
            if (uwep) {
                ...
                if (((mask == RIGHT_RING && URIGHTY)
                    ...
                    You("не можете освободить своё оружие %s, чтобы надеть кольцо.",
                        hand);
            ...
            if (uamul) {
                already_wearing("амулет");
                return ECMD_OK;
            }
        } else if (eyewear) {
            if (!has_head(gy.youmonst.data)) {
                You("нет головы, чтобы надеть %s.", ansimpleoname(obj));
                return ECMD_OK;
            }

            if (ublindf) {
                if (ublindf->otyp == TOWEL)
                    Your("%s уже закрыто полотенцем.",
                         body_part(FACE));
                else if (ublindf->otyp == BLINDFOLD) {
                    if (obj->otyp == LENSES)
                        already_wearing2("линзы", "повязка на глаза");
                    else
                        already_wearing("повязка на глаза");
                } else if (ublindf->otyp == LENSES) {
                    if (obj->otyp == BLINDFOLD)
                        already_wearing2("повязка на глаза", "какие-то линзы");
                    else
                        already_wearing("какие-то линзы");
                } else {
    ...
    if (armor) {
        ...
        if (obj == uarm)
            ...
            panic("броня, надета не как броня? [%08lx]", obj->owornmask);

        delay = -objects[obj->otyp].oc_delay;
        if (delay) {
            nomul(delay);
            gm.multi_reason = "одевание";
            gn.nomovemsg = "Вы заканчиваете процесс одевания.";
        } else {
        ...
        if (ring) {
            ...
            impossible("надевание неожиданного типа аксессуара: %s",
                       safe_typename(obj->otyp));
        }
    }
    return ECMD_TIME;
...
dowear(void)
{
    ...
    if (verysmall(gy.youmonst.data) || nohands(gy.youmonst.data)) {
        pline("Даже не пытайтесь.");
        return ECMD_OK;
    }
    if (uarm && uarmu && uarmc && uarmh && uarms && uarmg && uarmf
        && uleft && uright && uamul && ublindf) {
        /* 'W' message doesn't mention accessories */
        You("уже надет полный комплект брони.");
        return ECMD_OK;
    }
    otmp = getobj("надеть", wear_ok, GETOBJ_NOFLAGS);
...
doputon(void)
{
    ...
    if (uleft && uright && uamul && ublindf
        ...
        Your("%s%s заполнены, и вы уже носите амулет и %s.",
             humanoid(gy.youmonst.data) ? "колец-" : "",
             fingers_or_gloves(FALSE),
             (ublindf->otyp == LENSES) ? "какие-то линзы" : "повязка на глаза");
        return ECMD_OK;
    }
    otmp = getobj("надеть", puton_ok, GETOBJ_NOFLAGS);
    return otmp ? accessory_or_armor_on(otmp) : ECMD_CANCEL;
}
...
*/
    ...
    if (!uarmg && (leftfall || rightfall) && !nolimbs(gy.youmonst.data)) {
        ...
        Your("%s с ваших %s.",
             (leftfall && rightfall) ? "кольца соскальзывают" : "кольцо соскальзывает",
    ...
    if (u.twoweap && otmp) {
        ...
        hand = body_part(HAND);
        which = URIGHTY ? "левой " : "правой ";  /* text for the off hand */
        Your("%s %s%s с ваших %s%s.", otherwep, xfl ? "также " : "",
             otense(otmp, "соскальзывают"), which, hand);
        xfl++;
        wastwoweap = TRUE;
    ...
    if (otmp && otmp->otyp != AKLYS && !welded(otmp)) {
        ...
        if (bimanual(otmp)) {
            ...
            which = URIGHTY ? "правой " : "левой ";
        }
        pline("%s %s%s %s%s с ваших %s%s.",
              !strncmp(thiswep, "труп", 6) ? "Труп" : "Ваш",
              otherwep ? "другое " : "", thiswep, xfl ? "также " : "",
              otense(otmp, "соскальзывают"), which, hand);
        /* xfl++; */
        otmp->quan = savequan;
        setuwep((struct obj *) 0);
...
stuck_ring(struct obj *ring, int otyp)
{
    if (ring != uleft && ring != uright) {
        impossible("stuck_ring: ни левое, ни правое?");
        return (struct obj *) 0;
    }
...
/* also for praying; find worn item that confers "Неизменность" attribute */
struct obj *
unchanger(void)
{
...
select_off(struct obj *otmp)
{
    ...
    *buf = '\0'; /* lint suppression */
    ...
    if (otmp == uright || otmp == uleft) {
        ...
        if (nolimbs(gy.youmonst.data)) {
            pline("Кольцо застряло.");
            return 0;
        }
        glibdummy = cg.zeroobj;
        why = 0; /* the item which prevents ring removal */
        if (welded(uwep) && ((otmp == RING_ON_PRIMARY) || bimanual(uwep))) {
            Sprintf(buf, "освободить оружие %s", body_part(HAND));
            why = uwep;
        } else if (uarmg && (uarmg->cursed || Glib)) {
            Sprintf(buf, "снять ваши %s%s",
                    Glib ? "скользкие " : "", gloves_simple_name(uarmg));
            why = !Glib ? uarmg : &glibdummy;
        }
        if (why) {
            You("не можете %s, чтобы снять кольцо.", buf);
            set_bknown(why, 1);
            return 0;
        }
    }
    ...
    if (otmp == uarmg) {
        if (welded(uwep)) {
            You("не можете снять ваши %s, держа это %s.",
                c_gloves, is_sword(uwep) ? c_sword : c_weapon);
            set_bknown(uwep, 1);
            return 0;
        } else if (Glib) {
            pline("%s %s слишком скользкие, чтобы снять их.",
                  uarmg->unpaid ? "Труп" : "Ваш", /* simplified Shk_Your() */
                  gloves_simple_name(uarmg));
            return 0;
        }
    ...
    if (otmp == uarmf) {
        if (u.utrap && u.utraptype == TT_BEARTRAP) {
            pline("Медвежий капкан мешает вам вытащить вашу %s.",
                      body_part(FOOT));
            return 0;
        } else if (u.utrap && u.utraptype == TT_INFLOOR) {
            You("застряли в %s и не можете вытащить вашу %s.",
    ...
    if (otmp == uarm || otmp == uarmu) {
        ...
        if (uarmc && uarmc->cursed) {
            Sprintf(buf, "снять ваш %s", cloak_simple_name(uarmc));
            why = uarmc;
        } else if (otmp == uarmu && uarm && uarm->cursed) {
            Sprintf(buf, "снять ваш %s", c_suit);
            why = uarm;
        } else if (welded(uwep) && bimanual(uwep)) {
            Sprintf(buf, "освободить ваш %s",
                    is_sword(uwep) ? c_sword : (uwep->otyp == BATTLE_AXE)
                                                   ? c_axe
                                                   : c_weapon);
            why = uwep;
        ...
        if (why) {
            You("не можете %s, чтобы снять %s.", buf, the(xname(otmp)));
            set_bknown(why, 1);
            return 0;
        }
    }
...
do_takeoff(void)
{
    ...
    if (doff->what == W_WEP) {
        if (!cursed(uwep)) {
            setuwep((struct obj *) 0);
            if (was_twoweap)
                You("больше не держите никакое оружие.");
            else
                You("%s.", empty_handed());
        }
    } else if (doff->what == W_SWAPWEP) {
        setuswapwep((struct obj *) 0);
        You("%sбольше не %s.", was_twoweap ? "сейчас " : "",
            was_twoweap ? "держите два оружия сразу"
                        : "приготовили второе оружие");
    } else if (doff->what == W_QUIVER) {
        setuqwep((struct obj *) 0);
        You("больше не приготовлено боеприпасов.");
        ...
        impossible("do_takeoff: снятие %lx", doff->what);
    }
...
take_off(void)
{
    ...
    if (doff->what == 0L) {
        You("заканчиваете %s.", doff->disrobing);
        return 0;
        ...
        impossible("take_off: снятие %lx", doff->what);
        return 0; /* force done */
    }
...
better_not_take_that_off(struct obj *otmp)
{
    ...
    if (corpse
        ...
        Snprintf(buf, sizeof buf,
            "Снять ваш %s, несмотря на то что несёте мёртвого %s?",
...
doddoremarm(void)
{
    ...
    if (svc.context.takeoff.what || svc.context.takeoff.mask) {
        You("продолжаете %s.", svc.context.takeoff.disrobing);
        ...
        You("ничего не надето.");
        return ECMD_OK;
    }

    add_valid_menu_class(0); /* reset */
    if (flags.menu_style != MENU_TRADITIONAL
        || (result = ggetobj("снять", select_off, 0, FALSE,
                             (unsigned *) 0)) < -1)
...
menu_remarm(int retry)
{
    ...
    if (retry) {
        ...
        n = query_category("Предметы какого типа вы хотите снять?",
        ...
        i = ggetobj("снять", select_off, 0, TRUE, &ggofeedback);
    ...
    n = query_objlist("Что вы хотите снять?", &gi.invent,
    ...
    if (n > 0) {
        ...
        There("больше нечего снять или отложить.");
    }
    return 0;
}
...
disintegrate_arm(struct obj *atmp)
{
    ...
    if ((otmp = maybe_destroy_armor(uarmc, atmp, &resistedc)) != 0) {
        urgent_pline("Ваш %s рассыпается и превращается в пыль!",
        ...
        if (otmp->lamplit)
            end_burn(otmp, FALSE);
        urgent_pline("Ваш %s %s в пыль и %s на %s!",
                     /* suit might be "чешуя дракона" so vtense() is needed */
                     suit, vtense(suit, "превращается"), vtense(suit, "падает"),
                     surface(u.ux, u.uy));
    } else if (!resistedc && !resistedsuit
             && (otmp = maybe_destroy_armor(uarmu, atmp, &resisted)) != 0) {
        urgent_pline("Ваш %s рассыпается на мелкие нити и разваливается!",
                     shirt_simple_name(otmp)); /* always "рубашка" */
    } else if ((otmp = maybe_destroy_armor(uarmh, atmp, &resisted)) != 0) {
        urgent_pline("Ваш %s превращается в пыль и уносится!",
                     helm_simple_name(otmp)); /* "шлем" or "шляпа" */
    } else if ((otmp = maybe_destroy_armor(uarmg, atmp, &resisted)) != 0) {
        urgent_pline("Ваши %s исчезают!", gloves_simple_name(otmp));
        losing_gloves = TRUE;
    } else if ((otmp = maybe_destroy_armor(uarmf, atmp, &resisted)) != 0) {
        urgent_pline("Ваши %s распадаются!", boots_simple_name(otmp));
    } else if ((otmp = maybe_destroy_armor(uarms, atmp, &resisted)) != 0) {
        urgent_pline("Ваш %s рассыпается!", shield_simple_name(otmp));
    } else {
...
inaccessible_equipment(
    struct obj *obj,
    const char *verb, /* "окунуть" or "смазать жиром", or null to avoid messages */
...
#define BLOCKSACCESS(x) (anycovering || ((x)->cursed && (x)->bknown))
    ...
    if (obj == uarmu
        ...
        if (verb) {
            ...
            /* if sameprefix, use yname and xname to get "ваш плащ и доспех"
               or "плащ и доспех Манлобби"; otherwise, use yname and yname
               to get "ваш плащ и доспех Манлобби" or vice versa */
