...
quest_info(int typ)
{
    ...
    default:
        impossible("quest_info: недопустимый тип %d", typ);
    }
    return 0;
}
...
convert_arg(char c)
{
    ...
    case 'R':
        ...
        break;
    case 's':
        str = (flags.female) ? "сестра" : "брат";
        break;
    case 'S':
        str = (flags.female) ? "дочь" : "сын";
        break;
    case 'l':
        str = ldrname();
        break;
    ...
    char **rawtext)
{
    ...
    if (!L) {
        if (showerror)
            impossible("com_pager: nhl_init() не удался");
        goto compagerdone;
    }

    if (!nhl_loadlua(L, QTEXT_FILE)) {
        if (showerror)
            impossible("com_pager: файл %s не найден.", QTEXT_FILE);
        goto compagerdone;
    }
    ...
    if (!lua_istable(L, -1)) {
        if (showerror)
            impossible("com_pager: questtext в %s не является таблицей lua",
                       QTEXT_FILE);
        goto compagerdone;
    }
    ...
    if (!lua_istable(L, -1)) {
        if (showerror)
            impossible("com_pager: questtext[%s] в %s не является таблицей lua",
                       section, QTEXT_FILE);
        goto compagerdone;
    }
 ...
 tryagain:
    ...
    if (!lua_istable(L, -1)) {
        ...
        if (showerror) {
            if (!fallback_msgid)
                impossible(
                      "com_pager: questtext[%s][%s] в %s не является таблицей lua",
                           section, msgid, QTEXT_FILE);
            else
                impossible(
           "com_pager: questtext[%s][%s] и [][%s] в %s не являются таблицами lua",
    ...
    if (!text) {
        ...
        if (nelems < 2) {
            if (showerror)
                impossible(
              "com_pager: questtext[%s][%s] в %s не является массивом строк",
