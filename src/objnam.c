...
staticfn boolean ru_inflect_match(const char *, const char *);
staticfn void ru_wish_normalize(char *, const char *, size_t);
staticfn int ru_name_to_mon(const char *, int *);
...
case SCROLL_CLASS:
    ...
    case SPBOOK_CLASS:
        if (otyp != SPE_NOVEL) {
            Strcpy(buf, "книга заклинаний");
        } else {
            Strcpy(buf, !nn ? "книга" : "роман");
            nn = 0;
        }
        break;
    case RING_CLASS:
    ...
    default:
        if (nn) {
            ...
            if (ocl->oc_class == GEM_CLASS)
                Strcat(buf,
                       (ocl->oc_material == MINERAL) ? " камень" : " самоцвет");
            if (un)
    ...
    boolean juice) /* whether or not to append " juice" to the name */
{
    ...
    /* Skip makesingular for Russian fruit names */
    Sprintf(buf, "%s%s",
            (*fruit_nam & 0x80) ? fruit_nam : makesingular(fruit_nam),
            juice ? " сок" : "");
    return buf;
}
    ...
    unsigned cxn_flags) /* bitmask of CXN_xxx values */
{
    ...
    case AMULET_CLASS:
        if (!dknown)
            ...
            xcalled(buf, BUFSZ - PREFIX, "амулет", un);
        else
            Sprintf(buf, "%s амулет", dn);
        break;
    case WEAPON_CLASS:
        if (is_poisonable(obj) && obj->opoisoned)
            Strcpy(buf, "отравленный ");
        FALLTHROUGH;
        /*FALLTHRU*/
    case VENOM_CLASS:
    case TOOL_CLASS:
        ...
        if (typ == LENSES)
            ...
            Strcpy(buf, (obj->spe < 3) ? "влажное " : "мокрое ");
        ...
        if (typ == FIGURINE && omndx != NON_PM) {
            ...
            ConcatF2(buf, 0, " %s%s", just_an(anbuf, pm_name), pm_name);
        } else if (is_wet_towel(obj)) {
            if (wizard)
    ...
    case FOOD_CLASS:
        ...
        if (typ == SLIME_MOLD) {
            ...
            if (!f) {
                ...
                if (pluralize) {
                    ...
                    if (*buf & 0x80) {
                        /* Russian fruit names already singular/plural;
                           ru_makeplural() handles normalization */
        ...
        if (iflags.partly_eaten_hack && obj->oeaten) {
            ...
            Concat(buf, 0, "частично съеденное ");
        }
        if (obj->globby) { /* 5.0 added "medium" to replace no-prefix */
            ConcatF2(buf, 0, "%s %s", (obj->owt <= 100) ? "маленький"
                                      : (obj->owt <= 300) ? "средний"
                                        : (obj->owt <= 500) ? "большой"
                                          : "очень большой",
                     actualn);
    ...
    case ROCK_CLASS:
        if (typ == STATUE && omndx != NON_PM) {
            ...
            Snprintf(buf, bufspaceleft, "%s%s %s%s",
                     (Role_if(PM_ARCHEOLOGIST)
                      && (obj->spe & CORPSTAT_HISTORIC) != 0) ? "историческая "
                       : "",
                     actualn,
            ...
            Strcat(strcpy(buf, "следующий "), actualn); /* "next boulder" */
        ...
        break;
    case BALL_CLASS:
        Sprintf(buf, "%sтяжёлый железный шар",
                (obj->owt > ocl->oc_weight) ? "очень " : "");
        break;
    case POTION_CLASS:
        if (dknown && obj->odiluted)
            Strcpy(buf, "разбавленное ");
        if (nn || un || !dknown) {
            Strcat(buf, "зелье");
            if (!dknown)
                break;
            if (nn) {
                ...
                if (typ == POT_WATER && bknown
                    ...
                    Strcat(buf, obj->blessed ? "святое " : "нечистое ");
                }
                Strcat(buf, actualn);
            } else {
    ...
    case WAND_CLASS:
        if (!dknown)
            ...
            Sprintf(buf, "жезл %s", actualn);
        else if (un)
            xcalled(buf, BUFSZ - PREFIX, "жезл", un);
        else
            Sprintf(buf, "%s жезл", dn);
        break;
    case SPBOOK_CLASS:
        if (typ == SPE_NOVEL) { /* 3.6 tribute */
            if (!dknown)
                ...
                xcalled(buf, BUFSZ - PREFIX, "книга", un);
            else
                Sprintf(buf, "%s книга", dn);
            break;
            /* end of tribute */
        } else if (!dknown) {
            Strcpy(buf, "книга заклинаний");
            ...
            xcalled(buf, BUFSZ - PREFIX, "книга заклинаний", un);
        } else
            Sprintf(buf, "%s книга заклинаний", dn);
        break;
    case RING_CLASS:
        if (!dknown)
            Strcpy(buf, "кольцо");
        else if (nn)
            Sprintf(buf, "кольцо %s", actualn);
        else if (un)
            xcalled(buf, BUFSZ - PREFIX, "кольцо", un);
        else
            Sprintf(buf, "%s кольцо", dn);
        break;
    case GEM_CLASS: {
        const char *rock = (ocl->oc_material == MINERAL) ? "камень" : "самоцвет";
    ...
    if (program_state.gameover && obj->o_id && bufspaceleft > 0) {
        ...
        case CANDY_BAR:
            ...
            if (*lbl)
                ConcatF1(buf, 0, " с надписью \"%s\"", lbl);
            break;
        case HAWAIIAN_SHIRT:
            ConcatF1(buf, 0, " с узором %s",
...
minimal_xname(struct obj *obj)
{
    ...
    if (!strncmp(bufp, "непроклятый ", 12))
        bufp += 12;
...
mshot_xname(struct obj *obj)
{
    ...
    if (gm.m_shot.n > 1 && gm.m_shot.o == obj->otyp) {
        ...
        Sprintf(tmpbuf, "%d-я ", gm.m_shot.i);
...
add_erosion_words(struct obj *obj, char *prefix)
{
    ...
    if (obj->oeroded && !iscrys) {
        switch (obj->oeroded) {
        case 2:
            Strcat(prefix, "сильно ");
            break;
        case 3:
            Strcat(prefix, "основательно ");
            break;
        }
        Strcat(prefix, is_rustprone(obj) ? "ржавое "
                       : is_crackable(obj) ? "треснутое "
                         : "обгоревшее ");
    }
    if (obj->oeroded2 && !iscrys) {
        ...
        Strcat(prefix, is_corrodeable(obj) ? "проржавевшее " : "гнилое ");
    }
    ...
    if (rknown && obj->oerodeproof)
        Strcat(prefix, iscrys ? "укреплённое "
                       : is_rustprone(obj) ? "нержавеющее "
                         : is_corrodeable(obj) ? "некоррозируемое "
                           : is_flammable(obj) ? "огнеупорное "
                             : is_crackable(obj) ? "закалённое " /* hardened */
                               : is_rottable(obj) ? "гнилоустойчивое "
                                 : "");
}
    ...
    unsigned doname_flags) /* special case requests */
{
    ...
    if (!strncmp(bp, "отравленный ", 12) && obj->opoisoned) {
        bp += 12; /* doesn't affect bp_eos or bpspaceleft */
        ispoisoned = TRUE;
    }
    ...
    if (bknown && obj->oclass != COIN_CLASS
         ...
         */
        if (obj->cursed)
            Strcat(prefix, "проклятый ");
        else if (obj->blessed)
            Strcat(prefix, "благословенный ");
    ...
    if (lknown && Is_box(obj)) {
        if (obj->obroken)
            ...
            Strcat(prefix, "сломанный ");
        else if (obj->olocked)
            Strcat(prefix, "запертый ");
        else
            Strcat(prefix, "незапертый ");
    }

    if (obj->greased)
        Strcat(prefix, "смазанное ");
    ...
    if (cknown && Has_contents(obj) && bpspaceleft > 0) {
        ...
        ConcatF1(bp, 0, " (содержит %ld предметов)", itemcount);
    }
    ...
    case ARMOR_CLASS:
        if (obj->owornmask & W_ARMOR) {
            Concat(bp, 0,
                   ...
                   : doffing(obj) ? " (снимается)"
                     : donning(obj) ? " (надевается)"
                       : " (надето)");
            ...
            if (bp_eos[-1] == ')') {
                ...
                if (!Blind && obj->lamplit && artifact_light(obj))
                    ConcatF1(bp, 1, ", %s горит)", arti_light_description(obj));
            }
        }
        FALLTHROUGH;
        /*FALLTHRU*/
    ...
    case TOOL_CLASS:
        ...
        if (obj->otyp == LEASH && obj->leashmon != 0) {
            ...
            if (mlsh && !DEADMONSTER(mlsh)) {
                ConcatF1(bp, 0, " (привязан к %s)", noit_mon_nam(mlsh));
            } else {
                if (mlsh) /*&& DEADMONSTER(mlsh)*/
        ...
        if (obj->otyp == CANDELABRUM_OF_INVOCATION) {
            ...
            Sprintf(suffix, "%s%s", plur(obj->spe),
                    !obj->lamplit ? " прикреплено" : ", горит");
            ConcatF2(bp, 0, " (%d из 7 свечей%s)", obj->spe, suffix);
            break;
            ...
            if (Is_candle(obj)) {
                ...
                if (turns_left < full_burn_time)
                    Strcat(prefix, "частично использованное ");
            }
            if (obj->lamplit)
                Concat(bp, 0, " (горит)");
            break;
 ...
 ring:  /* normal rings reach here 'naturally'; meat ring jumps here */
        if (obj->owornmask & W_RINGR)
            Concat(bp, 0, " (на правой руке");
        if (obj->owornmask & W_RINGL)
            Concat(bp, 0, " (на левой руке");
    ...
    case FOOD_CLASS:
        if (obj->oeaten)
            Strcat(prefix, "частично съеденное ");
        if (obj->otyp == CORPSE) {
    ...
    case CHAIN_CLASS:
        ...
        if (obj->owornmask & (W_BALL | W_CHAIN))
            ConcatF1(bp, 0, " (%s к тебе)",
                     (obj->owornmask & W_BALL) ? "железный шар" : "железная цепь");
        break;
    }
    ...
    if ((obj->owornmask & W_WEP) && !gm.mrg_to_wielded) {
        ...
        if ((obj->quan != 1L
            ...
            /* note: Sting's glow message, if added, will insert text
               in front of "(оружие в руке)"'s closing paren */
            if (bimanual(obj))
                Concat(bp, 0, " (оружие в обеих руках)");
            else
                Concat(bp, 0, URIGHTY ? " (оружие в правой руке)"
                                      : " (оружие в левой руке)");
    ...
    if (obj->owornmask & W_SWAPWEP) {
        if (u.twoweap)
            ...
            Concat(bp, 0, " (запасное оружие; не в руках)");
    }
    ...
    if (iflags.suppress_price || program_state.restoring) {
        ...
        ConcatF2(bp, 0, " (%s, %s)",
                 obj->unpaid ? "не оплачено" : "содержимое", pricebuf);
        ...
        if (price > 0L) {
            ...
            ConcatF2(bp, 0, " (%s, %s)",
                     nochrg ? "содержимое" : "на продажу", pricebuf);
        } else if (nochrg > 0) {
            Concat(bp, 0, " (бесплатно)");
...
killer_xname(struct obj *obj)
{
    ...
    if (obj->otyp == CORPSE) {
        ...
        buf = nextobuf();
        Sprintf(buf, "смертельный слизевой гриб");
    } else {
        buf = xname(obj);
    }
...
{
    /* RU translation: no articles needed */
    *outbuf = '\0';
    return outbuf;
}
...
{
    static const char and_contents[] = " и его содержимое";
    char *p;
    ...
    if (Has_contents(obj)) {
        ...
        if (!obj->no_charge) {
            if (!strncmp(p, "a ", 2))
                ...
                p += 3;
            p = strprepend(p, obj->unpaid ? "неоплаченный " : "ваш ");
        }
...
vtense(const char *subj, const char *verb)
{
    ...
    /* Russian verbs arrive already inflected by their callers */
    if (*verb & 0x80)
 ...
 */
/* Russian nominative-plural support for object names.
 * NetHack's makeplural()/plur() follow English rules and would tack an
 * ASCII "'s"/"s" onto Cyrillic nouns (e.g. "арбалет болт's").  This
 * uses an explicit singular:plural table keyed by the trailing head noun
 * so stacked item names pluralize naturally in Russian.  The table is one
 * space-separated "sg:pl" token, parsed once on first use.  Nouns not in
 * the table are returned unchanged (never given a Latin suffix). */
static char *
ru_makeplural(const char *oldstr)
{
    static const char *table = "";
    static boolean built = FALSE;
    static struct rupl { const char *sg; const char *pl; struct rupl *next; }
            *head = 0;
    char *str, *p, *word, *colon, *space, *buf, *out;
    int prelen;
            ...
            *colon = '\0';
            ...
            if (space) {
                *space = '\0';
                ...
                word = colon + 1 + (int) strlen(colon + 1); /* to NUL */
            }
            if (*p) {
                struct rupl *np = (struct rupl *) alloc(sizeof *np);
                size_t sl = strlen(p), pll = strlen(colon + 1);
                char *block = (char *) alloc((unsigned) (sl + pll + 2));
                Strcpy(block, p);
                Strcpy(block + sl + 1, colon + 1);
                np->sg = block;
                np->pl = block + sl + 1;
                np->next = head;
                head = np;
    ...
    for (struct rupl *cur = head; cur; cur = cur->next) {
        if (!strcmp(cur->sg, word)) {
            prelen = (int) (word - str);
            out = nextobuf();
            Strcpy(out, str);
            out[prelen] = '\0';
...
makeplural(const char *oldstr)
{
    ...
    *str = '\0';
    ...
    Strcpy(str, oldstr);
    /* Russian object names: use the dedicated RU pluralizer above rather
       than English rules, so no ASCII suffix is appended to Cyrillic. */
    if (str[0] & 0x80) {
        char *obufp = ru_makeplural(str);
        if (obufp && obufp != str) {
            Strcpy(str, obufp);
            releaseobuf(obufp);
        }
    ...
    boolean retry_inverted) /* optional extra "of" handling */
{
    ...
    /* Russian: tolerate case inflection of the trailing letter(s) */
    if (ru_inflect_match(u_str, o_str))
        return TRUE;
...
wizterrainwish(struct _readobjnam_data *d)
{
    ...
    for (trap = NO_TRAP + 1; trap < TRAPNUM; trap++) {
        ...
        if ((t = maketrap(x, y, trap)) != 0) {
            ...
            tname = trapname(trap, TRUE);
            pline("%s%s.", An(tname),
                  (trap != MAGIC_PORTAL) ? "" : " в никуда");
        } else {
            pline("Создание %s не удалось.", an(tname));
        }
        return &hands_obj;
    }
    ...
    if (!BSTRCMPI(bp, p - 8, "fountain")) {
        ...
        pline("%sфонтан.", lev->blessedftn ? "волшебный " : "");
        madeterrain = TRUE;
    ...
    /* also matches "molten lava" */
        ...
        del_engr_at(x, y);
        if (!is_dbridge) {
            pline("%s из расплавленной лавы.",
                  (lev->typ == LAVAPOOL) ? "озеро" : "стена");
            if (!(Levitation || Flying) || lev->typ == LAVAWALL)
                pooleffects(FALSE);
        } else {
            dbterrainmesg("Лава", x, y);
        }
        ...
        pline("%s алтарь.", An(align_str(al)));
        madeterrain = TRUE;
        ...
        if (IS_GRAVE(lev->typ)) {
            ...
            pline("%sмогила.", lev->disturbed ? "потревоженная " : "");
            madeterrain = TRUE;
        } else {
            pline("Здесь нельзя разместить могилу.");
            badterrain = TRUE;
        }
        ...
        if (lev->typ == DOOR || lev->typ == SDOOR
            ...
            if (lev->doormask & D_TRAPPED)
                Strcat(dbuf, "западённая ");
            if (lev->doormask & D_LOCKED)
                Strcat(dbuf, "запертая ");
            if (lev->typ == SDOOR) {
                Strcat(dbuf, "секретная дверь");
            } else {
                ...
                if (lev->doormask & D_CLOSED)
                    Strcat(dbuf, "закрытая ");
                if (lev->doormask & D_ISOPEN)
                    Strcat(dbuf, "открытая ");
                if (lev->doormask & D_BROKEN)
                    Strcat(dbuf, "сломанная ");
                if ((lev->doormask & ~D_TRAPPED) == D_NODOOR)
                    Strcat(dbuf, "проём без двери");
                else
                    Strcat(dbuf, "дверь");
            }
            ...
            Strcpy(dbuf, secret ? "секретная дверь" : "дверь");
            pline("%s требует дверь или стену.", upstart(dbuf));
            badterrain = TRUE;
        }
        ...
        if (lev->typ == CORR) {
            ...
            pline("Секретный коридор.");
            madeterrain = TRUE;
        } else {
            pline("Секретный коридор можно создать только на коридоре.");
            badterrain = TRUE;
        }
        ...
        if (oldtyp == ROOM
            ...
            dbterrainmesg("Пол", x, y);
            madeterrain = TRUE;
        } else {
            pline("Комната|пол|земля здесь не разрешены.");
            badterrain = TRUE;
        }
    }
...
dbterrainmesg(
    ...
    coordxy x, coordxy y)
{
    pline("%s %s подъёмный мост.", newtype,
          (levl[x][y].typ == DRAWBRIDGE_UP) ? "перед" : "под");
}
...
/* Russian wish support: does user string 'want' correspond to the
   canonical object name 'have' modulo Russian case inflection
   ("лечения" for "лечение", "защиты" for "защита")?  Latin text is
   left to fuzzymatch(). */
staticfn boolean
ru_inflect_match(const char *want, const char *have)
{
    char a[BUFSZ], b[BUFSZ];
    int alen, blen, common;

    if (!want || !have || !(*want & 0x80) || !(*have & 0x80))
        return FALSE;
    copynchars(a, want, (int) sizeof a - 1);
    copynchars(b, have, (int) sizeof b - 1);
    alen = (int) strlen(a);
    blen = (int) strlen(b);
    if (alen < 2 || blen < 2)
        return FALSE;
    common = (alen < blen) ? alen : blen;
    if (alen != blen) {
        /* allow one or two extra trailing letters on either side */
        if (alen - blen > 2 || blen - alen > 2)
            return FALSE;
        return (strnicmp(a, b, common) == 0);
    }
    /* same length: allow a single differing final letter (the usual
       Russian declension change: "свитка" vs "свиток", "сна" vs "сон") */
    if (strnicmp(a, b, common - 1) != 0)
        return FALSE;
    return (a[common - 1] != b[common - 1]);
}

/* name_to_mon() wrapper that also accepts inflected final words
   ("орка" for "орк", "хобгоблина" for "хобгоблин") */
staticfn int
ru_name_to_mon(const char *str, int *gend)
{
    char buf[BUFSZ];
    int m, len, k;
    ...
    if (!str)
        return NON_PM;
    m = name_to_mon(str, gend);
    if (m != NON_PM)
        return m;
    if (!(*str & 0x80))
        return NON_PM;
    copynchars(buf, str, (int) sizeof buf - 1);
    len = (int) strlen(buf);
    for (k = 1; k <= 2 && len - k > 0; k++) {
        buf[len - k] = '\0';
        m = name_to_mon(buf, gend);
        if (m != NON_PM)
            return m;
    }
    return NON_PM;
...
/* Translate a Russian wish phrase into the English tokens that
   readobjnam()'s parser already understands.  Object names are copied
   through unchanged; they're matched separately against objects[]. */
staticfn void
ru_wish_normalize(char *dst, const char *src, size_t dstsz)
{
    static NEARDATA const char *const ru_exact[][2] = {
        { "святая вода", "holy water" },
        { "священная вода", "holy water" },
        { "неосвящённая вода", "unholy water" },
        ...
        { "синее стекло", "blue glass" },
        { "красное стекло", "red glass" },
        { "зелёное стекло", "green glass" },
        { "желтое стекло", "yellow glass" },
        { "жёлтое стекло", "yellow glass" },
        { "чёрное стекло", "black glass" },
        { "черное стекло", "black glass" },
        { "коричневое стекло", "brown glass" },
        { "жёлто-коричневое стекло", "yellow-brown glass" },
        { "желто-коричневое стекло", "yellow-brown glass" },
        { "желтовато-коричневое стекло", "yellow-brown glass" },
        { "жёлтовато-коричневое стекло", "yellow-brown glass" },
        { "оранжевое стекло", "orange glass" },
        { "фиолетовое стекло", "violet glass" },
        { "пурпурное стекло", "purple glass" },
        { "пара ботинок", "pair of boots" },
        { "пара сапог", "pair of boots" },
        { "пара перчаток", "pair of gloves" },
        { "пара рукавиц", "pair of gloves" },
        { "пара туфель", "pair of shoes" },
        { 0, 0 }
    };
    static NEARDATA const char *const ru_mods[][2] = {
        { "комок ", "glob of " },
        { "комка ", "glob of " },
        { "сгусток ", "glob of " },
        { "сгустка ", "glob of " },
        { "очень ", "very " },
        { "совершенно ", "thoroughly " },
        { "полностью ", "thoroughly " },
        { "благословенное ", "blessed " },
        { "благословенный ", "blessed " },
        { "благословенная ", "blessed " },
        { "благословенные ", "blessed " },
        { "освящённое ", "blessed " },
        { "освящённый ", "blessed " },
        { "освящённая ", "blessed " },
        { "святое ", "blessed " },
        ...
        { "святая ", "blessed " },
        { "святые ", "blessed " },
        { "неблагословенное ", "uncursed " },
        { "неблагословенный ", "uncursed " },
        { "неблагословенная ", "uncursed " },
        { "неблагословенные ", "uncursed " },
        { "проклятое ", "cursed " },
        { "проклятый ", "cursed " },
        { "проклятая ", "cursed " },
        { "проклятые ", "cursed " },
        ...
        { "огромный ", "large " },
        { "огромная ", "large " },
        { "частично съеденное ", "partly eaten " },
        { "частично съеденный ", "partly eaten " },
        { "частично съеденная ", "partly eaten " },
        { "частично съеденные ", "partly eaten " },
        { "надкусанное ", "partly eaten " },
        { "надкусанный ", "partly eaten " },
        { "надкусанная ", "partly eaten " },
        { "надкусанные ", "partly eaten " },
        { "историческое ", "historic " },
        ...
        { "сгоревший ", "burnt " },
        { "сгоревшая ", "burnt " },
        { "потрескавшееся ", "cracked " },
        { "потрескавшийся ", "cracked " },
        { "потрескавшаяся ", "cracked " },
        { "треснутое ", "cracked " },
        { "треснутый ", "cracked " },
        { "треснутая ", "cracked " },
        { "коррозированное ", "corroded " },
        { "коррозированный ", "corroded " },
        { "коррозированная ", "corroded " },
        { "кородированное ", "corroded " },
        { "кородированный ", "corroded " },
        { "прогнившее ", "rotted " },
        { "прогнивший ", "rotted " },
        { "прогнившая ", "rotted " },
        { "гнилое ", "rotted " },
        ...
        { "сгнившее ", "rotted " },
        { "сгнивший ", "rotted " },
        { "разбавленное ", "diluted " },
        { "разбавленный ", "diluted " },
        { "разбавленная ", "diluted " },
        { "неподписанное ", "unlabeled " },
        { "неподписанный ", "unlabeled " },
        { "неподписанная ", "unlabeled " },
        { "непомеченное ", "unlabeled " },
        { "непомеченный ", "unlabeled " },
        { "огнеупорное ", "fireproof " },
        { "огнеупорный ", "fireproof " },
        { "огнеупорная ", "fireproof " },
        { "износостойкое ", "erodeproof " },
        { "износостойкий ", "erodeproof " },
        { "закалённое ", "erodeproof " },
        { "закалённый ", "erodeproof " },
        { "укреплённое ", "erodeproof " },
        { "укреплённый ", "erodeproof " },
        { "оживляющее ", "zombifying " },
        { "оживляющий ", "zombifying " },
        { "зомбирующее ", "zombifying " },
        { "зомбирующий ", "zombifying " },
        { 0, 0 }
    };
    char *o = dst, *end;
    const char *p = src;
    ...
    /* copy leading count / charge sign / spaces verbatim */
    while (*p && (digit(*p) || *p == '+' || *p == '-' || *p == ' ')) {
        if (o < end)
            *o++ = *p;
        p++;
    }
    /* translate leading modifier words */
    for (;;) {
        int matched = 0;

        for (i = 0; ru_mods[i][0]; i++) {
            size_t l = strlen(ru_mods[i][0]), e = strlen(ru_mods[i][1]);

            if (!strnicmp(p, ru_mods[i][0], l)) {
                if ((size_t) (end - o) < e)
                    break;
                (void) memcpy(o, ru_mods[i][1], e);
                o += e;
                p += l;
                matched = 1;
                break;
    ...
    /* whole-phrase synonyms for whatever remains */
    for (i = 0; ru_exact[i][0]; i++)
        if (!strcmpi(p, ru_exact[i][0])) {
            p = ru_exact[i][1];
            break;
        }
    while (*p && o < end)
...
readobjnam_postparse1(struct _readobjnam_data *d)
{
    ...
    if (!strcmpi(d->bp, "glob") || !BSTRCMPI(d->bp, d->bp + i - 5, " glob")
        ...
        d->mntmp = ru_name_to_mon(!d->p ? d->bp
        ...
        /* For valid pudding types, map directly to glob object type */
        if (d->mntmp >= PM_GRAY_OOZE && d->mntmp <= PM_BLACK_PUDDING) {
            d->typ = GLOB_OF_GRAY_OOZE + (d->mntmp - PM_GRAY_OOZE);
            return 2; /*goto typfnd;*/
        }
        /* Non-pudding or unrecognized: fall through as before */
        ...
        if (!strstri(d->bp, "wand ") && !strstri(d->bp, "spellbook ")
            ...
            if ((d->p = strstri(d->bp, "tin of ")) != 0) {
                if (!strcmpi(d->p + 7, "spinach")) {
                    ...
                    d->tvariety = d->tinv;
                    d->mntmp = ru_name_to_mon(d->p + 7 + d->tmp,
                                              &d->mgend);
                }
                d->typ = TIN;
                return 2; /*goto typfnd;*/
            } else if ((d->p = strstri(d->bp, " of ")) != 0
                       && ((d->mntmp = ru_name_to_mon(d->p + 4, &d->mgend))
                           >= LOW_PM))
                *d->p = 0;
        }
    }
    /* Russian: "труп орка", "статуя орка", "статуэтка орка", "яйцо орка" */
    if (d->typ == 0 && d->mntmp == NON_PM && (*d->bp & 0x80)) {
        static NEARDATA const struct {
            const char *word;
            int what;
        } ru_corpse[] = {
            { "труп", CORPSE },
        ...
        for (ci = 0; ci < (int) SIZE(ru_corpse); ci++) {
            size_t wl = strlen(ru_corpse[ci].word);

            if (!strncmpi(d->bp, ru_corpse[ci].word, wl)
                && d->bp[wl] == ' ') {
                char *tail = d->bp + wl + 1;
                int m = ru_name_to_mon(tail, &d->mgend);

                if (m >= LOW_PM) {
                    d->mntmp = m;
                    *(tail - 1) = '\0'; /* keep just the type word */
                    d->typ = ru_corpse[ci].what;
                    return 2; /*goto typfnd;*/
                }
            }
        }
...
readobjnam_postparse2(struct _readobjnam_data *d)
{
    ...
    if (!BSTRCMPI(d->bp, d->p - 6, " stone")
        ...
        } else { /* try to construct canonical form for Russian names */
            static NEARDATA const char *const glasscol_en[] = {
                "white", "blue", "red", "green", "yellow",
                "black", "brown", "yellowbrown", "yellow-brown",
                "yellow brown", "orange", "violet", "purple",
            };
            static NEARDATA const char *const glasscol_ru[] = {
                "белого",       /* белого */
                "синего",       /* синего */
                "красного",  /* красного */
                "зелёного",  /* зелёного */
                ...
                "желтовато-коричневого",
                "желтовато-коричневого",
                "оранжевого",  /* оранжевого */
                "фиолетового",  /* фиолетового */
                "фиолетового",
            };
            char tbuf[BUFSZ], *s_end;
            int ci;

            /* strip trailing " glass" from the color portion */
            s_end = eos(s);
            if (s_end - s >= 6 && !strcmpi(s_end - 6, " glass"))
                *(s_end - 6) = '\0';

            for (ci = 0; ci < (int) SIZE(glasscol_en); ci++)
                if (!strcmpi(s, glasscol_en[ci]))
                    break;
            Sprintf(tbuf, "бесполезный кусок %s стекла",
                    (ci < (int) SIZE(glasscol_en)) ? glasscol_ru[ci] : s);
            Strcpy(d->bp, tbuf);
        }
    }
...
readobjnam_postparse3(struct _readobjnam_data *d)
{
    ...
    /* Russian generic class and subclass words.  Only tried after the
       exact name search fails, so "плащ невидимости" still wins over a
       plain "плащ".  "зелье лечения" is rewritten to just "лечения"
       and re-searched within POTION_CLASS. */
    if (!d->oclass && d->bp && (*d->bp & 0x80)) {
        static NEARDATA const struct {
            const char *word; /* Russian generic word */
            int cls;          /* class to assign */
            int f_o_range;    /* specific subclass, or 0 for any */
            int l_o_range;
        } ru_class[] = {
            { "зелье", POTION_CLASS, 0, 0 },
            { "зелья", POTION_CLASS, 0, 0 },
            { "зелий", POTION_CLASS, 0, 0 },
            { "свиток", SCROLL_CLASS, 0, 0 },
            { "свитка", SCROLL_CLASS, 0, 0 },
            { "свитки", SCROLL_CLASS, 0, 0 },
            { "жезл", WAND_CLASS, 0, 0 },
            { "жезла", WAND_CLASS, 0, 0 },
            { "палочка", WAND_CLASS, 0, 0 },
            { "палочки", WAND_CLASS, 0, 0 },
            { "перстень", RING_CLASS, 0, 0 },
            { "перстня", RING_CLASS, 0, 0 },
            { "кольцо", RING_CLASS, 0, 0 },
            { "кольца", RING_CLASS, 0, 0 },
            { "амулет", AMULET_CLASS, 0, 0 },
            { "амулета", AMULET_CLASS, 0, 0 },
            { "камень", GEM_CLASS, 0, 0 },
            { "камня", GEM_CLASS, 0, 0 },
            { "книга заклинаний", SPBOOK_CLASS, 0, 0 },
            { "инструмент", TOOL_CLASS, 0, 0 },
            { "инструменты", TOOL_CLASS, 0, 0 },
            { "оружие", WEAPON_CLASS, 0, 0 },
            { "доспех", ARMOR_CLASS, 0, 0 },
            { "доспехи", ARMOR_CLASS, 0, 0 },
            { "броня", ARMOR_CLASS, 0, 0 },
            { "брони", ARMOR_CLASS, 0, 0 },
            { "еда", FOOD_CLASS, 0, 0 },
            { "пища", FOOD_CLASS, 0, 0 },
            { "провизия", FOOD_CLASS, 0, 0 },
            { "сумка", TOOL_CLASS, SACK, BAG_OF_TRICKS },
            { "сумки", TOOL_CLASS, SACK, BAG_OF_TRICKS },
            { "лампа", TOOL_CLASS, OIL_LAMP, MAGIC_LAMP },
            { "лампы", TOOL_CLASS, OIL_LAMP, MAGIC_LAMP },
            { "свеча", TOOL_CLASS, TALLOW_CANDLE, WAX_CANDLE },
            { "свечи", TOOL_CLASS, TALLOW_CANDLE, WAX_CANDLE },
            { "рог", TOOL_CLASS, TOOLED_HORN, HORN_OF_PLENTY },
            { "рога", TOOL_CLASS, TOOLED_HORN, HORN_OF_PLENTY },
            { "щит", ARMOR_CLASS, SMALL_SHIELD, SHIELD_OF_REFLECTION },
            { "щита", ARMOR_CLASS, SMALL_SHIELD, SHIELD_OF_REFLECTION },
            { "шляпа", ARMOR_CLASS, FEDORA, DUNCE_CAP },
            { "шляпы", ARMOR_CLASS, FEDORA, DUNCE_CAP },
            { "шапка", ARMOR_CLASS, FEDORA, DUNCE_CAP },
            { "шапки", ARMOR_CLASS, FEDORA, DUNCE_CAP },
            { "шлем", ARMOR_CLASS, ELVEN_LEATHER_HELM, HELM_OF_TELEPATHY },
            { "шлема", ARMOR_CLASS, ELVEN_LEATHER_HELM, HELM_OF_TELEPATHY },
            { "каска", ARMOR_CLASS, ELVEN_LEATHER_HELM, HELM_OF_TELEPATHY },
            { "перчатки", ARMOR_CLASS, LEATHER_GLOVES,
              GAUNTLETS_OF_DEXTERITY },
            { "перчатка", ARMOR_CLASS, LEATHER_GLOVES,
              GAUNTLETS_OF_DEXTERITY },
            { "рукавицы", ARMOR_CLASS, LEATHER_GLOVES,
              GAUNTLETS_OF_DEXTERITY },
            { "рукавица", ARMOR_CLASS, LEATHER_GLOVES,
              GAUNTLETS_OF_DEXTERITY },
            { "ботинки", ARMOR_CLASS, LOW_BOOTS, LEVITATION_BOOTS },
            { "ботинок", ARMOR_CLASS, LOW_BOOTS, LEVITATION_BOOTS },
            { "сапоги", ARMOR_CLASS, LOW_BOOTS, LEVITATION_BOOTS },
            { "сапог", ARMOR_CLASS, LOW_BOOTS, LEVITATION_BOOTS },
            { "туфли", ARMOR_CLASS, LOW_BOOTS, IRON_SHOES },
            { "туфель", ARMOR_CLASS, LOW_BOOTS, IRON_SHOES },
            { "плащ", ARMOR_CLASS, MUMMY_WRAPPING, CLOAK_OF_DISPLACEMENT },
            { "плаща", ARMOR_CLASS, MUMMY_WRAPPING, CLOAK_OF_DISPLACEMENT },
            { "рубашка", ARMOR_CLASS, HAWAIIAN_SHIRT, T_SHIRT },
            { "рубашки", ARMOR_CLASS, HAWAIIAN_SHIRT, T_SHIRT },
            { "меч", WEAPON_CLASS, SHORT_SWORD, KATANA },
            { "меча", WEAPON_CLASS, SHORT_SWORD, KATANA },
            { "серый камень", GEM_CLASS, LUCKSTONE, FLINT },
            { 0, 0, 0, 0 },
        };
        int ci;

        for (ci = 0; ru_class[ci].word; ci++) {
            size_t wl = strlen(ru_class[ci].word);

            if (!strncmpi(d->bp, ru_class[ci].word, wl)
                && (d->bp[wl] == ' ' || d->bp[wl] == '\0')) {
                d->oclass = ru_class[ci].cls;
                if (ru_class[ci].f_o_range) {
                    d->typ = rnd_class(ru_class[ci].f_o_range,
                                       ru_class[ci].l_o_range);
                    return 2; /*goto typfnd;*/
                }
                if (d->bp[wl] == ' ') {
                    (void) memmove(d->bp, d->bp + wl + 1,
                                   strlen(d->bp + wl + 1) + 1);
                    d->p = eos(d->bp);
                    return 6; /*goto retry;*/
                }
                return 4; /*goto any;*/ /* random object of the class */
            }
        }
    }
...
readobjnam(char *bp, struct obj *no_wish)
{
    ...
    /* Russian input: translate modifier words and a few whole-phrase
       synonyms into the English tokens the parser understands, then
       parse from a scratch copy (the original is kept in origbp) */
    {
        const char *q;
        boolean hasru = FALSE;

        for (q = bp; *q; q++)
            if (*q & 0x80) { /* NB: signed char on purpose */
                hasru = TRUE;
                break;
            }
        if (hasru) {
            ru_wish_normalize(d.fruitbuf, bp, sizeof d.fruitbuf);
            ru_wish_normalize(d.globbuf, bp, sizeof d.globbuf);
            d.bp = d.globbuf;
 ...
 typfnd:
    ...
    if ((is_quest_artifact(d.otmp)
        ...
        d.otmp = &hands_obj;
        pline("На мгновение ты ощущаешь %s в своих %s, но оно исчезает!",
...
suit_simple_name(struct obj *suit)
{
    ...
    if (suit) {
        if (Is_dragon_mail(suit))
            return "кольчуга дракона";
        else if (Is_dragon_scales(suit))
            return "чешуя дракона";
        suitnm = OBJ_NAME(objects[suit->otyp]);
        if (strstr(suitnm, "доспех"))
            return "доспех";
        else if (strstr(suitnm, "кольчуга"))
            return "кольчуга";
        else if (strstr(suitnm, "куртка"))
            return "куртка";
    }
    return "доспех";
}
...
helm_simple_name(struct obj *helmet)
{
     ...
     */
    return !hard_helmet(helmet) ? "шляпа" : "шлем";
}
...
{
    static const char gauntlets[] = "рукавицы";
...
shield_simple_name(struct obj *shield)
{
    if (shield) {
        ...
        if (shield->otyp == SHIELD_OF_REFLECTION)
            return shield->dknown ? "серебряный щит" : "гладкий щит";
        /*
