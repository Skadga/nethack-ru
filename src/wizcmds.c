    ...
    for (;;) {
        ...
        if (mtmp) {
            ...
            if (!iflags.menu_requested) {
                ...
                You("%s %s!", nonliving(mtmp->data) ? "уничтожаете" : "убиваете", Mn);
                ...
                pline("%s %s.", upstart(Mn),
                      nonliving(mtmp->data) ? "уничтожен" : "убит");
...
wiz_level_change(void)
{
    ...
    if (newlevel == u.ulevel) {
        ...
        if (u.ulevel == 1) {
            You("уже настолько неопытны, насколько возможно.");
            return ECMD_OK;
        }
        if (newlevel < 1)
            newlevel = 1;
        ...
        if (u.ulevel >= MAXULEV) {
            You("уже настолько опытны, насколько возможно.");
            return ECMD_OK;
        }
        if (newlevel > MAXULEV)
            newlevel = MAXULEV;
...
wiz_fuzzer(void)
{
    if (flags.suppress_alert < FEATURE_NOTICE_VER(3,7,0)) {
        pline("Тестер fuzz заставит NetHack выполнять случайные нажатия клавиш.");
        There("нет обычного способа выйти из этого режима.");
    }
...
wiz_smell(void)
{
    ...
    if (!olfaction(gy.youmonst.data)) {
        You("не способны улавливать запахи в вашем нынешнем облике.");
        return ECMD_OK;
    }
    ...
    do {
        ...
        if (mptr) {
            ...
            You("не чуете там никакого монстра.");
    ...
    d_level *nextlevl) /* default destination for wiz_migrate_mons() */
{
    ...
    if (here + nxtlv + other == 0) {
        pline("Сейчас никакие монстры не мигрируют.");
    } else {
        pline(
...
{
    pline("NHUUID фиая этой игры: { %s }.", svn.nhuuid);
    return ECMD_OK;
}
...
wiz_migrate_mons(void)
{
    ...
    if (tolevel.dnum || tolevel.dlevel)
        ...
        pline("Отсюда туда не добраться.");
