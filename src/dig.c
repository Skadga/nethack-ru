...
mkcavearea(boolean rockit)
{
    ...
    if (rockit) {
        ...
        pline("Крах!  Потолок обрушивается вокруг вас!");
    } else {
        pline("Таинственная сила %s пещеру вокруг вас!",
              (levl[u.ux][u.uy].typ == CORR) ? "создаёт" : "расширяет");
    }
...
{
    const char *verb =
        (madeby == BY_YOU && uwep && is_axe(uwep)) ? "рубить" : "копать";
    ...
    case DIGCHECK_FAIL_AIRLEVEL:
        You("не можете %s воздух.", verb);
        break;
    case DIGCHECK_FAIL_ALTAR:
        pline("Алтарь слишком прочен, чтобы его разломать.");
        break;
    case DIGCHECK_FAIL_BOULDER:
        There("недостаточно места, чтобы %s здесь.", verb);
        break;
    case DIGCHECK_FAIL_ONLADDER:
        pline("Лестница сопротивляется вашим усилиям.");
        break;
    case DIGCHECK_FAIL_ONSTAIRS:
        pline_The("лестница слишком крепка, чтобы %s.", verb);
        break;
    case DIGCHECK_FAIL_THRONE:
        pline("Трон слишком прочен, чтобы его разломать.");
        break;
    case DIGCHECK_FAIL_CANTDIG:
    case DIGCHECK_FAIL_TOOHARD:
    case DIGCHECK_FAIL_UNDESTROYABLETRAP:
        pline_The("здесь слишком трудно %s: %s не поддаётся.", verb, surface(x, y));
        break;
    case DIGCHECK_FAIL_WATERLEVEL:
        pline_The("%s плещется и успокаивается.", hliquid("water"));
        break;
...
dig(void)
{
    ...
    const char *verb = (!uwep || is_pick(uwep)) ? "копать" : "рубить";
    ...
    if (svc.context.digging.down) {
        ...
        if (IS_TREE(lev->typ) && !may_dig(dpx, dpy)
            ...
            pline("Похоже, это дерево окаменело.");
            return 0;
        }
        if (IS_OBSTRUCTED(lev->typ) && !may_dig(dpx, dpy)
            && dig_typ(uwep, dpx, dpy) == DIGTYP_ROCK) {
            pline("Слишком трудно %s %s.",
                  verb, is_db_wall(dpx, dpy) ? "подъёмный мост" : "стену");
            return 0;
        }
    }
    if (Fumbling && !rn2(3)) {
        ...
        case 0:
            if (!welded(uwep)) {
                You("роняете %s.", yname(uwep));
                dropx(uwep);
            } else {
                if (u.usteed)
                    pline("%s отскакивает и бьёт %s!", The(xname(uwep)),
                          mon_nam(u.usteed));
                else
                    pline("Ой!  %s отскакивает и бьёт вас!", The(xname(uwep)));
                set_wounded_legs(RIGHT_SIDE, 5 + rnd(5));
            }
            break;
        case 1:
            Soundeffect(se_bang_weapon_side, 100);
            pline("Бах!  Вы бьёте плашмя %s!",
                  the(xname(uwep)));
            wake_nearby(FALSE);
            break;
        default:
            Your("размах промахивается.");
            break;
        }
        return 0;
    }
        ...
        if (svc.context.digging.effort <= 50
            ...
            if (rnl(7) > (Fumbling ? 1 : 4)) {
                ...
                if (dmg < 1)
                    ...
                    dmg = (dmg + 1) / 2;
                You("ударяете себя по ноге.");
                Sprintf(kbuf, "отрубая собственную ногу");
                losehp(Maybe_Half_Phys(dmg), kbuf, KILLED_BY);
            } else {
                You("разрушаете медвежий капкан %s.",
    ...
    if (svc.context.digging.effort > 100) {
        ...
        if (digtyp == DIGTYP_STATUE
            ...
            if (break_statue(obj))
                digtxt = "Статуя разлетается вдребезги.";
            else
            ...
            if (digtyp == DIGTYP_TREE) {
                ...
                digtxt = "Вам удаётся отколоть немного камня.";
            ...
            digtxt = "Вы проделываете проём в стене.";
        } else if (lev->typ == SDOOR) {
            cvt_sdoor_to_door(lev); /* ->typ = DOOR */
            digtxt = "Вы проламываетесь сквозь секретную дверь!";
            if (!(lev->doormask & D_TRAPPED))
                lev->doormask = D_BROKEN;
        } else if (closed_door(dpx, dpy)) {
            Sprintf(digbuf, "Вы пробиваетесь сквозь дверь %s!",
                    simpleonames(uwep));
            digtxt = digbuf;
            if (shopedge) {
        ...
        if (Is_earthlevel(&u.uz) && !rn2(3)) {
            ...
            if (makemon(&mons[mndx], dpx, dpy, MM_NOMSG))
                pline("Мусор от ваших раскопок оживает!");
        }
 ...
 cleanup:
        ...
        static const char *const d_target[6] = { "",        "камень", "статую",
                                                 "валун", "дверь", "дерево" };
        int dig_target = dig_typ(uwep, dpx, dpy);

        if (IS_WALL(lev->typ) || dig_target == DIGTYP_DOOR) {
            if (*in_rooms(dpx, dpy, SHOPBASE)) {
                pline("Эта %s слишком крепкая, чтобы %s.",
                      IS_DOOR(lev->typ) ? "дверь" : "стена", verb);
                return 0;
            }
        ...
        if (!gd.did_dig_msg) {
            You("бьёте %s со всей силы.", d_target[dig_target]);
            wake_nearby(FALSE);
            gd.did_dig_msg = TRUE;
        }
    }
...
digactualhole(coordxy x, coordxy y, struct monst *madeby, int ttyp)
{
    ...
    in_thru = (ttyp == HOLE ? "сквозь" : "в");
    if (madeby_u) {
        if (x != u.ux || y != u.uy)
            You("выкапываете рядом %s.", tname);
        else
            You("выкапываете %s %s %s.", an(tname), in_thru, surface_type);
    } else if (!madeby_obj && canseemon(madeby)) {
        pline("%s выкапывает %s %s %s.", Monnam(madeby), an(tname), in_thru,
              surface_type);
    } else if (cansee(x, y) && flags.verbose) {
        if (IS_STWALL(old_typ))
            pline_The("%s рассыпается в %s.", surface_type, an(tname));
        else
            pline("%s появляется в %s.", An(tname), surface_type);
    }
    if (IS_FURNITURE(old_typ) && cansee(x, y))
        pline_The("%s проваливается в %s!", furniture, tname);
    ...
    if (ttyp == PIT) {
        if (shopdoor && heros_fault)
            pay_for_damage("разрушать", FALSE);
        else
        ...
        if (at_u) {
            ...
            if (is_flyer(mtmp->data) || is_floater(mtmp->data)) {
                if (canseemon(mtmp))
                    pline("%s %s над ямой.", Monnam(mtmp),
                          (is_flyer(mtmp->data)) ? "пролетает" : "парит");
            } else if (mtmp != madeby)
        ...
        if (at_u) {
            ...
            if (u.ustuck || wont_fall) {
                ...
                if (*u.ushops && heros_fault)
                    ...
                    pay_for_damage("врываться в", TRUE);
                You("проваливаетесь...");
            ...
            if (mtmp) {
                ...
                if (teleport_pet(mtmp, FALSE)) {
                    ...
                    if (Is_stronghold(&u.uz)) {
                        ...
                        if (canseemon(mtmp))
                            pline("%s избегает ловушки.", Monnam(mtmp));
                        return;
                    } else {
...
dighole(boolean pit_only, boolean by_magic, coord *cc)
{
    ...
    if ((ttmp && (undestroyable_trap(ttmp->ttyp) || nohole))
        ...
        pline_The("здесь слишком трудно копать.");
        ...
        pline_The("%s яростно всплескивается на мгновение, затем успокаивается.",
        ...
        if (pit_only) {
            pline("Подъёмный мост, похоже, слишком прочен для копания.");
        } else {
        ...
        if (ttmp && is_pit(ttmp->ttyp)
            && rn2(2)) {
            pline_The((dig_x != u.ux || dig_y != u.uy) ? "валун оседает в соседнюю яму."
                                                       : "валун оседает в яму.");
            ...
            pline("КАДУМ!  Валун падает внутрь!");
            wake_nearby(FALSE);
        ...
        if (typ == ROOM) {
            ...
            liquid_flow(dig_x, dig_y, typ, ttmp,
                        "Пока вы копаете, яма заполняется %s!");
            retval = TRUE;
        }
...
dig_up_grave(coord *cc)
{
    ...
    if (Role_if(PM_ARCHEOLOGIST)) {
        ...
        You_feel("как подлый грабитель могил!");
    } else if (Role_if(PM_SAMURAI)) {
        adjalign(-sgn(u.ualign.type));
        You("тревожите почтенных покойников!");
    } else if (u.ualign.type == A_LAWFUL) {
        if (u.ualign.record > -10)
            adjalign(-1);
        You("осквернили святость этой могилы!");
    }
    ...
    case 0:
    case 1:
        You("откапываете труп.");
        if ((otmp = mk_tt_object(CORPSE, dig_x, dig_y)) != 0)
            otmp->age -= (TAINT_AGE + 1); /* this is an *OLD* corpse */
        break;
    case 2:
        if (!Blind)
            pline("%s!", Hallucination ? "Чувак!  Восставшие мертвецы"
                                       : "Хозяин могилы крайне расстроен");
        (void) makemon(mkclass(S_ZOMBIE, 0), dig_x, dig_y, MM_NOMSG);
        break;
    case 3:
        if (!Blind)
            pline("%s!", Hallucination ? "Хочу свою мумию!"
                                       : "Вы потревожили гробницу");
        (void) makemon(mkclass(S_MUMMY, 0), dig_x, dig_y, MM_NOMSG);
        break;
    default:
        /* No corpse */
        pline("Могила не занята.  Странно...");
        break;
    }
...
use_pick_axe(struct obj *obj)
{
    ...
    /* Check tool */
    if (obj != uwep) {
        if (wield_tool(obj, "размахнуться")) {
    ...
    ispick = is_pick(obj);
    verb = ispick ? "копать" : "рубить";

    if (u.utrap && u.utraptype == TT_WEB) {
        pline("%s вы не можете %s, запутавшись в паутине.",
              /* res==0 => no prior message;
                 res==1 => just got "You now wield a pick-axe." message */
              !res ? "К сожалению," : "Но", verb);
        return res;
    }
    ...
    *dsp = 0;
    Sprintf(qbuf, "В каком направлении вы хотите %s? [%s]", verb, dirsyms);
    if (!getdir(qbuf))
...
use_pick_axe2(struct obj *obj)
{
    ...
    const char *verbing = ispick ? "копать" : "рубить";

    if (u.uswallow && do_attack(u.ustuck)) {
        ; /* return 1 */
    } else if (Underwater) {
        pline("Бурление препятствует вашим попыткам %s.", verbing);
    } else if (u.dz < 0) {
        if (Levitation)
            You("у вас недостаточно рычага.");
        else
            You_cant("дотянуться до потолка.");
        ...
        if (dam <= 0)
            dam = 1;
        You("бьёте себя %s.", yname(uwep));
        Sprintf(buf, "собственной %s", OBJ_NAME(objects[obj->otyp]));
        ...
        if (dig_target == DIGTYP_UNDIGGABLE) {
            ...
            if (trap && trap->ttyp == WEB) {
                ...
                pline("%s запуталось в паутине.", Yobjnam2(obj, "запуталось"));
                /* you ought to be able to let go; tough luck */
                /* (maybe `move_into_trap()' would be better) */
                nomul(-d(2, 2));
                gm.multi_reason = "застряли в паутине";
                gn.nomovemsg = "Вы высвобождаетесь.";
                ...
                const char *what = boulder ? "валун" : "статуя";

                if (!ispick) {
                    boolean vibrate = !rn2(3);

                    pline("Искры летят, когда вы бьёте %s.%s", what,
                ...
                if (idx != DIR_ERR) {
                    ...
                    You("расчищаете немного мусора между ямами.");
                }
            } else if (u.utrap && u.utraptype == TT_PIT
                       && (trap_with_u = t_at(u.ux, u.uy)) != 0) {
                You("размахиваете %s, но щебню некуда деваться.",
                    yobjnam(obj, (char *) 0));
            } else {
                You("размахиваете %s в воздухе.", yobjnam(obj, (char *) 0));
            }
        } else {
            static const char *const d_action[6] = { "махать", "копать",
                                                     "дробить статую",
                                                     "бить по валуну",
                                                     "рубить дверь",
                                                     "рубить дерево" };
            ...
            if (svc.context.digging.pos.x != rx
                ...
                if (!svc.context.digging.quiet)
                    You("начинаете %s.", d_action[dig_target]);
            } else {
                You("%s %s.", svc.context.digging.chew ? "начинаете" : "продолжаете",
                    d_action[dig_target]);
        ...
        You("не можете долго пробыть под %s.",
            is_pool(u.ux, u.uy) ? "водой" : " лавой");
        ...
        if (!u.utrap)
            cant_reach_floor(u.ux, u.uy, FALSE, TRUE, FALSE);
               ...
               && (!trap || (trap->ttyp != LANDMINE
                             && trap->ttyp != BEAR_TRAP))) {
        pline("%s лишь царапает %s.", Yobjnam2(obj, (char *) 0),
              surface(u.ux, u.uy));
        u_wipe_engr(3);
    } else {
        if (svc.context.digging.pos.x != u.ux
            ...
            assign_level(&svc.context.digging.level, &u.uz);
            svc.context.digging.effort = 0;
            You("начинаете %s вниз.", verbing);
            if (*u.ushops) {
                shopdig(0);
                add_damage(u.ux, u.uy, SHOP_PIT_COST);
            }
        } else
            You("продолжаете %s вниз.", verbing);
        gd.did_dig_msg = FALSE;
        set_occupation(dig, verbing, 0);
    }
    return ECMD_TIME;
    ...
    if (in_town(x, y)
        ...
        if (mtmp) {
            ...
            if (zap || svc.context.digging.warned) {
                verbalize("Стой, вандал!  Ты арестован!");
                ...
                if (IS_DOOR(lev->typ))
                    ...
                    str = "фонтан";
                verbalize("Эй, перестань портить %s!", str);
...
mdig_tunnel(struct monst *mtmp)
{
    ...
    if (IS_WALL(here->typ)) {
        ...
        if (flags.verbose && !rn2(5)) {
            ...
            You_hear("обрушивающийся камень.");
        }
        if (*in_rooms(mtmp->mx, mtmp->my, SHOPBASE))
            add_damage(mtmp->mx, mtmp->my, 0L);
        if (svl.level.flags.is_maze_lev) {
    ...
    if (unexpected) {
        ...
        if (!Hallucination) {
            ...
            static const char *const draft_reaction[] = {
                "зачисляетесь", "маршируете", "протестуете", "убегаете",
            };
            int dridx;
            ...
            You_feel("словно %s.", draft_reaction[dridx]);
        }
    }
}
...
zap_dig(void)
{
    ...
    if (u.uswallow) {
        ...
        if (!is_whirly(mtmp->data)) {
            if (digests(mtmp->data))
                You("пробиваете брюхо %s!", mon_nam(mtmp));
        ...
        if (!Is_airlevel(&u.uz) && !Is_waterlevel(&u.uz) && !Underwater) {
            if (u.dz < 0 || On_stairs(u.ux, u.uy)) {
                ...
                if (On_stairs(u.ux, u.uy)) {
                    ...
                    pline_The("луч отскакивает от %s и попадает в %s.",
                              stway->isladder ? "лестницу" : "ступени",
                              "потолок");
                }
        ...
        if (pitdig) { /* we are already in a pit if this is true */
            ...
            if (IS_WALL(room->typ)) {
                if (!(room->wall_info & W_NONDIGGABLE)) {
                    ...
                    pline("Стена светится, затем гаснет.");
                break;
                ...
                if (!(room->wall_info & W_NONDIGGABLE)) {
                    ...
                    pline("Дерево вздрагивает, но невредимо.");
                break;
                ...
                if (!(room->wall_info & W_NONDIGGABLE)) {
                    ...
                    pline("Камень светится, затем гаснет.");
                break;
            }
    ...
    if (shopdoor || shopwall)
        pay_for_damage(shopdoor ? "разрушить" : "врываться в", FALSE);
    return;
}
...
adj_pit_checks(coord *cc, char *msg)
{
    ...
    const char *foundation_msg =
                 "Основание слишком прочно, чтобы пробить его под этим углом.";
    ...
    *msg = '\0';
    ...
    if (is_pool(cc->x, cc->y) || is_lava(cc->x, cc->y)) {
        ...
        /* if (room->wall_info & W_NONDIGGABLE) */
        Strcpy(msg, "Корни дерева светятся, затем гаснут.");
        return FALSE;
    } else if (ltyp == STONE || ltyp == SCORR) {
        if (room->wall_info & W_NONDIGGABLE) {
            Strcpy(msg, "Камень светится, затем гаснет.");
            return FALSE;
        }
    } else if (ltyp == IRONBARS) {
        /* "set of iron bars" */
        Strcpy(msg, "Прутья уходят гораздо глубже вашей ямы.");
        return FALSE;
#if 0
        ...
        Strcpy(msg, "Под раковиной остаётся спутанный клубок труб.");
        return FALSE;
    } else if (On_ladder(cc->x, cc->y)) {
        Strcpy(msg, "Лестница остаётся невредимой.");
        return FALSE;
    } else {
        ...
        if (IS_FOUNTAIN(ltyp))
            ...
            supporting = "подъёмного моста";

        if (supporting) {
            Sprintf(msg, "Несущая конструкция %s остаётся неповреждённой.",
                    supporting);
            return FALSE;
        }
    }
...
pit_flow(struct trap *trap, schar filltyp)
{
    ...
    if (trap && filltyp != ROOM && is_pit(trap->ttyp)) {
        ...
        liquid_flow(t.tx, t.ty, filltyp, trap,
                    u_at(t.tx, t.ty)
                        ? "Соседняя яма внезапно заполняется %s!"
                        : (char *) 0);
...
bury_an_obj(struct obj *otmp, boolean *dealloced)
{
    ...
    if (otmp == uball) {
        ...
        pline("Железный шар закапывается!");
    }
...
bury_objs(int x, int y)
{
    ...
    if (costly && loss) {
        You("должны %s %ld %s за закопанный товар.", shkname(shkp), loss,
            currency(loss));
    }
}
...
rot_corpse(anything *arg, long timeout)
{
    ...
    if (on_floor) {
        ...
        if (flags.verbose) {
            ...
            Your("Ваш%s %s %s%c", obj == uwep ? " сжимаемый" : "", cname,
                 otense(obj, "истлел"), obj == uwep ? '!' : '.');
        }
        if (obj->owornmask) {
...
bury_you(void)
{
    ...
    if (!Levitation && !Flying) {
        if (u.uswallow)
            You_feel("ощущение, будто падаете в ловушку!");
        else
...
escape_tomb(void)
{
    ...
    if ((Teleportation || can_teleport(gy.youmonst.data))
        ...
        You("пытаетесь применить заклинание телепортации.");
...
wiz_debug_cmd_bury(void)
{
    ...
    if (before == 0)
        ...
        pline("Нет предметов здесь или рядом, чтобы закопать.");
    else if (diff == 0)
        ...
        pline("Нет закопанных предметов.");
    else
        /* usual case; if uball got buried, uchain went away and won't be
           counted as buried */
        pline("Закопано предметов: %d%s.", diff, plur(diff));
    return ECMD_OK;
}
#endif /* DEBUG */
