...
/* return the masculine or feminine form of an adjective, depending on the
   monster's gender; monsters never have neuter or plural gender here */
staticfn const char *
ru_gender_adj(struct monst *mtmp, const char *m_form, const char *f_form)
{
    return (Mgender(mtmp) == FEMALE) ? f_form : m_form;
}
...
alreadynamed(struct monst *mtmp, char *monnambuf, char *usrbuf)
{
    ...
    if (!*usrbuf) { /* attempt to erase existing name */
        boolean name_not_title = (has_mgivenname(mtmp)
                                  ...
                                  || mtmp->isshk);
        pline("%s предпочитает сохранить своё %s.", upstart(monnambuf),
              name_not_title ? "имя" : "звание");
        return TRUE;
               ...
               || ((p = strstri(monnambuf, "невидимый ")) != 0
                   && fuzzymatch(usrbuf, p + 10, " -_", TRUE))
               || ((p = strstri(monnambuf, "невидимая ")) != 0
                   && fuzzymatch(usrbuf, p + 10, " -_", TRUE))
        ...
        if (is_rider(mtmp->data)) {
            ...
            pline("%s уже так называется.", upstart(monnambuf));
        } else {
            pline("%s уже называется %s.",
        ...
        pline("%s не хочет, чтобы его называли %s.", upstart(monnambuf), usrbuf);
        return TRUE;
    }
    return FALSE;
}
...
do_mgivenname(void)
{
    ...
    if (Hallucination) {
        You("всё равно бы его не узнали.");
        return;
    }
    cc.x = u.ux;
    cc.y = u.uy;
    if (getpos(&cc, FALSE, "монстра, которого хотите назвать") < 0
        || !isok(cc.x, cc.y))
        return;
    cx = cc.x, cy = cc.y;
    ...
    if (u_at(cx, cy)) {
        if (u.usteed && canspotmon(u.usteed)) {
            ...
            pline("Это существо %s называется %s и не может быть переименовано.",
    ...
    if (!do_swallow && (!mtmp
        ...
        pline("Я не вижу там монстра.");
        return;
    }
    /* special case similar to the one in lookat() */
    Sprintf(qbuf, "Как вы хотите назвать %s?",
    ...
    if ((mtmp->data->geno & G_UNIQ) && !mtmp->ispriest) {
        if (!alreadynamed(mtmp, monnambuf, buf))
            pline("%s не любит, когда его так называют!", upstart(monnambuf));
    } else if (mtmp->isshk
               && !(Deaf || helpless(mtmp)
                    || mtmp->data->msound <= MS_ANIMAL)) {
        if (!alreadynamed(mtmp, monnambuf, buf)) {
            SetVoice(mtmp, 0, 80, 0);
            verbalize("Я — %s, а не %s.", shkname(mtmp), buf);
        }
    } else if (mtmp->ispriest || mtmp->isminion || mtmp->isshk
               || mtmp->data == &mons[PM_GHOST] || has_ebones(mtmp)) {
        if (!alreadynamed(mtmp, monnambuf, buf))
            pline("%s не примет имя %s.", upstart(monnambuf), buf);
    } else {
...
do_oname(struct obj *obj)
{
    ...
    if (obj->otyp == SPE_NOVEL) {
        pline("%s уже имеет опубликованное имя.", Ysimple_name2(obj));
        return;
    }

    Sprintf(qbuf, "Что вы хотите назвать %s ",
            is_plural(obj) ? "эти" : "этот");
    (void) safe_qbuf(qbuf, qbuf, "?", obj, xname, simpleonames, "предмет");
    /* use getlin() to get a name string from the player */
    ...
    if ((aname = artifact_name(buf, &objtyp, TRUE)) != 0
        ...
        pline("Когда вы вырезали, ваш %s поскользнулся.", body_part(HAND));
...
docallcmd(void)
{
    ...
    add_menu(win, &nul_glyphinfo, &any, abc ? 0 : any.a_char, 'C',
             ATR_NONE, clr, "монстр", MENU_ITEMFLAGS_NONE);
    if (gi.invent) {
 ...
 docallcmd:
    ...
    case 'i': /* name an individual object in inventory */
        obj = getobj("имя", name_ok, GETOBJ_PROMPT);
        if (obj)
            do_oname(obj);
        break;
    case 'o': /* name a type of object in inventory */
        obj = getobj("назвать", call_ok, GETOBJ_NOFLAGS);
        if (obj) {
            ...
            if (!obj->dknown) {
                You("никогда бы не узнали другой.");
#if 0
            } else if (call_ok(obj) == GETOBJ_EXCLUDE) {
                You("знаете их так же, как всегда.");
#endif
            } else {
                docall(obj);
            }
...
docall(struct obj *obj)
{
    ...
    if (obj->oclass == POTION_CLASS && obj->fromsink)
        ...
        Sprintf(qbuf, "Назовите поток %s жидкости:",
                OBJ_DESCR(objects[obj->otyp]));
    else
        (void) safe_qbuf(qbuf, "Назовите ", ":", obj,
                         docall_xname, simpleonames, "вещь");
    /* pointer to old name */
...
namefloorobj(void)
{
    ...
    Sprintf(buf, "объект на карте (или '.' для того, что %s вас)",
    ...
    if (!obj) {
        /* "под вами" is safe here since there's no object to hide under */
        There("не видно никакого предмета %s.",
              u_at(cc.x, cc.y) ? "под вами" : "рядом");
        return;
    }
    ...
    if (Hallucination) {
        ...
        pline("%s %s называет вас \"%s.\"",
              The(buf), use_plural ? "решают" : "решает",
              unames[rn2_on_display_rng(SIZE(unames))]);
    } else if (call_ok(obj) == GETOBJ_EXCLUDE) {
        pline("%s %s нельзя присвоить имя типа.",
              use_plural ? "Те" : "Тот", buf);
    } else if (!obj->dknown) {
        You("не знаете %s %s достаточно хорошо, чтобы назвать %s.",
            use_plural ? "те" : "тот", buf, use_plural ? "их" : "оно");
    } else {
        docall(obj);
    }
    if (fakeobj) {
 ...
 *
 * If the monster would be referred to as "оно" or if the monster has a name
 ...
 *              would have been "оно", return "кто-то" if humanoid or
 *              "что-то" otherwise.
 *
    ...
    boolean called)
{
    ...
    if (mtmp == &gy.youmonst)
        return strcpy(buf, "вы"); /* ignore article, "invisible", &c */
    ...
    /* unseen monsters, etc.; usually "оно" but sometimes more specific;
    ...
    if (do_it) {
        ...
        Strcpy(buf, !augment_it ? "оно"
                    : (!do_hallu ? s_one : !rn2(2)) ? "кто-то"
                      : "что-то");
        return buf;
    }
    ...
    if (mtmp->isshk && !do_hallu && !do_mappear) {
        if (adjective && article == ARTICLE_THE) {
            ...
            if (mdat != &mons[PM_SHOPKEEPER] || do_invis){
                Strcat(buf, " ");
                if (do_invis)
                    Strcat(buf, ru_gender_adj(mtmp, "невидимый ", "невидимая "));
                Strcat(buf, pm_name);
            }
        }
        return buf;
    ...
    if (do_saddle && (mtmp->misc_worn_check & W_SADDLE) && !Blind
        && !Hallucination)
        Strcat(buf, ru_gender_adj(mtmp, "осёдланный ", "осёдланная "));
    ...
    if (do_hallu) {
      ...
      if (has_ebones(mtmp)) {
        ...
        if (mdat == &mons[PM_GHOST]) {
            Sprintf(eos(buf), "призрак по имени %s", name);
            name_at_start = TRUE;
        } else if (called) {
            Sprintf(eos(buf), "%s по имени %s", pm_name, name);
    ...
    switch (article) {
    case ARTICLE_YOUR:
        Strcpy(buf2, ru_gender_adj(mtmp, "ваш ", "ваша "));
        break;
    case ARTICLE_THE:
        Strcpy(buf2, "");
        break;
...
/* in between noit_mon_nam() and mon_nam(); if the latter would pick "оно",
   use "кто-то" (for humanoids) or "что-то" (for others) instead */
char *
    ...
    char *outbuf)
{
    ...
    if (mon->data == &mons[PM_HIGH_CLERIC] && !Hallucination
        ...
        Strcpy(outbuf, "");
        Strcat(outbuf, mon->female ? "верховная жрица" : "верховный жрец");
    } else {
...
{
    char *verbs, selfbuf[40]; /* sizeof "себя" suffices */

    /* "себя"/"себя"/"себя", maybe "себя" if hallucinating */
    Strcpy(selfbuf, mon_nam_too(mon, mon));
    /* verb starts plural; this will yield singular except for "себя" */
    verbs = vtense(selfbuf, verb);
    if (!strcmp(verb, verbs)) { /* a match indicates that it stayed plural */
        monnamtext = makeplural(monnamtext);
        /* for "оно", makeplural() produces "их" but we want "they" */
...
minimal_monnam(struct monst *mon, boolean ckloc)
{
    ...
    if (!mon) {
        Strcpy(outbuf, "[Пустой монстр]");
    } else if ((ptr = mon->data) == 0) {
        Strcpy(outbuf, "[Пустые данные монстра]");
    } else if (ptr < &mons[0]) {
        Sprintf(outbuf, "[Неверные данные монстра %s < %s]",
        ...
        Sprintf(outbuf, "%s%s <%d,%d>",
                mon->mtame ? "ручной " : mon->mpeaceful ? "мирный " : "",
...
obj_pmname(struct obj *obj)
{
    ...
    return "двуногий искатель-глоркум";
}
...
static NEARDATA const char *const hliquids[] = {
    "йогурт", "ооблек", "сгустки крови", "разбавленная вода", "очищенная вода",
    "растворимый кофе", "чай", "травяной настой", "жидкая радуга",
    "сливочная пена", "глинтвейн", "бульон", "нектар", "грог", "флаббер",
    "кетчуп", "медленный свет", "масло", "винегрет", "жидкий кристалл", "мёд",
    "карамельный соус", "чернила", "внутренняя влага глаза", "заменитель молока",
    "фруктовый сок", "светящаяся лава", "желудочный сок", "минеральная вода",
    "сироп от кашля", "живое серебро", "сладкий купорос", "серая слизь", "розовая слизь",
    "космический латте", "костяное масло", "заварной крем", "смалец", "уксус", "креозот",
    ...
    const char *liquidpref) /* use as-is when not hallucintg (unless empty) */
{
    ...
    /* RU localization: map English liquid selectors to Russian words */
    if (liquidpref && *liquidpref
        && (!strcmp(liquidpref, "water") || !strcmp(liquidpref, "lava")))
        return !strcmp(liquidpref, "water") ? "вода" : "лава";
...
christen_orc(struct monst *mtmp, const char *gang, const char *other)
{
    ...
    if (gang)
        sz += (int) (strlen(gang) + sizeof " из банды " - sizeof "");
    else if (other)
        sz += (int) strlen(other);
    ...
    if (sz < BUFSZ) {
        ...
        if (gang) {
            Sprintf(buf, "%s из банды %s", upstart(orcname),
