...
teleds(coordxy nux, coordxy nuy, int teleds_flags)
{
    ...
    if (is_teleport && flags.verbose)
        You("материализуетесь в %s месте!",
            (nux == u.ux0 && nuy == u.uy0) ? "том же" : "ином");
...
scrolltele(struct obj *scroll)
{
    ...
    if (noteleport_level(&gy.youmonst) && !wizard) {
        pline("Таинственная сила не даёт вам телепортироваться!");
        if (scroll)
            learnscroll(scroll); /* this is obviously a teleport scroll */
        return;
    }

    /* don't show trap if "Извините..." */
    if (!Blinded)
        make_blinded(0L, FALSE);

    if ((u.uhave.amulet || On_W_tower_level(&u.uz)) && !rn2(3)) {
        You_feel("на мгновение чувствуете дезориентацию.");
        ...
        if (!wizard || y_n("Переопределить?") != 'y')
            return;
    }
    if (((Teleport_control || (scroll && scroll->blessed)) && !Stunned)
        || wizard) {
        if (unconscious()) {
            pline("Будучи без сознания, вы не можете управлять своей телепортацией.");
        } else {
            char whobuf[BUFSZ];

            Strcpy(whobuf, "вы");
            if (u.usteed)
                Sprintf(eos(whobuf), " и %s", mon_nam(u.usteed));
            pline("Куда %s хочет телепортироваться?", whobuf);
            if (scroll)
                learnscroll(scroll);
            cc.x = u.ux;
            cc.y = u.uy;
...
#define REMOVESPELL 4
    ...
    if (!iflags.menu_requested) {
        ...
        static const struct tporttypes {
             ...
             */
            { 'n', "обычный ^T по команде; без заклинания, с учётом ограничений" },
            { 's', "через наложение заклинания; без врождённой телепортации" },
            { 't', "попробовать ^T без умения; без заклинания" },
            { 'w', "режим отладки; игнорировать ограничения" }, /* trad wizard mode */
        };
        ...
        end_menu(win, "Каким способом вы хотите телепортироваться?");
    ...
    boolean break_the_rules) /* True: wizard mode ^T */
{
    ...
    if (trap) {
        if (trap->ttyp == LEVEL_TELEP && trap->tseen) {
            if (y_n("Здесь находится телепортер уровня. Запустить его?") == 'y') {
            ...
            if (trap->once) {
                pline("Это телепорт хранилища, можно использовать только один раз.");
                if (y_n("Прыгнуть?") == 'n') {
                    trap = 0;
                } else {
                    deltrap(trap);
            ...
            if (trap)
                You("%s на телепортационную ловушку.", u_locomotion("jump"));
        } else
            trap = 0;
    }
        ...
        if (!Teleportation || (u.ulevel < (Role_if(PM_WIZARD) ? 8 : 12)
            ...
            if (!castit && !break_the_rules) {
                You("%s.", (!Teleportation ? ((knownsp != spe_Unknown)
                                              ? "не можете сотворить это заклинание"
                                              : "не знаете этого заклинания")
                            : "не можете телепортироваться по желанию"));
                return 0;
            }
        }
...
#endif
        if (u.uhunger <= 10) {
            cantdoit = "слишком слабы от голода";
        } else if (ACURR(A_STR) < 4) {
            cantdoit = "недостаточно сильны";
        } else if (energy > u.uen) {
            cantdoit = "недостаточно энергии";
        }
        if (cantdoit) {
            You("%s %s.", cantdoit,
                castit ? "для заклинания телепортации" : "для телепортации");
            return 0;
        } else if (check_capacity(
                       "Ваша концентрация нарушается из-за переносимого груза.")) {
...
level_tele(void)
{
    static const char get_there_from[] = "добраться туда отсюда из %s.";
    int newlev;
    d_level newlevel;
    ...
    if ((u.uhave.amulet || In_endgame(&u.uz) || In_sokoban(&u.uz))
        && !wizard) {
        You_feel("сильно сбились на миг.");
        return;
    }
    if ((Teleport_control && !Stunned) || wizard) {
        ...
        Strcpy(qbuf, "На какой уровень вы хотите телепортироваться?");
        do {
            if (iflags.menu_requested) {
            ...
            if (++trycnt == 2) {
                if (wizard)
                    Strcat(qbuf, " [введите число, имя или ? для меню]");
                else
                    Strcat(qbuf, " [введите число или имя]");
            }
 ...
 levTport_menu:
        ...
        if (newlev == 0) {
            if (trycnt >= 10)
                goto random_levtport;
            if (ynq("Отправиться в Небытие. Вы уверены?") != 'y')
                return;
            You("%s в агонии, когда ваше тело начинает искажаться...",
                is_silent(gy.youmonst.data) ? "корчиться" : "кричать");
            display_nhwindow(WIN_MESSAGE, FALSE);
            You("перестаёте существовать.");
            if (gi.invent)
                Your("вещи с глухим стуком падают на %s.",
                     surface(u.ux, u.uy));
            svk.killer.format = NO_KILLER_PREFIX;
            Strcpy(svk.killer.name, "покончил с собой");
            done(DIED);
            pline("Энергизированное облако пыли начинает сгущаться.");
            Your("тело вновь материализуется%s.",
                 gi.invent ? ", и вы собираете все свои вещи" : "");
            return;
        }
 ...
 random_levtport:
    ...
    if (newlev < 0 && !force_dest) {
        ...
        if (newlev <= -10) {
            You("прибываете в рай.");
            SetVoice((struct monst *) 0, 0, 80, voice_deity);
            verbalize("Ты пришёл рано, но мы тебя впустим.");
            svk.killer.format = NO_KILLER_PREFIX;
            Strcpy(svk.killer.name, "отправился на небеса преждевременно");
        } else if (newlev == -9) {
            You_feel("безумное счастье.");
            pline("(На самом деле, вы на седьмом небе!)");
            display_nhwindow(WIN_MESSAGE, FALSE);
        } else
            You("парите высоко над облаками...");

        if (svk.killer.name[0]) {
            ; /* arrival in heaven is pending */
        } else if (Levitation) {
            escape_by_flying = "мягко опуститься на землю";
        } else if (Flying) {
            escape_by_flying = "слететь вниз на землю";
        } else {
            pline("К сожалению, вы не умеете летать.");
            You("стремительно падаете несколько тысяч футов навстречу гибели.");
            Sprintf(svk.killer.name,
                    "телепортировался из подземелья и пал ",
                    uhis());
            svk.killer.format = NO_KILLER_PREFIX;
        }
    }
    ...
    if (svk.killer.name[0]) { /* the chosen destination was not survivable */
        ...
        escape_by_flying = "оказываетесь на поверхности";
    ...
    schedule_goto(&newlevel, UTOTYPE_NONE, (char *) 0,
                  flags.verbose ? "Вы материализуетесь на другом уровне!"
                                : (char *) 0);
...
domagicportal(struct trap *ttmp)
{
    ...
    You("активировали магический портал!");
    ...
    if (In_endgame(&u.uz) && !u.uhave.amulet) {
        You_feel("голова кружится на миг, но ничего не происходит...");
        return;
    }
    ...
    if (In_tutorial(&u.uz) && !In_tutorial(&target_level)) {
        ...
        stunmsg = "Возобновление обычной игры.";
    } else {
        totype = UTOTYPE_PORTAL;
        stunmsg = !Stunned ? "Вы чувствуете лёгкое головокружение."
                            : "Вы чувствуете головокружение сильнее.";
...
level_tele_trap(struct trap *trap, unsigned int trflags)
{
    ...
    if ((trflags & (VIASITTING | FORCETRAP)) != 0) {
        Strcpy(verbbuf, "активировали"); /* follows "You sit down." */
        intentional = TRUE;
    } else
        Sprintf(verbbuf, "%s на", u_locomotion("step"));
    You("%s ловушку телепорта уровня!", verbbuf);
    ...
    if (Hallucination || Teleport_control)
        You("кратко ощущаете %s.", Hallucination ? "ориентацию" : "центрированность");
    else
        You_feel("%sдезориентацию.", Confusion ? "ещё большую " : "");
    ...
    unsigned rlocflags)
{
    ...
    if (domsg && (canspotmon(mtmp) || appearmsg || mtmp == u.ustuck)) {
        ...
        const char *next = (du <= 2) ? " рядом с вами" : 0, /* next2u() */
                   *nearu = (du <= BOLT_LIM * BOLT_LIM) ? " поблизости" : 0;

        set_msg_xy(x, y);
        mtmp->mstrategy &= ~STRAT_APPEARMSG; /* one chance only */
        if (mtmp == u.ustuck && !u_at(u.ux0, u.uy0)) {
            You("и %s телепортируетесь вместе.", mon_nam(mtmp));
        } else if (telemsg && (couldsee(x, y) || sensemon(mtmp))) {
            pline("%s исчезает и вновь появляется%s.",
                  Monnam(mtmp),
                  next ? next
                  : nearu ? nearu
                    : ((olddu = distu(oldx, oldy)) == du) ? ""
                      : (du < olddu) ? " ближе к вам"
                        : " дальше");
        } else {
            pline("%s %s%s%s!",
                  appearmsg ? Amonnam(mtmp) : Monnam(mtmp),
                  appearmsg ? "внезапно " : "",
                  !Blind ? "появляется" : "прибывает",
    ...
    boolean via_rloc)
{
    ...
    pline("Куда телепортировать %s на (<%d,%d>)?",
          noit_mon_nam(mon), mon->mx, mon->my);
    /* getpos '?' will show "Move the cursor to <where to teleport Foo>:" */
    Sprintf(tcbuf, "куда телепортировать %s", noit_mon_nam(mon));
    if (getpos(cc_p, FALSE, tcbuf) >= 0 && !u_at(cc_p->x, cc_p->y)) {
        if (via_rloc
            ...
            return TRUE;
        if (!iflags.debug_fuzzer) {
            Sprintf(tcbuf, "<%d,%d> не считается подходящим; всё равно принудительно?",
                    mon->mx, mon->my);
            if (y_n(tcbuf) == 'y')
                return TRUE;
        }
    }
    pline("%s пункт назначения.", via_rloc ? "Выбор случайного" : "Использование производного");
    return FALSE;
}
...
tele_restrict(struct monst *mon)
{
    if (noteleport_level(mon)) {
        if (canseemon(mon))
            pline("Таинственная сила не даёт %s телепортироваться!",
                  mon_nam(mon));
        return TRUE;
    }
    return FALSE;
...
mtele_trap(struct monst *mtmp, struct trap *trap, int in_sight)
{
    ...
    if (teleport_pet(mtmp, FALSE)) {
        ...
        if (in_sight) {
            if (canseemon(mtmp))
                pline("%s выглядит дезориентированным.", monname);
            else
                pline("%s внезапно исчезает!", monname);
            seetrap(trap);
        }
    }
}
    ...
    int in_sight)
{
    ...
    if (teleport_pet(mtmp, force_it)) {
        ...
        if (is_hole(tt)) {
            if (Is_stronghold(&u.uz)) {
                ...
                if (in_sight && trap->tseen)
                    pline_mon(mtmp, "%s избегает %s.",
                              Monnam(mtmp),
                             (tt == HOLE) ? "дыру" : "ловушку");
            ...
            if (mon_has_amulet(mtmp) || In_endgame(&u.uz)
                ...
                if (in_sight)
                    pline_mon(mtmp,
                             "%s, кажется, очень дезориентирован на мгновение.",
                             Monnam(mtmp));
                return Trap_Effect_Finished;
            }
            if (tt == NO_TRAP) {
                ...
                if (nlev == depth(&u.uz)) {
                    if (in_sight)
                        pline_mon(mtmp, "%s вздрагивает на мгновение.",
                                  Monnam(mtmp));
                    return Trap_Effect_Finished;
                }
        ...
        if (in_sight) {
            pline_mon(mtmp, "Внезапно %s %s.", mon_nam(mtmp),
                     (tt == HOLE) ? "проваливается в дыру"
                   : (tt == TRAPDOOR) ? "падает сквозь люк"
                   : "исчезает из виду");
            if (trap)
                seetrap(trap);
        }
    ...
    boolean give_feedback)
{
    ...
    if (svl.level.flags.stasis_until >= svm.moves) {
        if (give_feedback)
            pline("Таинственная сила не даёт вам телепортировать %s!",
                  mon_nam(mtmp));
        return FALSE;
    } else if (mtmp->ispriest && *in_rooms(mtmp->mx, mtmp->my, TEMPLE)) {
        if (give_feedback)
            pline("%s сопротивляется вашей магии!", Monnam(mtmp));
        return FALSE;
    } else if (engulfing_u(mtmp) && noteleport_level(mtmp)) {
        if (give_feedback)
            You("больше не находитесь внутри %s!", mon_nam(mtmp));
        unstuck(mtmp);
