...
{               /* negative => conditional (msg if change made) */
    ...
    if ((ndx == A_INT || ndx == A_WIS) && uarmh && uarmh->otyp == DUNCE_CAP) {
        if (msgflg == 0)
            Your("шапка сжимаетс€ на мгновение, затем снова расслабл€етс€.");
        return FALSE;
    }
    ...
    if (ACURR(ndx) == old_acurr) {
        if (msgflg == 0 && flags.verbose) {
            if (ABASE(ndx) == old_abase && AMAX(ndx) == old_amax) {
                pline("¬ы %s, насколько %s возможно.",
...
static const struct poison_effect_message {
    ...
    { You_feel, "слабее" },             /* A_STR */
    { Your, "мозг горит огнЄм" },       /* A_INT */
    { Your, "рассудок помрачЄн" },  /* A_WIS */
    { Your, "мышцы не слушаютс€ вас" }, /* A_DEX */
    { You_feel, "себ€ очень больным" },          /* A_CON */
    { You, "покрываетесь сыпью" }       /* A_CHA */
};
           ...
           boolean exclaim) /* emphasis */
{
    ...
    if (typ == A_STR && ACURR(A_STR) == STR19(25))
        msg_txt = "слабее от природы";
    else if (typ == A_CON && ACURR(A_CON) == 25)
        msg_txt = "больны изнутри";
    ...
    boolean thrown_weapon) /* thrown weapons are less deadly */
{
    ...
    if (!blast && !strstri(reason, "poison")) {
        const char *gend = "";
        unsigned char lc = (unsigned char) reason[strlen(reason) - 1];

        /* RU: no articles; short passive verb agrees with noun ending */
        if (lc == 'а' || lc == '€')
            gend = "а";
        else if (lc == 'о' || lc == 'е')
            gend = "о";
            ...
            gend = "ы";
        pline("ќтравлен%s %s!", gend, reason);
    }
    if (Poison_resistance) {
        if (blast)
            shieldeff(u.ux, u.uy);
        pline("яд, похоже, на вас не действует.");
        return;
    }
    ...
    int reason) /* A_CG_CONVERT, A_CG_HELM_ON, or A_CG_HELM_OFF */
{
    ...
    if (reason == A_CG_CONVERT) {
        ...
        livelog_printf(LL_ALIGNMENT, "навсегда порвал с %s",
                       aligns[1 - newalign].adj);
        u.ualignbase[A_CURRENT] = (aligntyp) newalign;
        /* worn helm of opposite alignment might block change */
        if (!uarmh || uarmh->otyp != HELM_OF_OPPOSITE_ALIGNMENT)
            u.ualign.type = u.ualignbase[A_CURRENT];
        You("обретаете %sчувство нового направлени€.",
