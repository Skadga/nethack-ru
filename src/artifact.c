    ...
    winid tmpwin) /* supplied by dodiscover(); type is NHW_TEXT */
{
    ...
    for (i = 0; i < NROFARTIFACTS; i++) {
        ...
        if (!strcmp(algnstr, "непричастный"))
            algnstr = "без выравнивания";
            ...
            if (youdefend) {
                ...
                if (u.uenmax > 0) {
                    ...
                    if (u.uen > 0)
                        u.uen--;
                    disp.botl = TRUE;
                    You("теряете магическую энергию!");
                }
            } else {
                ...
                if (youattack && attacktype(mdef->data, AT_MAGC)) {
                    ...
                    u.uen++;
                    disp.botl = TRUE;
                    You("поглощаете магическую энергию!");
                }
            }
        }
        break;
            ...
            if (!youdefend) {
                ...
                *dmgptr = 2 * mdef->mhp + FATAL_DAMAGE_MODIFIER;
                ...
                if (Hallucination && !flags.female)
                    pline("Молодец, Генри, но эта была не Энн.");
                observe_object(otmp);
                return TRUE;
            } else {
...
{
    if (carried(obj))
        You_feel("всплеск силы, но, похоже, ничего не происходит.");
}
...
invoke_energy_boost(struct obj *obj)
{
    ...
    if (epboost) {
        u.uen += epboost;
        disp.botl = TRUE;
        You_feel("себя вновь наполненным энергией.");
    } else {
        nothing_special(obj);
        return ECMD_TIME;
    }
...
invoke_create_portal(struct obj *obj)
{
    ...
    if (u.uhave.amulet || In_endgame(&u.uz) || In_endgame(&newlev)
        ...
        You_feel("сильную дезориентацию на мгновение.");
    } else {
        if (!Blind)
            You("окружены мерцающей сферой!");
        else
            You_feel("невесомость на мгновение.");
...
arti_invoke(struct obj *obj)
{
    ...
    if (oart->inv_prop > LAST_PROP) {
        ...
        case CONFLICT:
            if (on)
                You_feel("себя подстрекателем.");
            else
                You_feel("ослабление напряжения вокруг вас.");
            break;
        case LEVITATION:
            if (on) {
                float_up();
        ...
        case INVIS:
            ...
            if (on)
                ...
                Your("тело, похоже, перестаёт исчезать...");
            break;
        }
    }
