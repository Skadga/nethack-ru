    ...
    boolean exclude_cookie)
{
    ...
    if (rumors) {
        ...
        do {
            ...
            if (gt.true_rumor_size == 0L) { /* if this is 1st outrumor() */
                ...
                if (gt.true_rumor_size < 0L) { /* init failed */
                    Sprintf(rumor_buf, "Ошибка чтения файла \"%.80s\".", RUMORFILE);
                    return rumor_buf;
                }
            }
            /*
            ...
            case -1:
                ...
                break;
            default:
                impossible("странное значение истинности для слуха");
                return strcpy(rumor_buf, "Ой...");
            }
            Strcpy(rumor_buf,
        ...
        if (count >= 50)
            impossible("Не удаётся найти слух без печенья?");
...
rumor_check(void)
{
    ...
    if (rumors) {
         ...
         */
        Sprintf(rumor_buf,
               "T нач=%06ld (%06lx), кон=%06ld (%06lx), разм=%06ld (%06lx)",
        ...
        Sprintf(rumor_buf,
               "F нач=%06ld (%06lx), кон=%06ld (%06lx), разм=%06ld (%06lx)",
 ...
 no_rumors: /* file could be opened but init_rumors() didn't like it */
    ...
    others_check("Надписи:", ENGRAVEFILE, &tmpwin);
    others_check("Эпитафии:", EPITAPHFILE, &tmpwin);
    others_check("Ложные чудовища:", BOGUSMONFILE, &tmpwin);
    ...
    if (fh) {
        if (tmpwin == WIN_ERR) {
            *winptr = tmpwin = create_nhwindow(NHW_TEXT);
            if (tmpwin == WIN_ERR) {
                ...
                impossible(errfmt, fname, "не удаётся создать временное окно");
                goto closeit;
            }
        }
        putstr(tmpwin, 0, "");
        ...
        *line = '\0';
        if (!dlb_fgets(line, sizeof line, fh)) {
            Sprintf(xbuf, errfmt, fname, "ошибка; не удаётся прочитать строку комментария");
            putstr(tmpwin, 0, xbuf);
            goto closeit;
        }
        if (*line != '#') {
            Sprintf(xbuf, errfmt, fname,
                    "неверно; первая строка не является строкой комментария:");
            putstr(tmpwin, 0, xbuf);
            /* show the bad line; we don't know whether it has been
               encrypted via xcrypt() so show it both ways */
            if ((endp = strchr(line, '\n')) != 0)
                *endp = 0;
            putstr(tmpwin, 0, "- первая строка, как есть");
            putstr(tmpwin, 0, line);
            putstr(tmpwin, 0, "- xcrypt первой строки");
        ...
        *line = '\0';
        if (!dlb_fgets(line, sizeof line, fh) || *line == '\n') {
            Sprintf(xbuf, errfmt, fname,
                    !*line ? "не удаётся прочитать первую не-комментарийную строку"
                           : "первая не-комментарийная строка пуста");
            putstr(tmpwin, 0, xbuf);
            goto closeit;
        }
        ++entrycount;
        ...
        putstr(tmpwin, 0, xcrypt(line, xbuf));
        if (!dlb_fgets(line, sizeof line, fh)) {
            putstr(tmpwin, 0, "(нет второй записи)");
        } else {
            ++entrycount;
            if ((endp = strchr(line, '\n')) != 0)
                *endp = 0;
            ...
            if (entrycount == 2) {
                putstr(tmpwin, 0, "(только две записи)");
            } else {
    ...
    int mechanism)
{
    static const char fortune_msg[] =
        "В этом печенье есть клочок бумаги внутри.";
    const char *line;
    char buf[BUFSZ];
    ...
    if (reading) {
        ...
        if (is_fainted() && mechanism == BY_COOKIE) {
            ...
            if (mechanism == BY_COOKIE)
                pline(fortune_msg);
            pline("Какая жалость, что ты не можешь этого прочитать!");
            return;
        }
    }
    ...
    if (!*line)
        line = "Файл слухов NetHack закрыт на ремонт.";
    switch (mechanism) {
    case BY_ORACLE:
        /* Oracle delivers the rumor */
        pline("Верная своему слову, Оракул %sговорит: ",
              (!rn2(4) ? "между делом "
                       : (!rn2(3) ? "невозмутимо "
                                  : (rn2(2) ? "бесстрастно " : ""))));
...
outoracle(boolean special, boolean delphi)
{
    ...
    if (oracles) {
        ...
        if (delphi)
            putstr(tmpwin, 0,
                   special
                     ? "Оракул презрительно забирает всё твоё золото и говорит:"
                     : "Оракул минуту медитирует, а затем произносит:");
        else
            putstr(tmpwin, 0, "Сообщение гласит:");
        putstr(tmpwin, 0, "");
...
doconsult(struct monst *oracl)
{
    ...
    if (!oracl) {
        There("здесь некого спросить.");
        return ECMD_OK;
    } else if (!oracl->mpeaceful) {
        pline("%s не расположен к консультациям.", Monnam(oracl));
        return ECMD_OK;
    } else if (!umoney) {
        You("у тебя нет золота.");
        return ECMD_OK;
    }

    Sprintf(qbuf, "\"Согласен ли ты на небольшую консультацию?\" (%d %s)",
    ...
    case 'y':
        if (umoney < (long) minor_cost) {
            You("у тебя даже нет столько золота!");
            return ECMD_OK;
        }
        u_pay = minor_cost;
        break;
    case 'n':
        if (umoney <= (long) minor_cost /* don't even ask */
            ...
            return ECMD_OK;
        Sprintf(qbuf, "\"Тогда желаешь ли ты большую консультацию?\" (%d %s)",
...
couldnt_open_file(const char *filename)
{
    ...
    impossible("Не удаётся открыть файл '%s'.", filename);
...
init_CapMons(void)
{
    ...
    if (wizard && explicitdebug("CapMons")) {
        ...
        putstr(tmpwin, 0,
              "Имена типов чудовищ с заглавной буквы, обычно с предшествующим \"the\":");
        for (i = 0; i < CapMonSiz - 1; ++i) {
