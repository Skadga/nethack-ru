        ...
        if (!Protection_from_shape_changers
            ...
            if (!Deaf && !canseemon(mon)) {
                ...
                if (howler) {
                    ...
                    You_hear("как %s воет на луну.", howler);
...
new_were(struct monst *mon)
{
    ...
    if (pm < LOW_PM) {
        impossible("неизвестный оборотень %s.",
                    mon->data->pmnames[NEUTRAL]);
        return;
    }

    if (canseemon(mon) && !Hallucination)
        pline("%s превращаетс€ в %s.", Monnam(mon),
              is_human(&mons[pm]) ? "человека"
...
you_were(void)
{
    ...
    if (controllable_poly) {
        ...
        Sprintf(qbuf, "’отите превратитьс€ в %s?",
...
you_unwere(boolean purify)
{
    ...
    if (!Unchanging && is_were(gy.youmonst.data)
        && !monster_nearby()
        && (!controllable_poly
            || !paranoid_query(ParanoidWerechange, "ќстатьс€ в звериной форме?")))
        rehumanize();
