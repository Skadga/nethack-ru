...
moveloop_preamble(boolean resuming)
{
    ...
    if (flags.friday13) {
        pline("Осторожно! В пятницу 13-го могут произойти нехорошие вещи.");
        change_luck(-1);
    }
        ...
        do { /* hero can't move this turn loop */
            ...
            if (!monscanmove && u.umovement < NORMAL_SPEED) {
                ...
                if (svm.moves >= 1000000000L) {
                    ...
                    urgent_pline("Подземелье капитулирует.");
                    done(ESCAPED);
                }
 ...
 * maintained and not a list of special cases. */
    ...
    if (u.uhave.amulet && !u.uevent.amulet_wish) {
        ...
        display_nhwindow(WIN_MESSAGE, TRUE);
        urgent_pline("Амулет дарует вам желание!");
        makewish();
    }
...
regen_pw(int wtcap)
{
    if (u.uen < u.uenmax
        ...
        disp.botl = TRUE;
        if (u.uen == u.uenmax)
            interrupt_multi("Вы чувствуете прилив энергии.");
    }
}
...
regen_hp(int wtcap)
{
    ...
    if (reached_full)
        interrupt_multi("Вы в полном здравии.");
}
...
#ifdef STATUS_HILITES
    if (!iflags.hilite_delta) {
        (void) parse_status_hl1("hunger/Satiated/green", TRUE);
    }
#endif
...
welcome(boolean new_game) /* false => restoring an old game */
{
    ...
    if (!new_game && Upolyd && ugenocided()) {
        ...
        pline("Вы вернулись, но внутри вы всё ещё чувствуете %s.", udeadinside());
        return;
    }

    if (Hallucination)
        pline("NetHack снят перед аудиторией из нежити.");
    ...
    *buf = '\0';
    ...
    pline(new_game ? "%s, %s, добро пожаловать в NetHack! Вы —%s."
                   : "%s, %s, %s, с возвращением в NetHack!",
          Hello((struct monst *) 0), svp.plname, buf);

    if (new_game) {
        /* guarantee that 'major' event category is never empty */
        livelog_printf(LL_ACHIEVE, "%s %s вошёл в подземелье",
                       svp.plname, buf);
    } else {
