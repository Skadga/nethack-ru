...
pri_move(struct monst *priest)
{
    ...
    if (!priest->mpeaceful
        ...
        if (monnear(priest, u.ux, u.uy)) {
            if (Displaced)
                Your("Ваша замещённая проекция не обманет %s!", mon_nam(priest));
    ...
    if ((priest = findpriest((char) roomno)) != 0) {
        ...
        if (can_speak && !Deaf && svm.moves >= epri_p->intone_time) {
            ...
            if (sanctum && !Hallucination)
                priest->ispriest = 0;
            pline("%s произносит:",
                  canseemon(priest) ? Monnam(priest) : "Голос где-то рядом");
        ...
        if (sanctum && Is_sanctum(&u.uz)) {
            if (priest->mpeaceful) {
                /* first time inside */
                msg1 = "Неверный, вы вошли в Святилище Молоха!";
                msg2 = "Прочь!";
                priest->mpeaceful = 0;
                ...
                msg1 = "Вы оскверняете это место своим присутствием!";
            }
        } else if (svm.moves >= epri_p->enter_time) {
            Sprintf(buf, "Странник, вы входите в %s место!",
                    !shrined ? "осквернённое" : "священное");
            msg1 = buf;
        }
        ...
        if (!sanctum) {
            if (!shrined || !p_coaligned(priest)
                ...
                msg1 = "испытываете%s зловещее предчувствие...";
                msg2 = (!shrined || !p_coaligned(priest)) ? "" : " странное";
                this_time = &epri_p->hostile_time;
                other_time = &epri_p->peaceful_time;
            } else {
                msg1 = "испытываете %sчувство покоя.";
                msg2 = (u.ualign.record >= ALGN_DEVOUT) ? "" : "необычное ";
        ...
        case 0:
            ...
            break;
        case 1:
            You_feel("будто за вами следят.");
            break;
        case 2:
            pline("По вашему %s пробегает дрожь.", body_part(SPINE));
            break;
        default:
        ...
        if (!rn2(5)
            ...
            if (canspotmon(mtmp))
                pline("Рядом с вами появляется%s призрак%c",
                      ngen < 5 ? " огромный" : "",
                      ngen < 10 ? '!' : '.');
            else
                You("ощущаете чьё-то присутствие рядом!");
            mtmp->mpeaceful = 0;
            set_malign(mtmp);
            if (flags.verbose)
                You("напуганы до смерти и не можете пошевелиться.");
            nomul(-3);
            gm.multi_reason = "ужас перед призраком";
...
priest_talk(struct monst *priest)
{
    ...
    if (!u.uconduct.gnostic++)
        livelog_printf(LL_CONDUCT,
                       "отвергли атеизм, посоветовавшись с %s",
                       mon_nam(priest));

    if (priest->mflee || (!priest->ispriest && coaligned && strayed)) {
        pline("%s не хочет иметь с вами дела!", Monnam(priest));
        priest->mpeaceful = 0;
        return;
    }
    ...
    if (!inhistemple(priest) || !priest->mpeaceful || helpless(priest)) {
        static const char *const cranky_msg[3] = {
            "Слова, говоришь?  Вот тебе словечко!",
            "Разговаривать?  Вот что я скажу!",
            "Странник, я больше не желаю с тобой говорить."
        };

        if (helpless(priest)) {
            pline("%s приходит в себя из %s задумчивости!", Monnam(priest),
                  mhis(priest));
    ...
    if (priest->mpeaceful && *in_rooms(priest->mx, priest->my, TEMPLE)
        ...
        SetVoice(priest, 0, 80, 0);
        verbalize(
              "Прочь!  Ты оскверняешь это священное место своим присутствием.");
        priest->mpeaceful = 0;
        return;
    }
    if (!money_cnt(gi.invent)) {
        if (coaligned && !strayed) {
            ...
            if (pmoney > 0L) {
                ...
                bits = (Hallucination) ? currency(pmoney)
                                       : (pmoney == 1L) ? "монету" : "монеты";
                /* Note: two bits is actually 25 cents.  Hmm. */
                pline("%s даёт вам %s%s за пиво.", Monnam(priest),
                      (pmoney == 1L) ? "одну " : "две ", bits);
                money2u(priest, pmoney > 1L ? 2 : 1);
            } else
                pline("%s проповедует добродетель бедности.", Monnam(priest));
            exercise(A_WIS, TRUE);
        } else
            pline("%s не заинтересован.", Monnam(priest));
        return;
    } else {
        ...
        Sprintf(buf, "Сколько вы предложите (рекомендуется: %ld или %ld)?",
                suggested * quan, suggested * quan * 2);

        if (flags.debug)
            pline("%s просит вас сделать взнос в храм (базовый: %ld).",
                  Monnam(priest), suggested);
        else
            pline("%s просит вас сделать взнос в храм.",
                  Monnam(priest));
        if ((offer = bribe(priest, buf)) == 0) {
            SetVoice(priest, 0, 80, 0);
            verbalize("Ты пожалеешь о своём поступке!");
            if (coaligned)
                adjalign(-1);
            ...
            if (money_cnt(gi.invent) > (offer * 2L)) {
                ...
                SetVoice(priest, 0, 80, 0);
                verbalize("Благодарю тебя за твой вклад.");
                /* give player some token */
                exercise(A_WIS, TRUE);
            }
        } else if (offer < suggested * quan * 2) {
            SetVoice(priest, 0, 80, 0);
            verbalize("Ты и вправду благочестивый человек.");
            if (money_cnt(gi.invent) < (offer * 2L)) {
                if (coaligned && u.ualign.record <= ALGN_SINNED)
                    adjalign(1);
            }
            verbalize("Даю тебе своё благословение.");
            ...
            if (u.ublessed > orig_ublessed) {
                verbalize("Ты был вознаграждён за своё усердие.");
            } else {
                verbalize("Твоя бескорыстная щедрость глубоко ценится.");
            }
        } else {
            SetVoice(priest, 0, 80, 0);
            verbalize("Твоя бескорыстная щедрость глубоко ценится.");
...
ghod_hitsu(struct monst *priest)
{
    ...
    switch (rn2(3)) {
    case 0:
        pline("%s громко гневается: \"Ты будешь страдать!\"",
              a_gname_at(ax, ay));
        break;
    case 1:
        pline("Голос %s гремит:  \"Как ты посмел причинять вред моему слуге!\"",
              s_suffix(a_gname_at(ax, ay)));
        break;
    default:
        pline("%s громогласно объявляет: \"Ты оскверняешь моё святилище!\"",
              a_gname_at(ax, ay));
        break;
    }
