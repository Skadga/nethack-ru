...
static NEARDATA const char *const stoned_texts[] = {
    "Вы замедляетесь.",            /* 5 */
    "Ваши конечности коченеют.",       /* 4 */
    "Ваши конечности превратились в камень.", /* 3 */
    "Вы превратились в камень.",        /* 2 */
    "Вы — статуя."                 /* 1 */
};
...
static NEARDATA const char *const vomiting_texts[] = {
    "Вы ощущаете лёгкую тошноту.", /* 14 */
    "Вы слегка сбиты с толку.",       /* 11 */
    "Вы не можете думать прямо.", /* 8 */
    "Вы чувствуете себя ужасно больным.", /* 5 */
    "Вот-вот вырвет."            /* 2 */
};
...
vomiting_dialogue(void)
{
    ...
    case 0:
        ...
        if (!cantvomit(gy.youmonst.data)) {
            ...
            if (u.uhs < FAINTING)
                You("%s!", !Hallucination ? "Вырвало" : "выблевало куски");
        }
        vomit();
        break;
    default:
...
static NEARDATA const char *const choke_texts[] = {
    "Вам трудно дышать.",
    "Вы задыхаетесь, хватая ртом воздух.",
...
static NEARDATA const char *const choke_texts2[] = {
    "Ваша %s сжимается.",
    "Вашей крови трудно добраться до мозга.",
    "Давление на вашу %s растёт.",
    "Ваше сознание угасает.",
    "Вы задыхаетесь."
};
...
static NEARDATA const char *const sickness_texts[] = {
    "Ваша болезнь ощущается хуже.",
    "Ваша болезнь серьёзна.",
    "Вы у порога Смерти.",
};
...
sickness_dialogue(void)
{
    ...
    if (i > 0L && i <= SIZE(sickness_texts) && (j % 2) != 0) {
        ...
        if (Hallucination && strstri(buf, "Death's door")) {
            ...
            Sprintf(eos(buf), "  %s приглашает вас внутрь.",
...
static NEARDATA const char *const levi_texts[] = {
    "Вы немного опускаетесь.",
    "Вы неуверенно покачиваетесь %s %s."
};
...
levitation_dialogue(void)
{
    ...
    if (((HLevitation & TIMEOUT) % 2L) && i > 0L && i <= SIZE(levi_texts)) {
        ...
        if (strchr(s, '%')) {
            ...
            urgent_pline(s, danger ? "над" : "в",
                         danger ? (is_lava(u.ux, u.uy) ? "лавой"
                                                         : "водой")
                                 : "воздухе");
        } else
            pline1(s);
...
static NEARDATA const char *const slime_texts[] = {
    "Вы немного превращаетесь в %s.",   /* 5 */
    "Ваши конечности становятся студенистыми.",   /* 4 */
    "Ваша кожа начинает отслаиваться.", /* 3 */
    "Вы превращаетесь в %s.",       /* 2 */
    "Вы стали %s."             /* 1 */
};
...
{
    if (Slimed) {
        make_slimed(0L, "Покрывающая вас слизь сгорела!");
    }
}
...
slimed_to_death(struct kinfo *kptr)
{
    ...
    if (kptr && kptr->name[0]) {
        ...
        Strcpy(svk.killer.name, "превратились в зелёную слизь");
    }
    dealloc_killer(kptr);
...
static NEARDATA const char *const phaze_texts[] = {
    "Вы начинаете чувствовать вздутие.",
    "Вы чувствуете себя довольно дряблым.",
};
...
static NEARDATA const char *const region_texts[] = {
    "Похоже, у вас небольшие проблемы с дыханием.",
    "Воздух здесь кажется спёртым.",
};
...
nh_timeout(void)
{
    ...
    if (u.usptime) {
        if (--u.usptime == 0 && u.uspellprot) {
            ...
            find_ac();
            if (!Blind)
                Norep("%s дымка вокруг вас %s.", hcolor(NH_GOLDEN),
                      u.uspellprot ? "становится менее плотной" : "рассеивается");
        }
    }

    if (u.ugallop) {
        if (--u.ugallop == 0L && u.usteed)
            pline("%s перестаёт галопировать.", Monnam(u.usteed));
    }
        ...
        if ((upp->intrinsic & TIMEOUT) && !(--upp->intrinsic & TIMEOUT)) {
            ...
            case STONED:
                if (kptr && kptr->name[0]) {
                    ...
                    svk.killer.format = NO_KILLER_PREFIX;
                    Strcpy(svk.killer.name, "превратились в камень");
                }
                dealloc_killer(kptr);
            ...
            case SICK:
                ...
                if ((u.usick_type & SICK_NONVOMITABLE) == 0
                    && rn2(100) < ACURR(A_CON)) {
                    You("оправились от болезни.");
                    make_sick(0, NULL, FALSE, SICK_ALL);
                    exercise(A_CON, FALSE);
                    adjattrib(A_CON, -1, 1);
                    break;
                }
                urgent_pline("Вы умираете от своей болезни.");
                if (kptr && kptr->name[0]) {
                    svk.killer.format = kptr->format;
                    Strcpy(svk.killer.name, kptr->name);
                } else {
            ...
            case INVIS:
                ...
                if (!Invis && !BInvis && !Blind) {
                    You(!See_invisible
                            ? "больше не невидимы."
                            : "больше не видите себя насквозь.");
                    stop_occupation();
                }
                break;
            case SEE_INVIS:
            ...
            case ACID_RES:
                if (!Acid_resistance) {
                    ...
                    if (!Unaware)
                        You("больше не чувствуете себя в безопасности от кислоты.");
                }
                break;
            case STONE_RES:
                if (!Stone_resistance) {
                    ...
                    if (!Unaware)
                        You("больше не чувствуете себя защищённым от окаменения.");
            ...
            case FIRE_RES:
                ...
                if (!Fire_resistance)
                    Your("временная способность переживать горение закончилась.");
                break;
            case WWALKING:
                /* [see fire resistance] */
                if (!Wwalking)
                    Your("временная способность ходить по жидкости закончилась.");
                break;
            case DISPLACED:
            ...
            case WARN_OF_MON:
                ...
                if (!Warn_of_mon) {
                    ...
                    if (wptr)
                        You("больше не предупреждаетесь о %s.",
            ...
            case PASSES_WALLS:
                if (!Passes_walls) {
                    if (stuck_in_wall())
                        You_feel("снова в тисках.");
                    else
                        pline("Вы снова стали %s.",
                              !Upolyd ? "обычным" : "необычным");
                }
                break;
            case MAGICAL_BREATHING:
                if (!Breathless) {
                    if (region_danger())
                        You("кашляете%s",
                            Poison_resistance ? "." : " и сплёвываете кровь!");
                }
                break;
            case STRANGLED:
                svk.killer.format = KILLED_BY;
                Strcpy(svk.killer.name,
                       (u.uburied) ? "удушьем" : "удушением");
...
fall_asleep(int how_long, boolean wakeup_msg)
{
    ...
    u.usleep = svm.moves;
    gn.nomovemsg = wakeup_msg ? "Вы просыпаетесь." : You_can_move_again;
}
...
hatch_egg(anything *arg, long timeout)
{
    ...
    if (mon) {
        ...
        case OBJ_INVENT:
            ...
            if (!cansee_hatchspot)
                You_feel("%s %s из вашей сумки!", something,
                         locomotion(mon->data, "выпадает"));
            else
                You_see("%s %s из вашей сумки!", monnambuf,
                        locomotion(mon->data, "выпадает"));
            if (yours) {
                pline("%s %s \"%s%s\"",
                      siblings ? "Их" : "Его",
                      (is_silent(mon->data) || Deaf) ? "писк слышен"
                                                     : "писк слышится",
                      flags.female ? "мамочка" : "папочка",
                      egg->spe ? "." : "?");
            } else if (mon->data->mlet == S_DRAGON && !Deaf) {
                SetVoice(mon, 0, 80, 0);
                verbalize("Глип!"); /* Mything eggs :-) */
            }
            break;
        ...
        case OBJ_FLOOR:
            if (cansee_hatchspot) {
                knows_egg = TRUE;
                You_see("%s вылупляется.", monnambuf);
        ...
        case OBJ_MINVENT:
            if (cansee_hatchspot) {
                ...
                if (canseemon(mon2)
                    ...
                    Sprintf(carriedby, "мешка %s",
                            s_suffix(a_monnam(mon2)));
                    knows_egg = TRUE;
                } else if (is_pool(mon->mx, mon->my)) {
                    Strcpy(carriedby, "пустой воды");
                } else {
                    Strcpy(carriedby, "воздуха");
                }
                You_see("%s %s из %s!", monnambuf,
                        locomotion(mon->data, "выпадает"), carriedby);
            }
            break;
#if 0
        case OBJ_MIGRATING:
    ...
    if (otmp && on_foot) { /* trip over something in particular */
        ...
        */
        what = (iflags.last_msg == PLNMSG_ONE_ITEM_HERE)
                ? ((otmp->quan == 1L) ? "оно"
                      : Hallucination ? "они" : "их")
                : (otmp->dknown || !Blind)
                      ? doname(otmp)
                      : ((otmp2 = sobj_at(ROCK, u.ux, u.uy)) == 0
                             ? something
                             : (otmp2->quan == 1L ? "камень" : "камни"));
        if (Hallucination) {
            what = strcpy(buf, what);
            buf[0] = highc(buf[0]);
            pline("Ба! %s цапает вашу %s!", what,
                  body_part(FOOT));
        } else {
            You("спотыкаетесь о %s.", what);
        }
        if (!uarmf && otmp->otyp == CORPSE
            && touch_petrifies(&mons[otmp->corpsenm]) && !Stone_resistance) {
            Sprintf(svk.killer.name, "споткнувшись о труп %s",
        ...
        if (on_foot) {
            switch (rn2(4)) {
            case 1:
                You("спотыкаетесь о собственн%s %s.",
                    Hallucination ? "ый локоть" : "ые ноги");
                break;
            case 2:
                You("поскальзываетесь %s.",
                    Hallucination ? "на банановой кожуре" : "и чуть не падаете");
                break;
            case 3:
                You("барахтаетесь.");
                break;
        ...
        /* mounted; saddle should never end up being Null here;
           don't fall off when it happens to be cursed */
            ...
            switch (rn2(4)) {
            case 1:
                Your("%s выскальзывают из стремян.",
                     makeplural(body_part(FOOT)));
                break;
            case 2:
                You("отпускаете поводья.");
                break;
            case 3:
                You("ударяетесь о луку седла.");
                break;
            default:
                You("соскальзываете набок с седла.");
                break;
            }
            dismount_steed(DISMOUNT_FELL);
        }
...
see_lamp_flicker(struct obj *obj, const char *tailer)
{
    ...
    case OBJ_INVENT:
    case OBJ_MINVENT:
        pline("%s мерцает%s.", Yname2(obj), tailer);
        break;
    case OBJ_FLOOR:
        You_see("%s мерцает%s.", an(xname(obj)), tailer);
        break;
    }
}
...
lantern_message(struct obj *obj)
{
    ...
    switch (obj->where) {
    case OBJ_INVENT:
        Your("фонарь тускнеет.");
        if (Hallucination)
            pline("Батарейки ещё не изобретены.");
        break;
    case OBJ_FLOOR:
        You_see("фонарь тускнеет.");
        break;
    case OBJ_MINVENT:
        pline("Лампа %s тускнеет.", s_suffix(Monnam(obj->ocarry)));
        break;
    }
}
...
burn_object(anything *arg, long timeout)
{
    ...
    case POT_OIL:
        ...
        if (canseeit) {
            ...
            case OBJ_INVENT:
                need_invupdate = TRUE;
                Your("масляное зелье сгорело дотла.");
                break;
            case OBJ_MINVENT:
                pline("Масляное зелье сгорело дотла.");
                break;
            case OBJ_FLOOR:
                You_see("горящее масляное зелье гаснет.");
                need_newsym = TRUE;
                break;
            }
        }
        ...
        case 25:
            if (canseeit) {
                if (obj->otyp == BRASS_LANTERN) {
                    ...
                    case OBJ_INVENT:
                    case OBJ_MINVENT:
                        pline("%s вот-вот погаснет.", Yname2(obj));
                        break;
                    case OBJ_FLOOR:
                        You_see("%s вот-вот погаснет.", an(xname(obj)));
                        break;
                    }
                }
            }
        ...
        case 0:
            ...
            if (canseeit || bytouch) {
                ...
                case OBJ_INVENT:
                    need_invupdate = TRUE;
                    if (obj->otyp == BRASS_LANTERN)
                        Your("фонарь исчерпал заряд.");
                    else
                        pline("%s погас.", Yname2(obj));
                    break;
                case OBJ_MINVENT:
                    if (obj->otyp == BRASS_LANTERN)
                        pline("Фонарь исчерпал заряд.");
                    else
                        pline("%s погас.", Yname2(obj));
                    break;
                case OBJ_FLOOR:
                    if (obj->otyp == BRASS_LANTERN)
                        You_see("фонарь исчерпал заряд.");
                    else
                        You_see("%s гаснет.", an(xname(obj)));
                    break;
                }
            }
            end_burn(obj, FALSE);
    ...
    case WAX_CANDLE:
        ...
        case 75:
            if (canseeit)
                ...
                case OBJ_MINVENT:
                    pline("%s %s.",
                          menorah ? "свечи подсвечника" : (many ? "свечи" : "свеча"),
                          many ? "догорают" : "догорает");
                    break;
                case OBJ_FLOOR:
                    You_see("%s %s.",
                            menorah ? "свечи подсвечника" : (many ? "несколько свечей" : "свеча"),
                            many ? "догорают" : "догорает");
                    break;
                }
            break;
        ...
        case 15:
            if (canseeit)
                ...
                case OBJ_MINVENT:
                    pline("%s %s %s!",
                          many ? "Пламя ваших" : "Пламя вашей",
                          menorah ? "свечей подсвечника" : (many ? "свечей" : "свечи"),
                          "дрожит");
                    break;
                case OBJ_FLOOR:
                    You_see("%s %s %s.",
            ...
            if (canseeit || bytouch) {
                if (menorah) {
                    ...
                    case OBJ_INVENT:
                        ...
                        /*FALLTHRU*/
                    case OBJ_MINVENT:
                        pline("Пламя %s подсвечника гаснет.",
                              many ? "свечей" : "свечи");
                        break;
                    case OBJ_FLOOR:
                        You_see("Пламя %s подсвечника гаснет.",
                                many ? "свечей" : "свечи");
                        break;
                    }
                } else {
                    ...
                    case OBJ_FLOOR:
                         ...
                         */
                        You_see("%s %s!", many ? xname(obj) : an(xname(obj)),
                                many ? "сгорели" : "сгорела");
                        need_newsym = TRUE;
                        break;
                    }
                    ...
                    /* post message */
                    pline(Hallucination
                              ? (many ? "Они вопят!" : "Оно вопит!")
                              : Blind ? "" : (many ? "Их пламя гаснет."
                                                   : "Его пламя гаснет."));
                }
            }
            end_burn(obj, FALSE);
...
do_storms(void)
{
    ...
    if (levl[u.ux][u.uy].typ == CLOUD) {
        ...
        if (!u.uinvulnerable) {
            stop_occupation();
            nomul(-3);
            gm.multi_reason = "скрываясь от грозы";
            gn.nomovemsg = 0;
        }
    } else
        You_hear("грохочущий шум.");
}
...
print_queue(winid win, timer_element *base)
{
    ...
    if (!base) {
        ...
        putstr(win, 0, "таймер  id   вид   вызов");
...
wiz_timeout_queue(void)
{
    ...
    Sprintf(buf, "Текущее время = %ld.", svm.moves);
    putstr(win, 0, buf);
    putstr(win, 0, "");
    putstr(win, 0, "Активная очередь таймеров:");
    putstr(win, 0, "");
    ...
    putstr(win, 0, "");
    if (!count) {
        putstr(win, 0, "Нет временных свойств.");
    } else {
        putstr(win, 0, "Временные свойства:");
        putstr(win, 0, "");
        for (i = 0; (propname = propertynames[i].prop_name) != 0; ++i) {
            p = propertynames[i].prop_num;
            intrinsic = u.uprops[p].intrinsic;
            if (intrinsic & TIMEOUT) {
                if (specindx > 0 && i >= specindx) {
                    putstr(win, 0, " -- задаётся только через #wizintrinsic --");
                    specindx = 0;
                }
    ...
    if (u.uswldtim) {
        ...
        Sprintf(buf, "Обратный отсчёт заглатывания: %u.", u.uswldtim);
        putstr(win, 0, buf);
    }
    if (u.uinvault) {
        putstr(win, 0, "");
        Sprintf(buf, "Счётчик хранилища: %d.", u.uinvault);
        putstr(win, 0, buf);
    }
    ...
    if (svl.level.flags.stasis_until >= svm.moves) {
        putstr(win, 0, "");
        Sprintf(buf, "Уровень блокирует телепортацию на %ld %s.",
