...
cost(struct obj *otmp)
{
    ...
    default:
        impossible("¬ы не можете написать такой странный свиток!");
    }
    return 1000;
}
...
dowrite(struct obj *pen)
{
    ...
    if (nohands(gy.youmonst.data)) {
        You("¬ам нужны руки, чтобы писать!");
        return ECMD_OK;
    } else if (Glib) {
    ...
    paper = getobj("писать на", write_ok, GETOBJ_NOFLAGS);
    if (!paper)
        return ECMD_CANCEL;
    ...
    typeword = (paper->otyp == SPE_NOVEL) ? "книга"
               : (paper->oclass == SPBOOK_CLASS) ? "сборник заклинаний"
                 : "свиток";
    if (Blind) {
        if (!paper->dknown) {
            You("не знаете, пустой ли тот %s или нет.", typeword);
            return ECMD_OK;
        } else if (paper->oclass == SPBOOK_CLASS) {
            /* can't write a magic book while blind */
            pline("%s не может создать текст брайлем.",
    ...
    /* what to write */
    Sprintf(qbuf, " акой тип %s вы хотите написать?", typeword);
    getlin(qbuf, namebuf);
    ...
    There("нет такой %s!", typeword);
    return ECMD_TIME;
 found:

    if (i == SCR_BLANK_PAPER || i == SPE_BLANK_PAPER) {
        You_cant("написать это!");
        pline("Ёто непристойно!");
        return ECMD_TIME;
    } else if (i == SPE_NOVEL) {
        boolean fanfic = !rn2(3), tearup = !rn2(3);

        if (!fanfic) {
            You("%s написать ¬еликую …ендорскую Ќовеллу, но %s вдохновение.",
                !tearup ? "готовитесь" : "пытаетесь",
                !Hallucination ? "не хватает" : "слишком много");
        } else {
            You("%sпишете по-насто€щему %s фанфик.",
                !tearup ? "начинаете " : "",
                !Hallucination ? "посредственный" : "потр€сающий");
        }
        if (!tearup) {
            You("отказываетесь от этой идеи.");
        } else {
            You("разрываете его.");
            useup(paper);
        }
        ...
        pline("ѕростой искатель приключений не смог бы написать это.");
        return ECMD_TIME;
    } else if (by_descr && paper->oclass == SPBOOK_CLASS
               && !objects[i].oc_name_known) {
        /* can't write unknown spellbooks by description */
        pline("Ќе хватает информации, чтобы продолжить.");
        return ECMD_TIME;
    }
    ...
    if (!u.uconduct.literate++)
        livelog_printf(LL_CONDUCT,
                       "стал грамотным, написав %s", an(typeword));
    ...
    if (pen->spe < basecost / 2) {
        Your("маркер слишком сухой, чтобы написать это!");
    ...
    if (pen->spe < actualcost) {
        pen->spe = 0;
        Your("маркер высыхает!");
        /* scrolls disappear, spellbooks don't */
        if (paper->oclass == SPBOOK_CLASS) {
            pline("—борник заклинаний остаЄтс€ незавершЄнным, и ваша запись угасает.");
            update_inventory(); /* pen charges */
        } else {
            pline("—виток теперь бесполезен и исчезает!");
            useup(paper);
        }
        obfree(new_obj, (struct obj *) 0);
        return ECMD_TIME;
    ...
    if (!objects[new_obj->otyp].oc_name_known
        ...
        && rnl(((Role_if(PM_WIZARD) && paper->oclass != SPBOOK_CLASS)
               ...
               ? 5 : 15)) {
        You("%s написать это.", by_descr ? "не можете" : "не знаете как");
        /* scrolls disappear, spellbooks don't */
        if (paper->oclass == SPBOOK_CLASS) {
            You(
      "пишете своим лучшим почерком:  \"ћой дневник\", но оно быстро угасает.");
            update_inventory(); /* pen charges */
        } else {
            if (by_descr) {
                ...
                Sprintf(namebuf, "%s был здесь!", svp.plname);
            You("пишете \"%s\", и свиток исчезает.", namebuf);
            useup(paper);
        }
        obfree(new_obj, (struct obj *) 0);
        return ECMD_TIME;
    ...
    if (Blind && rnl(3)) {
        ...
        You("не удаЄтс€ правильно написать свиток, и он исчезает.");
        useup(paper);
        obfree(new_obj, (struct obj *) 0);
        return ECMD_TIME;
    }
    ...
    if (new_obj->oclass == SPBOOK_CLASS) {
        ...
        pline("—борник заклинаний странно искривл€етс€ и становитс€ %s.",
    ...
    new_obj = hold_another_object(new_obj, "ќй!  %s выпадает из вашей руки!",
...
new_book_description(int booktype, char *outbuf)
{
    ...
    Sprintf(outbuf, "%s%s", *comp_p ? "в " : "", descr);
    return outbuf;
}
